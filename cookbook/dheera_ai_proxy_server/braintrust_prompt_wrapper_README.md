# Braintrust Prompt Wrapper for Dheera AI

This directory contains a wrapper server that enables Dheera AI to use prompts from [Braintrust](https://www.braintrust.dev/) through the generic prompt management API.

## Architecture

```
┌─────────────┐         ┌──────────────────────┐         ┌─────────────┐
│   Dheera AI   │ ──────> │  Wrapper Server      │ ──────> │  Braintrust │
...
### 1. Generic Prompt Manager (`dheera_ai/integrations/generic_prompt_management/`)

A generic client that can work with any API implementing the `/beta/dheera_ai_prompt_management` endpoint.
...
- Implements the `/beta/dheera_ai_prompt_management` endpoint
- Fetches prompts from Braintrust API
- Transforms Braintrust response format to Dheera AI format
...
```bash
pip install fastapi uvicorn httpx dheera_ai
```
...
### Step 2: Use with Dheera AI

```python
import dheera_ai
from dheera_ai.integrations.generic_prompt_management import GenericPromptManager
...
curl -H "Authorization: Bearer YOUR_BRAINTRUST_TOKEN" \
     "http://localhost:8080/beta/dheera_ai_prompt_management?prompt_id=YOUR_PROMPT_ID"
...
**Transformed to Dheera AI Format:**
```json
{
...
The wrapper automatically maps these Braintrust parameters to Dheera AI:
...
1. Create a wrapper server that implements `/beta/dheera_ai_prompt_management`
2. Transform the provider's response to Dheera AI format
...
This wrapper is part of the Dheera AI project and follows the same license.
