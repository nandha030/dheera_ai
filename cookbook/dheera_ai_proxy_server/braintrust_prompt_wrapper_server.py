"""
Mock server that implements the /beta/dheera_ai_prompt_management endpoint
...
expected by DheeraAI's generic prompt management client.
...
app = FastAPI(
    ...
    description="Wrapper server for Braintrust prompts to work with DheeraAI",
    version="1.0.0",
)
...
def transform_braintrust_message(message: Dict[str, Any]) -> Dict[str, str]:
    """
    Transform a Braintrust message to DheeraAI format.
...
def transform_braintrust_response(
    ...
    """
    Transform Braintrust API response to DheeraAI prompt management format.
    ...
    for braintrust_param, dheera_ai_param in param_mapping.items():
        if braintrust_param in params:
            value = params[braintrust_param]
            if value is not None:
                optional_params[dheera_ai_param] = value
...
@app.get("/beta/dheera_ai_prompt_management")
async def get_prompt(
    ...
    """
    Fetch a prompt from Braintrust and transform it to DheeraAI format.
...
async def root():
    ...
    return {
        "service": "Braintrust Prompt Wrapper for DheeraAI",
        "version": "1.0.0",
        "endpoints": {
            "prompt_management": "/beta/dheera_ai_prompt_management?prompt_id=<id>",
            "health": "/health",
        },
        "documentation": "/docs",
    }
