# Helm Chart for Dheera AI

> [!IMPORTANT]
> This is community maintained, Please make an issue if you run into a bug
> We recommend using [Docker or Kubernetes for production deployments](https://docs.dheera_ai.ai/docs/proxy/prod)
...
### Dheera AI Proxy Deployment Settings

| Name                        | Description                                                                                                                                                                                                                                   | Value                     |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| `replicaCount`              | The number of Dheera AI Proxy pods to be deployed                                                                                                                                                                                               | `1`                       |
| `masterkeySecretName`       | The name of the Kubernetes Secret that contains the Master API Key for Dheera AI. If not specified, use the generated secret name.                                                                                                              | N/A                       |
| `masterkeySecretKey`        | The key within the Kubernetes Secret that contains the Master API Key for Dheera AI. If not specified, use `masterkey` as the key.                                                                                                              | N/A                       |
| `masterkey`                 | The Master API Key for Dheera AI. If not specified, a random key in the `sk-...` format is generated.                                                                                                                                           | N/A                       |
| `environmentSecrets`        | An optional array of Secret object names. The keys and values in these secrets will be presented to the Dheera AI proxy pod as environment variables. See below for an example Secret object.                                                   | `[]`                      |
| `environmentConfigMaps`     | An optional array of ConfigMap object names. The keys and values in these configmaps will be presented to the Dheera AI proxy pod as environment variables. See below for an example Secret object.                                             | `[]`                      |
| `image.repository`          | Dheera AI Proxy image repository                                                                                                                                                                                                                | `docker.dheera_ai.ai/berriai/dheera_ai` |
| `image.pullPolicy`          | Dheera AI Proxy image pull policy                                                                                                                                                                                                               | `IfNotPresent`            |
| `image.tag`                 | Overrides the image tag whose default the latest version of Dheera AI at the time this chart was published.                                                                                                                                     | `""`                      |
| `imagePullSecrets`          | Registry credentials for the Dheera AI and initContainer images.                                                                                                                                                                                | `[]`                      |
| `serviceAccount.create`     | Whether or not to create a Kubernetes Service Account for this deployment. The default is `false` because Dheera AI has no need to access the Kubernetes API.                                                                                   | `false`                   |
...
| `proxy_config.*`            | See [values.yaml](./values.yaml) for default settings. Rendered into the ConfigMap’s `config.yaml` only when `proxyConfigMap.create=true`. See [example_config_yaml](../../../dheera_ai/proxy/example_config_yaml/) for configuration examples. | `N/A`                     |
| `extraContainers[]`         | An array of additional containers to be deployed as sidecars alongside the Dheera AI Proxy.                                                                                                                                                     |
| `pdb.enabled`               | Enable a PodDisruptionBudget for the Dheera AI proxy Deployment                                                                                                                                                                                 | `false`                   |
...
| `db.database`             | If `db.useExisting` is `true`, the name of the existing database to connect to.                                                                                                                                                                                                           | `dheera_ai`                                                                                  |
...
(from the `dheera_ai` pod's perspective) URL published by the `<RELEASE>-dheera_ai`
Kubernetes Service. If the deployment uses the default settings for this
service, the **Proxy Endpoint** should be set to `http://<RELEASE>-dheera_ai:4000`.

The **Proxy Key** is the value specified for `masterkey` or, if a `masterkey`
was not provided to the helm command line, the `masterkey` is a randomly
generated string in the `sk-...` format stored in the `<RELEASE>-dheera_ai-masterkey` Kubernetes Secret.

```bash
kubectl -n dheera_ai get secret <RELEASE>-dheera_ai-masterkey -o jsonpath="{.data.masterkey}"
```
