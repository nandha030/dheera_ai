...
from dheera_ai.types.integrations.datadog_llm_obs import DatadogLLMObsInitParams
...
from dheera_ai.dheera_ai_core_utils.logging_callback_manager import LoggingCallbackManager
import httpx
import dotenv
from dheera_ai.llms.custom_httpx.async_client_cleanup import register_async_client_cleanup

dheera_ai_mode = os.getenv("DHEERA_AI_MODE", "DEV")  # "PRODUCTION", "DEV"
if dheera_ai_mode == "DEV":
    dotenv.load_dotenv()
...
email: Optional[str] = (
    None  # Not used anymore, will be removed in next MAJOR release - https://github.com/BerriAI/dheera_ai/discussions/648
)
token: Optional[str] = (
    None  # Not used anymore, will be removed in next MAJOR release - https://github.com/BerriAI/dheera_ai/discussions/648
)
...
drop_params = bool(os.getenv("DHEERA_AI_DROP_PARAMS", False))
modify_params = bool(os.getenv("DHEERA_AI_MODIFY_PARAMS", False))
retry = True
### AUTH ###
...
use_dheera_ai_proxy: bool = (
    False  # when True, requests will be sent to the specified dheera_ai proxy endpoint
)
...
default_soft_budget: float = (
    DEFAULT_SOFT_BUDGET  # by default all dheera_ai proxy keys have a soft budget of 50.0
)
...
model_fallbacks: Optional[List] = None  # Deprecated for 'dheera_ai.fallbacks'
model_cost_map_url: str = os.getenv(
    "DHEERA_AI_MODEL_COST_MAP_URL",
...
upperbound_key_generate_params: Optional[DheeraAI_UpperboundKeyGenerateParams] = None
...
force_ipv4: bool = (
    False  # when True, dheera_ai will force ipv4 for all LLM requests. Some users have seen httpx ConnectionError when using ipv6.
)
...
def add_known_models():
    for key, value in model_cost.items():
        if value.get("dheera_ai_provider") == "openai" and not is_openai_finetune_model(
            key
        ):
        ...
        elif value.get("dheera_ai_provider") == "datarobot":
        ...
        elif value.get("dheera_ai_provider") == "vertex_ai-language-models":
            vertex_language_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-vision-models":
        ...
        elif value.get("dheera_ai_provider") == "vertex_ai-embedding-models":
            vertex_embedding_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-anthropic_models":
            key = key.replace("vertex_ai/", "")
            vertex_anthropic_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-llama_models":
            key = key.replace("vertex_ai/", "")
            vertex_llama3_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-deepseek_models":
            key = key.replace("vertex_ai/", "")
            vertex_deepseek_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-mistral_models":
            key = key.replace("vertex_ai/", "")
            vertex_mistral_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-ai21_models":
            key = key.replace("vertex_ai/", "")
            vertex_ai_ai21_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-image-models":
            key = key.replace("vertex_ai/", "")
            vertex_ai_image_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-video-models":
            key = key.replace("vertex_ai/", "")
            vertex_ai_video_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-openai_models":
            key = key.replace("vertex_ai/", "")
            vertex_openai_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-minimax_models":
            key = key.replace("vertex_ai/", "")
            vertex_minimax_models.add(key)
        elif value.get("dheera_ai_provider") == "vertex_ai-moonshot_models":
            key = key.replace("vertex_ai/", "")
        ...
        elif value.get("dheera_ai_provider") == "bedrock_converse":
            bedrock_converse_models.add(key)
        elif value.get("dheera_ai_provider") == "deepinfra":
            deepinfra_models.add(key)
        elif value.get("dheera_ai_provider") == "perplexity":
            perplexity_models.add(key)
        elif value.get("dheera_ai_provider") == "watsonx":
            watsonx_models.add(key)
        elif value.get("dheera_ai_provider") == "gemini":
            gemini_models.add(key)
        elif value.get("dheera_ai_provider") == "fireworks_ai":
            # ignore the 'up-to', '-to-' model names -> not real models. just for cost tracking based on model params.
            if "-to-" not in key and "fireworks-ai-default" not in key:
                fireworks_ai_models.add(key)
        elif value.get("dheera_ai_provider") == "fireworks_ai-embedding-models":
            # ignore the 'up-to', '-to-' model names -> not real models. just for cost tracking based on model params.
            if "-to-" not in key:
                fireworks_ai_embedding_models.add(key)
        elif value.get("dheera_ai_provider") == "text-completion-codestral":
            text_completion_codestral_models.add(key)
        elif value.get("dheera_ai_provider") == "xai":
            xai_models.add(key)
        elif value.get("dheera_ai_provider") == "zai":
            zai_models.add(key)
        elif value.get("dheera_ai_provider") == "fal_ai":
            fal_ai_models.add(key)
        elif value.get("dheera_ai_provider") == "deepseek":
            deepseek_models.add(key)
        elif value.get("dheera_ai_provider") == "runwayml":
            runwayml_models.add(key)
        elif value.get("dheera_ai_provider") == "meta_llama":
            llama_models.add(key)
        elif value.get("dheera_ai_provider") == "nscale":
            nscale_models.add(key)
        elif value.get("dheera_ai_provider") == "azure_ai":
            azure_ai_models.add(key)
        elif value.get("dheera_ai_provider") == "voyage":
            voyage_models.add(key)
        elif value.get("dheera_ai_provider") == "infinity":
            infinity_models.add(key)
        elif value.get("dheera_ai_provider") == "databricks":
            databricks_models.add(key)
        elif value.get("dheera_ai_provider") == "cloudflare":
            cloudflare_models.add(key)
        elif value.get("dheera_ai_provider") == "codestral":
            codestral_models.add(key)
        elif value.get("dheera_ai_provider") == "friendliai":
            friendliai_models.add(key)
        elif value.get("dheera_ai_provider") == "palm":
            palm_models.add(key)
        elif value.get("dheera_ai_provider") == "groq":
            groq_models.add(key)
        elif value.get("dheera_ai_provider") == "azure":
            azure_models.add(key)
        elif value.get("dheera_ai_provider") == "azure_anthropic":
            azure_anthropic_models.add(key)
        elif value.get("dheera_ai_provider") == "anyscale":
            anyscale_models.add(key)
        elif value.get("dheera_ai_provider") == "cerebras":
            cerebras_models.add(key)
        elif value.get("dheera_ai_provider") == "galadriel":
            galadriel_models.add(key)
        elif value.get("dheera_ai_provider") == "nvidia_nim":
            nvidia_nim_models.add(key)
        elif value.get("dheera_ai_provider") == "sambanova":
            sambanova_models.add(key)
        elif value.get("dheera_ai_provider") == "sambanova-embedding-models":
            sambanova_embedding_models.add(key)
        elif value.get("dheera_ai_provider") == "novita":
            novita_models.add(key)
        elif value.get("dheera_ai_provider") == "nebius-chat-models":
            nebius_models.add(key)
        elif value.get("dheera_ai_provider") == "nebius-embedding-models":
            nebius_embedding_models.add(key)
        elif value.get("dheera_ai_provider") == "aiml":
            aiml_models.add(key)
        elif value.get("dheera_ai_provider") == "assemblyai":
            assemblyai_models.add(key)
        elif value.get("dheera_ai_provider") == "jina_ai":
            jina_ai_models.add(key)
        elif value.get("dheera_ai_provider") == "snowflake":
            snowflake_models.add(key)
        elif value.get("dheera_ai_provider") == "gradient_ai":
            gradient_ai_models.add(key)
        elif value.get("dheera_ai_provider") == "featherless_ai":
            featherless_ai_models.add(key)
        elif value.get("dheera_ai_provider") == "deepgram":
            deepgram_models.add(key)
        elif value.get("dheera_ai_provider") == "elevenlabs":
            elevenlabs_models.add(key)
        elif value.get("dheera_ai_provider") == "heroku":
            heroku_models.add(key)
        elif value.get("dheera_ai_provider") == "dashscope":
            dashscope_models.add(key)
        elif value.get("dheera_ai_provider") == "moonshot":
            moonshot_models.add(key)
        elif value.get("dheera_ai_provider") == "publicai":
            publicai_models.add(key)
        elif value.get("dheera_ai_provider") == "v0":
            v0_models.add(key)
        elif value.get("dheera_ai_provider") == "morph":
            morph_models.add(key)
        elif value.get("dheera_ai_provider") == "lambda_ai":
            lambda_ai_models.add(key)
        elif value.get("dheera_ai_provider") == "hyperbolic":
            hyperbolic_models.add(key)
        elif value.get("dheera_ai_provider") == "recraft":
            recraft_models.add(key)
        elif value.get("dheera_ai_provider") == "cometapi":
            cometapi_models.add(key)
        elif value.get("dheera_ai_provider") == "oci":
            oci_models.add(key)
        elif value.get("dheera_ai_provider") == "volcengine":
            volcengine_models.add(key)
        elif value.get("dheera_ai_provider") == "wandb":
            wandb_models.add(key)
        elif value.get("dheera_ai_provider") == "ovhcloud":
            ovhcloud_models.add(key)
        elif value.get("dheera_ai_provider") == "ovhcloud-embedding-models":
            ovhcloud_embedding_models.add(key)
        elif value.get("dheera_ai_provider") == "lemonade":
            lemonade_models.add(key)
        elif value.get("dheera_ai_provider") == "docker_model_runner":
            docker_model_runner_models.add(key)
        elif value.get("dheera_ai_provider") == "amazon_nova":
...
from dheera_ai.dheera_ai_core_utils.core_helpers import remove_index_from_tool_calls
...
from dheera_ai.llms.openai.completion.transformation import OpenAITextCompletionConfig
...
from .llms.dheera_ai_proxy.responses.transformation import (
...
from .llms.dheera_ai_proxy.chat.transformation import DheeraAIProxyChatConfig
...
# PublicAI now uses JSON-based configuration (see dheera_ai/llms/openai_like/providers.json)
...
from .llms.custom_httpx.async_client_cleanup import close_dheera_ai_async_clients
...
# Interactions API is available as dheera_ai.interactions module
# Usage: dheera_ai.interactions.create(), dheera_ai.interactions.get(), etc.
...
import dheera_ai.anthropic_interface as anthropic
...
if TYPE_CHECKING:
    from dheera_ai.types.utils import ModelInfo as _ModelInfoType
    ...
    from dheera_ai.caching.llm_caching_handler import LLMClientCache
    from dheera_ai.types.llms.bedrock import COHERE_EMBEDDING_INPUT_TYPES
...
def __getattr__(name: str) -> Any:
    ...
    # Lazy load dheera_ai_logging functions
    if name in DHEERA_AI_LOGGING_NAMES:
        from ._lazy_imports import _lazy_import_dheera_ai_logging
        return _lazy_import_dheera_ai_logging(name)
...
# ALL_DHEERA_AI_RESPONSE_TYPES is lazy-loaded via __getattr__ to avoid loading utils at import time
