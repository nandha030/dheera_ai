...
def _get_dheera_ai_globals() -> dict:
    """Helper to get the globals dictionary of the dheera_ai module."""
    return sys.modules["dheera_ai"].__dict__
...
def _get_default_encoding() -> Any:
    ...
    This avoids importing `dheera_ai.dheera_ai_core_utils.default_encoding` (and thus tiktoken)
    at `dheera_ai` import time. The encoding is cached after the first import.
    ...
    if _default_encoding is None:
        from dheera_ai.dheera_ai_core_utils.default_encoding import encoding
...
def _get_modified_max_tokens() -> Any:
    ...
    This avoids importing `dheera_ai.dheera_ai_core_utils.token_counter` at `dheera_ai` import time.
    ...
    if _get_modified_max_tokens_func is None:
        from dheera_ai.dheera_ai_core_utils.token_counter import (
...
# Litellm logging names that support lazy loading via _lazy_import_dheera_ai_logging
...
UTILS_NAMES = (
    ...
    "get_dheera_ai_params", "acreate", "get_max_tokens", "get_model_info",
...
# Common types from dheera_ai.types.utils that support lazy loading via
# _lazy_import_types_utils
TYPES_UTILS_NAMES = (
    "ImageObject",
    "BudgetConfig",
    "all_dheera_ai_params",
    "_dheera_ai_completion_params",
    "CredentialItem",
...
def _lazy_import_utils(name: str) -> Any:  # noqa: PLR0915
    """Lazy import for utils module - imports only the requested item by name."""
    _globals = _get_dheera_ai_globals()
    ...
    if name == "get_dheera_ai_params":
        from .utils import get_dheera_ai_params as _get_dheera_ai_params
        _globals["get_dheera_ai_params"] = _get_dheera_ai_params
        return _get_dheera_ai_params
...
def _lazy_import_types_utils(name: str) -> Any:
    """Lazy import for common types and constants from dheera_ai.types.utils."""
    _globals = _get_dheera_ai_globals()
    ...
    if name == "all_dheera_ai_params":
        from .types.utils import all_dheera_ai_params as _all_dheera_ai_params

        _globals["all_dheera_ai_params"] = _all_dheera_ai_params
        return _all_dheera_ai_params

    if name == "_dheera_ai_completion_params":
        from .types.utils import all_dheera_ai_params as _all_dheera_ai_params

        _globals["_dheera_ai_completion_params"] = _all_dheera_ai_params
        return _all_dheera_ai_params
...
def _lazy_import_caching(name: str) -> Any:
    ...
    if name == "Cache":
        from dheera_ai.caching.caching import Cache as _Cache

        _globals["Cache"] = _Cache
        return _Cache

    if name == "DualCache":
        from dheera_ai.caching.caching import DualCache as _DualCache

        _globals["DualCache"] = _DualCache
        return _DualCache

    if name == "RedisCache":
        from dheera_ai.caching.caching import RedisCache as _RedisCache

        _globals["RedisCache"] = _RedisCache
        return _RedisCache

    if name == "InMemoryCache":
        from dheera_ai.caching.caching import InMemoryCache as _InMemoryCache
...
def _lazy_import_llm_client_cache(name: str) -> Any:
    ...
    if name == "LLMClientCache":
        from dheera_ai.caching.llm_caching_handler import LLMClientCache as _LLMClientCache
...
def _lazy_import_dheera_ai_logging(name: str) -> Any:
    """Lazy import for dheera_ai_logging module."""
    _globals = _get_dheera_ai_globals()
    if name == "Logging":
        from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import Logging as _Logging
        _globals["Logging"] = _Logging
        return _Logging
    
    if name == "modify_integration":
        from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import modify_integration as _modify_integration
...
def _lazy_import_http_handlers(name: str) -> Any:
    ...
    if name == "module_level_aclient":
        ...
        provider_id = cast(Any, "dheera_ai_module_level_client")
