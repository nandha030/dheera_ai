...
class A2ACostCalculator:
    ...
    def calculate_a2a_cost(
        ...
        Args:
            dheera_ai_logging_obj: The DheeraAI logging object containing call details
        ...
        # Get dheera_ai_params for cost parameters
        dheera_ai_params = model_call_details.get("dheera_ai_params", {}) or {}

        # Check for cost_per_query (fixed cost per query)
        if dheera_ai_params.get("cost_per_query") is not None:
            return float(dheera_ai_params["cost_per_query"])

        # Check for token-based pricing
        input_cost_per_token = dheera_ai_params.get("input_cost_per_token")
        output_cost_per_token = dheera_ai_params.get("output_cost_per_token")
