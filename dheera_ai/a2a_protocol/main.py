"""
DheeraAI A2A SDK functions.

Provides standalone functions with @client decorator for DheeraAI logging integration.
"""
...
from dheera_ai.a2a_protocol.streaming_iterator import A2AStreamingIterator
from dheera_ai.a2a_protocol.utils import A2ARequestUtils
...
def _set_usage_on_logging_obj(
    ...
    """
    Set usage on dheera_ai_logging_obj for standard logging payload.

    Args:
        kwargs: The kwargs dict containing dheera_ai_logging_obj
        prompt_tokens: Number of input tokens
        completion_tokens: Number of output tokens
    """
    dheera_ai_logging_obj = kwargs.get("dheera_ai_logging_obj")
    if dheera_ai_logging_obj is not None:
        ...
        dheera_ai_logging_obj.model_call_details["usage"] = usage
...
def _set_agent_id_on_logging_obj(
    ...
    """
    Set agent_id on dheera_ai_logging_obj for SpendLogs tracking.
    ...
    if dheera_ai_logging_obj is not None:
        ...
        dheera_ai_logging_obj.model_call_details["agent_id"] = agent_id
...
def _get_a2a_model_info(a2a_client: Any, kwargs: Dict[str, Any]) -> str:
    ...
    # Set on dheera_ai_logging_obj if available (for standard logging payload)
    dheera_ai_logging_obj = kwargs.get("dheera_ai_logging_obj")
    if dheera_ai_logging_obj is not None:
        dheera_ai_logging_obj.model = model
        dheera_ai_logging_obj.custom_llm_provider = custom_llm_provider
...
async def asend_message(
    ...
    **kwargs: Any,
) -> DheeraAISendMessageResponse:
    """
    Async: Send a message to an A2A agent.

    Uses the @client decorator for DheeraAI logging and tracking.
    If dheera_ai_params contains custom_llm_provider, routes through the completion bridge.
    ...
    Args:
        ...
        dheera_ai_params: Optional dict with custom_llm_provider, model, etc. for completion bridge
        agent_id: Optional agent ID for tracking in SpendLogs
        **kwargs: Additional arguments passed to the client decorator

    Returns:
        DheeraAISendMessageResponse (wraps a2a SendMessageResponse with _hidden_params)
    ...
    if custom_llm_provider:
        ...
        # Convert to DheeraAISendMessageResponse
        return DheeraAISendMessageResponse.from_dict(response_dict)
    ...
    # Wrap in DheeraAI response type for _hidden_params support
    response = DheeraAISendMessageResponse.from_a2a_response(a2a_response)
...
def send_message(
    ...
    request: "SendMessageRequest",
    **kwargs: Any,
) -> Union[DheeraAISendMessageResponse, Coroutine[Any, Any, DheeraAISendMessageResponse]]:
    """
...
async def asend_message_streaming(
    ...
    agent_card = getattr(a2a_client, "_dheera_ai_agent_card", None) or getattr(a2a_client, "agent_card", None)
    ...
    logging_obj = Logging(
        ...
        dheera_ai_call_id=str(request.id),
    ...
    # Propagate dheera_ai_params for spend logging (includes cost_per_query, etc.)
    _dheera_ai_params = dheera_ai_params.copy() if dheera_ai_params else {}
    # Merge metadata into dheera_ai_params.metadata (required for proxy cost tracking)
    if metadata:
        _dheera_ai_params["metadata"] = metadata
    if proxy_server_request:
        _dheera_ai_params["proxy_server_request"] = proxy_server_request

    logging_obj.dheera_ai_params = _dheera_ai_params
    logging_obj.optional_params = _dheera_ai_params  # used by cost calc
    logging_obj.model_call_details["dheera_ai_params"] = _dheera_ai_params
...
async def create_a2a_client(
    ...
    Example:
        ```python
        from dheera_ai.a2a_protocol import create_a2a_client, asend_message
    ...
    # Use DheeraAI's cached httpx client
    ...
    a2a_client._dheera_ai_agent_card = agent_card  # type: ignore[attr-defined]
