"""
Handler for A2A to DheeraAI completion bridge.

Routes A2A requests through dheera_ai.acompletion based on custom_llm_provider.
...
from dheera_ai.a2a_protocol.dheera_ai_completion_bridge.pydantic_ai_transformation import (
...
class A2ACompletionBridgeHandler:
    """
    Static methods for handling A2A requests via DheeraAI completion.
    """
    ...
    async def handle_non_streaming(
        ...
        """
        Handle non-streaming A2A request via dheera_ai.acompletion.

        Args:
            request_id: A2A JSON-RPC request ID
            params: A2A MessageSendParams containing the message
            dheera_ai_params: Agent's dheera_ai_params (custom_llm_provider, model, etc.)
        ...
        custom_llm_provider = dheera_ai_params.get("custom_llm_provider")
        model = dheera_ai_params.get("model", "agent")
        ...
        # Add dheera_ai_params (contains api_key, client_id, client_secret, tenant_id, etc.)
        dheera_ai_params_to_add = {
            k: v for k, v in dheera_ai_params.items()
            if k not in ("model", "custom_llm_provider")
        }
        completion_params.update(dheera_ai_params_to_add)

        # Call dheera_ai.acompletion
        response = await dheera_ai.acompletion(**completion_params)
    ...
    async def handle_streaming(
        ...
        """
        Handle streaming A2A request via dheera_ai.acompletion with stream=True.
        ...
        # Call dheera_ai.acompletion with streaming
        response = await dheera_ai.acompletion(**completion_params)
