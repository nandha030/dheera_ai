    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params):
        if len(non_default_params.keys()) > 0:
            ...
            for k in keys:
                if (
                    dheera_ai.drop_params is True and k not in supported_params
                ):  # drop the unsupported non-default values
                    non_default_params.pop(k, None)
                elif k not in supported_params:
                    raise dheera_ai.utils.UnsupportedParamsError(
                        status_code=500,
                        message="k={}, not supported by {}. Supported params={}. To drop it from the call, set `dheera_ai.drop_params = True`.".format(
    ...
    if custom_llm_provider == "openai":
        ...
        supported_params = (
            dheera_ai.AzureOpenAIAssistantsAPIConfig().get_supported_openai_create_message_params()
        )
        _check_valid_arg(supported_params=supported_params)
        optional_params = dheera_ai.AzureOpenAIAssistantsAPIConfig().map_openai_params_create_message_params(
