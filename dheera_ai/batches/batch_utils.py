...
from dheera_ai.types.llms.openai import Batch
from dheera_ai.types.utils import CallTypes, ModelResponse, Usage
...
def calculate_vertex_ai_batch_cost_and_usage(
    ...
    for response in vertex_ai_batch_responses:
        if response.get("status") == "JOB_STATE_SUCCEEDED":  # Check if response was successful
            ...
            # Create a real DheeraAI logging object
            logging_obj = Logging(
                ...
                dheera_ai_call_id="batch_" + str(uuid.uuid4()),
                function_id="batch_processing",
                dheera_ai_trace_id=str(uuid.uuid4()),
...
async def _get_batch_output_file_content_as_dictionary(
    ...
    """
    from dheera_ai.files.main import afile_content
...
def _get_batch_job_cost_from_file_content(
    ...
    try:
        ...
        for _item in file_content_dictionary:
            if _batch_response_was_successful(_item):
                _response_body = _get_response_from_batch_job_output_file(_item)
                total_cost += dheera_ai.completion_cost(
