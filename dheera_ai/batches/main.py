...
from dheera_ai.llms.anthropic.batches.handler import AnthropicBatchesHandler
from dheera_ai.llms.azure.batches.handler import AzureBatchesAPI
from dheera_ai.llms.bedrock.batches.handler import BedrockBatchesHandler
...
from dheera_ai.llms.openai.openai import OpenAIBatchesAPI
from dheera_ai.llms.vertex_ai.batches.handler import VertexAIBatchPrediction
...
def _resolve_timeout(
    optional_params: GenericDheeraAIParams,
    kwargs: Dict[str, Any],
    ...
    Args:
        optional_params: GenericDheeraAIParams object containing timeout
...
async def acreate_batch(
    ...
    **kwargs,
    ...
    DheeraAI Equivalent of POST: https://api.openai.com/v1/batches
    """
    try:
    ...
    try:
        ...
        try:
            ...
            verbose_logger.exception(
                f"dheera_ai.batches.main.py::create_batch() - Error inferring custom_llm_provider - {str(e)}"
            )

        _is_async = kwargs.pop("acreate_batch", False) is True
        dheera_ai_params = dict(GenericDheeraAIParams(**kwargs))
        dheera_ai_logging_obj: DheeraAILoggingObj = cast(
            DheeraAILoggingObj, kwargs.get("dheera_ai_logging_obj", None)
        )
        ### TIMEOUT LOGIC ###
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            raise dheera_ai.exceptions.BadRequestError(
                message="DheeraAI doesn't support custom_llm_provider={} for 'create_batch'".format(
                    custom_llm_provider
                ),
                model="n/a",
                ...
                response=httpx.Response(
                    ...
                    request=httpx.Request(method="create_batch", url="https://github.com/BerriAI/dheera_ai"),  # type: ignore
                ),
            )
        return response
    except Exception as e:
...
async def aretrieve_batch(
    ...
    **kwargs,
    ...
    DheeraAI Equivalent of GET https://api.openai.com/v1/batches/{batch_id}
    """
    try:
        loop = asyncio.get_event_loop()
...
def _handle_retrieve_batch_providers_without_provider_config(
    ...
    if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
        ...
        raise dheera_ai.exceptions.BadRequestError(
            message="DheeraAI doesn't support {} for 'create_batch'. Only 'openai' is supported.".format(
                custom_llm_provider
            ),
            model="n/a",
            llm_provider=custom_llm_provider,
            response=httpx.Response(
                status_code=400,
                content="Unsupported provider",
                request=httpx.Request(method="create_thread", url="https://github.com/BerriAI/dheera_ai"),  # type: ignore
            ),
        )
    return response
    ...
    **kwargs,
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            api_base = optional_params.api_base or dheera_ai.api_base or get_secret_str("AZURE_API_BASE")  # type: ignore
            api_version = (
                optional_params.api_version
                or dheera_ai.api_version
                or get_secret_str("AZURE_API_VERSION")
            ...
            raise dheera_ai.exceptions.BadRequestError(
                message="DheeraAI doesn't support {} for 'list_batch'. Supported providers: openai, azure, vertex_ai.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
...
async def acancel_batch(
    ...
    **kwargs,
    ...
    DheeraAI Equivalent of POST https://api.openai.com/v1/batches/{batch_id}/cancel
    """
    try:
        loop = asyncio.get_event_loop()
    ...
    **kwargs,
    ...
    try:
        ...
        try:
            ...
            verbose_logger.exception(
                f"dheera_ai.batches.main.py::cancel_batch() - Error inferring custom_llm_provider - {str(e)}"
            )
        optional_params = GenericDheeraAIParams(**kwargs)
        dheera_ai_params = get_dheera_ai_params(
            custom_llm_provider=custom_llm_provider,
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            raise dheera_ai.exceptions.BadRequestError(
                message="DheeraAI doesn't support {} for 'cancel_batch'. Only 'openai' and 'azure' are supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="cancel_batch", url="https://github.com/BerriAI/dheera_ai"),  # type: ignore
                ),
            )
        return response
    except Exception as e:
...
def _handle_async_invoke_status(
    ...
    from dheera_ai.llms.bedrock.embed.embedding import BedrockEmbedding
    ...
    async def _async_get_status():
        ...
        # Transform response to a DheeraAIBatch object
        from dheera_ai.types.llms.openai import BatchJobStatus
        ...
        # Map AWS status values to DheeraAI expected values
