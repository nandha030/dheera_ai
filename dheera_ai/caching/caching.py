...
from dheera_ai.constants import CACHED_STREAMING_CHUNK_DELAY
from dheera_ai.dheera_ai_core_utils.model_param_helper import ModelParamHelper
from dheera_ai.types.caching import *
from dheera_ai.types.utils import EmbeddingResponse, all_dheera_ai_params
...
#### DheeraAI.Completion / Embedding Cache ####
class Cache:
    def __init__(
        self,
        type: Optional[DheeraAICacheType] = DheeraAICacheType.LOCAL,
        mode: Optional[
            CacheMode
        ...
        **kwargs,
        ...
        Returns:
            None. Cache is set as a dheera_ai param
        """
        if type == DheeraAICacheType.REDIS:
        ...
        elif type == DheeraAICacheType.REDIS_SEMANTIC:
        ...
        elif type == DheeraAICacheType.QDRANT_SEMANTIC:
        ...
        elif type == DheeraAICacheType.LOCAL:
            self.cache = InMemoryCache()
        elif type == DheeraAICacheType.S3:
            self.cache = S3Cache(
        ...
        elif type == DheeraAICacheType.GCS:
            self.cache = GCSCache(
                bucket_name=gcs_bucket_name,
                path_service_account=gcs_path_service_account,
                gcs_path=gcs_path,
            )
        elif type == DheeraAICacheType.AZURE_BLOB:
            self.cache = AzureBlobCache(
                account_url=azure_account_url,
                container=azure_blob_container,
            )
        elif type == DheeraAICacheType.DISK:
            self.cache = DiskCache(disk_cache_dir=disk_cache_dir)
        if "cache" not in dheera_ai.input_callback:
            dheera_ai.input_callback.append("cache")
        if "cache" not in dheera_ai.success_callback:
            dheera_ai.logging_callback_manager.add_dheera_ai_success_callback("cache")
        if "cache" not in dheera_ai._async_success_callback:
            dheera_ai.logging_callback_manager.add_dheera_ai_async_success_callback("cache")
        ...
        if self.type == DheeraAICacheType.LOCAL and default_in_memory_ttl is not None:
            self.ttl = default_in_memory_ttl

        if (
            self.type == DheeraAICacheType.REDIS
            or self.type == DheeraAICacheType.REDIS_SEMANTIC
    ...
    def get_cache_key(self, **kwargs) -> str:
        ...
        Args:
            **kwargs: kwargs to dheera_ai.completion() or embedding()
        ...
        dheera_ai_param_kwargs = all_dheera_ai_params
        for param in kwargs:
            if param in combined_kwargs:
                ...
                param not in dheera_ai_param_kwargs
            ):  # check if user passed in optional param - e.g. top_k
                if (
                    dheera_ai.enable_caching_on_provider_specific_optional_params is True
    ...
    def _get_model_param_value(self, kwargs: dict) -> str:
        ...
        1. If caching groups are set, then return the caching group as the model https://docs.dheera_ai.ai/docs/routing#caching-across-model-groups
        2. Else if a model_group is set, then return the model_group as the model. This is used for all requests sent through the dheera_ai.Router()
        3. Else use the `model` passed in kwargs
        """
        metadata: Dict = kwargs.get("metadata", {}) or {}
        dheera_ai_params: Dict = kwargs.get("dheera_ai_params", {}) or {}
        metadata_in_dheera_ai_params: Dict = dheera_ai_params.get("metadata", {}) or {}
        model_group: Optional[str] = metadata.get(
            "model_group"
        ) or metadata_in_dheera_ai_params.get("model_group")
    ...
    def _get_file_param_value(self, kwargs: dict) -> str:
        ...
        return (
            ...
            or dheera_ai_params.get("file_name")
        )

    def _get_preset_cache_key_from_kwargs(self, **kwargs) -> Optional[str]:
        """
        Get the preset cache key from kwargs["dheera_ai_params"]
        ...
        """
        if kwargs:
            if "dheera_ai_params" in kwargs:
                return kwargs["dheera_ai_params"].get("preset_cache_key", None)
        return None
    ...
    def _set_preset_cache_key_in_kwargs(self, preset_cache_key: str, **kwargs) -> None:
        ...
        Placed in kwargs["dheera_ai_params"]
        """
        if kwargs:
            if "dheera_ai_params" in kwargs:
                kwargs["dheera_ai_params"]["preset_cache_key"] = preset_cache_key
    ...
    def get_cache(self, dynamic_cache_object: Optional[BaseCache] = None, **kwargs):
        ...
        Args:
            *args: args to dheera_ai.completion() or embedding()
            **kwargs: kwargs to dheera_ai.completion() or embedding()
    ...
    def add_cache(self, result, **kwargs):
        ...
        try:
            ...
            verbose_logger.exception(f"DheeraAI Cache: Excepton add_cache: {str(e)}")
...
def enable_cache(
    ...
    **kwargs,
    ...
    Raises:
        None
    """
    print_verbose("DheeraAI: Enabling Cache")
    if "cache" not in dheera_ai.input_callback:
        dheera_ai.input_callback.append("cache")
    if "cache" not in dheera_ai.success_callback:
        dheera_ai.logging_callback_manager.add_dheera_ai_success_callback("cache")
    ...
    print_verbose(f"DheeraAI: Cache enabled, dheera_ai.cache={dheera_ai.cache}")
    print_verbose(f"DheeraAI Cache: {vars(dheera_ai.cache)}")
...
def update_cache(
    ...
    **kwargs,
    ...
    """
    print_verbose("DheeraAI: Updating Cache")
    dheera_ai.cache = Cache(
        type=type,
        host=host,
        port=port,
    ...
    print_verbose(f"DheeraAI: Cache Updated, dheera_ai.cache={dheera_ai.cache}")
    print_verbose(f"DheeraAI Cache: {vars(dheera_ai.cache)}")


def disable_cache():
    """
    Disable the cache used by DheeraAI.

    This function disables the cache used by the DheeraAI module. It removes the cache-related callbacks from the input_callback, success_callback, and _async_success_callback lists. It also sets the dheera_ai.cache attribute to None.
    ...
    print_verbose("DheeraAI: Disabling Cache")
    with suppress(ValueError):
        dheera_ai.input_callback.remove("cache")
        dheera_ai.success_callback.remove("cache")
        dheera_ai._async_success_callback.remove("cache")

    dheera_ai.cache = None
    print_verbose(f"DheeraAI: Cache disabled, dheera_ai.cache={dheera_ai.cache}")
