...
from dheera_ai.caching.caching import S3Cache
...
from dheera_ai.types.caching import CachedEmbedding
...
class LLMCachingHandler:
    def __init__(
        ...
        from dheera_ai.caching import DualCache, RedisCache
        ...
        if dheera_ai.cache is not None and isinstance(dheera_ai.cache.cache, RedisCache):
            self.dual_cache: Optional[DualCache] = DualCache(
                redis_cache=dheera_ai.cache.cache,
    ...
    async def _async_get_cache(
        ...
        if (
            (kwargs.get("caching", None) is None and dheera_ai.cache is not None)
            ...
            if dheera_ai.cache is not None and self._is_call_type_supported_by_cache(
                ...
                if cached_result is not None and not isinstance(cached_result, list):
                    ...
                    self._update_dheera_ai_logging_obj_environment(
                    ...
                    cache_key = dheera_ai.cache.get_cache_key(**kwargs)
                    if (
    ...
    def _sync_get_cache(
        ...
        if dheera_ai.cache is not None and self._is_call_type_supported_by_cache(
            ...
            cached_result = dheera_ai.cache.get_cache(**new_kwargs)
            ...
            for val in non_null_list:
                ...
                if cr is not None:
                    ...
                    if isinstance(kwargs_input_as_list[idx], str):
                        from dheera_ai.utils import token_counter
    ...
    def _async_log_cache_hit_on_callbacks(
        ...
        Args:
            logging_obj (DheeraAILoggingObj): The logging object.
    ...
    async def _retrieve_from_cache(
        ...
        if call_type == CallTypes.aembedding.value:
            ...
            for idx, i in enumerate(new_kwargs["input"]):
                preset_cache_key = dheera_ai.cache.get_cache_key(
                    **{**new_kwargs, "input": i}
                )
                tasks.append(
                    dheera_ai.cache.async_get_cache(
            ...
            if dheera_ai.cache._supports_async() is True:
                ## check if dual cache is supported ##
                cached_result = await dheera_ai.cache.async_get_cache(
                    dynamic_cache_object=self.dual_cache, **new_kwargs
                )
            else:  # fallback for caches that don't support async
                cached_result = dheera_ai.cache.get_cache(
                    dynamic_cache_object=self.dual_cache, **new_kwargs
                )
    ...
    def _convert_cached_result_to_model_response(
        ...
        Returns:
            Optional[Any]:
        """
        from dheera_ai.utils import convert_to_model_response_object
    ...
    async def async_set_cache(
        ...
        if self._should_store_result_in_cache(
            ...
            if (
                isinstance(result, dheera_ai.ModelResponse)
                or isinstance(result, dheera_ai.EmbeddingResponse)
                ...
                if (
                    ...
                    asyncio.create_task(
                        dheera_ai.cache.async_add_cache_pipeline(
                            result, dynamic_cache_object=self.dual_cache, **new_kwargs
                        )
                    )
                else:
                    asyncio.create_task(
                        dheera_ai.cache.async_add_cache(
                ...
                asyncio.create_task(dheera_ai.cache.async_add_cache(result, **new_kwargs))
    ...
    def sync_set_cache(
        ...
        if self._should_store_result_in_cache(
            ...
            dheera_ai.cache.add_cache(result, **new_kwargs)
    ...
    def _should_store_result_in_cache(
        ...
        """
        return (
            (dheera_ai.cache is not None)
            and dheera_ai.cache.supported_call_types is not None
            and (str(original_function.__name__) in dheera_ai.cache.supported_call_types)
    ...
    def _is_call_type_supported_by_cache(
        ...
        Defined on `dheera_ai.types.utils.CallTypes`
    ...
    async def _add_streaming_response_to_cache(self, processed_chunk: ModelResponse):
        ...
        - If 'streaming_chunk' has a 'finish_reason' then assemble a dheera_ai.ModelResponse object
    ...
    def _update_dheera_ai_logging_obj_environment(
        self,
        logging_obj: DheeraAILoggingObj,
        model: str,
        kwargs: Dict[str, Any],
        ...
        """
        Helper function to update the DheeraAILoggingObj environment variables.

        Args:
            logging_obj (DheeraAILoggingObj): The logging object to update.
        ...
        if dheera_ai.cache is not None:
            dheera_ai_params["preset_cache_key"] = (
                dheera_ai.cache._get_preset_cache_key_from_kwargs(**kwargs)
            )
        else:
            dheera_ai_params["preset_cache_key"] = None
