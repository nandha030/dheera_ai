...
from dheera_ai.dheera_ai_core_utils.llm_cost_calc.utils import (
    CostCalculatorUtils,
    _generic_cost_per_character,
    generic_cost_per_token,
    select_cost_metric_for_model,
)
from dheera_ai.llms.anthropic.cost_calculation import (
    cost_per_token as anthropic_cost_per_token,
)
from dheera_ai.llms.azure.cost_calculation import (
    cost_per_token as azure_openai_cost_per_token,
)
from dheera_ai.llms.base_llm.search.transformation import SearchResponse
from dheera_ai.llms.bedrock.cost_calculation import (
    cost_per_token as bedrock_cost_per_token,
)
from dheera_ai.llms.databricks.cost_calculator import (
    cost_per_token as databricks_cost_per_token,
)
from dheera_ai.llms.deepseek.cost_calculator import (
    cost_per_token as deepseek_cost_per_token,
)
from dheera_ai.llms.fireworks_ai.cost_calculator import (
    cost_per_token as fireworks_ai_cost_per_token,
)
from dheera_ai.llms.gemini.cost_calculator import cost_per_token as gemini_cost_per_token
from dheera_ai.llms.lemonade.cost_calculator import (
    cost_per_token as lemonade_cost_per_token,
)
from dheera_ai.llms.openai.cost_calculation import (
    cost_per_second as openai_cost_per_second,
)
from dheera_ai.llms.openai.cost_calculation import cost_per_token as openai_cost_per_token
from dheera_ai.llms.perplexity.cost_calculator import (
    cost_per_token as perplexity_cost_per_token,
)
from dheera_ai.llms.together_ai.cost_calculator import get_model_params_and_category
from dheera_ai.llms.vertex_ai.cost_calculator import (
    cost_per_character as google_cost_per_character,
)
from dheera_ai.llms.vertex_ai.cost_calculator import (
    cost_per_token as google_cost_per_token,
)
from dheera_ai.llms.vertex_ai.cost_calculator import cost_router as google_cost_router
from dheera_ai.llms.xai.cost_calculator import cost_per_token as xai_cost_per_token
...
from dheera_ai.types.rerank import RerankBilledUnits, RerankResponse
...
def cost_per_token(  # noqa: PLR0915
    ...
    model_cost_ref = dheera_ai.model_cost
    ...
    if custom_llm_provider is not None:
        ...
        _, custom_llm_provider, _, _ = dheera_ai.get_llm_provider(model=model)
    ...
    if len(model_parts) > 1:
        ...
        model_without_prefix = model
    """
    Code block that formats model to lookup in dheera_ai.model_cost
    ...
    if call_type == "speech" or call_type == "aspeech":
        speech_model_info = dheera_ai.get_model_info(
        ...
        from dheera_ai.search import search_provider_cost_per_query
        ...
        from dheera_ai.llms.dashscope.cost_calculator import (
...
def get_replicate_completion_pricing(completion_response: dict, total_time=0.0):
    ...
    # for all dheera_ai currently supported LLMs, almost all requests go to a100_80gb
...
def _get_provider_for_cost_calc(
    ...
    try:
        ...
        verbose_logger.debug(
            f"dheera_ai.cost_calculator.py::_get_provider_for_cost_calc() - Error inferring custom_llm_provider - {str(e)}"
        )
        return None
...
def _select_model_name_for_cost_calc(
    ...
    if custom_pricing is True:
        if router_model_id is not None and router_model_id in dheera_ai.model_cost:
...
def _is_known_usage_objects(usage_obj):
    ...
    return (
        isinstance(usage_obj, dheera_ai.Usage)
...
def _apply_cost_discount(
    ...
    if custom_llm_provider and custom_llm_provider in dheera_ai.cost_discount_config:
        discount_percent = dheera_ai.cost_discount_config[custom_llm_provider]
...
def _store_cost_breakdown_in_logging_obj(
    dheera_ai_logging_obj: Optional[LitellmLoggingObject],
    ...
    Args:
        dheera_ai_logging_obj: The logging object to store breakdown in
    ...
    try:
        ...
        dheera_ai_logging_obj.set_cost_breakdown(
...
def completion_cost(  # noqa: PLR0915
    ...
    router_model_id: Optional[str] = None,
    dheera_ai_logging_obj: Optional[LitellmLoggingObject] = None,
    ### SERVICE TIER ###
    service_tier: Optional[str] = None,  # for OpenAI service tier pricing
) -> float:
    """
    Calculate the cost of a given completion call fot GPT-3.5-turbo, llama2, any dheera_ai supported llm.

    Parameters:
        completion_response (dheera_ai.ModelResponses): [Required] The response received from a DheeraAI completion request.
    ...
    Exceptions:
        Raises exception if model not in the dheera_ai model cost map. Register model, via custom pricing or PR - https://github.com/BerriAI/dheera_ai/blob/main/model_prices_and_context_window.json
        ...
        for idx, model in enumerate(potential_model_names):
            try:
                ...
                if completion_response is not None and (
                    ...
                    if isinstance(usage_obj, BaseModel) and not _is_known_usage_objects(
                        ...
                        setattr(
                            completion_response,
                            "usage",
                            dheera_ai.Usage(**usage_obj.model_dump()),
                        )
                    if usage_obj is None:
                        _usage = {}
                    elif isinstance(usage_obj, BaseModel):
                ...
                if call_type in (
                    ...
                    from dheera_ai.a2a_protocol.cost_calculator import A2ACostCalculator
                ...
                if custom_llm_provider is None:
                    try:
                        ...
                        verbose_logger.debug(
                            "dheera_ai.cost_calculator.py::completion_cost() - Error inferring custom_llm_provider - {}".format(
                                str(e)
                            )
                        )
                if CostCalculatorUtils._call_type_has_image_response(
                    ...
                    prompt_characters = dheera_ai.utils._count_characters(text=prompt)
                elif (
                    ...
                    completion_response, DheeraAIRealtimeStreamLoggingObject
                ):
                    if (
                    ...
                    from dheera_ai.proxy._experimental.mcp_server.cost_calculator import (
                        MCPCostCalculator,
                    )
                ...
                # see https://replicate.com/pricing
                    ...
                    model in dheera_ai.replicate_models or "replicate" in model
                ) and model not in dheera_ai.model_cost:
                ...
                if (
                    ...
                    if len(messages) > 0:
                        prompt_string = dheera_ai.utils.get_formatted_prompt(
                            data={"messages": messages}, call_type="completion"
                        )

                        prompt_characters = dheera_ai.utils._count_characters(
                            text=prompt_string
                        )
                    if completion_response is not None and isinstance(
                        completion_response, ModelResponse
                    ):
                        completion_string = dheera_ai.utils.get_response_string(
                            response_obj=completion_response
                        )
                        completion_characters = dheera_ai.utils._count_characters(
                ...
                verbose_logger.debug(
                    "dheera_ai.cost_calculator.py::completion_cost() - Error calculating cost for model={} - {}".format(
                        model, str(e)
                    )
                )
...
def get_response_cost_from_hidden_params(
    ...
    if (
        additional_headers
        and "llm_provider-x-dheera_ai-response-cost" in additional_headers
    ):
        response_cost = additional_headers["llm_provider-x-dheera_ai-response-cost"]
        if response_cost is None:
            return None
        return float(additional_headers["llm_provider-x-dheera_ai-response-cost"])
    return None
...
def ocr_cost(
    ...
    try:
        model_info: Optional[ModelInfo] = dheera_ai.get_model_info(
            model=model, custom_llm_provider=custom_llm_provider
        )
    except Exception:
        model_info = None
...
def vector_store_search_cost(
    ...
    if model is not None and "/" in model:
        api_type, custom_llm_provider, _, _ = dheera_ai.get_llm_provider(
            model=model,
        )
...
def default_image_cost_calculator(
    ...
    for _model in models_to_check:
        if _model is not None and _model in dheera_ai.model_cost:
            cost_info = dheera_ai.model_cost[_model]
            break
...
def default_video_cost_calculator(
    ...
    if cost_info is None and custom_llm_provider:
        ...
        if prefixed_model in dheera_ai.model_cost:
            cost_info = dheera_ai.model_cost[prefixed_model]
    if cost_info is None:
        raise Exception(
            f"Model not found in cost map. Tried checking {models_to_check}"
        )
...
class RealtimeAPITokenUsageProcessor(BaseTokenUsageProcessor):
    ...
    def create_logging_realtime_object(
        ...
        return DheeraAIRealtimeStreamLoggingObject(
            usage=usage,
            results=results,
        )
...
def handle_realtime_stream_cost_calculation(
    ...
    potential_model_names.append(dheera_ai_model_name)
