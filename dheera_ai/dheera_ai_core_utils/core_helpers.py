...
def add_missing_spend_metadata_to_dheera_ai_metadata(
    dheera_ai_metadata: dict, metadata: dict
) -> dict:
    """
    Helper to get dheera_ai metadata for spend tracking

    PATCH for issue where both `dheera_ai_metadata` and `metadata` are present in the kwargs
...
def get_metadata_variable_name_from_kwargs(
    ...
    """
    return "dheera_ai_metadata" if "dheera_ai_metadata" in kwargs else "metadata"
    
def get_dheera_ai_metadata_from_kwargs(kwargs: dict):
    """
    Helper to get dheera_ai metadata from all dheera_ai request kwargs

    Return `dheera_ai_metadata` if it exists, otherwise return `metadata`
    """
    ...
    if dheera_ai_params:
        ...
        if dheera_ai_metadata and metadata:
            dheera_ai_metadata = add_missing_spend_metadata_to_dheera_ai_metadata(
                dheera_ai_metadata, metadata
            )
...
def _get_parent_otel_span_from_kwargs(
    ...
    try:
        ...
        if "dheera_ai_parent_otel_span" in _metadata:
            return _metadata["dheera_ai_parent_otel_span"]
        elif (
            dheera_ai_params is not None
            and dheera_ai_params.get("metadata") is not None
            and "dheera_ai_parent_otel_span" in dheera_ai_params.get("metadata", {})
        ):
            return dheera_ai_params["metadata"]["dheera_ai_parent_otel_span"]
        elif "dheera_ai_parent_otel_span" in kwargs:
            return kwargs["dheera_ai_parent_otel_span"]
        return None
...
def safe_deep_copy(data):
    ...
    The DheeraAI request may contain objects that cannot be pickled/deep-copied
    ...
    if dheera_ai.safe_memory_mode is True:
        return data

    dheera_ai_parent_otel_span: Optional[Any] = None
    # Step 1: Remove the dheera_ai_parent_otel_span
    dheera_ai_parent_otel_span = None
    if isinstance(data, dict):
        # remove dheera_ai_parent_otel_span since this is not picklable
        if "metadata" in data and "dheera_ai_parent_otel_span" in data["metadata"]:
            dheera_ai_parent_otel_span = data["metadata"].pop("dheera_ai_parent_otel_span")
            data["metadata"]["dheera_ai_parent_otel_span"] = "placeholder"
        if (
            "dheera_ai_metadata" in data
            and "dheera_ai_parent_otel_span" in data["dheera_ai_metadata"]
        ):
            dheera_ai_parent_otel_span = data["dheera_ai_metadata"].pop(
                "dheera_ai_parent_otel_span"
            )
            data["dheera_ai_metadata"]["dheera_ai_parent_otel_span"] = "placeholder"
    ...
    # Step 3: re-add the dheera_ai_parent_otel_span after doing a deep copy
    if isinstance(data, dict) and dheera_ai_parent_otel_span is not None:
        if "metadata" in data and "dheera_ai_parent_otel_span" in data["metadata"]:
            data["metadata"]["dheera_ai_parent_otel_span"] = dheera_ai_parent_otel_span
        if (
            "dheera_ai_metadata" in data
            and "dheera_ai_parent_otel_span" in data["dheera_ai_metadata"]
        ):
            data["dheera_ai_metadata"][
                "dheera_ai_parent_otel_span"
            ] = dheera_ai_parent_otel_span
    return new_data
...
def filter_exceptions_from_params(data: Any, max_depth: int = 20) -> Any:
    ...
    if obj_type_name in ["Logging", "DheeraAILoggingObj"]:
        return None
...
def filter_internal_params(data: dict, additional_internal_params: Optional[set] = None) -> dict:
    """
    Filter out DheeraAI internal parameters that shouldn't be sent to provider APIs.
    
    This removes internal/MCP-related parameters that are used by DheeraAI internally
