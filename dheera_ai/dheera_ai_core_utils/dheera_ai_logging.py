...
from dheera_ai._logging import _is_debugging_on, verbose_logger
from dheera_ai._uuid import uuid
from dheera_ai.batches.batch_utils import _handle_completed_batch
from dheera_ai.caching.caching import DualCache, InMemoryCache
from dheera_ai.caching.caching_handler import LLMCachingHandler
...
from dheera_ai.integrations.arize.arize import ArizeLogger
...
from dheera_ai.integrations.deepeval.deepeval import DeepEvalLogger
from dheera_ai.integrations.mlflow import MlflowLogger
from dheera_ai.integrations.sqs import SQSLogger
from dheera_ai.dheera_ai_core_utils.get_dheera_ai_params import get_dheera_ai_params
from dheera_ai.dheera_ai_core_utils.llm_cost_calc.tool_call_cost_tracking import (
...
from dheera_ai.llms.base_llm.ocr.transformation import OCRResponse
from dheera_ai.llms.base_llm.search.transformation import SearchResponse
from dheera_ai.responses.utils import ResponseAPILoggingUtils
from dheera_ai.types.agents import DheeraAISendMessageResponse
from dheera_ai.types.containers.main import ContainerObject
...
from dheera_ai.types.utils import (
    ...
    ImageResponse,
    DheeraAIBatch,
    DheeraAILoggingBaseClass,
    DheeraAIRealtimeStreamLoggingObject,
    ModelResponse,
    ModelResponseStream,
    RawRequestTypedDict,
...
from dheera_ai.utils import _get_base_model_from_metadata, executor, print_verbose
...
try:
    from dheera_ai_enterprise.enterprise_callbacks.callback_controls import (
    ...
    from dheera_ai_enterprise.dheera_ai_core_utils.dheera_ai_logging import (
    ...
    verbose_logger.debug(
        f"[Non-Blocking] Unable to import GenericAPILogger - DheeraAI Enterprise Feature - {str(e)}"
    )
...
class ServiceTraceIDCache:
    ...
    def get_cache(self, dheera_ai_call_id: str, service_name: str) -> Optional[str]:
        key_name = "{}:{}".format(service_name, dheera_ai_call_id)
        response = self.cache.get_cache(key=key_name)
        return response

    def set_cache(self, dheera_ai_call_id: str, service_name: str, trace_id: str) -> None:
        key_name = "{}:{}".format(service_name, dheera_ai_call_id)
...
class Logging(DheeraAILoggingBaseClass):
    ...
    stream_options = None
    dheera_ai_request_debug: bool = False
    ...
    def __init__(
        ...
        self.dheera_ai_call_id = dheera_ai_call_id
        self.dheera_ai_trace_id: str = dheera_ai_trace_id or str(uuid.uuid4())
        ...
        # INITIAL DHEERA_AI_PARAMS
        dheera_ai_params = {}
        if kwargs is not None:
            dheera_ai_params = get_dheera_ai_params(**kwargs)
            dheera_ai_params = scrub_sensitive_keys_in_metadata(dheera_ai_params)

        self.dheera_ai_params = dheera_ai_params
        ...
        self.model_call_details: Dict[str, Any] = {
            "dheera_ai_trace_id": dheera_ai_trace_id,
    ...
    def process_dynamic_callbacks(self):
        ...
        If a callback is in dheera_ai._known_custom_logger_compatible_callbacks, it needs to be intialized and added to the respective dynamic_* callback list.
        """
    ...
    def _process_dynamic_callback_list(
        ...
        - If a callback is in dheera_ai._known_custom_logger_compatible_callbacks,
        ...
        for callback in callback_list:
            if (
                ...
                and callback in dheera_ai._known_custom_logger_compatible_callbacks
            ):
    ...
    def update_environment_variables(
        ...
        **additional_params,
        ...
        if model is not None:
            self.model = model
        self.user = user
        self.dheera_ai_params = {
            **self.dheera_ai_params,
            **scrub_sensitive_keys_in_metadata(dheera_ai_params),
        }
        self.dheera_ai_request_debug = dheera_ai_params.get("dheera_ai_request_debug", False)
        self.logger_fn = dheera_ai_params.get("logger_fn", None)
        ...
        self.model_call_details.update(
            {
                ...
                "dheera_ai_params": self.dheera_ai_params,
                "start_time": self.start_time,
                "stream": self.stream,
                "user": user,
                "call_type": str(self.call_type),
                "dheera_ai_call_id": self.dheera_ai_call_id,
        ...
        custom_pricing_keys = CustomPricingDheeraAIParams.model_fields.keys()
        for key in custom_pricing_keys:
            if dheera_ai_params.get(key) is not None:
    ...
    def _should_run_prompt_management_hooks_without_prompt_id(
        ...
        if dheera_ai.vector_store_registry is not None:
            if dheera_ai.vector_store_registry.get_vector_store_to_run(
    ...
    def get_custom_logger_for_prompt_management(
        ...
        for callback_name in dheera_ai._known_custom_logger_compatible_callbacks:
        ...
        if dheera_ai.vector_store_registry is not None:
            ...
            if (
                ...
                and vector_store_custom_logger not in dheera_ai.callbacks
            ):
    ...
    def _pre_call(self, input, api_key, model=None, additional_args={}):
        ...
        self.model_call_details["dheera_ai_params"]["api_base"] = (
            self._get_masked_api_base(additional_args.get("api_base", ""))
        )

    def pre_call(self, input, api_key, model=None, additional_args={}):  # noqa: PLR0915
        # Log the exact input to the LLM API
        dheera_ai.error_logs["PRE_CALL"] = locals()
        try:
            self._pre_call(
                input=input,
                api_key=api_key,
            ...
            if (
                ...
                _dheera_ai_params = self.model_call_details.get("dheera_ai_params", {})
                _metadata = _dheera_ai_params.get("metadata", {}) or {}
                try:
            ...
            if getattr(self, "logger_fn", None) and callable(self.logger_fn):
                try:
                    ...
                    verbose_logger.exception(
                        "DheeraAI.LoggingError: [Non-Blocking] Exception occurred while logging {}".format(
                            str(e)
                        )
                    )
            ...
            callbacks = dheera_ai.input_callback + (self.dynamic_input_callbacks or [])
                ...
                try:
                    if callback == "supabase" and supabaseClient is not None:
                        ...
                        supabaseClient.input_log_event(
                            ...
                            dheera_ai_call_id=self.dheera_ai_params["dheera_ai_call_id"],
                    ...
                    verbose_logger.exception(
                        "dheera_ai.Logging.pre_call(): Exception occured - {}".format(
                            str(e)
                        )
                    )
                    verbose_logger.debug(
                        f"DheeraAI.Logging: is sentry capture exception initialized {capture_exception}"
                    )
    ...
    def _print_llm_call_debugging_log(
        ...
        Prints the RAW curl command sent from DheeraAI
        """
        if _is_debugging_on() or self.dheera_ai_request_debug:
            if json_logs:
                masked_headers = self._get_masked_headers(headers)
                if self.dheera_ai_request_debug:
    ...
    def _get_masked_headers(
        ...
        Masks the headers of the request sent from DheeraAI
        """
    ...
    def post_call(
        ...
        dheera_ai.error_logs["POST_CALL"] = locals()
        ...
        try:
            ...
            for callback in callbacks:
                try:
                    ...
                    verbose_logger.exception(
                        "DheeraAI.LoggingError: [Non-Blocking] Exception occurred while post-call logging with integrations {}".format(
                            str(e)
                        )
                    )
                    verbose_logger.debug(
    ...
    async def async_post_mcp_tool_call_hook(
        ...
        callbacks = self.get_combined_callback_list(
            ...
            global_callbacks=dheera_ai.success_callback,
        )
    ...
    def _response_cost_calculator(
        ...
        dheera_ai_model_name: Optional[str] = None,
        ...
        custom_pricing = use_custom_pricing_for_model(
            dheera_ai_params=(
                self.dheera_ai_params if hasattr(self, "dheera_ai_params") else None
            )
        )
        ...
        try:
            response_cost_calculator_kwargs = {
                ...
                "model": dheera_ai_model_name or self.model,
                "cache_hit": cache_hit,
    ...
    def should_run_callback(
        self, callback: dheera_ai.CALLBACK_TYPES, dheera_ai_params: dict, event_hook: str
    ) -> bool:
        if dheera_ai.global_disable_no_log_param:
            return True

        if dheera_ai_params.get("no-log", False) is True:
        ...
        if (
            ...
            verbose_logger.debug(
                f"Callback {callback} disabled via x-dheera_ai-disable-callbacks header for {event_hook} event"
            )
            return False
    ...
    def _process_hidden_params_and_response_cost(
        ...
        if hidden_params:
            if self.model_call_details.get("dheera_ai_params") is not None:
                self.model_call_details["dheera_ai_params"].setdefault("metadata", {})
                if self.model_call_details["dheera_ai_params"]["metadata"] is None:
                    self.model_call_details["dheera_ai_params"]["metadata"] = {}
                self.model_call_details["dheera_ai_params"]["metadata"]["hidden_params"] = getattr(logging_result, "_hidden_params", {})  # type: ignore
    ...
    def _transform_usage_objects(self, result):
        if isinstance(result, ResponsesAPIResponse):
            ...
            from dheera_ai.dheera_ai_core_utils.llm_cost_calc.usage_object_transformation import (
    ...
    def _success_handler_helper_fn(
        ...
        try:
            ...
            if (
                ...
                dheera_ai._current_cost += dheera_ai.completion_cost(
                    model=self.model,
                    prompt="",
            ...
            raise Exception(f"[Non-Blocking] DheeraAI.Success_Call Error: {str(e)}")
    ...
    def _is_recognized_call_type_for_logging(
        ...
        if (
            ...
            or isinstance(logging_result, DheeraAIBatch)
            or isinstance(logging_result, ResponsesAPIResponse)
            or isinstance(logging_result, OpenAIFileObject)
            or isinstance(logging_result, DheeraAIRealtimeStreamLoggingObject)
            ...
            or isinstance(logging_result, DheeraAISendMessageResponse)  # A2A
    ...
    def success_handler(  # noqa: PLR0915
        ...
        verbose_logger.debug(
            f"Logging Details DheeraAI-Success Call: Cache_hit={cache_hit}"
        )
        ...
        try:
            ...
            if complete_streaming_response is not None:
                verbose_logger.debug(
                    "Logging Details DheeraAI-Success Call streaming complete"
                )
            ...
            for callback in callbacks:
                try:
                    dheera_ai_params = self.model_call_details.get("dheera_ai_params", {})
                    ...
                    if callback == "supabase" and supabaseClient is not None:
                        ...
                        supabaseClient.log_event(
                            ...
                            dheera_ai_call_id=(
                                current_call_id
                                if (
                                    current_call_id := dheera_ai_params.get(
                                        "dheera_ai_call_id"
                                    )
                                )
                                is not None
                    ...
                    if (
                        callback == "openmeter"
                        and self.model_call_details.get("dheera_ai_params", {}).get(
                            "acompletion", False
                        )
                        is not True
                        and self.model_call_details.get("dheera_ai_params", {}).get(
                    ...
                    print_verbose(
                        f"DheeraAI.LoggingError: [Non-Blocking] Exception occurred while success logging with integrations {traceback.format_exc()}"
                    )
                    print_verbose(
                        f"DheeraAI.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(
                "DheeraAI.LoggingError: [Non-Blocking] Exception occurred while success logging {}".format(
                    str(e)
                ),
            )
    ...
    async def async_success_handler(  # noqa: PLR0915
        ...
        """
        print_verbose(
            "Logging Details DheeraAI-Async Success Call, cache_hit={}".format(cache_hit)
        )
        if not self.should_run_logging(
        ...
        if self.call_type == CallTypes.aretrieve_batch.value and isinstance(
            result, DheeraAIBatch
        ):
            dheera_ai_params = self.dheera_ai_params or {}
            dheera_ai_metadata = dheera_ai_params.get("dheera_ai_metadata", {})
            if (
                dheera_ai_metadata.get("batch_ignore_default_logging", False) is True
            ):  # polling job will query these frequently, don't spam db logs
                return

            from dheera_ai.proxy.openai_files_endpoints.common_utils import (
        ...
        if complete_streaming_response is not None:
            ...
            except dheera_ai.NotFoundError:
                verbose_logger.warning(
        ...
        callbacks = self.get_combined_callback_list(
            ...
            global_callbacks=dheera_ai._async_success_callback,
        )
        ...
        for callback in callbacks:
            if isinstance(callback, CustomGuardrail):
                ...
                result = redact_message_input_output_from_custom_logger(
                    result=result, dheera_ai_logging_obj=self, custom_logger=callback
                )
                self.model_call_details, result = await callback.async_logging_hook(
                    kwargs=self.model_call_details,
                    result=result,
        ...
        for callback in callbacks:
            ...
            try:
                ...
                verbose_logger.error(
                    f"DheeraAI.LoggingError: [Non-Blocking] Exception occurred while success logging {traceback.format_exc()}"
                )
                self._handle_callback_failure(callback=callback)
                pass
    ...
    def _handle_callback_failure(self, callback: Any):
        ...
        try:
            ...
            all_callbacks = dheera_ai.logging_callback_manager._get_all_callbacks()
    ...
    def _failure_handler_helper_fn(
        ...
        if hasattr(exception, "headers") and isinstance(exception.headers, dict):
            self.model_call_details.setdefault("dheera_ai_params", {})
            metadata = (
                self.model_call_details["dheera_ai_params"].get("metadata", {}) or {}
            )
    ...
    async def special_failure_handlers(self, exception: Exception):
        ...
        """
        from dheera_ai.types.router import RouterErrors

        dheera_ai_params: dict = self.model_call_details.get("dheera_ai_params") or {}
        metadata = dheera_ai_params.get("metadata") or {}
        ...
        for callback in dheera_ai._async_failure_callback:
            if isinstance(callback, CustomLogger):  # custom logger class
    ...
    def failure_handler(  # noqa: PLR0915
        ...
        verbose_logger.debug(
            f"Logging Details DheeraAI-Failure Call: {dheera_ai.failure_callback}"
        )
        if not self.should_run_logging(
        ...
        try:
            ...
            callbacks = self.get_combined_callback_list(
                ...
                global_callbacks=dheera_ai.failure_callback,
            )
                ...
                try:
                    ...
                    if callback == "sentry":
                        ...
                        supabaseClient.log_event(
                            ...
                            start_time=start_time,
                            end_time=end_time,
                            dheera_ai_call_id=self.model_call_details["dheera_ai_call_id"],
                            print_verbose=print_verbose,
                        )
                    if (
                        callable(callback) and customLogger is not None
                    ...
                    print_verbose(
                        f"DheeraAI.LoggingError: [Non-Blocking] Exception occurred while failure logging with integrations {str(e)}"
                    )
                    print_verbose(
                        f"DheeraAI.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(
                "DheeraAI.LoggingError: [Non-Blocking] Exception occurred while failure logging {}".format(
                    str(e)
                )
            )
    ...
    async def async_failure_handler(
        ...
        callbacks = self.get_combined_callback_list(
            ...
            global_callbacks=dheera_ai._async_failure_callback,
        )
        ...
        for callback in callbacks:
            try:
                ...
                verbose_logger.exception(
                    "DheeraAI.LoggingError: [Non-Blocking] Exception occurred while failure \
                        logging {}\nCallback={}".format(
                        str(e), callback
                    )
                )
    ...
    def _get_trace_id(self, service_name: Literal["langfuse"]) -> Optional[str]:
        ...
        if service_name == "langfuse":
            trace_id = in_memory_trace_id_cache.get_cache(
                dheera_ai_call_id=self.dheera_ai_call_id, service_name=service_name
            )
    ...
    def _should_run_sync_callbacks_for_async_calls(self) -> bool:
        ...
        _filtered_success_callbacks = self._remove_internal_dheera_ai_callbacks(
    ...
    def _remove_internal_dheera_ai_callbacks(self, callbacks: List) -> List:
        """
        Creates a filtered list of callbacks, excluding internal DheeraAI callbacks.
        ...
        """
        filtered = [
            cb for cb in callbacks if not self._is_internal_dheera_ai_proxy_callback(cb)
        ]
    ...
    def _is_internal_dheera_ai_proxy_callback(self, cb) -> bool:
    ...
    def _remove_internal_custom_logger_callbacks(self, callbacks: List) -> List:
        ...
        for _c in callbacks:
            if isinstance(_c, CustomLogger):
                ...
                isinstance(_c, str)
                and _c in dheera_ai._known_custom_logger_compatible_callbacks
            ):
                continue
    ...
    def _handle_anthropic_messages_response_logging(self, result: Any) -> ModelResponse:
        ...
        if httpx_response and isinstance(httpx_response, httpx.Response):
            result = dheera_ai.AnthropicConfig().transform_response(
            ...
            result = dheera_ai.AnthropicConfig().transform_parsed_response(
    ...
    def _handle_non_streaming_google_genai_generate_content_response_logging(
        ...
        result = dheera_ai.VertexGeminiConfig()._transform_google_generate_content_to_openai_model_response(
    ...
    def _handle_a2a_response_logging(self, result: Any) -> Any:
        ...
        Args:
            result: The DheeraAISendMessageResponse from the A2A call
    ...
    try:
        for callback in callback_list:
            if callback == "sentry":
                ...
                from dheera_ai.integrations.weights_biases import WeightsBiasesLogger
...
def _init_custom_logger_compatible_class(  # noqa: PLR0915
    ...
    llm_router: Optional[
        Any
    ],  # expect dheera_ai.Router, but typing errors due to circular import
    ...
    try:
        ...
        if logging_integration == "agentops":  # Add AgentOps initialization
            ...
            from dheera_ai.integrations.opentelemetry import (
                OpenTelemetry,
                OpenTelemetryConfig,
            )
            ...
            from dheera_ai.proxy.hooks.dynamic_rate_limiter import (
            ...
            if llm_router is not None and isinstance(llm_router, dheera_ai.Router):
            ...
            from dheera_ai.proxy.hooks.dynamic_rate_limiter_v3 import (
            ...
            from dheera_ai.integrations.langfuse.langfuse_otel import LangfuseOtelLogger
            from dheera_ai.integrations.opentelemetry import (
                OpenTelemetry,
                OpenTelemetryConfig,
            )
            ...
            from dheera_ai.integrations.opentelemetry import OpenTelemetryConfig
            from dheera_ai.integrations.weave.weave_otel import (
                WeaveOtelLogger,
            ...
            from dheera_ai.integrations.bitbucket.bitbucket_prompt_manager import (
            ...
            bitbucket_config = getattr(dheera_ai, "global_bitbucket_config", None)
            if bitbucket_config is None:
                raise ValueError(
                    "BitBucket configuration not found. Please set dheera_ai.global_bitbucket_config first."
                )
            ...
            from dheera_ai.integrations.gitlab.gitlab_prompt_manager import (
                GitLabPromptManager,
            )
            ...
            # Get global BitBucket config
            gitlab_config = getattr(dheera_ai, "global_gitlab_config", None)
            if gitlab_config is None:
                raise ValueError(
                    "Gitlab configuration not found. Please set dheera_ai.global_gitlab_config first."
                )
...
def _get_custom_logger_settings_from_proxy_server(callback_name: str) -> Dict:
    ...
    """
    if dheera_ai.callback_settings:
        return dict(dheera_ai.callback_settings.get(callback_name, {}))
    return {}


def use_custom_pricing_for_model(dheera_ai_params: Optional[dict]) -> bool:
    """
    Check if the model uses custom pricing

    Returns True if any of `SPECIAL_MODEL_INFO_PARAMS` are present in `dheera_ai_params` or `model_info`
    """
    if dheera_ai_params is None:
        return False

    metadata: dict = dheera_ai_params.get("metadata", {}) or {}
    model_info: dict = metadata.get("model_info", {}) or {}

    custom_pricing_keys = CustomPricingDheeraAIParams.model_fields.keys()
    for key in custom_pricing_keys:
        if dheera_ai_params.get(key, None) is not None:
            return True
...
class StandardLoggingPayloadSetup:
    ...
    def get_standard_logging_metadata(
        ...
        if dheera_ai_params is not None:
            prompt_id = cast(Optional[str], dheera_ai_params.get("prompt_id", None))
            prompt_variables = cast(
                Optional[dict], dheera_ai_params.get("prompt_variables", None)
            )
    ...
    def get_model_cost_information(
        ...
        if model_cost_name is None:
            ...
            try:
                _model_cost_information = dheera_ai.get_model_info(
    ...
    def get_hidden_params(
        ...
        clean_hidden_params = StandardLoggingHiddenParams(
            ...
            dheera_ai_overhead_time_ms=None,
            batch_models=None,
    ...
    def _generate_cold_storage_object_key(
        ...
        from dheera_ai.integrations.s3 import get_s3_object_key

        # Only generate object key if cold storage is configured
        cold_storage_custom_logger = dheera_ai.cold_storage_custom_logger
        if cold_storage_custom_logger is None:
            return None

        try:
            # Generate file name in same format as dheera_ai.utils.get_logging_id
            ...
            try:
                custom_logger = dheera_ai.logging_callback_manager.get_active_custom_logger_for_callback_name(
    ...
    def get_error_information(
        ...
        from dheera_ai.constants import MAXIMUM_TRACEBACK_LINES_TO_LOG
    ...
    def _get_standard_logging_payload_trace_id(
        ...
        """
        Returns the `dheera_ai_trace_id` for this request

        This helps link sessions when multiple requests are made in a single session
        """
        dynamic_dheera_ai_session_id = dheera_ai_params.get("dheera_ai_session_id")
        dynamic_dheera_ai_trace_id = dheera_ai_params.get("dheera_ai_trace_id")

        # Note: we recommend using `dheera_ai_session_id` for session tracking
        # `dheera_ai_trace_id` is an internal dheera_ai param
        if dynamic_dheera_ai_session_id:
            return str(dynamic_dheera_ai_session_id)
        elif dynamic_dheera_ai_trace_id:
            return str(dynamic_dheera_ai_trace_id)
        else:
            return logging_obj.dheera_ai_trace_id
    ...
    def _get_user_agent_tags(proxy_server_request: dict) -> Optional[List[str]]:
        ...
        """
        if dheera_ai.disable_add_user_agent_to_request_tags is True:
            return None
    ...
    def _get_extra_header_tags(proxy_server_request: dict) -> Optional[List[str]]:
        ...
        """
        extra_headers: List[str] = dheera_ai.extra_spend_tag_headers or []
    ...
    def _get_request_tags(
        dheera_ai_params: dict, proxy_server_request: dict
    ) -> List[str]:
        # check for 'tags' in both 'metadata' and 'dheera_ai_metadata'
        metadata = dheera_ai_params.get("metadata") or {}
        dheera_ai_metadata = dheera_ai_params.get("dheera_ai_metadata") or {}
        if metadata.get("tags", []):
            request_tags = metadata.get("tags", [])
        elif dheera_ai_metadata.get("tags", []):
            request_tags = dheera_ai_metadata.get("tags", [])
        else:
            request_tags = []
...
def get_standard_logging_object_payload(
    ...
    try:
        ...
        proxy_server_request = dheera_ai_params.get("proxy_server_request") or {}

        metadata: dict = (
            dheera_ai_params.get("dheera_ai_metadata")
            or dheera_ai_params.get("metadata", None)
            or {}
        )
        ...
        id = response_obj.get("id", kwargs.get("dheera_ai_call_id"))

        _model_id = metadata.get("model_info", {}).get("id", "")
        _model_group = metadata.get("model_group", "")

        request_tags = StandardLoggingPayloadSetup._get_request_tags(
            dheera_ai_params=dheera_ai_params, proxy_server_request=proxy_server_request
        )
        ...
        custom_pricing = use_custom_pricing_for_model(dheera_ai_params=dheera_ai_params)
...
def emit_standard_logging_payload(payload: StandardLoggingPayload):
    if os.getenv("DHEERA_AI_PRINT_STANDARD_LOGGING_PAYLOAD"):
...
def scrub_sensitive_keys_in_metadata(dheera_ai_params: Optional[dict]):
    if dheera_ai_params is None:
        dheera_ai_params = {}
    ...
    if "langfuse_masking_function" in metadata:
        ...
        if callable(masking_fn):
            dheera_ai_params["_langfuse_masking_function"] = masking_fn
        dheera_ai_params["metadata"] = metadata
        ...
        for k, v in metadata["user_api_key_metadata"].items():
            if k == "logging":  # prevent logging user logging keys
                cleaned_user_api_key_metadata[k] = (
                    "scrubbed_by_dheera_ai_for_sensitive_keys"
                )
            else:
