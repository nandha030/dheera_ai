...
class ExceptionCheckers:
    ...
    def is_error_str_context_window_exceeded(error_str: str) -> bool:
        ...
        known_exception_substrings = [
            ...
            # See: https://github.com/BerriAI/dheera_ai/issues/XXXX
    ...
    try:
        ...
        if not _response_headers:
            _response_headers = getattr(
                original_exception, "dheera_ai_response_headers", None
            )
    except Exception:
        return None
...
def extract_and_raise_dheera_ai_exception(
    ...
    """
    Covers scenario where dheera_ai sdk calling proxy.

    Enables raising the special errors raised by dheera_ai, eg. ContextWindowExceededError.

    Relevant Issue: https://github.com/BerriAI/dheera_ai/issues/7259
    """
    pattern = r"dheera_ai\.\w+Error"
    ...
    if match:
        ...
        exception_name = exception_name.strip().replace("dheera_ai.", "")
        raised_exception_obj = getattr(dheera_ai, exception_name, None)
...
def exception_type(  # type: ignore  # noqa: PLR0915
    ...
    if any(
        ...
        for exc_type in dheera_ai.DHEERA_AI_EXCEPTION_TYPES
    ):
    ...
    if dheera_ai.suppress_debug_info is False:
        print()  # noqa
        print(  # noqa
            "\033[1;31mGive Feedback / Get Help: https://github.com/BerriAI/dheera_ai/issues/new\033[0m"  # noqa
        )  # noqa
        print(  # noqa
            "DheeraAI.Info: If you need to debug this error, use `dheera_ai._turn_on_debug()'."  # noqa
        )  # noqa
        print()  # noqa

    dheera_ai_response_headers = _get_response_headers(
    ...
    try:
        ...
        if model:
            ...
            extra_information = ""
            try:
                _api_base = dheera_ai.get_api_base(
                    model=model, optional_params=extra_kwargs
                )
                messages = dheera_ai.get_first_chars_messages(kwargs=completion_kwargs)
                ...
                if (
                    messages
                    and len(messages) > 0
                    and dheera_ai.redact_messages_in_exceptions is False
                ):
                    extra_information += f"\nMessages: `{messages}`"
                ...
                # on dheera_ai proxy add key name + team to exceptions
            ...
            if (
                ...
                raise Timeout(
                    ...
                    model=model,
                    llm_provider=custom_llm_provider,
                    dheera_ai_debug_info=extra_information,
                )

            if (
                custom_llm_provider == "dheera_ai_proxy"
            ):  # handle special case where calling dheera_ai proxy + exception str contains error message
            ...
            if (
                ...
                if message is not None and isinstance(
                    message, str
                ):  # done to prevent user-confusion. Relevant issue - https://github.com/BerriAI/dheera_ai/issues/1414
                ...
                if hasattr(original_exception, "status_code"):
                    ...
                    if original_exception.status_code == 401:
                        ...
                        exception_mapping_worked = True
                        raise dheera_ai.InternalServerError(
                            message=f"AnthropicException - {error_str}. Handle with `dheera_ai.InternalServerError`.",
                            llm_provider="anthropic",
                            model=model,
                            response=getattr(original_exception, "response", None),
                        )
                        ...
                        exception_mapping_worked = True
                        raise dheera_ai.ServiceUnavailableError(
                            message=f"AnthropicException - {error_str}. Handle with `dheera_ai.ServiceUnavailableError`.",
                            llm_provider="anthropic",
                            model=model,
                            response=getattr(original_exception, "response", None),
                        )
            ...
            elif custom_llm_provider in dheera_ai._openai_like_providers:
                ...
                if (
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"BedrockException - {error_str}\n. Enable 'dheera_ai.modify_params=True' (for PROXY do: `dheera_ai_settings::modify_params: True`) to insert a dummy assistant message and fix this error.",
                        model=model,
                        llm_provider="bedrock",
                        response=getattr(original_exception, "response", None),
                    )
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"BedrockException - {error_str}\n. Pass in default user message via `completion(..,user_continue_message=)` or enable `dheera_ai.modify_params=True`.\nFor Proxy: do via `dheera_ai_settings::modify_params: True` or user_continue_message under `dheera_ai_params`",
                        model=model,
                        llm_provider="bedrock",
                        response=getattr(original_exception, "response", None),
                    )
                ...
                if "Unable to locate credentials" in error_str:
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"dheera_ai.BadRequestError: SagemakerException - {error_str}",
                        model=model,
                ...
                if (
                    ...
                    exception_mapping_worked = True
                    raise BadRequestError(
                        message=f"dheera_ai.BadRequestError: {custom_llm_provider}Exception - {error_str}",
                        model=model,
                        llm_provider=custom_llm_provider,
                        response=httpx.Response(
                            status_code=400,
                ...
                if "400 Request payload size exceeds" in error_str:
                    ...
                    exception_mapping_worked = True
                    raise dheera_ai.InternalServerError(
                        message=f"dheera_ai.InternalServerError: {custom_llm_provider}Exception - {error_str}",
                        model=model,
                        llm_provider=custom_llm_provider,
                        response=httpx.Response(
                            status_code=500,
                    ...
                    exception_mapping_worked = True
                    raise RateLimitError(
                        message=f"dheera_ai.RateLimitError: {custom_llm_provider}Exception - {error_str}",
                        model=model,
                        llm_provider=custom_llm_provider,
                        dheera_ai_debug_info=extra_information,
                        response=httpx.Response(
                ...
                if hasattr(original_exception, "status_code"):
                    ...
                    if original_exception.status_code == 429:
                        exception_mapping_worked = True
                        raise RateLimitError(
                            message=f"dheera_ai.RateLimitError: {custom_llm_provider.capitalize()}Exception - {error_str}",
                            model=model,
                            llm_provider=custom_llm_provider,
                            dheera_ai_debug_info=extra_information,
                            response=httpx.Response(
                ...
                if "Internal server error" in error_str:
                    ...
                    exception_mapping_worked = True
                    from dheera_ai.llms.azure.exception_mapping import (
        ...
        if (
            ...
            """
            For unmapped exceptions - raise the exception with traceback - https://github.com/BerriAI/dheera_ai/issues/4201
            """
            exception_mapping_worked = True
        ...
        if exception_mapping_worked:
            setattr(e, "dheera_ai_response_headers", dheera_ai_response_headers)
            raise e
        else:
            for error_type in dheera_ai.DHEERA_AI_EXCEPTION_TYPES:
            ...
            setattr(raised_exc, "dheera_ai_response_headers", dheera_ai_response_headers)
            raise raised_exc
    ...
    try:
        ...
        if logger_fn and callable(logger_fn):
            try:
                ...
                verbose_logger.debug(
                    f"DheeraAI.LoggingError: [Non-Blocking] Exception occurred while logging {traceback.format_exc()}"
                )
    except Exception:
        verbose_logger.debug(
            f"DheeraAI.LoggingError: [Non-Blocking] Exception occurred while logging {traceback.format_exc()}"
...
def _add_key_name_and_team_to_alert(request_info: str, metadata: dict) -> str:
    """
    Internal helper function for dheera_ai proxy
