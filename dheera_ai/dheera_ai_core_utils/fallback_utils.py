...
from dheera_ai.dheera_ai_core_utils.core_helpers import safe_deep_copy, filter_internal_params
...
async def async_completion_with_fallbacks(**kwargs):
    ...
    base_kwargs = {**kwargs, **nested_kwargs, "dheera_ai_call_id": dheera_ai_call_id}

    # fields to remove
    base_kwargs.pop("model", None)  # Remove model as it will be set per fallback
    dheera_ai_logging_obj = base_kwargs.pop("dheera_ai_logging_obj", None)
    ...
    raise Exception(
        f"{most_recent_exception_str}. All fallback attempts failed. Enable verbose logging with `dheera_ai.set_verbose=True` for details."
    )
