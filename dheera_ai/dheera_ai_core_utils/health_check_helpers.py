...
class HealthCheckHelpers:
    ...
    async def ahealth_check_wildcard_models(
        ...
        if len(cheapest_models) == 0:
            raise Exception(
                f"Unable to health check wildcard model for provider {custom_llm_provider}. Add a model on your config.yaml or contribute here - https://github.com/BerriAI/dheera_ai/blob/main/model_prices_and_context_window.json"
            )
    ...
    def _update_model_params_with_health_check_tracking_information(
        ...
        _metadata_variable_name = "dheera_ai_metadata"
        dheera_ai_metadata = HealthCheckHelpers._get_metadata_for_health_check_call()
        model_params[_metadata_variable_name] = dheera_ai_metadata
        model_params = DheeraAIProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
            data=model_params,
            user_api_key_dict=UserAPIKeyAuth.get_dheera_ai_internal_health_check_user_api_key_auth(),
    ...
    def get_mode_handlers(
        ...
        from dheera_ai.dheera_ai_core_utils.health_check_utils import _filter_model_params
        from dheera_ai.realtime_api.main import _realtime_health_check

        return {
            "chat": lambda: dheera_ai.acompletion(
                **model_params,
            ),
            "completion": lambda: dheera_ai.atext_completion(
                **_filter_model_params(model_params=model_params),
                prompt=prompt or "test",
            ),
            "embedding": lambda: dheera_ai.aembedding(
                **_filter_model_params(model_params=model_params),
                input=input or ["test"],
            ),
            "audio_speech": lambda: dheera_ai.aspeech(
                **{
                    **_filter_model_params(model_params=model_params),
                    **(
                        {"voice": "alloy"}
            ...
            "audio_transcription": lambda: dheera_ai.atranscription(
                **_filter_model_params(model_params=model_params),
                file=get_audio_file_for_health_check(),
            ),
            "image_generation": lambda: dheera_ai.aimage_generation(
                **_filter_model_params(model_params=model_params),
                prompt=prompt,
            ),
            "video_generation": lambda: dheera_ai.avideo_generation(
                **_filter_model_params(model_params=model_params),
                prompt=prompt or "test video generation",
            ),
            "rerank": lambda: dheera_ai.arerank(
                **_filter_model_params(model_params=model_params),
                query=prompt or "",
            ...
            "batch": lambda: dheera_ai.alist_batches(
                **_filter_model_params(model_params=model_params),
            ),
            "responses": lambda: dheera_ai.aresponses(
                **_filter_model_params(model_params=model_params),
                input=prompt or "test",
            ),
            "ocr": lambda: dheera_ai.aocr(
                **_filter_model_params(model_params=model_params),
                document={
                    "type": "document_url",
