...
import dheera_ai
from dheera_ai.constants import OPENAI_FILE_SEARCH_COST_PER_1K_CALLS
...
class StandardBuiltInToolCostTracking:
    ...
    def _handle_web_search_cost(
        ...
        from dheera_ai.llms import get_cost_for_web_search_request
        ...
        if custom_llm_provider is None and model_info is not None:
            custom_llm_provider = model_info["dheera_ai_provider"]
    ...
    def _safe_get_model_info(
        ...
        try:
            return dheera_ai.get_model_info(
                model=model, custom_llm_provider=custom_llm_provider
            )
        except Exception:
            return None
    ...
    def get_cost_for_file_search(
        ...
        if provider == "azure":
            from dheera_ai.constants import AZURE_FILE_SEARCH_COST_PER_GB_PER_DAY
    ...
    def get_cost_for_vector_store(
        ...
        if provider == "azure":
            from dheera_ai.constants import AZURE_VECTOR_STORE_COST_PER_GB_PER_DAY
    ...
    def _get_code_interpreter_cost_from_model_map(
        ...
        try:
            ...
            if model_key and model_key in dheera_ai.model_cost:
                return dheera_ai.model_cost[model_key].get("code_interpreter_cost_per_session")
