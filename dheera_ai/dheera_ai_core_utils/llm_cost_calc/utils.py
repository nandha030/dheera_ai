...
def _generic_cost_per_character(
    ...
    try:
        ...
        verbose_logger.exception(
            "dheera_ai.dheera_ai_core_utils.llm_cost_calc.utils.py::cost_per_character(): Exception occured - {}\nDefaulting to None".format(
                str(e)
            )
        )
...
def _get_cost_per_unit(
    ...
    if isinstance(cost_per_unit, str):
        try:
            ...
            verbose_logger.exception(
                f"dheera_ai.dheera_ai_core_utils.llm_cost_calc.utils.py::calculate_cost_per_component(): Exception occured - {cost_per_unit}\nDefaulting to 0.0"
            )
            ...
            if suffix in cost_key:
                ...
                if isinstance(fallback_cost, str):
                    try:
                        ...
                        verbose_logger.exception(
                            f"dheera_ai.dheera_ai_core_utils.llm_cost_calc.utils.py::_get_cost_per_unit(): Exception occured - {fallback_cost}\nDefaulting to 0.0"
                        )
...
def generic_cost_per_token(
    ...
    Input:
        - model: str, the model name without provider prefix
        - usage: DheeraAI Usage block, containing anthropic caching information
...
class CostCalculatorUtils:
    ...
    def route_image_generation_cost_calculator(
        ...
        from dheera_ai.llms.azure_ai.image_generation.cost_calculator import (
            cost_calculator as azure_ai_image_cost_calculator,
        )
        from dheera_ai.llms.bedrock.image.cost_calculator import (
            cost_calculator as bedrock_image_cost_calculator,
        )
        from dheera_ai.llms.gemini.image_generation.cost_calculator import (
            cost_calculator as gemini_image_cost_calculator,
        )
        from dheera_ai.llms.recraft.cost_calculator import (
            cost_calculator as recraft_image_cost_calculator,
        )
        from dheera_ai.llms.vertex_ai.image_generation.cost_calculator import (
        ...
        if custom_llm_provider == dheera_ai.LlmProviders.VERTEX_AI.value:
            if isinstance(completion_response, ImageResponse):
                return vertex_ai_image_cost_calculator(
                    model=model,
                    image_response=completion_response,
                )
        elif custom_llm_provider == dheera_ai.LlmProviders.BEDROCK.value:
            if isinstance(completion_response, ImageResponse):
        ...
        elif custom_llm_provider == dheera_ai.LlmProviders.RECRAFT.value:
            from dheera_ai.llms.recraft.cost_calculator import (
                cost_calculator as recraft_image_cost_calculator,
            )
        ...
        elif custom_llm_provider == dheera_ai.LlmProviders.AIML.value:
            from dheera_ai.llms.aiml.image_generation.cost_calculator import (
        ...
        elif custom_llm_provider == dheera_ai.LlmProviders.COMETAPI.value:
            from dheera_ai.llms.cometapi.image_generation.cost_calculator import (
        ...
        elif custom_llm_provider == dheera_ai.LlmProviders.GEMINI.value:
            if call_type in (
                CallTypes.image_edit.value,
                CallTypes.aimage_edit.value,
            ):
                from dheera_ai.llms.gemini.image_edit.cost_calculator import (
        ...
        elif custom_llm_provider == dheera_ai.LlmProviders.AZURE_AI.value:
            return azure_ai_image_cost_calculator(
                model=model,
                image_response=completion_response,
            )
        elif custom_llm_provider == dheera_ai.LlmProviders.FAL_AI.value:
            from dheera_ai.llms.fal_ai.cost_calculator import (
        ...
        elif custom_llm_provider == dheera_ai.LlmProviders.RUNWAYML.value:
            from dheera_ai.llms.runwayml.cost_calculator import (
