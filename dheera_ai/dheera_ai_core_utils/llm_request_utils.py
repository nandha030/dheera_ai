...
def _ensure_extra_body_is_safe(extra_body: Optional[Dict]) -> Optional[Dict]:
    ...
    Relevant Issue: https://github.com/BerriAI/dheera_ai/issues/4140
    """
...
def pick_cheapest_chat_models_from_llm_provider(custom_llm_provider: str, n=1):
    ...
    """
    if custom_llm_provider not in dheera_ai.models_by_provider:
        return []

    known_models = dheera_ai.models_by_provider.get(custom_llm_provider, [])
    model_costs = []
...
def get_proxy_server_request_headers(dheera_ai_params: Optional[dict]) -> dict:
    """
    Get the `proxy_server_request` headers from the dheera_ai_params.\

    Use this if you want to access the request headers made to DheeraAI proxy server.
    """
