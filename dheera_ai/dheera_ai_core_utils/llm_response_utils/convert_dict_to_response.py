...
from dheera_ai.types.llms.databricks import DatabricksTool
...
from dheera_ai.types.utils import Logprobs as TextCompletionLogprobs
from dheera_ai.types.utils import (
    Message,
    ModelResponse,
    ModelResponseStream,
...
def _safe_convert_created_field(created_value) -> int:
    ...
    (Unix timestamp with fractional seconds), but DheeraAI expects an integer.
...
def convert_tool_call_to_json_mode(
    ...
    if _should_convert_tool_call_to_json_mode(
        ...
        if json_mode_content_str is not None:
            message = dheera_ai.Message(content=json_mode_content_str)
            finish_reason = "stop"
...
class DheeraAIResponseObjectHandler:
    @staticmethod
    ...
    def convert_chat_to_text_completion(
        ...
        """
        transformed_logprobs = DheeraAIResponseObjectHandler._convert_provider_response_logprobs_to_text_completion_logprobs(
            response=response,
...
def convert_to_model_response_object(  # noqa: PLR0915
    ...
    try:
        if response_type == "completion" and (
            ...
            if "usage" in response_object and response_object["usage"] is not None:
                usage_object = dheera_ai.Usage(**response_object["usage"])
            ...
            special_keys = list(dheera_ai.ModelResponse.model_fields.keys())
            ...
            return DheeraAIResponseObjectHandler.convert_to_image_response(
