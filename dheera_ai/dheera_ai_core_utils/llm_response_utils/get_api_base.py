...
from ...dheera_ai_core_utils.get_llm_provider_logic import get_llm_provider
from ...types.router import DheeraAI_Params


def get_api_base(
    model: str, optional_params: Union[dict, DheeraAI_Params]
) -> Optional[str]:
    """
    Returns the api base used for calling the model.

    Parameters:
    - model: str - the model passed to dheera_ai.completion()
    - optional_params - the 'dheera_ai_params' in router.completion *OR* additional params passed to dheera_ai.completion - eg. api_base, api_key, etc. See `DheeraAI_Params` - https://github.com/BerriAI/dheera_ai/blob/f09e6ba98d65e035a79f73bc069145002ceafd36/dheera_ai/router.py#L67
    ...
    Example:
    ```
    from dheera_ai import get_api_base
    ...
    try:
        if isinstance(optional_params, DheeraAI_Params):
            _optional_params = optional_params
        elif "model" in optional_params:
            _optional_params = DheeraAI_Params(**optional_params)
        else:  # prevent needing to copy and pop the dict
            _optional_params = DheeraAI_Params(
    ...
    if dheera_ai.model_alias_map and model in dheera_ai.model_alias_map:
        model = dheera_ai.model_alias_map[model]
    try:
        (
            model,
            custom_llm_provider,
    ...
    if (
        ...
        from dheera_ai.types.llms.vertex_ai import VertexPartnerProvider
