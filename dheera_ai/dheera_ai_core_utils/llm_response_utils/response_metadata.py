...
from dheera_ai.dheera_ai_core_utils.logging_utils import DheeraAILoggingObject
...
class ResponseMetadata:
    """
    Handles setting and managing `_hidden_params`, `response_time_ms`, and `dheera_ai_overhead_time_ms` for DheeraAI responses
    """
    ...
    def set_hidden_params(
        self, logging_obj: DheeraAILoggingObject, model: Optional[str], kwargs: dict
    ) -> None:
        ...
        new_params = {
            "dheera_ai_call_id": getattr(logging_obj, "dheera_ai_call_id", None),
            "api_base": get_api_base(model=model or "", optional_params=kwargs),
            "model_id": model_id,
            "response_cost": logging_obj._response_cost_calculator(
                result=self.result, dheera_ai_model_name=model, router_model_id=model_id
            ),
    ...
    def set_timing_metrics(
        ...
        #########################################################
        # 2. Add DheeraAI overhead duration
        #########################################################
        ...
        if llm_api_duration_ms is not None:
            ...
            self._update_hidden_params(
                {
                    "dheera_ai_overhead_time_ms": overhead_ms,
                }
            )

        #########################################################
        # 3. Add duration for reading from cache
        # In this case overhead from dheera_ai is the difference between the cache read duration and the total response time
        #########################################################
        if (
...
def update_response_metadata(
    ...
    Updates response metadata, adds the following:
        ...
        - response._hidden_params["dheera_ai_overhead_time_ms"]
