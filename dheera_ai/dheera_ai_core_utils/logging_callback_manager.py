...
from dheera_ai.integrations.additional_logging_utils import AdditionalLoggingUtils
...
from dheera_ai.types.utils import CallbacksByType
...
class LoggingCallbackManager:
    """
    A centralized class that allows easy add / remove callbacks for dheera_ai.
    ...
    def add_dheera_ai_input_callback(self, callback: Union[CustomLogger, str]):
        """
        Add a input callback to dheera_ai.input_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=dheera_ai.input_callback
        )

    def add_dheera_ai_service_callback(
        self, callback: Union[CustomLogger, str, Callable]
    ):
        """
        Add a service callback to dheera_ai.service_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=dheera_ai.service_callback
        )

    def add_dheera_ai_callback(self, callback: Union[CustomLogger, str, Callable]):
        """
        Add a callback to dheera_ai.callbacks

        Ensures no duplicates are added.
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=dheera_ai.callbacks  # type: ignore
        )

    def add_dheera_ai_success_callback(
        self, callback: Union[CustomLogger, str, Callable]
    ):
        """
        Add a success callback to `dheera_ai.success_callback`
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=dheera_ai.success_callback
        )

    def add_dheera_ai_failure_callback(
        self, callback: Union[CustomLogger, str, Callable]
    ):
        """
        Add a failure callback to `dheera_ai.failure_callback`
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=dheera_ai.failure_callback
        )

    def add_dheera_ai_async_success_callback(
        self, callback: Union[CustomLogger, Callable, str]
    ):
        """
        Add a success callback to dheera_ai._async_success_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=dheera_ai._async_success_callback
        )

    def add_dheera_ai_async_failure_callback(
        self, callback: Union[CustomLogger, Callable, str]
    ):
        """
        Add a failure callback to dheera_ai._async_failure_callback
        """
        self._safe_add_callback_to_list(
            callback=callback, parent_list=dheera_ai._async_failure_callback
        )
    ...
    def _add_custom_callback_generic_api_str(
        ...
        """
        callback_config = dheera_ai.callback_settings.get(callback)
    ...
    def _get_all_callbacks(self) -> List[Union[CustomLogger, Callable, str]]:
        """
        Get all callbacks from dheera_ai.callbacks, dheera_ai.success_callback, dheera_ai.failure_callback, dheera_ai._async_success_callback, dheera_ai._async_failure_callback
        """
        return (
            dheera_ai.callbacks
            + dheera_ai.success_callback
    ...
    def get_callbacks_by_type(self) -> CallbacksByType:
        ...
        success_callbacks = set(
            dheera_ai.success_callback + dheera_ai._async_success_callback
        )
        failure_callbacks = set(
            dheera_ai.failure_callback + dheera_ai._async_failure_callback
        )
        general_callbacks = set(dheera_ai.callbacks)
