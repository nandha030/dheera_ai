...
if TYPE_CHECKING:
    ...
    from dheera_ai import ModelResponse as _ModelResponse
    from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import (
        Logging as DheeraAILoggingObject,
    )

    DheeraAIModelResponse = _ModelResponse
    Span = Union[_Span, Any]
else:
    DheeraAIModelResponse = Any
...
def _get_parent_otel_span_from_logging_obj(
    ...
    Args:
        logging_obj: The DheeraAI logging object containing model call details
...
def convert_dheera_ai_response_object_to_str(
    response_obj: Union[Any, DheeraAIModelResponse],
) -> Optional[str]:
    """
    Get the string of the response object from DheeraAI

    """
    if isinstance(response_obj, dheera_ai.ModelResponse):
        response_str = ""
...
def _set_duration_in_model_call_details(
    logging_obj: Any,  # we're not guaranteed this will be `DheeraAILoggingObject`
    start_time: datetime,
    end_time: datetime,
):
