...
import dheera_ai.types.llms
from dheera_ai import verbose_logger
from dheera_ai._uuid import uuid
from dheera_ai.llms.custom_httpx.http_handler import HTTPHandler, get_async_httpx_client
from dheera_ai.types.files import get_file_extension_from_mime_type
from dheera_ai.types.llms.anthropic import *
from dheera_ai.types.llms.bedrock import CachePointBlock
from dheera_ai.types.llms.bedrock import MessageBlock as BedrockMessageBlock
from dheera_ai.types.llms.custom_http import httpxSpecialProvider
from dheera_ai.types.llms.ollama import OllamaVisionModelObject
...
from dheera_ai.types.llms.vertex_ai import FunctionCall as VertexFunctionCall
from dheera_ai.types.llms.vertex_ai import FunctionResponse as VertexFunctionResponse
from dheera_ai.types.llms.vertex_ai import PartType as VertexPartType
from dheera_ai.types.utils import GenericImageParsingChunk
...
}  # similar to autogen. Only used if `dheera_ai.modify_params=True`.
...
async def _afetch_and_extract_template(
    ...
    """
    from dheera_ai.dheera_ai_core_utils.prompt_templates.huggingface_template_handler import (
    ...
    if chat_template is None:
        ...
        if model in dheera_ai.known_tokenizer_config:
            tokenizer_config = dheera_ai.known_tokenizer_config[model]
        else:
            tokenizer_config = await get_config_fn(hf_model_name=model)
            dheera_ai.known_tokenizer_config.update({model: tokenizer_config})
...
def anthropic_messages_pt_xml(messages: list):
    ...
    1. Anthropic supports roles like "user" and "assistant", (here dheera_ai translates system-> assistant)
    2. The first message always needs to be of role "user"
    3. Each message must alternate between "user" and "assistant" (this is not addressed as now by dheera_ai)
    ...
    if not new_messages or new_messages[0]["role"] != "user":
        if dheera_ai.modify_params:
            new_messages.insert(
                0, {"role": "user", "content": [{"type": "text", "text": "."}]}
            )
        else:
            raise Exception(
                "Invalid first message. Should always start with 'role'='user' for Anthropic. System prompt is sent separately for Anthropic. set 'dheera_ai.modify_params = True' or 'dheera_ai_settings:modify_params = True' on proxy, to insert a placeholder user message - '.' as the first message, "
            )
...
def convert_to_gemini_tool_call_result(
    ...
    """
    from dheera_ai.types.llms.vertex_ai import BlobType
...
def anthropic_messages_pt(  # noqa: PLR0915
    ...
    if len(messages) == 0:
        if not dheera_ai.modify_params:
            raise dheera_ai.BadRequestError(
                message=f"Anthropic requires at least one non-system message. Either provide one, or set `dheera_ai.modify_params = True` // `dheera_ai_settings::modify_params: True` to add the dummy user message - {DEFAULT_USER_CONTINUE_MESSAGE_TYPED}.",
                model=model,
...
from dheera_ai.types.llms.cohere import (
    CallObject,
    ChatHistory,
    ChatHistoryChatBot,
    ChatHistorySystem,
...
def amazon_titan_pt(
    messages: list,
):  # format - https://github.com/BerriAI/dheera_ai/issues/1896
    """
...
from dheera_ai.types.llms.bedrock import ContentBlock as BedrockContentBlock
from dheera_ai.types.llms.bedrock import DocumentBlock as BedrockDocumentBlock
from dheera_ai.types.llms.bedrock import ImageBlock as BedrockImageBlock
from dheera_ai.types.llms.bedrock import SourceBlock as BedrockSourceBlock
from dheera_ai.types.llms.bedrock import ToolBlock as BedrockToolBlock
from dheera_ai.types.llms.bedrock import (
    ToolInputSchemaBlock as BedrockToolInputSchemaBlock,
)
from dheera_ai.types.llms.bedrock import ToolJsonSchemaBlock as BedrockToolJsonSchemaBlock
from dheera_ai.types.llms.bedrock import ToolResultBlock as BedrockToolResultBlock
from dheera_ai.types.llms.bedrock import (
    ToolResultContentBlock as BedrockToolResultContentBlock,
)
from dheera_ai.types.llms.bedrock import ToolSpecBlock as BedrockToolSpecBlock
from dheera_ai.types.llms.bedrock import ToolUseBlock as BedrockToolUseBlock
from dheera_ai.types.llms.bedrock import VideoBlock as BedrockVideoBlock
...
class BedrockImageProcessor:
    ...
    def _validate_format(mime_type: str, image_format: str) -> str:
        ...
        supported_image_formats = (
            dheera_ai.AmazonConverseConfig().get_supported_image_types()
        )
        supported_doc_formats = (
            dheera_ai.AmazonConverseConfig().get_supported_document_types()
        )
        supported_video_formats = (
            dheera_ai.AmazonConverseConfig().get_supported_video_types()
        )
    ...
    def _get_document_format(mime_type: str, supported_doc_formats: List[str]) -> str:
        ...
        Relevant Issue: https://github.com/BerriAI/dheera_ai/issues/12260
        ...
        #################
        # dheera_ai runs on docker containers and `mimetypes` depends on the installed mimetypes of the OS
...
def _insert_assistant_continue_message(
    ...
    Conversation blocks and tool result blocks cannot be provided in the same turn. Issue: https://github.com/BerriAI/dheera_ai/issues/6053
    """
...
def get_user_message_block_or_continue_message(
    ...
    Relevant Issue: https://github.com/BerriAI/dheera_ai/issues/7169
    """
    content_block = message.get("content", None)

    # Handle None case
    if content_block is None or (
        user_continue_message is None and dheera_ai.modify_params is False
    ):
...
def get_assistant_message_block_or_continue_message(
    ...
    # Handle Base case
    if content_block is None or (
        assistant_continue_message is None and dheera_ai.modify_params is False
    ):
        return skip_empty_text_blocks(message=message)
...
class BedrockConverseMessagesProcessor:
                    ...
                    for element in message_block["content"]:
                        if isinstance(element, dict):
                            if element["type"] == "text":
                                ...
                                _parts.append(_part)
                            _cache_point_block = (
                                dheera_ai.AmazonConverseConfig()._get_cache_point_block(
                                    message_block=cast(
            ...
            if user_content:
                if len(contents) > 0 and contents[-1]["role"] == "user":
                    if (
                        ...
                        or dheera_ai.modify_params is True
                    ):
    ...
    def add_thinking_blocks_to_assistant_content(
        ...
        Relevant Issue: https://github.com/BerriAI/dheera_ai/issues/9063
        """
...
def make_valid_bedrock_tool_name(input_tool_name: str) -> str:
    ...
    if input_tool_name != valid_string:
        ...
        dheera_ai.bedrock_tool_name_mappings.set_cache(
...
def _bedrock_tools_pt(tools: List) -> List[BedrockToolBlock]:
    ...
    """
    from dheera_ai.dheera_ai_core_utils.prompt_templates.common_utils import unpack_defs
...
def response_schema_prompt(model: str, response_schema: dict) -> str:
    ...
    if f"{model}/response_schema_prompt" in dheera_ai.custom_prompt_dict:
        custom_prompt_details = dheera_ai.custom_prompt_dict[
            f"{model}/response_schema_prompt"
        ]  # allow user to define custom response schema prompt by model
    elif "response_schema_prompt" in dheera_ai.custom_prompt_dict:
        custom_prompt_details = dheera_ai.custom_prompt_dict["response_schema_prompt"]
...
def prompt_factory(
    ...
    if custom_llm_provider == "ollama":
        ...
        if (
            ...
            or dheera_ai.supports_vision(model=model)
            or dheera_ai.supports_vision(model=custom_llm_provider + "/" + model)
        ):
            return _gemini_vision_convert_messages(messages=messages)
        else:
            return gemini_text_image_pt(messages=messages)
    elif custom_llm_provider == "mistral":
        return dheera_ai.MistralConfig()._transform_messages(
        ...
        from dheera_ai.llms.watsonx.chat.transformation import IBMWatsonXChatConfig
