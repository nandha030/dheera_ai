...
if TYPE_CHECKING:
    from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import (
        Logging as _DheeraAILoggingObject,
    )

    DheeraAILoggingObject = _DheeraAILoggingObject
else:
    DheeraAILoggingObject = Any


def redact_message_input_output_from_custom_logger(
    dheera_ai_logging_obj: DheeraAILoggingObject, result, custom_logger: CustomLogger
):
    if (
        hasattr(custom_logger, "message_logging")
        and custom_logger.message_logging is not True
    ):
        return perform_redaction(dheera_ai_logging_obj.model_call_details, result)
    return result
...
def _redact_choice_content(choice):
    ...
    if isinstance(choice, dheera_ai.Choices):
        choice.message.content = "redacted-by-dheera_ai"
        if hasattr(choice.message, "reasoning_content"):
            choice.message.reasoning_content = "redacted-by-dheera_ai"
        if hasattr(choice.message, "thinking_blocks"):
            choice.message.thinking_blocks = None
    elif isinstance(choice, dheera_ai.utils.StreamingChoices):
        choice.delta.content = "redacted-by-dheera_ai"
        if hasattr(choice.delta, "reasoning_content"):
            choice.delta.reasoning_content = "redacted-by-dheera_ai"
        ...
        if hasattr(output_item, "content") and isinstance(output_item.content, list):
            for content_part in output_item.content:
                if hasattr(content_part, "text"):
                    content_part.text = "redacted-by-dheera_ai"
            ...
            if hasattr(output_item, "summary") and isinstance(output_item.summary, list):
                for summary_item in output_item.summary:
                    if hasattr(summary_item, "text"):
                        summary_item.text = "redacted-by-dheera_ai"
...
def perform_redaction(model_call_details: dict, result):
    ...
    model_call_details["messages"] = [
        {"role": "user", "content": "redacted-by-dheera_ai"}
    ]
    ...
    if result is not None:
        ...
        if (asyncio.iscoroutine(result) or 
            ...
            return {"text": "redacted-by-dheera_ai"}
        
        _result = copy.deepcopy(result)
        if isinstance(_result, dheera_ai.ModelResponse):
            if hasattr(_result, "choices") and _result.choices is not None:
                for choice in _result.choices:
                    _redact_choice_content(choice)
        elif isinstance(_result, dheera_ai.ResponsesAPIResponse):
        ...
        elif isinstance(_result, dheera_ai.EmbeddingResponse):
...
def should_redact_message_logging(model_call_details: dict) -> bool:
    ...
    metadata_field = get_metadata_variable_name_from_kwargs(dheera_ai_params)
    metadata = dheera_ai_params.get(metadata_field, {})
    ...
    possible_request_headers = [
        "dheera_ai-enable-message-redaction",  # old header. maintain backwards compatibility
        "x-dheera_ai-enable-message-redaction",  # new header
    ]
    ...
    if (
        dheera_ai.turn_off_message_logging is not True
    ...
    if request_headers and bool(
        request_headers.get("dheera_ai-disable-message-redaction", False)
    ):
        return False
...
def redact_message_input_output_from_logging(
    ...
    only redacts when dheera_ai.turn_off_message_logging == True
    """
...
def redact_user_api_key_info(metadata: dict) -> dict:
    ...
    ```
    """
    if dheera_ai.redact_user_api_key_info is not True:
        return metadata
