...
from dheera_ai.dheera_ai_core_utils.model_response_utils import (
    is_model_response_stream_empty,
)
from dheera_ai.dheera_ai_core_utils.redact_messages import DheeraAILoggingObject
from dheera_ai.dheera_ai_core_utils.thread_pool_executor import executor
from dheera_ai.types.llms.openai import ChatCompletionChunk
...
class CustomStreamWrapper:
    def __init__(
        ...
        self.logging_obj: DheeraAILoggingObject = logging_obj
        self.completion_stream = completion_stream
        self.sent_first_chunk = False
        self.sent_last_chunk = False

        dheera_ai_params: GenericDheeraAIParams = GenericDheeraAIParams(
            **self.logging_obj.model_call_details.get("dheera_ai_params", {})
        )
        self.merge_reasoning_content_in_choices: bool = (
            dheera_ai_params.merge_reasoning_content_in_choices or False
        )
        ...
        _model_info: Dict = dheera_ai_params.model_info or {}
        ...
        self._hidden_params = {
            ...
            "api_base": _api_base,
        }  # returned as x-dheera_ai-model-id response header in proxy
    ...
    def safety_checker(self) -> None:
        """
        Fixes - https://github.com/BerriAI/dheera_ai/issues/5158
        ...
        """
        if len(self.chunks) >= dheera_ai.REPEATED_STREAMING_CHUNK_LIMIT:
            # Get the last n chunks
            last_chunks = self.chunks[-dheera_ai.REPEATED_STREAMING_CHUNK_LIMIT :]
            ...
            if all(content == last_contents[0] for content in last_contents):
                ...
                ):  # ignore empty content - https://github.com/BerriAI/dheera_ai/issues/5158#issuecomment-2287156946
    ...
    def handle_baseten_chunk(self, chunk):
        try:
            ...
            verbose_logger.exception(
                "dheera_ai.CustomStreamWrapper.handle_baseten_chunk(): Exception occured - {}".format(
                    str(e)
                )
            )
            return ""
    ...
    def model_response_creator(
        ...
        ):  # maintain same 'created' across all chunks - https://github.com/BerriAI/dheera_ai/issues/11437
    ...
    def chunk_creator(self, chunk: Any):  # type: ignore  # noqa: PLR0915
        ...
        try:
            ...
            if (
                ...
                and self.custom_llm_provider in dheera_ai._custom_providers
            ):
                ...
                if anthropic_response_obj["usage"] is not None:
                    setattr(
                        model_response,
                        "usage",
                        dheera_ai.Usage(**anthropic_response_obj["usage"]),
                    )
                        ...
                        try:
                            ...
                            if "Part has no text." in str(e):
                                ...
                                try:
                                    ...
                                    raise e
                                _delta_obj = dheera_ai.utils.Delta(
                                    content=None,
                                    tool_calls=[
                                        {
                ...
                response_obj = cast(
                    Dict[str, Any],
                    dheera_ai.CodestralTextCompletionConfig()._chunk_parser(chunk),
                )
                completion_obj["content"] = response_obj["text"]
                print_verbose(f"completion obj content: {completion_obj['content']}")
                if response_obj["is_finished"]:
                ...
                if response_obj["usage"] is not None:
                    if isinstance(response_obj["usage"], dict):
                        ...
                        setattr(
                            model_response,
                            "usage",
                            dheera_ai.Usage(**response_obj["usage"].model_dump()),
                        )
                ...
                if original_chunk.choices and len(original_chunk.choices) > 0:
                    ...
                    if delta is not None and (
                        ...
                        try:
                            ...
                            verbose_logger.exception(
                                "dheera_ai.CustomStreamWrapper.chunk_creator(): Exception occured - {}".format(
                                    str(e)
                                )
                            )
                            model_response.choices[0].delta = Delta()
    ...
    def set_logging_event_loop(self, loop):
        """
        import dheera_ai, asyncio

        loop = asyncio.get_event_loop() # 👈 gets the current event loop

        response = dheera_ai.completion(.., stream=True)
    ...
    def run_success_logging_and_cache_storage(self, processed_chunk, cache_hit: bool):
        ...
        """
        if dheera_ai.disable_streaming_logging is True:
            """
            [NOT RECOMMENDED]
            Set this via `dheera_ai.disable_streaming_logging = True`.
        ...
        try:
            ...
            while True:
                ...
                if chunk is not None and chunk != b"":
                    ...
                    ## LOGGING
                    if not dheera_ai.disable_streaming_logging:
                        executor.submit(
    ...
    def fetch_sync_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            ...
            self.completion_stream = self.make_call(client=dheera_ai.module_level_client)
    ...
    async def fetch_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            ...
            self.completion_stream = await self.make_call(
                client=dheera_ai.module_level_aclient
            )
    ...
    async def __anext__(self):  # noqa: PLR0915
        ...
        try:
            ...
            traceback_exception += "\nDheeraAI Default Request Timeout - {}".format(
                dheera_ai.request_timeout
            )
