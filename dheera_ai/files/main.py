...
from dheera_ai.llms.anthropic.files.handler import AnthropicFilesHandler
from dheera_ai.llms.azure.files.handler import AzureOpenAIFilesAPI
from dheera_ai.llms.bedrock.files.handler import BedrockFilesHandler
...
from dheera_ai.llms.openai.openai import FileDeleted, FileObject, OpenAIFilesAPI
from dheera_ai.llms.vertex_ai.files.handler import VertexAIFilesHandler
...
async def acreate_file(
    ...
    **kwargs,
    ...
    DheeraAI Equivalent of POST: POST https://api.openai.com/v1/files
    """
    try:
...
def create_file(
    ...
    **kwargs,
    ...
    try:
        ...
        dheera_ai_params_dict = dict(**kwargs)
        logging_obj = cast(
            Optional[DheeraAILoggingObj], kwargs.get("dheera_ai_logging_obj")
        )
        ...
        if provider_config is not None:
            ...
            raise dheera_ai.exceptions.BadRequestError(
                message="DheeraAI doesn't support {} for 'create_file'. Only ['openai', 'azure', 'vertex_ai'] are supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                ...
                response=httpx.Response(
                    ...
                    request=httpx.Request(method="create_file", url="https://github.com/BerriAI/dheera_ai"),  # type: ignore
                ),
            )
        return response
    except Exception as e:
...
async def afile_retrieve(
    ...
    **kwargs,
    ...
    DheeraAI Equivalent of GET https://api.openai.com/v1/files
    """
    try:
        loop = asyncio.get_event_loop()
    ...
    **kwargs,
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            raise dheera_ai.exceptions.BadRequestError(
                message="DheeraAI doesn't support {} for 'file_retrieve'. Only 'openai' and 'azure' are supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
...
async def afile_delete(
    ...
    **kwargs,
    ...
    DheeraAI Equivalent of DELETE https://api.openai.com/v1/files
    """
    try:
        loop = asyncio.get_event_loop()
    ...
    **kwargs,
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            raise dheera_ai.exceptions.BadRequestError(
                message="DheeraAI doesn't support {} for 'delete_batch'. Only 'openai' is supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
    ...
    **kwargs,
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            raise dheera_ai.exceptions.BadRequestError(
                message="DheeraAI doesn't support {} for 'file_list'. Only 'openai' and 'azure' are supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="file_list", url="https://github.com/BerriAI/dheera_ai"),  # type: ignore
                ),
            )
        return response
    except Exception as e:
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            response = bedrock_files_instance.file_content(
                ...
                file_content_request=_file_content_request,
                api_base=optional_params.api_base,
                optional_params=dheera_ai_params_dict,
                timeout=timeout,
                max_retries=optional_params.max_retries,
            )
        else:
            raise dheera_ai.exceptions.BadRequestError(
                message="DheeraAI doesn't support {} for 'custom_llm_provider'. Supported providers are 'openai', 'azure', 'vertex_ai', 'bedrock'.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
