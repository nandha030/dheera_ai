...
from dheera_ai.llms.azure.fine_tuning.handler import AzureOpenAIFineTuningAPI
from dheera_ai.llms.openai.fine_tuning.handler import OpenAIFineTuningAPI
from dheera_ai.llms.vertex_ai.fine_tuning.handler import VertexFineTuningAPI
from dheera_ai.secret_managers.main import get_secret_str
from dheera_ai.types.llms.openai import FineTuningJobCreate, Hyperparameters
from dheera_ai.types.router import *
from dheera_ai.types.utils import DheeraAIFineTuningJob
from dheera_ai.utils import client, supports_httpx_timeout
...
def create_fine_tuning_job(
    ...
    extra_body: Optional[Dict[str, str]] = None,
    **kwargs,
) -> Union[DheeraAIFineTuningJob, Coroutine[Any, Any, DheeraAIFineTuningJob]]:
    """
    ...
    try:
        ...
        if custom_llm_provider == "openai":
            ...
            response = openai_fine_tuning_apis_instance.create_fine_tuning_job(
                ...
                client=kwargs.get(
                    "client", None
                ),  # note, when we add this to `GenericDheeraAIParams` it impacts a lot of other tests + linting
            )
        # Azure OpenAI
    ...
    **kwargs,
    ...
    try:
        ...
        # Azure OpenAI
            ...
            api_base = optional_params.api_base or dheera_ai.api_base or get_secret("AZURE_API_BASE")  # type: ignore
    ...
    **kwargs,
    ...
    try:
        ...
        # Azure OpenAI
            ...
            raise dheera_ai.exceptions.BadRequestError(
                message="DheeraAI doesn't support {} for 'retrieve_fine_tuning_job'. Only 'openai' and 'azure' are supported.".format(
                    custom_llm_provider
                ),
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="retrieve_fine_tuning_job", url="https://github.com/BerriAI/dheera_ai"),  # type: ignore
                ),
            )
        return response
    except Exception as e:
