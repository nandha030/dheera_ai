...
from dheera_ai.dheera_ai_core_utils.json_validation_rule import normalize_tool_schema
...
class GoogleGenAIAdapter:
    """Adapter for transforming Google GenAI generate_content requests to/from dheera_ai.completion format"""
    ...
    def translate_generate_content_to_completion(
        ...
        **kwargs,
        ...
        """
        Transform generate_content request to dheera_ai completion format
        ...
        #########################################################
        # forward any dheera_ai specific params
        #########################################################
        completion_request_dict = dict(completion_request)
        if dheera_ai_params:
            completion_request_dict = self._add_generic_dheera_ai_params_to_request(
    ...
    def _add_generic_dheera_ai_params_to_request(
        self,
        completion_request_dict: Dict[str, Any],
        dheera_ai_params: Optional[GenericDheeraAIParams] = None,
    ) -> dict:
        """Add generic dheera_ai params to request. e.g add api_base, api_key, api_version, etc.
        ...
        Returns:
            Dict[str, Any]
        """
        allowed_fields = GenericDheeraAIParams.model_fields.keys()
        if dheera_ai_params:
            dheera_ai_dict = dheera_ai_params.model_dump(exclude_none=True)
            for key, value in dheera_ai_dict.items():
    ...
    def translate_completion_to_generate_content(
        ...
        """
        Transform dheera_ai completion response to Google GenAI generate_content format

        Args:
            response: ModelResponse from dheera_ai.completion
    ...
    def translate_streaming_completion_to_generate_content(
        ...
        """
        Transform streaming dheera_ai completion chunk to Google GenAI generate_content format

        Args:
            response: Streaming ModelResponse chunk from dheera_ai.completion
