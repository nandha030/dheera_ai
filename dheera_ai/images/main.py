...
if TYPE_CHECKING:
    from dheera_ai.images.utils import ImageEditRequestUtils

import httpx

import dheera_ai
from dheera_ai.utils import exception_type, get_dheera_ai_params
# client is imported from dheera_ai as it's a decorator
from dheera_ai import client
from dheera_ai.constants import DEFAULT_IMAGE_ENDPOINT_MODEL
...
from dheera_ai.dheera_ai_core_utils.mock_functions import mock_image_generation
from dheera_ai.llms.base_llm import BaseImageEditConfig, BaseImageGenerationConfig
...
from dheera_ai.types.llms.openai import ImageGenerationRequestQuality
from dheera_ai.types.router import GenericDheeraAIParams
from dheera_ai.types.utils import (
    DHEERA_AI_IMAGE_VARIATION_PROVIDERS,
    LlmProviders,
...
def image_generation(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    try:
        ...
        model_response: ImageResponse = dheera_ai.utils.ImageResponse()
        ...
        dheera_ai_params = all_dheera_ai_params
        ...
        logging: Logging = dheera_ai_logging_obj
        ...
        #########################################################
            ...
            dheera_ai.LlmProviders.RECRAFT,
            dheera_ai.LlmProviders.AIML,
            dheera_ai.LlmProviders.GEMINI,
            dheera_ai.LlmProviders.FAL_AI,
            dheera_ai.LlmProviders.STABILITY,
            dheera_ai.LlmProviders.RUNWAYML,
            dheera_ai.LlmProviders.VERTEX_AI,
        ):
            ...
            # Resolve api_base from dheera_ai.api_base if not explicitly provided
            _api_base = api_base or dheera_ai.api_base
            dheera_ai_params_dict["api_base"] = _api_base
            ...
            or custom_llm_provider == LlmProviders.DHEERA_AI_PROXY.value
...
def image_variation(
    ...
    **kwargs,
    ...
    client = kwargs.get("client", None)
    # get logging object
    dheera_ai_logging_obj = cast(DheeraAILoggingObj, kwargs.get("dheera_ai_logging_obj"))

    # get the dheera_ai params
    dheera_ai_params = get_dheera_ai_params(**kwargs)
    # get the custom llm provider
    model, custom_llm_provider, dynamic_api_key, api_base = get_llm_provider(
        model=model,
        custom_llm_provider=dheera_ai_params.get("custom_llm_provider", None),
        api_base=dheera_ai_params.get("api_base", None),
        api_key=dheera_ai_params.get("api_key", None),
    )

    # route to the correct provider w/ the params
    try:
        llm_provider = LlmProviders(custom_llm_provider)
        image_variation_provider = DHEERA_AI_IMAGE_VARIATION_PROVIDERS(llm_provider)
    except ValueError:
        raise ValueError(
            f"Invalid image variation provider: {custom_llm_provider}. Supported providers are: {DHEERA_AI_IMAGE_VARIATION_PROVIDERS}"
        )
    ...
    if provider_config is None:
        raise ValueError(
            f"image variation provider has no known model info config - required for getting api keys, etc.: {custom_llm_provider}. Supported providers are: {DHEERA_AI_IMAGE_VARIATION_PROVIDERS}"
        )

    api_key = provider_config.get_api_key(dheera_ai_params.get("api_key", None))
    api_base = provider_config.get_api_base(dheera_ai_params.get("api_base", None))

    if image_variation_provider == DHEERA_AI_IMAGE_VARIATION_PROVIDERS.OPENAI:
        if api_key is None:
        ...
        response = openai_image_variations.image_variations(
            ...
            model=model,
            image=image,
            timeout=dheera_ai_params.get("timeout", None),
            custom_llm_provider=custom_llm_provider,
            logging_obj=dheera_ai_logging_obj,
            optional_params={},
            dheera_ai_params=dheera_ai_params,
        )
    elif image_variation_provider == DHEERA_AI_IMAGE_VARIATION_PROVIDERS.TOPAZ:
        if api_key is None:
        ...
        response = base_llm_aiohttp_handler.image_variations(
            ...
            model=model,
            image=image,
            timeout=dheera_ai_params.get("timeout", None) or DEFAULT_REQUEST_TIMEOUT,
            custom_llm_provider=custom_llm_provider,
            logging_obj=dheera_ai_logging_obj,
            optional_params={},
            dheera_ai_params=dheera_ai_params,
...
def image_edit(
    ...
    **kwargs,
    ...
    try:
        ...
        if custom_llm_provider in dheera_ai._custom_providers:
            custom_handler: Optional[CustomLLM] = None
            for item in dheera_ai.custom_provider_map:
                if item["provider"] == custom_llm_provider:
                    custom_handler = item["custom_handler"]
