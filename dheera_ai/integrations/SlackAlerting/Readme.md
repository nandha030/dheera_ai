# Slack Alerting on Dheera AI Gateway 

This folder contains the Slack Alerting integration for Dheera AI Gateway. 

## Folder Structure 

- `slack_alerting.py`: This is the main file that handles sending different types of alerts
- `batching_handler.py`: Handles Batching + sending Httpx Post requests to slack. Slack alerts are sent every 10s or when events are greater than X events. Done to ensure dheera_ai has good performance under high traffic
...
## Further Reading
- [Doc setting up Alerting on Dheera AI Proxy (Gateway)](https://docs.dheera_ai.ai/docs/proxy/alerting)