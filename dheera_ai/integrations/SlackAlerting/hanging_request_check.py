...
class AlertingHangingRequestCheck:
    ...
    async def add_request_to_hanging_request_check(
        ...
        request_metadata = get_dheera_ai_metadata_from_kwargs(kwargs=request_data)
        ...
        if request_data.get("deployment", None) is not None and isinstance(
            ...
            api_base = dheera_ai.get_api_base(
                model=model,
                optional_params=request_data["deployment"].get("dheera_ai_params", {}),
            )

        hanging_request_data = HangingRequestData(
            request_id=request_data.get("dheera_ai_call_id", ""),
            model=model,
            api_base=api_base,
    ...
    async def send_hanging_request_alert(
        ...
        """
        from dheera_ai.integrations.SlackAlerting.slack_alerting import AlertType
