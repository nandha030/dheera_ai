...
from dheera_ai.constants import HOURS_IN_A_DAY
from dheera_ai.integrations.custom_batch_logger import CustomBatchLogger
from dheera_ai.integrations.SlackAlerting.budget_alert_types import get_budget_alert_type
from dheera_ai.integrations.SlackAlerting.hanging_request_check import (
...
from dheera_ai.types.integrations.slack_alerting import *
...
class SlackAlerting(CustomBatchLogger):
    ...
    def _response_taking_too_long_callback_helper(
        ...
        try:
            ...
            api_base = dheera_ai.get_api_base(model=model, optional_params=dheera_ai_params)
    ...
    async def response_taking_too_long_callback(
        ...
        if dheera_ai.turn_off_message_logging or dheera_ai.redact_messages_in_exceptions:
            messages = "Message not logged. dheera_ai.redact_messages_in_exceptions=True"
        request_info = f"\nRequest Model: `{model}`\nAPI Base: `{api_base}`\nMessages: `{messages}`"
        ...
        if time_difference_float > self.alerting_threshold:
            ...
            if (
                kwargs is not None
                and "dheera_ai_params" in kwargs
                and "metadata" in kwargs["dheera_ai_params"]
            ):
                _metadata: dict = kwargs["dheera_ai_params"]["metadata"]
    ...
    async def send_daily_reports(self, router) -> bool:  # noqa: PLR0915
        ...
        # format alert -> return the dheera_ai model name + api base
        ...
        for i in range(len(top_5_failed)):
            ...
            if isinstance(_deployment, dict):
                deployment_name = _deployment["dheera_ai_params"].get("model", "")
            else:
                return False

            api_base = dheera_ai.get_api_base(
                model=deployment_name,
                optional_params=(
                    _deployment["dheera_ai_params"] if _deployment is not None else {}
                ),
            )
            if api_base is None:
                api_base = ""
    ...
    async def budget_alerts(
        ...
        """
        ## PREVENTITIVE ALERTING ## - https://github.com/BerriAI/dheera_ai/issues/2727
...
"""
    ...
    async def region_outage_alerts(
        ...
        ### GET PROVIDER ###
        provider = deployment.dheera_ai_params.custom_llm_provider
        if provider is None:
            model, provider, _, _ = dheera_ai.get_llm_provider(model=model)

        ### GET REGION ###
        region_name = deployment.dheera_ai_params.region_name
        if region_name is None:
            region_name = dheera_ai.utils._get_model_region(
                custom_llm_provider=provider, dheera_ai_params=deployment.dheera_ai_params
            )
        ...
        try:
            ...
            if provider is None:
                try:
                    ...
                    provider = ""
            api_base = dheera_ai.get_api_base(
                model=model, optional_params=deployment.dheera_ai_params
            )
    ...
    async def model_added_alert(
        self, model_name: str, dheera_ai_model_name: str, passed_model_info: Any
    ):
        base_model_from_user = getattr(passed_model_info, "base_model", None)
        model_info = {}
        base_model = ""
        if base_model_from_user is not None:
            model_info = dheera_ai.model_cost.get(base_model_from_user, {})
            base_model = f"Base Model: `{base_model_from_user}`\n"
        else:
            model_info = dheera_ai.model_cost.get(dheera_ai_model_name, {})
        model_info_str = ""
...
"""
    ...
    async def send_key_created_or_user_invited_email(
        ...
        try:
            from dheera_ai.proxy.utils import send_email
            ...
            if email_logo_url is None:
                email_logo_url = DHEERA_AI_LOGO_URL
            if email_support_contact is None:
                email_support_contact = DHEERA_AI_SUPPORT_CONTACT
            ...
            email_html_content = "Alert from DheeraAI Server"
    ...
    async def send_email_alert_using_smtp(
        ...
        if webhook_event.event == "budget_crossed":
            ...
            <img src="{email_logo_url}" alt="DheeraAI Logo" width="150" height="50" />
        ...
        if webhook_event.event_group == Litellm_EntityType.TEAM:
            from dheera_ai.integrations.email_alerting import send_team_budget_alert
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            if "daily_reports" in self.alert_types:
                ...
                if isinstance(response_obj, dheera_ai.ModelResponse) and (
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        _dheera_ai_params = kwargs.get("dheera_ai_params", {})
        _model_info = _dheera_ai_params.get("model_info", {}) or {}
    ...
    async def send_fallback_stats_from_prometheus(self):
        ...
        """
        try:
            from dheera_ai.integrations.prometheus_helpers.prometheus_api import (
    ...
    async def _request_is_completed(self, request_data: Optional[dict]) -> bool:
        ...
        if (
            request_data.get("dheera_ai_status", "") != "success"
            and request_data.get("dheera_ai_status", "") != "fail"
        ):
            ## CHECK IF CACHE IS UPDATED
            dheera_ai_call_id = request_data.get("dheera_ai_call_id", "")
            status: Optional[str] = await self.internal_usage_cache.async_get_cache(
                key="request_status:{}".format(dheera_ai_call_id), local_only=True
            )
