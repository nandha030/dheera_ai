...
import dheera_ai
from dheera_ai.proxy._types import AlertType
...
async def _add_langfuse_trace_id_to_alert(
    ...
    -> dheera_ai_call_id
    """
    if "langfuse" not in dheera_ai.logging_callback_manager._get_all_callbacks():
        return None
    #########################################################
    ...
    if (
        ...
        and request_data.get("dheera_ai_logging_obj", None) is not None
    ):
        trace_id: Optional[str] = None
        dheera_ai_logging_obj: Logging = request_data["dheera_ai_logging_obj"]

        for _ in range(3):
            trace_id = dheera_ai_logging_obj._get_trace_id(service_name="langfuse")
            if trace_id is not None:
                break
            await asyncio.sleep(3)  # wait 3s before retrying for trace id
        #########################################################
        langfuse_object = dheera_ai_logging_obj._get_callback_object(
