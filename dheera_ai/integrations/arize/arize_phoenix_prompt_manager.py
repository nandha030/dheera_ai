"""
Arize Phoenix prompt manager that integrates with DheeraAI's prompt management system.
...
class ArizePhoenixPromptManager(CustomPromptManagement):
    ...
    dheera_ai completion() function by implementing the PromptManagementBase interface.
    ...
    def pre_call_hook(
        ...
        **kwargs,
        ...
        """
        if not prompt_id:
            return messages, dheera_ai_params
        ...
        try:
            ...
            # Update dheera_ai_params with prompt metadata
            ...
            if prompt_metadata.get("model") and not self.ignore_prompt_manager_model:
                dheera_ai_params["model"] = prompt_metadata["model"]
            ...
            if not self.ignore_prompt_manager_optional_params:
                for param in [
                    ...
                    if param in prompt_metadata:
                        dheera_ai_params[param] = prompt_metadata[param]

            return final_messages, dheera_ai_params

        except Exception as e:
            # Log error but don't fail the call
            import dheera_ai

            dheera_ai._logging.verbose_proxy_logger.error(
