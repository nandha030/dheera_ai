...
class BraintrustLogger(CustomLogger):
    ...
    async def create_default_project_and_experiment(self):
        project = await self.global_braintrust_http_handler.post(
            f"{self.api_base}/project", headers=self.headers, json={"name": "dheera_ai"}
        )
    ...
    def log_success_event(  # noqa: PLR0915
        ...
        try:
            ...
            dynamic_metadata = dheera_ai_params.get("metadata", {}) or {}
            ...
            if usage_obj and isinstance(usage_obj, dheera_ai.Usage):
                dheera_ai.utils.get_logging_id(start_time, response_obj)
                metrics = {
