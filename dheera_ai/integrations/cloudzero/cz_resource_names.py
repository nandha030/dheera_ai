...
"""CloudZero Resource Names (CZRN) generation and validation for DheeraAI resources."""
...
class CZRNGenerator:
    """Generate CloudZero Resource Names (CZRNs) for DheeraAI resources."""
    ...
    def create_from_dheera_ai_data(self, row: dict[str, Any]) -> str:
        """Create a CZRN from DheeraAI daily spend data.
        
        CZRN format: czrn:<service-type>:<provider>:<region>:<owner-account-id>:<resource-type>:<cloud-local-id>
        
        For DheeraAI resources, we map:
        - service-type: 'dheera_ai' (the service managing the LLM calls)
        - provider: The custom_llm_provider (e.g., 'openai', 'anthropic', 'azure')
        - region: 'cross-region' (DheeraAI operates across regions)
    ...
    def _normalize_provider(self, provider: str) -> str:
        ...
        provider_map = {
            dheera_ai.LlmProviders.AZURE.value: 'azure',
            dheera_ai.LlmProviders.AZURE_AI.value: 'azure',
            dheera_ai.LlmProviders.ANTHROPIC.value: 'anthropic',
            dheera_ai.LlmProviders.BEDROCK.value: 'aws',
            dheera_ai.LlmProviders.VERTEX_AI.value: 'gcp',
            dheera_ai.LlmProviders.GEMINI.value: 'google',
            dheera_ai.LlmProviders.COHERE.value: 'cohere',
            dheera_ai.LlmProviders.HUGGINGFACE.value: 'huggingface',
            dheera_ai.LlmProviders.REPLICATE.value: 'replicate',
            dheera_ai.LlmProviders.TOGETHER_AI.value: 'together-ai',
        }

        normalized = provider.lower().replace('_', '-')

        # use dheera_ai custom llm provider if not in provider_map
