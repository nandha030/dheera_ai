...
# CHANGELOG: 2025-01-19 - Initial database module for DheeraAI data extraction (erik.peterson)

"""Database connection and data extraction for DheeraAI."""
...
class DheeraAIDatabase:
    """Handle DheeraAI PostgreSQL database connections and queries."""
    ...
    async def get_usage_data(
        ...
        """Retrieve usage data from DheeraAI daily user spend table."""
        ...
        SELECT
            ...
            tt.team_alias
        FROM "DheeraAI_DailyUserSpend" dus
        LEFT JOIN "DheeraAI_VerificationToken" vt ON dus.api_key = vt.token
        LEFT JOIN "DheeraAI_TeamTable" tt ON vt.team_id = tt.team_id
        {where_clause}
    ...
    async def get_table_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            user_count = await self._get_table_row_count("DheeraAI_DailyUserSpend")

            # Get column structure from user spend table
            query = """
            SELECT column_name, data_type, is_nullable
            FROM information_schema.columns
            WHERE table_name = 'DheeraAI_DailyUserSpend'
            ...
            return {
                ...
                "table_name": "DheeraAI_DailyUserSpend",
            }
        except Exception as e:
    ...
    async def discover_all_tables(self) -> Dict[str, Any]:
        """Discover all tables in the DheeraAI database and their schemas."""
        client = self._ensure_prisma_client()

        try:
            # Get all DheeraAI tables
            dheera_ai_tables_query = """
            SELECT table_name 
            FROM information_schema.tables 
            WHERE table_schema = 'public' 
            AND table_name LIKE 'DheeraAI_%'
            ORDER BY table_name;
            """
            tables_response = await client.db.query_raw(dheera_ai_tables_query)
