...
"""Transform DheeraAI data to CloudZero AnyCost CBF format."""
...
class CBFTransformer:
    """Transform DheeraAI usage data to CloudZero Billing Format (CBF)."""
    ...
    def transform(self, data: pl.DataFrame) -> pl.DataFrame:
        """Transform DheeraAI data to CBF format, dropping records with zero successful_requests or invalid CZRNs."""
        if data.is_empty():
            return pl.DataFrame()
    ...
    def _create_cbf_record(self, row: dict[str, Any]) -> CBFRecord:
        """Create a single CBF record from DheeraAI daily spend row."""
        ...
        resource_id = self.czrn_generator.create_from_dheera_ai_data(row)
        ...
        cbf_record = {
            ...
            'resource/service': service_type,  # Maps to CZRN service-type (dheera_ai)
