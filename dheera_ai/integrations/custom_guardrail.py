...
class CustomGuardrail(CustomLogger):
    ...
    def get_disable_global_guardrail(self, data: dict) -> Optional[bool]:
        ...
        metadata = data.get("dheera_ai_metadata") or data.get("metadata", {})
    ...
    async def async_pre_call_deployment_hook(
        ...
        dheera_ai_guardrails = kwargs.get("guardrails")
        if dheera_ai_guardrails is None or not isinstance(dheera_ai_guardrails, list):
            return kwargs
    ...
    async def async_post_call_success_deployment_hook(
        ...
        # should run guardrail
        dheera_ai_guardrails = request_data.get("guardrails")
        if dheera_ai_guardrails is None or not isinstance(dheera_ai_guardrails, list):
            return response
        ...
        if self.default_on is True and disable_global_guardrail is not True:
            if self._event_hook_is_event_type(event_type):
                if isinstance(self.event_hook, Mode):
                    try:
                        from dheera_ai_enterprise.integrations.custom_guardrail import (
                            EnterpriseCustomGuardrailHelper,
                        )
                    except ImportError:
                        raise ImportError(
                            "Setting tag-based guardrails is only available in dheera_ai-enterprise. You must be a premium user to use this feature."
                        )
    ...
    def get_guardrail_dynamic_request_body_params(self, request_data: dict) -> dict:
        ...
        Args:
            request_data: The original `request_data` passed to DheeraAI Proxy
        """
    ...
    def add_standard_logging_guardrail_information_to_request_data(
        ...
        from dheera_ai.types.utils import GuardrailMode
        ...
        elif "dheera_ai_metadata" in request_data:
            _append_guardrail_info(request_data["dheera_ai_metadata"])
        else:
            verbose_logger.warning(
    ...
    def update_in_memory_dheera_ai_params(self, dheera_ai_params: LitellmParams) -> None:
        """
        Update the guardrails dheera_ai params in memory
        """
        for key, value in vars(dheera_ai_params).items():
    ...
    def get_guardrails_messages_for_call_type(
        ...
        #########################################################
        # /responses
        # User/System messages are stored in the "input" key, use dheera_ai transformation to get the messages
        #########################################################
        if (
