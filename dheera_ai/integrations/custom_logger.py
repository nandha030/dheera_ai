...
from dheera_ai.constants import DEFAULT_MAX_RECURSE_DEPTH_SENSITIVE_DATA_MASKER
from dheera_ai.types.integrations.argilla import ArgillaItem
from dheera_ai.types.llms.openai import AllMessageValues, ChatCompletionRequest
...
if TYPE_CHECKING:
    ...
    from dheera_ai.types.router import PreRoutingHookResponse
...
class CustomLogger:  # https://docs.dheera_ai.ai/docs/observability/custom_callback#callback-class
    ...
    def get_callback_env_vars(callback_name: Optional[str] = None) -> List[str]:
        ...
        try:
            from dheera_ai.proxy._types import AllCallbacks
        except Exception:
            return []
        ...
        params = getattr(callback_info, "dheera_ai_callback_params", None)
        if not params:
            return []
    ...
    #### ADAPTERS #### Allow calling 100+ LLMs in custom format - https://github.com/BerriAI/dheera_ai/pulls

    def translate_completion_input_params(
        self, kwargs
    ) -> Optional[ChatCompletionRequest]:
        """
        Translates the input params, from the provider's native format to the dheera_ai.completion() format.
        """
        pass
    ...
    ]:  # raise exception if invalid, return a str for the user to receive - if rejected, or return a modified dictionary for passing into dheera_ai
        pass
    ...
    def _truncate_text(self, text: str, max_length: int) -> str:
        ...
        return (
            text[:max_length]
            + "...truncated by dheera_ai, this logger does not support large content"
    ...
    def _select_metadata_field(
        ...
        1. If `dheera_ai_metadata` is in the request kwargs, use it
        2. Otherwise, use `metadata`
        """
        from dheera_ai.constants import DHEERA_AI_METADATA_FIELD, OLD_DHEERA_AI_METADATA_FIELD

        if request_kwargs is None:
            return None
        if DHEERA_AI_METADATA_FIELD in request_kwargs:
            return DHEERA_AI_METADATA_FIELD
        return OLD_DHEERA_AI_METADATA_FIELD
