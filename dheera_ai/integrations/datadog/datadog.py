...
`async_log_success_event` - used by dheera_ai proxy to send logs to datadog
`log_success_event` - sync version of logging to DataDog, only used on dheera_ai Python SDK, if user opts in to using sync functions
...
from dheera_ai.types.integrations.datadog import *
from dheera_ai.types.services import ServiceLoggerPayload, ServiceTypes
...
class DataDogLogger(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        `DHEERA_AI_DD_AGENT_HOST` - hostname or IP of DataDog agent, example = `"localhost"`
        `DHEERA_AI_DD_AGENT_PORT` - port of DataDog agent (default: 10518 for logs)
        ...
        try:
            ...
            #########################################################
            # Handle datadog_params set as dheera_ai.datadog_params
            #########################################################
            ...
            # Use DHEERA_AI_DD_AGENT_HOST to avoid conflicts with ddtrace's DD_AGENT_HOST
            dd_agent_host = os.getenv("DHEERA_AI_DD_AGENT_HOST")
            if dd_agent_host:
    ...
    def _get_datadog_params(self) -> Dict:
        """
        Get the datadog_params from dheera_ai.datadog_params

        These are params specific to initializing the DataDogLogger e.g. turn_off_message_logging
        """
        dict_datadog_params: Dict = {}
        if dheera_ai.datadog_params is not None:
            if isinstance(dheera_ai.datadog_params, DatadogInitParams):
                dict_datadog_params = dheera_ai.datadog_params.model_dump()
            elif isinstance(dheera_ai.datadog_params, Dict):
                # only allow params that are of DatadogInitParams
                dict_datadog_params = DatadogInitParams(**dheera_ai.datadog_params).model_dump()
    ...
    def _configure_dd_agent(self, dd_agent_host: str) -> None:
        ...
        """
        dd_agent_port = os.getenv("DHEERA_AI_DD_AGENT_PORT", "10518")  # default port for logs
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        """
        try:
            if dheera_ai.datadog_use_v1 is True:
    ...
    def _create_v0_logging_payload(
        ...
        (Not Recommended) If you want this to get logged set `dheera_ai.datadog_use_v1 = True`
        """
        ...
        call_type = kwargs.get("call_type", "dheera_ai.completion")
    ...
    def _get_datadog_tags(
        ...
        base_tags = {
            ...
            "service": os.getenv("DD_SERVICE", "dheera_ai"),
    ...
    def _get_datadog_source():
        return os.getenv("DD_SOURCE", "dheera_ai")

    @staticmethod
    def _get_datadog_service():
        return os.getenv("DD_SERVICE", "dheera_ai-server")
