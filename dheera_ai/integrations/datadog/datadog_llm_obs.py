...
class DataDogLLMObsLogger(DataDogLogger, CustomBatchLogger):
    def __init__(self, **kwargs):
        try:
            ...
            #########################################################
            # Handle datadog_llm_observability_params set as dheera_ai.datadog_llm_observability_params
            #########################################################
    ...
    def _get_datadog_llm_obs_params(self) -> Dict:
        """
        Get the datadog_llm_observability_params from dheera_ai.datadog_llm_observability_params
        ...
        if dheera_ai.datadog_llm_observability_params is not None:
            if isinstance(
                dheera_ai.datadog_llm_observability_params, DatadogLLMObsInitParams
            ):
                dict_datadog_llm_obs_params = (
                    dheera_ai.datadog_llm_observability_params.model_dump()
                )
            elif isinstance(dheera_ai.datadog_llm_observability_params, Dict):
                # only allow params that are of DatadogLLMObsInitParams
                dict_datadog_llm_obs_params = DatadogLLMObsInitParams(
                    **dheera_ai.datadog_llm_observability_params
                ).model_dump()
    ...
    def create_llm_obs_payload(
        ...
        payload: LLMObsPayload = LLMObsPayload(
            ...
            name=metadata.get("name", "dheera_ai_llm_call"),
            meta=meta,
    ...
    def _get_dd_llm_obs_payload_metadata(
        ...
        """
        Fields to track in DD LLM Observability metadata from dheera_ai standard logging payload
        """
    ...
    def _get_latency_metrics(
        ...
        # DheeraAI overhead time
        hidden_params = standard_logging_payload.get("hidden_params", {})
        dheera_ai_overhead_ms = hidden_params.get("dheera_ai_overhead_time_ms")
        if dheera_ai_overhead_ms is not None:
            latency_metrics["dheera_ai_overhead_time_ms"] = dheera_ai_overhead_ms
