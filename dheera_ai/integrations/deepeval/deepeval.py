...
from dheera_ai.integrations.deepeval.api import Api, Endpoints, HttpMethods
from dheera_ai.integrations.deepeval.types import (
    BaseApiSpan,
    SpanApiType,
    TraceApi,
    TraceSpanApiStatus,
)
from dheera_ai.integrations.deepeval.utils import (
...
class DeepEvalLogger(CustomLogger):
    """Logs dheera_ai traces to DeepEval's platform."""

    def __init__(self, *args, **kwargs):
        api_key = os.getenv("CONFIDENT_API_KEY")
        self.dheera_ai_environment = os.getenv("LITELM_ENVIRONMENT", "development")
        validate_environment(self.dheera_ai_environment)
        if not api_key:
            raise ValueError(
    ...
    def _prepare_trace_api(
        ...
        trace_api = self._create_trace_api(
            ...
            start_time=_start_time,
            end_time=_end_time,
            dheera_ai_environment=self.dheera_ai_environment,
        )
    ...
    def _create_base_api_span(
        ...
        return BaseApiSpan(
            ...
            name=(
                "dheera_ai_success_callback" if is_success else "dheera_ai_failure_callback"
            ),
            status=(
    ...
    def _create_trace_api(
        ...
        return TraceApi(
            ...
            startTime=str(start_time),
            endTime=str(end_time),
            environment=dheera_ai_environment,
        )
