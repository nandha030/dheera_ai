# What is this?
## On Success events log cost to Lago - https://github.com/BerriAI/dheera_ai/issues/3639
...
class LagoLogger(CustomLogger):
    ...
    def _common_logic(self, kwargs: dict, response_obj) -> dict:
        response_obj.get("id", kwargs.get("dheera_ai_call_id"))
        ...
        if (
            isinstance(response_obj, dheera_ai.ModelResponse)
        ...
        user_id = dheera_ai_params["metadata"].get("user_api_key_user_id", None)
        team_id = dheera_ai_params["metadata"].get("user_api_key_team_id", None)
        dheera_ai_params["metadata"].get("user_api_key_org_id", None)
