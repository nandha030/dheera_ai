...
from dheera_ai.constants import MAX_LANGFUSE_INITIALIZED_CLIENTS
from dheera_ai.dheera_ai_core_utils.core_helpers import safe_deep_copy
from dheera_ai.dheera_ai_core_utils.redact_messages import redact_user_api_key_info
from dheera_ai.llms.custom_httpx.http_handler import _get_httpx_client
from dheera_ai.secret_managers.main import str_to_bool
from dheera_ai.types.integrations.langfuse import *
from dheera_ai.types.llms.openai import HttpxBinaryResponseContent, ResponsesAPIResponse
...
class LangFuseLogger:
    ...
    def __init__(
        ...
        if Version(self.langfuse_sdk_version) >= Version("2.6.0"):
            parameters["sdk_integration"] = "dheera_ai"
    ...
    def safe_init_langfuse_client(self, parameters: dict) -> Langfuse:
        ...
        if dheera_ai.initialized_langfuse_clients >= MAX_LANGFUSE_INITIALIZED_CLIENTS:
            raise Exception(
                f"Max langfuse clients reached: {dheera_ai.initialized_langfuse_clients} is greater than {MAX_LANGFUSE_INITIALIZED_CLIENTS}"
            )
        langfuse_client = Langfuse(**parameters)
        dheera_ai.initialized_langfuse_clients += 1
        verbose_logger.debug(
            f"Created langfuse client number {dheera_ai.initialized_langfuse_clients}"
        )
        return langfuse_client

    @staticmethod
    def add_metadata_from_header(dheera_ai_params: dict, metadata: dict) -> dict:
        """
        Adds metadata from proxy request headers to Langfuse logging if keys start with "langfuse_"
        and overwrites dheera_ai_params.metadata if already included.
        ...
        if dheera_ai_params.get("proxy_server_request") is None:
            return metadata
        ...
        proxy_headers = (
            dheera_ai_params.get("proxy_server_request", {}).get("headers", {}) or {}
        )
    ...
    def log_event_on_langfuse(
        ...
        try:
            ...
            metadata = (
                dheera_ai_params.get("metadata", {}) or {}
            )  # if dheera_ai_params['metadata'] == None
            metadata = self.add_metadata_from_header(dheera_ai_params, metadata)
    ...
    def _get_langfuse_input_output_content(
        ...
        if (
            ...
            or isinstance(response_obj, dheera_ai.EmbeddingResponse)
        ):
            input = prompt
            output = None
            ...
            response_obj, dheera_ai.HttpxBinaryResponseContent
        ):
            input = prompt
            output = "speech-output"
        elif response_obj is not None and isinstance(
            response_obj, dheera_ai.TextCompletionResponse
        ):
            input = prompt
            output = self._get_text_completion_content_for_langfuse(response_obj)
        elif response_obj is not None and isinstance(
            response_obj, dheera_ai.ImageResponse
        ):
            input = prompt
            output = response_obj.get("data", None)
        elif response_obj is not None and isinstance(
            response_obj, dheera_ai.TranscriptionResponse
        ):
            input = prompt
            output = response_obj.get("text", None)
        elif response_obj is not None and isinstance(
            response_obj, dheera_ai.RerankResponse
        ):
            input = prompt
            output = response_obj.results
        elif response_obj is not None and isinstance(
            response_obj, dheera_ai.ResponsesAPIResponse
        ):
            input = prompt
    ...
    def _log_langfuse_v1(
        ...
        trace = self.Langfuse.trace(  # type: ignore
            CreateTrace(  # type: ignore
                name=metadata.get("generation_name", "dheera_ai-completion"),
                input=input,
                output=output,
                userId=user_id,
            )
    ...
    def _log_langfuse_v2(  # noqa: PLR0915
        ...
        try:
            ...
            # we clean out all extra dheera_ai metadata params before logging
            ...
            if isinstance(metadata, dict):
                for key, value in metadata.items():
                    # generate langfuse tags - Default Tags sent to Langfuse from DheeraAI Proxy
                    if (
                        dheera_ai.langfuse_default_tags is not None
                        and isinstance(dheera_ai.langfuse_default_tags, list)
                        and key in dheera_ai.langfuse_default_tags
                    ):
                        tags.append(f"{key}:{value}")

                    # clean dheera_ai metadata before logging
                    if key in [
                        "headers",
                        "endpoint",
                        "caching_groups",
            ...
            # However, we skip standard_logging_object.trace_id if it's a UUID (from dheera_ai_trace_id default),
            # as we want to fall back to dheera_ai_call_id instead for better traceability.
            ...
            if trace_id is None and standard_logging_object is not None:
                ...
                # This primarily filters out default dheera_ai_trace_id UUIDs, while still allowing user-provided
            ...
            # Fallback to dheera_ai_call_id if no trace_id found
            if trace_id is None:
                trace_id = dheera_ai_call_id
            ...
            masking_function = dheera_ai_params.get("_langfuse_masking_function") or clean_metadata.pop("langfuse_masking_function", None)
            ...
            if trace_name is None and existing_trace_id is None:
                # just log `dheera_ai-{call_type}` as the trace name
                ## DO NOT SET TRACE_NAME if trace-id set. this can lead to overwriting of past traces.
                trace_name = f"dheera_ai-{kwargs.get('call_type', 'completion')}"
            ...
            if existing_trace_id is not None:
                ...
                if "input" in update_trace_keys:
                    trace_params["input"] = (
                        input if not mask_input else "redacted-by-dheera_ai"
                    )
                if "output" in update_trace_keys:
                    trace_params["output"] = (
                        output if not mask_output else "redacted-by-dheera_ai"
                    )
                ...
                trace_params = {
                    ...
                    "input": input if not mask_input else "redacted-by-dheera_ai",
            ...
            if debug is True or (isinstance(debug, str) and debug.lower() == "true"):
                if "metadata" in trace_params:
                    ...
                    trace_params["metadata"]["metadata_passed_to_dheera_ai"] = metadata
                else:
                    trace_params["metadata"] = {"metadata_passed_to_dheera_ai": metadata}
            ...
            clean_metadata["dheera_ai_response_cost"] = cost
            ...
            proxy_server_request = dheera_ai_params.get("proxy_server_request", None)
            ...
            if response_obj is not None:
                if (
                    ...
                    generation_id = dheera_ai.utils.get_logging_id(
            ...
            if generation_name is None:
                ...
                # If using dheera_ai proxy user `key_alias` if not None
                # If `key_alias` is None, just log `dheera_ai-{call_type}` as the generation name
                _user_api_key_alias = cast(
                    Optional[str], clean_metadata.get("user_api_key_alias", None)
                )
                generation_name = (
                    f"dheera_ai-{cast(str, kwargs.get('call_type', 'completion'))}"
                )
                if _user_api_key_alias is not None:
                    generation_name = f"dheera_ai:{_user_api_key_alias}"
            ...
            generation_params = {
                ...
                "input": input if not mask_input else "redacted-by-dheera_ai",
                "output": output if not mask_output else "redacted-by-dheera_ai",
                "usage": usage,
            ...
            # Return the trace_id we set (which should be dheera_ai_call_id when no explicit trace_id provided)
    ...
    def add_default_langfuse_tags(self, tags, kwargs, metadata):
        """
        Helper function to add dheera_ai default langfuse tags

        - Special DheeraAI tags:
            - cache_hit
            - cache_key

        """
        if dheera_ai.langfuse_default_tags is not None and isinstance(
            dheera_ai.langfuse_default_tags, list
        ):
            if "cache_hit" in dheera_ai.langfuse_default_tags:
                _cache_hit_value = kwargs.get("cache_hit", False)
                tags.append(f"cache_hit:{_cache_hit_value}")
            if "cache_key" in dheera_ai.langfuse_default_tags:
                _hidden_params = metadata.get("hidden_params", {}) or {}
                _cache_key = _hidden_params.get("cache_key", None)
                if _cache_key is None and dheera_ai.cache is not None:
                    # fallback to using "preset_cache_key"
                    _preset_cache_key = dheera_ai.cache._get_preset_cache_key_from_kwargs(
                        **kwargs
                    )
