...
from dheera_ai.types.integrations.langfuse_otel import (
    LangfuseOtelConfig,
...
if TYPE_CHECKING:
    ...
    from dheera_ai.types.integrations.arize import Protocol as _Protocol
...
class LangfuseOtelLogger(OpenTelemetry):
    ...
    def _extract_langfuse_metadata(kwargs: dict) -> dict:
        """
        Extracts Langfuse metadata from the standard DheeraAI kwargs structure.

        1. Reads kwargs["dheera_ai_params"]["metadata"] if present and is a dict.
        ...
        try:
            ...
            metadata = _LFLogger.add_metadata_from_header(dheera_ai_params, metadata)  # type: ignore
        except Exception:
    ...
    def _set_metadata_attributes(span: Span, metadata: dict):
        ...
        from dheera_ai.integrations.arize._utils import safe_set_attribute
