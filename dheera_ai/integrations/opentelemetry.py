...
from dheera_ai.types.services import ServiceLoggerPayload
...
if TYPE_CHECKING:
    ...
    from dheera_ai.proxy.proxy_server import UserAPIKeyAuth as _UserAPIKeyAuth
...
DHEERA_AI_TRACER_NAME = os.getenv("OTEL_TRACER_NAME", "dheera_ai")
DHEERA_AI_METER_NAME = os.getenv("DHEERA_AI_METER_NAME", "dheera_ai")
DHEERA_AI_LOGGER_NAME = os.getenv("DHEERA_AI_LOGGER_NAME", "dheera_ai")
# Remove the hardcoded DHEERA_AI_RESOURCE dictionary - we'll create it properly later
RAW_REQUEST_SPAN_NAME = "raw_gen_ai_request"
DHEERA_AI_REQUEST_SPAN_NAME = "dheera_ai_request"


def _get_dheera_ai_resource():
    """
    Create a proper OpenTelemetry Resource that respects OTEL_RESOURCE_ATTRIBUTES
    while maintaining backward compatibility with DheeraAI-specific environment variables.
    """
    from opentelemetry.sdk.resources import OTELResourceDetector, Resource

    # Create base resource attributes with DheeraAI-specific defaults
    # These will be overridden by OTEL_RESOURCE_ATTRIBUTES if present
    base_attributes: Dict[str, Optional[str]] = {
        "service.name": os.getenv("OTEL_SERVICE_NAME", "dheera_ai"),
        ...
        "model_id": os.getenv(
            "OTEL_MODEL_ID", os.getenv("OTEL_SERVICE_NAME", "dheera_ai")
        ),
    }

    # Create base resource with DheeraAI-specific defaults
    ...
    # This ensures OTEL_RESOURCE_ATTRIBUTES overrides DheeraAI defaults
...
class OpenTelemetryConfig:
    ...
    def from_env(cls):
        ...
        enable_metrics: bool = (
            os.getenv("DHEERA_AI_OTEL_INTEGRATION_ENABLE_METRICS", "false").lower()
            == "true"
        )
        enable_events: bool = (
            os.getenv("DHEERA_AI_OTEL_INTEGRATION_ENABLE_EVENTS", "false").lower()
            == "true"
        )
...
class OpenTelemetry(CustomLogger):
    def __init__(
        ...
        **kwargs,
        ...
        self._init_otel_logger_on_dheera_ai_proxy()

    def _init_otel_logger_on_dheera_ai_proxy(self):
        """
        Initializes OpenTelemetry for dheera_ai proxy server
        ...
        if "otel" not in dheera_ai.service_callback and all(
            not isinstance(cb, OpenTelemetry) for cb in dheera_ai.service_callback
        ):
            dheera_ai.service_callback.append(self)
    ...
    def _init_tracing(self, tracer_provider):
        ...
        if tracer_provider is None:
            ...
            try:
                ...
                if not isinstance(existing_provider, ProxyTracerProvider):
                    ...
                    tracer_provider = TracerProvider(resource=_get_dheera_ai_resource())
        ...
        # grab our tracer
        self.tracer = trace.get_tracer(DHEERA_AI_TRACER_NAME)
    ...
    def _init_metrics(self, meter_provider):
        ...
        if meter_provider is None:
            ...
            meter_provider = MeterProvider(
                metric_readers=[_metric_reader], resource=_get_dheera_ai_resource()
            )
        ...
        self._response_duration_histogram = meter.create_histogram(
            ...
            description="Total LLM API generation time (excludes DheeraAI overhead)",
            unit="s",
        )
    ...
    def _init_logs(self, logger_provider):
        ...
        if logger_provider is None:
            dheera_ai_resource = _get_dheera_ai_resource()
            logger_provider = OTLoggerProvider(resource=dheera_ai_resource)
                ...
                for key, value in event_metadata.items():
                    ...
                    if isinstance(value, dict):
                        try:
                            ...
                            value = "dheera_ai logging error - could_not_json_serialize"
                    self.safe_set_attribute(
                        span=service_logging_span,
                        key=key,
                        value=value,
    ...
    async def async_post_call_success_hook(
        ...
        dheera_ai_logging_obj = data.get("dheera_ai_logging_obj")

        if dheera_ai_logging_obj is not None and isinstance(
            dheera_ai_logging_obj, DheeraAILogging
        ):
            kwargs = dheera_ai_logging_obj.model_call_details
    ...
    def _get_tracer_with_dynamic_headers(self, dynamic_headers: dict):
        ...
        temp_provider = TracerProvider(resource=_get_dheera_ai_resource())
        temp_provider.add_span_processor(
            self._get_span_processor(dynamic_headers=dynamic_headers)
        )

        return temp_provider.get_tracer(DHEERA_AI_TRACER_NAME)
    ...
    def _handle_success(self, kwargs, response_obj, start_time, end_time):
        ...
        if get_secret_bool("USE_OTEL_DHEERA_AI_REQUEST_SPAN"):
    ...
    def _maybe_log_raw_request(
        ...
        if dheera_ai.turn_off_message_logging or not self.message_logging:
            return
    ...
    def _record_metrics(self, kwargs, response_obj, start_time, end_time):
        ...
        params = kwargs.get("dheera_ai_params") or {}
    ...
    def _record_time_to_first_token_metric(self, kwargs: dict, common_attrs: dict):
        ...
        # This excludes DheeraAI overhead and measures pure LLM API latency
    ...
    def _record_response_duration_metric(
        ...
        This excludes DheeraAI overhead and measures only the LLM provider's response time.
        Works for both streaming and non-streaming requests.
        
        Mirrors Prometheus's dheera_ai_llm_api_latency_metric.
    ...
    def _emit_semantic_logs(self, kwargs, response_obj, span: Span):
        ...
        otel_logger = get_logger(DHEERA_AI_LOGGER_NAME)

        # Get the resource from the logger provider
        logger_provider = get_logger_provider()
        resource = (
            getattr(logger_provider, "_resource", None) or _get_dheera_ai_resource()
        )

        parent_ctx = span.get_span_context()
        provider = (kwargs.get("dheera_ai_params") or {}).get(
    ...
    def _handle_failure(self, kwargs, response_obj, start_time, end_time):
        ...
        # Span 1: Requst sent to dheera_ai SDK
        otel_tracer: Tracer = self.get_tracer_to_use_for_request(kwargs)
    ...
    def set_attributes(  # noqa: PLR0915
        ...
        try:
            if self.callback_name == "langtrace":
                from dheera_ai.integrations.langtrace import LangtraceAttributes
                ...
                from dheera_ai.integrations.langfuse.langfuse_otel import (
                    LangfuseOtelLogger,
                )
                ...
                from dheera_ai.integrations.weave.weave_otel import set_weave_otel_attributes
            ...
            self.safe_set_attribute(
                ...
                value=dheera_ai_params.get("custom_llm_provider", "Unknown"),
            )
    ...
    def set_raw_request_attributes(self, span: Span, kwargs, response_obj):
        try:
            ...
            dheera_ai_params = kwargs.get("dheera_ai_params", {}) or {}
            custom_llm_provider = dheera_ai_params.get("custom_llm_provider", "Unknown")
            ...
            if _raw_response and isinstance(_raw_response, str):
                ...
                try:
                    ...
                    verbose_logger.debug(
                        "dheera_ai.integrations.opentelemetry.py::set_raw_request_attributes() - raw_response not json string - {}".format(
                            _raw_response
                        )
                    )
    ...
    def _get_span_name(self, kwargs):
        ...
        return DHEERA_AI_REQUEST_SPAN_NAME
    ...
    def _get_span_context(self, kwargs, default_span: Optional[Span] = None):
        ...
        _metadata = dheera_ai_params.get("metadata", {}) or {}
        parent_otel_span = _metadata.get("dheera_ai_parent_otel_span", None)
    ...
    def create_dheera_ai_proxy_request_started_span(
        self,
        start_time: datetime,
        headers: dict,
    ) -> Optional[Span]:
