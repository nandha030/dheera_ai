...
from dheera_ai import _logging


def normalize_provider_name(provider: Optional[str]) -> Optional[str]:
    """
    Normalize DheeraAI provider names to standardized string names.
    
    Args:
        provider: DheeraAI internal provider name
...
def extract_opik_metadata(
    dheera_ai_metadata: Dict[str, Any],
    ...
    _logging.verbose_logger.debug(
        f"dheera_ai_opik_metadata - {json.dumps(opik_meta, default=str)}"
    )
...
def extract_and_build_metadata(
    ...
    - Cost information from dheera_ai_kwargs (calculated after completion)
    ...
    Args:
        ...
        dheera_ai_kwargs: Original DheeraAI kwargs (includes response_cost)
    ...
    # response_cost is calculated by DheeraAI after completion and added to kwargs
    # See: dheera_ai/dheera_ai_core_utils/llm_response_utils/response_metadata.py
    if "response_cost" in dheera_ai_kwargs:
        metadata["cost"] = {
            "total_tokens": dheera_ai_kwargs["response_cost"],
            "currency": "USD",
        }

    # Add debug info if cost calculation failed
    if "response_cost_failure_debug_info" in dheera_ai_kwargs:
        metadata["response_cost_failure_debug_info"] = dheera_ai_kwargs[
