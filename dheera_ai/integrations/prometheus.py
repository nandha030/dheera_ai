# used for /metrics endpoint on DheeraAI Proxy
#### What this does ####
...
import dheera_ai
from dheera_ai._logging import print_verbose, verbose_logger
from dheera_ai.integrations.custom_logger import CustomLogger
from dheera_ai.proxy._types import DheeraAI_TeamTable, UserAPIKeyAuth
from dheera_ai.types.integrations.prometheus import *
from dheera_ai.types.integrations.prometheus import _sanitize_prometheus_label_name
...
def _get_cached_end_user_id_for_cost_tracking():
    ...
    if _get_end_user_id_for_cost_tracking is None:
        from dheera_ai.utils import get_end_user_id_for_cost_tracking
...
class PrometheusLogger(CustomLogger):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            self.dheera_ai_proxy_failed_requests_metric = self._counter_factory(
                name="dheera_ai_proxy_failed_requests_metric",
                documentation="Total number of failed responses from proxy - the client did not get a success response from dheera_ai proxy",
                labelnames=self.get_labels_for_metric(
                    "dheera_ai_proxy_failed_requests_metric"
                ),
            )

            self.dheera_ai_proxy_total_requests_metric = self._counter_factory(
                name="dheera_ai_proxy_total_requests_metric",
                documentation="Total number of requests made to the proxy server - track number of client side requests",
                labelnames=self.get_labels_for_metric(
                    "dheera_ai_proxy_total_requests_metric"
                ),
            )

            # request latency metrics
            self.dheera_ai_request_total_latency_metric = self._histogram_factory(
                "dheera_ai_request_total_latency_metric",
                "Total latency (seconds) for a request to DheeraAI",
                labelnames=self.get_labels_for_metric(
                    "dheera_ai_request_total_latency_metric"
                ),
            ...
            self.dheera_ai_llm_api_latency_metric = self._histogram_factory(
                "dheera_ai_llm_api_latency_metric",
                "Total latency (seconds) for a models LLM API call",
                labelnames=self.get_labels_for_metric("dheera_ai_llm_api_latency_metric"),
                buckets=LATENCY_BUCKETS,
            )

            self.dheera_ai_llm_api_time_to_first_token_metric = self._histogram_factory(
                "dheera_ai_llm_api_time_to_first_token_metric",
            ...
            # Counter for spend
            self.dheera_ai_spend_metric = self._counter_factory(
                "dheera_ai_spend_metric",
                "Total spend on LLM requests",
                labelnames=self.get_labels_for_metric("dheera_ai_spend_metric"),
            )

            # Counter for total_output_tokens
            self.dheera_ai_tokens_metric = self._counter_factory(
                "dheera_ai_total_tokens_metric",
                "Total number of input + output tokens from LLM requests",
                labelnames=self.get_labels_for_metric("dheera_ai_total_tokens_metric"),
            )

            self.dheera_ai_input_tokens_metric = self._counter_factory(
                "dheera_ai_input_tokens_metric",
                "Total number of input tokens from LLM requests",
                labelnames=self.get_labels_for_metric("dheera_ai_input_tokens_metric"),
            )

            self.dheera_ai_output_tokens_metric = self._counter_factory(
                "dheera_ai_output_tokens_metric",
                "Total number of output tokens from LLM requests",
                labelnames=self.get_labels_for_metric("dheera_ai_output_tokens_metric"),
            )

            # Remaining Budget for Team
            self.dheera_ai_remaining_team_budget_metric = self._gauge_factory(
                "dheera_ai_remaining_team_budget_metric",
                "Remaining budget for team",
                labelnames=self.get_labels_for_metric(
                    "dheera_ai_remaining_team_budget_metric"
                ),
            ...
            # Max Budget for Team
            self.dheera_ai_team_max_budget_metric = self._gauge_factory(
                "dheera_ai_team_max_budget_metric",
                "Maximum budget set for team",
                labelnames=self.get_labels_for_metric("dheera_ai_team_max_budget_metric"),
            )

            # Team Budget Reset At
            self.dheera_ai_team_budget_remaining_hours_metric = self._gauge_factory(
                "dheera_ai_team_budget_remaining_hours_metric",
            ...
            self.dheera_ai_remaining_api_key_budget_metric = self._gauge_factory(
                "dheera_ai_remaining_api_key_budget_metric",
                "Remaining budget for api key",
                labelnames=self.get_labels_for_metric(
                    "dheera_ai_remaining_api_key_budget_metric"
                ),
            ...
            self.dheera_ai_api_key_max_budget_metric = self._gauge_factory(
                "dheera_ai_api_key_max_budget_metric",
            ...
            self.dheera_ai_api_key_budget_remaining_hours_metric = self._gauge_factory(
                "dheera_ai_api_key_budget_remaining_hours_metric",
            ...
            ########################################
            # DheeraAI Virtual API KEY metrics
            ########################################
            # Remaining MODEL RPM limit for API Key
            self.dheera_ai_remaining_api_key_requests_for_model = self._gauge_factory(
                "dheera_ai_remaining_api_key_requests_for_model",
                "Remaining Requests API Key can make for model (model based rpm limit on key)",
                labelnames=["hashed_api_key", "api_key_alias", "model"],
            )

            # Remaining MODEL TPM limit for API Key
            self.dheera_ai_remaining_api_key_tokens_for_model = self._gauge_factory(
                "dheera_ai_remaining_api_key_tokens_for_model",
            ...
            self.dheera_ai_remaining_requests_metric = self._gauge_factory(
                "dheera_ai_remaining_requests",
            ...
            self.dheera_ai_remaining_tokens_metric = self._gauge_factory(
                "dheera_ai_remaining_tokens",
            ...
            self.dheera_ai_overhead_latency_metric = self._histogram_factory(
                "dheera_ai_overhead_latency_metric",
                "Latency overhead (milliseconds) added by DheeraAI processing",
                labelnames=self.get_labels_for_metric(
                    "dheera_ai_overhead_latency_metric"
                ),
                buckets=LATENCY_BUCKETS,
            ...
            self.dheera_ai_provider_remaining_budget_metric = self._gauge_factory(
                "dheera_ai_provider_remaining_budget_metric",
                "Remaining budget for provider - used when you set provider budget limits",
                labelnames=["api_provider"],
            )

            # Metric for deployment state
            self.dheera_ai_deployment_state = self._gauge_factory(
                "dheera_ai_deployment_state",
                "LLM Deployment Analytics - The state of the deployment: 0 = healthy, 1 = partial outage, 2 = complete outage",
                labelnames=self.get_labels_for_metric("dheera_ai_deployment_state"),
            )

            self.dheera_ai_deployment_cooled_down = self._counter_factory(
                "dheera_ai_deployment_cooled_down",
                "LLM Deployment Analytics - Number of times a deployment has been cooled down by DheeraAI load balancing logic. exception_status is the status of the exception that caused the deployment to be cooled down",
                # labelnames=_logged_llm_labels + [EXCEPTION_STATUS],
                labelnames=self.get_labels_for_metric("dheera_ai_deployment_cooled_down"),
            )

            self.dheera_ai_deployment_success_responses = self._counter_factory(
                name="dheera_ai_deployment_success_responses",
                documentation="LLM Deployment Analytics - Total number of successful LLM API calls via dheera_ai",
                labelnames=self.get_labels_for_metric(
                    "dheera_ai_deployment_success_responses"
                ),
            )
            self.dheera_ai_deployment_failure_responses = self._counter_factory(
                name="dheera_ai_deployment_failure_responses",
                ...
                labelnames=self.get_labels_for_metric(
                    "dheera_ai_deployment_failure_responses"
                ),
            )

            self.dheera_ai_deployment_total_requests = self._counter_factory(
                name="dheera_ai_deployment_total_requests",
                documentation="LLM Deployment Analytics - Total number of LLM API calls via dheera_ai - success + failure",
                labelnames=self.get_labels_for_metric(
            ...
            self.dheera_ai_deployment_latency_per_output_token = self._histogram_factory(
                name="dheera_ai_deployment_latency_per_output_token",
                documentation="LLM Deployment Analytics - Latency per output token",
                labelnames=self.get_labels_for_metric(
                    "dheera_ai_deployment_latency_per_output_token"
                ),
            )

            self.dheera_ai_deployment_successful_fallbacks = self._counter_factory(
                "dheera_ai_deployment_successful_fallbacks",
                "LLM Deployment Analytics - Number of successful fallback requests from primary model -> fallback model",
                self.get_labels_for_metric("dheera_ai_deployment_successful_fallbacks"),
            )

            self.dheera_ai_deployment_failed_fallbacks = self._counter_factory(
                "dheera_ai_deployment_failed_fallbacks",
                "LLM Deployment Analytics - Number of failed fallback requests from primary model -> fallback model",
                self.get_labels_for_metric("dheera_ai_deployment_failed_fallbacks"),
            )

            # Callback Logging Failure Metrics
            self.dheera_ai_callback_logging_failures_metric = self._counter_factory(
                name="dheera_ai_callback_logging_failures_metric",
                documentation="Total number of failures when emitting logs to callbacks (e.g. s3_v2, langfuse, etc)",
                labelnames=["callback_name"],
            )

            self.dheera_ai_llm_api_failed_requests_metric = self._counter_factory(
                name="dheera_ai_llm_api_failed_requests_metric",
                documentation="deprecated - use dheera_ai_proxy_failed_requests_metric",
                labelnames=[
                    "end_user",
                    "hashed_api_key",
                    "api_key_alias",
            ...
            self.dheera_ai_requests_metric = self._counter_factory(
                name="dheera_ai_requests_metric",
                documentation="deprecated - use dheera_ai_proxy_total_requests_metric. Total number of LLM calls to dheera_ai - track total per API Key, team, user",
                labelnames=self.get_labels_for_metric("dheera_ai_requests_metric"),
            )
    ...
    def _parse_prometheus_config(self) -> Dict[str, List[str]]:
        ...
        config = dheera_ai.prometheus_metrics_config
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        dheera_ai_params = kwargs.get("dheera_ai_params", {}) or {}
        _metadata = dheera_ai_params.get("metadata", {})
        get_end_user_id_for_cost_tracking = _get_cached_end_user_id_for_cost_tracking()
        
        end_user_id = get_end_user_id_for_cost_tracking(
            dheera_ai_params, service_type="prometheus"
        )
        ...
        if (
            ...
            from dheera_ai.proxy.utils import hash_token
        ...
        if (
            ...
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="dheera_ai_proxy_total_requests_metric"
                ),
                enum_values=enum_values,
            )
            self.dheera_ai_proxy_total_requests_metric.labels(**_labels).inc()
    ...
    def _increment_token_metrics(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="dheera_ai_total_tokens_metric"
            ),
            enum_values=enum_values,
        )
        self.dheera_ai_tokens_metric.labels(**_labels).inc(
            standard_logging_payload["total_tokens"]
        )

        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="dheera_ai_input_tokens_metric"
            ),
            enum_values=enum_values,
        )
        self.dheera_ai_input_tokens_metric.labels(**_labels).inc(
            standard_logging_payload["prompt_tokens"]
        )

        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="dheera_ai_output_tokens_metric"
            ),
            enum_values=enum_values,
        )

        self.dheera_ai_output_tokens_metric.labels(**_labels).inc(
    ...
    async def _increment_remaining_budget_metrics(
        ...
        _team_spend = dheera_ai_params.get("metadata", {}).get(
            "user_api_key_team_spend", None
        )
        _team_max_budget = dheera_ai_params.get("metadata", {}).get(
            "user_api_key_team_max_budget", None
        )

        _api_key_spend = dheera_ai_params.get("metadata", {}).get(
            "user_api_key_spend", None
        )
        _api_key_max_budget = dheera_ai_params.get("metadata", {}).get(
    ...
    def _increment_top_level_request_and_spend_metrics(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="dheera_ai_requests_metric"
            ),
            enum_values=enum_values,
        )

        self.dheera_ai_requests_metric.labels(**_labels).inc()

        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="dheera_ai_spend_metric"
            ),
            enum_values=enum_values,
        )

        self.dheera_ai_spend_metric.labels(**_labels).inc(response_cost)
    ...
    def _set_virtual_key_rate_limit_metrics(
        ...
        from dheera_ai.proxy.common_utils.callback_utils import (
            get_model_group_from_dheera_ai_kwargs,
        )

        # Set remaining rpm/tpm for API Key + model
        # see parallel_request_limiter.py - variables are set there
        model_group = get_model_group_from_dheera_ai_kwargs(kwargs)
        remaining_requests_variable_name = (
            f"dheera_ai-key-remaining-requests-{model_group}"
        )
        remaining_tokens_variable_name = f"dheera_ai-key-remaining-tokens-{model_group}"
        ...
        self.dheera_ai_remaining_api_key_requests_for_model.labels(
            user_api_key, user_api_key_alias, model_group
        ).set(remaining_requests)

        self.dheera_ai_remaining_api_key_tokens_for_model.labels(
            user_api_key, user_api_key_alias, model_group
    ...
    def _set_latency_metrics(
        ...
        if (
            ...
            self.dheera_ai_llm_api_time_to_first_token_metric.labels(
                model,
                user_api_key,
                user_api_key_alias,
                user_api_team,
        ...
        if api_call_total_time_seconds is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="dheera_ai_llm_api_latency_metric"
                ),
                enum_values=enum_values,
            )
            self.dheera_ai_llm_api_latency_metric.labels(**_labels).observe(
        ...
        if total_time_seconds is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="dheera_ai_request_total_latency_metric"
                ),
                enum_values=enum_values,
            )
            self.dheera_ai_request_total_latency_metric.labels(**_labels).observe(
                total_time_seconds
            )
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            self.dheera_ai_llm_api_failed_requests_metric.labels(
                end_user_id,
                user_api_key,
                user_api_key_alias,
                model,
    ...
    async def async_post_call_failure_hook(
        ...
        try:
            ...
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="dheera_ai_proxy_failed_requests_metric"
                ),
                enum_values=enum_values,
            )
            self.dheera_ai_proxy_failed_requests_metric.labels(**_labels).inc()
    ...
    def set_llm_deployment_failure_metrics(self, request_kwargs: dict):
        ...
        try:
            ...
            _dheera_ai_params = request_kwargs.get("dheera_ai_params", {}) or {}
            dheera_ai_model_name = request_kwargs.get("model", None)
            ...
            llm_provider = _dheera_ai_params.get("custom_llm_provider", None)

            # Create enum_values for the label factory (always create for use in different metrics)
            enum_values = UserAPIKeyLabelValues(
                dheera_ai_model_name=dheera_ai_model_name,
                model_id=model_id,
                api_base=api_base,
            ...
            """
            log these labels
            ["dheera_ai_model_name", "model_id", "api_base", "api_provider"]
            """
            self.set_deployment_partial_outage(
                dheera_ai_model_name=dheera_ai_model_name or "",
                model_id=model_id,
                api_base=api_base,
            ...
            if exception is not None:
                ...
                _labels = prometheus_label_factory(
                    supported_enum_labels=self.get_labels_for_metric(
                        metric_name="dheera_ai_deployment_failure_responses"
                    ),
                    enum_values=enum_values,
                )
                self.dheera_ai_deployment_failure_responses.labels(**_labels).inc()

            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="dheera_ai_deployment_total_requests"
                ),
                enum_values=enum_values,
            )
            self.dheera_ai_deployment_total_requests.labels(**_labels).inc()
    ...
    def set_llm_deployment_success_metrics(
        ...
        try:
            ...
            api_base = standard_logging_payload["api_base"]
            _dheera_ai_params = request_kwargs.get("dheera_ai_params", {}) or {}
            _metadata = _dheera_ai_params.get("metadata", {})
            dheera_ai_model_name = request_kwargs.get("model", None)
            llm_provider = _dheera_ai_params.get("custom_llm_provider", None)
            ...
            if dheera_ai_overhead_time_ms := standard_logging_payload[
                "hidden_params"
            ].get("dheera_ai_overhead_time_ms"):
                _labels = prometheus_label_factory(
                    supported_enum_labels=self.get_labels_for_metric(
                        metric_name="dheera_ai_overhead_latency_metric"
                    ),
                    enum_values=enum_values,
                )
                self.dheera_ai_overhead_latency_metric.labels(**_labels).observe(
                    dheera_ai_overhead_time_ms / 1000
                )  # set as seconds
            ...
            if remaining_requests:
                ...
                _labels = prometheus_label_factory(
                    supported_enum_labels=self.get_labels_for_metric(
                        metric_name="dheera_ai_remaining_requests_metric"
                    ),
                    enum_values=enum_values,
                )
                self.dheera_ai_remaining_requests_metric.labels(**_labels).set(
                    remaining_requests
                )
            ...
            if remaining_tokens:
                _labels = prometheus_label_factory(
                    supported_enum_labels=self.get_labels_for_metric(
                        metric_name="dheera_ai_remaining_tokens_metric"
                    ),
                    enum_values=enum_values,
                )
                self.dheera_ai_remaining_tokens_metric.labels(**_labels).set(
                    remaining_tokens
                )

            """
            log these labels
            ["dheera_ai_model_name", "requested_model", model_id", "api_base", "api_provider"]
            """
            ...
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="dheera_ai_deployment_success_responses"
                ),
                enum_values=enum_values,
            )
            self.dheera_ai_deployment_success_responses.labels(**_labels).inc()
            ...
            if output_tokens is not None and output_tokens > 0:
                ...
                _labels = prometheus_label_factory(
                    supported_enum_labels=self.get_labels_for_metric(
                        metric_name="dheera_ai_deployment_latency_per_output_token"
                    ),
                    enum_values=enum_values,
                )
                self.dheera_ai_deployment_latency_per_output_token.labels(
                    **_labels
    ...
    async def log_success_fallback_event(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="dheera_ai_deployment_successful_fallbacks"
            ),
            enum_values=enum_values,
        )
        self.dheera_ai_deployment_successful_fallbacks.labels(**_labels).inc()
    ...
    async def log_failure_fallback_event(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="dheera_ai_deployment_failed_fallbacks"
            ),
            enum_values=enum_values,
        )
        self.dheera_ai_deployment_failed_fallbacks.labels(**_labels).inc()

    def set_dheera_ai_deployment_state(
        self,
        state: int,
        dheera_ai_model_name: str,
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="dheera_ai_deployment_state"
            ),
            enum_values=UserAPIKeyLabelValues(
                dheera_ai_model_name=dheera_ai_model_name,
                model_id=model_id,
        ...
        self.dheera_ai_deployment_state.labels(**_labels).set(state)
    ...
    def set_deployment_healthy(
        ...
        self.set_dheera_ai_deployment_state(
            0, dheera_ai_model_name, model_id, api_base, api_provider
        )
    ...
    def set_deployment_partial_outage(
        ...
        self.set_dheera_ai_deployment_state(
            1, dheera_ai_model_name, model_id, api_base, api_provider
        )
    ...
    def set_deployment_complete_outage(
        ...
        self.set_dheera_ai_deployment_state(
            2, dheera_ai_model_name, model_id, api_base, api_provider
        )
    ...
    def increment_deployment_cooled_down(
        ...
        """
        increment metric when dheera_ai.Router / load balancing logic places a deployment in cool down
        """
        self.dheera_ai_deployment_cooled_down.labels(
            dheera_ai_model_name, model_id, api_base, api_provider, exception_status
        ).inc()
    ...
    def increment_callback_logging_failure(
        ...
        """
        self.dheera_ai_callback_logging_failures_metric.labels(
    ...
    def track_provider_remaining_budget(
        ...
        """
        self.dheera_ai_provider_remaining_budget_metric.labels(provider).set(
    ...
    async def _initialize_team_budget_metrics(self):
        ...
        ) -> Tuple[List[DheeraAI_TeamTable], Optional[int]]:
    ...
    async def _initialize_api_key_budget_metrics(self):
        ...
        from dheera_ai.constants import UI_SESSION_TOKEN_TEAM_ID
    ...
    async def initialize_remaining_budget_metrics(self):
        ...
        """
        from dheera_ai.constants import PROMETHEUS_EMIT_BUDGET_METRICS_JOB_NAME
    ...
    async def _set_team_list_budget_metrics(self, teams: List[DheeraAI_TeamTable]):
    ...
    async def _set_team_budget_metrics_after_api_request(
        ...
        - Assemble a DheeraAI_TeamTable object
    ...
    async def _assemble_team_object(
        ...
        from dheera_ai.proxy.proxy_server import prisma_client, user_api_key_cache

        _total_team_spend = (spend or 0) + response_cost
        team_object = DheeraAI_TeamTable(
            team_id=team_id,
    ...
    def _set_team_budget_metrics(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="dheera_ai_remaining_team_budget_metric"
            ),
            enum_values=enum_values,
        )
        self.dheera_ai_remaining_team_budget_metric.labels(**_labels).set(
            self._safe_get_remaining_budget(
        ...
        if team.max_budget is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="dheera_ai_team_max_budget_metric"
                ),
                enum_values=enum_values,
            )
            self.dheera_ai_team_max_budget_metric.labels(**_labels).set(team.max_budget)

        if team.budget_reset_at is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="dheera_ai_team_budget_remaining_hours_metric"
                ),
                enum_values=enum_values,
            )
            self.dheera_ai_team_budget_remaining_hours_metric.labels(**_labels).set(
    ...
    def _set_key_budget_metrics(self, user_api_key_dict: UserAPIKeyAuth):
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="dheera_ai_remaining_api_key_budget_metric"
            ),
            enum_values=enum_values,
        )
        self.dheera_ai_remaining_api_key_budget_metric.labels(**_labels).set(
            self._safe_get_remaining_budget(
        ...
        if user_api_key_dict.max_budget is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="dheera_ai_api_key_max_budget_metric"
                ),
                enum_values=enum_values,
            )
            self.dheera_ai_api_key_max_budget_metric.labels(**_labels).set(
                user_api_key_dict.max_budget
            )

        if user_api_key_dict.budget_reset_at is not None:
            self.dheera_ai_api_key_budget_remaining_hours_metric.labels(**_labels).set(
                self._get_remaining_hours_for_budget_reset(
    ...
    def initialize_budget_metrics_cron_job(scheduler: AsyncIOScheduler):
        ...
        """
        from dheera_ai.constants import PROMETHEUS_BUDGET_METRICS_REFRESH_INTERVAL_MINUTES
        from dheera_ai.integrations.custom_logger import CustomLogger

        prometheus_loggers: List[CustomLogger] = (
            dheera_ai.logging_callback_manager.get_custom_loggers_for_type(
...
def prometheus_label_factory(
    ...
    if UserAPIKeyLabelNames.END_USER.value in filtered_labels:
        ...
        filtered_labels["end_user"] = get_end_user_id_for_cost_tracking(
            dheera_ai_params={"user_api_key_end_user_id": enum_values.end_user},
...
def get_custom_labels_from_metadata(metadata: dict) -> Dict[str, str]:
    ...
    """
    keys = dheera_ai.custom_prometheus_metadata_labels
...
def get_custom_labels_from_tags(tags: List[str]) -> Dict[str, str]:
    ...
    configured_tags = dheera_ai.custom_prometheus_tags
