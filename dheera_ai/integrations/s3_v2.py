...
from dheera_ai.constants import DEFAULT_S3_BATCH_SIZE, DEFAULT_S3_FLUSH_INTERVAL_SECONDS
...
from dheera_ai.types.integrations.s3_v2 import s3BatchLoggingElement
...
class S3Logger(CustomBatchLogger, BaseAWSLLM):
        ...
        **kwargs,
        ...
        try:
            verbose_logger.debug(
                f"in init s3 logger - s3_callback_params {dheera_ai.s3_callback_params}"
            )
    ...
    def _init_s3_params(
        ...
        """
        dheera_ai.s3_callback_params = dheera_ai.s3_callback_params or {}
        # read in .env variables - example os.environ/AWS_BUCKET_NAME
        for key, value in dheera_ai.s3_callback_params.items():
            if isinstance(value, str) and value.startswith("os.environ/"):
                dheera_ai.s3_callback_params[key] = dheera_ai.get_secret(value)

        self.s3_bucket_name = (
            dheera_ai.s3_callback_params.get("s3_bucket_name") or s3_bucket_name
        )
        self.s3_region_name = (
            dheera_ai.s3_callback_params.get("s3_region_name") or s3_region_name
        )
        self.s3_api_version = (
            dheera_ai.s3_callback_params.get("s3_api_version") or s3_api_version
        )
        self.s3_use_ssl = (
            dheera_ai.s3_callback_params.get("s3_use_ssl", True) if dheera_ai.s3_callback_params.get("s3_use_ssl") is not None else s3_use_ssl
        )
        self.s3_verify = (
            dheera_ai.s3_callback_params.get("s3_verify") if dheera_ai.s3_callback_params.get("s3_verify") is not None else s3_verify
        )
        self.s3_endpoint_url = (
            dheera_ai.s3_callback_params.get("s3_endpoint_url") or s3_endpoint_url
        )
        self.s3_aws_access_key_id = (
            dheera_ai.s3_callback_params.get("s3_aws_access_key_id")
            or s3_aws_access_key_id
        )

        self.s3_aws_secret_access_key = (
            dheera_ai.s3_callback_params.get("s3_aws_secret_access_key")
            or s3_aws_secret_access_key
        )

        self.s3_aws_session_token = (
            dheera_ai.s3_callback_params.get("s3_aws_session_token")
            or s3_aws_session_token
        )

        self.s3_aws_session_name = (
            dheera_ai.s3_callback_params.get("s3_aws_session_name") or s3_aws_session_name
        )

        self.s3_aws_profile_name = (
            dheera_ai.s3_callback_params.get("s3_aws_profile_name") or s3_aws_profile_name
        )

        self.s3_aws_role_name = (
            dheera_ai.s3_callback_params.get("s3_aws_role_name") or s3_aws_role_name
        )

        self.s3_aws_web_identity_token = (
            dheera_ai.s3_callback_params.get("s3_aws_web_identity_token")
            or s3_aws_web_identity_token
        )

        self.s3_aws_sts_endpoint = (
            dheera_ai.s3_callback_params.get("s3_aws_sts_endpoint") or s3_aws_sts_endpoint
        )

        self.s3_config = dheera_ai.s3_callback_params.get("s3_config") or s3_config
        self.s3_path = dheera_ai.s3_callback_params.get("s3_path") or s3_path
        # done reading dheera_ai.s3_callback_params
        self.s3_use_team_prefix = (
            bool(dheera_ai.s3_callback_params.get("s3_use_team_prefix", False))
            or s3_use_team_prefix
        )

        self.s3_use_key_prefix = (
                bool(dheera_ai.s3_callback_params.get("s3_use_key_prefix", False))
                or s3_use_key_prefix
        )

        self.s3_strip_base64_files = (
            bool(dheera_ai.s3_callback_params.get("s3_strip_base64_files", False))
    ...
    async def async_upload_data_to_s3(
        ...
        try:
            from dheera_ai.dheera_ai_core_utils.asyncify import asyncify
    ...
    def create_s3_batch_logging_element(
        ...
        s3_file_name = (
            dheera_ai.utils.get_logging_id(start_time, standard_logging_payload) or ""
        )
