...
class Supabase:
    ...
    def __init__(self):
        ...
        if self.supabase_url is None or self.supabase_key is None:
            raise ValueError(
                "DheeraAI Error, trying to use Supabase but url or key not passed. Create a table and set `dheera_ai.supabase_url=<your-url>` and `dheera_ai.supabase_key=<your-key>`"
            )
    ...
    def input_log_event(
        self, model, messages, end_user, dheera_ai_call_id, print_verbose
    ):
        try:
            print_verbose(
    ...
    def log_event(
        ...
        try:
            ...
            total_cost = dheera_ai.completion_cost(completion_response=response_obj)
            ...
            if "choices" in response_obj:
                ...
                data, count = (
                    self.supabase_client.table(self.supabase_table_name)
                    .upsert(supabase_data_obj, on_conflict="dheera_ai_call_id")
                    .execute()
                )
