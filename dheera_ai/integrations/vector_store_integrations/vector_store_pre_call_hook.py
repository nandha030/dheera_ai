...
import dheera_ai
import dheera_ai.vector_stores
from dheera_ai._logging import verbose_logger
from dheera_ai.integrations.custom_logger import CustomLogger
from dheera_ai.types.llms.openai import AllMessageValues, ChatCompletionUserMessage
...
class VectorStorePreCallHook(CustomLogger):
    ...
    2. Calls dheera_ai.vector_stores.search() to get relevant context
    ...
    async def async_get_chat_completion_prompt(
        ...
        try:
            ...
            prisma_client = None
            try:
                from dheera_ai.proxy.proxy_server import prisma_client as _prisma_client
            ...
            vector_stores_to_run: List[DheeraAI_ManagedVectorStore] = (
                await dheera_ai.vector_store_registry.pop_vector_stores_to_run_with_db_fallback(
            ...
            for vector_store_to_run in vector_stores_to_run:
                ...
                dheera_ai_params_for_vector_store = (
                    vector_store_to_run.get("dheera_ai_params", {}) or {}
                )
                # Call dheera_ai.vector_stores.search() with the required parameters
            ...
            if dheera_ai_logging_obj and all_search_results:
                dheera_ai_logging_obj.model_call_details["search_results"] = (
                    all_search_results
                )
    ...
    async def async_post_call_success_deployment_hook(
        ...
        try:
            ...
            dheera_ai_logging_obj = request_data.get("dheera_ai_logging_obj")
            if not dheera_ai_logging_obj:
                verbose_logger.debug("No dheera_ai_logging_obj in request_data")
                return None

            verbose_logger.debug(
                f"model_call_details keys: {list(dheera_ai_logging_obj.model_call_details.keys())}"
            )
            ...
            search_results: Optional[List[VectorStoreSearchResponse]] = (
                dheera_ai_logging_obj.model_call_details.get("search_results")
            )
