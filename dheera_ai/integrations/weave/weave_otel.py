...
from dheera_ai.integrations._types.open_inference import SpanAttributes as OpenInferenceSpanAttributes
from dheera_ai.integrations.arize import _utils
from dheera_ai.integrations.opentelemetry import OpenTelemetry, OpenTelemetryConfig
from dheera_ai.integrations.opentelemetry_utils.base_otel_llm_obs_attributes import (
    BaseLLMObsOTELAttributes,
    safe_set_attribute,
)
from dheera_ai.dheera_ai_core_utils.safe_json_dumps import safe_dumps
from dheera_ai.types.integrations.weave_otel import WeaveOtelConfig, WeaveSpanAttributes
...
def _set_weave_specific_attributes(span: Span, kwargs: dict[str, Any], response_obj: Any):
    ...
    custom_llm_provider = dheera_ai_params.get("custom_llm_provider") or ""
...
class WeaveOtelLogger(OpenTelemetry):
    """
    Weave (W&B) OpenTelemetry Logger for DheeraAI.
    ...
    Usage:
        dheera_ai.callbacks = ["weave_otel"]

        Or manually:
        from dheera_ai.integrations.weave.weave_otel import WeaveOtelLogger
        weave_logger = WeaveOtelLogger(callback_name="weave_otel")
        dheera_ai.callbacks = [weave_logger]
    ...
    def _start_primary_span(
        ...
        from the DheeraAI LLM call spans, creating proper nesting in Weave.
        """
    ...
    def _handle_success(self, kwargs, response_obj, start_time, end_time):
        ...
        they should be managed by the user code, not ended by DheeraAI.
        """
