...
from dheera_ai.types.utils import Message as LitellmMessage
from dheera_ai.types.utils import PromptTokensDetailsWrapper, ServerToolUse
...
if TYPE_CHECKING:
    ...
    LoggingClass = DheeraAILoggingObj
else:
    LoggingClass = Any
...
class AnthropicConfig(AnthropicModelInfo, BaseConfig):
    ...
    def get_json_schema_from_pydantic_object(
        ...
        )  # Relevant issue: https://github.com/BerriAI/dheera_ai/issues/7755
    ...
    def _map_tool_helper(  # noqa: PLR0915
        ...
        if tool["type"] == "function" or tool["type"] == "custom":
            ...
            from dheera_ai.types.llms.anthropic import AnthropicToolSearchToolRegex
            ...
            from dheera_ai.types.llms.anthropic import AnthropicToolSearchToolBM25
    ...
    def _map_openai_mcp_server_tool(
        ...
        from dheera_ai.types.llms.anthropic import AnthropicMcpServerToolConfiguration
    ...
    def _map_stop_sequences(
        ...
        if isinstance(stop, str):
            if (
                stop.isspace() and dheera_ai.drop_params is True
            ...
            for v in stop:
                if (
                    v.isspace() and dheera_ai.drop_params is True
                ):  # anthropic doesn't allow whitespace characters as stop-sequences
                    continue
                new_v.append(v)
            if len(new_v) > 0:
    ...
    def transform_request(
        ...
        if (
            ...
            if dheera_ai.modify_params:
                ...
                raise dheera_ai.UnsupportedParamsError(
                    message="Anthropic doesn't support tool calling without `tools=` param specified. Pass `tools=` param OR set `dheera_ai.modify_params = True` // `dheera_ai_settings::modify_params: True` to add dummy tool to the request.",
                    model="",
        ...
        if (
            ...
            if dheera_ai.modify_params:
                ...
                dheera_ai.verbose_logger.warning(
        ...
        ## Load Config
        config = dheera_ai.AnthropicConfig.get_config(model=model)
        ...
        _dheera_ai_metadata = dheera_ai_params.get("metadata", None)
        if (
            _dheera_ai_metadata
            and isinstance(_dheera_ai_metadata, dict)
            and "user_id" in _dheera_ai_metadata
            and _dheera_ai_metadata["user_id"] is not None
            and _valid_user_id(_dheera_ai_metadata["user_id"])
        ):
            optional_params["metadata"] = {"user_id": _dheera_ai_metadata["user_id"]}

        # Remove internal DheeraAI parameters that should not be sent to Anthropic API
    ...
    def transform_parsed_response(
        ...
        if "error" in completion_response:
            ...
            if (
                ...
                and not dheera_ai.disable_add_prefix_to_prompt
            ):
        ...
        try:
            if json_mode_content_str is not None:
                ...
                if (
                    ...
                    _message = dheera_ai.Message(content=json.dumps(values))
                    return _message
                else:
                    # a lot of the times the `values` key is not present in the tool response
                    # relevant issue: https://github.com/BerriAI/dheera_ai/issues/6741
                    _message = dheera_ai.Message(content=json.dumps(args))
                    return _message
        except json.JSONDecodeError:
            # json decode error does occur, return the original tool response str
            return dheera_ai.Message(content=json_mode_content_str)
        return None
