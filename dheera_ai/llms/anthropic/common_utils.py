...
from dheera_ai.types.llms.anthropic import AllAnthropicToolsValues, AnthropicMcpServerTool, ANTHROPIC_HOSTED_TOOLS
...
class AnthropicModelInfo(BaseLLMModelInfo):
    ...
    def get_anthropic_headers(
        ...
        if tool_search_used or programmatic_tool_calling_used or input_examples_used:
            from dheera_ai.types.llms.anthropic import ANTHROPIC_TOOL_SEARCH_BETA_HEADER
            betas.add(ANTHROPIC_TOOL_SEARCH_BETA_HEADER)
        
        # Effort parameter uses a separate beta header
        if effort_used:
            from dheera_ai.types.llms.anthropic import ANTHROPIC_EFFORT_BETA_HEADER
        ...
        if is_vertex_request is True:
            ...
            if web_search_tool_used:
                from dheera_ai.types.llms.anthropic import ANTHROPIC_BETA_HEADER_VALUES
    ...
    def validate_environment(
        ...
        if api_key is None:
            raise dheera_ai.AuthenticationError(
    ...
    def get_models(
        ...
        dheera_ai_model_names = []
        for model in models:
            stripped_model_name = model["id"]
            dheera_ai_model_name = "anthropic/" + stripped_model_name
            dheera_ai_model_names.append(dheera_ai_model_name)
        return dheera_ai_model_names
...
class AnthropicTokenCounter(BaseTokenCounter):
    ...
    async def count_tokens(
        ...
        from dheera_ai.proxy.utils import count_tokens_with_anthropic_api
