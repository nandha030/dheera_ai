...
import dheera_ai
from dheera_ai.llms.anthropic.experimental_pass_through.adapters.transformation import (
    AnthropicAdapter,
)
...
class DheeraAIMessagesToCompletionTransformationHandler:
    @staticmethod
    def _prepare_completion_kwargs(
        *,
        ...
        """Prepare kwargs for dheera_ai.completion/acompletion"""
        ...
        for key, value in extra_kwargs.items():
            if (
                key == "dheera_ai_logging_obj"
                and value is not None
                and isinstance(value, DheeraAILoggingObject)
            ):
    ...
    async def async_anthropic_messages_handler(
        ...
        **kwargs,
        ...
        completion_kwargs = (
            DheeraAIMessagesToCompletionTransformationHandler._prepare_completion_kwargs(
                max_tokens=max_tokens,
                messages=messages,
                model=model,
                metadata=metadata,
        ...
        completion_response = await dheera_ai.acompletion(**completion_kwargs)
    ...
    def anthropic_messages_handler(
        ...
        **kwargs,
        ...
        if _is_async is True:
            return DheeraAIMessagesToCompletionTransformationHandler.async_anthropic_messages_handler(
                max_tokens=max_tokens,
                messages=messages,
                model=model,
                metadata=metadata,
        ...
        completion_response = dheera_ai.completion(**completion_kwargs)
