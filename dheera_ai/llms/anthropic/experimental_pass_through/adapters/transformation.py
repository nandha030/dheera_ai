...
from dheera_ai.types.utils import Choices, ModelResponse, StreamingChoices, Usage

from .streaming_iterator import AnthropicStreamWrapper

if TYPE_CHECKING:
    from dheera_ai.types.llms.anthropic import ContentBlockContentBlockDict
...
class AnthropicAdapter:
    ...
    def translate_completion_input_params(
        ...
        translated_body = (
            DheeraAIAnthropicMessagesAdapter().translate_anthropic_to_openai(
    ...
    def translate_completion_output_params(
        ...
        return DheeraAIAnthropicMessagesAdapter().translate_openai_response_to_anthropic(
            response=response
        )
...
class DheeraAIAnthropicMessagesAdapter:
    def __init__(self):
        pass
    ...
    def translate_anthropic_to_openai(
        ...
        # Pass dheera_ai proxy specific metadata
        if "dheera_ai_metadata" in anthropic_message_request:
            # metadata will be passed to dheera_ai.acompletion(), it's a dheera_ai_param
            new_kwargs["metadata"] = anthropic_message_request.pop("dheera_ai_metadata")
    ...
    def _translate_streaming_openai_chunk_to_anthropic_content_block(
        ...
        from dheera_ai.types.llms.anthropic import TextBlock, ToolUseBlock
    ...
    def translate_streaming_openai_response_to_anthropic(
        ...
        if response.choices[0].finish_reason is not None:
            ...
            if getattr(response, "usage", None) is not None:
                dheera_ai_usage_chunk: Optional[Usage] = response.usage  # type: ignore
            elif (
                hasattr(response, "_hidden_params")
                and "usage" in response._hidden_params
            ):
                dheera_ai_usage_chunk = response._hidden_params["usage"]
            else:
                dheera_ai_usage_chunk = None
            if dheera_ai_usage_chunk is not None:
                usage_delta = UsageDelta(
                    input_tokens=dheera_ai_usage_chunk.prompt_tokens or 0,
                    output_tokens=dheera_ai_usage_chunk.completion_tokens or 0,
                )
            else:
