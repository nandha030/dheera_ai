...
import dheera_ai
from dheera_ai.constants import AZURE_OPERATION_POLLING_TIMEOUT, DEFAULT_MAX_RETRIES
                ...
                if isinstance(value, list):
                    for item in value:
                        if "file_id" in item:
                            ...
                            if dheera_ai.drop_params is True:
                                pass
                            else:
                                raise dheera_ai.utils.UnsupportedParamsError(
                                    message="Azure doesn't support {}. To drop it from the call, set `dheera_ai.drop_params = True.".format(
                                        value
                                    ),
                                    status_code=400,
                                )
...
class AzureChatCompletion(BaseAzureLLM, BaseLLM):
    ...
    def completion(  # noqa: PLR0915
        ...
        try:
            ...
            if "gateway.ai.cloudflare.com" in api_base:
                ...
                data = dheera_ai.AzureOpenAIGPT5Config().transform_request(
                    model=model,
                    messages=messages,
                ...
                data = dheera_ai.AzureOpenAIConfig().transform_request(
                    model=model,
                    messages=messages,
                    optional_params=optional_params,
                    dheera_ai_params=dheera_ai_params,
    ...
    async def acompletion(
        ...
        dheera_ai_params: Optional[dict] = {},
    ):
        response = None
        try:
    ...
    def embedding(
        ...
        try:
            ...
            if max_retries is None:
                max_retries = dheera_ai.DEFAULT_MAX_RETRIES
            ## LOGGING
            logging_obj.pre_call(
                input=input,
                api_key=api_key,
    ...
    def make_sync_azure_httpx_request(
        ...
        if client is None:
            ...
            sync_handler = HTTPHandler(**_params, client=dheera_ai.client_session)  # type: ignore
        else:
    ...
    def get_headers(
        ...
        client_session = dheera_ai.client_session or httpx.Client()
        if "gateway.ai.cloudflare.com" in api_base:
