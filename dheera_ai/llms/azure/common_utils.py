...
def get_azure_ad_token_from_oidc(
    ...
    client = dheera_ai.module_level_client
...
def get_azure_ad_token(
    ...
    Args:
        dheera_ai_params: Dictionary containing authentication parameters
    ...
    azure_ad_token_provider = dheera_ai_params.get("azure_ad_token_provider")
    azure_ad_token = dheera_ai_params.get("azure_ad_token") or get_secret_str(
        "AZURE_AD_TOKEN"
    )
    tenant_id = dheera_ai_params.get("tenant_id") or os.getenv("AZURE_TENANT_ID")
    client_id = dheera_ai_params.get("client_id") or os.getenv("AZURE_CLIENT_ID")
    client_secret = dheera_ai_params.get("client_secret") or os.getenv("AZURE_CLIENT_SECRET")
    azure_username = dheera_ai_params.get("azure_username") or os.getenv("AZURE_USERNAME")
    azure_password = dheera_ai_params.get("azure_password") or os.getenv("AZURE_PASSWORD")
    scope = dheera_ai_params.get("azure_scope") or os.getenv(
    ...
    # Try to get token provider from service principal or DefaultAzureCredential
        ...
        azure_ad_token_provider is None
        and dheera_ai.enable_azure_ad_token_refresh is True
    ):
        verbose_logger.debug(
        ...
        #########################################################
        # If dheera_ai.enable_azure_ad_token_refresh is True and no other token provider is available,
...
class BaseAzureLLM(BaseOpenAILLM):
    ...
    def _try_get_default_azure_credential_provider(
        ...
        """
        from dheera_ai.types.secret_managers.get_azure_ad_token_provider import (
            AzureCredentialType,
        )
    ...
    def initialize_azure_sdk_client(
        ...
        azure_ad_token = dheera_ai_params.get("azure_ad_token")

        # dheera_ai_params sometimes contains the key, but the value is None
        # We should respect environment variables in this case
        tenant_id = self._resolve_env_var(dheera_ai_params, "tenant_id", "AZURE_TENANT_ID")
        client_id = self._resolve_env_var(dheera_ai_params, "client_id", "AZURE_CLIENT_ID")
        client_secret = self._resolve_env_var(dheera_ai_params, "client_secret", "AZURE_CLIENT_SECRET")
        azure_username = self._resolve_env_var(dheera_ai_params, "azure_username", "AZURE_USERNAME")
        azure_password = self._resolve_env_var(dheera_ai_params, "azure_password", "AZURE_PASSWORD")
        scope = self._resolve_env_var(dheera_ai_params, "azure_scope", "AZURE_SCOPE")
        if scope is None:
            scope = "https://cognitiveservices.azure.com/.default"

        max_retries = dheera_ai_params.get("max_retries")
        timeout = dheera_ai_params.get("timeout")
        if (
            not api_key
        ...
        if api_version is None:
            api_version = os.getenv(
                "AZURE_API_VERSION", dheera_ai.AZURE_DEFAULT_API_VERSION
            )
    ...
    def _init_azure_client_for_cloudflare_ai_gateway(
        ...
        tenant_id = dheera_ai_params.get("tenant_id", os.getenv("AZURE_TENANT_ID"))
        client_id = dheera_ai_params.get("client_id", os.getenv("AZURE_CLIENT_ID"))
        scope = dheera_ai_params.get(
            "azure_scope",
        ...
        if client is None:
            ...
            azure_client_params: Dict[str, Any] = {
                "api_version": api_version,
                "base_url": f"{api_base}",
                "http_client": dheera_ai.client_session,
    ...
    def _base_validate_azure_environment(
        headers: dict, dheera_ai_params: Optional[GenericDheeraAIParams]
    ) -> dict:
        ...
        azure_ad_token = get_azure_ad_token(dheera_ai_params)
        if azure_ad_token:
    ...
    def _get_base_azure_url(
        api_base: Optional[str],
        dheera_ai_params: Optional[Union[GenericDheeraAIParams, Dict[str, Any]]],
        ...
        Args:
            ...
            dheera_ai_params: The dheera_ai parameters.
        ...
        dheera_ai_params = dheera_ai_params or {}
        api_version = (
            cast(Optional[str], dheera_ai_params.get("api_version"))
    ...
    def _resolve_env_var(self, dheera_ai_params: Dict[str, Any], param_key: str, env_var_key: str) -> Optional[str]:
        ...
        dheera_ai_params may contain the key with a None value, in which case we want
        to fallback to the environment variable.
        """
        param_value = dheera_ai_params.get(param_key)
