...
from dheera_ai.llms.azure_ai.agents.transformation import (
    AzureAIAgentsConfig,
    AzureAIAgentsError,
)
...
class AzureAIAgentsHandler:
    ...
    def _build_model_response(
        ...
        from dheera_ai.types.utils import Choices, Message, Usage
    ...
    def completion(
        ...
        if client is None:
            client = _get_httpx_client(params={"ssl_verify": dheera_ai_params.get("ssl_verify", None)})
    ...
    async def acompletion(
        ...
        if client is None:
            client = get_async_httpx_client(
                llm_provider=dheera_ai.LlmProviders.AZURE_AI,
    ...
    async def acompletion_stream(
        ...
        # Use DheeraAI's async HTTP client for streaming
        client = get_async_httpx_client(
            llm_provider=dheera_ai.LlmProviders.AZURE_AI,
            params={"ssl_verify": dheera_ai_params.get("ssl_verify", None)},
        )
