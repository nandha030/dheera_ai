...
class AzureAnthropicMessagesConfig(AnthropicMessagesConfig):
    ...
    def validate_anthropic_messages_environment(
        ...
        """
        # Convert dict to GenericDheeraAIParams if needed
        if isinstance(dheera_ai_params, dict):
            if api_key and "api_key" not in dheera_ai_params:
                dheera_ai_params = {**dheera_ai_params, "api_key": api_key}
            dheera_ai_params_obj = GenericDheeraAIParams(**dheera_ai_params)
        else:
            dheera_ai_params_obj = dheera_ai_params or GenericDheeraAIParams()
            if api_key and not dheera_ai_params_obj.api_key:
                dheera_ai_params_obj.api_key = api_key

        # Use Azure authentication logic
        headers = BaseAzureLLM._base_validate_azure_environment(
            headers=headers, dheera_ai_params=dheera_ai_params_obj
        )
