...
from dheera_ai.llms.base_llm.chat.transformation import DheeraAILoggingObj
from dheera_ai.llms.openai.common_utils import drop_params_from_unprocessable_entity_error
from dheera_ai.llms.openai.openai import OpenAIConfig
from dheera_ai.llms.xai.chat.transformation import XAIChatConfig
from dheera_ai.secret_managers.main import get_secret_str
from dheera_ai.types.llms.openai import AllMessageValues
from dheera_ai.types.utils import ModelResponse, ProviderField
from dheera_ai.utils import _add_path_to_api_base, supports_tool_choice
...
class AzureAIStudioConfig(OpenAIConfig):
    ...
    def get_complete_url(
        ...
        api_version = cast(Optional[str], dheera_ai_params.get("api_version"))
    ...
    def transform_request(
        ...
        return super().transform_request(model, messages, optional_params, dheera_ai_params, headers)
    ...
    def should_retry_llm_api_inside_llm_translation_on_http_error(
        self, e: httpx.HTTPStatusError, dheera_ai_params: dict
    ) -> bool:
        should_drop_params = dheera_ai_params.get("drop_params") or dheera_ai.drop_params
        ...
        if should_drop_params and "Extra inputs are not permitted" in error_text:
            ...
            return True
        return super().should_retry_llm_api_inside_llm_translation_on_http_error(e=e, dheera_ai_params=dheera_ai_params)
    ...
    def transform_request_on_unprocessable_entity_error(self, e: httpx.HTTPStatusError, request_data: dict) -> dict:
        ...
        if "unknown field: parameter index is not a valid field" in e.response.text and _messages is not None:
            dheera_ai.remove_index_from_tool_calls(
                messages=_messages,
            )
