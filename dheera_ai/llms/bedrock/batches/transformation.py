...
class BedrockBatchesConfig(BaseAWSLLM, BaseBatchesConfig):
    ...
    def transform_create_batch_request(
        ...
        output_bucket = dheera_ai_params.get("s3_output_bucket_name") or os.getenv("AWS_S3_OUTPUT_BUCKET_NAME")
        ...
        # Get IAM role ARN
        role_arn = (
            dheera_ai_params.get("aws_batch_role_arn") 
        ...
        if not role_arn:
            raise ValueError(
                ...
                "Set 'aws_batch_role_arn' in dheera_ai_params or AWS_BATCH_ROLE_ARN env var"
            )
        ...
        job_name = self.common_utils.generate_unique_job_name(model, prefix="dheera_ai")
        output_key = f"dheera_ai-batch-outputs/{job_name}/"
        ...
        s3_encryption_key_id = (
            dheera_ai_params.get("s3_encryption_key_id")
    ...
    def transform_create_batch_response(
        ...
        """
        Transform Bedrock batch creation response to DheeraAI format.
        """
        try:
        ...
        # Get original request data from dheera_ai_params if available
        original_request = dheera_ai_params.get("original_batch_request", {})
        
        # Create DheeraAI batch object
        return DheeraAIBatch(
    ...
    def transform_retrieve_batch_response(
        ...
        """
        Transform Bedrock batch retrieval response to DheeraAI format.
        """
        from dheera_ai.types.llms.bedrock import BedrockGetBatchResponse
        try:
