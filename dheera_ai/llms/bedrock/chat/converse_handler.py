...
import dheera_ai
from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import Logging as DheeraAILoggingObject
...
class BedrockConverseLLM(BaseAWSLLM):
    ...
    async def async_streaming(
        ...
        request_data = await dheera_ai.AmazonConverseConfig()._async_transform_request(
            model=model,
            messages=messages,
        ...
        prepped = self.get_request_headers(
            credentials=credentials,
            aws_region_name=dheera_ai_params.get("aws_region_name") or "us-west-2",
    ...
    async def async_completion(
        ...
        return dheera_ai.AmazonConverseConfig()._transform_response(
            model=model,
            response=response,
    ...
    def completion(  # noqa: PLR0915
        ...
        fake_stream = dheera_ai.AmazonConverseConfig().should_fake_stream(
            fake_stream=fake_stream,
            model=model,
            stream=stream,
            custom_llm_provider="bedrock",
        ...
        _data = dheera_ai.AmazonConverseConfig()._transform_request(
            model=model,
            messages=messages,
            optional_params=optional_params,
            dheera_ai_params=dheera_ai_params,
