...
from dheera_ai.utils import add_dummy_tool, has_tool_call_blocks, supports_reasoning
...
class AmazonConverseConfig(BaseConfig):
    ...
    def get_supported_openai_params(self, model: str) -> List[str]:
        ...
        if dheera_ai.utils.supports_tool_choice(
            model=model, custom_llm_provider=self.custom_llm_provider
        ) or dheera_ai.utils.supports_tool_choice(
    ...
    def map_tool_choice_values(
        ...
        if tool_choice == "none":
            if dheera_ai.drop_params is True or drop_params is True:
                return None
            else:
                raise dheera_ai.utils.UnsupportedParamsError(
                    message="Bedrock doesn't support tool_choice={}. To drop it from the call, set `dheera_ai.drop_params = True.".format(
                        tool_choice
                    ),
                    status_code=400,
                )
            ...
            raise dheera_ai.utils.UnsupportedParamsError(
                message="Bedrock doesn't support tool_choice={}. Supported tool_choice values=['auto', 'required', json object]. To drop it from the call, set `dheera_ai.drop_params = True.".format(
                    tool_choice
                ),
                status_code=400,
            )
    ...
    def update_optional_params_with_thinking_tokens(
        ...
        """
        from dheera_ai.constants import DEFAULT_MAX_TOKENS
    ...
    def _prepare_request_params(
        ...
        # These are DheeraAI internal parameters, not API parameters
        ...
        if (
            ...
            if dheera_ai.modify_params:
                ...
                raise dheera_ai.UnsupportedParamsError(
                    message="Bedrock doesn't support tool calling without `tools=` param specified. Pass `tools=` param OR set `dheera_ai.modify_params = True` // `dheera_ai_settings::modify_params: True` to add dummy tool to the request.",
                    model="",
                    llm_provider="bedrock",
                )
    ...
    async def _async_transform_request(
        ...
        bedrock_messages = (
            await BedrockConverseMessagesProcessor._bedrock_converse_messages_pt_async(
                ...
                model=model,
                llm_provider="bedrock_converse",
                user_continue_message=dheera_ai_params.pop("user_continue_message", None),
            )
        )
