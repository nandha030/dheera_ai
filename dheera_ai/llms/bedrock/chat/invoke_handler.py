"""
TODO: DELETE FILE. Bedrock LLM is no longer used. Goto `dheera_ai/llms/bedrock/chat/invoke_transformations/base_invoke_transformation.py`
"""
...
from dheera_ai.caching.caching import InMemoryCache
from dheera_ai.dheera_ai_core_utils.core_helpers import map_finish_reason
from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import Logging
from dheera_ai.dheera_ai_core_utils.logging_utils import track_llm_api_timing
...
from dheera_ai.types.utils import GenericStreamingChunk as GChunk
from dheera_ai.types.utils import (
    ModelResponse,
    ModelResponseStream,
    StreamingChoices,
...
from dheera_ai.utils import CustomStreamWrapper, get_secret
...
from dheera_ai.llms.bedrock.chat.invoke_transformations.amazon_openai_transformation import (
...
async def make_call(
    ...
    bedrock_invoke_provider: Optional[dheera_ai.BEDROCK_INVOKE_PROVIDERS_LITERAL] = None,
    stream_chunk_size: int = 1024,
):
    try:
        if client is None:
            client = get_async_httpx_client(
                llm_provider=dheera_ai.LlmProviders.BEDROCK
        ...
        if fake_stream:
            model_response: (
                ModelResponse
            ) = dheera_ai.AmazonConverseConfig()._transform_response(
                model=model,
                response=response,
...
class BedrockLLM(BaseAWSLLM):
    ...
    def process_response(  # noqa: PLR0915
            ...
            if provider == "cohere":
                ...
                if model.startswith("anthropic.claude-3"):
                    ...
                    if (
                        ...
                        # streaming_model_response.choices = [dheera_ai.utils.StreamingChoices()]
                        streaming_choice = dheera_ai.utils.StreamingChoices()
                        ...
                        if isinstance(model_response.choices[0], dheera_ai.Choices):
                            if getattr(
        ...
        if stream and provider == "ai21":
            ...
            streaming_choice = dheera_ai.utils.StreamingChoices()
            streaming_choice.index = model_response.choices[0].index
            delta_obj = dheera_ai.utils.Delta(
        ...
        if not hasattr(model_response, "usage") or getattr(model_response, "usage", None) is None:
            ...
            prompt_tokens = int(
                bedrock_input_tokens or dheera_ai.token_counter(messages=messages)
            )

            completion_tokens = int(
                bedrock_output_tokens
                or dheera_ai.token_counter(
    ...
    def completion(  # noqa: PLR0915
        ...
        if aws_region_name is None:
            ...
            if dheera_ai_aws_region_name is not None and isinstance(
                dheera_ai_aws_region_name, str
            ):
                aws_region_name = dheera_ai_aws_region_name
        ...
        if provider == "cohere":
            if model.startswith("cohere.command-r"):
                ## LOAD CONFIG
                config = dheera_ai.AmazonCohereChatConfig().get_config()
                ...
                ## LOAD CONFIG
                config = dheera_ai.AmazonCohereConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            if model.startswith("anthropic.claude-3"):
                ...
                ## LOAD CONFIG
                config = dheera_ai.AmazonAnthropicClaudeConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
                ...
                ## LOAD CONFIG
                config = dheera_ai.AmazonAnthropicConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in inference_params
                    ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = dheera_ai.AmazonAI21Config.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = dheera_ai.AmazonMistralConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
            ...
            ## LOAD CONFIG
            config = dheera_ai.AmazonTitanConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > amazon_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            ## LOAD CONFIG
            config = dheera_ai.AmazonLlamaConfig.get_config()
            for k, v in config.items():
                if (
                    k not in inference_params
                ):  # completion(top_k=3) > anthropic_config(top_k=3) <- allows for dynamic variables to be passed in
        ...
        if client is None:
            ...
            if timeout is not None:
                if isinstance(timeout, float) or isinstance(timeout, int):
                    timeout = httpx.Timeout(timeout)
                _params["timeout"] = timeout
            client = get_async_httpx_client(params=_params, llm_provider=dheera_ai.LlmProviders.BEDROCK)  # type: ignore
        else:
    ...
    def _get_provider_from_model_path(
        model_path: str,
    ) -> Optional[dheera_ai.BEDROCK_INVOKE_PROVIDERS_LITERAL]:
        """
        ...
        if len(parts) >= 1:
            provider = parts[0]
            if provider in get_args(dheera_ai.BEDROCK_INVOKE_PROVIDERS_LITERAL):
                return cast(dheera_ai.BEDROCK_INVOKE_PROVIDERS_LITERAL, provider)
        return None
    ...
    def _handle_converse_start_event(
        ...
        if start_obj is not None:
            if "toolUse" in start_obj and start_obj["toolUse"] is not None:
                ## check tool name was formatted by dheera_ai
...
class AmazonDeepSeekR1StreamDecoder(AWSEventStreamDecoder):
    def __init__(
        ...
        super().__init__(model=model)
        from dheera_ai.llms.bedrock.chat.invoke_transformations.amazon_deepseek_transformation import (
...
class MockResponseIterator:  # for returning ai21 streaming responses
    ...
    def _handle_json_mode_chunk(
        ...
        if self.json_mode is True and tool_calls is not None:
            message = dheera_ai.AnthropicConfig()._convert_tool_response_to_message(
