...
class AmazonTitanConfig(AmazonInvokeConfig, BaseConfig):
    ...
    def _map_and_modify_arg(
        ...
        """
        filter params to fit the required provider format, drop those that don't fit if user sets `dheera_ai.drop_params = True`.
        """
        filtered_stop = None
        if "stop" in supported_params and dheera_ai.drop_params:
