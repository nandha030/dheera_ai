...
if TYPE_CHECKING:
    from dheera_ai.types.llms.bedrock import BedrockCreateBatchRequest
...
from dheera_ai.llms.base_llm.base_utils import BaseLLMModelInfo
...
def init_bedrock_client(
    ...
    dheera_ai_aws_region_name = get_secret("AWS_REGION_NAME", None)
    ...
    ssl_verify = os.getenv("SSL_VERIFY", dheera_ai.ssl_verify)
    ...
    elif dheera_ai_aws_region_name:
        region_name = dheera_ai_aws_region_name
    ...
    if (
        ...
        # aws_access_key_id is not None, assume user is trying to auth using dheera_ai.completion
...
def get_bedrock_tool_name(response_tool_name: str) -> str:
    """
    If dheera_ai formatted the input tool name, we need to convert it back to the original name.
    ...
    if response_tool_name in dheera_ai.bedrock_tool_name_mappings.cache_dict:
        response_tool_name = dheera_ai.bedrock_tool_name_mappings.cache_dict[
            response_tool_name
        ]
...
class BedrockModelInfo(BaseLLMModelInfo):
    ...
    @staticmethod
    def get_non_dheera_ai_routing_model_name(model: str) -> str:
    ...
    def get_base_model(model: str) -> str:
        ...
        model = BedrockModelInfo.get_non_dheera_ai_routing_model_name(model=model)
    ...
    def get_bedrock_route(
        ...
        alt_model = BedrockModelInfo.get_non_dheera_ai_routing_model_name(model=model)
        if (
            base_model in dheera_ai.bedrock_converse_models
            or alt_model in dheera_ai.bedrock_converse_models
        ):
            return "converse"
        return "invoke"
    ...
    def get_bedrock_provider_config_for_messages_api(
        ...
        All other routes should return None since they will go through dheera_ai.completion
        """

        #########################################################
        # Converse routes should go through dheera_ai.completion()
        if BedrockModelInfo._explicit_converse_route(model):
            return None

        #########################################################
        # This goes through dheera_ai.AmazonAnthropicClaude3MessagesConfig()
        # Since bedrock Invoke supports Native Anthropic Messages API
        #########################################################
        if "claude" in model:
            return dheera_ai.AmazonAnthropicClaudeMessagesConfig()

        #########################################################
        # These routes will go through dheera_ai.completion()
        #########################################################
        return None
...
def get_bedrock_chat_config(model: str):
    ...
    bedrock_invoke_provider = dheera_ai.BedrockLLM.get_bedrock_invoke_provider(
        model=model
    )
    base_model = BedrockModelInfo.get_base_model(model)
    ...
    if bedrock_route == "converse" or bedrock_route == "converse_like":
        return dheera_ai.AmazonConverseConfig()
    elif bedrock_route == "openai":
        return dheera_ai.AmazonBedrockOpenAIConfig()
    elif bedrock_route == "agent":
        from dheera_ai.llms.bedrock.chat.invoke_agent.transformation import (
            AmazonInvokeAgentConfig,
        )
        return AmazonInvokeAgentConfig()
    elif bedrock_route == "agentcore":
        from dheera_ai.llms.bedrock.chat.agentcore.transformation import (
    ...
    if bedrock_invoke_provider == "amazon":
        return dheera_ai.AmazonTitanConfig()
    elif bedrock_invoke_provider == "anthropic":
        if (
            base_model
            in dheera_ai.AmazonAnthropicConfig.get_legacy_anthropic_model_names()
        ):
            return dheera_ai.AmazonAnthropicConfig()
        else:
            return dheera_ai.AmazonAnthropicClaudeConfig()
    elif bedrock_invoke_provider == "meta" or bedrock_invoke_provider == "llama":
        return dheera_ai.AmazonLlamaConfig()
    elif bedrock_invoke_provider == "ai21":
        return dheera_ai.AmazonAI21Config()
    elif bedrock_invoke_provider == "cohere":
        return dheera_ai.AmazonCohereConfig()
    elif bedrock_invoke_provider == "mistral":
        return dheera_ai.AmazonMistralConfig()
    elif bedrock_invoke_provider == "deepseek_r1":
        return dheera_ai.AmazonDeepSeekR1Config()
    elif bedrock_invoke_provider == "nova":
        return dheera_ai.AmazonInvokeNovaConfig()
    elif bedrock_invoke_provider == "qwen3":
        return dheera_ai.AmazonQwen3Config()
    elif bedrock_invoke_provider == "qwen2":
        return dheera_ai.AmazonQwen2Config()
    elif bedrock_invoke_provider == "twelvelabs":
        return dheera_ai.AmazonTwelveLabsPegasusConfig()
    else:
        return dheera_ai.AmazonInvokeConfig()
...
class CommonBatchFilesUtils:
    ...
    def get_bedrock_model_id_from_dheera_ai_model(self, model: str) -> str:
        """
        Extract the actual Bedrock model ID from DheeraAI model name.

        Args:
            model: DheeraAI model name (e.g., "bedrock/anthropic.claude-3-sonnet-20240229-v1:0")
    ...
    def generate_unique_job_name(self, model: str, prefix: str = "dheera_ai") -> str:
        """
        ...
        # Example: dheera_ai-batch-claude-266c398e
    ...
    def get_s3_bucket_and_key_from_config(
        ...
        # Get bucket name
        bucket_name = (
            dheera_ai_params.get("s3_bucket_name")
