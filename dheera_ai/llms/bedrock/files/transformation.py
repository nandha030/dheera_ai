...
from dheera_ai.files.utils import FilesAPIUtils
...
from dheera_ai.llms.base_llm.files.transformation import (
    BaseFilesConfig,
    DheeraAILoggingObj,
)
...
from dheera_ai.types.utils import ExtractedFileData, LlmProviders
from dheera_ai.utils import get_llm_provider
...
class BedrockFilesConfig(BaseAWSLLM, BaseFilesConfig):
    ...
    def _get_s3_object_name_from_batch_jsonl(
        ...
        named as: dheera_ai-bedrock-files/{model}/{uuid}
        """
        ...
        object_name = f"dheera_ai-bedrock-files-{_model}-{uuid.uuid4()}.jsonl"
        return object_name
    ...
    def get_complete_file_url(
        ...
        """
        bucket_name = dheera_ai_params.get("s3_bucket_name") or os.getenv("AWS_S3_BUCKET_NAME")
        if not bucket_name:
            raise ValueError("S3 bucket_name is required. Set 's3_bucket_name' in dheera_ai_params or AWS_S3_BUCKET_NAME env var")
    ...
    def _transform_openai_jsonl_content_to_bedrock_jsonl_content(
        ...
        for idx, _openai_jsonl_content in enumerate(openai_jsonl_content):
            ...
            try:
                ...
                verbose_logger.exception(f"dheera_ai.llms.bedrock.files.transformation.py::_transform_openai_jsonl_content_to_bedrock_jsonl_content() - Error inferring custom_llm_provider - {str(e)}")
    ...
    def transform_create_file_request(
        ...
        dheera_ai_params["upload_url"] = api_base
    ...
    def _sign_s3_request(
        ...
        Reuses the exact pattern from dheera_ai/integrations/s3_v2.py
        """
        try:
            import hashlib
    ...
    def _convert_https_url_to_s3_uri(self, https_url: str) -> tuple[str, str]:
        ...
        Example:
            Input: "https://s3.us-west-2.amazonaws.com/dheera_ai-proxy/file.jsonl"
            Output: ("s3://dheera_ai-proxy/file.jsonl", "file.jsonl")
        """
        import re
    ...
    def transform_create_file_response(
        ...
        upload_url = dheera_ai_params.get("upload_url")
        file_id: str = ""
        filename: str = ""
        if upload_url:
            # Convert HTTPS S3 URL to s3:// URI format
