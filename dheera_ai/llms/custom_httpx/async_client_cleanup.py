...
async def close_dheera_ai_async_clients():
    """
    Close all cached async HTTP clients to prevent resource leaks.

    This function iterates through all cached clients in dheera_ai's in-memory cache
    ...
    cache_dict = getattr(dheera_ai.in_memory_llm_clients_cache, "cache_dict", {})
    ...
    def cleanup_wrapper():
        try:
            ...
            if loop.is_running():
                ...
                loop.create_task(close_dheera_ai_async_clients())
            else:
                # Run the cleanup coroutine
                loop.run_until_complete(close_dheera_ai_async_clients())
        except Exception:
