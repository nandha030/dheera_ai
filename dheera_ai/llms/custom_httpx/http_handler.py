...
from dheera_ai.types.llms.custom_http import *
...
if TYPE_CHECKING:
    ...
    from dheera_ai.llms.custom_httpx.aiohttp_transport import DheeraAIAiohttpTransport
else:
    LlmProviders = Any
    DheeraAILoggingObject = Any
    DheeraAIAiohttpTransport = Any
...
headers = {
    "User-Agent": f"dheera_ai/{version}",
}
...
def _create_ssl_context(
    ...
    # Set SSL_ECDH_CURVE env var or dheera_ai.ssl_ecdh_curve to 'X25519' to disable PQC
...
def get_ssl_configuration(
    ...
    Args:
        ssl_verify: SSL verification setting. Can be:
            - None: Use default from environment/dheera_ai settings
    ...
    # Get ssl_verify from environment or dheera_ai settings if not provided
    ...
    ssl_security_level = os.getenv("SSL_SECURITY_LEVEL", dheera_ai.ssl_security_level)
    ssl_ecdh_curve = os.getenv("SSL_ECDH_CURVE", dheera_ai.ssl_ecdh_curve)
...
class AsyncHTTPHandler:
    ...
    def create_client(
        ...
        # /path/to/client.pem
        cert = os.getenv("SSL_CERTIFICATE", dheera_ai.ssl_certificate)
    ...
    async def post(
        ...
        timeout: Optional[Union[float, httpx.Timeout]] = None,
        stream: bool = False,
        logging_obj: Optional[DheeraAILoggingObject] = None,
        ...
        try:
            ...
            raise dheera_ai.Timeout(
                ...
                llm_provider="dheera_ai-httpx-handler",
                headers=headers,
            )
    ...
    def _create_async_transport(
        ...
        shared_session: Optional["ClientSession"] = None,
    ) -> Optional[Union[DheeraAIAiohttpTransport, AsyncHTTPTransport]]:
        """
        - Creates a transport for httpx.AsyncClient
            - if dheera_ai.force_ipv4 is True, it will return AsyncHTTPTransport with local_address="0.0.0.0"
            - [Default] It will return AiohttpTransport
            - Users can opt out of using AiohttpTransport by setting dheera_ai.use_aiohttp_transport to False
    ...
    def _should_use_aiohttp_transport() -> bool:
        """
        AiohttpTransport is the default transport for dheera_ai.
        ...
        ########################################################
        if (
            dheera_ai.disable_aiohttp_transport is True
    ...
    def _get_ssl_connector_kwargs(
        ...
        connector_kwargs: Dict[str, Any] = {
            "local_addr": ("0.0.0.0", 0) if dheera_ai.force_ipv4 else None,
        }
    ...
    def _create_aiohttp_transport(
        ...
        ########################################################
        trust_env: bool = dheera_ai.aiohttp_trust_env
        ...
        if shared_session is not None and not shared_session.closed:
            ...
            return DheeraAIAiohttpTransport(client=shared_session)
        ...
        return DheeraAIAiohttpTransport(
...
class HTTPHandler:
    ...
    def _create_sync_transport(self) -> Optional[HTTPTransport]:
        """
        Create an HTTP transport with IPv4 only if dheera_ai.force_ipv4 is True.
        ...
        if dheera_ai.force_ipv4:
            ...
            return getattr(dheera_ai, "sync_transport", None)
...
def get_async_httpx_client(
    ...
    # dheera_ai globals being fully populated during import time.
    cache = getattr(dheera_ai, "in_memory_llm_clients_cache", None)
    if cache is None:
        from dheera_ai.caching.llm_caching_handler import LLMClientCache

        cache = LLMClientCache()
        setattr(dheera_ai, "in_memory_llm_clients_cache", cache)
