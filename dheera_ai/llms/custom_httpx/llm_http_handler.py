...
from dheera_ai.dheera_ai_core_utils.realtime_streaming import RealTimeStreaming
...
from dheera_ai.llms.base_llm.base_model_iterator import MockResponseIterator
...
from dheera_ai.llms.base_llm.search.transformation import BaseSearchConfig, SearchResponse
...
from dheera_ai.responses.streaming_iterator import (
...
from dheera_ai.types.responses.main import DeleteResponseResult
...
class BaseLLMHTTPHandler:
        ...
        for i in range(max(max_retry_on_unprocessable_entity_error, 1)):
            try:
                ...
                should_retry = provider_config.should_retry_llm_api_inside_llm_translation_on_http_error(
                    e=e, dheera_ai_params=dheera_ai_params
                )
    ...
    async def async_completion(
        ...
        if client is None:
            ...
            async_httpx_client = get_async_httpx_client(
                llm_provider=dheera_ai.LlmProviders(custom_llm_provider),
                params={"ssl_verify": dheera_ai_params.get("ssl_verify", None)},
    ...
    def completion(
        ...
        provider_config = (
            ...
            or ProviderConfigManager.get_provider_chat_config(
                model=model, provider=dheera_ai.LlmProviders(custom_llm_provider)
            )
        )
    ...
    def make_sync_call(
        ...
        if client is None or not isinstance(client, HTTPHandler):
            sync_httpx_client = _get_httpx_client(
                {
                    "ssl_verify": dheera_ai_params.get("ssl_verify", None),
                }
            )
        else:
            sync_httpx_client = client
    ...
    def _prepare_ocr_request(
        ...
        Returns: (headers, complete_url, data, files)
        """
        from dheera_ai.llms.base_llm.ocr.transformation import OCRRequestData
        headers = provider_config.validate_environment(
            api_key=api_key,
            api_base=api_base,
            headers=headers or {},
    ...
    async def async_anthropic_messages_handler(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            async_httpx_client = get_async_httpx_client(
                llm_provider=dheera_ai.LlmProviders.ANTHROPIC
            )
        else:
            async_httpx_client = client
        ...
        provider_specific_header = cast(
            Optional[dheera_ai.types.utils.ProviderSpecificHeader],
        ...
        additional_drop_params = dheera_ai_params.get("additional_drop_params")
        ...
        headers, signed_json_body = anthropic_messages_provider_config.sign_request(
            ...
            optional_params=dict(
                dheera_ai_params
            ),  # dynamic aws_* params are passed under dheera_ai_params
    ...
    def response_api_handler(
        ...
        _is_async: bool = False,
        fake_stream: bool = False,
        dheera_ai_metadata: Optional[Dict[str, Any]] = None,
        shared_session: Optional["ClientSession"] = None,
    ) -> Union[
        ResponsesAPIResponse,
        BaseResponsesAPIStreamingIterator,
        ...
        if _is_async:
            ...
            return self.async_response_api_handler(
                ...
                client=client if isinstance(client, AsyncHTTPHandler) else None,
                fake_stream=fake_stream,
                dheera_ai_metadata=dheera_ai_metadata,
                shared_session=shared_session,
            )
    ...
    async def async_delete_response_api_handler(
        ...
        headers = responses_api_provider_config.validate_environment(
            headers=extra_headers or {}, model="None", dheera_ai_params=dheera_ai_params
        )
    ...
    def create_file(
        ...
        # Store the upload URL in dheera_ai_params for the transformation method
        dheera_ai_params_with_url = dict(dheera_ai_params)
        dheera_ai_params_with_url["upload_url"] = api_base

        return provider_config.transform_create_file_response(
            model=None,
            raw_response=upload_response,
            logging_obj=logging_obj,
            dheera_ai_params=dheera_ai_params_with_url,
        )
    ...
    def create_batch(
        ...
        return provider_config.transform_create_batch_response(
            ...
            logging_obj=logging_obj,
            dheera_ai_params=dheera_ai_params_with_request,
        )
    ...
    def retrieve_batch(
        ...
        transformed_request = provider_config.transform_retrieve_batch_request(
            batch_id=batch_id,
            optional_params=dheera_ai_params,
            dheera_ai_params=dheera_ai_params,
        )
    ...
    def video_generation_handler(
        ...
        headers = video_generation_provider_config.validate_environment(
            api_key=api_key or dheera_ai_params.get("api_key", None),
    ...
    async def async_container_create_handler(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            async_httpx_client = get_async_httpx_client(
                llm_provider=dheera_ai.LlmProviders.OPENAI,
                params={"ssl_verify": dheera_ai_params.get("ssl_verify", None)},
            )
        else:
            async_httpx_client = client
    ...
    async def async_vector_store_search_handler(
        ...
        headers = vector_store_provider_config.validate_environment(
            headers=extra_headers or {}, dheera_ai_params=dheera_ai_params
        )
        ...
        all_optional_params: Dict[str, Any] = dict(dheera_ai_params)
    ...
    def generate_content_handler(
        ...
        When _is_async=True, returns a coroutine instead of making the call directly.
        """
        from dheera_ai.google_genai.streaming_iterator import (
        ...
        try:
            if stream:
                ...
                return GoogleGenAIGenerateContentStreamingIterator(
                    ...
                    logging_obj=logging_obj,
                    generate_content_provider_config=generate_content_provider_config,
                    dheera_ai_metadata=dheera_ai_metadata or {},
                    custom_llm_provider=custom_llm_provider,
                    request_body=data,
                )
            else:
