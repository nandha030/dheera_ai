...
class ElevenLabsTextToSpeechConfig(BaseTextToSpeechConfig):
    ...
    def _resolve_voice_id(
        ...
        if mapped_voice is None:
            raise ValueError(
                "ElevenLabs voice_id is required. Pass `voice` when calling `dheera_ai.speech()`."
            )
    ...
    def _add_elevenlabs_specific_params(
        ...
        reserved_kwarg_keys = set(all_dheera_ai_params) | {
    ...
    def get_complete_url(
        ...
        voice_id = dheera_ai_params.get(self.ELEVENLABS_VOICE_ID_KEY)
        ...
        query_params = dheera_ai_params.get(self.ELEVENLABS_QUERY_PARAMS_KEY, {})
        if query_params:
