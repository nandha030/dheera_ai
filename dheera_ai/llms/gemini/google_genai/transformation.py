...
class GoogleGenAIConfig(BaseGoogleGenAIGenerateContentConfig, VertexLLM):
    ...
    def validate_environment(
        ...
        model: str,
        dheera_ai_params: Optional[Union[GenericDheeraAIParams, dict]],
    ) -> dict:
        default_headers = {
            "Content-Type": "application/json",
        }
        # Use the passed api_key first, then fall back to dheera_ai_params and environment
        gemini_api_key = api_key or self._get_google_ai_studio_api_key(
            dict(dheera_ai_params or {})
        )
    ...
    def _get_google_ai_studio_api_key(self, dheera_ai_params: dict) -> Optional[str]:
        return (
            dheera_ai_params.pop("api_key", None)
            or dheera_ai_params.pop("gemini_api_key", None)
    ...
    def _get_common_auth_components(
        ...
        """
        vertex_credentials = self.get_vertex_ai_credentials(dheera_ai_params)
        vertex_project = self.get_vertex_ai_project(dheera_ai_params)
        vertex_location = self.get_vertex_ai_location(dheera_ai_params)
    ...
    def _build_final_headers_and_url(
        ...
        """
        gemini_api_key = self._get_google_ai_studio_api_key(dheera_ai_params)
    ...
    def sync_get_auth_token_and_url(
        ...
        vertex_credentials, vertex_project, vertex_location = (
            self._get_common_auth_components(dheera_ai_params)
        )
