...
Implements the transformation between DheeraAI's unified vector store API
...
from dheera_ai.llms.gemini.common_utils import (
    GeminiError,
    GeminiModelInfo,
...
class GeminiVectorStoreConfig(BaseVectorStoreConfig):
    ...
    def validate_environment(
        ...
        if dheera_ai_params:
            api_key = dheera_ai_params.get("api_key") or get_api_key_from_env()
            if api_key:
                self._cached_api_key = api_key
        
        return headers

    def get_complete_url(self, api_base: Optional[str], dheera_ai_params: dict) -> str:
        """
    ...
    def transform_search_vector_store_request(
        ...
        # Get model from dheera_ai_params or use default
        # Note: File Search requires gemini-2.5-flash or later
        model = dheera_ai_params.get("model") or "gemini-2.5-flash"
        if model and model.startswith("gemini/"):
            model = model.replace("gemini/", "")

        # Get API key - Gemini requires it as a query parameter
        api_key = dheera_ai_params.get("api_key") or GeminiModelInfo.get_api_key()
        if not api_key:
        ...
        dheera_ai_logging_obj.model_call_details["query"] = query
        dheera_ai_logging_obj.model_call_details["vector_store_id"] = vector_store_id
    ...
    def transform_search_vector_store_response(
        ...
        try:
            ...
            query = dheera_ai_logging_obj.model_call_details.get("query", "")
