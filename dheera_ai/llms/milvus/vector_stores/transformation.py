...
class MilvusVectorStoreConfig(BaseVectorStoreConfig):
    ...
    Supports vector search with embeddings generated via dheera_ai.embeddings.
    """
    ...
    def validate_environment(
        ...
        if dheera_ai_params is not None:
            api_key = dheera_ai_params.api_key or get_secret_str("MILVUS_API_KEY")

        if not api_key:
            raise ValueError(
                "MILVUS_API_KEY is not set. Either set it in the dheera_ai_params or set the MILVUS_API_KEY environment variable."
            )
    ...
    def get_complete_url(
        ...
        if not api_base:
            raise ValueError(
                "Milvus API base URL is required. Set MILVUS_API_BASE environment variable or pass api_base in dheera_ai_params."
            )
    ...
    def transform_search_vector_store_request(
        ...
        Generates embeddings using dheera_ai.embeddings and constructs Azure AI Search request
        """
        # Convert query to string if it's a list
        if isinstance(query, list):
            query = " ".join(query)

        # Get embedding model from dheera_ai_params (required)
        embedding_model = dheera_ai_params.get("dheera_ai_embedding_model")
        if not embedding_model:
            raise ValueError(
                "embedding_model is required in dheera_ai_params for Milvus. You can call any dheera_ai embedding model."
                "Example: dheera_ai_params['embedding_model'] = 'azure/text-embedding-3-large'"
            )

        embedding_config = dheera_ai_params.get("dheera_ai_embedding_config", {})
        if not embedding_config:
            raise ValueError(
                "embedding_config is required in dheera_ai_params for Milvus. You can call any dheera_ai embedding model."
                "Example: dheera_ai_params['embedding_config'] = {'api_base': 'https://krris-mh44uf7y-eastus2.cognitiveservices.azure.com/', 'api_key': 'os.environ/AZURE_API_KEY', 'api_version': '2025-09-01'}"
            )

        # Get top_k (number of results to return)
        # Generate embedding for the query using dheera_ai.embeddings
        try:
        ...
        #########################################################
        dheera_ai_logging_obj.model_call_details["input"] = query
        dheera_ai_logging_obj.model_call_details["embedding_model"] = embedding_model
    ...
    def transform_search_vector_store_response(
        ...
        try:
            ...
            # Try to get text_field from optional_params first, then dheera_ai_params
            optional_params = dheera_ai_logging_obj.model_call_details.get(
                "optional_params", {}
            )
            text_field = optional_params.get("milvus_text_field", "")

            # Fallback to dheera_ai_params if not in optional_params

            if not text_field:
                text_field = dheera_ai_logging_obj.model_call_details.get(
                    "dheera_ai_params", {}
            ...
            return VectorStoreSearchResponse(
                ...
                search_query=dheera_ai_logging_obj.model_call_details.get("input", ""),
                data=search_results,
            )
