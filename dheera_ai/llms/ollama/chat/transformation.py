...
from dheera_ai.types.llms.ollama import (
...
class OllamaChatConfig(BaseConfig):
    ...
    def transform_request(
        ...
        dheera_ai_params["function_name"] = function_name
        ...
        for m in messages:
            if isinstance(
                m, BaseModel
            ):  # avoid message serialization issues - https://github.com/BerriAI/dheera_ai/issues/5319
    ...
    def transform_response(
        ...
        if (
            ...
            and dheera_ai_params.get("function_name") is not None
        ):
                ...
                tool_calls=[
                    {
                        ...
                        "function": {
                            "name": function_call.get(
                                "name", dheera_ai_params.get("function_name")
                            ),
            ...
            _message = dheera_ai.Message(**response_json_message)
            model_response.choices[0].message = _message  # type: ignore
        model_response.created = int(time.time())
        model_response.model = "ollama_chat/" + model
        prompt_tokens = response_json.get("prompt_eval_count", dheera_ai.token_counter(messages=messages))  # type: ignore
        completion_tokens = response_json.get(
            "eval_count",
            dheera_ai.token_counter(text=response_json["message"]["content"]),
        )
        setattr(
            model_response,
            "usage",
