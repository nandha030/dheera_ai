...
Translations handled by DheeraAI:
- modalities: image => drop param (if user opts in to dropping param)  
- role: system ==> translate to role 'user' 
- streaming => faked by DheeraAI 
...
class OpenAIOSeriesConfig(OpenAIGPTConfig):
                ...
                if temperature_value == 1:
                    ...
                    if dheera_ai.drop_params is True or drop_params is True:
                        ...
                        raise dheera_ai.utils.UnsupportedParamsError(
                            message="O-series models don't support temperature={}. Only temperature=1 is supported. To drop unsupported openai params from the call, set `dheera_ai.drop_params = True`".format(
                                temperature_value
                            ),
                            status_code=400,
                        )
    ...
    def is_model_o_series_model(self, model: str) -> bool:
        ...
        return model in dheera_ai.open_ai_chat_completion_models and any(
