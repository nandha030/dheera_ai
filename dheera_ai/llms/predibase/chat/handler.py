...
from dheera_ai.types.utils import DheeraAILoggingBaseClass
from dheera_ai.utils import Choices, CustomStreamWrapper, Message, ModelResponse, Usage
...
class PredibaseChatCompletion:
    ...
    def output_parser(self, generated_text: str):
        ...
        Initial issue that prompted this - https://github.com/BerriAI/dheera_ai/issues/763
        """
    ...
    def process_response(  # noqa: PLR0915
        ...
        stream: bool,
        logging_obj: DheeraAILoggingBaseClass,
        ...
        prompt_tokens = 0
        try:
            prompt_tokens = dheera_ai.token_counter(messages=messages)
        except Exception:
    ...
    def completion(
        ...
        headers = dheera_ai.PredibaseConfig().validate_environment(
            api_key=api_key,
            headers=headers,
            messages=messages,
        ...
        ## Load Config
        config = dheera_ai.PredibaseConfig.get_config()
        ...
        ### SYNC STREAMING
        if stream is True:
            response = dheera_ai.module_level_client.post(
                completion_url,
                headers=headers,
    ...
    async def async_completion(
        ...
        async_handler = get_async_httpx_client(
            llm_provider=dheera_ai.LlmProviders.PREDIBASE,
        ...
        try:
            ...
            for exception in dheera_ai.DHEERA_AI_EXCEPTION_TYPES:
