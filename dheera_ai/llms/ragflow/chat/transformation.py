...
class RAGFlowConfig(OpenAIConfig):
    ...
    def get_complete_url(
        ...
        Args:
            ...
            dheera_ai_params: DheeraAI parameters (may contain api_base)
            stream: Whether streaming is enabled
            
        Returns:
            Complete URL for the API call
        """
        # Get api_base from multiple sources: input param, dheera_ai_params, environment, or global dheera_ai setting
        if dheera_ai_params and hasattr(dheera_ai_params, 'api_base') and dheera_ai_params.api_base:
            api_base = api_base or dheera_ai_params.api_base
        ...
        if api_base is None:
            raise ValueError("api_base is required for RAGFlow provider. Set it via api_base parameter, RAGFLOW_API_BASE environment variable, or dheera_ai.api_base")
    ...
    def _get_openai_compatible_provider_info(
        ...
        # The model name will be stored in dheera_ai_params for use in requests
        _, _, actual_model = self._parse_ragflow_model(model)
        
        # Get api_base from multiple sources: input param, environment, or global dheera_ai setting
        dynamic_api_base = (
            api_base
            or dheera_ai.api_base
            or get_secret("RAGFLOW_API_BASE")
        ...
        # Get api_key from multiple sources: input param, environment, or global dheera_ai setting
        dynamic_api_key = (
            api_key
    ...
    def validate_environment(
        ...
        Args:
            ...
            optional_params: Optional parameters
            dheera_ai_params: DheeraAI parameters (may contain api_key)
            api_key: API key (from input params)
        ...
        """
        # Use api_key from dheera_ai_params if available, otherwise fall back to other sources
        if dheera_ai_params and hasattr(dheera_ai_params, 'api_key') and dheera_ai_params.api_key:
            api_key = api_key or dheera_ai_params.api_key
        
        # Get api_key from multiple sources: input param, dheera_ai_params, environment, or global dheera_ai setting
        api_key = (
            api_key
            or dheera_ai.api_key
            or get_secret_str("RAGFLOW_API_KEY")
        ...
        try:
            _, _, actual_model = self._parse_ragflow_model(model)
            # Store the actual model name in dheera_ai_params for use in transform_request
            dheera_ai_params["_ragflow_actual_model"] = actual_model
        except ValueError:
    ...
    def transform_request(
        ...
        Args:
            ...
            messages: Chat messages
            optional_params: Optional parameters
            dheera_ai_params: DheeraAI parameters (may contain _ragflow_actual_model)
            headers: Request headers
            
        Returns:
            Transformed request dictionary
        """
        # Get the actual model name from dheera_ai_params if available
        actual_model = dheera_ai_params.get("_ragflow_actual_model")
        ...
        return super().transform_request(
            actual_model, messages, optional_params, dheera_ai_params, headers
        )
