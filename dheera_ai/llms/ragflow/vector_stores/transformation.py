...
class RAGFlowVectorStoreConfig(BaseVectorStoreConfig):
    ...
    def get_auth_credentials(
        ...
        if api_key is None:
            raise ValueError("api_key is required (set RAGFLOW_API_KEY env var or pass in dheera_ai_params)")
        return {
            "headers": {
    ...
    def validate_environment(
        ...
        if api_key is None:
            raise ValueError("RAGFLOW_API_KEY is required (set env var or pass in dheera_ai_params)")
    ...
    def get_complete_url(
        ...
        - api_base in dheera_ai_params
        - Default: http://localhost:9380
        """
        api_base = (
            api_base
            or dheera_ai_params.get("api_base")
    ...
    def transform_create_vector_store_request(
        ...
        Maps DheeraAI params to RAGFlow dataset creation parameters.
