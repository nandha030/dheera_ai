...
class SagemakerLLM(BaseAWSLLM):
    ...
    def completion(  # noqa: PLR0915
        ...
        ## Load Config
        config = dheera_ai.SagemakerConfig.get_config()
        ...
        try:
            ...
            if "Inference Component Name header is required" in error_message:
                error_message += "\n pass in via `dheera_ai.completion(..., model_id={InferenceComponentName})`"
        ...
        try:
            if client is None:
                client = get_async_httpx_client(
                    llm_provider=dheera_ai.LlmProviders.SAGEMAKER
    ...
    async def async_streaming(
        ...
        if model_id is not None:  # Fixes https://github.com/BerriAI/dheera_ai/issues/8889
            prepared_request.headers.update(
                {"X-Amzn-SageMaker-Inference-Component": model_id}
            )
