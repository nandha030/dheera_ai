...
Handles the transformation between DheeraAI's OpenAI-compatible format and
...
from dheera_ai.llms.vertex_ai.agent_engine.sse_iterator import (
...
class VertexAgentEngineConfig(BaseConfig, VertexBase):
    ...
    def get_complete_url(
        ...
        # Get project and location from dheera_ai_params or environment
        vertex_project = self.safe_get_vertex_ai_project(dheera_ai_params)
        vertex_location = self.safe_get_vertex_ai_location(dheera_ai_params) or "us-central1"
        ...
        if not resource_path.startswith("projects/"):
            if not vertex_project:
                raise ValueError(
                    ...
                    "Set via dheera_ai_params['vertex_project'] or VERTEXAI_PROJECT env var."
                )
    ...
    def _get_auth_headers(
        ...
        vertex_credentials = self.safe_get_vertex_ai_credentials(dheera_ai_params)
        vertex_project = self.safe_get_vertex_ai_project(dheera_ai_params)
    ...
    def _get_user_id(self, optional_params: dict) -> str:
        ...
        if user_id:
            return user_id
        # Generate a user ID
        return f"dheera_ai-user-{str(uuid.uuid4())[:8]}"
    ...
    def validate_environment(
        ...
        auth_headers = self._get_auth_headers(optional_params, dheera_ai_params)
    ...
    def _calculate_usage(
        ...
        """Calculate token usage using DheeraAI's token counter."""
        try:
    ...
    def transform_response(
        ...
        """
        Transform Vertex Agent Engine response to DheeraAI ModelResponse format.
