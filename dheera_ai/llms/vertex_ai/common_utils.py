...
from dheera_ai.constants import DEFAULT_MAX_RECURSE_DEPTH
from dheera_ai.dheera_ai_core_utils.prompt_templates.common_utils import unpack_defs
from dheera_ai.llms.base_llm.base_utils import BaseLLMModelInfo, BaseTokenCounter
...
from dheera_ai.types.llms.vertex_ai import PartType, Schema
from dheera_ai.types.utils import TokenCountResponse
from dheera_ai.utils import supports_response_schema, supports_system_messages
...
def get_vertex_ai_model_route(
    model: str, dheera_ai_params: Optional[dict] = None
) -> VertexAIModelRoute:
    """
    ...
    Args:
        ...
        dheera_ai_params: Optional dheera_ai parameters dict that may contain base_model for routing
    ...
    # Check base_model in dheera_ai_params for gemini override
    if dheera_ai_params and dheera_ai_params.get("base_model") is not None:
        if "gemini" in dheera_ai_params["base_model"]:
    ...
    if model.isdigit() and dheera_ai_params and dheera_ai_params.get("api_base"):
        return VertexAIModelRoute.GEMINI
...
def get_supports_system_message(
    ...
    try:
        ...
        if dheera_ai.VertexGeminiConfig._is_model_gemini_spec_model(model):
            supports_system_message = True
    except Exception as e:
        verbose_logger.warning(
            "Unable to identify if system message supported. Defaulting to 'False'. Received error message - {}\nAdd it here - https://github.com/BerriAI/dheera_ai/blob/main/model_prices_and_context_window.json".format(
                str(e)
            )
        )
...
def _get_vertex_url(
    ...
    model = dheera_ai.VertexGeminiConfig.get_model_for_vertex_ai_url(model=model)
...
def _get_gemini_url(
    ...
    if mode == "chat":
        ...
        raise ValueError(
            "DheeraAI's `gemini/` route does not support image generation yet. Let us know if you need this feature by opening an issue at https://github.com/BerriAI/dheera_ai/issues"
        )
    else:
...
def _check_text_in_content(parts: List[PartType]) -> bool:
    ...
    check that user_content has 'text' parameter.
        ...
        - Relevant Issue: https://github.com/BerriAI/dheera_ai/issues/5515
    """
...
def _filter_anyof_fields(schema_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
    When anyof is present, only keep the anyof field and its contents - otherwise VertexAI will throw an error - https://github.com/BerriAI/dheera_ai/issues/11164
...
def construct_target_url(
    ...
    Handle cachedContent scenario - https://github.com/BerriAI/dheera_ai/issues/5460
...
def is_global_only_vertex_model(model: str) -> bool:
    ...
    """
    from dheera_ai.utils import get_supported_regions
...
class VertexAITokenCounter(BaseTokenCounter):
    ...
    async def count_tokens(
        ...
        count_tokens_params_request = copy.deepcopy(
            deployment.get("dheera_ai_params", {})
        )
        ...
        if VertexAIPartnerModels.is_vertex_partner_model(model_to_use):
            ...
            # Extract vertex-specific params from dheera_ai_params
            ...
            result = await partner_models_handler.count_tokens(
                ...
                dheera_ai_params=count_tokens_params_request,
                vertex_project=vertex_project,
                vertex_location=vertex_location,
            ...
            from dheera_ai.llms.vertex_ai.count_tokens.handler import VertexAITokenCounter
