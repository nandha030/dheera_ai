...
from dheera_ai.types.llms.vertex_ai import GcsBucketResponse
...
class VertexAIFilesConfig(VertexBase, BaseFilesConfig):
    ...
    def validate_environment(
        ...
        if not api_key:
            api_key, _ = self.get_access_token(
                credentials=dheera_ai_params.get("vertex_credentials"),
                project_id=dheera_ai_params.get("vertex_project"),
            )
            if not api_key:
    ...
    def _get_gcs_object_name_from_batch_jsonl(
        ...
        named as: dheera_ai-vertex-{model}-{uuid}
        """
        _model = openai_jsonl_content[0].get("body", {}).get("model", "")
        if "publishers/google/models" not in _model:
            _model = f"publishers/google/models/{_model}"
        object_name = f"dheera_ai-vertex-files/{_model}/{uuid.uuid4()}"
        return object_name
    ...
    def get_complete_file_url(
        ...
        """
        bucket_name = dheera_ai_params.get("bucket_name") or os.getenv("GCS_BUCKET_NAME")
        if not bucket_name:
