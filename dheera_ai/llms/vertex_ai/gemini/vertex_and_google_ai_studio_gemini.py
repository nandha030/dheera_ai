...
from dheera_ai.types.llms.anthropic import AnthropicThinkingParam
from dheera_ai.types.llms.gemini import BidiGenerateContentServerMessage
...
if TYPE_CHECKING:
    ...
    from dheera_ai.types.utils import ModelResponseStream, StreamingChoices
...
class VertexGeminiConfig(VertexAIBaseConfig, BaseConfig):
    ...
    def map_tool_choice_values(
        ...
        if tool_choice == "none":
            ...
            raise dheera_ai.utils.UnsupportedParamsError(
                message="VertexAI doesn't support tool_choice={}. Supported tool_choice values=['auto', 'required', json object]. To drop it from the call, set `dheera_ai.drop_params = True.".format(
                    tool_choice
                ),
                status_code=400,
            )
    ...
    def _transform_computer_use_config(
        ...
        Args:
            computer_use_config: The computer use configuration from DheeraAI
    ...
    def _extract_google_maps_retrieval_config(
        ...
        Args:
            google_maps_config: The googleMaps tool configuration from DheeraAI
    ...
    def _map_function(  # noqa: PLR0915
        ...
        for tool in value:
            ...
            # Handle tools with 'type' field (OpenAI spec compliance) Ignore this field -> https://github.com/BerriAI/dheera_ai/issues/14644#issuecomment-3342061838
            elif "type" in tool:
            ...
            if tool_name and (
                ...
                verbose_logger.warning(
                    "Invalid tool={}. Use `dheera_ai.set_verbose` or `dheera_ai --detailed_debug` to see raw request."
                )
    ...
    def map_openai_params(  # noqa: PLR0915
        ...
        for param, value in non_default_params.items():
            if param == "temperature":
                ...
                if value is False and not (
                    drop_params or dheera_ai.drop_params
        ...
        if dheera_ai.vertex_ai_safety_settings is not None:
            optional_params["safety_settings"] = dheera_ai.vertex_ai_safety_settings
            ...
            if "functionCall" in part:
                ...
                if is_function_call is True:
                    ...
                    if thought_signature:
                        ...
                        if dheera_ai.enable_preview_features:
    ...
    def is_candidate_token_count_inclusive(usage_metadata: UsageMetadata) -> bool:
        ...
        Addresses - https://github.com/BerriAI/dheera_ai/pull/10141#discussion_r2052272035
        """
    ...
    def _create_streaming_choice(
        ...
        """
        from dheera_ai.types.utils import Delta, StreamingChoices
    ...
    def transform_response(
        ...
        try:
            ...
            raise VertexAIError(
                message="Received={}, Error converting to valid response block={}. File an issue if dheera_ai error - https://github.com/BerriAI/dheera_ai/issues".format(
...
async def make_call(
    ...
    if client is None:
        client = get_async_httpx_client(
            llm_provider=dheera_ai.LlmProviders.VERTEX_AI,
        )
...
class VertexLLM(VertexBase):
    ...
    async def async_completion(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            client = get_async_httpx_client(
                params=_async_client_params, llm_provider=dheera_ai.LlmProviders.VERTEX_AI
            )
        else:
...
class ModelResponseIterator:
    ...
    def handle_valid_json_chunk(self, chunk: str) -> Optional["ModelResponseStream"]:
        ...
        try:
            ...
            # See: https://github.com/BerriAI/dheera_ai/issues/16562
    ...
    def handle_accumulated_json_chunk(
        ...
        chunk = dheera_ai.CustomStreamWrapper._strip_sse_data_from_chunk(chunk) or ""
