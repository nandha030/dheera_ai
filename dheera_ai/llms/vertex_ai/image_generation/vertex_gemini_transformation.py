...
from dheera_ai.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import VertexLLM
from dheera_ai.secret_managers.main import get_secret_str
from dheera_ai.types.llms.openai import (
    AllMessageValues,
    OpenAIImageGenerationOptionalParams,
)
from dheera_ai.types.utils import ImageObject, ImageResponse, ImageUsage, ImageUsageInputTokensDetails
...
class VertexAIGeminiImageGenerationConfig(BaseImageGenerationConfig, VertexLLM):
    ...
    def _resolve_vertex_project(self) -> Optional[str]:
        return (
            ...
            or getattr(dheera_ai, "vertex_project", None)
    ...
    def _resolve_vertex_location(self) -> Optional[str]:
        return (
            ...
            or getattr(dheera_ai, "vertex_location", None)
    ...
    def _resolve_vertex_credentials(self) -> Optional[str]:
        return (
            ...
            or getattr(dheera_ai, "vertex_credentials", None)
    ...
    def get_complete_url(
        ...
        # First check dheera_ai_params (where vertex_ai_project/vertex_ai_location are passed)
        # then fall back to environment variables and other sources
        vertex_project = self.safe_get_vertex_ai_project(dheera_ai_params) or self._resolve_vertex_project()
        vertex_location = self.safe_get_vertex_ai_location(dheera_ai_params) or self._resolve_vertex_location()
    ...
    def validate_environment(
        ...
        _api_base = dheera_ai_params.get("api_base") or api_base
        if _api_base is not None:
            return headers
        
        # First check dheera_ai_params (where vertex_ai_project/vertex_ai_credentials are passed)
        # then fall back to environment variables and other sources
        vertex_project = self.safe_get_vertex_ai_project(dheera_ai_params) or self._resolve_vertex_project()
        vertex_credentials = self.safe_get_vertex_ai_credentials(dheera_ai_params) or self._resolve_vertex_credentials()
    ...
    def transform_image_generation_response(
        ...
        """
        Transform Gemini image generation response to dheera_ai ImageResponse format
        """
        try:
