...
Uses:
- dheera_ai.files.acreate_file for uploading files to GCS
- Vertex AI RAG Engine REST API for importing files into corpus (via httpx)

Key differences from OpenAI:
- Files must be uploaded to GCS first (via dheera_ai.files.acreate_file)
...
if TYPE_CHECKING:
    ...
    from dheera_ai.types.rag import RAGIngestOptions
...
class VertexAIRAGIngestion(BaseRAGIngestion):
    ...
    Uses dheera_ai.files.acreate_file for GCS upload, then imports into RAG corpus.
    ...
    async def _upload_file_to_gcs(
        ...
        """
        Upload file to GCS using dheera_ai.files.acreate_file.
        ...
        # Set GCS_BUCKET_NAME env var for dheera_ai.files.create_file
        ...
        try:
            # Create file tuple for dheera_ai.files.acreate_file
            file_tuple = (filename, file_content, content_type)

            verbose_logger.debug(
                f"Uploading file to GCS via dheera_ai.files.acreate_file: {filename} "
                f"(bucket: {self.gcs_bucket})"
            )

            # Upload to GCS using DheeraAI's file upload
    ...
    def _build_transformation_config(self) -> Any:
        ...
        from dheera_ai.types.rag import RAGChunkingStrategy
    ...
    async def store(
        ...
        Vertex AI workflow:
        1. Upload file to GCS via dheera_ai.files.acreate_file
