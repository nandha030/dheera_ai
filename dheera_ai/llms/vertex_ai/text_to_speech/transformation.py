...
from dheera_ai.types.llms.vertex_ai import VERTEX_CREDENTIALS_TYPES
from dheera_ai.types.llms.vertex_ai_text_to_speech import (
...
class VertexAITextToSpeechConfig(BaseTextToSpeechConfig, VertexBase):
    ...
    def dispatch_text_to_speech(
        ...
        **kwargs: Any,
        ...
        vertex_credentials = self.safe_get_vertex_ai_credentials(dheera_ai_params_dict)
        vertex_project = self.safe_get_vertex_ai_project(dheera_ai_params_dict)
        vertex_location = self.safe_get_vertex_ai_location(dheera_ai_params_dict)
        ...
        # Store credentials in dheera_ai_params for use in transform methods
    ...
    def _validate_vertex_input(
        ...
        if use_ssml:
            ...
            # DheeraAI will auto-detect if text is in ssml format
    ...
    def transform_text_to_speech_request(
        ...
        """
        # Get Vertex AI credentials from dheera_ai_params
        vertex_credentials: Optional[VERTEX_CREDENTIALS_TYPES] = dheera_ai_params.get(
            "vertex_credentials"
        )
        vertex_project: Optional[str] = dheera_ai_params.get("vertex_project")
        ...
        auth_header, _ = self._get_token_and_url(
            ...
            vertex_credentials=vertex_credentials,
            vertex_project=vertex_project,
            vertex_location=dheera_ai_params.get("vertex_location"),
            stream=False,
            custom_llm_provider="vertex_ai_beta",
        ...
        # 1. dheera_ai_params by dispatch method
        # 2. optional_params by map_openai_params
        voice_dict = (
            dheera_ai_params.get("vertex_voice_dict")
