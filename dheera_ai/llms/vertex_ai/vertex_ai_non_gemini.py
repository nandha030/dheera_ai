...
from dheera_ai.llms.bedrock.common_utils import ModelResponseIterator
from dheera_ai.llms.custom_httpx.http_handler import _DEFAULT_TTL_FOR_HTTPX_CLIENTS
from dheera_ai.types.llms.vertex_ai import *
from dheera_ai.utils import CustomStreamWrapper, ModelResponse, Usage
...
def _get_client_from_cache(client_cache_key: str):
    return dheera_ai.in_memory_llm_clients_cache.get_cache(client_cache_key)
...
def completion(  # noqa: PLR0915
    ...
    try:
        ...
        raise VertexAIError(
            status_code=400,
            message="vertexai import failed please run `pip install google-cloud-aiplatform`. This is required for the 'vertex_ai/' route on DheeraAI",
        )
    ...
    try:
        ...
        ## Load Config
        config = dheera_ai.VertexAIConfig.get_config()
        ...
        if (
        ...
        elif model in dheera_ai.vertex_text_models:
        ...
        elif model in dheera_ai.vertex_code_text_models:
            ...
            fake_stream = True
        elif model in dheera_ai.vertex_code_chat_models:  # vertex_code_llm_models
        ...
        ## RESPONSE OBJECT
        if isinstance(completion_response, dheera_ai.Message):
        ...
        ## CALCULATING USAGE
        if model in dheera_ai.vertex_language_models and response_obj is not None:
            ...
            # init prompt tokens
            # this block attempts to get usage from response_obj if it exists, if not it uses the dheera_ai token counter
        ...
        if isinstance(e, VertexAIError):
            raise e
        raise dheera_ai.APIConnectionError(
