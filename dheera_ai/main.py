...
# client must be imported from dheera_ai as it's a decorator used at function definition time
from dheera_ai import client

# Other utils are imported directly to avoid circular imports
from dheera_ai.utils import exception_type, get_dheera_ai_params, get_optional_params

# Logging is imported lazily when needed to avoid loading dheera_ai_logging at import time
if TYPE_CHECKING:
...
from dheera_ai.exceptions import DheeraAIUnknownProvider
...
from dheera_ai.dheera_ai_core_utils.audio_utils.utils import (
    calculate_request_duration,
    get_audio_file_for_health_check,
)
from dheera_ai.dheera_ai_core_utils.dd_tracing import tracer
from dheera_ai.dheera_ai_core_utils.get_provider_specific_headers import (
    ProviderSpecificHeaderUtils,
)
from dheera_ai.dheera_ai_core_utils.health_check_utils import (
...
from dheera_ai.dheera_ai_core_utils.mock_functions import (
    mock_embedding,
...
from dheera_ai.llms.base_llm import BaseConfig, BaseImageGenerationConfig
...
from dheera_ai.llms.vertex_ai.common_utils import (
    VertexAIModelRoute,
    get_vertex_ai_model_route,
)
from dheera_ai.realtime_api.main import _realtime_health_check
...
from dheera_ai.types.utils import RawRequestTypedDict, StreamingChoices
...
from .dheera_ai_core_utils.core_helpers import safe_deep_copy
from .dheera_ai_core_utils.fallback_utils import (
    async_completion_with_fallbacks,
    completion_with_fallbacks,
)
from .dheera_ai_core_utils.prompt_templates.common_utils import (
    get_completion_messages,
    update_messages_with_model_file_ids,
)
from .dheera_ai_core_utils.prompt_templates.factory import (
    custom_prompt,
...
from .dheera_ai_core_utils.streaming_chunk_builder_utils import ChunkProcessor
...
class DheeraAI:
    def __init__(
        ...
        *,
        ...
        max_retries: Optional[int] = dheera_ai.num_retries,
...
async def acompletion(
    ...
    **kwargs,
    ...
    #########################################################
    #########################################################
    dheera_ai_logging_obj = kwargs.get("dheera_ai_logging_obj", None)
    if isinstance(dheera_ai_logging_obj, DheeraAILoggingObj) and (
        dheera_ai_logging_obj.should_run_prompt_management_hooks(
        ...
        (
            ...
            messages,
            _,
        ) = await dheera_ai_logging_obj.async_get_chat_completion_prompt(
            model=model,
            messages=messages,
            non_default_params=kwargs,
            prompt_id=kwargs.get("prompt_id", None),
        ...
        # relevant issue: https://github.com/BerriAI/dheera_ai/issues/11404
        #########################################################
    ...
    fallbacks = fallbacks or dheera_ai.model_fallbacks
    if fallbacks is not None:
        ...
        if response is None:
            raise Exception(
                "No response from fallbacks. Got none. Turn on `dheera_ai.set_verbose=True` to see more details."
            )
        return response
    ...
    try:
        ...
        if (
            ...
            response = dheera_ai.OpenAITextCompletionConfig().convert_to_chat_model_response_object(
                response_object=response,
                model_response_object=dheera_ai.ModelResponse(),
            )
...
def _handle_mock_potential_exceptions(
    ...
    if isinstance(mock_response, Exception):
    ...
    elif isinstance(mock_response, str) and mock_response == "dheera_ai.RateLimitError":
        ...
        and mock_response == "dheera_ai.ContextWindowExceededError"
    ):
        ...
        isinstance(mock_response, str)
        and mock_response == "dheera_ai.InternalServerError"
    ):
...
def mock_completion(
    ...
    **kwargs,
    ...
    Returns:
        dheera_ai.ModelResponse: A ModelResponse simulating a completion response with the specified model, messages, and mock response.
    ...
    try:
        ...
        if isinstance(mock_response, str) and mock_response.startswith(
            ...
            mock_response = dheera_ai.MockException(
        ...
        if n is None:
            ...
            _all_choices = []
            for i in range(n):
                _choice = dheera_ai.utils.Choices(
                    index=i,
        ...
        try:
            _, custom_llm_provider, _, _ = dheera_ai.utils.get_llm_provider(model=model)
...
def completion(  # type: ignore # noqa: PLR0915
    ...
    **kwargs,
    ...
    if not skip_mcp_handler and tools:
        from dheera_ai.responses.mcp.chat_completions_handler import (
    ...
    ### COPY MESSAGES ### - related issue https://github.com/BerriAI/dheera_ai/discussions/4489
    ...
    dheera_ai_params = {}  # used to prevent unbound var errors
    ## PROMPT MANAGEMENT HOOKS ##
    ...
    if isinstance(dheera_ai_logging_obj, DheeraAILoggingObj) and (
        ...
        (
            ...
            messages,
            optional_params,
        ) = dheera_ai_logging_obj.get_chat_completion_prompt(
            model=model,
            messages=messages,
    ...
    try:
        ...
        logging: DheeraAILoggingObj = cast(DheeraAILoggingObj, dheera_ai_logging_obj)
        fallbacks = fallbacks or dheera_ai.model_fallbacks
        if fallbacks is not None:
            return completion_with_fallbacks(**args)
        if model_list is not None:
            deployments = [
                m["dheera_ai_params"] for m in model_list if m["model_name"] == model
            ]
            return dheera_ai.batch_completion_models(deployments=deployments, **args)
        if dheera_ai.model_alias_map and model in dheera_ai.model_alias_map:
            model = dheera_ai.model_alias_map[
                model
            ]  # update the model to the actual value if an alias has been passed in
        model_response = ModelResponse()
        setattr(model_response, "usage", dheera_ai.Usage())
        if (
        ...
        if dheera_ai.add_function_to_prompt and optional_params.get(
        ...
        # For logging - save the values of the dheera_ai-specific params passed in
        dheera_ai_params = get_dheera_ai_params(
            ...
            custom_llm_provider=custom_llm_provider,
            api_base=api_base,
            dheera_ai_call_id=kwargs.get("dheera_ai_call_id", None),
            model_alias_map=dheera_ai.model_alias_map,
            ...
            dheera_ai_session_id=kwargs.get("dheera_ai_session_id"),
            ...
            use_dheera_ai_proxy=kwargs.get("use_dheera_ai_proxy", False),
            api_version=api_version,
            ...
            max_retries=max_retries,
            timeout=timeout,
            dheera_ai_request_debug=kwargs.get("dheera_ai_request_debug", False),
        )
        cast(DheeraAILoggingObj, logging).update_environment_variables(
            model=model,
            user=user,
        ...
        ## RESPONSES API BRIDGE LOGIC ## - check if model has 'mode: responses' in dheera_ai.model_cost map
        model_info, model = responses_api_bridge_check(
            model=model, custom_llm_provider=custom_llm_provider
        )

        if model_info.get("mode") == "responses":
            from dheera_ai.completion_extras import responses_api_bridge
        ...
        if custom_llm_provider == "azure":
            ...
            api_base = api_base or dheera_ai.api_base or get_secret("AZURE_API_BASE")
            ...
            azure_ad_token_provider = dheera_ai_params.get(
                "azure_ad_token_provider", None
            )

            headers = headers or dheera_ai.headers
            ...
            if dheera_ai.AzureOpenAIO1Config().is_o_series_model(model=model):
                ## LOAD CONFIG - if set
                config = dheera_ai.AzureOpenAIO1Config.get_config()
                ...
                ## LOAD CONFIG - if set
                config = dheera_ai.AzureOpenAIConfig.get_config()
                for k, v in config.items():
                    if (
                        k not in optional_params
                    ):  # completion(top_k=3) > azure_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            api_base = api_base or dheera_ai.api_base or get_secret_str("AZURE_API_BASE")
            ...
            if azure_ai_route == "agents":
                from dheera_ai.llms.azure_ai.agents import AzureAIAgentsConfig
            ...
            # Check if this is a Claude model - route to Azure Anthropic handler
                ...
                response = azure_anthropic_chat_completions.completion(
                    ...
                    api_base=api_base,
                    acompletion=acompletion,
                    custom_prompt_dict=dheera_ai.custom_prompt_dict,
                    model_response=model_response,
                    print_verbose=print_verbose,
                    optional_params=optional_params,
                    dheera_ai_params=dheera_ai_params,
            ...
            in dheera_ai.openai_text_completion_compatible_providers
            ...
            ## LOAD CONFIG - if set
            config = dheera_ai.OpenAITextCompletionConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
                ):  # completion(top_k=3) > openai_text_config(top_k=3) <- allows for dynamic variables to be passed in
                    optional_params[k] = v
            if dheera_ai.organization:
                openai.organization = dheera_ai.organization
            ...
            if (
                ...
                _response = dheera_ai.OpenAITextCompletionConfig().convert_to_chat_model_response_object(
            ...
            api_key = (
                api_key
                or dheera_ai.api_key  # for deepinfra/perplexity/anyscale/friendliai we check in get_llm_provider and pass in the api key from there
                or dheera_ai.groq_key
            ...
            ## LOAD CONFIG - if set
            config = dheera_ai.GroqChatConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
            ...
            headers = headers or dheera_ai.headers
            ## LOAD CONFIG - if set
            config = dheera_ai.GenAIHubOrchestrationConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
                ):  # completion(top_k=3) > openai_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            if (
                dheera_ai.enable_preview_features and metadata is not None
            ...
            ## LOAD CONFIG - if set
            config = dheera_ai.OpenAIConfig.get_config()
            for k, v in config.items():
                if (
                    k not in optional_params
                ):  # completion(top_k=3) > openai_config(top_k=3) <- allows for dynamic variables to be passed in
            ...
            api_key = api_key or dheera_ai.api_key or get_secret("MISTRAL_API_KEY")
            api_base = (
                api_base
                or dheera_ai.api_base
            ...
            or model in dheera_ai.replicate_models
        ):
            # Setting the relevant API KEY for replicate, replicate defaults to using os.environ.get("REPLICATE_API_TOKEN")
            replicate_key = (
                api_key
                or dheera_ai.replicate_key
                or dheera_ai.api_key
            ...
            custom_prompt_dict = custom_prompt_dict or dheera_ai.custom_prompt_dict
            ...
            disable_url_suffix = get_secret_bool("DHEERA_AI_ANTHROPIC_DISABLE_URL_SUFFIX")
            if (
                ...
                verbose_logger.debug(
                    "DHEERA_AI_ANTHROPIC_DISABLE_URL_SUFFIX is set, skipping /v1/complete suffix"
                )
            ...
            if (
                ...
                verbose_logger.debug(
                    "DHEERA_AI_ANTHROPIC_DISABLE_URL_SUFFIX is set, skipping /v1/messages suffix"
                )
            ...
            model_response = aleph_alpha.completion(
                ...
                logger_fn=logger_fn,
                encoding=_get_encoding(),
                default_max_tokens_to_sample=dheera_ai.max_tokens,
            ...
            huggingface_key = (
                api_key
                or dheera_ai.huggingface_key
                or os.environ.get("HF_TOKEN")
                or os.environ.get("HUGGINGFACE_API_KEY")
                or dheera_ai.api_key
            )
            hf_headers = headers or dheera_ai.headers
            response = base_llm_http_handler.completion(
                model=model,
                messages=messages,
                headers=hf_headers,
            ...
            api_key = (
                api_key or get_secret_str("COMPACTIFAI_API_KEY") or dheera_ai.api_key
            )
            ...
            api_key = (
                api_key
                or dheera_ai.api_key  # for databricks we check in get_llm_provider and pass in the api key from there
                or dheera_ai.databricks_key
            ...
            api_key = (
                api_key
                or dheera_ai.api_key
                or dheera_ai.openrouter_key
                or get_secret("OPENROUTER_API_KEY")
                or get_secret("OR_API_KEY")
            )

            openrouter_site_url = get_secret("OR_SITE_URL") or "https://dheera_ai.ai"
            ...
            _headers = headers or dheera_ai.headers
            if _headers:
            ...
            ## Load Config
            config = dheera_ai.OpenrouterConfig.get_config()
            for k, v in config.items():
                if k == "extra_body":
            ...
            api_key = (
                api_key or dheera_ai.api_key or get_secret("VERCEL_AI_GATEWAY_API_KEY")
            )

            vercel_site_url = get_secret("VERCEL_SITE_URL") or "https://dheera_ai.ai"
            ...
            ## Load Config
            config = dheera_ai.VercelAIGatewayConfig.get_config()
            for k, v in config.items():
                if k == "extra_body":
            ...
            or (model in dheera_ai.together_ai_models)
        ):
            """
            ...
            api_base = api_base or dheera_ai.api_base or get_secret("GEMINI_API_BASE")
            ...
            response = vertex_chat_completion.completion(  # type: ignore
                ...
                dheera_ai_params=dheera_ai_params,  # type: ignore
                logger_fn=logger_fn,
                encoding=_get_encoding(),
            ...
            api_base = api_base or dheera_ai.api_base or get_secret("VERTEXAI_API_BASE")

            new_params = safe_deep_copy(optional_params or {})
            model_route = get_vertex_ai_model_route(
                model=model, dheera_ai_params=dheera_ai_params
            )
            ...
            if model_route == VertexAIModelRoute.PARTNER_MODELS:
                ...
                # Update dheera_ai_params with vertex credentials
                dheera_ai_params["vertex_project"] = vertex_ai_project
                dheera_ai_params["vertex_location"] = vertex_ai_location
                dheera_ai_params["vertex_credentials"] = vertex_credentials
            ...
            tenant_id = (
                ...
                or dheera_ai.predibase_tenant_id
            ...
            api_key = api_key or dheera_ai.api_key or get_secret("CODESTRAL_API_KEY")

            text_completion_model_response = dheera_ai.TextCompletionResponse(
                stream=stream
            )
            ...
            api_base = dheera_ai.api_base or api_base
            response = base_llm_http_handler.completion(
                model=model,
                stream=stream,
                messages=messages,
            ...
            api_key = (
                api_key
                or dheera_ai.cloudflare_api_key
                or dheera_ai.api_key
        ...
        elif custom_llm_provider == "petals" or model in dheera_ai.petals_models:
            api_base = api_base or dheera_ai.api_base
            ...
            if stream is True:  ## [BETA]
                ...
                return response
            response = model_response
        elif custom_llm_provider == "snowflake" or model in dheera_ai.snowflake_models:
            try:
                client = (
        ...
        elif custom_llm_provider == "ovhcloud" or model in dheera_ai.ovhcloud_models:
            api_key = (
                api_key
                or dheera_ai.ovhcloud_key
            ...
            url = dheera_ai.api_base or api_base or ""
            if url is None or url == "":
                raise ValueError(
                    "api_base not set. Set api_base or dheera_ai.api_base for custom endpoints"
                )

            """
            assume input to custom LLM api bases follow this format:
            resp = dheera_ai.module_level_client.post(
                api_base,
                json={
            ...
            custom_llm_provider in dheera_ai._custom_providers
        ):  # Assume custom LLM provider
            # Get the Custom Handler
            custom_handler: Optional[CustomLLM] = None
            for item in dheera_ai.custom_provider_map:
                if item["provider"] == custom_llm_provider:
                    custom_handler = item["custom_handler"]

            if custom_handler is None:
                raise DheeraAIUnknownProvider(
                    model=model, custom_llm_provider=custom_llm_provider
                )
            ...
            (
                ...
                api_base=api_base or dheera_ai.api_base,
                api_key=api_key or dheera_ai.api_key,
            )
...
def completion_with_retries(*args, **kwargs):
    """
    Executes a dheera_ai.completion() with 3 retries
    """
    try:
        import tenacity
    except Exception as e:
...
def embedding(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    - dheera_ai_call_id: The call ID for dheera_ai logging.
    - dheera_ai_logging_obj: The dheera_ai logging object.
    ...
    default_params = openai_params + dheera_ai_params
    non_default_params = {
    ...
    logging: DheeraAILoggingObj = dheera_ai_logging_obj  # type: ignore
    ...
    try:
        ...
        if azure is True or custom_llm_provider == "azure":
            ...
            model in dheera_ai.open_ai_embedding_models
            or custom_llm_provider == "openai"
            or custom_llm_provider == "together_ai"
            or custom_llm_provider == "nvidia_nim"
            or custom_llm_provider == "dheera_ai_proxy"
        ):
            api_base = (
                api_base
                or dheera_ai.api_base
            ...
            api_base = api_base or dheera_ai.api_base or get_secret("DATABRICKS_API_BASE")  # type: ignore
            ...
            api_base = (
                api_base or dheera_ai.api_base or get_secret_str("OPENAI_LIKE_API_BASE")
            )
            ...
            gemini_api_key = api_key or get_api_key_from_env() or dheera_ai.api_key

            api_base = api_base or dheera_ai.api_base or get_secret_str("GEMINI_API_BASE")
            ...
            api_key = api_key or dheera_ai.api_key or get_secret_str("MISTRAL_API_KEY")
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
                api_base=api_base,
            ...
            api_key = (
                api_key or dheera_ai.api_key or get_secret_str("FIREWORKS_AI_API_KEY")
            )
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
            ...
            api_key = api_key or dheera_ai.api_key or get_secret_str("NEBIUS_API_KEY")
            api_base = (
                api_base
                or dheera_ai.api_base
            ...
            api_key = api_key or dheera_ai.api_key or get_secret_str("WANDB_API_KEY")
            api_base = (
                api_base
                or dheera_ai.api_base
            ...
            api_key = api_key or dheera_ai.api_key or get_secret_str("SAMBANOVA_API_KEY")
            api_base = (
                api_base
                or dheera_ai.api_base
            ...
            api_key = api_key or dheera_ai.api_key or get_secret_str("OVHCLOUD_API_KEY")
            api_base = (
                api_base
                or dheera_ai.api_base
                or get_secret_str("OVHCLOUD_API_BASE")
        ...
        ## LOGGING
        dheera_ai_logging_obj.post_call(
            input=input,
            api_key=api_key,
            original_response=str(e),
        )
    ...
    *args, **kwargs
    ...
    try:
        ...
        if (
            ...
            text_completion_response = dheera_ai.utils.DheeraAIResponseObjectHandler.convert_chat_to_text_completion(
...
def text_completion(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    # default values for all optional params are none, dheera_ai only passes them to the llm when they are set to non None values
    ...
    if isinstance(prompt, list) and len(prompt) > 0 and isinstance(prompt[0], str):
        ...
        raise Exception(
            f"Unmapped prompt format. Your prompt is neither a list of strings nor a string. prompt={prompt}. File an issue - https://github.com/BerriAI/dheera_ai/issues"
        )
    ...
    if _model is not None and (
        ...
        if _model not in dheera_ai.open_ai_chat_completion_models:
    ...
    text_completion_response = (
        dheera_ai.utils.DheeraAIResponseObjectHandler.convert_chat_to_text_completion(
            response=response,
            text_completion_response=text_completion_response,
        )
    )
...
async def aadapter_completion(
    *, adapter_id: str, **kwargs
    ...
    try:
        ...
        for item in dheera_ai.adapters:
            if item["id"] == adapter_id:
                translation_obj = item["adapter"]

        if translation_obj is None:
            raise ValueError(
                "No matching adapter given. Received 'adapter_id'={}, dheera_ai.adapters={}".format(
                    adapter_id, dheera_ai.adapters
                )
            )
...
def moderation(
    ...
    return dheera_ai.utils.DheeraAIResponseObjectHandler.convert_to_moderation_response(
...
async def amoderation(
    ...
    **kwargs,
    ...
    # update dheera_ai_logging_obj with environment variables
    custom_llm_provider = custom_llm_provider or dheera_ai.LlmProviders.OPENAI.value
    if dheera_ai_logging_obj is not None:
...
def transcription(
    ...
    **kwargs,
    ...
    if dheera_ai_logging_obj:
        dheera_ai_logging_obj.model_call_details["client"] = str(client)

    if max_retries is None:
        max_retries = openai.DEFAULT_MAX_RETRIES

    model_response = dheera_ai.utils.TranscriptionResponse()
    ...
    if custom_llm_provider == "azure":
        ...
        api_version = (
            api_version or dheera_ai.api_version or get_secret_str("AZURE_API_VERSION")
        )
        ...
        custom_llm_provider in dheera_ai.openai_compatible_providers
    ):
        api_base = (
            api_base
            or dheera_ai.api_base
        ...
        api_key = api_key or dheera_ai.api_key or dheera_ai.openai_key or get_secret("OPENAI_API_KEY")  # type: ignore
...
def speech(  # noqa: PLR0915
    ...
    **kwargs,
    ...
    if timeout is None:
        timeout = dheera_ai.request_timeout

    if max_retries is None:
        max_retries = dheera_ai.num_retries or openai.DEFAULT_MAX_RETRIES
    dheera_ai_params_dict = get_dheera_ai_params(**kwargs)
    ...
    logging_obj: DheeraAILoggingObj = cast(
        DheeraAILoggingObj, kwargs.get("dheera_ai_logging_obj")
    )
    ...
    if (
        ...
        api_key = (
            api_key
            or dheera_ai.api_key  # for deepinfra/perplexity/anyscale we check in get_llm_provider and pass in the api key from there
            or dheera_ai.openai_key
            or get_secret("OPENAI_API_KEY")
        )  # type: ignore
        ...
        if model.startswith("speech/"):
            ...
            response = azure_config.dispatch_text_to_speech(  # type: ignore
                ...
                voice=voice,
                optional_params=optional_params,
                dheera_ai_params_dict=dheera_ai_params_dict,
            ...
            api_base = api_base or dheera_ai.api_base or get_secret("AZURE_API_BASE")  # type: ignore

            api_version = api_version or dheera_ai.api_version or get_secret("AZURE_API_VERSION")  # type: ignore
        ...
        if api_base is not None:
            dheera_ai_params_dict["api_base"] = api_base
        if api_key is not None:
            dheera_ai_params_dict["api_key"] = api_key
        ...
        generic_optional_params = GenericDheeraAIParams(**kwargs)
        ...
        # Store Vertex AI specific params in dheera_ai_params_dict
        dheera_ai_params_dict.update(
            {
    ...
    if response is None:
        raise Exception(
            "Unable to map the custom llm provider={} to a known provider={}.".format(
                custom_llm_provider, dheera_ai.provider_list
            )
        )
    return response
...
async def ahealth_check(
    ...
    """
    from dheera_ai.dheera_ai_core_utils.cached_imports import get_dheera_ai_logging_class
    ...
    Logging = get_dheera_ai_logging_class()
    ...
    model_params["dheera_ai_logging_obj"] = dheera_ai_logging_obj
    model_params = (
    ...
    try:
        ...
        if model in dheera_ai.model_cost and mode is None:
            mode = dheera_ai.model_cost[model].get("mode")
...
####### HELPER FUNCTIONS ################
## Set verbose to true -> ```dheera_ai.set_verbose = True```
...
def config_completion(**kwargs):
    if dheera_ai.config_path is not None:
        config_args = read_config_args(dheera_ai.config_path)
        # overwrite any args passed in with config args
        return completion(**kwargs, **config_args)
    else:
        raise ValueError(
            "No config path set, please set a config path using `dheera_ai.config_path = 'path/to/config.json'`"
        )
...
def stream_chunk_builder(  # noqa: PLR0915
    ...
    try:
        ...
        if isinstance(
            chunks[0]["choices"][0], dheera_ai.utils.TextChoices
        ):  # route to the text completion logic
        ...
        # See: https://github.com/BerriAI/dheera_ai/issues/17737
        ...
        if dheera_ai.include_cost_in_streaming_usage and logging_obj is not None:
            setattr(
        ...
        verbose_logger.exception(
            "dheera_ai.main.py::stream_chunk_builder() - Exception occurred - {}".format(
                str(e)
            )
        )
        raise dheera_ai.APIError(
