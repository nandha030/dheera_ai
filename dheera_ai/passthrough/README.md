...
response = dheera_ai.llm_passthrough_route(
    ...
    api_key=None,
    json={
        "model": "swapped-for-dheera_ai-model",
        "input": "Hello, world!",
    }
)
...
```bash
dheera_ai proxy --config config.yaml
...
See [VLLMModelInfo](https://github.com/BerriAI/dheera_ai/blob/main/dheera_ai/llms/vllm/common_utils.py) for an example.
