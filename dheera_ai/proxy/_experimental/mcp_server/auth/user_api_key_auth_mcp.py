...
from dheera_ai.proxy._types import DheeraAI_TeamTable, SpecialHeaders, UserAPIKeyAuth
...
class MCPRequestHandler:
    ...
    1. Authentication via DheeraAI API keys
    ...
    DHEERA_AI_API_KEY_HEADER_NAME_PRIMARY = SpecialHeaders.custom_dheera_ai_api_key.value
    DHEERA_AI_API_KEY_HEADER_NAME_SECONDARY = SpecialHeaders.openai_authorization.value

    # This is the header to use if you want DheeraAI to use this header for authenticating to the MCP server
    DHEERA_AI_MCP_AUTH_HEADER_NAME = SpecialHeaders.mcp_auth.value

    DHEERA_AI_MCP_SERVERS_HEADER_NAME = SpecialHeaders.mcp_servers.value

    DHEERA_AI_MCP_ACCESS_GROUPS_HEADER_NAME = SpecialHeaders.mcp_access_groups.value
    ...
    async def process_mcp_request(
        ...
        dheera_ai_api_key = (
            MCPRequestHandler.get_dheera_ai_api_key_from_headers(headers) or ""
        )
        ...
        mcp_servers_header = headers.get(
            MCPRequestHandler.DHEERA_AI_MCP_SERVERS_HEADER_NAME
        )
        ...
        # elif dheera_ai_api_key == "":
        #     from fastapi import HTTPException

        #     raise HTTPException(
        #         status_code=401,
        #         detail="DheeraAI API key is missing. Please add it or use OAuth authentication.",
        #         headers={
        ...
        #     )
            ...
            validated_user_api_key_auth = await user_api_key_auth(
                api_key=dheera_ai_api_key, request=request
            )
    ...
    def _get_mcp_auth_header_from_headers(headers: Headers) -> Optional[str]:
        """
        Get the header passed to DheeraAI to pass to downstream MCP servers

        By default dheera_ai will check for the header `x-mcp-auth` by setting one of the following:
    ...
    def _get_mcp_server_auth_headers_from_headers(
        ...
        for header_name, header_value in headers.items():
            if header_name.lower().startswith(prefix):
                ...
                if (
                    header_name.lower()
                    == MCPRequestHandler.DHEERA_AI_MCP_ACCESS_GROUPS_HEADER_NAME.lower()
                    or header_name.lower()
                    == MCPRequestHandler.DHEERA_AI_MCP_SERVERS_HEADER_NAME.lower()
                ):
                    continue
    ...
    def _get_mcp_client_side_auth_header_name() -> str:
        ...
        MCP_CLIENT_SIDE_AUTH_HEADER_NAME: str = (
            MCPRequestHandler.DHEERA_AI_MCP_AUTH_HEADER_NAME
        )
        if get_secret_str("DHEERA_AI_MCP_CLIENT_SIDE_AUTH_HEADER_NAME") is not None:
            MCP_CLIENT_SIDE_AUTH_HEADER_NAME = (
                get_secret_str("DHEERA_AI_MCP_CLIENT_SIDE_AUTH_HEADER_NAME")
    ...
    @staticmethod
    def get_dheera_ai_api_key_from_headers(headers: Headers) -> Optional[str]:
        """
        Get the Litellm API key from the headers using case-insensitive lookup

        1. Check if `x-dheera_ai-api-key` is in the headers
        ...
        api_key = headers.get(MCPRequestHandler.DHEERA_AI_API_KEY_HEADER_NAME_PRIMARY)
        if api_key:
            return api_key

        auth_header = headers.get(
            MCPRequestHandler.DHEERA_AI_API_KEY_HEADER_NAME_SECONDARY
        )
        if auth_header:
            return auth_header
    ...
    async def _get_key_object_permission(
        ...
        from dheera_ai.proxy.auth.auth_checks import get_object_permission
    ...
    async def _get_team_object_permission(
        ...
        team_obj: Optional[DheeraAI_TeamTable] = await get_team_object(
    ...
    def get_mcp_access_groups_from_headers(headers: Headers) -> Optional[List[str]]:
        ...
        mcp_access_groups_header = headers.get(
            MCPRequestHandler.DHEERA_AI_MCP_ACCESS_GROUPS_HEADER_NAME
        )
