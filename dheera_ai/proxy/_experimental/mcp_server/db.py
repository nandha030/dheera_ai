...
async def get_all_mcp_servers(
    ...
    try:
        ...
        verbose_proxy_logger.debug(
            "dheera_ai.proxy._experimental.mcp_server.db.py::get_all_mcp_servers - {}".format(
                str(e)
            )
        )
        return []
...
) -> Optional[DheeraAI_MCPServerTable]:
    """
...
async def get_mcp_servers(
    ...
    final_mcp_servers: List[DheeraAI_MCPServerTable] = []
    for _mcp_server in _mcp_servers:
        final_mcp_servers.append(DheeraAI_MCPServerTable(**_mcp_server.model_dump()))
...
) -> List[DheeraAI_ObjectPermissionTable]:
    """
