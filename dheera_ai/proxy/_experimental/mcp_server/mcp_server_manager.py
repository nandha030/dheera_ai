...
from dheera_ai.types.mcp import MCPAuth, MCPStdioConfig
from dheera_ai.types.mcp_server.mcp_server_manager import (
    MCPInfo,
    MCPOAuthMetadata,
    MCPServer,
)
...
class MCPServerManager:
    ...
    async def load_servers_from_config(
        ...
        Args:
            ...
            mcp_aliases: Optional dictionary mapping aliases to server names from dheera_ai_settings
        """
    ...
    def _register_openapi_tools(self, spec_path: str, server: MCPServer, base_url: str):
        ...
        """
        from dheera_ai.proxy._experimental.mcp_server.openapi_to_mcp_generator import (
            build_input_schema,
    ...
    def remove_server(self, mcp_server: DheeraAI_MCPServerTable):
        """
    ...
    async def add_update_server(self, mcp_server: DheeraAI_MCPServerTable):
        try:
    ...
    def _create_during_hook_task(
        ...
        from dheera_ai.types.mcp import MCPDuringCallRequestObject
    ...
    async def _call_regular_mcp_tool(
        ...
        # the exact case of server alias/name (e.g., '1dheera_aiagcgateway' vs '1DheeraAIAGCGateway')
    ...
    async def _add_mcp_servers_from_db_to_in_memory_registry(self):
        from dheera_ai.proxy._experimental.mcp_server.db import get_all_mcp_servers
    ...
    def get_public_mcp_servers(self) -> List[MCPServer]:
        ...
        if dheera_ai.public_mcp_servers is None:
            return servers
        for server_id in dheera_ai.public_mcp_servers:
            server = self.get_mcp_server_by_id(server_id)
            if server:
                servers.append(server)
        return servers
    ...
    async def get_all_mcp_servers_with_health_and_teams(
        ...
        list_mcp_servers: List[DheeraAI_MCPServerTable] = []
