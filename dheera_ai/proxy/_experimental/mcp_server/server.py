...
from dheera_ai.proxy._experimental.mcp_server.utils import (
    DHEERA_AI_MCP_SERVER_DESCRIPTION,
    DHEERA_AI_MCP_SERVER_NAME,
    DHEERA_AI_MCP_SERVER_VERSION,
)
from dheera_ai.proxy._types import UserAPIKeyAuth
from dheera_ai.types.mcp import MCPAuth
from dheera_ai.types.mcp_server.mcp_server_manager import MCPInfo, MCPServer
from dheera_ai.types.utils import StandardLoggingMCPToolCall
from dheera_ai.utils import client

# Check if MCP is available
# "mcp" requires python 3.10 or higher, but several dheera_ai users use python 3.8
# We're making this conditional import to avoid breaking users who use python 3.8.
# TODO: Make this a util function for dheera_ai client usage
...
if MCP_AVAILABLE:
    ...
    from dheera_ai.proxy._experimental.mcp_server.auth.dheera_ai_auth_handler import (
        MCPAuthenticatedUser,
    )
    from dheera_ai.proxy._experimental.mcp_server.mcp_server_manager import (
        global_mcp_server_manager,
    )
    from dheera_ai.proxy._experimental.mcp_server.sse_transport import SseServerTransport
    ...
    # required dependency for `dheera_ai` pip package
    ######################################################
    ...
    server: Server = Server(
        name=DHEERA_AI_MCP_SERVER_NAME,
        version=DHEERA_AI_MCP_SERVER_VERSION,
    )
    ...
    async def call_mcp_tool(
        ...
        **kwargs: Any,
        ...
        if dheera_ai_logging_obj:
            ...
            dheera_ai_logging_obj.model = f"MCP: {name}"
        ...
        #########################################################
        if dheera_ai_logging_obj:
            dheera_ai_logging_obj.post_call(original_response=response)
            end_time = datetime.now()
            await dheera_ai_logging_obj.async_post_mcp_tool_call_hook(
            ...
            dheera_ai_logging_obj.call_type = CallTypes.call_mcp_tool.value
            ...
            await dheera_ai_logging_obj.async_success_handler(
    ...
    async def _handle_managed_mcp_tool(
        ...
        oauth2_headers: Optional[Dict[str, str]] = None,
        raw_headers: Optional[Dict[str, str]] = None,
        dheera_ai_logging_obj: Optional[Any] = None,
    ) -> CallToolResult:
    ...
    app = FastAPI(
        title=DHEERA_AI_MCP_SERVER_NAME,
        description=DHEERA_AI_MCP_SERVER_DESCRIPTION,
        version=DHEERA_AI_MCP_SERVER_VERSION,
        lifespan=lifespan,
    )
