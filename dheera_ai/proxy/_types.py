...
from dheera_ai.types.llms.openai import AllMessageValues, OpenAIFileObject
...
from dheera_ai.types.mcp_server.mcp_server_manager import MCPInfo
from dheera_ai.types.router import RouterErrors, UpdateRouterConfig
from dheera_ai.types.secret_managers.main import KeyManagementSystem
from dheera_ai.types.utils import (
    ...
    ImageResponse,
    DheeraAIBatch,
    DheeraAIFineTuningJob,
    DheeraAIPydanticObjectBase,
    ModelResponse,
    ProviderField,
...
from dheera_ai.types.videos.main import VideoObject
...
class DheeraAITeamRoles(enum.Enum):
    # team admin
    TEAM_ADMIN = "admin"
    # team member
    TEAM_MEMBER = "user"
...
class LitellmUserRoles(str, enum.Enum):
    ...
    def description(self):
        ...
        """
        descriptions = {
            "proxy_admin": "admin over dheera_ai proxy, has all permissions",
...
class LitellmTableNames(str, enum.Enum):
    """
    Enum for Table Names used by DheeraAI
    """

    TEAM_TABLE_NAME = "DheeraAI_TeamTable"
    USER_TABLE_NAME = "DheeraAI_UserTable"
    KEY_TABLE_NAME = "DheeraAI_VerificationToken"
    PROXY_MODEL_TABLE_NAME = "DheeraAI_ProxyModelTable"
    MANAGED_FILE_TABLE_NAME = "DheeraAI_ManagedFileTable"


class Litellm_EntityType(enum.Enum):
    """
    Enum for types of entities on dheera_ai
...
class DheeraAIRoutes(enum.Enum):
    openai_route_names = [
        "chat_completion",
        "completion",
        "embeddings",
    ...
    public_routes = set(
        [
            ...
            "/.well-known/dheera_ai-ui-config",
            "/public/model_hub",
            "/public/agent_hub",
            "/public/mcp_hub",
            "/public/dheera_ai_model_cost_map",
        ]
    )
...
class DheeraAIPromptInjectionParams(DheeraAIPydanticObjectBase):
...
class ProxyChatCompletionRequest(DheeraAIPydanticObjectBase):
    """
    Pydantic model for chat completion requests that includes both OpenAI standard fields
    and DheeraAI-specific parameters. This replaces the previous TypedDict version.
    """
    ...
    # DheeraAI-specific metadata param (from original ChatCompletionRequest)
    metadata: Optional[Dict[str, Any]] = None

    # Optional DheeraAI params
...
class ModelInfoDelete(DheeraAIPydanticObjectBase):
    id: str


class ModelInfo(DheeraAIPydanticObjectBase):
    id: Optional[str]
...
class ProviderInfo(DheeraAIPydanticObjectBase):
    name: str
    fields: List[ProviderField]


class BlockUsers(DheeraAIPydanticObjectBase):
    user_ids: List[str]  # required


class ModelParams(DheeraAIPydanticObjectBase):
    model_name: str
...
class DheeraAI_ObjectPermissionBase(DheeraAIPydanticObjectBase):
...
class GenerateRequestBase(DheeraAIPydanticObjectBase):
    """
    ...
    object_permission: Optional[DheeraAI_ObjectPermissionBase] = None


class AllowedVectorStoreIndexItem(DheeraAIPydanticObjectBase):
    index_name: str
...
class DheeraAIKeyType(str, enum.Enum):
    """
...
class GenerateKeyRequest(KeyRequestBase):
    ...
    key_type: Optional[DheeraAIKeyType] = Field(
        default=DheeraAIKeyType.DEFAULT,
...
class GenerateKeyResponse(KeyRequestBase):
    ...
    organization_id: Optional[str] = None
    dheera_ai_budget_table: Optional[Any] = None
...
class KeyRequest(DheeraAIPydanticObjectBase):
...
class DheeraAI_ModelTable(DheeraAIPydanticObjectBase):
    id: Optional[int] = None
    model_aliases: Optional[Union[str, dict]] = None  # json dump the dict
    created_by: str
    updated_by: str
    team: Optional["DheeraAI_TeamTable"] = None

    model_config = ConfigDict(protected_namespaces=())


class DheeraAI_ProxyModelTable(DheeraAIPydanticObjectBase):
    model_id: str
    model_name: str
    dheera_ai_params: dict
    model_info: dict
    ...
    def check_potential_json_str(cls, values):
        if isinstance(values.get("dheera_ai_params"), str):
            try:
                values["dheera_ai_params"] = json.loads(values["dheera_ai_params"])
            except json.JSONDecodeError:
                pass
...
class NewMCPServerRequest(DheeraAIPydanticObjectBase):
...
class UpdateMCPServerRequest(DheeraAIPydanticObjectBase):
    server_id: str
    server_name: Optional[str] = None
    alias: Optional[str] = None
    description: Optional[str] = None
...
class DheeraAI_MCPServerTable(DheeraAIPydanticObjectBase):
    """Represents a DheeraAI_MCPServerTable record"""
...
class MakeMCPServersPublicRequest(DheeraAIPydanticObjectBase):
    mcp_server_ids: List[str]


class NewUserRequestTeam(DheeraAIPydanticObjectBase):
    team_id: str
...
class DeleteUserRequest(DheeraAIPydanticObjectBase):
    user_ids: List[str]  # required
...
class BudgetNewRequest(DheeraAIPydanticObjectBase):
...
class BudgetRequest(DheeraAIPydanticObjectBase):
    budgets: List[str]


class BudgetDeleteRequest(DheeraAIPydanticObjectBase):
    id: str


class CustomerBase(DheeraAIPydanticObjectBase):
    user_id: str
    alias: Optional[str] = None
    spend: float = 0.0
    ...
    budget_id: Optional[str] = None
    dheera_ai_budget_table: Optional[BudgetNewRequest] = None
    blocked: bool = False
...
class UpdateCustomerRequest(DheeraAIPydanticObjectBase):
    """
...
class DeleteCustomerRequest(DheeraAIPydanticObjectBase):
    """
...
class MemberBase(DheeraAIPydanticObjectBase):
...
class TeamBase(DheeraAIPydanticObjectBase):
...
class GlobalEndUsersSpend(DheeraAIPydanticObjectBase):
    api_key: Optional[str] = None
    startTime: Optional[datetime] = None
    endTime: Optional[datetime] = None


class UpdateTeamRequest(DheeraAIPydanticObjectBase):
    """
...
class ResetTeamBudgetRequest(DheeraAIPydanticObjectBase):
    """
...
class DeleteTeamRequest(DheeraAIPydanticObjectBase):
    team_ids: List[str]  # required


class BlockTeamRequest(DheeraAIPydanticObjectBase):
    team_id: str  # required


class BlockKeyRequest(DheeraAIPydanticObjectBase):
    key: str  # required


class AddTeamCallback(DheeraAIPydanticObjectBase):
    callback_name: str
...
class TeamCallbackMetadata(DheeraAIPydanticObjectBase):
...
class DheeraAI_ObjectPermissionTable(DheeraAIPydanticObjectBase):
    """Represents a DheeraAI_ObjectPermissionTable record"""
...
class DheeraAI_TeamTable(TeamBase):
    ...
    dheera_ai_model_table: Optional[DheeraAI_ModelTable] = None
    object_permission: Optional[DheeraAI_ObjectPermissionTable] = None
    updated_at: Optional[datetime] = None
    created_at: Optional[datetime] = None
...
class DheeraAI_TeamTableCachedObj(DheeraAI_TeamTable):
    last_refreshed_at: Optional[float] = None


class TeamRequest(DheeraAIPydanticObjectBase):
    teams: List[str]


class DheeraAI_BudgetTable(DheeraAIPydanticObjectBase):
    """Represents user-controllable params for a DheeraAI_BudgetTable record"""
...
class DheeraAI_BudgetTableFull(DheeraAI_BudgetTable):
    """Represents all params for a DheeraAI_BudgetTable record"""

    budget_reset_at: Optional[datetime] = None
    created_at: datetime


class DheeraAI_TeamMemberTable(DheeraAI_BudgetTable):
    """
...
class NewOrganizationRequest(DheeraAI_BudgetTable):
    organization_id: Optional[str] = None
...
class OrganizationRequest(DheeraAIPydanticObjectBase):
    organizations: List[str]


class DeleteOrganizationRequest(DheeraAIPydanticObjectBase):
    organization_ids: List[str]  # required


class TeamDefaultSettings(DheeraAIPydanticObjectBase):
    team_id: str

    model_config = ConfigDict(
        extra="allow"
    )  # allow params not defined here, these fall in dheera_ai.completion(**kwargs)


class DynamoDBArgs(DheeraAIPydanticObjectBase):
    billing_mode: Literal["PROVISIONED_THROUGHPUT", "PAY_PER_REQUEST"]
    read_capacity_units: Optional[int] = None
    write_capacity_units: Optional[int] = None
    ssl_verify: Optional[bool] = None
    region_name: str
    user_table_name: str = "DheeraAI_UserTable"
    key_table_name: str = "DheeraAI_VerificationToken"
    config_table_name: str = "DheeraAI_Config"
    spend_table_name: str = "DheeraAI_SpendLogs"
...
class PassThroughGuardrailSettings(DheeraAIPydanticObjectBase):
    """
...
class PassThroughGenericEndpoint(DheeraAIPydanticObjectBase):
    ...
    path: str = Field(description="The route to be added to the DheeraAI Proxy Server.")
    target: str = Field(
    ...
    auth: bool = Field(
        default=False,
        description="Whether authentication is required for the pass-through endpoint. If True, requests to the endpoint will require a valid DheeraAI API key.",
    )
...
class PassThroughEndpointResponse(DheeraAIPydanticObjectBase):
    endpoints: List[PassThroughGenericEndpoint]


class ConfigFieldUpdate(DheeraAIPydanticObjectBase):
    field_name: str
    field_value: Any
    config_type: Literal["general_settings"]


class ConfigFieldDelete(DheeraAIPydanticObjectBase):
    config_type: Literal["general_settings"]
    field_name: str


class CallbackDelete(DheeraAIPydanticObjectBase):
    callback_name: str
...
class ConfigList(DheeraAIPydanticObjectBase):
    field_name: str
    field_type: str
    field_description: str
    field_value: Any
...
class UserHeaderMapping(DheeraAIPydanticObjectBase):
    """
    Map an incoming HTTP header to a DheeraAI user role.
    """

    header_name: str
    dheera_ai_user_role: Literal[
        LitellmUserRoles.INTERNAL_USER,
...
class ConfigGeneralSettings(DheeraAIPydanticObjectBase):
    """
...
class ConfigYAML(DheeraAIPydanticObjectBase):
    """
    ...
    dheera_ai_settings: Optional[dict] = Field(
        None,
        description="dheera_ai Module settings. See __init__.py for all, example dheera_ai.drop_params=True, dheera_ai.set_verbose=True, dheera_ai.api_base, dheera_ai.cache",
    )
    general_settings: Optional[ConfigGeneralSettings] = None
    router_settings: Optional[UpdateRouterConfig] = Field(
        None,
        description="dheera_ai router object settings. See router.py __init__ for all, example router.num_retries=5, router.timeout=5, router.max_retries=5, router.retry_after=5",
    )

    model_config = ConfigDict(protected_namespaces=())


class DheeraAI_VerificationToken(DheeraAIPydanticObjectBase):
    token: Optional[str] = None
    key_name: Optional[str] = None
    key_alias: Optional[str] = None
    spend: float = 0.0
    ...
    blocked: Optional[bool] = None
    dheera_ai_budget_table: Optional[dict] = None
...
class DheeraAI_VerificationTokenView(DheeraAI_VerificationToken):
    """
    Combined view of dheera_ai verification token + dheera_ai team table (select values)
    """
    ...
    def __init__(self, **kwargs):
        # Handle dheera_ai_budget_table_* keys
        for key, value in list(kwargs.items()):
            if key.startswith("dheera_ai_budget_table_") and value is not None:
                # Extract the corresponding attribute name
                attr_name = key.replace("dheera_ai_budget_table_", "")
...
class UserAPIKeyAuth(
    DheeraAI_VerificationTokenView
    ...
    def check_api_key(cls, values):
        if values.get("api_key") is not None:
            values.update(
                {"token": cls._safe_hash_dheera_ai_api_key(values.get("api_key"))}
            )
            if isinstance(values.get("api_key"), str):
                values.update(
                    {"api_key": cls._safe_hash_dheera_ai_api_key(values.get("api_key"))}
                )
        return values

    @classmethod
    def _safe_hash_dheera_ai_api_key(cls, api_key: str) -> str:
        """
        Helper to ensure all logged keys are hashed
        Covers:
        1. Regular API keys from DheeraAI DB
        2. JWT tokens used for connecting to DheeraAI API
        """
    ...
    @classmethod
    def get_dheera_ai_internal_health_check_user_api_key_auth(cls) -> "UserAPIKeyAuth":
        """
        Returns a `UserAPIKeyAuth` object for the dheera_ai internal health check service account.
    ...
    @classmethod
    def get_dheera_ai_cli_user_api_key_auth(cls) -> "UserAPIKeyAuth":
        """
        Returns a `UserAPIKeyAuth` object for the dheera_ai internal health check service account.

        This is used to track number of requests/spend for health check calls.
        """
        from dheera_ai.constants import LITTELM_CLI_SERVICE_ACCOUNT_NAME
    ...
    @classmethod
    def get_dheera_ai_internal_jobs_user_api_key_auth(cls) -> "UserAPIKeyAuth":
        """
        Returns a `UserAPIKeyAuth` object for internal DheeraAI jobs like key rotation.

        This is used to track actions performed by automated system jobs.
        """
        from dheera_ai.constants import DHEERA_AI_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME

        return cls(
            api_key=DHEERA_AI_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME,
            team_id="system",
            key_alias=DHEERA_AI_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME,
            team_alias="system",
            user_id="system",
        )


class UserInfoResponse(DheeraAIPydanticObjectBase):
...
class DheeraAI_Config(DheeraAIPydanticObjectBase):
    param_name: str
    param_value: Dict


class DheeraAI_OrganizationMembershipTable(DheeraAIPydanticObjectBase):
    """
    ...
    dheera_ai_budget_table: Optional[DheeraAI_BudgetTable] = None

    model_config = ConfigDict(protected_namespaces=())


class DheeraAI_OrganizationTableUpdate(DheeraAI_BudgetTable):
    """Represents user-controllable params for a DheeraAI_OrganizationTable record"""
    ...
    @classmethod
    def set_model_info(cls, values):
        for field in DheeraAI_ManagementEndpoint_MetadataFields:
...
class DheeraAI_UserTable(DheeraAIPydanticObjectBase):
    user_id: str
    max_budget: Optional[float] = None
    spend: float = 0.0
    model_max_budget: Optional[Dict] = {}
    ...
    rpm_limit: Optional[int] = None
    user_role: Optional[str] = None
    organization_memberships: Optional[List[DheeraAI_OrganizationMembershipTable]] = None
    teams: List[str] = []
    sso_user_id: Optional[str] = None
    budget_duration: Optional[str] = None
    budget_reset_at: Optional[datetime] = None
...
class DheeraAI_OrganizationTable(DheeraAIPydanticObjectBase):
    """Represents user-controllable params for a DheeraAI_OrganizationTable record"""
    ...
    created_by: str
    updated_by: str
    users: Optional[List[DheeraAI_UserTable]] = None
    dheera_ai_budget_table: Optional[DheeraAI_BudgetTable] = None
...
class DheeraAI_OrganizationTableWithMembers(DheeraAI_OrganizationTable):
    """Returned by the /organization/info endpoint and /organization/list endpoint"""

    members: List[DheeraAI_OrganizationMembershipTable] = []
    teams: List[DheeraAI_TeamTable] = []
    dheera_ai_budget_table: Optional[DheeraAI_BudgetTable] = None
    created_at: datetime
    updated_at: datetime


class NewOrganizationResponse(DheeraAI_OrganizationTable):
    organization_id: str  # type: ignore
    created_at: datetime
    updated_at: datetime


class DheeraAI_UserTableFiltered(BaseModel):  # done to avoid exposing sensitive data
    user_id: str
    user_email: Optional[str] = None


class DheeraAI_UserTableWithKeyCount(DheeraAI_UserTable):
    key_count: int = 0


class DheeraAI_EndUserTable(DheeraAIPydanticObjectBase):
    user_id: str
    blocked: bool
    alias: Optional[str] = None
    spend: float = 0.0
...
class DheeraAI_TagTable(DheeraAIPydanticObjectBase):
    tag_name: str
    description: Optional[str] = None
    models: List[str] = []
...
class DheeraAI_SpendLogs(DheeraAIPydanticObjectBase):
    request_id: str
    api_key: str
    model: Optional[str] = ""
...
class DheeraAI_ErrorLogs(DheeraAIPydanticObjectBase):
    request_id: Optional[str] = str(uuid.uuid4())
    api_base: Optional[str] = ""
    model_group: Optional[str] = ""
    dheera_ai_model_name: Optional[str] = ""
...
class DheeraAI_AuditLogs(DheeraAIPydanticObjectBase):
    id: str
    updated_at: datetime
...
class DheeraAI_SpendLogs_ResponseObject(DheeraAIPydanticObjectBase):
    response: Optional[List[Union[DheeraAI_SpendLogs, Any]]] = None


class TokenCountRequest(DheeraAIPydanticObjectBase):
    model: str
...
class CallInfo(DheeraAIPydanticObjectBase):
...
class InvitationNew(DheeraAIPydanticObjectBase):
    user_id: str


class InvitationUpdate(DheeraAIPydanticObjectBase):
    invitation_id: str
    is_accepted: bool


class InvitationDelete(DheeraAIPydanticObjectBase):
    invitation_id: str


class InvitationModel(DheeraAIPydanticObjectBase):
    id: str
    user_id: str
    is_accepted: bool
...
class InvitationClaim(DheeraAIPydanticObjectBase):
    invitation_link: str
    user_id: str
    password: str


class ConfigFieldInfo(DheeraAIPydanticObjectBase):
    field_name: str
    field_value: Any


class CallbackOnUI(DheeraAIPydanticObjectBase):
    dheera_ai_callback_name: str
    dheera_ai_callback_params: Optional[list]
    ui_callback_name: str


class AllCallbacks(DheeraAIPydanticObjectBase):
    langfuse: CallbackOnUI = CallbackOnUI(
        dheera_ai_callback_name="langfuse",
        ui_callback_name="Langfuse",
        dheera_ai_callback_params=[
            "LANGFUSE_PUBLIC_KEY",
            "LANGFUSE_SECRET_KEY",
            "LANGFUSE_HOST",
        ],
    ...
    openmeter: CallbackOnUI = CallbackOnUI(
        dheera_ai_callback_name="openmeter",
    ...
    custom_callback_api: CallbackOnUI = CallbackOnUI(
        dheera_ai_callback_name="custom_callback_api",
        dheera_ai_callback_params=["GENERIC_LOGGER_ENDPOINT", "GENERIC_LOGGER_HEADERS"],
        ui_callback_name="Custom Callback API",
    )

    generic_api: CallbackOnUI = CallbackOnUI(
        dheera_ai_callback_name="generic_api",
        dheera_ai_callback_params=["GENERIC_LOGGER_ENDPOINT", "GENERIC_LOGGER_HEADERS"],
        ui_callback_name="Custom Callback API",
    )

    datadog: CallbackOnUI = CallbackOnUI(
        dheera_ai_callback_name="datadog",
        dheera_ai_callback_params=["DD_API_KEY", "DD_SITE"],
        ui_callback_name="Datadog",
    )

    braintrust: CallbackOnUI = CallbackOnUI(
        dheera_ai_callback_name="braintrust",
        dheera_ai_callback_params=["BRAINTRUST_API_KEY", "BRAINTRUST_API_BASE"],
        ui_callback_name="Braintrust",
    )

    langsmith: CallbackOnUI = CallbackOnUI(
        dheera_ai_callback_name="langsmith",
        dheera_ai_callback_params=[
            "LANGSMITH_API_KEY",
            "LANGSMITH_PROJECT",
    ...
    traceloop: CallbackOnUI = CallbackOnUI(
        dheera_ai_callback_name="traceloop",
        dheera_ai_callback_params=[
            "TRACELOOP_API_KEY",
        ],
...
class SpendLogsMetadata(TypedDict):
    ...
    dheera_ai_overhead_time_ms: Optional[
        float
    ]  # DheeraAI overhead time in milliseconds
...
class SpanAttributes(str, enum.Enum):
    ...
    # I chose to not add a new dependency to dheera_ai for this
...
class ManagementEndpointLoggingPayload(DheeraAIPydanticObjectBase):
    route: str
    request_data: dict
...
class ProxyException(Exception):
    ...
    def __init__(
        ...
        # Should map to 429 errors https://github.com/BerriAI/dheera_ai/issues/2487
        if (
...
class CommonProxyErrors(str, enum.Enum):
    ...
    not_premium_user = "You must be a DheeraAI Enterprise user to use this feature. If you have a license please set `DHEERA_AI_LICENSE` in your env. Get a 7 day trial key here: https://www.dheera_ai.ai/enterprise#trial. \nPricing: https://www.dheera_ai.ai/#pricing"
    max_parallel_request_limit_reached = (
        "Crossed TPM / RPM / Max Parallel Request Limit"
    )
    missing_enterprise_package = "Missing dheera_ai-enterprise package. Please install it to use this feature. Run `pip install dheera_ai-enterprise`"
    missing_enterprise_package_docker = (
        "This uses the enterprise folder - only available on the Docker image."
    )


class SpendCalculateRequest(DheeraAIPydanticObjectBase):
...
class VirtualKeyEvent(DheeraAIPydanticObjectBase):
...
class CreatePassThroughEndpoint(DheeraAIPydanticObjectBase):
    path: str
    target: str
    headers: dict


class DheeraAI_TeamMembership(DheeraAIPydanticObjectBase):
    user_id: str
    team_id: str
    budget_id: Optional[str] = None
    spend: Optional[float] = 0.0
    dheera_ai_budget_table: Optional[DheeraAI_BudgetTable]

    def safe_get_team_member_rpm_limit(self) -> Optional[int]:
        if self.dheera_ai_budget_table is not None:
            return self.dheera_ai_budget_table.rpm_limit
        return None

    def safe_get_team_member_tpm_limit(self) -> Optional[int]:
        if self.dheera_ai_budget_table is not None:
            return self.dheera_ai_budget_table.tpm_limit
        return None
...
class MemberAddRequest(DheeraAIPydanticObjectBase):
...
class OrgMemberAddRequest(DheeraAIPydanticObjectBase):
...
class TeamAddMemberResponse(DheeraAI_TeamTable):
    updated_users: List[DheeraAI_UserTable]
    updated_team_memberships: List[DheeraAI_TeamMembership]


class OrganizationAddMemberResponse(DheeraAIPydanticObjectBase):
    organization_id: str
    updated_users: List[DheeraAI_UserTable]
    updated_organization_memberships: List[DheeraAI_OrganizationMembershipTable]


class MemberDeleteRequest(DheeraAIPydanticObjectBase):
    user_id: Optional[str] = None
    user_email: Optional[str] = None
...
class MemberUpdateResponse(DheeraAIPydanticObjectBase):
    user_id: str
    user_email: Optional[str] = None
...
class TeamInfoResponseObjectTeamTable(DheeraAI_TeamTable):
    team_member_budget_table: Optional[DheeraAI_BudgetTable] = None
...
class TeamInfoResponseObject(TypedDict):
    ...
    keys: List
    team_memberships: List[DheeraAI_TeamMembership]


class TeamListResponseObject(DheeraAI_TeamTable):
    team_memberships: List[DheeraAI_TeamMembership]
...
class SpecialHeaders(enum.Enum):
    """Used by user_api_key_auth.py to get dheera_ai key"""
    ...
    custom_dheera_ai_api_key = "x-dheera_ai-api-key"
    mcp_auth = "x-mcp-auth"
...
class LitellmMetadataFromRequestHeaders(TypedDict, total=False):
    """
    Headers a user can pass that will get added to dheera_ai metadata for the request
    """
...
class UserManagementEndpointParamDocStringEnums(str, enum.Enum):
    ...
    user_role_doc_str = """Optional[str] - Specify a user role - "proxy_admin", "proxy_admin_viewer", "internal_user", "internal_user_viewer", "team", "customer". Info about each role here: `https://github.com/BerriAI/dheera_ai/dheera_ai/proxy/_types.py#L20`"""
...
DheeraAI_ManagementEndpoint_MetadataFields = [
    "model_rpm_limit",
    "model_tpm_limit",
    "rpm_limit_type",
    "tpm_limit_type",
...
DheeraAI_ManagementEndpoint_MetadataFields_Premium = [
    "guardrails",
    "tags",
...
class ProviderBudgetResponseObject(DheeraAIPydanticObjectBase):
    """
...
class ProviderBudgetResponse(DheeraAIPydanticObjectBase):
    """
...
UI_TEAM_ID = "dheera_ai-dashboard"


class JWTAuthBuilderResult(TypedDict):
    is_proxy_admin: bool
    team_object: Optional[DheeraAI_TeamTable]
    user_object: Optional[DheeraAI_UserTable]
    end_user_object: Optional[DheeraAI_EndUserTable]
    org_object: Optional[DheeraAI_OrganizationTable]
    token: str
    team_id: Optional[str]
    user_id: Optional[str]
    end_user_id: Optional[str]
    org_id: Optional[str]
    team_membership: Optional[DheeraAI_TeamMembership]
...
class OIDCPermissions(DheeraAIPydanticObjectBase):
    models: Optional[List[str]] = None
...
class JWTDheeraAIRoleMap(BaseModel):
    jwt_role: str
    dheera_ai_role: LitellmUserRoles
...
class DheeraAI_JWTAuth(DheeraAIPydanticObjectBase):
    """
    A class to define the roles and permissions for a DheeraAI Proxy w/ JWT Auth.
    ...
    - user_id_jwt_field: The field in the JWT token that stores the user id (maps to `DheeraAIUserTable`). Use this for internal employees.
    - user_email_jwt_field: The field in the JWT token that stores the user email (maps to `DheeraAIUserTable`). Use this for internal employees.
    - user_allowed_email_subdomain: If specified, only emails from specified subdomain will be allowed to access proxy.
    - end_user_id_jwt_field: The field in the JWT token that stores the end-user ID (maps to `DheeraAIEndUserTable`). Turn this off by setting to `None`. Enables end-user cost tracking. Use this for external customers.
    - public_key_ttl: Default - 600s. TTL for caching public JWT keys.
    - public_allowed_routes: list of allowed routes for authenticated but unknown dheera_ai role jwt tokens.
    - enforce_rbac: If true, enforce RBAC for all routes.
    - custom_validate: A custom function to validates the JWT token.
    - oidc_userinfo_endpoint: OIDC UserInfo endpoint URL. When set along with oidc_userinfo_enabled, DheeraAI will call this endpoint with the access token to retrieve user identity information.
    ...
    admin_jwt_scope: str = "dheera_ai_proxy_admin"
    ...
    jwt_dheera_ai_role_map: Optional[List[JWTDheeraAIRoleMap]] = None
    ...
    oidc_userinfo_endpoint: Optional[str] = Field(
        default=None,
        description="OIDC UserInfo endpoint URL. If set, DheeraAI will call this endpoint with the access token to retrieve user identity information.",
    )
...
class DefaultInternalUserParams(DheeraAIPydanticObjectBase):
    """
...
class DheeraAI_ManagedFileTable(DheeraAIPydanticObjectBase):
    unified_file_id: str
...
class DheeraAI_ManagedObjectTable(DheeraAIPydanticObjectBase):
    unified_object_id: str
    model_object_id: str
    file_purpose: Literal["batch", "fine-tune"]
    file_object: Union[DheeraAIBatch, DheeraAIFineTuningJob]
...
class DheeraAI_ManagedVectorStoresTable(DheeraAIPydanticObjectBase):
    vector_store_id: str
    ...
    dheera_ai_credential_name: Optional[str]
    dheera_ai_params: Optional[Dict[str, Any]]


class ResponseDheeraAI_ManagedVectorStore(TypedDict, total=False):
    vector_store: DheeraAI_ManagedVectorStoresTable
