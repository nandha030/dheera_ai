"""
A2A Protocol endpoints for DheeraAI Proxy.

Allows clients to invoke agents through DheeraAI using the A2A protocol.
The A2A SDK can point to DheeraAI's URL and invoke agents registered with DheeraAI.
"""
...
async def get_agent_card(
    ...
    The URL in the agent card is rewritten to point to the DheeraAI proxy,
    so all subsequent A2A calls go through DheeraAI for logging and cost tracking.
    """
    ...
    try:
        ...
        # Copy and rewrite URL to point to DheeraAI proxy
...
async def invoke_agent_a2a(
    ...
    try:
        ...
        # Get dheera_ai_params (may include custom_llm_provider for completion bridge)
        dheera_ai_params = agent.dheera_ai_params or {}
        ...
        # Set up data dict for dheera_ai processing
        body.update({
            "model": f"a2a_agent/{agent_name}",
            "custom_llm_provider": "a2a_agent",
        })

        # Add dheera_ai data (user_api_key, user_id, team_id, etc.)
