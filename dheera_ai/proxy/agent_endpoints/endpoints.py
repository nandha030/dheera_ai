"""
Agent endpoints for registering + discovering agents via DheeraAI.
...
from dheera_ai.proxy._types import CommonProxyErrors, LitellmUserRoles, UserAPIKeyAuth
from dheera_ai.proxy.auth.user_api_key_auth import user_api_key_auth
from dheera_ai.types.agents import (
    AgentConfig,
...
async def get_agents(
    ...
    from dheera_ai.proxy.agent_endpoints.auth.agent_permission_handler import (
        AgentRequestHandler,
    )
    ...
    try:
        ...
        for agent in returned_agents:
            if agent.dheera_ai_params is None:
                agent.dheera_ai_params = {}
            agent.dheera_ai_params["is_public"] = (
                dheera_ai.public_agent_groups is not None
                and (agent.agent_id in dheera_ai.public_agent_groups)
            )
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.agent_endpoints.get_agents(): Exception occurred - {}".format(
                str(e)
            )
        )
        raise HTTPException(
...
async def make_agent_public(
    ...
    try:
        ...
        if dheera_ai.public_agent_groups is None:
            dheera_ai.public_agent_groups = []
        # handle duplicates
        if agent.agent_id in dheera_ai.public_agent_groups:
            raise HTTPException(
                status_code=400,
                detail=f"Agent with name {agent.agent_name} already in public agent groups",
            )
        dheera_ai.public_agent_groups.append(agent.agent_id)
        ...
        config["dheera_ai_settings"]["public_agent_groups"] = dheera_ai.public_agent_groups

        # Save the updated config
        await proxy_config.save_config(new_config=config)

        verbose_proxy_logger.debug(
            f"Updated public agent groups to: {dheera_ai.public_agent_groups} by user: {user_api_key_dict.user_id}"
        )

        return {
            "message": "Successfully updated public agent groups",
            "public_agent_groups": dheera_ai.public_agent_groups,
...
async def make_agents_public(
    ...
    try:
        ...
        dheera_ai.public_agent_groups = request.agent_ids
