...
async def anthropic_response(  # noqa: PLR0915
    ...
    """
    Use `{PROXY_BASE_URL}/anthropic/v1/messages` instead - [Docs](https://docs.dheera_ai.ai/docs/anthropic_completion).
    ...
    try:
        ...
        import uuid
        from dheera_ai.types.utils import AnthropicMessagesResponse
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.anthropic_response(): Exception occured - {}".format(
                str(e)
            )
        )

        # Extract model_id from request metadata (same as success path)
        dheera_ai_metadata = data.get("dheera_ai_metadata", {}) or {}
...
async def count_tokens(
    ...
    """
    from dheera_ai.proxy.proxy_server import token_counter as internal_token_counter
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.anthropic_endpoints.count_tokens(): Exception occurred - {}".format(
                str(e)
            )
        )
        raise HTTPException(
