...
from dheera_ai.caching.dual_cache import LimitedSizeOrderedDict
...
from dheera_ai.proxy._types import (
    ...
    DheeraAI_EndUserTable,
    Litellm_EntityType,
    DheeraAI_JWTAuth,
    DheeraAI_ObjectPermissionTable,
...
from dheera_ai.proxy.utils import PrismaClient, ProxyLogging, log_db_metrics
...
all_routes = DheeraAIRoutes.openai_routes.value + DheeraAIRoutes.management_routes.value
...
async def common_checks(
    ...
    7. [OPTIONAL] If 'dheera_ai.max_budget' is set (>0), is proxy under budget
    ...
    if (
        ...
        if user_budget < user_object.spend:
            raise dheera_ai.BudgetExceededError(
    ...
    if end_user_object is not None and end_user_object.dheera_ai_budget_table is not None:
        end_user_budget = end_user_object.dheera_ai_budget_table.max_budget
    ...
    if (
        ...
        is_mcp_route = route in DheeraAIRoutes.mcp_routes.value or RouteChecks.check_route_access(
            route=route, allowed_routes=DheeraAIRoutes.mcp_routes.value
        )
    ...
    if (
        ...
        if global_proxy_spend > dheera_ai.max_budget:
            raise dheera_ai.BudgetExceededError(
                current_cost=global_proxy_spend, max_budget=dheera_ai.max_budget
            )
    ...
    if _request_metadata.get("guardrails"):
        ...
        from dheera_ai.proxy.guardrails.guardrail_helpers import can_modify_guardrails
    ...
    token_type: Literal["ui", "api"] = (
        "ui" if token_team is not None and token_team == "dheera_ai-dashboard" else "api"
    )
...
def _is_ui_route(
    route: str,
    user_obj: Optional[DheeraAI_UserTable] = None,
) -> bool:
    """
    - Check if the route is a UI used route
    """
    # this token is only used for managing the ui
    allowed_routes = DheeraAIRoutes.ui_routes.value
...
def _get_user_role(
    user_obj: Optional[DheeraAI_UserTable],
...
def _is_user_proxy_admin(user_obj: Optional[DheeraAI_UserTable]):
    if user_obj is None:
        return False
...
def _allowed_routes_check(user_route: str, allowed_routes: list) -> bool:
    ...
    - allowed_routes: List[str|DheeraAIRoutes] - the list of allowed routes for the user.
    """
    from starlette.routing import compile_path

    for allowed_route in allowed_routes:
        if allowed_route in DheeraAIRoutes.__members__:
            for template in DheeraAIRoutes[allowed_route].value:
...
def allowed_routes_check(
    ...
    user_route: str,
    dheera_ai_proxy_roles: DheeraAI_JWTAuth,
) -> bool:
    """
    ...
    if user_role == LitellmUserRoles.PROXY_ADMIN:
        is_allowed = _allowed_routes_check(
            ...
            allowed_routes=dheera_ai_proxy_roles.admin_allowed_routes,
        )
        return is_allowed

    elif user_role == LitellmUserRoles.TEAM:
        if dheera_ai_proxy_roles.team_allowed_routes is None:
            """
            ...
            return is_allowed
        elif dheera_ai_proxy_roles.team_allowed_routes is not None:
            is_allowed = _allowed_routes_check(
                user_route=user_route,
                allowed_routes=dheera_ai_proxy_roles.team_allowed_routes,
            )
            return is_allowed
    return False
...
def get_actual_routes(allowed_routes: list) -> list:
    ...
    for route_name in allowed_routes:
        try:
            route_value = DheeraAIRoutes[route_name].value
...
async def get_default_end_user_budget(
...
) -> Optional[DheeraAI_BudgetTable]:
    """
    Fetches the default end user budget from the database if dheera_ai.max_end_user_budget_id is configured.
    ...
    Returns:
        DheeraAI_BudgetTable if configured and found, None otherwise
    """
    if prisma_client is None or dheera_ai.max_end_user_budget_id is None:
        return None
    
    cache_key = f"default_end_user_budget:{dheera_ai.max_end_user_budget_id}"
    
    # Check cache first
    cached_budget = await user_api_key_cache.async_get_cache(key=cache_key)
    if cached_budget is not None:
        return DheeraAI_BudgetTable(**cached_budget)
    ...
    try:
        ...
        if budget_record is None:
            verbose_proxy_logger.warning(
                f"Default end user budget not found in database: {dheera_ai.max_end_user_budget_id}"
            )
            return None
        ...
        return DheeraAI_BudgetTable(**budget_record.dict())
...
async def _apply_default_budget_to_end_user(
    end_user_obj: DheeraAI_EndUserTable,
    ...
    if end_user_obj.dheera_ai_budget_table is not None:
        return end_user_obj
    
    # If no default budget configured, return as-is
    if dheera_ai.max_end_user_budget_id is None:
        return end_user_obj
    ...
    if default_budget is not None:
        ...
        end_user_obj.dheera_ai_budget_table = default_budget
        verbose_proxy_logger.debug(
            f"Applied default budget {dheera_ai.max_end_user_budget_id} to end user {end_user_obj.user_id}"
        )
...
def _check_end_user_budget(
    ...
    Raises:
        dheera_ai.BudgetExceededError: If end user has exceeded their budget
    """
    if route in DheeraAIRoutes.info_routes.value:
        return
    
    if end_user_obj.dheera_ai_budget_table is None:
        return
    
    end_user_budget = end_user_obj.dheera_ai_budget_table.max_budget
...
async def get_end_user_object(
    ...
    Returns:
        DheeraAI_EndUserTable if found, None otherwise
    """
    ...
    if cached_user_obj is not None:
        return_obj = DheeraAI_EndUserTable(**cached_user_obj)
    ...
    try:
        ...
        # Convert to DheeraAI_EndUserTable object
        _response = DheeraAI_EndUserTable(**response.dict())
        ...
        if isinstance(e, dheera_ai.BudgetExceededError):
            raise e
        return None
...
async def get_tag_objects_batch(
    ...
    Returns:
        Dictionary mapping tag_name to DheeraAI_TagTable object
    """
    if prisma_client is None:
        return {}
    ...
    for tag_name in tag_names:
        ...
        if cached_tag is not None:
            if isinstance(cached_tag, dict):
                tag_objects[tag_name] = DheeraAI_TagTable(**cached_tag)
            else:
    ...
    if uncached_tags:
        try:
            ...
            for db_tag in db_tags:
                ...
                tag_objects[tag_name] = DheeraAI_TagTable(**db_tag.dict())
        except Exception as e:
...
async def get_tag_object(
    ...
    Returns:
        DheeraAI_TagTable object if found, None otherwise
    """
...
async def get_team_membership(
    ...
    parent_otel_span: Optional[Span] = None,
    proxy_logging_obj: Optional[ProxyLogging] = None,
) -> Optional["DheeraAI_TeamMembership"]:
    """
    ...
    """
    from dheera_ai.proxy._types import DheeraAI_TeamMembership
    ...
    if cached_membership_obj is not None:
        return DheeraAI_TeamMembership(**cached_membership_obj)
    ...
    try:
        ...
        _response = DheeraAI_TeamMembership(**response.dict())
...
async def get_user_object(
    ...
    - if valid, return DheeraAI_UserTable object with defined limits
    ...
    if not check_db_only:
        ...
        if cached_user_obj is not None:
            if isinstance(cached_user_obj, dict):
                return DheeraAI_UserTable(**cached_user_obj)
            elif isinstance(cached_user_obj, DheeraAI_UserTable):
    ...
    try:
        ...
        if (
            ...
            # dump each organization membership to type DheeraAI_OrganizationMembershipTable
            _dumped_memberships = [
                DheeraAI_OrganizationMembershipTable(**membership.model_dump())
                for membership in response.organization_memberships
                if membership is not None
            ]
            response.organization_memberships = _dumped_memberships

        _response = DheeraAI_UserTable(**dict(response))
...
async def _get_team_db_check(
    ...
    if response is None and team_id_upsert:
        ...
        response = DheeraAI_TeamTable(**created_team_dict)
    return response
...
async def _get_team_object_from_user_api_key_cache(
    ...
    _response = DheeraAI_TeamTableCachedObj(**response.dict())
...
async def _get_team_object_from_cache(
...
) -> Optional[DheeraAI_TeamTableCachedObj]:
    cached_team_obj: Optional[DheeraAI_TeamTableCachedObj] = None
    ...
    if cached_team_obj is not None:
        if isinstance(cached_team_obj, dict):
            return DheeraAI_TeamTableCachedObj(**cached_team_obj)
        elif isinstance(cached_team_obj, DheeraAI_TeamTableCachedObj):
...
async def get_team_object(
    ...
    - if valid, return DheeraAI_TeamTable object with defined limits
    - if not, then raise an error
...
class ExperimentalUIJWTToken:
    @staticmethod
    def get_experimental_ui_login_jwt_auth_token(user_info: DheeraAI_UserTable) -> str:
    ...
    def get_cli_jwt_auth_token(
        user_info: DheeraAI_UserTable, team_id: Optional[str] = None
    ) -> str:
        """
        ...
        from dheera_ai.constants import CLI_JWT_TOKEN_NAME
        from dheera_ai.proxy.common_utils.encrypt_decrypt_utils import (
            encrypt_value_helper,
        )
    ...
    def get_key_object_from_ui_hash_key(
        ...
        from dheera_ai.proxy.auth.user_api_key_auth import UserAPIKeyAuth
        from dheera_ai.proxy.common_utils.encrypt_decrypt_utils import (
...
async def get_object_permission(
    ...
    parent_otel_span: Optional[Span] = None,
    proxy_logging_obj: Optional[ProxyLogging] = None,
) -> Optional[DheeraAI_ObjectPermissionTable]:
    """
    - Check if object permission id in proxy ObjectPermissionTable
    - if valid, return DheeraAI_ObjectPermissionTable object
    - if not, then raise an error
    """
    if prisma_client is None:
        raise Exception(
    ...
    if cached_obj_permission is not None:
        if isinstance(cached_obj_permission, dict):
            return DheeraAI_ObjectPermissionTable(**cached_obj_permission)
        elif isinstance(cached_obj_permission, DheeraAI_ObjectPermissionTable):
    ...
    try:
        ...
        return DheeraAI_ObjectPermissionTable(**response.dict())
    except Exception:
        return None
...
async def get_org_object(
    ...
    parent_otel_span: Optional[Span] = None,
    proxy_logging_obj: Optional[ProxyLogging] = None,
) -> Optional[DheeraAI_OrganizationTable]:
    """
    - Check if org id in proxy Org Table
    - if valid, return DheeraAI_OrganizationTable object
    - if not, then raise an error
    """
    if prisma_client is None:
        raise Exception(
    ...
    if cached_org_obj is not None:
        if isinstance(cached_org_obj, dict):
            return DheeraAI_OrganizationTable(**cached_org_obj)
        elif isinstance(cached_org_obj, DheeraAI_OrganizationTable):
...
def _can_object_call_model(
    ...
    if model in dheera_ai.model_alias_map:
        potential_models.append(dheera_ai.model_alias_map[model])
...
async def can_key_call_model(
    ...
    valid_token: UserAPIKeyAuth,
    llm_router: Optional[dheera_ai.Router],
) -> Literal[True]:
    """
    Checks if token can call a given model
...
async def _tag_max_budget_check(
    ...
    """
    from dheera_ai.proxy.common_utils.http_parsing_utils import get_tags_from_request_body
    ...
    for tag_name in tags:
        ...
        if (
            tag_object.dheera_ai_budget_table is not None
            and tag_object.dheera_ai_budget_table.max_budget is not None
            and tag_object.spend is not None
            and tag_object.spend > tag_object.dheera_ai_budget_table.max_budget
        ):
            raise dheera_ai.BudgetExceededError(
                current_cost=tag_object.spend,
                max_budget=tag_object.dheera_ai_budget_table.max_budget,
                message=f"Budget has been exceeded! Tag={tag_name} Current cost: {tag_object.spend}, Max budget: {tag_object.dheera_ai_budget_table.max_budget}",
            )
...
async def vector_store_access_check(
    ...
    vector_store_ids_to_run = dheera_ai.vector_store_registry.get_vector_store_ids_to_run(
...
def _can_object_call_vector_stores(
    ...
    object_permissions: Optional[DheeraAI_ObjectPermissionTable],
):
    """
    Raises ProxyException if the object (key, team, org) cannot access the specific vector store.
    """
