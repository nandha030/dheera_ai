...
from dheera_ai import Router, provider_list
from dheera_ai._logging import verbose_proxy_logger
from dheera_ai.proxy._types import *
from dheera_ai.types.router import CONFIGURABLE_CLIENTSIDE_AUTH_PARAMS
...
async def pre_db_read_auth_checks(
    ...
    """
    from dheera_ai.proxy.proxy_server import general_settings, llm_router, premium_user
...
def route_in_additonal_public_routes(current_route: str):
    ...
    In order to use this the dheera_ai config.yaml should have the following in general_settings:

    ```yaml
    general_settings:
        master_key: sk-1234
        public_routes: ["DheeraAIRoutes.public_routes", "/spend/calculate", "/api/*"]
    ```
    """
...
def get_end_user_id_from_request_body(
    ...
    # Check 3: 'dheera_ai_metadata.user' in request_body (commonly Anthropic)
    dheera_ai_metadata = request_body.get("dheera_ai_metadata")
    if isinstance(dheera_ai_metadata, dict):
        user_from_dheera_ai_metadata = dheera_ai_metadata.get("user")
        if user_from_dheera_ai_metadata is not None:
            return str(user_from_dheera_ai_metadata)
