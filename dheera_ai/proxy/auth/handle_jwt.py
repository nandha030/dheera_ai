...
JWT token must have 'dheera_ai_proxy_admin' in scope.
"""
...
from dheera_ai.proxy.auth.auth_checks import can_team_access_model
...
class JWTHandler:
    ...
    - if role="dheera_ai_proxy_user" -> allow making calls + info. Can not edit budgets
    """
    ...
    def update_environment(
        ...
        self.dheera_ai_jwtauth = dheera_ai_jwtauth
        self.leeway = leeway
    ...
    def _rbac_role_from_role_mapping(self, token: dict) -> Optional[RBAC_ROLES]:
        ...
        """
        if self.dheera_ai_jwtauth.role_mappings is None:
            return None
        ...
        for role_mapping in self.dheera_ai_jwtauth.role_mappings:
    ...
    def get_rbac_role(self, token: dict) -> Optional[RBAC_ROLES]:
        ...
        Resolves: https://github.com/BerriAI/dheera_ai/issues/6793
    ...
    def is_admin(self, scopes: list) -> bool:
        if self.dheera_ai_jwtauth.admin_jwt_scope in scopes:
            return True
        return False

    def get_team_ids_from_jwt(self, token: dict) -> List[str]:

        if self.dheera_ai_jwtauth.team_ids_jwt_field is not None:
            team_ids: Optional[List[str]] = get_nested_value(
                data=token,
                key_path=self.dheera_ai_jwtauth.team_ids_jwt_field,
                default=[],
            )
            return team_ids or []
    ...
    def get_end_user_id(
        ...
        try:
            if self.dheera_ai_jwtauth.end_user_id_jwt_field is not None:
                user_id = get_nested_value(
                    data=token,
                    key_path=self.dheera_ai_jwtauth.end_user_id_jwt_field,
    ...
    def is_required_team_id(self) -> bool:
        ...
        - False: if not
        """
        if self.dheera_ai_jwtauth.team_id_jwt_field is None:
            return False
        return True
    ...
    def is_enforced_email_domain(self) -> bool:
        ...
        if self.dheera_ai_jwtauth.user_allowed_email_domain is not None and isinstance(
            self.dheera_ai_jwtauth.user_allowed_email_domain, str
        ):
            return True
        return False
    ...
    def get_team_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        try:
            if self.dheera_ai_jwtauth.team_id_jwt_field is not None:
                # Use a sentinel value to detect if the path actually exists
                sentinel = object()
                team_id = get_nested_value(
                    data=token,
                    key_path=self.dheera_ai_jwtauth.team_id_jwt_field,
                    default=sentinel,
                )
                if team_id is sentinel:
                    # Path doesn't exist, use team_id_default if available
                    if self.dheera_ai_jwtauth.team_id_default is not None:
                        return self.dheera_ai_jwtauth.team_id_default
                    else:
                        return default_value
                # At this point, team_id is not the sentinel, so it should be a string
                return team_id  # type: ignore[return-value]
            elif self.dheera_ai_jwtauth.team_id_default is not None:
                team_id = self.dheera_ai_jwtauth.team_id_default
            else:
                team_id = None
        except KeyError:
    ...
    def is_upsert_user_id(self, valid_user_email: Optional[bool] = None) -> bool:
        ...
        if valid_user_email is False:
            return False
        return self.dheera_ai_jwtauth.user_id_upsert

    def get_user_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        try:
            if self.dheera_ai_jwtauth.user_id_jwt_field is not None:
                user_id = get_nested_value(
                    data=token,
                    key_path=self.dheera_ai_jwtauth.user_id_jwt_field,
                    default=default_value,
                )
            else:
                user_id = default_value
    ...
    def get_user_roles(
        ...
        """
        try:
            if self.dheera_ai_jwtauth.user_roles_jwt_field is not None:
                user_roles = get_nested_value(
                    data=token,
                    key_path=self.dheera_ai_jwtauth.user_roles_jwt_field,
                    default=default_value,
                )
            else:
    ...
    def map_jwt_role_to_dheera_ai_role(self, token: dict) -> Optional[LitellmUserRoles]:
        """Map roles from JWT to DheeraAI user roles"""
        if not self.dheera_ai_jwtauth.jwt_dheera_ai_role_map:
            return None
        ...
        for mapping in self.dheera_ai_jwtauth.jwt_dheera_ai_role_map:
            for role in jwt_roles:
                if fnmatch.fnmatch(role, mapping.jwt_role):
                    return mapping.dheera_ai_role
        return None
    ...
    def get_jwt_role(
        ...
        """
        try:
            if self.dheera_ai_jwtauth.roles_jwt_field is not None:
                user_roles = get_nested_value(
                    data=token,
                    key_path=self.dheera_ai_jwtauth.roles_jwt_field,
                    default=default_value,
                )
            else:
                user_roles = default_value
    ...
    def is_allowed_user_role(self, user_roles: Optional[List[str]]) -> bool:
        ...
        if (
            ...
            and self.dheera_ai_jwtauth.user_allowed_roles is not None
            and any(
                role in self.dheera_ai_jwtauth.user_allowed_roles for role in user_roles
            )
        ):
            return True
        return False
    ...
    def get_user_email(
        ...
        try:
            if self.dheera_ai_jwtauth.user_email_jwt_field is not None:
                user_email = get_nested_value(
                    data=token,
                    key_path=self.dheera_ai_jwtauth.user_email_jwt_field,
                    default=default_value,
                )
            else:
                user_email = None
    ...
    def get_object_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        try:
            if self.dheera_ai_jwtauth.object_id_jwt_field is not None:
                object_id = get_nested_value(
                    data=token,
                    key_path=self.dheera_ai_jwtauth.object_id_jwt_field,
                    default=default_value,
                )
            else:
    ...
    def get_org_id(self, token: dict, default_value: Optional[str]) -> Optional[str]:
        try:
            if self.dheera_ai_jwtauth.org_id_jwt_field is not None:
                org_id = get_nested_value(
                    data=token,
                    key_path=self.dheera_ai_jwtauth.org_id_jwt_field,
                    default=default_value,
                )
            else:
                org_id = None
    ...
    async def get_public_key(self, kid: Optional[str]) -> dict:
        ...
        for key_url in keys_url_list:
            cache_key = f"dheera_ai_jwt_auth_keys_{key_url}"
            ...
            if cached_keys is None:
                ...
                await self.user_api_key_cache.async_set_cache(
                    key=cache_key,
                    value=keys,
                    ttl=self.dheera_ai_jwtauth.public_key_ttl,  # cache for 10 mins
                )
            else:
                keys = cached_keys
    ...
    def is_allowed_domain(self, user_email: str) -> bool:
        if self.dheera_ai_jwtauth.user_allowed_email_domain is None:
            return True

        email_domain = user_email.split("@")[-1]  # Extract domain from email
        if email_domain == self.dheera_ai_jwtauth.user_allowed_email_domain:
            return True
        else:
            return False
    ...
    async def get_oidc_userinfo(self, token: str) -> dict:
        ...
        """
        if not self.dheera_ai_jwtauth.oidc_userinfo_endpoint:
            raise Exception(
        ...
        verbose_proxy_logger.debug(
            f"Calling OIDC UserInfo endpoint: {self.dheera_ai_jwtauth.oidc_userinfo_endpoint}"
        )
        
        try:
            # Call the UserInfo endpoint with the access token
            response = await self.http_handler.get(
                url=self.dheera_ai_jwtauth.oidc_userinfo_endpoint,
                headers={
            ...
            await self.user_api_key_cache.async_set_cache(
                key=cache_key,
                value=userinfo,
                ttl=self.dheera_ai_jwtauth.oidc_userinfo_cache_ttl,
            )
...
class JWTAuthManager:
    ...
    async def check_rbac_role(
        ...
        if jwt_handler.dheera_ai_jwtauth.enforce_rbac is True:
    ...
    async def check_admin_access(
        ...
        is_allowed = allowed_routes_check(
            ...
            user_route=route,
            dheera_ai_proxy_roles=jwt_handler.dheera_ai_jwtauth,
        )
        if not is_allowed:
            allowed_routes: List[Any] = jwt_handler.dheera_ai_jwtauth.admin_allowed_routes
    ...
    async def find_and_validate_specific_team_id(
    ...
    ) -> Tuple[Optional[str], Optional[DheeraAI_TeamTable]]:
        ...
        if not individual_team_id and jwt_handler.is_required_team_id() is True:
            raise Exception(
                f"No team id found in token. Checked team_id field '{jwt_handler.dheera_ai_jwtauth.team_id_jwt_field}'"
            )

        ## VALIDATE TEAM OBJECT ###
        team_object: Optional[DheeraAI_TeamTable] = None
        if individual_team_id:
            team_object = await get_team_object(
                ...
                team_id_upsert=jwt_handler.dheera_ai_jwtauth.team_id_upsert,
            )
    ...
    async def find_team_with_model_access(
        ...
        if not team_ids:
            if jwt_handler.dheera_ai_jwtauth.enforce_team_based_model_access:
                raise HTTPException(
                    status_code=403,
    ...
    async def get_objects(
        ...
        org_object: Optional[DheeraAI_OrganizationTable] = None
        if org_id:
            org_object = (
        ...
        user_object: Optional[DheeraAI_UserTable] = None
        if user_id:
            user_object = (
        ...
        end_user_object: Optional[DheeraAI_EndUserTable] = None
        if end_user_id:
            end_user_object = (
        ...
        team_membership_object: Optional[DheeraAI_TeamMembership] = None
    ...
    def get_team_id_from_header(
        ...
        """
        Extract team_id from x-dheera_ai-team-id header if present.
        ...
        header_team_id = normalized_headers.get("x-dheera_ai-team-id")
        ...
        if header_team_id not in allowed_team_ids:
            raise HTTPException(
                status_code=403,
                detail=f"Team '{header_team_id}' from x-dheera_ai-team-id header is not in your JWT's allowed teams. Allowed teams: {list(allowed_team_ids)}",
            )

        verbose_proxy_logger.debug(
            f"Using team_id from x-dheera_ai-team-id header: {header_team_id}"
        )
    ...
    async def sync_user_role_and_teams(
        ...
        1. The user role on DheeraAI DB is in sync with the IDP provider role
        ...
        """
        if not jwt_handler.dheera_ai_jwtauth.sync_user_role_and_teams:
            return None
    ...
    async def auth_builder(
        ...
        if jwt_handler.dheera_ai_jwtauth.oidc_userinfo_enabled:
            verbose_proxy_logger.debug(
        ...
        if jwt_handler.dheera_ai_jwtauth.custom_validate:
            if not jwt_handler.dheera_ai_jwtauth.custom_validate(jwt_valid_token):
                raise HTTPException(
                    status_code=403,
        ...
        if (
            jwt_handler.dheera_ai_jwtauth.enforce_scope_based_access
            and jwt_handler.dheera_ai_jwtauth.scope_mappings
        ):
            JWTAuthManager.check_scope_based_access(
                scope_mappings=jwt_handler.dheera_ai_jwtauth.scope_mappings,
                scopes=scopes,
        ...
        ## Check if team_id is specified via x-dheera_ai-team-id header
