# What is this?
## If dheera_ai license in env, checks if it's valid
import base64
import json
import os
...
from dheera_ai.constants import NON_LLM_CONNECTION_TIMEOUT
from dheera_ai.llms.custom_httpx.http_handler import HTTPHandler

if TYPE_CHECKING:
    from dheera_ai.proxy._types import EnterpriseLicenseData
...
class LicenseCheck:
    ...
    base_url = "https://license.dheera_ai.ai"

    def __init__(self) -> None:
        self.license_str = os.getenv("DHEERA_AI_LICENSE", None)
    ...
    def _verify(self, license_str: str) -> bool:
        verbose_proxy_logger.debug(
            "dheera_ai.proxy.auth.dheera_ai_license.py::_verify - Checking license against {}/verify_license - {}".format(
        ...
        try:  # don't impact user, if call fails
            ...
            verbose_proxy_logger.debug(
                "dheera_ai.proxy.auth.dheera_ai_license.py::_verify - License={} is premium={}".format(
                    license_str, premium
                )
            )
            return premium
            ...
            verbose_proxy_logger.exception(
                "dheera_ai.proxy.auth.dheera_ai_license.py::_verify - Unable to verify License={} via api. - {}".format(
                    license_str, str(e)
                )
            )
            return False
    ...
    def is_premium(self) -> bool:
        ...
        2. _verify: checks if license is valid calling dheera_ai API. This is the old way we were generating/validating license
        """
        try:
            verbose_proxy_logger.debug(
                "dheera_ai.proxy.auth.dheera_ai_license.py::is_premium() - ENTERING 'IS_PREMIUM' - DheeraAI License={}".format(
                    self.license_str
                )
            )
            ...
            verbose_proxy_logger.debug(
                "dheera_ai.proxy.auth.dheera_ai_license.py::is_premium() - Updated 'self.license_str' - {}".format(
                    self.license_str
                )
            )
    ...
    def verify_license_without_api_request(self, public_key, license_key):
        try:
            ...
            verbose_proxy_logger.debug(
                "dheera_ai.proxy.auth.dheera_ai_license.py::verify_license_without_api_request - Unable to verify License locally. - {}".format(
                    str(e)
                )
            )
            return False
