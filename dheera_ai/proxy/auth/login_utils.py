...
from dheera_ai.proxy.management_endpoints.internal_user_endpoints import user_update
...
from dheera_ai.proxy.utils import PrismaClient, get_server_root_path
...
async def authenticate_user(
    ...
    _user_row: Optional[DheeraAI_UserTable] = None
    user_role: Optional[
        Literal[
    ...
    if secrets.compare_digest(username, ui_username) and secrets.compare_digest(
        ...
        key_user_id = DHEERA_AI_PROXY_ADMIN_NAME
        if (
            os.getenv("PROXY_ADMIN_ID", None) is not None
            and os.environ["PROXY_ADMIN_ID"] == user_id
        ) or user_id == DHEERA_AI_PROXY_ADMIN_NAME:
            # checks if user is admin
            key_user_id = os.getenv("PROXY_ADMIN_ID", DHEERA_AI_PROXY_ADMIN_NAME)
        ...
        if get_secret_bool("EXPERIMENTAL_UI_LOGIN"):
            ...
            user_info: Optional[DheeraAI_UserTable] = None
