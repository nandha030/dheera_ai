...
from dheera_ai.proxy._types import SpecialModelNames, UserAPIKeyAuth
...
def get_provider_models(
    provider: str, dheera_ai_params: Optional[DheeraAI_Params] = None
) -> Optional[List[str]]:
    """
    Returns the list of known models by provider
    """
    if provider == "*":
        return get_valid_models(dheera_ai_params=dheera_ai_params)

    if provider in dheera_ai.models_by_provider:
        provider_models = get_valid_models(
            custom_llm_provider=provider, dheera_ai_params=dheera_ai_params
        )
...
def get_known_models_from_wildcard(
    wildcard_model: str, dheera_ai_params: Optional[DheeraAI_Params] = None
) -> List[str]:
    try:
    ...
    if dheera_ai_params is None:  # need dheera_ai params to extract dheera_ai model name
        return []

    try:
        provider = dheera_ai_params.model.split("/", 1)[0]
    except ValueError:
    ...
    wildcard_models = get_provider_models(
        provider=provider, dheera_ai_params=dheera_ai_params
    )
...
def _get_wildcard_models(
    ...
    for model in unique_models:
        if _check_wildcard_routing(model=model):
            ...
            ## get dheera_ai params from model
                    ...
                    for router_model in model_list:
                        wildcard_models = get_known_models_from_wildcard(
                            ...
                            dheera_ai_params=DheeraAI_Params(
                                **router_model["dheera_ai_params"]  # type: ignore
                            ),
                        )
