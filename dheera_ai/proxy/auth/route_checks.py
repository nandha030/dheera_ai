...
class RouteChecks:
    ...
    def should_call_route(route: str, valid_token: UserAPIKeyAuth):
        ...
        """
        try:
            from dheera_ai_enterprise.proxy.auth.route_checks import EnterpriseRouteChecks
    ...
    def is_virtual_key_allowed_to_call_route(
        ...
        ## check if 'allowed_route' is a field name in DheeraAIRoutes
        if any(
            allowed_route in DheeraAIRoutes._member_names_
            for allowed_route in valid_token.allowed_routes
        ):
            for allowed_route in valid_token.allowed_routes:
                if allowed_route in DheeraAIRoutes._member_names_:
                    if RouteChecks.check_route_access(
                        route=route,
                        allowed_routes=DheeraAIRoutes._member_map_[allowed_route].value,
                    ):
                        return True
    ...
    def _raise_admin_only_route_exception(
        ...
        Args:
            user_obj (Optional[DheeraAI_UserTable]): The user object
    ...
    def non_proxy_admin_allowed_routes_check(
        ...
        if RouteChecks.is_llm_api_route(route=route):
            ...
            route in DheeraAIRoutes.info_routes.value
            ...
            route in DheeraAIRoutes.global_spend_tracking_routes.value
            ...
            and RouteChecks.check_route_access(
                route=route, allowed_routes=DheeraAIRoutes.internal_user_routes.value
            )
        ):
            pass
            ...
            route=route, allowed_routes=DheeraAIRoutes.org_admin_allowed_routes.value
        ):
            pass
        elif (
            ...
            and RouteChecks.check_route_access(
                route=route,
                allowed_routes=DheeraAIRoutes.internal_user_view_only_routes.value,
            )
        ):
            pass
        elif RouteChecks.check_route_access(
            route=route, allowed_routes=DheeraAIRoutes.self_managed_routes.value
    ...
    def custom_admin_only_route_check(route: str):
        from dheera_ai.proxy.proxy_server import general_settings, premium_user
    ...
    def is_llm_api_route(route: str) -> bool:
        ...
        if route in DheeraAIRoutes.openai_routes.value:
            return True

        if route in DheeraAIRoutes.anthropic_routes.value:
            return True
        ...
        if RouteChecks.check_route_access(
            route=route, allowed_routes=DheeraAIRoutes.agent_routes.value
        ):
            return True

        # fuzzy match routes like "/v1/threads/thread_49EIN5QF32s4mH20M7GFKdlZ"
        # Check for routes with placeholders
        for openai_route in DheeraAIRoutes.openai_routes.value:
        ...
        for _llm_passthrough_route in DheeraAIRoutes.mapped_pass_through_routes.value:
    ...
    def is_management_route(route: str) -> bool:
        ...
        """
        return route in DheeraAIRoutes.management_routes.value
    ...
    def _check_proxy_admin_viewer_access(
        ...
        if RouteChecks.check_route_access(
            route=route, allowed_routes=DheeraAIRoutes.management_routes.value
        ):
            ...
            route=route, allowed_routes=DheeraAIRoutes.admin_viewer_routes.value
        ):
            # Allow access to admin viewer routes (read-only admin endpoints)
            return
        elif RouteChecks.check_route_access(
            route=route, allowed_routes=DheeraAIRoutes.global_spend_tracking_routes.value
        ):
