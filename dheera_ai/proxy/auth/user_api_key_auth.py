"""
This file handles authentication for the DheeraAI Proxy.

it checks if the user passed a valid API Key to the DheeraAI Proxy
...
from dheera_ai.proxy.auth.auth_exception_handler import UserAPIKeyAuthExceptionHandler
...
from dheera_ai.proxy.auth.handle_jwt import JWTAuthManager, JWTHandler
from dheera_ai.proxy.auth.oauth2_check import Oauth2Handler
from dheera_ai.proxy.auth.oauth2_proxy_hook import handle_oauth2_proxy_request
...
try:
    from dheera_ai_enterprise.proxy.auth.user_api_key_auth import (
...
custom_dheera_ai_key_header = APIKeyHeader(
    name=SpecialHeaders.custom_dheera_ai_api_key.value,
    auto_error=False,
...
def _apply_budget_limits_to_end_user_params(
    ...
    budget_info: DheeraAI_BudgetTable,
    end_user_id: str,
) -> None:
    """
...
async def get_global_proxy_spend(
    ...
    if dheera_ai.max_budget > 0:  # user set proxy max budget
        # check cache
        ...
        if global_proxy_spend is not None:
            user_info = CallInfo(
                user_id=dheera_ai_proxy_admin_name,
...
def get_api_key(
    custom_dheera_ai_key_header: Optional[str],
    api_key: str,
    ...
    if isinstance(custom_dheera_ai_key_header, str):
        passed_in_key = custom_dheera_ai_key_header
        api_key = _get_bearer_token_or_received_api_key(custom_dheera_ai_key_header)
        ...
        for endpoint in pass_through_endpoints:
            if endpoint.get("path", "") == route:
                ...
                if headers is not None:
                    header_key: str = headers.get("dheera_ai_user_api_key", "")
...
async def check_api_key_for_custom_headers_or_pass_through_endpoints(
    ...
    for mapped_route in DheeraAIRoutes.mapped_pass_through_routes.value:  # type: ignore
        if route.startswith(mapped_route):
            is_mapped_pass_through_route = True
    if is_mapped_pass_through_route:
        if request.headers.get("dheera_ai_user_api_key") is not None:
            api_key = request.headers.get("dheera_ai_user_api_key") or ""
        ...
        for endpoint in pass_through_endpoints:
            if isinstance(endpoint, dict) and endpoint.get("path", "") == route:
                ...
                if (
                    ...
                    - check the langfuse public key if it contains the dheera_ai api key
                    """
                    import base64
                    ...
                    if headers is not None:
                        header_key = headers.get("dheera_ai_user_api_key", "")
                        if (
...
async def _user_api_key_auth_builder(  # noqa: PLR0915
    ...
    azure_apim_header: Optional[str],
    request_data: dict,
    custom_dheera_ai_key_header: Optional[str] = None,
) -> UserAPIKeyAuth:
    ...
    try:
        ...
        api_key, passed_in_key = get_api_key(
            custom_dheera_ai_key_header=custom_dheera_ai_key_header,
            api_key=api_key,
        ...
        # if user wants to pass DheeraAI_Master_Key as a custom header, example pass dheera_ai keys as X-DheeraAI-Key: Bearer sk-1234
        custom_dheera_ai_key_header_name = general_settings.get("dheera_ai_key_header_name")
        if custom_dheera_ai_key_header_name is not None:
            api_key = get_api_key_from_custom_header(
                request=request,
                custom_dheera_ai_key_header_name=custom_dheera_ai_key_header_name,
            )

        if open_telemetry_logger is not None:
            parent_otel_span = (
                open_telemetry_logger.create_dheera_ai_proxy_request_started_span(
        ...
        if (
            route in DheeraAIRoutes.public_routes.value  # type: ignore
        ...
        if general_settings.get("enable_jwt_auth", False) is True:
            ...
            if is_jwt:
                ...
                token = result["token"]
                team_membership: Optional[DheeraAI_TeamMembership] = result.get(
        ...
        if end_user_id:
            try:
                ...
                if _end_user_object is not None:
                    ...
                    if _end_user_object.dheera_ai_budget_table is not None:
                        _apply_budget_limits_to_end_user_params(
                            end_user_params=end_user_params,
                            budget_info=_end_user_object.dheera_ai_budget_table,
                            end_user_id=end_user_id,
                        )
                elif dheera_ai.max_end_user_budget_id is not None:
        ...
        if (
            ...
            try:
                team_obj: DheeraAI_TeamTableCachedObj = await get_team_object(
        ...
        if route in DheeraAIRoutes.master_key_only_routes.value:  # type: ignore
            raise Exception(
        ...
        if valid_token is None:
            if isinstance(
                ...
                assert api_key.startswith(
                    "sk-"
                ), "DheeraAI Virtual Key expected. Received={}, expected to start with 'sk-'.".format(
                    api_key
                )  # prevent token hashes from being used
            else:
                verbose_logger.warning(
                    "dheera_ai.proxy.proxy_server.user_api_key_auth(): Warning - Key is not a string. Got type={}".format(
            ...
            try:
                ...
                if e.code == 401 or e.code == "401":
                    e.message = "Authentication Error, Invalid proxy server token passed. Received API Key = {}, Key Hash (Token) ={}. Unable to find token in cache or `DheeraAI_VerificationTokenTable`".format(
        ...
        if valid_token is not None:
            ...
            if valid_token.user_id is not None:
                try:
                    ...
                    verbose_logger.debug(
                        "dheera_ai.proxy.auth.user_api_key_auth.py::user_api_key_auth() - Unable to get user from db/cache. Setting user_obj to None. Exception received - {}".format(
                            str(e)
                        )
                    )
                    user_obj = None
            ...
            if valid_token.team_member_spend is not None:
                ...
                if prisma_client is not None:
                    ...
                    if (
                        ...
                        and team_member_info.dheera_ai_budget_table is not None
                    ):
                        team_member_budget = (
                            team_member_info.dheera_ai_budget_table.max_budget
                        )
            ...
            if valid_token.team_id is not None:
                _team_obj: Optional[DheeraAI_TeamTable] = DheeraAI_TeamTable(
                    team_id=valid_token.team_id,
            ...
            global_proxy_spend = None
            if (
                dheera_ai.max_budget > 0 and prisma_client is not None
        ...
        # check if token is from dheera_ai-ui, dheera_ai ui makes keys to allow users to login with sso. These keys can only be used for DheeraAI UI functions
...
async def user_api_key_auth(
    ...
    custom_dheera_ai_key_header: Optional[str] = fastapi.Security(
        custom_dheera_ai_key_header
    ),
) -> UserAPIKeyAuth:
    """
...
def get_api_key_from_custom_header(
    request: Request, custom_dheera_ai_key_header_name: str
) -> str:
    """
    ...
    Args:
        ...
        custom_dheera_ai_key_header_name (str): Custom header name

    Returns:
        Optional[str]: API key
    """
    api_key: str = ""
    # use this as the virtual key passed to dheera_ai proxy
    custom_dheera_ai_key_header_name = custom_dheera_ai_key_header_name.lower()
    _headers = {k.lower(): v for k, v in request.headers.items()}
    verbose_proxy_logger.debug(
        "searching for custom_dheera_ai_key_header_name= %s, in headers=%s",
        custom_dheera_ai_key_header_name,
        _headers,
    )
    custom_api_key = _headers.get(custom_dheera_ai_key_header_name)
    if custom_api_key:
        api_key = _get_bearer_token(api_key=custom_api_key)
        verbose_proxy_logger.debug(
            "Found custom API key using header: {}, setting api_key={}".format(
                custom_dheera_ai_key_header_name, abbreviate_api_key(api_key)
            )
        )
    else:
        verbose_proxy_logger.exception(
            f"No DheeraAI Virtual Key pass. Please set header={custom_dheera_ai_key_header_name}: Bearer <api_key>"
        )
    return api_key
