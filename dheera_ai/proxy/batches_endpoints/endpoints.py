...
from dheera_ai.batches.main import CancelBatchRequest, RetrieveBatchRequest
...
from dheera_ai.proxy.utils import handle_exception_on_proxy, is_known_model
from dheera_ai.types.llms.openai import DheeraAIBatchCreateRequest
...
async def create_batch(  # noqa: PLR0915
    ...
    try:
        ...
        _create_batch_data = DheeraAIBatchCreateRequest(**data)
        ...
        if model_from_file_id is not None and input_file_id:
            ...
            response = await dheera_ai.acreate_batch(
        ...
        ):  # dheera_ai_proxy:application/octet-stream;unified_id,c4843482-b176-4901-8292-7523fd0f2c6e;target_model_names,gpt-4o-mini
            ...
            model_param = (
                ...
                or request.headers.get("x-dheera_ai-model")
            )
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.create_batch(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def retrieve_batch(
    ...
    try:
        ...
        if model_from_id is not None:
            ...
            response = await dheera_ai.aretrieve_batch(
                custom_llm_provider=credentials["custom_llm_provider"],
                **data  # type: ignore
            )
        ...
        elif dheera_ai.enable_loadbalancing_on_batch_endpoints is True or unified_batch_id:
            if llm_router is None:
                raise HTTPException(
                    status_code=500,
                    detail={
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.retrieve_batch(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def list_batches(
    ...
    try:
        ...
        if model_param:
            ...
            response = await dheera_ai.alist_batches(
                custom_llm_provider=credentials["custom_llm_provider"],
                after=after,
                limit=limit,
                **data  # type: ignore
...
async def cancel_batch(
    ...
    try:
        ...
        if model_from_id is not None:
            ...
            response = await dheera_ai.acancel_batch(
                custom_llm_provider=credentials["custom_llm_provider"],
                **data  # type: ignore
            )
