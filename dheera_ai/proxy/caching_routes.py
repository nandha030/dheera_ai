...
from dheera_ai.types.caching import CachePingResponse, HealthCheckCacheParams
...
def _extract_cache_params() -> Dict[str, Any]:
    ...
    if dheera_ai.cache is None:
        return {}
    try:
        cache_params = vars(dheera_ai.cache.cache)
        cleaned_params = (
...
async def cache_ping():
    ...
    """
    dheera_ai_cache_params: Dict[str, Any] = {}
    cleaned_cache_params: Dict[str, Any] = {}
    try:
        if dheera_ai.cache is None:
            raise HTTPException(
                status_code=503, detail="Cache not initialized. dheera_ai.cache is None"
            )
        dheera_ai_cache_params = masker.mask_dict(vars(dheera_ai.cache))
        # remove field that might reference itself
        dheera_ai_cache_params.pop("cache", None)
        cleaned_cache_params = _extract_cache_params()

        if dheera_ai.cache.type == "redis":
            ping_response = await dheera_ai.cache.ping()
            verbose_proxy_logger.debug(
                "/cache/ping: ping_response: " + str(ping_response)
            )
            # add cache does not return anything
            await dheera_ai.cache.async_add_cache(
                result="test_key",
                model="test-model",
                messages=[{"role": "user", "content": "test from dheera_ai"}],
            )
            verbose_proxy_logger.debug("/cache/ping: done with set_cache()")

            return CachePingResponse(
                status="healthy",
                cache_type=str(dheera_ai.cache.type),
                ping_response=True,
                set_cache_response="success",
                dheera_ai_cache_params=safe_dumps(dheera_ai_cache_params),
...
async def cache_delete(request: Request):
    """
    Endpoint for deleting a key from the cache. All responses from dheera_ai proxy have `x-dheera_ai-cache-key` in the headers
    ...
    try:
        ...
        if dheera_ai.cache.type == "redis":
            await dheera_ai.cache.delete_cache_keys(keys=keys)
            return {
                "status": "success",
            }
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Cache type {dheera_ai.cache.type} does not support deleting a key. only `redis` is supported",
            )
    except Exception as e:
        raise HTTPException(
            status_code=500,
...
async def cache_redis_info():
    ...
    try:
        ...
        if not (
            dheera_ai.cache.type == "redis"
            and isinstance(dheera_ai.cache.cache, RedisCache)
        ):
            raise HTTPException(
                status_code=500,
                detail=f"Cache type {dheera_ai.cache.type} does not support redis info",
            )

        # Get client information (handles CLIENT LIST restrictions gracefully)
        client_list, num_clients = _get_redis_client_info(dheera_ai.cache.cache)

        # Get Redis server information
        redis_info = dheera_ai.cache.cache.info()
...
async def cache_flushall():
    ...
    try:
        ...
        if dheera_ai.cache.type == "redis" and isinstance(
            dheera_ai.cache.cache, RedisCache
        ):
            dheera_ai.cache.cache.flushall()
            return {
                "status": "success",
            }
        else:
            raise HTTPException(
                status_code=500,
                detail=f"Cache type {dheera_ai.cache.type} does not support flushing",
            )
    except Exception as e:
        raise HTTPException(
            status_code=503,
