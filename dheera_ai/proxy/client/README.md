# Dheera AI Proxy Client

A Python client library for interacting with the Dheera AI proxy server. This client provides a clean, typed interface for managing models, keys, credentials, and making chat completions.
...
```python
from dheera_ai.proxy.client import Client

# Initialize the client
client = Client(
    base_url="http://localhost:4000",  # Your Dheera AI proxy server URL
...
Make chat completion requests to your Dheera AI proxy:
...
client.model_groups.create(
    name="gpt4-group",
    models=[
        {"model_name": "gpt-4", "dheera_ai_params": {"api_key": "key1"}},
        {"model_name": "gpt-4-backup", "dheera_ai_params": {"api_key": "key2"}}
    ]
)
...
```python
from dheera_ai.proxy.client import UsersManagementClient
...
to the Dheera AI proxy server. This is useful when you need more control or when
...
```python
from dheera_ai.proxy.client.exceptions import UnauthorizedError
...
The Dheera AI CLI supports SSO authentication through a polling-based approach that works with any OAuth-compatible SSO provider.
...
sequenceDiagram
    ...
    participant Proxy as Dheera AI Proxy
    participant SSO as SSO Provider
    
    CLI->>CLI: Generate key ID (sk-uuid)
    CLI->>Browser: Open /sso/key/generate?source=dheera_ai-cli&key=sk-uuid
    
    Browser->>Proxy: GET /sso/key/generate?source=dheera_ai-cli&key=sk-uuid
    Proxy->>Proxy: Set cli_state = dheera_ai-session-token:sk-uuid
    Proxy->>SSO: Redirect with state=dheera_ai-session-token:sk-uuid
    
    SSO->>Browser: Show login page
    Browser->>SSO: User authenticates
    SSO->>Proxy: Redirect to /sso/callback?state=dheera_ai-session-token:sk-uuid
    
    Proxy->>Proxy: Check if state starts with "dheera_ai-session-token:"
    ...
    CLI->>CLI: Save key to ~/.dheera_ai/token.json
```
...
- **`dheera_ai-proxy login`** - Start SSO authentication flow
- **`dheera_ai-proxy logout`** - Clear stored authentication token
- **`dheera_ai-proxy whoami`** - Show current authentication status
...
3. **SSO Redirect**: Proxy sets the formatted state (`dheera_ai-session-token:sk-uuid`) as OAuth state parameter and redirects to SSO provider
...
6. **Key Generation**: Proxy detects CLI login (state starts with "dheera_ai-session-token:") and generates API key with pre-specified ID
7. **Polling**: CLI polls `/sso/cli/poll/{key_id}` endpoint until key is ready
8. **Token Storage**: CLI saves the authentication token to `~/.dheera_ai/token.json`
...
Authentication tokens are stored in `~/.dheera_ai/token.json` with restricted file permissions (600). The stored token includes:
...
dheera_ai-proxy whoami

# Logout
dheera_ai-proxy logout
``` 