...
The Dheera AI Proxy CLI is a command-line tool for managing your Dheera AI proxy server. It provides commands for managing models, viewing server status, and interacting with the proxy server.
...
- `DHEERA_AI_PROXY_URL`: Base URL of the Dheera AI proxy server (default: http://localhost:4000)
- `DHEERA_AI_PROXY_API_KEY`: API key for authentication

## Global Options

- `--version`, `-v`: Print the Dheera AI Proxy client and server version and exit.

Example:

```bash
dheera_ai-proxy version
# or
dheera_ai-proxy --version
# or
dheera_ai-proxy -v
...
The CLI provides several commands for managing models on your Dheera AI proxy server:
...
```bash
dheera_ai-proxy models list [--format table|json]
```
...
```bash
dheera_ai-proxy models info [options]
```
...
```bash
dheera_ai-proxy models add <model-name> [options]
```
...
```bash
dheera_ai-proxy models add gpt-4 -p api_key=sk-123 -p api_base=https://api.openai.com -i description="GPT-4 model"
```
...
```bash
dheera_ai-proxy models get [--id MODEL_ID] [--name MODEL_NAME]
```
...
```bash
dheera_ai-proxy models update <model-id> [options]
```
...
```bash
dheera_ai-proxy models import models.yaml
```

Options:

- `--dry-run`: Show what would be imported without making any changes.
- `--only-models-matching-regex <regex>`: Only import models where `dheera_ai_params.model` matches the given regex.
...
```bash
dheera_ai-proxy models import models.yaml --dry-run
```

3. Only import models where the model name contains 'gpt':

```bash
dheera_ai-proxy models import models.yaml --only-models-matching-regex gpt
```

4. Only import models with access group containing 'beta':

```bash
dheera_ai-proxy models import models.yaml --only-access-groups-matching-regex beta
```

5. Combine both filters:

```bash
dheera_ai-proxy models import models.yaml --only-models-matching-regex gpt --only-access-groups-matching-regex beta
```

### Credentials Management

The CLI provides commands for managing credentials on your Dheera AI proxy server:
...
```bash
dheera_ai-proxy credentials list [--format table|json]
```
...
```bash
dheera_ai-proxy credentials create <credential-name> --info <json-string> --values <json-string>
```
...
```bash
dheera_ai-proxy credentials create azure-cred \
...
```bash
dheera_ai-proxy credentials get <credential-name>
```
...
```bash
dheera_ai-proxy credentials delete <credential-name>
```

### Keys Management

The CLI provides commands for managing API keys on your Dheera AI proxy server:
...
```bash
dheera_ai-proxy keys list [--format table|json] [options]
```
...
```bash
dheera_ai-proxy keys generate [options]
```
...
```bash
dheera_ai-proxy keys generate --models gpt-4,gpt-3.5-turbo --spend 100 --duration 24h --key-alias my-key --team-id team123
```
...
```bash
dheera_ai-proxy keys delete [--keys <comma-separated-keys>] [--key-aliases <comma-separated-aliases>]
```
...
```bash
dheera_ai-proxy keys info --key <key-hash>
```
...
The CLI provides commands for managing users on your Dheera AI proxy server:
...
```bash
dheera_ai-proxy users create --email user@example.com --role internal_user --alias "Alice" --team team1 --max-budget 100.0
```
...
```bash
dheera_ai-proxy users delete <user-id-1> <user-id-2>
```

### Chat Commands

The CLI provides commands for interacting with chat models through your Dheera AI proxy server:
...
```bash
dheera_ai-proxy chat completions <model> [options]
```
...
The CLI provides commands for making direct HTTP requests to your Dheera AI proxy server:
...
```bash
dheera_ai-proxy http request <method> <uri> [options]
```
...
1. List models:
```bash
dheera_ai-proxy http request GET /models
```

2. Create a chat completion:
```bash
dheera_ai-proxy http request POST /chat/completions -j '{"model": "gpt-4", "messages": [{"role": "user", "content": "Hello"}]}'
```

3. Test connection with custom headers:
```bash
dheera_ai-proxy http request GET /health/test_connection -H "X-Custom-Header:value"
```
...
```bash
dheera_ai-proxy models add gpt-4 -p api_key=sk-123 -p max_tokens=2048
```

3. Get model information in JSON format:

```bash
dheera_ai-proxy models info --format json
```

4. Update model parameters:

```bash
dheera_ai-proxy models update model-123 -p temperature=0.7 -i description="Updated model"
```
...
```bash
dheera_ai-proxy http request POST /chat/completions \
...
# Get user info
dheera_ai-proxy users get --id u1

# Create a user
dheera_ai-proxy users create --email a@b.com --role internal_user --alias "Alice" --team team1 --max-budget 100.0

# Delete users
dheera_ai-proxy users delete u1 u2
```
