...
def get_stored_api_key() -> Optional[str]:
    ...
    return get_dheera_ai_gateway_api_key()
...
def login(ctx: click.Context):
    """Login to DheeraAI proxy using SSO authentication"""
    from dheera_ai._uuid import uuid
    from dheera_ai.constants import DHEERA_AI_CLI_SOURCE_IDENTIFIER
    from dheera_ai.proxy.client.cli.interface import show_commands
    ...
    try:
        ...
        sso_url = f"{base_url}/sso/key/generate?source={DHEERA_AI_CLI_SOURCE_IDENTIFIER}&key={key_id}"
...
def whoami():
    ...
    if not token_data:
        click.echo("❌ Not authenticated. Run 'dheera_ai-proxy login' to authenticate.")
        return
