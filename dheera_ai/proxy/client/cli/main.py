...
from dheera_ai.proxy.client.health import HealthManagementClient
...
def print_version(base_url: str, api_key: Optional[str]):
    ...
    click.echo(f"DheeraAI Proxy CLI Version: {dheera_ai_version}")
    if base_url:
        click.echo(f"DheeraAI Proxy Server URL: {base_url}")
    try:
        health_client = HealthManagementClient(base_url=base_url, api_key=api_key)
        server_version = health_client.get_server_version()
        if server_version:
            click.echo(f"DheeraAI Proxy Server Version: {server_version}")
        else:
            click.echo("DheeraAI Proxy Server Version: (unavailable)")
...
@click.option(
    ...
    help="Show the DheeraAI Proxy CLI and server version and exit.",
...
@click.option(
    "--base-url",
    envvar="DHEERA_AI_PROXY_URL",
    show_envvar=True,
    default="http://localhost:4000",
    help="Base URL of the DheeraAI proxy server",
)
@click.option(
    "--api-key",
    envvar="DHEERA_AI_PROXY_API_KEY",
    show_envvar=True,
    help="API key for authentication",
)
@click.pass_context
def cli(ctx: click.Context, base_url: str, api_key: Optional[str]) -> None:
    """DheeraAI Proxy CLI - Manage your DheeraAI proxy server"""
...
def version(ctx: click.Context):
    """Show the DheeraAI Proxy CLI and server version."""
