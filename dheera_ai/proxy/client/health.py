...
class HealthManagementClient:
    """
    Client for interacting with the health endpoints of the DheeraAI proxy server.
    """
    ...
    def get_readiness(self) -> Dict[str, Any]:
        """
        Check the readiness of the DheeraAI proxy server.
    ...
    def get_server_version(self) -> Optional[str]:
        """
        Get the DheeraAI server version from the readiness endpoint.

        Returns:
            Optional[str]: The server version if available, otherwise None.
        """
        readiness = self.get_readiness()
        return readiness.get("dheera_ai_version") 