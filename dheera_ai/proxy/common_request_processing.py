...
from dheera_ai.proxy._types import ProxyException, UserAPIKeyAuth
...
from dheera_ai.types.utils import ServerToolUse
...
from dheera_ai.types.utils import ModelResponse, ModelResponseStream, Usage
...
def _get_cost_breakdown_from_logging_obj(
    dheera_ai_logging_obj: Optional[DheeraAILoggingObj],
    ...
    """
    if not dheera_ai_logging_obj or not hasattr(dheera_ai_logging_obj, "cost_breakdown"):
        return None, None

    cost_breakdown = dheera_ai_logging_obj.cost_breakdown
...
class ProxyBaseLLMRequestProcessing:
    ...
    def get_custom_headers(
        *,
        ...
        dheera_ai_logging_obj: Optional[DheeraAILoggingObj] = None,
        **kwargs,
    ) -> dict:
        ...
        headers = {
            "x-dheera_ai-call-id": call_id,
            "x-dheera_ai-model-id": model_id,
            "x-dheera_ai-cache-key": cache_key,
            "x-dheera_ai-model-api-base": (
                api_base.split("?")[0] if api_base else None
            ),  # don't include query params, risk of leaking sensitive info
            "x-dheera_ai-version": version,
            "x-dheera_ai-model-region": model_region,
            "x-dheera_ai-response-cost": str(response_cost),
            "x-dheera_ai-response-cost-original": (
                str(original_cost) if original_cost is not None else None
            ),
            "x-dheera_ai-response-cost-discount-amount": (
                str(discount_amount) if discount_amount is not None else None
            ),
            "x-dheera_ai-key-tpm-limit": str(user_api_key_dict.tpm_limit),
            "x-dheera_ai-key-rpm-limit": str(user_api_key_dict.rpm_limit),
            "x-dheera_ai-key-max-budget": str(user_api_key_dict.max_budget),
            "x-dheera_ai-key-spend": str(updated_spend),
            "x-dheera_ai-response-duration-ms": str(
                hidden_params.get("_response_ms", None)
            ),
            "x-dheera_ai-overhead-duration-ms": str(
                hidden_params.get("dheera_ai_overhead_time_ms", None)
            ),
            "x-dheera_ai-fastest_response_batch_completion": (
                str(fastest_response_batch_completion)
                if fastest_response_batch_completion is not None
                else None
            ),
            "x-dheera_ai-timeout": str(timeout) if timeout is not None else None,
    ...
    async def common_processing_pre_call_logic(
    ...
    ) -> Tuple[dict, DheeraAILoggingObj]:
        start_time = datetime.now()  # start before calling guardrail hooks

        self.data = await add_dheera_ai_data_to_request(
            data=self.data,
            request=request,
        ...
        if (
            ...
            and self.data["model"] in dheera_ai.model_alias_map
        ):
            self.data["model"] = dheera_ai.model_alias_map[self.data["model"]]
        ...
        self.data["dheera_ai_call_id"] = request.headers.get(
            "x-dheera_ai-call-id", str(uuid.uuid4())
        )
        ### CALL HOOKS ### - modify/reject incoming data before calling the model

        ## LOGGING OBJECT ## - initialize logging object for logging success/failure events for call
        ## IMPORTANT Note: - initialize this before running pre-call checks. Ensures we log rejected requests to langfuse.
        logging_obj, self.data = dheera_ai.utils.function_setup(
        ...
        self.data["dheera_ai_logging_obj"] = logging_obj
    ...
    async def base_process_llm_request(
        ...
        # Fallback: extract model_id from dheera_ai_metadata if not in hidden_params
        if not model_id:
            dheera_ai_metadata = self.data.get("dheera_ai_metadata", {}) or {}
            model_info = dheera_ai_metadata.get("model_info", {}) or {}
        ...
        asyncio.create_task(
            proxy_logging_obj.update_request_status(
                dheera_ai_call_id=self.data.get("dheera_ai_call_id", ""), status="success"
            )
        )
        if self._is_streaming_request(
            ...
            custom_headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
                user_api_key_dict=user_api_key_dict,
                call_id=logging_obj.dheera_ai_call_id,
                model_id=model_id,
                cache_key=cache_key,
                api_base=api_base,
                version=version,
    ...
    async def _handle_llm_api_exception(
        ...
        verbose_proxy_logger.exception(
            f"dheera_ai.proxy.proxy_server._handle_llm_api_exception(): Exception occured - {str(e)}"
        )
        ...
        model_id = self.maybe_get_model_id(_dheera_ai_logging_obj)

        custom_headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
            user_api_key_dict=user_api_key_dict,
            call_id=(
                _dheera_ai_logging_obj.dheera_ai_call_id if _dheera_ai_logging_obj else None
            ),
            model_id=model_id,
            version=version,
            response_cost=0,
            ...
            request_data=self.data,
            timeout=timeout,
            dheera_ai_logging_obj=_dheera_ai_logging_obj,
        )
    ...
    def _process_chunk_with_cost_injection(chunk: Any, model_name: str) -> Any:
        ...
        """
        if not getattr(dheera_ai, "include_cost_in_streaming_usage", False):
            return chunk
    ...
    def _inject_cost_into_usage_dict(obj: dict, model_name: str) -> Optional[dict]:
        ...
        if obj.get("type") == "message_delta" and isinstance(obj.get("usage"), dict):
            ...
            try:
                cost_val = dheera_ai.completion_cost(
    ...
    def maybe_get_model_id(
        self, _logging_obj: Optional[DheeraAILoggingObj]
    ) -> Optional[str]:
        """
        ...
        model_id = None
        if _logging_obj:
            # 1. Try getting from dheera_ai_params (updated during call)
            if hasattr(_logging_obj, "dheera_ai_params") and _logging_obj.dheera_ai_params:
                # First check direct model_info path (set by router.py with selected deployment)
                model_info = _logging_obj.dheera_ai_params.get("model_info") or {}
                model_id = model_info.get("id", None)

                # Fallback to nested metadata path
                if not model_id:
                    metadata = _logging_obj.dheera_ai_params.get("metadata") or {}
            ...
            if not model_id:
                ...
                if _kwargs:
                    dheera_ai_params = _kwargs.get("dheera_ai_params", {})
                    # First check direct model_info path
                    model_info = dheera_ai_params.get("model_info") or {}
                    model_id = model_info.get("id", None)
        ...
        # 3. Final fallback to self.data["dheera_ai_metadata"] (for routes like /v1/responses that populate data before error)
        if not model_id:
            dheera_ai_metadata = self.data.get("dheera_ai_metadata", {}) or {}
            model_info = dheera_ai_metadata.get("model_info", {}) or {}
            model_id = model_info.get("id", None)
