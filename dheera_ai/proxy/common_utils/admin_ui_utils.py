def show_missing_vars_in_env():
    ...
    from dheera_ai.proxy.proxy_server import master_key, prisma_client

    if prisma_client is None and master_key is None:
        return HTMLResponse(
            content=missing_keys_form(
                missing_key_names="DATABASE_URL, DHEERA_AI_MASTER_KEY"
            ),
            status_code=200,
        )
    ...
    if master_key is None:
        return HTMLResponse(
            content=missing_keys_form(missing_key_names="DHEERA_AI_MASTER_KEY"),
            status_code=200,
        )
    return None
    ...
    missing_keys_html_form = """
        ...
        <body>
            <div class="container">
                ...
                <pre>
    <span class="env-var">DHEERA_AI_MASTER_KEY="sk-1234"</span> <span class="comment"># Your master key for the proxy server. Can use this to send /chat/completion requests etc</span>
    <span class="env-var">DHEERA_AI_SALT_KEY="sk-XXXXXXXX"</span> <span class="comment"># Can NOT CHANGE THIS ONCE SET - It is used to encrypt/decrypt credentials stored in DB. If value of 'DHEERA_AI_SALT_KEY' changes your models cannot be retrieved from DB</span>
