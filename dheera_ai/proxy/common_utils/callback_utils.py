...
import dheera_ai
from dheera_ai import get_secret
from dheera_ai._logging import verbose_proxy_logger
from dheera_ai.integrations.custom_logger import CustomLogger
from dheera_ai.proxy._types import CommonProxyErrors, DheeraAIPromptInjectionParams
...
def initialize_callbacks_on_proxy(  # noqa: PLR0915
    ...
    from dheera_ai.integrations.custom_logger import CustomLogger
    from dheera_ai.dheera_ai_core_utils.logging_callback_manager import (
    ...
    if isinstance(value, list):
        ...
        for callback in value:  # ["presidio", <my-custom-callback>]
            ...
            if (
                ...
                presidio_logging_only: Optional[bool] = dheera_ai_settings.get(
                ...
                try:
                    from dheera_ai_enterprise.enterprise_callbacks.llama_guard import (
                ...
                from dheera_ai.proxy.guardrails.guardrail_hooks.lakera_ai import (
                    lakeraAI_Moderation,
                )
                ...
                from dheera_ai.proxy.guardrails.guardrail_hooks.aporia_ai.aporia_ai import (
                    AporiaGuardrail,
                )
                ...
                try:
                    from dheera_ai_enterprise.enterprise_callbacks.llm_guard import (
                ...
                if "prompt_injection_params" in dheera_ai_settings:
                    prompt_injection_params_in_config = dheera_ai_settings[
                        "prompt_injection_params"
                    ]
                    prompt_injection_params = DheeraAIPromptInjectionParams(
                ...
                from dheera_ai.proxy.hooks.batch_redis_get import (
                ...
                from dheera_ai.proxy.hooks.azure_content_safety import (
                    _PROXY_AzureContentSafety,
                )

                azure_content_safety_params = dheera_ai_settings[
        ...
        if isinstance(dheera_ai.callbacks, list):
            dheera_ai.callbacks.extend(imported_list)
        else:
            dheera_ai.callbacks = imported_list  # type: ignore
    ...
    verbose_proxy_logger.debug(
        f"{blue_color_code} Initialized Callbacks - {dheera_ai.callbacks} {reset_color_code}"
    )


def get_model_group_from_dheera_ai_kwargs(kwargs: dict) -> Optional[str]:
    _dheera_ai_params = kwargs.get("dheera_ai_params", None) or {}
    _metadata = (
        _dheera_ai_params.get(get_metadata_variable_name_from_kwargs(kwargs)) or {}
    )
...
def get_remaining_tokens_and_requests_from_request_data(data: Dict) -> Dict[str, str]:
    """
    Helper function to return x-dheera_ai-key-remaining-tokens-{model_group} and x-dheera_ai-key-remaining-requests-{model_group}
    ...
    # Remaining Requests
    remaining_requests_variable_name = f"dheera_ai-key-remaining-requests-{model_group}"
    remaining_requests = _metadata.get(remaining_requests_variable_name, None)
    if remaining_requests:
        headers[f"x-dheera_ai-key-remaining-requests-{h11_model_group_name}"] = (
            remaining_requests
        )
    ...
    if remaining_tokens:
        headers[f"x-dheera_ai-key-remaining-tokens-{h11_model_group_name}"] = (
            remaining_tokens
        )
...
def get_logging_caching_headers(request_data: Dict) -> Optional[Dict]:
    ...
    if not _metadata:
        _metadata = request_data.get("dheera_ai_metadata", None)
    if not isinstance(_metadata, dict):
        _metadata = {}
    headers = {}
    if "applied_guardrails" in _metadata:
        headers["x-dheera_ai-applied-guardrails"] = ",".join(
            _metadata["applied_guardrails"]
        )

    if "semantic-similarity" in _metadata:
        headers["x-dheera_ai-semantic-similarity"] = str(_metadata["semantic-similarity"])
...
def add_guardrail_response_to_standard_logging_object(
    dheera_ai_logging_obj: Optional["DheeraAILogging"],
    ...
    standard_logging_object: Optional[StandardLoggingPayload] = (
        dheera_ai_logging_obj.model_call_details.get("standard_logging_object")
    )
