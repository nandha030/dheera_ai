...
def check_file_size_under_limit(
    ...
    if llm_router is not None and request_data["model"] in router_model_names:
        try:
            ...
            if (
                deployment
                and deployment.dheera_ai_params is not None
                and deployment.dheera_ai_params.max_file_size_mb is not None
            ):
                max_file_size_mb = deployment.dheera_ai_params.max_file_size_mb
        except Exception as e:
...
async def convert_upload_files_to_file_data(
    ...
    """
    Convert FastAPI UploadFile objects to file data tuples for dheera_ai.
    
    Converts UploadFile objects to tuples of (filename, content, content_type)
    which is the format expected by httpx and dheera_ai's HTTP handlers.
