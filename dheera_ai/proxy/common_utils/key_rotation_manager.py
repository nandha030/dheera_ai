...
class KeyRotationManager:
    ...
    async def _find_keys_needing_rotation(self) -> List[DheeraAI_VerificationToken]:
        """
    ...
    def _should_rotate_key(self, key: DheeraAI_VerificationToken, now: datetime) -> bool:
        """
    ...
    async def _rotate_key(self, key: DheeraAI_VerificationToken):
        """
        ...
        system_user = UserAPIKeyAuth.get_dheera_ai_internal_jobs_user_api_key_auth()
        
        # Use existing regenerate key function
        response = await regenerate_key_fn(
            data=regenerate_request,
            user_api_key_dict=system_user,
            dheera_ai_changed_by=DHEERA_AI_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME
        )
