...
class ResetBudgetJob:
    ...
    async def reset_budget(
        ...
        if self.prisma_client is not None:
            ### RESET KEY BUDGET ###
            await self.reset_budget_for_dheera_ai_keys()

            ### RESET USER BUDGET ###
            await self.reset_budget_for_dheera_ai_users()

            ## Reset Team Budget
            await self.reset_budget_for_dheera_ai_teams()

            ### RESET ENDUSER (Customer) BUDGET and corresponding Budget duration ###
            await self.reset_budget_for_dheera_ai_budget_table()

    async def reset_budget_for_dheera_ai_team_members(
        self, budgets_to_reset: List[DheeraAI_BudgetTableFull]
    ):
        """
    ...
    async def reset_budget_for_dheera_ai_budget_table(self):
        """
        Resets the budget for all DheeraAI End-Users (Customers), and Team Members if their budget has expired
        The corresponding Budget duration is also updated.
        """

        now = datetime.now(timezone.utc)
        start_time = time.time()
        endusers_to_reset: Optional[List[DheeraAI_EndUserTable]] = None
        budgets_to_reset: Optional[List[DheeraAI_BudgetTableFull]] = None
        updated_endusers: List[DheeraAI_EndUserTable] = []
        failed_endusers = []
        try:
            ...
            if budgets_to_reset is not None and len(budgets_to_reset) > 0:
                ...
                await self.reset_budget_for_dheera_ai_team_members(
    ...
    async def reset_budget_for_dheera_ai_keys(self):
        """
        Resets the budget for all the dheera_ai keys

        Catches Exceptions and logs them
        """
        now = datetime.utcnow()
        start_time = time.time()
        keys_to_reset: Optional[List[DheeraAI_VerificationToken]] = None
        try:
            ...
            updated_keys: List[DheeraAI_VerificationToken] = []
            failed_keys = []
    ...
    async def reset_budget_for_dheera_ai_users(self):
        """
        Resets the budget for all DheeraAI Internal Users if their budget has expired
        """
        now = datetime.utcnow()
        start_time = time.time()
        users_to_reset: Optional[List[DheeraAI_UserTable]] = None
        try:
    ...
    async def reset_budget_for_dheera_ai_teams(self):
        """
        Resets the budget for all DheeraAI Internal Teams if their budget has expired
        """
        now = datetime.utcnow()
        start_time = time.time()
        teams_to_reset: Optional[List[DheeraAI_TeamTable]] = None
        try:
            teams_to_reset = await self.prisma_client.get_data(
                table_name="team", query_type="find_all", reset_at=now
            )
            updated_teams: List[DheeraAI_TeamTable] = []
            failed_teams = []
    ...
    async def _reset_budget_common(
        item: Union[DheeraAI_TeamTable, DheeraAI_UserTable, DheeraAI_VerificationToken],
        ...
        try:
            ...
            if hasattr(item, "budget_duration") and item.budget_duration is not None:
                ...
                from dheera_ai.proxy.common_utils.timezone_utils import (
    ...
    async def _reset_budget_for_team(
        team: DheeraAI_TeamTable, current_time: datetime
    ) -> Optional[DheeraAI_TeamTable]:
    ...
    async def _reset_budget_for_user(
        user: DheeraAI_UserTable, current_time: datetime
    ...
    async def _reset_budget_reset_at_date(
        budget: DheeraAI_BudgetTableFull, current_time: datetime
    ) -> DheeraAI_BudgetTableFull:
        try:
    ...
    async def _reset_budget_for_key(
        key: DheeraAI_VerificationToken, current_time: datetime
