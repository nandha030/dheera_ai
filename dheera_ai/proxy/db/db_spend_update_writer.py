...
from dheera_ai.constants import DB_SPEND_UPDATE_JOB_NAME
...
from dheera_ai.proxy.db.db_transaction_queue.daily_spend_update_queue import (
    DailySpendUpdateQueue,
)
from dheera_ai.proxy.db.db_transaction_queue.pod_lock_manager import PodLockManager
from dheera_ai.proxy.db.db_transaction_queue.redis_update_buffer import RedisUpdateBuffer
from dheera_ai.proxy.db.db_transaction_queue.spend_update_queue import SpendUpdateQueue
...
class DBSpendUpdateWriter:
    ...
    async def update_database(
        # DheeraAI management object fields
        self,
        token: Optional[str],
        ...
        completion_response: Optional[Union[dheera_ai.ModelResponse, Any, Exception]],
        ...
        from dheera_ai.proxy.utils import ProxyUpdateSpend, hash_token
        ...
        try:
            ...
            asyncio.create_task(
                self._update_user_db(
                    ...
                    dheera_ai_proxy_budget_name=dheera_ai_proxy_budget_name,
    ...
    async def _update_user_db(
        ...
        dheera_ai_proxy_budget_name: Optional[str],
        end_user_id: Optional[str] = None,
    ):
        """
        - Update that user's row
        - Update dheera_ai-proxy-budget row (global proxy spend)
        """
        ## if an end-user is passed in, do an upsert - we can't guarantee they already exist in db
        existing_user_obj = await user_api_key_cache.async_get_cache(key=user_id)
        if existing_user_obj is not None and isinstance(existing_user_obj, dict):
            existing_user_obj = DheeraAI_UserTable(**existing_user_obj)
        try:
            if prisma_client is not None:  # update
                ...
                if (
                    ...
                    user_ids.append(dheera_ai_proxy_budget_name)
    ...
    async def _commit_spend_updates_to_db(  # noqa: PLR0915
        ...
        await DBSpendUpdateWriter._update_entity_spend_in_db(
            ...
            table_accessor="dheera_ai_tagtable",
    ...
    async def _update_entity_spend_in_db(
        ...
        """
        from dheera_ai.proxy.utils import _raise_failed_update_spend_exception
    ...
    async def update_daily_user_spend(
        ...
        """
        Batch job to update DheeraAI_DailyUserSpend table using in-memory daily_spend_transactions
        """
    ...
    async def update_daily_team_spend(
        ...
        """
        Batch job to update DheeraAI_DailyTeamSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
    ...
    async def update_daily_org_spend(
        ...
        """
        Batch job to update DheeraAI_DailyOrganizationSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
    ...
    async def update_daily_end_user_spend(
        ...
        """
        Batch job to update DheeraAI_DailyEndUserSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
            ...
            table_name="dheera_ai_dailyenduserspend",
    ...
    async def update_daily_agent_spend(
        ...
        """
        Batch job to update DheeraAI_DailyAgentSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
            ...
            table_name="dheera_ai_dailyagentspend",
    ...
    async def update_daily_tag_spend(
        ...
        """
        Batch job to update DheeraAI_DailyTagSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
            ...
            entity_type="tag",
            entity_id_field="tag",
            table_name="dheera_ai_dailytagspend",
