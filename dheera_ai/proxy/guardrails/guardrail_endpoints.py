...
from dheera_ai.proxy.guardrails.guardrail_registry import GuardrailRegistry
    ...
    for guardrail in guardrails_config:
        guardrail_configs.append(
            GuardrailInfoResponse(
                ...
                dheera_ai_params=guardrail.get("dheera_ai_params"),
...
async def list_guardrails_v2():
    ...
    try:
        ...
        # get guardrails initialized on dheera_ai config.yaml
            ...
            if guardrail.get("guardrail_id") not in seen_guardrail_ids:
                guardrail_configs.append(
                    GuardrailInfoResponse(
                        guardrail_id=guardrail.get("guardrail_id"),
                        guardrail_name=guardrail.get("guardrail_name"),
                        dheera_ai_params=dict(guardrail.get("dheera_ai_params") or {}),
...
async def patch_guardrail(guardrail_id: str, request: PatchGuardrailRequest):
    ...
    try:
        ...
        # Update dheera_ai_params if default_on is provided or pii_entities_config is provided
        dheera_ai_params = LitellmParams(
            **dict(existing_guardrail.get("dheera_ai_params", {}))
        )
        if request.dheera_ai_params is not None:
            requested_dheera_ai_params = request.dheera_ai_params.model_dump(
                exclude_unset=True
            )
            dheera_ai_params_dict = dheera_ai_params.model_dump(exclude_unset=True)
            dheera_ai_params_dict.update(requested_dheera_ai_params)
            dheera_ai_params = LitellmParams(**dheera_ai_params_dict)
...
async def get_guardrail_info(guardrail_id: str):
    ...
    from dheera_ai.proxy.guardrails.guardrail_registry import IN_MEMORY_GUARDRAIL_HANDLER
    from dheera_ai.proxy.proxy_server import prisma_client
    from dheera_ai.types.guardrails import GUARDRAIL_DEFINITION_LOCATION
    ...
    try:
        ...
        dheera_ai_params: Optional[Union[LitellmParams, dict]] = result.get(
            "dheera_ai_params"
        )
        result_dheera_ai_params_dict = (
            dheera_ai_params.model_dump(exclude_none=True)
            if isinstance(dheera_ai_params, LitellmParams)
            else dheera_ai_params
        ) or {}
        masked_dheera_ai_params_dict = _get_masked_values(
            result_dheera_ai_params_dict,
            unmasked_length=4,
        ...
        return GuardrailInfoResponse(
            ...
            dheera_ai_params=masked_dheera_ai_params_dict,
...
async def get_guardrail_ui_settings():
    ...
    """
    from dheera_ai.proxy.guardrails.guardrail_hooks.dheera_ai_content_filter.patterns import (
        PATTERN_CATEGORIES,
...
async def get_provider_specific_params():
    ...
    from dheera_ai.proxy.guardrails.guardrail_registry import guardrail_class_registry
...
async def apply_guardrail(
    ...
    try:
        ...
        if active_guardrail is None:
            raise HTTPException(
                status_code=404,
                detail=f"Guardrail '{request.guardrail_name}' not found. Please ensure the guardrail is configured in your DheeraAI proxy.",
            )
