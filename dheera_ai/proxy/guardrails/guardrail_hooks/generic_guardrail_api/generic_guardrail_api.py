...
from dheera_ai.types.proxy.guardrails.guardrail_hooks.generic_guardrail_api import (
...
class GenericGuardrailAPI(CustomGuardrail):
    """
    Generic Guardrail API integration for DheeraAI.

    This integration allows you to use any guardrail API that follows the
    DheeraAI Basic Guardrail API spec without needing to write custom integration code.
        ...
        **kwargs,
        ...
        if not base_url:
            raise ValueError(
                ...
                "Set GENERIC_GUARDRAIL_API_BASE environment variable or pass it in dheera_ai_params"
            )

        # Append the endpoint path if not already present
        if not base_url.endswith("/beta/dheera_ai_basic_guardrail_api"):
            base_url = base_url.rstrip("/")
            self.api_base = f"{base_url}/beta/dheera_ai_basic_guardrail_api"
        else:
    ...
    def _extract_user_api_key_metadata(
        ...
        Args:
            request_data: Request data dictionary that may contain:
                ...
                - dheera_ai_metadata (for output responses) with user_api_key_* fields
        ...
        # 1. For output responses: dheera_ai_metadata (set by handlers with prefixed keys)
        # 2. For input requests: metadata (already present in request_data with prefixed keys)
        dheera_ai_metadata = request_data.get("dheera_ai_metadata", {})
        top_level_metadata = request_data.get("metadata", {})

        # Merge both sources, preferring dheera_ai_metadata if both exist
        metadata_dict = {**top_level_metadata, **dheera_ai_metadata}
    ...
    async def apply_guardrail(
        ...
        guardrail_request = GenericGuardrailAPIRequest(
            dheera_ai_call_id=logging_obj.dheera_ai_call_id if logging_obj else None,
            dheera_ai_trace_id=logging_obj.dheera_ai_trace_id if logging_obj else None,
            texts=texts,
