...
from dheera_ai.types.proxy.guardrails.guardrail_hooks.hiddenlayer import (
    HiddenlayerAction,
    HiddenlayerMessages,
)
...
class HiddenlayerGuardrail(CustomGuardrail):
    ...
    async def apply_guardrail(
        ...
        if not headers and logging_obj and logging_obj.model_call_details:
            headers = (
                logging_obj.model_call_details.get("dheera_ai_params", {})
                .get("metadata", {})
                .get("headers", {})
            )

        hl_request_metadata["requester_id"] = (
            headers.get("hl-requester-id") or "DheeraAI"
        )
