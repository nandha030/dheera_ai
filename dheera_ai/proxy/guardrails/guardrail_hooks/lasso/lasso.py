...
from dheera_ai.integrations.custom_guardrail import dc as global_cache
...
class LassoGuardrail(CustomGuardrail):
    """
    Lasso Security Guardrail integration for DheeraAI.
    ...
    async def async_post_call_success_hook(
        ...
        if isinstance(response, dheera_ai.ModelResponse):
            ...
            if response_messages:
                # Include dheera_ai_call_id from original data for conversation_id consistency
                response_data = {
                    "messages": response_messages,
                    "dheera_ai_call_id": data.get("dheera_ai_call_id"),
                }
    ...
    def _get_or_generate_conversation_id(self, data: dict, cache: DualCache) -> str:
        ...
        This method ensures session consistency by using dheera_ai_call_id as a cache key.
        ...
        Example:
            ...
            >>> data = {"dheera_ai_call_id": "call_123"}
            >>> conversation_id = guardrail._get_or_generate_conversation_id(data, cache)
            >>> # Returns consistent ID for same dheera_ai_call_id

        Args:
            data: The request data containing dheera_ai_call_id
        ...
        # Get the dheera_ai_call_id which is consistent across all hooks for this request
        dheera_ai_call_id = data.get("dheera_ai_call_id")

        if not dheera_ai_call_id:
            # Fallback to generating a new ULID if no dheera_ai_call_id available
            return self._generate_ulid()

        # Use dheera_ai_call_id as cache key for conversation_id
        cache_key = f"lasso_conversation_id:{dheera_ai_call_id}"
    ...
    def get_config_model() -> Optional[Type["GuardrailConfigModel"]]:
        from dheera_ai.types.proxy.guardrails.guardrail_hooks.lasso import (
