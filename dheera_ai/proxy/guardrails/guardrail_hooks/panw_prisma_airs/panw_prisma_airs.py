#!/usr/bin/env python3
"""
Palo Alto Networks Prisma AI Runtime Security (AIRS) Guardrail Integration for DheeraAI
...
from dheera_ai.integrations.custom_guardrail import (
    CustomGuardrail,
...
from dheera_ai.types.utils import CallTypesLiteral, ModelResponse
...
class PanwPrismaAirsHandler(CustomGuardrail):
    """
    DheeraAI Built-in Guardrail for Palo Alto Networks Prisma AI Runtime Security (AIRS).
    ...
    Configuration:
        ...
        app_name: Application name for tracking in Prisma AIRS analytics (default: "DheeraAI")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Handle app_name: Default to "DheeraAI", or prefix user's app_name with "DheeraAI-"
        if app_name:
            self.app_name = f"DheeraAI-{app_name}"
        else:
            self.app_name = "DheeraAI"
    ...
    def _extract_response_text(self, response: ModelResponse) -> str:
        ...
        """
        try:
            from dheera_ai.types.utils import Choices
    ...
    async def _call_panw_api(  # noqa: PLR0915
        ...
        # Use dheera_ai_trace_id as Prisma AIRS AI Session ID for session grouping
        transaction_id = metadata.get("dheera_ai_trace_id") if metadata else None
        if not transaction_id:
            transaction_id = call_id or str(uuid.uuid4())

        # Build Prisma AIRS API metadata
        # Handle app_name: DheeraAI by default, or DheeraAI-{user_app_name} if user provides one
        user_app_name = metadata.get("app_name") if metadata else None
        if user_app_name:
            app_name_value = f"DheeraAI-{user_app_name}"
        else:
            app_name_value = self.app_name  # Defaults to "DheeraAI"

        panw_metadata = {
            "app_user": (
                metadata.get("app_user") or metadata.get("user") or "dheera_ai_user"
            )
            if metadata
            else "dheera_ai_user",
            "ai_model": metadata.get("model", "unknown") if metadata else "unknown",
            "app_name": app_name_value,
            "source": "dheera_ai_builtin_guardrail",
        }

        # Include user_ip if available (from DheeraAI metadata or request)
        ...
        try:
            # Use DheeraAI's async HTTP client
    ...
    def _prepare_metadata_from_request(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        - app_name: Application identifier (will be prefixed with "DheeraAI-")
        ...
        metadata = {
            "user": data.get("user") or "dheera_ai_user",
        ...
        # Include dheera_ai_trace_id for session tracking
        if data.get("dheera_ai_trace_id"):
            metadata["dheera_ai_trace_id"] = data["dheera_ai_trace_id"]
    ...
    def _check_and_mark_scanned(self, data: dict, scan_type: str) -> bool:
        ...
        """
        call_id = data.get("dheera_ai_call_id")
        if not call_id:
            call_id = str(uuid.uuid4())
            data["dheera_ai_call_id"] = call_id

        scan_key = f"_panw_{scan_type}_scanned_{call_id}"
        dheera_ai_metadata = data.setdefault("dheera_ai_metadata", {})

        if dheera_ai_metadata.get(scan_key):
            verbose_proxy_logger.debug(
                f"PANW Prisma AIRS: Skipping duplicate {scan_type}-call scan"
            )
            return True  # Already scanned

        dheera_ai_metadata[scan_key] = True
    ...
    async def _scan_and_process_streaming_response(
        ...
        scan_result = await self._call_panw_api(
            ...
            is_response=True,
            metadata=metadata,
            call_id=request_data.get("dheera_ai_call_id"),
        )
    ...
    async def async_post_call_streaming_iterator_hook(
        ...
        # Check if guardrail should run for this request
        from dheera_ai.types.guardrails import GuardrailEventHooks as EventHooks
    ...
    def get_config_model() -> Optional[Type["GuardrailConfigModel"]]:
        from dheera_ai.types.proxy.guardrails.guardrail_hooks.panw_prisma_airs import (
