...
from dheera_ai.types.utils import LLMResponseTypes
...
class PillarGuardrail(CustomGuardrail):
    """
    Pillar Security Guardrail for DheeraAI.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Note:
            DheeraAI virtual key context (user_id, team_id, key_alias, etc.) is always
            automatically passed as X-DheeraAI-* headers to enable application/user tracking.
        """
    ...
    def _prepare_headers(self, user_api_key_dict: UserAPIKeyAuth) -> Dict[str, str]:
        ...
        # Always add DheeraAI virtual key context headers (metadata excluded for security)
        context_mapping = {
            "X-DheeraAI-Key-Name": user_api_key_dict.key_name,
            "X-DheeraAI-Key-Alias": user_api_key_dict.key_alias,
            "X-DheeraAI-User-Id": user_api_key_dict.user_id,
            "X-DheeraAI-User-Email": user_api_key_dict.user_email,
            "X-DheeraAI-Team-Id": user_api_key_dict.team_id,
            "X-DheeraAI-Team-Name": user_api_key_dict.team_alias,
            "X-DheeraAI-Org-Id": user_api_key_dict.org_id,
        }
    ...
    def _extract_model_and_provider(self, data: dict) -> Tuple[str, str]:
        ...
        # Use DheeraAI's standard provider detection and model cleaning
        try:
    ...
    def _prepare_payload(self, data: dict) -> Dict[str, Any]:
        ...
        # User ID: use DheeraAI user field
    ...
    def get_config_model() -> Optional[Type["GuardrailConfigModel"]]:
        ...
        """
        from dheera_ai.types.proxy.guardrails.guardrail_hooks.pillar import (
