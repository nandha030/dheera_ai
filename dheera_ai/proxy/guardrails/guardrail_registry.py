# dheera_ai/proxy/guardrails/guardrail_registry.py
...
def get_guardrail_initializer_from_hooks():
    ...
    try:
        ...
        for item in os.listdir(hooks_dir):
            ...
            module_path = f"dheera_ai.proxy.guardrails.guardrail_hooks.{item}"
            try:
                # Import the module
...
class GuardrailRegistry:
    ...
    async def add_guardrail_to_db(
        ...
        try:
            ...
            dheera_ai_params_obj: Any = guardrail.get("dheera_ai_params", {})
            if hasattr(dheera_ai_params_obj, "model_dump"):
                dheera_ai_params_dict = dheera_ai_params_obj.model_dump()
            else:
                dheera_ai_params_dict = (
                    dict(dheera_ai_params_obj) if dheera_ai_params_obj else {}
                )
            dheera_ai_params: str = safe_dumps(dheera_ai_params_dict)
...
class InMemoryGuardrailHandler:
    ...
    def initialize_guardrail(
        ...
        dheera_ai_params_data = guardrail["dheera_ai_params"]
        verbose_proxy_logger.debug("dheera_ai_params= %s", dheera_ai_params_data)

        if isinstance(dheera_ai_params_data, dict):
            dheera_ai_params = LitellmParams(**dheera_ai_params_data)
        else:
            dheera_ai_params = dheera_ai_params_data

        if (
            "category_thresholds" in dheera_ai_params_data
            and dheera_ai_params_data["category_thresholds"]
        ):
            lakera_category_thresholds = LakeraCategoryThresholds(
                **dheera_ai_params_data["category_thresholds"]
            )
            dheera_ai_params.category_thresholds = lakera_category_thresholds

        if dheera_ai_params.api_key and dheera_ai_params.api_key.startswith("os.environ/"):
            dheera_ai_params.api_key = str(get_secret(dheera_ai_params.api_key))

        if dheera_ai_params.api_base and dheera_ai_params.api_base.startswith(
            "os.environ/"
        ):
            dheera_ai_params.api_base = str(get_secret(dheera_ai_params.api_base))

        guardrail_type = dheera_ai_params.guardrail
        ...
        if initializer:
            custom_guardrail_callback = initializer(dheera_ai_params, guardrail)
    ...
    def initialize_custom_guardrail(
        ...
        This initializes it by adding it to the dheera_ai callback manager
        """
        ...
        dheera_ai.logging_callback_manager.add_dheera_ai_callback(_guardrail_callback)  # type: ignore
    ...
    def update_in_memory_guardrail(
        ...
        - updates the guardrail params in dheera_ai.callback_manager
        """
        ...
        if custom_guardrail_callback:
            updated_dheera_ai_params = cast(
                LitellmParams, guardrail.get("dheera_ai_params", {})
            )
            custom_guardrail_callback.update_in_memory_dheera_ai_params(
                dheera_ai_params=updated_dheera_ai_params
            )

    def delete_in_memory_guardrail(self, guardrail_id: str) -> None:
        """
        Delete a guardrail in memory and remove from dheera_ai callbacks.
        """
        # Remove from in-memory storage
        self.IN_MEMORY_GUARDRAILS.pop(guardrail_id, None)

        # Remove the callback from dheera_ai.callbacks
        ...
        if custom_guardrail_callback:
            dheera_ai.logging_callback_manager.remove_callback_from_list_by_object(
                callback_list=dheera_ai.callbacks,
    ...
    def _has_guardrail_params_changed(
        ...
        # Compare dheera_ai_params
        existing_params = existing.get("dheera_ai_params")
        new_params = new_guardrail.get("dheera_ai_params")
        ...
        if existing_dict is not None and new_dict is not None:
            ...
            changed_fields = {"dheera_ai_params": {"old": existing_dict, "new": new_dict}}
    ...
    def reinitialize_guardrail(
        ...
        Removes old callback from dheera_ai.callbacks and creates fresh instance.
        """
        guardrail_id = guardrail.get("guardrail_id")
        if not guardrail_id:
        ...
        # Initialize fresh (will add new callback to dheera_ai.callbacks)
...
########################################################
# In Memory Guardrail Handler for DheeraAI Proxy
########################################################
