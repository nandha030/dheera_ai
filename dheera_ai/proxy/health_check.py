# This file runs a health check for the LLM, used on dheera_ai/proxy
...
from dheera_ai.constants import HEALTH_CHECK_TIMEOUT_SECONDS, DEFAULT_HEALTH_CHECK_PROMPT
...
def _clean_endpoint_data(endpoint_data: dict, details: Optional[bool] = True):
    ...
    """
    endpoint_data.pop("dheera_ai_logging_obj", None)
    return (
    ...
    for is_healthy, model in zip(results, model_list):
        ...
        if isinstance(is_healthy, dict) and "error" not in is_healthy:
            healthy_endpoints.append(
                _clean_endpoint_data({**dheera_ai_params, **is_healthy}, details)
            )
            ...
            unhealthy_endpoints.append(_clean_endpoint_data(dheera_ai_params, details))

    return healthy_endpoints, unhealthy_endpoints


def _update_dheera_ai_params_for_health_check(
    model_info: dict, dheera_ai_params: dict
) -> dict:
    """
    ...
    if _health_check_model is not None:
        dheera_ai_params["model"] = _health_check_model
    if model_info.get("mode", None) == "audio_speech":
        dheera_ai_params["voice"] = model_info.get("health_check_voice", "alloy")
    ...
    if dheera_ai_params["model"].startswith("bedrock/"):
...
async def perform_health_check(
    ...
    if not model_list:
        if cli_model:
            model_list = [
                {"model_name": cli_model, "dheera_ai_params": {"model": cli_model}}
            ]
        else:
            return [], []
    ...
    if model is not None:
        _new_model_list = [
            x for x in model_list if x["dheera_ai_params"]["model"] == model
        ]
        if _new_model_list == []:
