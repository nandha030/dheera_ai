...
from dheera_ai.constants import HEALTH_CHECK_TIMEOUT_SECONDS
...
from dheera_ai.proxy.health_check import (
    _clean_endpoint_data,
    _update_dheera_ai_params_for_health_check,
...
async def health_services_endpoint(  # noqa: PLR0915
    ...
    try:
        ...
        if (
            ...
            or (service in dheera_ai.success_callback and service != "langfuse")
        ):
            _ = await dheera_ai.acompletion(
                model="openai/dheera_ai-mock-response-model",
                messages=[{"role": "user", "content": "Hey, how's it going?"}],
                user="dheera_ai:/health/services",
        ...
        if service == "slack" or service == "slack_budget_alerts":
            if "slack" in general_settings.get("alerting", []):
                # test_message = f"""\n🚨 `ProjectedLimitExceededError` 💸\n\n`Key Alias:` dheera_ai-ui-test-alert \n`Expected Day of Error`: 28th March \n`Current Spend`: $100.00 \n`Projected Spend at end of month`: $1000.00 \n`Soft Limit`: $700"""
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.health_services_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
...
def _build_model_param_to_info_mapping(model_list: list) -> dict:
    ...
    for model in model_list:
        ...
        model_param = dheera_ai_params.get("model")
...
async def health_endpoint(
    ...
    try:
        if llm_model_list is None:
            # if no router set, check if user set a model using dheera_ai --model ollama/llama2
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.py::health_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def shared_health_check_status_endpoint(
    ...
    """
    from dheera_ai.proxy.proxy_server import redis_usage_cache, use_shared_health_check
...
async def active_callbacks():
    """
    Returns a list of dheera_ai level settings
    ...
    {
        "alerting": _alerting,
        "dheera_ai.callbacks": dheera_ai_callbacks,
        "dheera_ai.input_callback": dheera_ai_input_callbacks,
        "dheera_ai.failure_callback": dheera_ai_failure_callbacks,
        "dheera_ai.success_callback": dheera_ai_success_callbacks,
        "dheera_ai._async_success_callback": dheera_ai_async_success_callbacks,
        "dheera_ai._async_failure_callback": dheera_ai_async_failure_callbacks,
        "dheera_ai._async_input_callback": dheera_ai_async_input_callbacks,
        "all_dheera_ai_callbacks": all_dheera_ai_callbacks,
        "num_callbacks": len(all_dheera_ai_callbacks),
        "num_alerting": _num_alerting,
        "dheera_ai.request_timeout": dheera_ai.request_timeout,
    }
    ```
    """

    from dheera_ai.proxy.proxy_server import general_settings, proxy_logging_obj

    _alerting = str(general_settings.get("alerting"))
    # get success callbacks

    dheera_ai_callbacks = [str(x) for x in dheera_ai.callbacks]
    dheera_ai_input_callbacks = [str(x) for x in dheera_ai.input_callback]
    dheera_ai_failure_callbacks = [str(x) for x in dheera_ai.failure_callback]
    dheera_ai_success_callbacks = [str(x) for x in dheera_ai.success_callback]
    dheera_ai_async_success_callbacks = [str(x) for x in dheera_ai._async_success_callback]
    dheera_ai_async_failure_callbacks = [str(x) for x in dheera_ai._async_failure_callback]
    dheera_ai_async_input_callbacks = [str(x) for x in dheera_ai._async_input_callback]

    all_dheera_ai_callbacks = (
        dheera_ai_callbacks
        + dheera_ai_input_callbacks
        + dheera_ai_failure_callbacks
        + dheera_ai_success_callbacks
        + dheera_ai_async_success_callbacks
        + dheera_ai_async_failure_callbacks
        + dheera_ai_async_input_callbacks
    )
...
async def health_readiness():
    ...
    """
    from dheera_ai.proxy.proxy_server import prisma_client, version
    ...
    try:
        ...
        try:
            ...
            success_callback_names = [
                callback_name(x) for x in dheera_ai.success_callback
            ]
        except AttributeError:
            # don't let this block the /health/readiness response, if we can't convert to str -> return dheera_ai.success_callback
            success_callback_names = dheera_ai.success_callback

        # check Cache
        cache_type = None
        if dheera_ai.cache is not None:
            from dheera_ai.caching.caching import RedisSemanticCache

            cache_type = dheera_ai.cache.type

            if isinstance(dheera_ai.cache.cache, RedisSemanticCache):
                # ping the cache
                # TODO: @ishaan-jaff - we should probably not ping the cache on every /health/readiness check
                try:
                    index_info = await dheera_ai.cache.cache._index_info()
                except Exception as e:
...
@router.get(
    "/health/liveliness",  # Historical DheeraAI name; doesn't match k8s terminology but kept for backwards compatibility
    tags=["health"],
)
@router.get(
...
async def test_model_connection(
    ...
    dheera_ai_params: Dict = fastapi.Body(
        None,
        description="Parameters for dheera_ai.completion, dheera_ai.embedding for the health check",
    ),
    ...
    curl -X POST 'http://localhost:4000/health/test_connection' \\
      ...
      -d '{
        "dheera_ai_params": {
            ...
            "dheera_ai_credential_name": null,
            "api_key": "6xxxxxxx",
            "api_base": "https://dheera_ai8397336933.openai.azure.com/openai/deployments/gpt-4o/chat/completions?api-version=2024-10-21",
        },
        "mode": "chat"
      }'
    ```
    ...
    try:
        ...
        await ModelManagementAuthChecks.can_user_make_model_call(
            model_params=Deployment(
                ...
                dheera_ai_params=DheeraAI_Params(**dheera_ai_params),
        ...
        dheera_ai_params = _update_dheera_ai_params_for_health_check(
            model_info={},
            dheera_ai_params=dheera_ai_params,
        )
        mode = mode or dheera_ai_params.pop("mode", None)

        result = await run_with_timeout(
            dheera_ai.ahealth_check(
                model_params=dheera_ai_params,
                mode=mode,
                prompt="test from dheera_ai",
                input=["test from dheera_ai"],
            ),
        ...
        cleaned_result = _clean_endpoint_data(
            {**dheera_ai_params, **result}, details=True
        )
        ...
        verbose_proxy_logger.debug(
            f"dheera_ai.proxy.health_endpoints.test_model_connection(): Exception occurred - {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
