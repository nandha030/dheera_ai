...
class _PROXY_BatchRedisRequests(CustomLogger):
    ...
    def __init__(self):
        if dheera_ai.cache is not None:
            ...
            )  # map the dheera_ai 'get_cache' function to our custom function
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            Check if a key starting with `dheera_ai:<api_key>:<call_type:` exists in-memory

            If no, then get relevant cache from redis
            """
            api_key = user_api_key_dict.api_key

            cache_key_name = f"dheera_ai:{api_key}:{call_type}"
            ...
            if in_memory_cache_exists is False and dheera_ai.cache is not None:
                """
                - Check if `dheera_ai.Cache` is redis
                - Get the relevant values
                """
                if dheera_ai.cache.type is not None and isinstance(
                    ...
                    keys = await dheera_ai.cache.cache.async_scan_iter(
                    ...
                    if len(keys) > 0:
                        key_value_dict = (
                            await dheera_ai.cache.cache.async_batch_get_cache(
                                key_list=keys
                            )
                        )
            ...
            verbose_proxy_logger.error(
                "dheera_ai.proxy.hooks.batch_redis_get.py::async_pre_call_hook(): Exception occured - {}".format(
                    str(e)
                )
            )
    ...
    async def async_get_cache(self, *args, **kwargs):
        ...
        try:  # never block execution
            ...
            elif dheera_ai.cache is not None:
            ...
            if (
                ...
                if cached_result is None:
                    cached_result = await dheera_ai.cache.cache.async_get_cache(
                        cache_key, *args, **kwargs
                    )
                ...
                return dheera_ai.cache._get_cache_logic(
