...
import dheera_ai
from dheera_ai import ModelResponse, Router
...
from dheera_ai.proxy.hooks.rate_limiter_utils import convert_priority_to_percent
...
class _PROXY_DynamicRateLimitHandlerV3(CustomLogger):
    ...
    def _get_saturation_check_cache_ttl(self) -> int:
        ...
        return dheera_ai.priority_reservation_settings.saturation_check_cache_ttl
    ...
    async def _get_saturation_value_from_cache(
        ...
        TTL is configurable via dheera_ai.priority_reservation_settings.saturation_check_cache_ttl
    ...
    def _get_priority_weight(
        ...
        """Get the weight for a given priority from dheera_ai.priority_reservation"""
        weight: float = dheera_ai.priority_reservation_settings.default_priority
        if (
            dheera_ai.priority_reservation is None
            or priority not in dheera_ai.priority_reservation
        ):
    ...
    def _normalize_priority_weights(
        ...
        """
        if dheera_ai.priority_reservation is None:
            return {}

        # Convert all values to percentages first
        weights: Dict[str, float] = {}
        for k, v in dheera_ai.priority_reservation.items():
    ...
    def _get_priority_allocation(
        ...
        """
        # Check if this key has an explicit priority in dheera_ai.priority_reservation
        has_explicit_priority = (
            priority is not None
            and dheera_ai.priority_reservation is not None
            and priority in dheera_ai.priority_reservation
        )
        ...
        if has_explicit_priority and priority is not None:
            ...
            priority_weight = dheera_ai.priority_reservation_settings.default_priority
    ...
    async def _check_rate_limits(
        ...
        saturation_threshold = (
            dheera_ai.priority_reservation_settings.saturation_threshold
        )
                    ...
                    if descriptor_key == "model_saturation_check":
                        raise HTTPException(
                            ...
                            headers={
                                ...
                                "x-dheera_ai-priority": priority or "default",
                            },
                        )
                    ...
                    # Priority limit exceeded (ONLY enforce when saturated)
                        ...
                        raise HTTPException(
                            ...
                            headers={
                                ...
                                "rate_limit_type": str(status["rate_limit_type"]),
                                "x-dheera_ai-priority": priority or "default",
                                "x-dheera_ai-saturation": f"{saturation:.2%}",
                            },
                        )
        ...
        if priority_descriptors:
            ...
            data["dheera_ai_proxy_rate_limit_response"] = combined_response
        else:
            data["dheera_ai_proxy_rate_limit_response"] = model_increment_response
    ...
    async def async_post_call_success_hook(
        ...
        try:
            ...
            if isinstance(response, ModelResponse):
                ...
                additional_headers["x-dheera_ai-priority"] = priority or "default"
                additional_headers["x-dheera_ai-rate-limiter-version"] = "v3"
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            ...
            dheera_ai_parent_otel_span = _get_parent_otel_span_from_kwargs(kwargs)
            ...
            has_explicit_priority = (
                ...
                and dheera_ai.priority_reservation is not None
                and key_priority in dheera_ai.priority_reservation
            )
