...
DHEERA_AI_PREFIX_STORED_VIRTUAL_KEYS = "dheera_ai/"
...
class KeyManagementEventHooks:
    ...
    async def async_key_generated_hook(
        ...
        """
        from dheera_ai.proxy.management_helpers.audit_logs import (
            create_audit_log_for_update,
        )
        from dheera_ai.proxy.proxy_server import dheera_ai_proxy_admin_name
    ...
    async def async_key_rotated_hook(
        ...
        # store the audit log
        if dheera_ai.store_audit_logs is True and existing_key_row.token is not None:
            asyncio.create_task(
                create_audit_log_for_update(
                    request_data=DheeraAI_AuditLogs(
                        id=str(uuid.uuid4()),
    ...
    async def async_key_deleted_hook(
        data: KeyRequest,
        keys_being_deleted: List[DheeraAI_VerificationToken],
        response: dict,
        user_api_key_dict: UserAPIKeyAuth,
        dheera_ai_changed_by: Optional[str] = None,
    ):
        ...
        if dheera_ai.store_audit_logs is True and data.keys is not None:
    ...
    async def _store_virtual_key_in_secret_manager(secret_name: str, secret_token: str):
        ...
        """
        if dheera_ai._key_management_settings is not None:
            if dheera_ai._key_management_settings.store_virtual_keys is True:
                from dheera_ai.secret_managers.base_secret_manager import (
                    BaseSecretManager,
                )

                # store the key in the secret manager
                if isinstance(dheera_ai.secret_manager_client, BaseSecretManager):
                    tags = getattr(dheera_ai._key_management_settings, "tags", None)
                    description = getattr(
                        dheera_ai._key_management_settings, "description", None
                    )
                    verbose_proxy_logger.debug(
                        f"Creating secret with {secret_name} and tags={tags} and description={description}"
                    )

                    await dheera_ai.secret_manager_client.async_write_secret(
    ...
    async def _rotate_virtual_key_in_secret_manager(
        ...
        if dheera_ai._key_management_settings is not None:
            if dheera_ai._key_management_settings.store_virtual_keys is True:
                ...
                # store the key in the secret manager
                if isinstance(dheera_ai.secret_manager_client, BaseSecretManager):
                    await dheera_ai.secret_manager_client.async_rotate_secret(
    ...
    def _get_secret_name(secret_name: str) -> str:
        if dheera_ai._key_management_settings.prefix_for_stored_virtual_keys.endswith(
            "/"
        ):
            return f"{dheera_ai._key_management_settings.prefix_for_stored_virtual_keys}{secret_name}"
        else:
            return f"{dheera_ai._key_management_settings.prefix_for_stored_virtual_keys}/{secret_name}"
                ...
                if isinstance(dheera_ai.secret_manager_client, BaseSecretManager):
                    for key in keys_being_deleted:
                        if key.key_alias is not None:
                            await dheera_ai.secret_manager_client.async_delete_secret(
                                secret_name=KeyManagementEventHooks._get_secret_name(
                                    key.key_alias
                                )
                            )
    ...
    def _is_email_sending_enabled() -> bool:
        ...
        try:
            from dheera_ai_enterprise.enterprise_callbacks.send_emails.base_email import (
                BaseEmailLogger,
            )
    ...
    async def _send_key_created_email(response: dict):
        ...
        try:
            ...
            from dheera_ai_enterprise.types.enterprise_callbacks.send_emails import (
        ...
        ##########################
        if "email" in general_settings.get("alerting", []):
            from dheera_ai.proxy._types import WebhookEvent
