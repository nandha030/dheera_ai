...
import dheera_ai
from dheera_ai import DualCache, ModelResponse
...
from dheera_ai.proxy._types import CommonProxyErrors, CurrentItemRateLimit, UserAPIKeyAuth
...
class _PROXY_MaxParallelRequestsHandler(CustomLogger):
    ...
    async def async_pre_call_hook(  # noqa: PLR0915
        ...
        if (
            ...
            _remaining_limits_data = {
                f"dheera_ai-key-remaining-tokens-{_model}": _remaining_tokens,
                f"dheera_ai-key-remaining-requests-{_model}": _remaining_requests,
            }
    ...
    async def async_log_success_event(  # noqa: PLR0915
        ...
        dheera_ai_parent_otel_span: Union[Span, None] = _get_parent_otel_span_from_kwargs(
            kwargs=kwargs
        )
        try:
            self.print_verbose("INSIDE parallel request limiter ASYNC SUCCESS LOGGING")

            global_max_parallel_requests = kwargs["dheera_ai_params"]["metadata"].get(
                "global_max_parallel_requests", None
            )
            user_api_key = kwargs["dheera_ai_params"]["metadata"]["user_api_key"]
            user_api_key_user_id = kwargs["dheera_ai_params"]["metadata"].get(
                "user_api_key_user_id", None
            )
            user_api_key_team_id = kwargs["dheera_ai_params"]["metadata"].get(
                "user_api_key_team_id", None
            )
            user_api_key_model_max_budget = kwargs["dheera_ai_params"]["metadata"].get(
                "user_api_key_model_max_budget", None
            )
            user_api_key_end_user_id = kwargs.get("user")

            user_api_key_metadata = (
                kwargs["dheera_ai_params"]["metadata"].get("user_api_key_metadata", {})
                or {}
            )
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            dheera_ai_parent_otel_span: Union[
                Span, None
            ] = _get_parent_otel_span_from_kwargs(kwargs=kwargs)
            _metadata = kwargs["dheera_ai_params"].get("metadata", {}) or {}
    ...
    async def get_internal_user_object(
        ...
        It uses the `get_user_object` function from `dheera_ai.proxy.auth.auth_checks`
        ...
        """
        from dheera_ai._logging import verbose_proxy_logger
        from dheera_ai.proxy.auth.auth_checks import get_user_object
