...
from dheera_ai.constants import DEFAULT_PROMPT_INJECTION_SIMILARITY_THRESHOLD
...
from dheera_ai.proxy._types import DheeraAIPromptInjectionParams, UserAPIKeyAuth
...
class _OPTIONAL_PromptInjectionDetection(CustomLogger):
    ...
    def __init__(
        self,
        prompt_injection_params: Optional[DheeraAIPromptInjectionParams] = None,
    ):
    ...
    async def async_moderation_hook(  # type: ignore
        ...
        if (
            ...
            if isinstance(response, dheera_ai.ModelResponse) and isinstance(
                response.choices[0], dheera_ai.Choices
            ):
