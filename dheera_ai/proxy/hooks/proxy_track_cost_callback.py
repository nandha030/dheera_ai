...
from dheera_ai.proxy.auth.auth_checks import log_db_metrics
...
class _ProxyDBLogger(CustomLogger):
    ...
    async def async_post_call_failure_hook(
        ...
        if "dheera_ai_params" not in request_data:
            request_data["dheera_ai_params"] = {}
        
        existing_dheera_ai_params = request_data.get("dheera_ai_params", {})
        existing_dheera_ai_metadata = existing_dheera_ai_params.get("metadata", {}) or {}
        
        # Preserve tags from existing metadata
        if existing_dheera_ai_metadata.get("tags"):
            existing_metadata["tags"] = existing_dheera_ai_metadata.get("tags")
        
        request_data["dheera_ai_params"]["proxy_server_request"] = (
            request_data.get("proxy_server_request") or existing_dheera_ai_params.get("proxy_server_request") or {}
        )
        request_data["dheera_ai_params"]["metadata"] = existing_metadata
        
        # Preserve model name and custom_llm_provider
        if "model" not in request_data:
            request_data["model"] = existing_dheera_ai_params.get("model") or request_data.get("model", "")
        if "custom_llm_provider" not in request_data:
            request_data["custom_llm_provider"] = existing_dheera_ai_params.get("custom_llm_provider") or request_data.get("custom_llm_provider", "")
    ...
    async def _PROXY_track_cost_callback(
        ...
        completion_response: Optional[
            Union[dheera_ai.ModelResponse, Any]
        ],  # response from completion
        start_time=None,
        end_time=None,  # start/end time for completion
    ):
        from dheera_ai.proxy.proxy_server import proxy_logging_obj, update_cache
        ...
        try:
            ...
            metadata = get_dheera_ai_metadata_from_kwargs(kwargs=kwargs)
            ...
            model = kwargs.get("model", "")
            metadata = get_dheera_ai_metadata_from_kwargs(kwargs=kwargs)
            dheera_ai_metadata = kwargs.get("dheera_ai_params", {}).get(
                "dheera_ai_metadata", {}
            )
            old_metadata = kwargs.get("dheera_ai_params", {}).get("metadata", {})
