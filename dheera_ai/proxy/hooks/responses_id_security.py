...
from dheera_ai.types.utils import CallTypesLiteral, LLMResponseTypes, SpecialEnums
...
class ResponsesIDSecurity(CustomLogger):
    ...
    def _decrypt_response_id(
        ...
        if decrypted_value.startswith(SpecialEnums.LITELM_MANAGED_FILE_ID_PREFIX.value):
            # Expected format: "dheera_ai_proxy:responses_api:response_id:{response_id};user_id:{user_id}"
            parts = decrypted_value.split(";")

            if len(parts) >= 2:
                # Extract response_id from "dheera_ai_proxy:responses_api:response_id:{response_id}"
    ...
    def _get_signing_key(self) -> Optional[str]:
        ...
        salt_key = os.getenv("DHEERA_AI_SALT_KEY", None)
        if salt_key is None:
            salt_key = master_key
        return salt_key
    ...
    def _encrypt_response_id(
        self,
        response: BaseDheeraAIOpenAIResponseObject,
        user_api_key_dict: "UserAPIKeyAuth",
    ) -> BaseDheeraAIOpenAIResponseObject:
        ...
        if (
            ...
            encrypted_response_id = SpecialEnums.DHEERA_AI_MANAGED_RESPONSE_API_RESPONSE_ID_COMPLETE_STR.value.format(
                response_id,
    ...
    ) -> AsyncGenerator[BaseDheeraAIOpenAIResponseObject, None]:
        from dheera_ai.proxy.proxy_server import general_settings

        async for chunk in response:
            if (
                isinstance(chunk, BaseDheeraAIOpenAIResponseObject)
