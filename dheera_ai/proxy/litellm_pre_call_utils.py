...
from dheera_ai.types.llms.anthropic import ANTHROPIC_API_HEADERS
...
if TYPE_CHECKING:
    from dheera_ai.proxy.proxy_server import ProxyConfig as _ProxyConfig
...
def _get_metadata_variable_name(request: Request) -> str:
    ...
    For all /thread or /assistant endpoints we need to call this "dheera_ai_metadata"

    For ALL other endpoints we call this "metadata
    """
    if RouteChecks._is_assistants_api_request(request):
        return "dheera_ai_metadata"

    if any(route in request.url.path for route in DHEERA_AI_METADATA_ROUTES):
        return "dheera_ai_metadata"
...
def convert_key_logging_metadata_to_callback(
    ...
    ):  # assume 'success_and_failure' = dheera_ai.callbacks
        if team_callback_settings_obj.success_callback is None:
            team_callback_settings_obj.success_callback = []
        if team_callback_settings_obj.failure_callback is None:
            team_callback_settings_obj.failure_callback = []
    ...
    for var, value in data.callback_vars.items():
        ...
        team_callback_settings_obj.callback_vars[var] = str(
            dheera_ai.utils.get_secret(value, default_value=value) or value
        )
...
def clean_headers(
    headers: Headers, dheera_ai_key_header_name: Optional[str] = None
) -> dict:
    """
    Removes dheera_ai api key from headers
    """
    clean_headers = {}
    dheera_ai_key_lower = (
        dheera_ai_key_header_name.lower() if dheera_ai_key_header_name is not None else None
    )

    for header, value in headers.items():
        header_lower = header.lower()
        # Check if header should be excluded: either in special headers cache or matches custom dheera_ai key
        if header_lower not in _SPECIAL_HEADERS_CACHE and (
            dheera_ai_key_lower is None or header_lower != dheera_ai_key_lower
        ):
            clean_headers[header] = value
    return clean_headers


class DheeraAIProxyRequestSetup:
    @staticmethod
    def _get_timeout_from_request(headers: dict) -> Optional[float]:
        ...
        ```
        """
        timeout_header = headers.get("x-dheera_ai-timeout", None)
    ...
    def _get_stream_timeout_from_request(headers: dict) -> Optional[float]:
        ...
        """
        stream_timeout_header = headers.get("x-dheera_ai-stream-timeout", None)
    ...
    def _get_num_retries_from_request(headers: dict) -> Optional[int]:
        ...
        Workaround for client request from Vercel's AI SDK.
        """
        num_retries_header = headers.get("x-dheera_ai-num-retries", None)
    ...
    def _get_spend_logs_metadata_from_request_headers(headers: dict) -> Optional[dict]:
        ...
        spend_logs_metadata_header = headers.get("x-dheera_ai-spend-logs-metadata", None)
    ...
    def add_internal_user_from_user_mapping(
        ...
        if not user_header_mapping:
            return user_api_key_dict
        header_name = DheeraAIProxyRequestSetup.get_internal_user_header_from_mapping(
            user_header_mapping
        )
        if not header_name:
            return user_api_key_dict
        header_value = DheeraAIProxyRequestSetup._get_case_insensitive_header(
            headers, header_name
        )
        if header_value:
    ...
    def get_user_from_headers(
        ...
        user = DheeraAIProxyRequestSetup._get_case_insensitive_header(
            headers, header_name
        )
        if user is not None:
    ...
    def add_headers_to_llm_call(
        ...
        returned_headers = DheeraAIProxyRequestSetup._get_forwardable_headers(headers)

        if dheera_ai.add_user_information_to_llm_headers is True:
            dheera_ai_logging_metadata_headers = (
                DheeraAIProxyRequestSetup.get_sanitized_user_information_from_key(
                    user_api_key_dict=user_api_key_dict
                )
            )
            for k, v in dheera_ai_logging_metadata_headers.items():
                if v is not None:
                    returned_headers["x-dheera_ai-{}".format(k)] = v
    ...
    def add_headers_to_llm_call_by_model_group(
        ...
        """
        from dheera_ai.proxy.auth.auth_checks import _check_model_access_helper
        ...
        if (
            ...
            and dheera_ai.model_group_settings is not None
            and dheera_ai.model_group_settings.forward_client_headers_to_llm_api
            is not None
            and _check_model_access_helper(
                model=data_model,
                llm_router=llm_router,
                models=dheera_ai.model_group_settings.forward_client_headers_to_llm_api,
                team_model_aliases=user_api_key_dict.team_model_aliases,
                team_id=user_api_key_dict.team_id,
            )  # handles aliases, wildcards, etc.
        ):

            _headers = DheeraAIProxyRequestSetup.add_headers_to_llm_call(
    ...
    def get_internal_user_header_from_mapping(user_header_mapping) -> Optional[str]:
        ...
        for item in items:
            if not isinstance(item, dict):
                continue
            role = item.get("dheera_ai_user_role")
    ...
    @staticmethod
    def add_dheera_ai_data_for_backend_llm_call(
        *,
        headers: dict,
        user_api_key_dict: UserAPIKeyAuth,
        ...
        if (
            ...
            if _headers != {}:
                data["headers"] = _headers
        _organization = DheeraAIProxyRequestSetup.get_openai_org_id_from_headers(
        ...
        stream_timeout = DheeraAIProxyRequestSetup._get_stream_timeout_from_request(
            headers
        )
        if stream_timeout is not None:
            data["stream_timeout"] = stream_timeout

        num_retries = DheeraAIProxyRequestSetup._get_num_retries_from_request(headers)
    ...
    @staticmethod
    def add_dheera_ai_metadata_from_request_headers(
        headers: dict,
        data: dict,
        _metadata_variable_name: str,
    ) -> dict:
        """
        Add dheera_ai metadata from request headers

        Relevant issue: https://github.com/BerriAI/dheera_ai/issues/14008
        """
        from dheera_ai.proxy._types import LitellmMetadataFromRequestHeaders

        metadata_from_headers = LitellmMetadataFromRequestHeaders()
        spend_logs_metadata = (
            DheeraAIProxyRequestSetup._get_spend_logs_metadata_from_request_headers(
                headers
            )
        )
    ...
    def add_key_level_controls(
        ...
        if "tags" in key_metadata and key_metadata["tags"] is not None:
            data[_metadata_variable_name]["tags"] = (
                DheeraAIProxyRequestSetup._merge_tags(
        ...
        ## KEY-LEVEL METADATA
        data = DheeraAIProxyRequestSetup.add_management_endpoint_metadata_to_request_metadata(
            data=data,
    ...
    def add_request_tag_to_metadata(
        ...
        if "x-dheera_ai-tags" in headers:
            if isinstance(headers["x-dheera_ai-tags"], str):
                _tags = headers["x-dheera_ai-tags"].split(",")
                tags = [tag.strip() for tag in _tags]
            elif isinstance(headers["x-dheera_ai-tags"], list):
                tags = headers["x-dheera_ai-tags"]
...
async def add_dheera_ai_data_to_request(  # noqa: PLR0915
    data: dict,
    request: Request,
    user_api_key_dict: UserAPIKeyAuth,
    proxy_config: ProxyConfig,
    ...
    """
    Adds DheeraAI-specific data to the request.
    ...
    from dheera_ai.proxy.proxy_server import llm_router, premium_user
    from dheera_ai.types.proxy.dheera_ai_pre_call_utils import SecretFields

    _headers = clean_headers(
        request.headers,
        dheera_ai_key_header_name=(
            general_settings.get("dheera_ai_key_header_name")
    ...
    data.update(
        DheeraAIProxyRequestSetup.add_dheera_ai_data_for_backend_llm_call(
            headers=_headers,
            user_api_key_dict=user_api_key_dict,
    ...
    data.update(
        DheeraAIProxyRequestSetup.add_dheera_ai_metadata_from_request_headers(
            headers=_headers,
            data=data,
            _metadata_variable_name=_metadata_variable_name,
        )
    ...
    data = DheeraAIProxyRequestSetup.add_headers_to_llm_call_by_model_group(
        data=data, headers=_headers, user_api_key_dict=user_api_key_dict
    )

    user_api_key_dict = DheeraAIProxyRequestSetup.add_internal_user_from_user_mapping(
        general_settings, user_api_key_dict, _headers
    )

    # Parse user info from headers
    user = DheeraAIProxyRequestSetup.get_user_from_headers(_headers, general_settings)
    if user is not None:
    ...
    # Parse dheera_ai_metadata if it's a string (e.g., from multipart/form-data or extra_body)
    if "dheera_ai_metadata" in data and data["dheera_ai_metadata"] is not None:
        if isinstance(data["dheera_ai_metadata"], str):
            parsed_dheera_ai_metadata = safe_json_loads(data["dheera_ai_metadata"])
            if not isinstance(parsed_dheera_ai_metadata, dict):
                verbose_proxy_logger.warning(
                    f"Failed to parse 'dheera_ai_metadata' as JSON dict. Received value: {data['dheera_ai_metadata']}"
                )
            else:
                data["dheera_ai_metadata"] = parsed_dheera_ai_metadata
        # Merge dheera_ai_metadata into the metadata variable (preserving existing values)
        if isinstance(data["dheera_ai_metadata"], dict):
            for key, value in data["dheera_ai_metadata"].items():
                if key not in data[_metadata_variable_name]:
                    data[_metadata_variable_name][key] = value

    data = DheeraAIProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
        data=data,
        user_api_key_dict=user_api_key_dict,
        _metadata_variable_name=_metadata_variable_name,
    )
    data[_metadata_variable_name]["dheera_ai_api_version"] = version
    ...
    if "tags" in team_metadata and team_metadata["tags"] is not None:
        data[_metadata_variable_name]["tags"] = DheeraAIProxyRequestSetup._merge_tags(
            request_tags=data[_metadata_variable_name].get("tags"),
    ...
    ## TEAM-LEVEL METADATA
    data = (
        DheeraAIProxyRequestSetup.add_management_endpoint_metadata_to_request_metadata(
            data=data,
    ...
    # Add the OTEL Parent Trace before sending it DheeraAI
    data[_metadata_variable_name][
        "dheera_ai_parent_otel_span"
    ...
    tags = DheeraAIProxyRequestSetup.add_request_tag_to_metadata(
        llm_router=llm_router,
    ...
    if (
        ...
        and "dheera_ai_disabled_callbacks" in user_api_key_dict.metadata
    ):
        disabled_callbacks = user_api_key_dict.metadata["dheera_ai_disabled_callbacks"]
        if disabled_callbacks and isinstance(disabled_callbacks, list):
            data["dheera_ai_disabled_callbacks"] = disabled_callbacks
    ...
    verbose_proxy_logger.debug(
        "[PROXY] returned data from dheera_ai_pre_call_utils: %s", data
    )
    ...
    asyncio.create_task(
        service_logger_obj.async_service_success_hook(
            ...
            call_type="add_dheera_ai_data_to_request",
...
def _add_otel_traceparent_to_data(data: dict, request: Request):
    from dheera_ai.proxy.proxy_server import open_telemetry_logger

    if data is None:
        return
    if open_telemetry_logger is None:
        # if user is not use OTEL don't send extra_headers
        # relevant issue: https://github.com/BerriAI/dheera_ai/issues/4448
        return

    if dheera_ai.forward_traceparent_to_llm_provider is True:
        if request.headers:
            if "traceparent" in request.headers:
                # we want to forward this to the LLM Provider
                # Relevant issue: https://github.com/BerriAI/dheera_ai/issues/4419
