...
from dheera_ai.proxy._types import CommonProxyErrors, UserAPIKeyAuth
from dheera_ai.proxy.auth.user_api_key_auth import user_api_key_auth
from dheera_ai.types.utils import LlmProvidersSet
...
async def get_cost_discount_config(
    ...
    Returns the cost_discount_config from dheera_ai_settings.
    """
    ...
    try:
        ...
        # Get cost_discount_config from dheera_ai_settings
        dheera_ai_settings = config.get("dheera_ai_settings", {})
        cost_discount_config = dheera_ai_settings.get("cost_discount_config", {})
...
async def update_cost_discount_config(
    ...
    Updates the cost_discount_config in dheera_ai_settings.
    ...
    # Validate that all providers are valid DheeraAI providers
    invalid_providers = []
    ...
    try:
        ...
        # Ensure dheera_ai_settings exists
        if "dheera_ai_settings" not in config:
            config["dheera_ai_settings"] = {}
        
        # Update cost_discount_config
        config["dheera_ai_settings"]["cost_discount_config"] = cost_discount_config
        
        # Save the updated config to DB
        await proxy_config.save_config(new_config=config)
        
        # Update in-memory dheera_ai.cost_discount_config
        dheera_ai.cost_discount_config = cost_discount_config
