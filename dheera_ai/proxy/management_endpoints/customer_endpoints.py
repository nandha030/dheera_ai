...
async def unblock_user(data: BlockUsers):
    ...
    if (
        not any(isinstance(x, _ENTERPRISE_BlockedUserList) for x in dheera_ai.callbacks)
        or dheera_ai.blocked_user_list is None
    ):
        raise HTTPException(
            status_code=400,
            detail={
    ...
    if isinstance(dheera_ai.blocked_user_list, list):
        for id in data.user_ids:
            dheera_ai.blocked_user_list.remove(id)
    else:
        raise HTTPException(
            status_code=500,
            detail={
    ...
    return {"blocked_users": dheera_ai.blocked_user_list}
...
async def new_end_user(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.management_endpoints.customer_endpoints.new_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
...
@router.get(
    ...
    tags=["Customer Management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=DheeraAI_EndUserTable,
)
@router.get(
    "/end_user/info",
    tags=["Customer Management"],
...
async def end_user_info(
    ...
    Example curl:
    ```
    curl -X GET 'http://localhost:4000/customer/info?end_user_id=test-dheera_ai-user-4' \
        -H 'Authorization: Bearer sk-1234'
    ```
    """
    try:
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.management_endpoints.customer_endpoints.end_user_info(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def update_end_user(
    ...
    try:
        ...
        end_user_table_data_typed = DheeraAI_EndUserTable(
            **end_user_table_data.model_dump()
        )

        ## Get budget table data ##
        end_user_budget_table = end_user_table_data_typed.dheera_ai_budget_table
        ...
        for k, v in non_default_values.items():
            ...
            elif k in DheeraAI_BudgetTable.model_fields.keys():
                budget_table_data[k] = v

            elif k in DheeraAI_EndUserTable.model_fields.keys():
                update_end_user_table_data[k] = v
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.update_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def delete_end_user(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.delete_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
@router.get(
    ...
    tags=["Customer Management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=List[DheeraAI_EndUserTable],
)
@router.get(
    "/end_user/list",
    tags=["Customer Management"],
...
async def list_end_user(
    ...
    try:
        ...
        returned_response: List[DheeraAI_EndUserTable] = []
        for item in response:
            returned_response.append(DheeraAI_EndUserTable(**item.model_dump()))
        return returned_response
    
    except Exception as e:
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.management_endpoints.customer_endpoints.list_end_user(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
