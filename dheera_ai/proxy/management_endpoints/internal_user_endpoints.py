...
These are members of a Team on DheeraAI
...
from dheera_ai.proxy.hooks.user_management_event_hooks import UserManagementEventHooks
from dheera_ai.proxy.management_endpoints.common_daily_activity import (
    get_daily_activity,
...
from dheera_ai.proxy.utils import handle_exception_on_proxy
from dheera_ai.types.proxy.management_endpoints.common_daily_activity import (
    SpendAnalyticsPaginatedResponse,
)
from dheera_ai.types.proxy.management_endpoints.internal_user_endpoints import (
...
if TYPE_CHECKING:
    from dheera_ai.proxy.proxy_server import PrismaClient
...
def _update_internal_new_user_params(data_json: dict, data: NewUserRequest) -> dict:
    ...
    if dheera_ai.default_internal_user_params and (
        data.user_role != LitellmUserRoles.PROXY_ADMIN.value
        and data.user_role != LitellmUserRoles.PROXY_ADMIN
    ):
        for key, value in dheera_ai.default_internal_user_params.items():
    ...
    if (
        ...
        if (
            dheera_ai.max_internal_user_budget is not None
            and data_json.get("max_budget") is None
        ):
            data_json["max_budget"] = dheera_ai.max_internal_user_budget

        if (
            dheera_ai.internal_user_budget_duration is not None
            and data_json.get("budget_duration") is None
        ):
            data_json["budget_duration"] = dheera_ai.internal_user_budget_duration
    ...
    try:
        ...
        if e.status_code == 400 and (
            ...
            verbose_proxy_logger.debug(
                "dheera_ai.proxy.management_endpoints.internal_user_endpoints.new_user(): User already exists in team - {}".format(
                    str(e)
                )
            )
        else:
            verbose_proxy_logger.debug(
                "dheera_ai.proxy.management_endpoints.internal_user_endpoints.new_user(): Exception occured - {}".format(
                    str(e)
                )
            )
...
def check_if_default_team_set() -> Optional[Union[List[str], List[NewUserRequestTeam]]]:
    if dheera_ai.default_internal_user_params is None:
        return None
    teams = dheera_ai.default_internal_user_params.get("teams")
...
async def new_user(
    ...
    Internal Users can access DheeraAI Admin UI to make keys, request access to models.
    ...
    - user_role: Optional[str] - Specify a user role - "proxy_admin", "proxy_admin_viewer", "internal_user", "internal_user_viewer", "team", "customer". Info about each role here: `https://github.com/BerriAI/dheera_ai/dheera_ai/proxy/_types.py#L20`
    ...
    - aliases: Optional[dict] - Model aliases for the user - [Docs](https://dheera_ai.vercel.app/docs/proxy/virtual_keys#model-aliases)
    ...
    - model_max_budget: Optional[dict] - Model-specific max budget for user. [Docs](https://docs.dheera_ai.ai/docs/proxy/users#add-model-specific-budgets-to-keys)
    - model_rpm_limit: Optional[float] - Model-specific rpm limit for user. [Docs](https://docs.dheera_ai.ai/docs/proxy/users#add-model-specific-limits-to-keys)
    - model_tpm_limit: Optional[float] - Model-specific tpm limit for user. [Docs](https://docs.dheera_ai.ai/docs/proxy/users#add-model-specific-limits-to-keys)
    ...
    - object_permission: Optional[DheeraAI_ObjectPermissionBase] - internal user-specific object permission. Example - {"vector_stores": ["vector_store_1", "vector_store_2"]}. IF null or {} then no object permission.
    ...
    """
    try:
        from dheera_ai.proxy.proxy_server import _license_check, prisma_client
...
def get_team_from_list(
    team_list: Optional[Union[List[DheeraAI_TeamTable], List[TeamListResponseObject]]],
    team_id: str,
) -> Optional[Union[DheeraAI_TeamTable, DheeraAI_TeamMembership]]:
...
async def user_info(
    ...
    try:
        ...
        team_list = []
        team_id_list = []
        from dheera_ai.proxy.management_endpoints.team_endpoints import list_team
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.user_info(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def _get_user_info_for_proxy_admin():
    ...
    - get all teams in DheeraAI_TeamTable
    - get all keys in DheeraAI_VerificationToken table
    ...
    sql_query = """
        SELECT 
            (SELECT json_agg(t.*) FROM "DheeraAI_TeamTable" t) as teams,
            (SELECT json_agg(k.*) FROM "DheeraAI_VerificationToken" k WHERE k.team_id != 'dheera_ai-dashboard' OR k.team_id IS NULL) as keys
    """
    if prisma_client is None:
        raise Exception(
            "Database not connected. Connect a database to your proxy - https://docs.dheera_ai.ai/docs/simple_proxy#managing-auth---virtual-keys"
    ...
    # cast all keys to DheeraAI_VerificationToken
    keys_in_db = []
    for key in _keys_in_db:
        if key.get("models") is None:
            key["models"] = []
        keys_in_db.append(DheeraAI_VerificationToken(**key))

    # cast all teams to DheeraAI_TeamTable
    _teams_in_db: List = results[0]["teams"] or []
    _teams_in_db = [DheeraAI_TeamTable(**team) for team in _teams_in_db]
...
def _process_keys_for_user_info(
    keys: Optional[List[DheeraAI_VerificationToken]],
    all_teams: Optional[Union[List[DheeraAI_TeamTable], List[TeamListResponseObject]]],
):
    from dheera_ai.constants import UI_SESSION_TOKEN_TEAM_ID
    from dheera_ai.proxy.proxy_server import general_settings, dheera_ai_master_key_hash
    ...
    if keys is None:
        ...
        for key in keys:
            if (
                key.token == dheera_ai_master_key_hash
            ...
            # Filter out UI session tokens (team_id="dheera_ai-dashboard")
            ...
            if (
                ...
                and _key["team_id"] != "dheera_ai-dashboard"
            ):
...
def _update_internal_user_params(
    ...
    for k, v in data_json.items():
        if (
            ...
            and k not in DheeraAI_ManagementEndpoint_MetadataFields
    ...
    if "max_budget" not in non_default_values:
        if (
            is_internal_user and dheera_ai.max_internal_user_budget is not None
        ):  # applies internal user limits, if user role updated
            non_default_values["max_budget"] = dheera_ai.max_internal_user_budget

    if (
        "budget_duration" not in non_default_values
    ):  # applies internal user limits, if user role updated
        if is_internal_user and dheera_ai.internal_user_budget_duration is not None:
...
async def _update_single_user_helper(
    ...
    user_api_key_dict: UserAPIKeyAuth,
    dheera_ai_changed_by: Optional[str] = None,
) -> Dict[str, Any]:
    """
    ...
    """
    from dheera_ai.proxy.proxy_server import dheera_ai_proxy_admin_name, prisma_client
    ...
    if existing_user_row is not None:
        existing_user_row = DheeraAI_UserTable(
    ...
    if response is not None:
        try:
            ...
            if updated_user_row:
                user_row_typed = DheeraAI_UserTable(
...
async def user_update(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.user_update(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def get_users(
    ...
    # Prepare response
    user_list: List[DheeraAI_UserTableWithKeyCount] = []
    if users is not None:
        for user in users:
            user_list.append(
                DheeraAI_UserTableWithKeyCount(
    ...
    for user_id in data.user_ids:
        ...
        for team in fetch_all_teams:
            is_member_in_team, new_team_members = _cleanup_members_with_roles(
                existing_team_row=DheeraAI_TeamTable(**team.model_dump()),
...
async def add_internal_user_to_organization(
    ...
    Adds the user to DheeraAI_OrganizationMembership table
...
@router.get(
    ...
    include_in_schema=False,
    responses={
        200: {"model": List[DheeraAI_UserTableFiltered]},
    },
)
async def ui_view_users(
    ...
    try:
        ...
        return [DheeraAI_UserTableFiltered(**user.model_dump()) for user in users]
...
async def get_user_daily_activity(
    ...
    try:
        ...
        return await get_daily_activity(
            prisma_client=prisma_client,
            table_name="dheera_ai_dailyuserspend",
