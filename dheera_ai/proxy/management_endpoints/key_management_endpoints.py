...
from dheera_ai.proxy.auth.auth_utils import abbreviate_api_key
from dheera_ai.proxy.auth.user_api_key_auth import user_api_key_auth
from dheera_ai.proxy.common_utils.timezone_utils import get_budget_reset_time
from dheera_ai.proxy.hooks.key_management_event_hooks import KeyManagementEventHooks
from dheera_ai.proxy.management_endpoints.common_utils import (
    _is_user_team_admin,
...
from dheera_ai.proxy.management_helpers.object_permission_utils import (
...
from dheera_ai.proxy.management_helpers.utils import management_endpoint_wrapper
from dheera_ai.proxy.spend_tracking.spend_tracking_utils import _is_master_key
...
from dheera_ai.types.router import Deployment
...
def _is_team_key(data: Union[GenerateKeyRequest, DheeraAI_VerificationToken]):
    return data.team_id is not None


def _get_user_in_team(
    team_table: DheeraAI_TeamTableCachedObj, user_id: Optional[str]
) -> Optional[Member]:
    if user_id is None:
        return None
...
def _is_allowed_to_make_key_request(
    ...
    Relevant issue: https://github.com/BerriAI/dheera_ai/issues/7336
    """
    ...
    if team_id is not None:
        if (
            ...
            return True  # handle https://github.com/BerriAI/dheera_ai/issues/7482
...
def _team_key_operation_team_member_check(
    ...
    team_table: DheeraAI_TeamTableCachedObj,
    user_api_key_dict: UserAPIKeyAuth,
...
def _team_key_generation_check(
    ...
    if user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN.value:
        return True
    if (
        dheera_ai.key_generation_settings is not None
        and "team_key_generation" in dheera_ai.key_generation_settings
    ):
        _team_key_generation = dheera_ai.key_generation_settings["team_key_generation"]
    else:
...
def _personal_key_membership_check(
    ...
    if user_api_key_dict.user_role not in personal_key_generation["allowed_user_roles"]:
        raise HTTPException(
            status_code=400,
            detail=f"Personal key creation has been restricted by admin. Allowed roles={dheera_ai.key_generation_settings['personal_key_generation']['allowed_user_roles']}. Your role={user_api_key_dict.user_role}",  # type: ignore
        )
...
def _personal_key_generation_check(
    ...
    if (
        dheera_ai.key_generation_settings is None
        or dheera_ai.key_generation_settings.get("personal_key_generation") is None
    ):
        return True

    _personal_key_generation = dheera_ai.key_generation_settings["personal_key_generation"]  # type: ignore
...
def key_generation_check(
    team_table: Optional[DheeraAI_TeamTableCachedObj],
    user_api_key_dict: UserAPIKeyAuth,
    data: GenerateKeyRequest,
    route: KeyManagementRoutes,
) -> bool:
    ...
    if is_team_key:
        if team_table is None and dheera_ai.key_generation_settings is not None:
            raise HTTPException(
                status_code=400,
...
def handle_key_type(data: GenerateKeyRequest, data_json: dict) -> dict:
    ...
    if key_type == DheeraAIKeyType.LLM_API:
        data_json["allowed_routes"] = ["llm_api_routes"]
    elif key_type == DheeraAIKeyType.MANAGEMENT:
        data_json["allowed_routes"] = ["management_routes"]
    elif key_type == DheeraAIKeyType.READ_ONLY:
...
async def _common_key_generation_helper(  # noqa: PLR0915
    ...
    if dheera_ai.default_key_generate_params is not None:
        for elem in data:
            key, value = elem
            if value is None and key in [
                ...
                setattr(data, key, dheera_ai.default_key_generate_params.get(key, None))
            elif key == "models" and value == []:
                setattr(data, key, dheera_ai.default_key_generate_params.get(key, []))
            elif key == "metadata" and value == {}:
                setattr(data, key, dheera_ai.default_key_generate_params.get(key, {}))

    # check if user set default key/generate params on config.yaml
    if dheera_ai.upperbound_key_generate_params is not None:
        for elem in data:
            key, value = elem
            upperbound_value = getattr(
                dheera_ai.upperbound_key_generate_params, key, None
            )
    ...
    try:
        from dheera_ai_enterprise.proxy.management_endpoints.key_management_endpoints import (
        ...
        verbose_proxy_logger.debug(
            "dheera_ai.proxy.proxy_server.generate_key_fn(): Enterprise key management params not applied - {}".format(
                str(e)
            )
        )

    # TODO: @ishaan-jaff: Migrate all budget tracking to use DheeraAI_BudgetTable
    _budget_id = data.budget_id
    if prisma_client is not None and data.soft_budget is not None:
        # create the Budget Row for the DheeraAI Verification Token
        budget_row = DheeraAI_BudgetTable(
    ...
    for field in DheeraAI_ManagementEndpoint_MetadataFields_Premium:
    ...
    if data.key is not None and not data.key.startswith("sk-"):
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Invalid key format. DheeraAI Virtual Key must start with 'sk-'. Received: {data.key}"
            },
        )
    ...
    asyncio.create_task(
        KeyManagementEventHooks.async_key_generated_hook(
            ...
            response=response,
            user_api_key_dict=user_api_key_dict,
            dheera_ai_changed_by=dheera_ai_changed_by,
        )
    )
...
def _check_key_model_specific_limits(
    keys: List[DheeraAI_VerificationToken],
    data: Union[GenerateKeyRequest, UpdateKeyRequest],
...
def check_org_key_model_specific_limits(
    keys: List[DheeraAI_VerificationToken],
    org_table: DheeraAI_OrganizationTable,
    data: Union[GenerateKeyRequest, UpdateKeyRequest],
) -> None:
    """
    ...
    if org_table.dheera_ai_budget_table is not None:
        entity_rpm_limit = org_table.dheera_ai_budget_table.rpm_limit
        entity_tpm_limit = org_table.dheera_ai_budget_table.tpm_limit
...
async def generate_key_fn(
    ...
    dheera_ai_changed_by: Optional[str] = Header(
        None,
        description="The dheera_ai-changed-by header enables tracking of actions performed by authorized users on behalf of other users, providing an audit trail for accountability",
    ),
):
    """
    ...
    - aliases: Optional[dict] - Any alias mappings, on top of anything in the config.yaml model list. - https://docs.dheeraai.com/docs/proxy/virtual_keys#managing-auth---upgradedowngrade-models
    - config: Optional[dict] - any key-specific configs, overrides config in config.yaml
    - spend: Optional[int] - Amount spent by key. Default is 0. Will be updated by proxy whenever key is used. https://docs.dheeraai.com/docs/proxy/virtual_keys#managing-auth---tracking-spend
    ...
    - allowed_cache_controls: Optional[list] - List of allowed cache control values. Example - ["no-cache", "no-store"]. See all values - https://docs.dheeraai.com/docs/proxy/caching#turn-on--off-caching-per-request
    ...
    - enforced_params: Optional[List[str]] - List of enforced params for the key (Enterprise only). [Docs](https://docs.dheeraai.com/docs/proxy/enterprise#enforce-required-params-for-llm-requests)
    - prompts: Optional[List[str]] - List of prompts that the key is allowed to use.
    ...
    - object_permission: Optional[DheeraAI_ObjectPermissionBase] - key-specific object permission. Example - {"vector_stores": ["vector_store_1", "vector_store_2"], "agents": ["agent_1", "agent_2"], "agent_access_groups": ["dev_group"]}. IF null or {} then no object permission.
    ...
    try:
        ...
        team_table: Optional[DheeraAI_TeamTableCachedObj] = None
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.generate_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
def prepare_metadata_fields(
    ...
    """
    Check DheeraAI_ManagementEndpoint_MetadataFields (proxy/_types.py) for fields that are allowed to be updated
    """
    ...
    try:
        for k, v in data_json.items():
            if k in DheeraAI_ManagementEndpoint_MetadataFields:
                if isinstance(v, datetime):
                    casted_metadata[k] = v.isoformat()
                else:
                    casted_metadata[k] = v
            if k in DheeraAI_ManagementEndpoint_MetadataFields_Premium:
                from dheera_ai.proxy.utils import _premium_user_check
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.prepare_metadata_fields(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def prepare_key_update_data(
    ...
    existing_key_row: DheeraAI_VerificationToken,
):
    ...
    for k, v in data_json.items():
        if (
            k in DheeraAI_ManagementEndpoint_MetadataFields
            or k in DheeraAI_ManagementEndpoint_MetadataFields_Premium
        ):
            continue
...
def is_different_team(
    data: UpdateKeyRequest, existing_key_row: DheeraAI_VerificationToken
) -> bool:
...
async def update_key_fn(
    ...
    - prompts: Optional[List[str]] - List of prompts that the key is allowed to use.
    - blocked: Optional[bool] - Whether the key is blocked
    - aliases: Optional[dict] - Model aliases for the key - [Docs](https://dheera_ai.vercel.app/docs/proxy/virtual_keys#model-aliases)
    ...
    try:
        ...
        team_obj: Optional[DheeraAI_TeamTableCachedObj] = None
        if data.team_id is not None:
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.update_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
def validate_key_team_change(
    key: DheeraAI_VerificationToken,
    ...
    member_object = _get_user_in_team(
        team_table=cast(DheeraAI_TeamTableCachedObj, team), user_id=key.user_id
    )
    ...
    # this teams member permissions allow updating a
        ...
        team_table=cast(DheeraAI_TeamTableCachedObj, team),
...
async def delete_key_fn(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.delete_key_fn(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
        ...
        if llm_router._is_model_access_group_for_wildcard_route(
            ...
            if not premium_user:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail={
                        "error": "Setting a model access group on a wildcard model is only available for DheeraAI Enterprise users.{}".format(
...
async def generate_key_helper_fn(  # noqa: PLR0915
    ...
    budget_id: Optional[float] = None,  # budget id <-> DheeraAI_BudgetTable
    ...
    object_permission_id: Optional[
        str
    ] = None,  # object_permission_id <-> DheeraAI_ObjectPermissionTable
    ...
    if prisma_client is None:
        raise Exception(
            "Connect Proxy to database to generate keys - https://docs.dheeraai.com/docs/proxy/virtual_keys "
        )
    ...
    if token is None:
        if key is not None:
            ...
            token = f"sk-{secrets.token_urlsafe(LENGTH_OF_DHEERA_AI_GENERATED_KEY)}"
    ...
    try:
        ...
        if isinstance(saved_token["permissions"], str):
            if (
                ...
                raise ValueError(
                    "get_spend_routes permission is only available for DheeraAI Enterprise users"
                )
        ...
        if prisma_client is not None:
            ...
            if table_name is not None and table_name == "user":
                ...
                # the DheeraAI_VerificationToken table will increase in size if we don't do this check
                return user_data
            ...
            key_data["dheera_ai_budget_table"] = getattr(
                create_key_response, "dheera_ai_budget_table", None
            )
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.generate_key_helper_fn(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def _team_key_deletion_check(
    user_api_key_dict: UserAPIKeyAuth,
    key_info: DheeraAI_VerificationToken,
    prisma_client: PrismaClient,
    ...
    if is_team_key and key_info.team_id is not None:
        ...
        if (
            ...
            _team_key_generation = dheera_ai.key_generation_settings[
                "team_key_generation"
            ]
        else:
            _team_key_generation = TeamUIKeyGenerationConfig(
...
async def delete_verification_tokens(
    ...
    Returns:
        Tuple[Optional[Dict], List[DheeraAI_VerificationToken]]:
            ...
            List[DheeraAI_VerificationToken]:
    ...
    try:
        if prisma_client:
            ...
            # else
                ...
                for key in _keys_being_deleted:
                    ...
                    async def _delete_key(key: DheeraAI_VerificationToken):
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.delete_verification_tokens(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def _rotate_master_key(
    ...
    2. Decrypt the values
        - ModelTable
            - [{"model_name": "str", "dheera_ai_params": {}}]
        - ConfigTable
    ...
    await rotate_mcp_server_credentials_master_key(
        prisma_client=prisma_client,
        touched_by=user_api_key_dict.user_id or DHEERA_AI_PROXY_ADMIN_NAME,
    ...
    try:
        ...
        credentials = None
    if credentials:
        from dheera_ai.proxy.credential_endpoints.endpoints import update_db_credential
    ...
    if data and data.new_key is not None:
        ...
        if not data.new_key.startswith("sk-"):
            raise HTTPException(
                ...
                detail={
                    "error": "New key must start with 'sk-'. This is to distinguish a key hash (used by dheera_ai for logging / internal logic) from the actual key."
                },
            )
    else:
        new_token = f"sk-{secrets.token_urlsafe(LENGTH_OF_DHEERA_AI_GENERATED_KEY)}"
    return new_token
...
async def validate_key_list_check(
    ...
    complete_user_info = DheeraAI_UserTable(**complete_user_info_db_obj.model_dump())
...
async def get_admin_team_ids(
    complete_user_info: Optional[DheeraAI_UserTable],
    user_api_key_dict: UserAPIKeyAuth,
    prisma_client: PrismaClient,
) -> List[str]:
    """
    ...
    teams_pydantic_obj = [DheeraAI_TeamTable(**team.model_dump()) for team in teams]
...
) -> Optional[DheeraAI_VerificationToken]:
    """
...
async def test_key_logging(
    ...
    from dheera_ai.proxy.proxy_server import general_settings, proxy_config
    ...
    try:
        data = {
            "model": "openai/dheera_ai-key-health-test",
            "messages": [
                {
                    "role": "user",
                    "content": "Hello, this is a test from dheera_ai /key/health. No LLM API call was made for this",
                }
            ],
...
def validate_model_max_budget(model_max_budget: Optional[Dict]) -> None:
    ...
    try:
        ...
        if model_max_budget is not None:
            from dheera_ai.proxy.proxy_server import CommonProxyErrors, premium_user
        ...
        raise ValueError(
            f"Invalid model_max_budget: {str(e)}. Example of valid model_max_budget: https://docs.dheeraai.com/docs/proxy/users"
        )
