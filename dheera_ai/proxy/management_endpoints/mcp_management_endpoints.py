...
from dheera_ai.proxy._experimental.mcp_server.utils import (
...
if MCP_AVAILABLE:
    ...
    from dheera_ai.proxy._experimental.mcp_server.ui_session_utils import (
    ...
    from dheera_ai.types.mcp import MCPCredentials
    ...
    def _redact_mcp_credentials(
        mcp_server: DheeraAI_MCPServerTable,
    ) -> DheeraAI_MCPServerTable:
    ...
    def _redact_mcp_credentials_list(
        mcp_servers: Iterable[DheeraAI_MCPServerTable],
    ) -> List[DheeraAI_MCPServerTable]:
    ...
    def does_mcp_server_exist(
        mcp_server_records: Iterable[DheeraAI_MCPServerTable], mcp_server_id: str
    ) -> bool:
        """
    ...
    async def get_mcp_tools(
        ...
        """
        from dheera_ai.proxy._experimental.mcp_server.server import _list_mcp_tools
    ...
    @router.get(
        ...
        dependencies=[Depends(user_api_key_auth)],
        response_model=List[DheeraAI_MCPServerTable],
    )
    async def fetch_all_mcp_servers(
        ...
        aggregated_servers: Dict[str, DheeraAI_MCPServerTable] = {}
        ...
        if dheera_ai.public_mcp_servers is not None:
            for server in redacted_mcp_servers:
                if server.server_id in dheera_ai.public_mcp_servers:
    ...
    @router.get(
        ...
        dependencies=[Depends(user_api_key_auth)],
        response_model=DheeraAI_MCPServerTable,
    )
    ...
    async def make_mcp_servers_public(
        ...
        try:
            ...
            if dheera_ai.public_mcp_servers is None:
                dheera_ai.public_mcp_servers = []
            ...
            dheera_ai.public_mcp_servers = request.mcp_server_ids

            # Update config with new settings
            if "dheera_ai_settings" not in config or config["dheera_ai_settings"] is None:
            ...
            verbose_proxy_logger.debug(
                f"Updated public mcp servers to: {dheera_ai.public_mcp_servers} by user: {user_api_key_dict.user_id}"
            )

            return {
                "message": "Successfully updated public mcp servers",
                "public_mcp_servers": dheera_ai.public_mcp_servers,
