...
from dheera_ai.proxy.common_utils.encrypt_decrypt_utils import encrypt_value_helper
from dheera_ai.proxy.management_endpoints.common_utils import _is_user_team_admin
from dheera_ai.proxy.management_endpoints.team_endpoints import (
    team_model_add,
    update_team,
)
from dheera_ai.proxy.management_helpers.audit_logs import create_object_audit_log
...
def update_db_model(
    ...
    merged_deployment_dict = DeploymentTypedDict(
        ...
        dheera_ai_params=DheeraAIParamsTypedDict(
            **db_model.dheera_ai_params.model_dump(exclude_none=True)  # type: ignore
        ),
    ...
    # update dheera_ai params
    if updated_patch.dheera_ai_params:
        # Encrypt any sensitive values
        encrypted_params = {
            k: encrypt_value_helper(v)
            for k, v in updated_patch.dheera_ai_params.model_dump(
                exclude_none=True
            ).items()
        }

        merged_deployment_dict["dheera_ai_params"].update(encrypted_params)  # type: ignore
    ...
    if "dheera_ai_params" in merged_deployment_dict:
        prisma_compatible_model_dict["dheera_ai_params"] = json.dumps(
            merged_deployment_dict["dheera_ai_params"]
        )
...
async def patch_model(
    ...
    try:
        ...
        update_data["updated_by"] = (
            user_api_key_dict.user_id or dheera_ai_proxy_admin_name
        )
        ...
        asyncio.create_task(
            create_object_audit_log(
                ...
                dheera_ai_changed_by=user_api_key_dict.user_id,
...
async def _add_model_to_db(
...
) -> Optional[DheeraAI_ProxyModelTable]:
    # encrypt dheera_ai params #
    _dheera_ai_params_dict = model_params.dheera_ai_params.dict(exclude_none=True)
    _original_dheera_ai_model_name = model_params.dheera_ai_params.model
    for k, v in _dheera_ai_params_dict.items():
        encrypted_value = encrypt_value_helper(
            value=v, new_encryption_key=new_encryption_key
        )
        model_params.dheera_ai_params[k] = encrypted_value
    _data: dict = {
        "model_id": model_params.model_info.id,
        "model_name": model_params.model_name,
        "dheera_ai_params": model_params.dheera_ai_params.model_dump_json(exclude_none=True),  # type: ignore
    ...
    if should_create_model_in_db:
        ...
        model_response = DheeraAI_ProxyModelTable(**_data)
...
class ModelManagementAuthChecks:
    ...
    def can_user_make_team_model_call(
        team_id: str,
        user_api_key_dict: UserAPIKeyAuth,
        team_obj: Optional[DheeraAI_TeamTable] = None,
    ...
    async def can_user_make_model_call(
        ...
        if (
            ...
            team_obj = DheeraAI_TeamTable(**team_obj_row.model_dump())
...
#### [BETA] - This is a beta endpoint, format might change based on user feedback. - https://github.com/BerriAI/dheera_ai/issues/964
@router.post(
    "/model/delete",
...
async def add_new_model(
    ...
    - dheera_ai_params: dict - DheeraAI-specific parameters (required)
        - model: str - The actual model identifier, e.g., "azure/my-deployment-name" (required - this is the only required field in dheera_ai_params)
    ...
    try:
        ...
        model_response: Optional[DheeraAI_ProxyModelTable] = None
        # update DB
        if store_model_in_db is True:
            """
            - store model_list in db
            ...
            try:
                _original_dheera_ai_model_name = model_params.model_name
                ...
                if "slack" in _alerting:
                    ...
                    await proxy_logging_obj.slack_alerting_instance.model_added_alert(
                        model_name=model_params.model_name,
                        dheera_ai_model_name=_original_dheera_ai_model_name,
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.add_new_model(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def update_model(
    ...
    try:
        ...
        if _existing_dheera_ai_params is None:
            if (
        ...
        deployment = Deployment(**_existing_dheera_ai_params.model_dump())
        ...
        # update DB
        if store_model_in_db is True:
            _existing_dheera_ai_params_dict = dict(
                _existing_dheera_ai_params.dheera_ai_params
            )

            if model_params.dheera_ai_params is None:
                raise Exception("dheera_ai_params not provided")

            _new_dheera_ai_params_dict = model_params.dheera_ai_params.dict(
                exclude_none=True
            )

            ### ENCRYPT PARAMS ###
            for k, v in _new_dheera_ai_params_dict.items():
                encrypted_value = encrypt_value_helper(value=v)
                model_params.dheera_ai_params[k] = encrypted_value

            ### MERGE WITH EXISTING DATA ###
            merged_dictionary = {}
            _mp = model_params.dheera_ai_params.dict()

            for key, value in _mp.items():
                if value is not None:
                    merged_dictionary[key] = value
                elif (
                    key in _existing_dheera_ai_params_dict
                    and _existing_dheera_ai_params_dict[key] is not None
                ):
                    merged_dictionary[key] = _existing_dheera_ai_params_dict[key]
                else:
                    pass
            ...
            asyncio.create_task(
                create_object_audit_log(
                    ...
                    table_name=LitellmTableNames.PROXY_MODEL_TABLE_NAME,
                    before_value=(
                        _existing_dheera_ai_params.model_dump_json(exclude_none=True)
                        if isinstance(_existing_dheera_ai_params, BaseModel)
                        else None
                    ),
                    after_value=(
                        model_response.model_dump_json(exclude_none=True)
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.update_model(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def update_public_model_groups(
    ...
    try:
        ...
        import dheera_ai
        from dheera_ai.proxy.proxy_server import proxy_config, store_model_in_db
        ...
        dheera_ai.public_model_groups = request.model_groups

        # Load existing config
        config = await proxy_config.get_config()

        # Update config with new settings
        if "dheera_ai_settings" not in config:
            config["dheera_ai_settings"] = {}

        config["dheera_ai_settings"]["public_model_groups"] = request.model_groups
...
async def update_useful_links(
    ...
    try:
        ...
        dheera_ai.public_model_groups_links = request.useful_links
        ...
        config["dheera_ai_settings"]["public_model_groups_links"] = request.useful_links
...
def _deduplicate_dheera_ai_router_models(models: List[Dict]) -> List[Dict]:
    """
