...
from dheera_ai.proxy.auth.auth_checks import can_user_call_model
...
from dheera_ai.proxy.management_endpoints.common_utils import _set_object_metadata_field
...
from dheera_ai.proxy.management_endpoints.common_daily_activity import get_daily_activity
...
def handle_nested_budget_structure_in_organization_update_request(
    ...
    The UI sends nested budget data in 'dheera_ai_budget_table', but our
    model expects flat budget fields at the top level.
    """
    transformed_data = raw_data.copy()

    # Handle nested budget structure from UI
    if "dheera_ai_budget_table" in transformed_data:
        budget_data = transformed_data.pop("dheera_ai_budget_table", {})
        if budget_data:
            # Extract valid budget fields and merge into top level
            budget_fields = DheeraAI_BudgetTable.model_fields.keys()
...
async def new_organization(
    ...
    user_object_correct_type: Optional[DheeraAI_UserTable] = None
    ...
    if data.budget_id is None:
        ...
        """
        budget_params = DheeraAI_BudgetTable.model_fields.keys()

        # Only include Budget Params when creating an entry in dheera_ai_budgettable
        _json_data = data.json(exclude_none=True)
        _budget_data = {k: v for k, v in _json_data.items() if k in budget_params}
        budget_row = DheeraAI_BudgetTable(**_budget_data)
    ...
    organization_row = DheeraAI_OrganizationTable(
...
async def get_organization_daily_activity(
    ...
    return await get_daily_activity(
        prisma_client=prisma_client,
        table_name="dheera_ai_dailyorganizationspend",
...
async def _set_object_permission(
    ...
    """
    Creates the DheeraAI_ObjectPermissionTable record for the organization.
...
@router.patch(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=DheeraAI_OrganizationTableWithMembers,
)
async def update_organization(
    ...
    data = DheeraAI_OrganizationTableUpdate(**raw_data_with_flat_budget_fields)
    ...
    budget_fields = {
        ...
        if k in DheeraAI_BudgetTable.model_fields.keys() and v is not None
    }
...
async def handle_update_object_permission(
    data_json: dict,
    existing_organization_row: DheeraAI_OrganizationTable,
) -> dict:
    """
    Handle the update of object permission for an organization.

    - Upserts the new object permission into the DheeraAI_ObjectPermissionTable
...
@router.delete(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=List[DheeraAI_OrganizationTableWithMembers],
)
...
async def info_organization(organization_id: str):
    ...
    response_pydantic_obj = DheeraAI_OrganizationTableWithMembers(
        **response.model_dump()
    )
...
async def organization_member_add(
    ...
    2. Creates a new Internal User if the user_id or user_email is not found in DheeraAI_UserTable
    3. Add Internal User to the `DheeraAI_OrganizationMembership` table
    """
    try:
        from dheera_ai.proxy.proxy_server import prisma_client
...
async def find_member_if_email(
    ...
    """
    Find a member if the user_email is in DheeraAI_UserTable
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Unique user not found for user_email={user_email}. Potential duplicate OR non-existent user_email in DheeraAI_UserTable. Use 'user_id' instead."
            },
        )
    existing_user_email_row_pydantic = DheeraAI_UserTable(
...
@router.patch(
    ...
    tags=["organization management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=DheeraAI_OrganizationMembershipTable,
)
@management_endpoint_wrapper
async def organization_member_update(
    ...
    try:
        ...
        final_organization_membership_pydantic = DheeraAI_OrganizationMembershipTable(
...
) -> Tuple[DheeraAI_UserTable, DheeraAI_OrganizationMembershipTable]:
    """
    Add a member to an organization

    - Checks if member.user_id or member.user_email is in DheeraAI_UserTable
    - If not found, create a new user in DheeraAI_UserTable
    - Add user to organization in DheeraAI_OrganizationMembership
    """
    ...
    try:
        ...
        if existing_user_id_row is None and member.user_email is not None:
            try:
                ...
                raise ValueError(
                    f"Potential NON-Existent or Duplicate user email in DB: Error finding a unique instance of user_email={member.user_email} in DheeraAI_UserTable.: {e}"
                )
        ...
        if existing_user_id_row is None and existing_user_email_row is None:
            ...
            if _returned_user is not None:
                user_object = DheeraAI_UserTable(**_returned_user.model_dump())
            ...
            user_object = DheeraAI_UserTable(**existing_user_email_row.model_dump())
        elif existing_user_id_row is not None:
            user_object = DheeraAI_UserTable(**existing_user_id_row.model_dump())
        else:
            raise HTTPException(
                status_code=404,
                detail={
        ...
        if user_object is None:
            raise ValueError(
                f"User does not exist in DheeraAI_UserTable. user_id={member.user_id} and user_email={member.user_email}"
            )
        ...
        organization_membership = DheeraAI_OrganizationMembershipTable(
