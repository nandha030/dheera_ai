"""
✨ SCIM v2 Endpoints for DheeraAI Proxy using Internal User/Team Management
...
from dheera_ai.proxy.utils import _premium_user_check, handle_exception_on_proxy
...
class UserProvisionerHelpers:
    ...
    async def handle_existing_user_by_email(
        ...
        return await ScimTransformations.transform_dheera_ai_user_to_scim_user(
            updated_user
        )
...
def _build_scim_metadata(
    ...
    metadata: Dict[str, Any] = {
        "scim_metadata": DheeraAI_UserScimMetadata(
...
async def _create_user_if_not_exists(
    ...
    Returns:
        DheeraAI_UserTable if user was created, None if creation failed
    """
    from dheera_ai.proxy.management_endpoints.internal_user_endpoints import new_user
    ...
    try:
        ...
        if dheera_ai.default_internal_user_params:
            default_role = dheera_ai.default_internal_user_params.get("user_role")
...
async def _get_team_member_user_ids_from_team(team: DheeraAI_TeamTable) -> List[str]:
    """
...
async def get_users(
    ...
    try:
        ...
        for user in users:
            scim_user = await ScimTransformations.transform_dheera_ai_user_to_scim_user(
                user=user
            )
...
async def get_user(
    ...
    try:
        ...
        # Convert to SCIM format
        scim_user = await ScimTransformations.transform_dheera_ai_user_to_scim_user(user)
        return scim_user
...
async def get_group(
    ...
    try:
        ...
        scim_group = await ScimTransformations.transform_dheera_ai_team_to_scim_group(
            team
        )
...
async def patch_group(
    ...
    try:
        ...
        if refreshed_team:
            ...
            refreshed_current_members = set(
                await _get_team_member_user_ids_from_team(
                    DheeraAI_TeamTable(**refreshed_team.model_dump())
                )
            )
        ...
        # Convert to SCIM format and return
        scim_group = await ScimTransformations.transform_dheera_ai_team_to_scim_group(
            DheeraAI_TeamTable(**updated_team.model_dump())
        )
        return scim_group
