...
from dheera_ai.proxy.management_helpers.utils import (
    add_new_member,
    management_endpoint_wrapper,
)
from dheera_ai.proxy.utils import PrismaClient, handle_exception_on_proxy
from dheera_ai.router import Router
from dheera_ai.types.proxy.management_endpoints.common_daily_activity import (
...
class TeamMemberBudgetHandler:
    ...
    async def create_team_member_budget_table(
        data: Union[NewTeamRequest, DheeraAI_TeamTable],
        ...
        from dheera_ai.proxy._types import BudgetNewRequest
...
def _is_available_team(team_id: str, user_api_key_dict: UserAPIKeyAuth) -> bool:
    if dheera_ai.default_internal_user_params is None:
        return False
    if "available_teams" in dheera_ai.default_internal_user_params:
        return team_id in dheera_ai.default_internal_user_params["available_teams"]
    return False


async def get_all_team_memberships(
    prisma_client: PrismaClient, team_ids: List[str], user_id: Optional[str] = None
) -> List[DheeraAI_TeamMembership]:
    ...
    returned_tm: List[DheeraAI_TeamMembership] = []
    for tm in team_memberships:
        returned_tm.append(DheeraAI_TeamMembership(**tm.model_dump()))
...
async def _check_org_team_limits(
    ...
    if (
        ...
        and org_table.dheera_ai_budget_table is not None
        and org_table.dheera_ai_budget_table.max_budget is not None
        and data.max_budget > org_table.dheera_ai_budget_table.max_budget
    ):
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Team max_budget ({data.max_budget}) exceeds organization's max_budget ({org_table.dheera_ai_budget_table.max_budget}). Organization: {org_table.organization_id}"
            },
        )
    ...
    if (
        data.tpm_limit is not None
        and org_table.dheera_ai_budget_table is not None
        and org_table.dheera_ai_budget_table.tpm_limit is not None
        and data.tpm_limit > org_table.dheera_ai_budget_table.tpm_limit
    ):
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Team tpm_limit ({data.tpm_limit}) exceeds organization's tpm_limit ({org_table.dheera_ai_budget_table.tpm_limit}). Organization: {org_table.organization_id}"
            },
        )
    ...
    if (
        data.rpm_limit is not None
        and org_table.dheera_ai_budget_table is not None
        and org_table.dheera_ai_budget_table.rpm_limit is not None
        and data.rpm_limit > org_table.dheera_ai_budget_table.rpm_limit
    ):
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Team rpm_limit ({data.rpm_limit}) exceeds organization's rpm_limit ({org_table.dheera_ai_budget_table.rpm_limit}). Organization: {org_table.organization_id}"
            },
        )
    ...
    # Convert teams to DheeraAI_TeamTable objects
    team_objs: List[DheeraAI_TeamTable] = []
    for team in teams:
        team_objs.append(DheeraAI_TeamTable(**team.model_dump()))
...
@router.post(
    ...
    response_model=DheeraAI_TeamTable,
)
@management_endpoint_wrapper
async def new_team(  # noqa: PLR0915
    ...
    try:
        ...
        if data.max_budget is None:
            if (
                isinstance(dheera_ai.default_team_settings, list)
                and len(dheera_ai.default_team_settings) > 0
                and isinstance(dheera_ai.default_team_settings[0], dict)
            ):
                default_settings = dheera_ai.default_team_settings[0]
        ...
        if data.model_aliases is not None and isinstance(data.model_aliases, dict):
            dheera_ai_modeltable = DheeraAI_ModelTable(
        ...
        ## ADD TO TEAM TABLE
        complete_team_data = DheeraAI_TeamTable(
            **data_json,
            model_id=_model_id,
        )
        ...
        # Enterprise Feature - Audit Logging. Enable with dheera_ai.store_audit_logs = True
...
async def _create_team_update_audit_log(
    ...
    user_api_key_dict: UserAPIKeyAuth,
    dheera_ai_proxy_admin_name: str,
) -> None:
    """
    ...
    Args:
        ...
        dheera_ai_changed_by: Optional header indicating who made the change
        user_api_key_dict: User API key authentication details
        dheera_ai_proxy_admin_name: Name of the proxy admin
    """
        ...
        if model_id is None:
            ...
            model_dict = await prisma_client.db.dheera_ai_modeltable.upsert(
                ...
                data={
                    ...
                    "create": {**dheera_ai_modeltable.json(exclude_none=True)},  # type: ignore
                },
            )  # type: ignore
...
async def fetch_and_validate_organization(
    ...
    validate_team_org_change(
        team=DheeraAI_TeamTable(**existing_team_row.model_dump()),
        organization=DheeraAI_OrganizationTableWithMembers(**organization_row.model_dump()),
...
def validate_team_org_change(
    team: DheeraAI_TeamTable, organization: DheeraAI_OrganizationTableWithMembers, llm_router: Router
) -> bool:
    """
    ...
    if (
        team.max_budget
        and organization.dheera_ai_budget_table
        and organization.dheera_ai_budget_table.max_budget
        and team.max_budget > organization.dheera_ai_budget_table.max_budget
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has max_budget {team.max_budget} that is greater than the organization's max_budget {organization.dheera_ai_budget_table.max_budget}."
            },
        )
    ...
    if (
        team.tpm_limit
        and organization.dheera_ai_budget_table
        and organization.dheera_ai_budget_table.tpm_limit
        and team.tpm_limit > organization.dheera_ai_budget_table.tpm_limit
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has tpm_limit {team.tpm_limit} that is greater than the organization's tpm_limit {organization.dheera_ai_budget_table.tpm_limit}."
            },
        )
    if (
        team.rpm_limit
        and organization.dheera_ai_budget_table
        and organization.dheera_ai_budget_table.rpm_limit
        and team.rpm_limit > organization.dheera_ai_budget_table.rpm_limit
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has rpm_limit {team.rpm_limit} that is greater than the organization's rpm_limit {organization.dheera_ai_budget_table.rpm_limit}."
            },
        )
    return True
...
async def update_team(
    ...
    try:
        ...
        await _cache_team_object(
            ...
            team_table=DheeraAI_TeamTableCachedObj(**team_row.model_dump()),
            user_api_key_cache=user_api_key_cache,
...
async def handle_update_object_permission(
    data_json: dict, existing_team_row: DheeraAI_TeamTable
) -> dict:
    """
    Handle the update of object permission for a team.

    - IF there's no object_permission_id, then create a new entry in DheeraAI_ObjectPermissionTable
    - IF there's an object_permission_id, then update the entry in DheeraAI_ObjectPermissionTable
    """
...
async def _process_team_members(
    ...
    user_api_key_dict: UserAPIKeyAuth,
    dheera_ai_proxy_admin_name: str,
) -> Tuple[List[DheeraAI_UserTable], List[DheeraAI_TeamMembership]]:
...
async def _add_team_members_to_team(
    ...
    user_api_key_dict: UserAPIKeyAuth,
    dheera_ai_proxy_admin_name: str,
) -> Tuple[DheeraAI_TeamTable, List[DheeraAI_UserTable], List[DheeraAI_TeamMembership]]:
...
async def team_member_add(
    ...
    complete_team_data = DheeraAI_TeamTable(**existing_team_row.model_dump())
...
async def team_member_delete(
    ...
    existing_team_row = DheeraAI_TeamTable(**_existing_team_row.model_dump())
...
async def team_member_update(
    ...
    if data.role == "admin" and not premium_user:
        ...
        raise HTTPException(
            status_code=400,
            detail="Assigning team admins is a premium feature. You must be a DheeraAI Enterprise user to use this feature. If you have a license please set `DHEERA_AI_LICENSE` in your env. Get a 7 day trial key here: https://www.dheera_ai.ai/#trial. Pricing: https://www.dheera_ai.ai/#pricing",
        )
    if data.user_id is None and data.user_email is None:
        raise HTTPException(
            status_code=400,
...
async def delete_team(
    ...
    team_rows: List[DheeraAI_TeamTable] = []
    for team_id in data.team_ids:
        ...
        team_row_pydantic = DheeraAI_TeamTable(**team_row_base.model_dump())
    ...
    if dheera_ai.store_audit_logs is True:
        ...
        for team_id in data.team_ids:
            team_row: Optional[DheeraAI_TeamTable] = await prisma_client.get_data(  # type: ignore
...
def validate_membership(
    user_api_key_dict: UserAPIKeyAuth, team_table: DheeraAI_TeamTable
):
    if (
        user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN.value
...
def _unfurl_all_proxy_models(
    team_info: DheeraAI_TeamTable, llm_router: Router
) -> DheeraAI_TeamTable:
    if (
...
async def team_info(
    ...
    try:
        ...
        validate_membership(
            user_api_key_dict=user_api_key_dict,
            team_table=DheeraAI_TeamTable(**team_info.model_dump()),
        )
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.management_endpoints.team_endpoints.py::team_info - Exception occurred - {}\n{}".format(
...
async def list_available_teams(
    http_request: Request,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    response_model=List[DheeraAI_TeamTable],
):
    from dheera_ai.proxy.proxy_server import prisma_client
    ...
    available_teams = cast(
        Optional[List[str]],
        (
            dheera_ai.default_internal_user_params.get("available_teams")
    ...
    user_info_correct_type = DheeraAI_UserTable(**user_info.model_dump())
    ...
    available_teams_correct_type = [
        DheeraAI_TeamTable(**team.model_dump()) for team in available_teams_db
    ]
...
async def list_team(
    ...
    for team in filtered_response:
        _team_memberships: List[DheeraAI_TeamMembership] = []
...
async def get_paginated_teams(
    ...
    page_size: int = 10,
    page: int = 1,
) -> Tuple[List[DheeraAI_TeamTable], int]:
    """
    ...
    Returns:
        Tuple[List[DheeraAI_TeamTable], int] - (list of teams, total count)
    """
    try:
...
@router.get(
    ...
    responses={
        200: {"model": List[DheeraAI_TeamTable]},
    },
)
async def ui_view_teams(
    ...
    Returns:
        List[DheeraAI_SpendLogs]: Paginated list of matching user records
    """
    from dheera_ai.proxy.proxy_server import prisma_client
...
def add_new_models_to_team(
    team_obj: DheeraAI_TeamTable, new_models: List[str]
) -> List[str]:
    """
...
async def team_model_add(
    ...
    team_obj = DheeraAI_TeamTable(**team_row.model_dump())
...
async def get_team_daily_activity(
    ...
    return await get_daily_activity(
        prisma_client=prisma_client,
        table_name="dheera_ai_dailyteamspend",
