...
/sso/callback - returns JWT Redirect Response that redirects to DheeraAI UI
...
from dheera_ai.dheera_ai_core_utils.dot_notation_indexing import get_nested_value
...
from dheera_ai.proxy.auth.auth_checks import ExperimentalUIJWTToken, get_user_object
...
from dheera_ai.proxy.common_utils.admin_ui_utils import (
    admin_ui_disabled,
    show_missing_vars_in_env,
)
from dheera_ai.proxy.common_utils.html_forms.jwt_display_template import (
    jwt_display_template,
)
from dheera_ai.proxy.common_utils.html_forms.ui_login import html_form
from dheera_ai.proxy.management_endpoints.internal_user_endpoints import new_user
from dheera_ai.proxy.management_endpoints.sso_helper_utils import (
    check_is_admin_only_access,
    has_admin_ui_access,
)
from dheera_ai.proxy.management_endpoints.team_endpoints import new_team, team_member_add
from dheera_ai.proxy.management_endpoints.types import (
    CustomOpenID,
    get_dheera_ai_user_role,
    is_valid_dheera_ai_user_role,
)
...
from dheera_ai.secret_managers.main import get_secret_bool, str_to_bool
from dheera_ai.types.proxy.management_endpoints.ui_sso import *
from dheera_ai.types.proxy.ui_sso import ParsedOpenIDResult
...
async def google_login(
    ...
    if user_custom_ui_sso_sign_in_handler is not None:
        try:
            from dheera_ai_enterprise.proxy.auth.custom_sso_handler import (
...
def generic_response_convertor(
    ...
    if user_role_from_sso is not None:
        role = get_dheera_ai_user_role(user_role_from_sso)
        if role is not None:
            user_role = role
            verbose_proxy_logger.debug(
...
async def add_missing_team_member(
    user_info: Union[NewUserResponse, DheeraAI_UserTable], sso_teams: List[str]
):
    """
...
def get_disabled_non_admin_personal_key_creation():
    key_generation_settings = dheera_ai.key_generation_settings
...
) -> Optional[Union[DheeraAI_UserTable, NewUserResponse]]:
    try:
        potential_user_ids = []
        ...
        user_info: Optional[Union[DheeraAI_UserTable, NewUserResponse]] = None
        ...
        verbose_proxy_logger.debug(
            f"user_info: {user_info}; dheera_ai.default_internal_user_params: {dheera_ai.default_internal_user_params}"
        )

        # Upsert SSO User to DheeraAI DB
...
def _should_use_role_from_sso_response(sso_role: Optional[str]) -> bool:
    ...
    if not is_valid_dheera_ai_user_role(sso_role):
        verbose_proxy_logger.debug(
            f"SSO role '{sso_role}' is not a valid DheeraAI user role. "
...
def _build_sso_user_update_data(
    ...
    if sso_role is not None:
        ...
        # Only include if it's a valid DheeraAI role
...
def apply_user_info_values_to_sso_user_defined_values(
    user_info: Optional[Union[DheeraAI_UserTable, NewUserResponse]],
    user_defined_values: Optional[SSOUserDefinedValues],
...
async def auth_callback(request: Request, state: Optional[str] = None):  # noqa: PLR0915
    ...
    if state and state.startswith(f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:"):
...
async def cli_sso_callback(
    ...
    try:
        ...
        from dheera_ai.constants import CLI_SSO_SESSION_CACHE_KEY_PREFIX
...
async def cli_poll_key(key_id: str, team_id: Optional[str] = None):
    ...
    """
    from dheera_ai.constants import CLI_SSO_SESSION_CACHE_KEY_PREFIX
    from dheera_ai.proxy.auth.auth_checks import ExperimentalUIJWTToken
    ...
    try:
        ...
        if session_data:
            ...
            user_info = DheeraAI_UserTable(
                ...
                max_budget=dheera_ai.max_ui_session_budget,
            )
...
async def insert_sso_user(
    ...
    """
    Helper function to create a New User in DheeraAI DB after a successful SSO login

    Args:
        result_openid (OpenID): User information in OpenID format if the login was successful.
        user_defined_values (Optional[SSOUserDefinedValues], optional): DheeraAI SSOValues / fields that were read
    ...
    if dheera_ai.default_internal_user_params:
        user_defined_values.update(dheera_ai.default_internal_user_params)  # type: ignore
    ...
    if user_defined_values.get("user_role") == LitellmUserRoles.INTERNAL_USER.value:
        if user_defined_values.get("max_budget") is None:
            user_defined_values["max_budget"] = dheera_ai.max_internal_user_budget
...
async def get_ui_settings(request: Request):
    from dheera_ai.proxy.proxy_server import general_settings, proxy_state

    _proxy_base_url = os.getenv("PROXY_BASE_URL", None)
    _logout_url = os.getenv("PROXY_LOGOUT_URL", None)
    _api_doc_base_url = os.getenv("DHEERA_AI_UI_API_DOC_BASE_URL", None)
    ...
    return {
        ...
        "DHEERA_AI_UI_API_DOC_BASE_URL": _api_doc_base_url,
...
class SSOAuthenticationHandler:
    ...
    def _get_generic_sso_redirect_params(
        ...
        if state:
            ...
            # the dheera_ai proxy cli sends the "state" parameter to the proxy server for auth. We should maintain the state parameter for the cli if it is provided
    ...
    async def upsert_sso_user(
        result: Optional[Union[CustomOpenID, OpenID, dict]],
        user_info: Optional[Union[NewUserResponse, DheeraAI_UserTable]],
        user_email: Optional[str],
        user_defined_values: Optional[SSOUserDefinedValues],
        prisma_client: PrismaClient,
    ):
        """
        Connects the SSO Users to the User Table in DheeraAI DB

        - If user on DheeraAI DB, update the user_email and user_role (if SSO provides valid role) with the SSO values
        - If user not on DheeraAI DB, insert the user into DheeraAI DB
        """
        try:
            if user_info is not None:
                user_id = user_info.user_id
                ...
                verbose_proxy_logger.info(
                    "user not in DB, inserting user into DheeraAI DB"
                )
                # user not in DB, insert User into DheeraAI DB
            ...
            verbose_proxy_logger.exception(
                f"Error upserting SSO user into DheeraAI DB: {e}"
            )
            return user_info
    ...
    async def add_user_to_teams_from_sso_response(
        ...
        The `team_ids` field is populated by dheera_ai after processing the SSO response
        """
        if user_info is None:
            verbose_proxy_logger.debug(
                "User not found in DheeraAI DB, skipping team member addition"
            )
            return
    ...
    @staticmethod
    async def create_dheera_ai_team_from_sso_group(
        dheera_ai_team_id: str,
        dheera_ai_team_name: Optional[str] = None,
    ):
        """
        Creates a Litellm Team from a SSO Group ID

        Your SSO provider might have groups that should be created on DheeraAI

        Use this helper to create a Litellm Team from a SSO Group ID

        Args:
            dheera_ai_team_id (str): The ID of the Litellm Team
            dheera_ai_team_name (Optional[str]): The name of the Litellm Team
        """
        ...
        try:
            ...
            if team_obj:
                verbose_proxy_logger.debug(
                    f"Team already exists: {dheera_ai_team_id} - {dheera_ai_team_name}"
                )
                return

            team_request: NewTeamRequest = NewTeamRequest(
                team_id=dheera_ai_team_id,
                team_alias=dheera_ai_team_name,
            )
            if dheera_ai.default_team_params:
                team_request = SSOAuthenticationHandler._cast_and_deepcopy_dheera_ai_default_team_params(
                    default_team_params=dheera_ai.default_team_params,
                    dheera_ai_team_id=dheera_ai_team_id,
                    dheera_ai_team_name=dheera_ai_team_name,
            ...
            await new_team(
                ...
                user_api_key_dict=UserAPIKeyAuth(
                    token="",
                    key_alias=f"dheera_ai.{MicrosoftSSOHandler.__name__}",
                ),
            )
        except Exception as e:
    ...
    @staticmethod
    def _cast_and_deepcopy_dheera_ai_default_team_params(
        ...
        """
        Casts and deepcopies the dheera_ai.default_team_params to a NewTeamRequest object

        - Ensures we create a new DefaultTeamSSOParams object
        - Handle the case where dheera_ai.default_team_params is a dict or a DefaultTeamSSOParams object
        - Adds the dheera_ai_team_id and dheera_ai_team_name to the DefaultTeamSSOParams object
        """
        if isinstance(default_team_params, dict):
            _team_request = deepcopy(default_team_params)
            _team_request["team_id"] = dheera_ai_team_id
            _team_request["team_alias"] = dheera_ai_team_name
            team_request = NewTeamRequest(**_team_request)
        elif isinstance(dheera_ai.default_team_params, DefaultTeamSSOParams):
            _default_team_params = deepcopy(dheera_ai.default_team_params)
    ...
    def _get_cli_state(
        ...
        from dheera_ai.constants import (
            DHEERA_AI_CLI_SESSION_TOKEN_PREFIX,
            DHEERA_AI_CLI_SOURCE_IDENTIFIER,
        )

        if source == DHEERA_AI_CLI_SOURCE_IDENTIFIER and key:
            if existing_key:
                return f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:{key}:{existing_key}"
            else:
                return f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:{key}"
        else:
            return None
    ...
    async def get_redirect_response_from_openid(  # noqa: PLR0915
        ...
        from dheera_ai.proxy.utils import get_prisma_client_or_throw
        ...
        user_info = None
        user_id_models: List = []
        max_internal_user_budget = dheera_ai.max_internal_user_budget
        internal_user_budget_duration = dheera_ai.internal_user_budget_duration
        ...
        if user_defined_values is None:
            raise Exception(
                "Unable to map user identity to known values. 'user_defined_values' is None. File an issue - https://github.com/BerriAI/dheera_ai/issues"
            )
        ...
        verbose_proxy_logger.info(f"Redirecting to {dheera_ai_dashboard_ui}")
...
class MicrosoftSSOHandler:
    ...
    # used for debugging to show the user groups dheera_ai found from Graph API
    ...
    async def get_microsoft_callback_response(
        ...
        if app_roles:
            ...
            for role_str in app_roles:
                role = get_dheera_ai_user_role(role_str)
                if role is not None:
                    user_role = role
                    verbose_proxy_logger.debug(
    ...
    @staticmethod
    async def create_dheera_ai_teams_from_service_principal_team_ids(
        service_principal_teams: List[MicrosoftServicePrincipalTeam],
    ):
        """
        Creates Litellm Teams from the Service Principal Group IDs

        When a user sets a `SERVICE_PRINCIPAL_ID` in the env, dheera_ai will fetch groups under that service principal and create Litellm Teams from them
        """
        verbose_proxy_logger.debug(
        ...
        for service_principal_team in service_principal_teams:
            dheera_ai_team_id: Optional[str] = service_principal_team.get("principalId")
            dheera_ai_team_name: Optional[str] = service_principal_team.get(
                "principalDisplayName"
            )
            if not dheera_ai_team_id:
                verbose_proxy_logger.debug(
                    f"Skipping team creation for {dheera_ai_team_name} because it has no principalId"
                )
                continue

            await SSOAuthenticationHandler.create_dheera_ai_team_from_sso_group(
                dheera_ai_team_id=dheera_ai_team_id,
                dheera_ai_team_name=dheera_ai_team_name,
            )
...
async def debug_sso_login(request: Request):
    ...
    if (
        ...
        if premium_user is not True:
            raise ProxyException(
                message="You must be a DheeraAI Enterprise user to use SSO. If you have a license please set `DHEERA_AI_LICENSE` in your env. If you want to obtain a license meet with us here: https://calendly.com/d/4mp-gd3-k5k/dheera_ai-1-1-onboarding-chat You are seeing this error message because You set one of `MICROSOFT_CLIENT_ID`, `GOOGLE_CLIENT_ID`, or `GENERIC_CLIENT_ID` in your env. Please unset this",
                type=ProxyErrorTypes.auth_error,
                param="premium_user",
                code=status.HTTP_403_FORBIDDEN,
            )
