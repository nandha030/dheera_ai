...
import dheera_ai
from dheera_ai.proxy._types import CommonProxyErrors, InvitationNew, UserAPIKeyAuth
...
async def create_invitation_for_user(
    ...
    """
    Create an invitation for the user to onboard to DheeraAI Admin UI.
    """
    ...
    try:
        ...
        if "Foreign key constraint failed on the field" in str(e):
            raise HTTPException(
                status_code=400,
                detail={
                    "error": "User id does not exist in 'DheeraAI_UserTable'. Fix this by creating user via `/user/new`."
                },
            )
