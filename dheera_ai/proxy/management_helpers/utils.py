...
from dheera_ai.proxy._types import (  # key request types; user request types; team request types; customer request types
    BudgetNewRequest,
...
def get_new_internal_user_defaults(
    ...
    user_info = dheera_ai.default_internal_user_params or {}

    returned_dict: SSOUserDefinedValues = {
        "models": user_info.get("models") or [],
        "max_budget": user_info.get("max_budget", dheera_ai.max_internal_user_budget),
        "budget_duration": user_info.get(
            "budget_duration", dheera_ai.internal_user_budget_duration
        ),
...
async def handle_budget_for_entity(
    ...
    Args:
        ...
        dheera_ai_proxy_admin_name: Admin name for audit trail
...
async def add_new_member(
...
) -> Tuple[DheeraAI_UserTable, Optional[DheeraAI_TeamMembership]]:
    """
    ...
    """
    returned_user: Optional[DheeraAI_UserTable] = None
    returned_team_membership: Optional[DheeraAI_TeamMembership] = None
    ...
    if _budget_id and returned_user is not None and returned_user.user_id is not None:
        ...
        returned_team_membership = DheeraAI_TeamMembership(
