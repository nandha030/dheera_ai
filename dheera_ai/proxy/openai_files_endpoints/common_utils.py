...
def get_models_from_unified_file_id(unified_file_id: str) -> List[str]:
    ...
    Example:
    unified_file_id = "dheera_ai_proxy:application/octet-stream;unified_id,c4843482-b176-4901-8292-7523fd0f2c6e;target_model_names,gpt-4o-mini,gemini-2.0-flash"
...
def get_model_id_from_unified_batch_id(file_id: str) -> Optional[str]:
    ...
    Expected format: dheera_ai_proxy;model_id:{};llm_batch_id:{};llm_output_file_id:{}
    """
...
def encode_file_id_with_model(file_id: str, model: str) -> str:
    ...
    Format: <prefix>-<base64(dheera_ai:<original_id>;model,<model_name>)>
    ...
    Args:
        ...
        model: Model name from model_list (e.g., "gpt-4o-dheera_ai")
    
    Returns:
        Encoded ID starting with appropriate prefix and containing routing information
    
    Examples:
        encode_file_id_with_model("file-abc123", "gpt-4o-dheera_ai")
    ...
    """
    encoded_str = f"dheera_ai:{file_id};model,{model}"
...
def decode_model_from_file_id(encoded_id: str) -> Optional[str]:
    ...
    try:
        ...
        decoded = base64.urlsafe_b64decode(padded).decode()   
        if decoded.startswith("dheera_ai:") and ";model," in decoded:
...
def get_original_file_id(encoded_id: str) -> str:
    ...
    try:
        ...
        if decoded.startswith("dheera_ai:") and ";model," in decoded:
            match = re.search(r"dheera_ai:([^;]+);model,", decoded)
            if match:
...
def extract_model_from_sources(
    ...
    2. Request headers (x-dheera_ai-model)
