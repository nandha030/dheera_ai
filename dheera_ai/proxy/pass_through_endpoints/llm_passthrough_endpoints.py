...
Use dheera_ai with Anthropic SDK, Vertex AI SDK, Cohere SDK, etc.
"""
...
from dheera_ai.constants import BEDROCK_AGENT_RUNTIME_PASS_THROUGH_ROUTES
...
from dheera_ai.proxy.pass_through_endpoints.common_utils import get_dheera_ai_virtual_key
...
from dheera_ai.proxy.utils import is_known_model
...
def is_passthrough_request_using_router_model(
    request_body: dict, llm_router: Optional[dheera_ai.Router]
) -> bool:
    """
...
async def milvus_proxy_route(
    ...
    if not dheera_ai.vector_store_index_registry or not dheera_ai.vector_store_registry:
        raise HTTPException(
            status_code=500,
    ...
    is_vector_store_index = dheera_ai.vector_store_index_registry.is_vector_store_index(
    ...
    if not is_vector_store_index:
        raise HTTPException(
            status_code=400,
            detail=f"Collection {collection_name} is not a dheera_ai managed vector store index. Only dheera_ai managed vector store indexes are supported.",
        )
    ...
    index_object = (
        (
            dheera_ai.vector_store_index_registry.get_vector_store_index_by_name(
                vector_store_index_name=collection_name
            )
        )
        if dheera_ai.vector_store_index_registry is not None
        else None
    )
    if index_object is None:
        raise Exception(f"Vector store index not found for {collection_name}")

    vector_store_name = index_object.dheera_ai_params.vector_store_name
    vector_store_index = index_object.dheera_ai_params.vector_store_index
    ...
    vector_store = dheera_ai.vector_store_registry.get_dheera_ai_managed_vector_store_from_registry_by_name(
        vector_store_name=vector_store_name
    )
    if vector_store is None:
        raise Exception(f"Vector store not found for {vector_store_name}")
    dheera_ai_params = vector_store.get("dheera_ai_params") or {}
    ...
    base_target_url = provider_config.get_complete_url(
        api_base=dheera_ai_params.get("api_base"), dheera_ai_params=dheera_ai_params
    )
...
async def handle_bedrock_passthrough_router_model(
    ...
    Args:
        ...
        llm_router: The DheeraAI router instance
...
async def handle_bedrock_count_tokens(
    ...
    """
    from dheera_ai.llms.bedrock.count_tokens.handler import BedrockCountTokensHandler
    from dheera_ai.proxy.proxy_server import llm_router
    ...
    try:
        ...
        dheera_ai_params = {"user_api_key_dict": user_api_key_dict}
        ...
        if llm_router:
            ...
            if deployments and len(deployments) > 0:
                ...
                model_dheera_ai_params = deployment.get("dheera_ai_params", {})

                # Get the resolved model ID from the configuration
                if "model" in model_dheera_ai_params:
                    resolved_model = model_dheera_ai_params["model"]

                # Copy all dheera_ai_params - BaseAWSLLM will handle AWS credential discovery
                for key, value in model_dheera_ai_params.items():
                    if key != "user_api_key_dict":  # Don't overwrite user_api_key_dict
                        dheera_ai_params[key] = value  # type: ignore

        verbose_proxy_logger.debug(f"Count tokens dheera_ai_params: {dheera_ai_params}")
...
async def bedrock_proxy_route(
    ...
    aws_region_name = dheera_ai.utils.get_secret(secret_name="AWS_REGION_NAME")
    ...
    # Add or update query parameters
    from dheera_ai.llms.bedrock.chat import BedrockConverseLLM
...
async def assemblyai_proxy_route(
    ...
    from dheera_ai.proxy.pass_through_endpoints.llm_provider_handlers.assembly_passthrough_logging_handler import (
...
async def azure_proxy_route(
    ...
    Checks if the deployment id in the url is a dheera_ai model name. If so, it will route using the llm_router.allm_passthrough_route.
    """
    from dheera_ai.proxy.proxy_server import llm_router
    ...
    if len(parts) > 1 and llm_router:
        for part in parts:
            ...
            is_vector_store_index = (
                (
                    dheera_ai.vector_store_index_registry.is_vector_store_index(
            ...
            if is_router_model:
                ...
                provider_config = (
                    ProviderConfigManager.get_provider_vector_stores_config(
                        provider=dheera_ai.LlmProviders.AZURE_AI
                    )
                )
                if provider_config is None:
                ...
                base_target_url = dheera_ai_params.get("api_base")
                if base_target_url is None:
                ...
                return await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
                    ...
                    api_key=None,
                    custom_llm_provider=dheera_ai.LlmProviders.AZURE_AI,
    ...
    azure_api_key = passthrough_endpoint_router.get_credentials(
        custom_llm_provider=dheera_ai.LlmProviders.AZURE.value,
        region_name=None,
    )
    ...
    return await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
        ...
        custom_llm_provider=dheera_ai.LlmProviders.AZURE,
    )
...
def _override_vertex_params_from_router_credentials(
    ...
    Args:
        router_credentials: Optional vector store credentials from registry (DheeraAI_ManagedVectorStore)
    ...
    dheera_ai_params = router_credentials.get("dheera_ai_params", {})
    if not dheera_ai_params:
        verbose_proxy_logger.warning(
            "Vector store credentials found but dheera_ai_params is empty"
        )
        return vertex_project, vertex_location

    # Extract vertex_project and vertex_location from dheera_ai_params
    vector_store_project = dheera_ai_params.get("vertex_project")
    vector_store_location = dheera_ai_params.get("vertex_location")
    ...
    if vector_store_project:
        ...
        verbose_proxy_logger.warning(
            "Vector store credentials found but missing vertex_project in dheera_ai_params"
        )
    ...
    if vector_store_location:
        ...
        verbose_proxy_logger.warning(
            "Vector store credentials found but missing vertex_location in dheera_ai_params"
        )
...
async def _base_vertex_proxy_route(
    ...
    headers: dict = {}
    api_key_to_use = get_dheera_ai_virtual_key(request=request)
    user_api_key_dict = await user_api_key_auth(
        request=request,
...
async def vertex_discovery_proxy_route(
    ...
    # Extract vector store ID from endpoint if present (e.g., dataStores/test-dheera_ai-app_1761094730750)
    vector_store_credentials: Optional[DheeraAI_ManagedVectorStore] = None
...
async def openai_proxy_route(
    ...
    return await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
        ...
        custom_llm_provider=dheera_ai.LlmProviders.OPENAI,
    )
...
class BaseOpenAIPassThroughHandler:
    ...
    async def _base_openai_pass_through_handler(
        ...
        custom_llm_provider: dheera_ai.LlmProviders,
    ...
    @staticmethod
    def _join_url_paths(
        base_url: httpx.URL, path: str, custom_llm_provider: dheera_ai.LlmProviders
    ) -> str:
        """
