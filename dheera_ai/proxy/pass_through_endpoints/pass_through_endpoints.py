...
from dheera_ai.llms.custom_httpx.http_handler import get_async_httpx_client
from dheera_ai.passthrough import BasePassthroughUtils
...
from dheera_ai.types.utils import StandardLoggingUserAPIKeyMetadata
...
async def set_env_variables_in_header(custom_headers: Optional[dict]) -> Optional[dict]:
    ...
    for key, value in custom_headers.items():
        # langfuse Api requires base64 encoded headers - it's simpleer to just ask dheera_ai users to set their langfuse public and secret keys
...
async def chat_completion_pass_through_endpoint(  # noqa: PLR0915
    ...
    try:
        ...
        if data["model"] in dheera_ai.model_alias_map:
            data["model"] = dheera_ai.model_alias_map[data["model"]]
        ...
        if "api_key" in data:
            llm_response = asyncio.create_task(dheera_ai.aadapter_completion(**data))
        elif (
            ...
            llm_response = asyncio.create_task(llm_router.aadapter_completion(**data))
        elif user_model is not None:  # `dheera_ai --model <your-model-name>`
            llm_response = asyncio.create_task(dheera_ai.aadapter_completion(**data))
        else:
            raise HTTPException(
...
class HttpPassThroughEndpointHelpers(BasePassthroughUtils):
    ...
    def get_response_headers(
        ...
        if dheera_ai_call_id:
            return_headers["x-dheera_ai-call-id"] = dheera_ai_call_id
        if custom_headers:
    ...
    def _init_kwargs_for_pass_through_endpoint(
        ...
        """
        Filter out dheera_ai params from the request body
        """
        from dheera_ai.types.utils import all_dheera_ai_params

        _parsed_body = _parsed_body or {}

        dheera_ai_params_in_body = {}
        for k in all_dheera_ai_params:
            if k in _parsed_body:
                dheera_ai_params_in_body[k] = _parsed_body.pop(k, None)
        ...
        dheera_ai_metadata = dheera_ai_params_in_body.pop("dheera_ai_metadata", None)
        metadata = dheera_ai_params_in_body.pop("metadata", None)
        if dheera_ai_metadata:
            _metadata.update(dheera_ai_metadata)
        if metadata:
        ...
        kwargs = {
            "dheera_ai_params": {
                **dheera_ai_params_in_body,  # type: ignore
                "metadata": _metadata,
...
async def pass_through_request(  # noqa: PLR0915
    ...
    from dheera_ai.proxy.pass_through_endpoints.passthrough_guardrails import (
    ...
    # parsed request body
    _parsed_body: Optional[dict] = None
    # kwargs for pass through endpoint, contains metadata, dheera_ai_params, call_type, dheera_ai_call_id, passthrough_logging_payload
    ...
    try:
        ...
        if _parsed_body is None:
            _parsed_body = {}
        _parsed_body["dheera_ai_logging_obj"] = logging_obj
        ...
        if custom_llm_provider:
            ...
            logging_obj.model_call_details["dheera_ai_params"] = kwargs.get("dheera_ai_params", {})
        ...
        logging_obj.update_environment_variables(
            ...
            user="unknown",
            optional_params={},
            dheera_ai_params=kwargs["dheera_ai_params"],
            call_type="pass_through_endpoint",
        )
        logging_obj.model_call_details["dheera_ai_call_id"] = dheera_ai_call_id
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.pass_through_endpoint(): Exception occured - {}".format(
                str(e)
            )
        )
        ...
        # add user_api_key_dict, dheera_ai_call_id, passthrough_logging_payloa for logging
        if kwargs:
...
def _update_metadata_with_tags_in_header(request: Request, metadata: dict) -> dict:
    ...
    Checks both 'tags' and 'x-dheera_ai-tags' headers
    """
    tags_to_add = []
    ...
    _tags = request.headers.get("x-dheera_ai-tags")
    if _tags:
        tags_to_add.extend([tag.strip() for tag in _tags.split(",")])
...
def create_pass_through_route(
    ...
    try:
        ...
        dheera_ai.adapters = [{"id": adapter_id, "adapter": adapter}]
...
def create_websocket_passthrough_route(
    ...
    """
    from dheera_ai.proxy.auth.user_api_key_auth import user_api_key_auth_websocket
...
async def websocket_passthrough_request(  # noqa: PLR0915
    ...
    logging_obj.update_environment_variables(
        ...
        user="unknown",
        optional_params={},
        dheera_ai_params=dict(kwargs.get("dheera_ai_params", {})),
        call_type="pass_through_endpoint",
    )
    logging_obj.model_call_details["dheera_ai_call_id"] = dheera_ai_call_id
    ...
    try:
        ...
        async with connect(
            ...
            #     success_kwargs.setdefault('dheera_ai_params', {})
            #     success_kwargs['dheera_ai_params'].update({
            #         'proxy_server_request': {
            #             'body': {
...
class InitPassThroughEndpointHelpers:
    ...
    def is_registered_pass_through_route(route: str) -> bool:
        ...
        for mapped_route in DheeraAIRoutes.mapped_pass_through_routes.value:
...
async def initialize_pass_through_endpoints(
    ...
    from dheera_ai.proxy._types import CommonProxyErrors, DheeraAIRoutes
    from dheera_ai.proxy.proxy_server import (
        app,
    ...
    for endpoint in combined_pass_through_endpoints:
        ...
        if _auth is not None and str(_auth).lower() == "true":
            ...
            DheeraAIRoutes.openai_routes.value.append(_path)
...
async def _get_pass_through_endpoints_from_db(
    ...
    from dheera_ai.proxy.proxy_server import get_config_general_settings
