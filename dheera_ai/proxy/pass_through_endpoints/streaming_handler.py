...
from dheera_ai.proxy._types import PassThroughEndpointLoggingResultValues
...
from dheera_ai.types.utils import StandardPassThroughResponseObject
...
class PassThroughStreamingHandler:
    ...
    async def chunk_processor(
        ...
        try:
            ...
            async for chunk in response.aiter_bytes():
                ...
                if (
                    getattr(dheera_ai, "include_cost_in_streaming_usage", False)
                    and model_name
                ):
    ...
    async def _route_streaming_logging_to_handler(
        ...
        if dheera_ai_logging_obj._should_run_sync_callbacks_for_async_calls() is False:
            return

        executor.submit(
            dheera_ai_logging_obj.success_handler,
            result=standard_logging_response_object,
            end_time=end_time,
            cache_hit=False,
            start_time=start_time,
    ...
    def _extract_model_for_cost_injection(
        ...
        if hasattr(dheera_ai_logging_obj, "model_call_details"):
            model = dheera_ai_logging_obj.model_call_details.get("model")
            if model:
                return model
