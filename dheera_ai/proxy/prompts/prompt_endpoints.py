...
from dheera_ai.types.proxy.prompt_endpoints import TestPromptRequest
...
def create_versioned_prompt_spec(db_prompt) -> PromptSpec:
    ...
    from dheera_ai.types.prompts.init_prompts import PromptDheeraAIParams
    
    prompt_dict = db_prompt.model_dump()
    base_prompt_id = prompt_dict["prompt_id"]
    version = prompt_dict.get("version", 1)
    
    # Parse dheera_ai_params
    dheera_ai_params_data = prompt_dict.get("dheera_ai_params")
    if isinstance(dheera_ai_params_data, str):
        dheera_ai_params_data = json.loads(dheera_ai_params_data)
...
class PatchPromptRequest(BaseModel):
    dheera_ai_params: Optional[PromptDheeraAIParams] = None
    prompt_info: Optional[PromptInfo] = None
            ...
            for prompt_id in prompts:
                if prompt_id in IN_MEMORY_PROMPT_REGISTRY.IN_MEMORY_PROMPTS:
                    ...
                    prompt_copy = PromptSpec(
                        ...
                        dheera_ai_params=original_prompt.dheera_ai_params,
...
async def get_prompt_versions(
    ...
    for prompt in prompt_versions:
        ...
        versioned_prompt = PromptSpec(
            ...
            dheera_ai_params=prompt.dheera_ai_params,
...
async def get_prompt_info(
    ...
    prompt_spec_response = PromptSpec(
        ...
        dheera_ai_params=prompt_spec.dheera_ai_params,  # This preserves the versioned ID
    ...
    try:
        ...
        if prompt_callback is not None:
            ...
            if integration_name == "dotprompt":
                ...
                from dheera_ai.integrations.dotprompt.dotprompt_manager import (
                    DotpromptManager,
                )
                ...
                if isinstance(prompt_callback, DotpromptManager):
                    ...
                    if template is not None and len(template) == 1:
                        ...
                        prompt_template = PromptTemplateBase(
                            dheera_ai_prompt_id=template_id,  # id sent to prompt management tool
    ...
    try:
        ...
        prompt_db_entry = await prisma_client.db.dheera_ai_prompttable.create(
            data={
                ...
                "version": new_version,
                "dheera_ai_params": request.dheera_ai_params.model_dump_json(),
                "prompt_info": (
...
async def patch_prompt(
    ...
    - dheera_ai_params: DheeraAI parameters for the prompt
    ...
    try:
        ...
        updated_dheera_ai_params = (
            request.dheera_ai_params
            if request.dheera_ai_params is not None
            else existing_prompt.dheera_ai_params
        )
        ...
        # Ensure we have valid dheera_ai_params
        if updated_dheera_ai_params is None:
...
async def test_prompt(
    ...
    from dheera_ai.integrations.dotprompt.dotprompt_manager import DotpromptManager
    from dheera_ai.integrations.dotprompt.prompt_manager import (
        PromptManager,
        PromptTemplate,
    )
