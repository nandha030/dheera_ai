...
from dheera_ai.constants import DEFAULT_NUM_WORKERS_DHEERA_AI_PROXY
...
config_filename = "dheera_ai.secrets"
...
class DheeraAIDatabaseConnectionPool(Enum):
...
class ProxyInitializationHelpers:
    @staticmethod
    def _echo_dheera_ai_version():
        pkg_version = importlib.metadata.version("dheera_ai")  # type: ignore
        click.echo(f"\nDheeraAI: Current Version = {pkg_version}\n")

    @staticmethod
    def _run_health_check(host, port):
        print("\nDheeraAI: Health Testing models in config")  # noqa
    ...
    def _run_test_chat_completion(
        ...
        click.echo(
            f"\nDheeraAI: Making a test ChatCompletions request to your proxy. Model={request_model}"
        )
        import openai
        ...
        click.echo(f"\nDheeraAI: response from proxy {response}")

        print(  # noqa
            f"\n DheeraAI: Making a test ChatCompletions + streaming r equest to proxy. Model={request_model}"
        )
        ...
        for chunk in stream_response:
            click.echo(f"DheeraAI: streaming response from proxy {chunk}")
    ...
    def _get_default_unvicorn_init_args(
        ...
        uvicorn_args = {
            "app": "dheera_ai.proxy.proxy_server:app",
            "host": host,
            "port": port,
        }
    ...
    def _init_hypercorn_server(
        ...
        """
        Initialize dheera_ai with `hypercorn`
        """
        import asyncio
        ...
        print(  # noqa
            f"\033[1;32mDheeraAI Proxy: Starting server on {host}:{port} using Hypercorn\033[0m\n"  # noqa
        )  # noqa
        config = Config()
        ...
        if ssl_certfile_path is not None and ssl_keyfile_path is not None:
            print(  # noqa
                f"\033[1;32mDheeraAI Proxy: Using SSL with certfile: {ssl_certfile_path} and keyfile: {ssl_keyfile_path}\033[0m\n"  # noqa
            )
    ...
    def _run_gunicorn_server(
        ...
        class StandaloneApplication(gunicorn.app.base.BaseApplication):
            def __init__(self, app, options=None):
                ...
                print()  # noqa
                print(  # noqa
                    '\033[1;34mDheeraAI: Test your local proxy with: "dheera_ai --test" This runs an openai.ChatCompletion request to your proxy [In a new terminal tab]\033[0m\n'
                )
                print(  # noqa
                    f"\033[1;34mDheeraAI: Curl Command Test for your local proxy\n {curl_command} \033[0m\n"
                )
                print(  # noqa
                    "\033[1;34mDocs: https://docs.dheeraai.com/docs/simple_proxy\033[0m\n"
                )  # noqa
                print(  # noqa
            ...
            def load_config(self):
                # note: This Loads the gunicorn config - has nothing to do with DheeraAI Proxy config
        ...
        print(  # noqa
            f"\033[1;32mDheeraAI Proxy: Starting server on {host}:{port} with {num_workers} workers\033[0m\n"  # noqa
        )
        gunicorn_options = {
            "bind": f"{host}:{port}",
...
@click.option(
    "--num_workers",
    default=DEFAULT_NUM_WORKERS_DHEERA_AI_PROXY,
...
@click.option(
    "--model", "-m", default=None, help="The model name to pass to dheera_ai expects"
)
@click.option(
    "--alias",
    default=None,
    help='The alias for the model - use this to give a dheera_ai model name (e.g. "huggingface/codellama/CodeLlama-7b-Instruct-hf") a more user-friendly name ("codellama")',
)
@click.option(
    "--add_key", default=None, help="The model name to pass to dheera_ai expects"
)
...
@click.option(
    ...
    "-c",
    default=None,
    help="Path to the proxy configuration file (e.g. config.yaml). Usage `dheera_ai --config config.yaml`",
)
@click.option(
    "--max_budget",
    default=None,
...
def run_server(  # noqa: PLR0915
    ...
    if local:
        ...
        try:
            ...
            raise ModuleNotFoundError(
                f"Missing dependency {e}. Run `pip install 'dheera_ai[proxy]'`"
            )
        except ImportError as e:
            if "dheera_ai[proxy]" in str(e):
                # user is missing a proxy dependency, ask them to pip install dheera_ai[proxy]
                raise e
            else:
                # this is just a local/relative import error, user git cloned dheera_ai
                from proxy_server import (
                    KeyManagementSettings,
                    ProxyConfig,
                    app,
    ...
    if version is True:
        ProxyInitializationHelpers._echo_dheera_ai_version()
        return
    ...
    if test is True:
        ...
        try:
            ...
            raise ImportError(
                "uvicorn, gunicorn needs to be imported. Run - `pip install 'dheera_ai[proxy]'`"
            )
        ...
        if iam_token_db_auth or get_secret_bool("IAM_TOKEN_DB_AUTH"):
            from dheera_ai.proxy.auth.rds_iam_token import generate_iam_auth_token
        ...
        from dheera_ai.secret_managers.aws_secret_manager import decrypt_env_var
        ...
        if config is not None:
            ...
            try:
                ...
                raise ImportError(
                    "yaml needs to be imported. Run - `pip install 'dheera_ai[proxy]'`"
                )
            ...
            ### LITELLM SETTINGS ###
            dheera_ai_settings = _config.get("dheera_ai_settings", None)
            if (
                dheera_ai_settings is not None
                and "json_logs" in dheera_ai_settings
                and dheera_ai_settings["json_logs"] is True
            ):
                import dheera_ai

                dheera_ai.json_logs = True

                dheera_ai._turn_on_json()
            ### GENERAL SETTINGS ###
            ...
            if database_url is None and os.getenv("DATABASE_URL") is None:
                ...
                from dheera_ai.proxy.utils import construct_database_url_from_env_vars
            ...
            db_connection_pool_limit = general_settings.get(
                ...
                DheeraAIDatabaseConnectionPool.database_connection_pool_limit.value,
            )
            db_connection_timeout = general_settings.get(
                "database_connection_pool_timeout",
                DheeraAIDatabaseConnectionPool.database_connection_pool_timeout.value,
            )
        ...
        if (
            ...
            if is_prisma_runnable:
                from dheera_ai.proxy.db.check_migration import check_prisma_schema_diff
                from dheera_ai.proxy.db.prisma_client import (
                    PrismaManager,
        ...
        from dheera_ai.proxy.proxy_server import app  # noqa

        # --- SEPARATE HEALTH APP LOGIC ---
        # To run the health app separately, use:
        #   uvicorn dheera_ai.proxy.health_app_factory:build_health_app --factory --host 0.0.0.0 --port=4001
        ...
        if skip_server_startup:
            print(  # noqa
                "DheeraAI: Setup complete. Skipping server startup as requested."
            )
            return
