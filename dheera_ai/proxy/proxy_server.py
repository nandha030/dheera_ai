...
from dheera_ai.constants import (
    ...
    DHEERA_AI_EMBEDDING_PROVIDERS_SUPPORTING_INPUT_ARRAY_OF_TOKENS,
    DHEERA_AI_SETTINGS_SAFE_DB_OVERRIDES,
)
...
from dheera_ai.utils import load_credentials_from_list
...
sys.path.insert(
    0, os.path.abspath("../..")
)  # Adds the parent directory to the system path - for dheera_ai local dev
...
try:
    ...
    raise ImportError(f"Missing dependency {e}. Run `pip install 'dheera_ai[proxy]'`")
...
from dheera_ai.exceptions import RejectedRequestError
from dheera_ai.integrations.custom_guardrail import ModifyResponseException
...
from dheera_ai.dheera_ai_core_utils.sensitive_data_masker import SensitiveDataMasker
...
from dheera_ai.proxy._experimental.mcp_server.discoverable_endpoints import (
...
from dheera_ai.proxy._experimental.mcp_server.server import app as mcp_app
from dheera_ai.proxy._experimental.mcp_server.tool_registry import (
    global_mcp_tool_registry,
)
from dheera_ai.proxy._types import *
from dheera_ai.proxy.agent_endpoints.a2a_endpoints import router as a2a_router
from dheera_ai.proxy.agent_endpoints.agent_registry import global_agent_registry
from dheera_ai.proxy.agent_endpoints.endpoints import router as agent_endpoints_router
from dheera_ai.proxy.analytics_endpoints.analytics_endpoints import (
    router as analytics_router,
)
from dheera_ai.proxy.anthropic_endpoints.endpoints import router as anthropic_router
from dheera_ai.proxy.anthropic_endpoints.skills_endpoints import (
    router as anthropic_skills_router,
)
from dheera_ai.proxy.auth.auth_checks import (
    ExperimentalUIJWTToken,
    get_team_object,
    log_db_metrics,
)
from dheera_ai.proxy.auth.auth_utils import check_response_size_is_safe
from dheera_ai.proxy.auth.handle_jwt import JWTHandler
from dheera_ai.proxy.auth.dheera_ai_license import LicenseCheck
from dheera_ai.proxy.auth.model_checks import (
    get_all_fallbacks,
...
from dheera_ai.proxy.batches_endpoints.endpoints import router as batches_router

## Import All Misc routes here ##
from dheera_ai.proxy.caching_routes import router as caching_router
from dheera_ai.proxy.common_request_processing import (
    ProxyBaseLLMRequestProcessing,
    create_streaming_response,
)
from dheera_ai.proxy.common_utils.callback_utils import initialize_callbacks_on_proxy
from dheera_ai.proxy.common_utils.debug_utils import init_verbose_loggers
from dheera_ai.proxy.common_utils.debug_utils import router as debugging_endpoints_router
from dheera_ai.proxy.common_utils.encrypt_decrypt_utils import (
    decrypt_value_helper,
    encrypt_value_helper,
)
from dheera_ai.proxy.common_utils.html_forms.ui_login import build_ui_login_form
from dheera_ai.proxy.common_utils.http_parsing_utils import (
    _read_request_body,
    check_file_size_under_limit,
    get_form_data,
)
from dheera_ai.proxy.common_utils.load_config_utils import (
...
from dheera_ai.proxy.common_utils.proxy_state import ProxyState
from dheera_ai.proxy.common_utils.reset_budget_job import ResetBudgetJob
from dheera_ai.proxy.common_utils.swagger_utils import ERROR_RESPONSES
from dheera_ai.proxy.container_endpoints.endpoints import router as container_router
from dheera_ai.proxy.credential_endpoints.endpoints import router as credential_router
from dheera_ai.proxy.db.db_transaction_queue.spend_log_cleanup import SpendLogCleanup
from dheera_ai.proxy.db.exception_handler import PrismaDBExceptionHandler
from dheera_ai.proxy.discovery_endpoints import ui_discovery_endpoints_router
from dheera_ai.proxy.fine_tuning_endpoints.endpoints import router as fine_tuning_router
from dheera_ai.proxy.fine_tuning_endpoints.endpoints import set_fine_tuning_config
from dheera_ai.proxy.google_endpoints.endpoints import router as google_router
from dheera_ai.proxy.guardrails.guardrail_endpoints import router as guardrails_router
from dheera_ai.proxy.guardrails.init_guardrails import (
    init_guardrails_v2,
    initialize_guardrails,
)
from dheera_ai.proxy.health_check import perform_health_check
from dheera_ai.proxy.health_endpoints._health_endpoints import router as health_router
...
from dheera_ai.proxy.image_endpoints.endpoints import router as image_router
from dheera_ai.proxy.dheera_ai_pre_call_utils import add_dheera_ai_data_to_request
from dheera_ai.proxy.management_endpoints.budget_management_endpoints import (
    router as budget_management_router,
)
from dheera_ai.proxy.management_endpoints.cache_settings_endpoints import (
    router as cache_settings_router,
)
from dheera_ai.proxy.management_endpoints.callback_management_endpoints import (
    router as callback_management_endpoints_router,
)
from dheera_ai.proxy.management_endpoints.common_utils import _user_has_admin_view
from dheera_ai.proxy.management_endpoints.cost_tracking_settings import (
...
from dheera_ai.proxy.management_endpoints.model_access_group_management_endpoints import (
...
from dheera_ai.proxy.management_endpoints.model_management_endpoints import (
    ...
    _deduplicate_dheera_ai_router_models,
)
from dheera_ai.proxy.management_endpoints.model_management_endpoints import (
    router as model_management_router,
)
from dheera_ai.proxy.management_endpoints.organization_endpoints import (
    router as organization_router,
)
from dheera_ai.proxy.management_endpoints.router_settings_endpoints import (
    router as router_settings_router,
)
from dheera_ai.proxy.management_endpoints.scim.scim_v2 import scim_router
from dheera_ai.proxy.management_endpoints.tag_management_endpoints import (
    router as tag_management_router,
)
from dheera_ai.proxy.management_endpoints.team_callback_endpoints import (
    router as team_callback_router,
)
from dheera_ai.proxy.management_endpoints.team_endpoints import router as team_router
...
from dheera_ai.proxy.management_endpoints.ui_sso import router as ui_sso_router
from dheera_ai.proxy.management_endpoints.user_agent_analytics_endpoints import (
...
from dheera_ai.proxy.middleware.prometheus_auth_middleware import PrometheusAuthMiddleware
from dheera_ai.proxy.ocr_endpoints.endpoints import router as ocr_router
from dheera_ai.proxy.openai_files_endpoints.files_endpoints import (
...
from dheera_ai.proxy.prompts.prompt_endpoints import router as prompts_router
from dheera_ai.proxy.public_endpoints import router as public_endpoints_router
from dheera_ai.proxy.rag_endpoints.endpoints import router as rag_router
from dheera_ai.proxy.rerank_endpoints.endpoints import router as rerank_router
from dheera_ai.proxy.response_api_endpoints.endpoints import router as response_router
from dheera_ai.proxy.route_llm_request import route_request
from dheera_ai.proxy.search_endpoints.endpoints import router as search_router
from dheera_ai.proxy.search_endpoints.search_tool_management import (
    router as search_tool_management_router,
)
from dheera_ai.proxy.spend_tracking.cloudzero_endpoints import router as cloudzero_router
from dheera_ai.proxy.spend_tracking.spend_management_endpoints import (
    router as spend_management_router,
)
from dheera_ai.proxy.spend_tracking.spend_tracking_utils import get_logging_payload
from dheera_ai.proxy.types_utils.utils import get_instance_fn
from dheera_ai.proxy.ui_crud_endpoints.proxy_setting_endpoints import (
    router as ui_crud_endpoints_router,
)
from dheera_ai.proxy.utils import (
    PrismaClient,
    ProxyLogging,
    ProxyUpdateSpend,
    _cache_user_row,
...
from dheera_ai.proxy.vector_store_endpoints.endpoints import router as vector_store_router
from dheera_ai.proxy.vector_store_endpoints.management_endpoints import (
    router as vector_store_management_router,
)
from dheera_ai.proxy.vector_store_files_endpoints.endpoints import (
    router as vector_store_files_router,
)
from dheera_ai.proxy.vertex_ai_endpoints.langfuse_endpoints import (
    router as langfuse_router,
)
from dheera_ai.proxy.video_endpoints.endpoints import router as video_router
...
from dheera_ai.scheduler import FlowItem, Scheduler
from dheera_ai.secret_managers.aws_secret_manager import load_aws_kms
from dheera_ai.secret_managers.google_kms import load_google_kms
from dheera_ai.secret_managers.main import (
    get_secret,
    get_secret_bool,
    get_secret_str,
    str_to_bool,
)
from dheera_ai.types.integrations.slack_alerting import SlackAlertingArgs
...
from dheera_ai.types.proxy.management_endpoints.model_management_endpoints import (
    ModelGroupInfoProxy,
)
...
from dheera_ai.types.router import ModelInfo as RouterModelInfo
from dheera_ai.types.router import (
    RouterGeneralSettings,
    SearchToolTypedDict,
    updateDeployment,
)
from dheera_ai.types.scheduler import DefaultPriorities
from dheera_ai.types.secret_managers.main import (
    KeyManagementSettings,
    KeyManagementSystem,
)
from dheera_ai.types.utils import CredentialItem, CustomHuggingfaceTokenizer
from dheera_ai.types.utils import ModelInfo as ModelMapInfo
from dheera_ai.types.utils import RawRequestTypedDict, StandardLoggingPayload
from dheera_ai.utils import _add_custom_logger_callback_to_specific_event

try:
    from dheera_ai._version import version
except Exception:
    version = "0.0.0"
...
from dheera_ai.types.agents import AgentConfig

# import enterprise folder
enterprise_router = APIRouter()
try:
    # when using dheera_ai cli
    import dheera_ai.proxy.enterprise as enterprise
except Exception:
    # when using dheera_ai docker image
    try:
...
try:
    from dheera_ai_enterprise.proxy.enterprise_routes import router as _enterprise_router
    from dheera_ai_enterprise.proxy.proxy_server import EnterpriseProxyConfig
...
global_max_parallel_request_retries_env: Optional[str] = os.getenv(
    "DHEERA_AI_GLOBAL_MAX_PARALLEL_REQUEST_RETRIES"
)
...
global_max_parallel_request_retry_timeout_env: Optional[str] = os.getenv(
    "DHEERA_AI_GLOBAL_MAX_PARALLEL_REQUEST_RETRY_TIMEOUT"
)
...
ui_message = (
    f"👉 [```DheeraAI Admin Panel on /ui```]({ui_link}). Create, Edit Keys with SSO. Having issues? Try [```Fallback Login```]({fallback_login_link})"
)
...
async def proxy_shutdown_event():
    ...
    verbose_proxy_logger.info("Shutting down DheeraAI Proxy Server")
    if prisma_client:
        verbose_proxy_logger.debug("Disconnecting from Prisma")
        await prisma_client.disconnect()

    if dheera_ai.cache is not None:
        await dheera_ai.cache.disconnect()
    ...
    if "langfuse" in dheera_ai.success_callback:
        try:
            # flush langfuse logs on shutdow
            from dheera_ai.utils import langFuseLogger
...
async def proxy_startup_event(app: FastAPI):
    global prisma_client, master_key, use_background_health_checks, llm_router, llm_model_list, general_settings, proxy_budget_rescheduler_min_time, proxy_budget_rescheduler_max_time, dheera_ai_proxy_admin_name, db_writer_client, store_model_in_db, premium_user, _license_check, proxy_batch_polling_interval, shared_aiohttp_session
    import json

    init_verbose_loggers()
    ## CHECK PREMIUM USER
    verbose_proxy_logger.debug(
        "dheera_ai.proxy.proxy_server.py::startup() - CHECKING PREMIUM USER - {}".format(
            premium_user
        )
    )
    ...
    master_key = get_secret_str("DHEERA_AI_MASTER_KEY")
    ### LOAD CONFIG ###
    ...
    if env_config_yaml is not None:
        ...
        elif os.environ.get("DHEERA_AI_CONFIG_BUCKET_NAME") is not None and isinstance(
            worker_config, str
        ):
            (
                llm_router,
    ...
    if prisma_client is not None and dheera_ai.max_budget > 0:
        ProxyStartupEvent._add_proxy_budget_to_db(
            dheera_ai_proxy_budget_name=dheera_ai_proxy_admin_name
        )
...
def get_openapi_schema():
    ...
    from dheera_ai.proxy.common_utils.openapi_schema_compat import (
    ...
    from dheera_ai.proxy.common_utils.custom_openapi_spec import CustomOpenAPISpec
...
def custom_openapi():
    ...
    openai_routes = DheeraAIRoutes.openai_routes.value
...
try:
    ...
    dheera_ai_asset_prefix = "/dheera_ai-asset-prefix"
    ...
    is_non_root = os.getenv("DHEERA_AI_NON_ROOT", "").lower() == "true"
    runtime_ui_path = "/tmp/dheera_ai_ui"
            ...
            for filename in files:
                ...
                try:
                    ...
                    modified_content = content.replace(
                        f"{dheera_ai_asset_prefix}",
                        f"{server_root_path}",
                    )

                    # Replace the /.well-known/dheera_ai-ui-config with the server root path
                    modified_content = modified_content.replace(
                        "/dheera_ai/.well-known/dheera_ai-ui-config",
                        f"{server_root_path}/.well-known/dheera_ai-ui-config",
                    )
    ...
    app.mount(
        f"{dheera_ai_asset_prefix}/_next",
        StaticFiles(directory=os.path.join(ui_path, "_next")),
        name="next_static",
    )
    ...
    def _restructure_ui_html_files(ui_root: str) -> None:
        ...
        for current_root, _, files in os.walk(ui_root):
            ...
            if first_segment in {"_next", "dheera_ai-asset-prefix"}:
                continue
...
dheera_ai.logging_callback_manager.add_dheera_ai_callback(model_max_budget_limiter)
...
queue: List = []
dheera_ai_proxy_budget_name = "dheera_ai-proxy-budget"
dheera_ai_proxy_admin_name = DHEERA_AI_PROXY_ADMIN_NAME
ui_access_mode: Union[Literal["admin", "all"], Dict] = "all"
proxy_budget_rescheduler_min_time = PROXY_BUDGET_RESCHEDULER_MIN_TIME
proxy_budget_rescheduler_max_time = PROXY_BUDGET_RESCHEDULER_MAX_TIME
proxy_batch_polling_interval = PROXY_BATCH_POLLING_INTERVAL
proxy_batch_write_at = PROXY_BATCH_WRITE_AT
dheera_ai_master_key_hash = None
...
async def check_request_disconnection(request: Request, llm_api_call_task):
    ...
    - cancel the dheera_ai.router task
...
def load_from_azure_key_vault(use_azure_key_vault: bool = False):
    ...
    try:
        ...
        dheera_ai.secret_manager_client = client
        dheera_ai._key_management_system = KeyManagementSystem.AZURE_KEY_VAULT
    except Exception as e:
        _error_str = str(e)
        verbose_proxy_logger.exception(
...
def cost_tracking():
    ...
    if prisma_client is not None:
        dheera_ai.logging_callback_manager.add_dheera_ai_callback(_ProxyDBLogger())
...
async def update_cache(  # noqa: PLR0915
    ...
    async def _update_key_cache(token: str, response_cost: float):
        ...
        existing_spend_obj: DheeraAI_VerificationTokenView = await user_api_key_cache.async_get_cache(key=hashed_token)  # type: ignore
        verbose_proxy_logger.debug(
        ...
        if (
            ...
            and existing_spend_obj.dheera_ai_budget_table is not None
            and (
                _is_projected_spend_over_limit(
                    current_spend=new_spend,
                    soft_budget_limit=existing_spend_obj.dheera_ai_budget_table[
                        "soft_budget"
                    ],
                )
                is True
            ...
            projected_spend, projected_exceeded_date = _get_projected_spend_over_limit(
                current_spend=new_spend,
                soft_budget_limit=existing_spend_obj.dheera_ai_budget_table.get(
                    "soft_budget", None
                ),
            )  # type: ignore
            soft_limit = existing_spend_obj.dheera_ai_budget_table.get(
    ...
    async def _update_user_cache():
        ...
        try:
            ...
            global_proxy_spend = await user_api_key_cache.async_get_cache(
                key="{}:spend".format(dheera_ai_proxy_admin_name)
            )
            if global_proxy_spend is None:
                ...
                values_to_update_in_cache.append(
                    ("{}:spend".format(dheera_ai_proxy_admin_name), increment)
                )
        except Exception as e:
            verbose_proxy_logger.debug(
    ...
    async def _update_end_user_cache():
        ...
        try:
            ...
            existing_spend_obj = await user_api_key_cache.async_get_cache(key=_id)
            if existing_spend_obj is None:
                # if user does not exist in DheeraAI_UserTable, create a new user
    ...
    async def _update_team_cache():
        ...
        try:
            # Fetch the existing cost for the given user
            existing_spend_obj: Optional[DheeraAI_TeamTable] = (
    ...
    asyncio.create_task(
        user_api_key_cache.async_set_cache_pipeline(
            ...
            ttl=60,
            dheera_ai_parent_otel_span=parent_otel_span,
        )
    )
...
def run_ollama_serve():
    try:
        ...
        verbose_proxy_logger.debug(
            f"""
            DheeraAI Warning: proxy started with `ollama` model\n`ollama serve` failed with Exception{e}. \nEnsure you run `ollama serve`
        """
        )
...
async def _run_background_health_check():
    ...
    if use_shared_health_check and redis_usage_cache is not None:
        from dheera_ai.proxy.health_check_utils.shared_health_check_manager import (
    ...
    while True:
        ...
        if prisma_client is not None:
            ...
            from dheera_ai.proxy.health_endpoints._health_endpoints import (
...
class ProxyConfig:
    ...
    async def save_config(self, new_config: dict):
        ...
        - if api_key is passed, save that to the local environment or connected secret manage (maybe expose `dheera_ai.save_secret()`)
        """
    ...
    def load_team_config(self, team_id: str):
        ...
        ## LITELLM MODULE SETTINGS (e.g. dheera_ai.drop_params=True,..)
        dheera_ai_settings = config.get("dheera_ai_settings", {})
        all_teams_config = dheera_ai_settings.get("default_team_settings", None)
    ...
    def _init_cache(
        ...
        if "default_in_memory_ttl" in cache_params:
            dheera_ai.default_in_memory_ttl = cache_params["default_in_memory_ttl"]

        if "default_redis_ttl" in cache_params:
            dheera_ai.default_redis_ttl = cache_params["default_redis_ttl"]

        dheera_ai.cache = Cache(**cache_params)

        if dheera_ai.cache is not None and isinstance(
            dheera_ai.cache.cache, (RedisCache, RedisClusterCache)
        ):
            ## INIT PROXY REDIS USAGE CLIENT ##
            redis_usage_cache = dheera_ai.cache.cache
    ...
    def switch_on_llm_response_caching(self):
        ...
        Router passes caching=self.cache_responses to dheera_ai.completion()
        """
        global llm_router
        import dheera_ai
    ...
    async def get_config(self, config_file_path: Optional[str] = None) -> dict:
        ...
        if os.environ.get("DHEERA_AI_CONFIG_BUCKET_NAME") is not None:
            bucket_name = os.environ.get("DHEERA_AI_CONFIG_BUCKET_NAME")
            object_key = os.environ.get("DHEERA_AI_CONFIG_BUCKET_OBJECT_KEY")
            bucket_type = os.environ.get("DHEERA_AI_CONFIG_BUCKET_TYPE")
            verbose_proxy_logger.debug(
    ...
    def parse_search_tools(self, config: dict) -> Optional[List[SearchToolTypedDict]]:
        ...
        print(  # noqa
            "\033[32mDheeraAI: Proxy initialized with Search Tools:\033[0m"
        )  # noqa

        for search_tool in search_tools_raw:
            # Display loaded search tool
            search_tool_name = search_tool.get("search_tool_name", "")
            search_provider = search_tool.get("dheera_ai_params", {}).get(
                "search_provider", ""
            )
            print(f"\033[32m    {search_tool_name} ({search_provider})\033[0m")  # noqa

            # Handle os.environ/ variables in dheera_ai_params
            dheera_ai_params = search_tool.get("dheera_ai_params", {})
            if dheera_ai_params:
                for k, v in dheera_ai_params.items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        _v = v.replace("os.environ/", "")
                        v = get_secret(_v)
                        dheera_ai_params[k] = v
                search_tool["dheera_ai_params"] = dheera_ai_params
    ...
    def _load_environment_variables(self, config: dict):
        ...
        if environment_variables:
            ...
            # check if dheera_ai_license in general_settings
            if "DHEERA_AI_LICENSE" in environment_variables:
                _license_check.license_str = os.getenv("DHEERA_AI_LICENSE", None)
                premium_user = _license_check.is_premium()
        return

    async def load_config(  # noqa: PLR0915
        self, router: Optional[dheera_ai.Router], config_file_path: str
    ):
        """
        Load config values into proxy global state
        """
        global master_key, user_config_file_path, otel_logging, user_custom_auth, user_custom_auth_path, user_custom_key_generate, user_custom_sso, user_custom_ui_sso_sign_in_handler, use_background_health_checks, use_shared_health_check, health_check_interval, use_queue, proxy_budget_rescheduler_max_time, proxy_budget_rescheduler_min_time, ui_access_mode, dheera_ai_master_key_hash, proxy_batch_write_at, disable_spend_logs, prompt_injection_detection_obj, redis_usage_cache, store_model_in_db, premium_user, open_telemetry_logger, health_check_details, proxy_batch_polling_interval, config_passthrough_endpoints
        ...
        if callback_settings:
            dheera_ai.callback_settings = callback_settings

        ## LITELLM MODULE SETTINGS (e.g. dheera_ai.drop_params=True,..)
        dheera_ai_settings = config.get("dheera_ai_settings", None)
        if dheera_ai_settings is None:
            dheera_ai_settings = {}
        if dheera_ai_settings:
            # ANSI escape code for blue text
            blue_color_code = "\033[94m"
            reset_color_code = "\033[0m"
            for key, value in dheera_ai_settings.items():
                if key == "cache" and value is True:
                    print(f"{blue_color_code}\nSetting Cache on Proxy")  # noqa
                    from dheera_ai.caching.caching import Cache

                    cache_params = {}
                    if "cache_params" in dheera_ai_settings:
                        cache_params_in_config = dheera_ai_settings["cache_params"]
                    ...
                    if dheera_ai.cache is not None:
                        verbose_proxy_logger.debug(
                            f"{blue_color_code}Set Cache on DheeraAI Proxy{reset_color_code}"
                        )
                    ...
                    dheera_ai.guardrail_name_config_map = guardrail_name_config_map

                elif key == "global_prompt_directory":
                    from dheera_ai.integrations.dotprompt import (
                        set_global_prompt_directory,
                    )

                    set_global_prompt_directory(value)
                    verbose_proxy_logger.info(
                        f"{blue_color_code}Set Global Prompt Directory on DheeraAI Proxy{reset_color_code}"
                    )
                elif key == "global_bitbucket_config":
                    from dheera_ai.integrations.bitbucket import (
                        set_global_bitbucket_config,
                    )

                    set_global_bitbucket_config(value)
                    verbose_proxy_logger.info(
                        f"{blue_color_code}Set Global BitBucket Config on DheeraAI Proxy{reset_color_code}"
                    )
                elif key == "global_gitlab_config":
                    from dheera_ai.integrations.gitlab import set_global_gitlab_config

                    set_global_gitlab_config(value)
                    verbose_proxy_logger.info(
                        f"{blue_color_code}Set Global Gitlab Config on DheeraAI Proxy{reset_color_code}"
                    )
                elif key == "priority_reservation_settings":
                    from dheera_ai.types.utils import PriorityReservationSettings

                    dheera_ai.priority_reservation_settings = PriorityReservationSettings(
                        **value
                    )
                elif key == "callbacks":
                    ...
                    from dheera_ai.types.router import ModelGroupSettings

                    dheera_ai.model_group_settings = ModelGroupSettings(**value)
                    ...
                    verbose_proxy_logger.debug(
                        f"dheera_ai.post_call_rules: {dheera_ai.post_call_rules}"
                    )
                elif key == "max_internal_user_budget":
                    dheera_ai.max_internal_user_budget = float(value)  # type: ignore
                elif key == "default_max_internal_user_budget":
                    dheera_ai.default_max_internal_user_budget = float(value)
                    if dheera_ai.max_internal_user_budget is None:
                        dheera_ai.max_internal_user_budget = (
                            dheera_ai.default_max_internal_user_budget
                        )
                    ...
                    for callback in value:
                        ...
                        # these are dheera_ai callbacks - "langfuse", "sentry", "wandb"
                        else:
                            dheera_ai.logging_callback_manager.add_dheera_ai_success_callback(
                                callback
                            )
                            if "prometheus" in callback:
                                from dheera_ai.integrations.prometheus import (
                                    PrometheusLogger,
                                )
                    ...
                    print(  # noqa
                        f"{blue_color_code} Initialized Success Callbacks - {dheera_ai.success_callback} {reset_color_code}"
                    )  # noqa
                    ...
                    print(  # noqa
                        f"{blue_color_code} Initialized Failure Callbacks - {dheera_ai.failure_callback} {reset_color_code}"
                    )  # noqa
                    ...
                    verbose_proxy_logger.debug(
                        f"{blue_color_code} setting dheera_ai.{key}={value}{reset_color_code}"
                    )
                    setattr(dheera_ai, key, value)
                    ...
                    if value is not None and isinstance(value, dict):
                        ...
                        dheera_ai.upperbound_key_generate_params = (
                            DheeraAI_UpperboundKeyGenerateParams(**value)
                        )
                    else:
                        raise Exception(
        ...
        ## GENERAL SERVER SETTINGS (e.g. master key,..) # do this after initializing dheera_ai, to ensure sentry logging works for proxylogging
        ...
        if general_settings:
            ...
            master_key = general_settings.get(
                "master_key", get_secret("DHEERA_AI_MASTER_KEY", None)
            )
            ...
            if master_key is not None and isinstance(master_key, str):
                dheera_ai_master_key_hash = hash_token(master_key)
            ...
            if allowed_ips is not None and premium_user is False:
                raise ValueError(
                    "allowed_ips is an Enterprise Feature. Please add a valid DHEERA_AI_LICENSE to your envionment."
                )
            ## BUDGET RESCHEDULER ##
            ...
            # check if dheera_ai_license in general_settings
            if "dheera_ai_license" in general_settings:
                _license_check.license_str = general_settings["dheera_ai_license"]
                premium_user = _license_check.is_premium()

        router_params: dict = {
            "cache_responses": dheera_ai.cache
        ...
        if model_list:
            ...
            print(  # noqa
                "\033[32mDheeraAI: Proxy initialized with Config, Set models:\033[0m"
            )  # noqa
            for model in model_list:
                ### LOAD FROM os.environ/ ###
                for k, v in model["dheera_ai_params"].items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        model["dheera_ai_params"][k] = get_secret(v)
                print(f"\033[32m    {model.get('model_name', '')}\033[0m")  # noqa
                dheera_ai_model_name = model["dheera_ai_params"]["model"]
                dheera_ai_model_api_base = model["dheera_ai_params"].get("api_base", None)
                if "ollama" in dheera_ai_model_name and dheera_ai_model_api_base is None:
                    run_ollama_serve()
        ...
        if assistant_settings:
            for k, v in assistant_settings["dheera_ai_params"].items():
                if isinstance(v, str) and v.startswith("os.environ/"):
                    _v = v.replace("os.environ/", "")
                    v = os.getenv(_v)
                    assistant_settings["dheera_ai_params"][k] = v
        ...
        if router_settings and isinstance(router_settings, dict):
            arg_spec = inspect.getfullargspec(dheera_ai.Router)
        ...
        if prompts:
            from dheera_ai.proxy.prompts.init_prompts import init_prompts
        ...
        dheera_ai.credential_list = credential_list_dict
    ...
    async def _init_non_llm_configs(self, config: dict):
        ...
        if mcp_servers_config:
            ...
            # Get mcp_aliases from dheera_ai_settings if available
            dheera_ai_settings = config.get("dheera_ai_settings", {})
            mcp_aliases = dheera_ai_settings.get("mcp_aliases", None)
        ...
        if vector_store_registry_config:
            ...
            dheera_ai.vector_store_registry.load_vector_stores_from_config(
    ...
    def _load_alerting_settings(self, general_settings: dict):
        ...
        for _alert in _alerting_callbacks:
            if _alert == "slack":
                ...
                if _alert in dheera_ai._known_custom_logger_compatible_callbacks:
                    ...
                    if _logger is not None:
                        dheera_ai.logging_callback_manager.add_dheera_ai_callback(_logger)
        pass
    ...
    def initialize_secret_manager(
        ...
        if key_management_system is not None:
            if key_management_system == KeyManagementSystem.AZURE_KEY_VAULT.value:
                ...
                from dheera_ai.secret_managers.aws_secret_manager_v2 import (
                    AWSSecretsManagerV2,
                )

                AWSSecretsManagerV2.load_aws_secret_manager(
                    use_aws_secret_manager=True,
                    key_management_settings=dheera_ai._key_management_settings,
                )
                ...
                from dheera_ai.secret_managers.google_secret_manager import (
                    GoogleSecretManager,
                )

                GoogleSecretManager()
            elif key_management_system == KeyManagementSystem.HASHICORP_VAULT.value:
                from dheera_ai.secret_managers.hashicorp_secret_manager import (
                    HashicorpSecretManager,
                )

                HashicorpSecretManager()
            elif key_management_system == KeyManagementSystem.CYBERARK.value:
                from dheera_ai.secret_managers.cyberark_secret_manager import (
                ...
                from dheera_ai.secret_managers.custom_secret_manager_loader import (
    ...
    def get_model_info_with_id(self, model, db_model=False) -> RouterModelInfo:
        ...
        if premium_user is True:
            # seeing "created_at", "updated_at", "created_by", "updated_by" is a DheeraAI Enterprise Feature
            ...
            for model in model_list:
                ...
                if model_id is None:
                    ...
                    model_id = llm_router._generate_model_id(
                        ...
                        dheera_ai_params=model["dheera_ai_params"],
                    )
                else:
    ...
    def _add_deployment(self, db_models: list) -> int:
        ...
        ## ADD MODEL LOGIC
        for m in db_models:
            _dheera_ai_params = m.dheera_ai_params
            if isinstance(_dheera_ai_params, dict):
                # decrypt values
                for k, v in _dheera_ai_params.items():
                    if isinstance(v, str):
                        ...
                        _dheera_ai_params[k] = _value
                _dheera_ai_params = DheeraAI_Params(**_dheera_ai_params)

            else:
                verbose_proxy_logger.error(
                    f"Invalid model added to proxy db. Invalid dheera_ai params. dheera_ai_params={_dheera_ai_params}"
                )
            ...
            added = llm_router.upsert_deployment(
                deployment=Deployment(
                    ...
                    dheera_ai_params=_dheera_ai_params,
    ...
    def decrypt_model_list_from_db(self, new_models: list) -> list:
        ...
        for m in new_models:
            _dheera_ai_params = m.dheera_ai_params
            if isinstance(_dheera_ai_params, BaseModel):
                _dheera_ai_params = _dheera_ai_params.model_dump()
            if isinstance(_dheera_ai_params, dict):
                # decrypt values
                for k, v in _dheera_ai_params.items():
                    ...
                    _dheera_ai_params[k] = decrypted_value
                _dheera_ai_params = DheeraAI_Params(**_dheera_ai_params)
            else:
                verbose_proxy_logger.error(
                    f"Invalid model added to proxy db. Invalid dheera_ai params. dheera_ai_params={_dheera_ai_params}"
    ...
    def _add_callback_from_db_to_in_memory_dheera_ai_callbacks(
        self,
        callback: str,
        ...
        """
        Helper method to add a single callback to dheera_ai for specified event types.
        ...
        """
        if callback in dheera_ai._known_custom_logger_compatible_callbacks:
            ...
            if event_types == ["success"]:
                dheera_ai.logging_callback_manager.add_dheera_ai_success_callback(callback)
            elif event_types == ["failure"]:
                dheera_ai.logging_callback_manager.add_dheera_ai_failure_callback(callback)
    ...
    def _add_callbacks_from_db_config(self, config_data: dict) -> None:
        """
        Adds callbacks from DB config to dheera_ai
        """
        dheera_ai_settings = config_data.get("dheera_ai_settings", {}) or {}
        success_callbacks = dheera_ai_settings.get("success_callback", None)
        failure_callbacks = dheera_ai_settings.get("failure_callback", None)
        callbacks = dheera_ai_settings.get("callbacks", None)

        if success_callbacks is not None and isinstance(success_callbacks, list):
            for success_callback in success_callbacks:
                self._add_callback_from_db_to_in_memory_dheera_ai_callbacks(
                    callback=success_callback,
                    event_types=["success"],
                    existing_callbacks=dheera_ai.success_callback,
                )
        ...
        if failure_callbacks is not None and isinstance(failure_callbacks, list):
            for failure_callback in failure_callbacks:
                self._add_callback_from_db_to_in_memory_dheera_ai_callbacks(
                    ...
                    event_types=["failure"],
                    existing_callbacks=dheera_ai.failure_callback,
                )
        ...
        if callbacks is not None and isinstance(callbacks, list):
            for callback in callbacks:
                self._add_callback_from_db_to_in_memory_dheera_ai_callbacks(
                    ...
                    existing_callbacks=dheera_ai.callbacks,
                )
    ...
    async def _add_router_settings_from_db_config(
        ...
        """
        Adds router settings from DB config to dheera_ai proxy
        ...
        if llm_router is not None and prisma_client is not None:
            ...
            if (
                ...
                from dheera_ai.utils import _update_dictionary
    ...
    def _add_general_settings_from_db_config(
        ...
        """
        Adds general settings from DB config to dheera_ai proxy
    ...
    def _update_config_fields(
        ...
        elif param_name == "dheera_ai_settings" and isinstance(db_param_value, dict):
            for key, value in db_param_value.items():
                if (
                    key in DHEERA_AI_SETTINGS_SAFE_DB_OVERRIDES
    ...
    async def add_deployment(
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "dheera_ai.proxy.proxy_server.py::ProxyConfig:add_deployment - {}".format(
                    str(e)
                )
            )
    ...
    async def _init_sso_settings_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "dheera_ai.proxy.proxy_server.py::ProxyConfig:_init_sso_settings_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def _check_and_reload_model_cost_map(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            if should_reload:
                # Perform the reload
                from dheera_ai.dheera_ai_core_utils.get_model_cost_map import (
                    get_model_cost_map,
                )

                model_cost_map_url = dheera_ai.model_cost_map_url
                new_model_cost_map = get_model_cost_map(url=model_cost_map_url)
                dheera_ai.model_cost = new_model_cost_map
    ...
    def _get_prompt_spec_for_db_prompt(self, db_prompt):
        ...
        """
        from dheera_ai.proxy.prompts.prompt_endpoints import create_versioned_prompt_spec

        return create_versioned_prompt_spec(db_prompt=db_prompt)

    async def _init_prompts_in_db(self, prisma_client: PrismaClient):
        from dheera_ai.proxy.prompts.prompt_registry import IN_MEMORY_PROMPT_REGISTRY
        ...
        try:
            ...
            verbose_proxy_logger.debug(
                "dheera_ai.proxy.proxy_server.py::ProxyConfig:_init_prompts_in_db - {}".format(
                    str(e)
                )
            )

    async def _init_guardrails_in_db(self, prisma_client: PrismaClient):
        from dheera_ai.proxy.guardrails.guardrail_registry import (
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "dheera_ai.proxy.proxy_server.py::ProxyConfig:_init_guardrails_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def _init_vector_stores_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "dheera_ai.proxy.proxy_server.py::ProxyConfig:_init_vector_stores_in_db - {}".format(
                    str(e)
                )
            )

    async def _init_vector_store_indexes_in_db(self, prisma_client: PrismaClient):
        from dheera_ai.vector_stores.vector_store_registry import VectorStoreIndexRegistry
        ...
        try:
            ...
            if dheera_ai.vector_store_index_registry is None:
                dheera_ai.vector_store_index_registry = VectorStoreIndexRegistry(
                    vector_store_indexes=vector_store_indexes
                )
            else:
                for vector_store_index in vector_store_indexes:
                    dheera_ai.vector_store_index_registry.upsert_vector_store_index(
    ...
    async def _init_mcp_servers_in_db(self):
        from dheera_ai.proxy._experimental.mcp_server.utils import is_mcp_available
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "dheera_ai.proxy.proxy_server.py::ProxyConfig:_init_mcp_servers_in_db - {}".format(
                    str(e)
                )
            )

    async def _init_agents_in_db(self, prisma_client: PrismaClient):
        from dheera_ai.proxy.agent_endpoints.agent_registry import (
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "dheera_ai.proxy.proxy_server.py::ProxyConfig:_init_agents_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def _init_search_tools_in_db(self, prisma_client: PrismaClient):
        ...
        from dheera_ai.proxy.search_endpoints.search_tool_registry import (
            SearchToolRegistry,
        )
        from dheera_ai.router_utils.search_api_router import SearchAPIRouter
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "dheera_ai.proxy.proxy_server.py::ProxyConfig:_init_search_tools_in_db - {}".format(
                    str(e)
                )
            )
    ...
    async def delete_credentials(self, db_credentials: List[CredentialItem]):
        ...
        Compare to the dheera_ai.credential_list
        Delete any from dheera_ai.credential_list that are not in the all-up list
        """
        ## CONFIG credentials ##
        config = await self.get_config(config_file_path=user_config_file_path)
    ...
    async def get_credentials(self, prisma_client: PrismaClient):
        try:
            ...
            verbose_proxy_logger.exception(
                "dheera_ai.proxy_server.py::get_credentials() - Error getting credentials from DB - {}".format(
                    str(e)
                )
            )
            return []
...
async def initialize(  # noqa: PLR0915
    ...
    from dheera_ai.proxy.common_utils.banner import show_banner

    show_banner()
    if os.getenv("DHEERA_AI_DONT_SHOW_FEEDBACK_BOX", "").lower() != "true":
    ...
    if debug is True:  # this needs to be first, so users can see Router init debugg
        ...
        from dheera_ai._logging import (
            verbose_logger,
    ...
    if detailed_debug is True:
        ...
        # users can control proxy debugging using env variable = 'DHEERA_AI_LOG'
        dheera_ai_log_setting = os.environ.get("DHEERA_AI_LOG", "")
        if dheera_ai_log_setting is not None:
            if dheera_ai_log_setting.upper() == "INFO":
                import logging
            ...
            elif dheera_ai_log_setting.upper() == "DEBUG":
                import logging
    ...
    if api_version:
        os.environ["AZURE_API_VERSION"] = (
            api_version  # set this for azure - dheera_ai can read this from the env
        )
    ...
    if drop_params is True:  # dheera_ai-specific param
        dheera_ai.drop_params = True
        dynamic_config["general"]["drop_params"] = True
    if add_function_to_prompt is True:  # dheera_ai-specific param
        dheera_ai.add_function_to_prompt = True
        dynamic_config["general"]["add_function_to_prompt"] = True
    if max_budget:  # dheera_ai-specific param
        dheera_ai.max_budget = max_budget
...
async def async_assistants_data_generator(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.async_assistants_data_generator(): Exception occured - {}".format(
                str(e)
            )
        )
        ...
        verbose_proxy_logger.debug(
            f"\033[1;31mAn error occurred: {e}\n\n Debug this by setting `--debug`, e.g. `dheera_ai --model gpt-3.5-turbo --debug`"
        )
    ...
    try:
        ...
        async for chunk in proxy_logging_obj.async_post_call_streaming_iterator_hook(
            ...
            if isinstance(chunk, (ModelResponse, ModelResponseStream)):
                response_str = dheera_ai.get_response_string(response_obj=chunk)
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.async_data_generator(): Exception occured - {}".format(
                str(e)
            )
        )
        await proxy_logging_obj.post_call_failure_hook(
...
def get_dheera_ai_model_info(model: dict = {}):
    model_info = model.get("model_info", {})
    model_to_lookup = model.get("dheera_ai_params", {}).get("model", None)
    try:
        if "azure" in model_to_lookup or model_info.get("base_model"):
            model_to_lookup = model_info.get("base_model", None)
        dheera_ai_model_info = dheera_ai.get_model_info(model_to_lookup)
        return dheera_ai_model_info
    except Exception:
        # this should not block returning on /model/info
        # if dheera_ai does not have info on the model it should return {}
        return {}
...
class ProxyStartupEvent:
    ...
    def _initialize_jwt_auth(
        ...
        if general_settings.get("dheera_ai_jwtauth", None) is not None:
            for k, v in general_settings["dheera_ai_jwtauth"].items():
                if isinstance(v, str) and v.startswith("os.environ/"):
                    general_settings["dheera_ai_jwtauth"][k] = get_secret(v)
            dheera_ai_jwtauth = DheeraAI_JWTAuth(**general_settings["dheera_ai_jwtauth"])
        else:
    ...
    @classmethod
    def _add_proxy_budget_to_db(cls, dheera_ai_proxy_budget_name: str):
        """Adds a global proxy budget to db"""
        if dheera_ai.budget_duration is None:
            raise Exception(
        ...
        asyncio.create_task(
            generate_key_helper_fn(  # type: ignore
                request_type="user",
                table_name="user",
                user_id=dheera_ai_proxy_budget_name,
                duration=None,
                models=[],
                aliases={},
                config={},
                ...
                budget_duration=dheera_ai.budget_duration,
    ...
    async def _update_default_team_member_budget(cls):
        ...
        if dheera_ai.default_internal_user_params is None:
            return

        _teams = dheera_ai.default_internal_user_params.get("teams") or []
    ...
    async def initialize_scheduled_background_jobs(
        ...
        if general_settings.get("disable_spend_logs", False) is False:
            from dheera_ai.proxy.utils import _monitor_spend_logs_queue
        ...
        if llm_router is not None:
            try:
                from dheera_ai_enterprise.proxy.common_utils.check_batch_cost import (
                    CheckBatchCost,
                )
                ...
                verbose_proxy_logger.debug(
                    "Checking batch cost for DheeraAI Managed Files is an Enterprise Feature. Skipping..."
                )
                pass
    ...
    async def _initialize_spend_tracking_background_jobs(
        ...
        from dheera_ai.proxy.spend_tracking.cloudzero_endpoints import is_cloudzero_setup
        ...
        ########################################################
        if dheera_ai.prometheus_initialize_budget_metrics is True:
        ...
        ########################################################
        from dheera_ai.constants import (
            DHEERA_AI_KEY_ROTATION_CHECK_INTERVAL_SECONDS,
            DHEERA_AI_KEY_ROTATION_ENABLED,
        )

        key_rotation_enabled: Optional[bool] = str_to_bool(DHEERA_AI_KEY_ROTATION_ENABLED)
        verbose_proxy_logger.debug(f"key_rotation_enabled: {key_rotation_enabled}")

        if key_rotation_enabled is True:
            try:
                from dheera_ai.proxy.common_utils.key_rotation_manager import (
                    KeyRotationManager,
                )
                ...
                if prisma_client is not None:
                    ...
                    verbose_proxy_logger.debug(
                        f"Key rotation background job scheduled every {DHEERA_AI_KEY_ROTATION_CHECK_INTERVAL_SECONDS} seconds (DHEERA_AI_KEY_ROTATION_ENABLED=true)"
                    )
                    scheduler.add_job(
                        key_rotation_manager.process_rotations,
                        "interval",
                        seconds=DHEERA_AI_KEY_ROTATION_CHECK_INTERVAL_SECONDS,
                        id="key_rotation_job",
                    )
                else:
                    verbose_proxy_logger.warning(
            ...
            verbose_proxy_logger.debug(
                "Key rotation disabled (set DHEERA_AI_KEY_ROTATION_ENABLED=true to enable)"
            )
    ...
    def _init_dd_tracer(cls):
        ...
        """
        from dheera_ai.dheera_ai_core_utils.dd_tracing import (
...
async def model_info(
    ...
    # Use the actual dheera_ai model from the deployment to get provider info
    _, provider, _, _ = dheera_ai.get_llm_provider(model=deployment.dheera_ai_params.model)
...
async def chat_completion(  # noqa: PLR0915
    ...
    try:
        ...
        _chat_response = dheera_ai.ModelResponse()
        ...
        if data.get("stream", None) is not None and data["stream"] is True:
            _iterator = dheera_ai.utils.ModelResponseIterator(
                model_response=_chat_response, convert_to_delta=True
            )
            _streaming_response = dheera_ai.CustomStreamWrapper(
                completion_stream=_iterator,
                model=e.model,
                custom_llm_provider="cached_response",
                logging_obj=data.get("dheera_ai_logging_obj", None),
            )
        ...
        _usage = dheera_ai.Usage(prompt_tokens=0, completion_tokens=0, total_tokens=0)
...
async def completion(  # noqa: PLR0915
    ...
    try:
        ...
        if _data.get("stream", None) is not None and _data["stream"] is True:
            _text_response = dheera_ai.ModelResponse()
            ...
            _streaming_response = dheera_ai.TextCompletionStreamWrapper(
                completion_stream=_iterator,
                model=e.model,
            )
            ...
            _response = dheera_ai.TextCompletionResponse()
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.completion(): Exception occured - {}".format(
                str(e)
            )
        )
        error_msg = f"{str(e)}"
        ...
        if (
            ...
            if llm_router is not None and data.get("model") in router_model_names:
                ...
                if deployment is not None:
                    dheera_ai_params = deployment.get("dheera_ai_params", {}) or {}
                    dheera_ai_model = dheera_ai_params.get("model", "")
                    # Check if this provider supports token arrays
                    supports_token_arrays = dheera_ai_model in dheera_ai.open_ai_embedding_models or any(
                        dheera_ai_model.startswith(provider)
                        for provider in DHEERA_AI_EMBEDDING_PROVIDERS_SUPPORTING_INPUT_ARRAY_OF_TOKENS
                    )
                    if not supports_token_arrays:
                        ...
                        for i in data["input"]:
                            input_list.append(
                                dheera_ai.decode(model="gpt-3.5-turbo", tokens=i)
                            )
...
async def moderations(
    ...
    try:
        ...
        data = await add_dheera_ai_data_to_request(
            data=data,
            request=request,
            general_settings=general_settings,
            user_api_key_dict=user_api_key_dict,
        ...
        asyncio.create_task(
            proxy_logging_obj.update_request_status(
                dheera_ai_call_id=data.get("dheera_ai_call_id", ""), status="success"
            )
        )
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.moderations(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def audio_speech(
    ...
    try:
        ...
        dheera_ai_call_id = hidden_params.get("dheera_ai_call_id", None) or ""
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.audio_speech(): Exception occured - {}".format(
                str(e)
            )
        )
...
async def audio_transcriptions(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.audio_transcription(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def get_assistants(
    ...
    try:
        ...
        # for now use custom_llm_provider=="openai" -> this will change as DheeraAI adds more providers for acreate_batch
        if llm_router is None:
            raise HTTPException(
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.get_assistants(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def create_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.create_assistant(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def delete_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.delete_assistant(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def create_threads(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.create_threads(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def get_thread(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.get_thread(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def add_messages(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.add_messages(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def get_messages(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.get_messages(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def run_thread(
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.run_thread(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
from dheera_ai.llms.base_llm.base_utils import BaseTokenCounter
...
def _get_provider_token_counter(
    ...
    full_model = deployment.get("dheera_ai_params", {}).get("model", "")
    model: Optional[str] = None
    custom_llm_provider: Optional[str] = None

    try:
        # Use existing DheeraAI logic to determine provider
        model, custom_llm_provider, dynamic_api_key, api_base = get_llm_provider(
            model=full_model,
            custom_llm_provider=deployment.get("dheera_ai_params", {}).get(
                "custom_llm_provider"
            ),
            api_base=deployment.get("dheera_ai_params", {}).get("api_base"),
            api_key=deployment.get("dheera_ai_params", {}).get("api_key"),
        )
...
async def token_counter(request: TokenCountRequest, call_endpoint: bool = False):
    ...
    if llm_router is not None:
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "dheera_ai.proxy.proxy_server.token_counter(): Exception occured while getting deployment"
            )
            pass
    if deployment is not None:
        dheera_ai_model_name = deployment.get("dheera_ai_params", {}).get("model")
        model_info = deployment.get("model_info", {})
        load_credentials_from_list(deployment.get("dheera_ai_params", {}))
        # remove the custom_llm_provider_prefix in the dheera_ai_model_name
        if "/" in dheera_ai_model_name:
            dheera_ai_model_name = dheera_ai_model_name.split("/", 1)[1]

    model_to_use: str = (
        dheera_ai_model_name or request.model
    )  # use dheera_ai model name, if it's not avalable then fallback to request.model
    ...
    # Default DheeraAI token counting
    ...
    _tokenizer_used = dheera_ai.utils._select_tokenizer(
...
async def supported_openai_params(model: str):
    """
    Returns supported openai params for a given dheera_ai model name
    ...
    """
    try:
        model, custom_llm_provider, _, _ = dheera_ai.get_llm_provider(model=model)
        return {
...
def _check_if_model_is_team_model(
    models: List[DeploymentTypedDict], user_row: DheeraAI_UserTable
) -> List[Dict]:
    """
...
async def non_admin_all_models(
    ...
    unique_models = _deduplicate_dheera_ai_router_models(models=all_models)
...
def get_direct_access_models(
    user_db_object: DheeraAI_UserTable,
    llm_router: Router,
) -> List[str]:
    """
...
async def get_all_team_and_direct_access_models(
    ...
    if user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN:
        ...
        if user_db_object is not None:
            user_object = DheeraAI_UserTable(**user_db_object.model_dump())
...
async def model_info_v2(
    ...
    if llm_router is None:
        raise HTTPException(
            status_code=500,
            detail={
                "error": f"No model list passed, models router={llm_router}. You can add a model through the config.yaml or on the DheeraAI Admin UI."
            },
        )
    ...
    if user_model is not None:
        # if user does not use a config.yaml, https://github.com/BerriAI/dheera_ai/issues/2061
    ...
    # fill in model info based on config.yaml and dheera_ai model_prices_and_context_window.json
    for _model in all_models:
        ...
        # read dheera_ai model_prices_and_context_window.json to get the following:
        # input_cost_per_token, output_cost_per_token, max_tokens
        dheera_ai_model_info = get_dheera_ai_model_info(model=_model)

        # 2nd pass on the model, try seeing if we can find model in dheera_ai model_cost map
        if dheera_ai_model_info == {}:
            # use dheera_ai_param model_name to get model_info
            dheera_ai_params = _model.get("dheera_ai_params", {})
            dheera_ai_model = dheera_ai_params.get("model", None)
            try:
                dheera_ai_model_info = dheera_ai.get_model_info(model=dheera_ai_model)
            except Exception:
                dheera_ai_model_info = {}
        # 3rd pass on the model, try seeing if we can find model but without the "/" in model cost map
        if dheera_ai_model_info == {}:
            # use dheera_ai_param model_name to get model_info
            dheera_ai_params = _model.get("dheera_ai_params", {})
            dheera_ai_model = dheera_ai_params.get("model", None)
            split_model = dheera_ai_model.split("/")
            if len(split_model) > 0:
                dheera_ai_model = split_model[-1]
            try:
                dheera_ai_model_info = dheera_ai.get_model_info(
                    model=dheera_ai_model, custom_llm_provider=split_model[0]
                )
            except Exception:
                dheera_ai_model_info = {}
        for k, v in dheera_ai_model_info.items():
        ...
        _model = remove_sensitive_info_from_deployment(
            _model, excluded_keys={"dheera_ai_credential_name"}
        )
...
async def model_metrics_exceptions(
    ...
    sql_query = """
        WITH cte AS (
            SELECT
                CASE WHEN api_base = '' THEN dheera_ai_model_name ELSE CONCAT(dheera_ai_model_name, '-', api_base) END AS combined_model_api_base,
                exception_type,
                COUNT(*) AS num_rate_limit_exceptions
            FROM "DheeraAI_ErrorLogs"
            WHERE
...
def _get_proxy_model_info(model: dict) -> dict:
    ...
    # read dheera_ai model_prices_and_context_window.json to get the following:
    # input_cost_per_token, output_cost_per_token, max_tokens
    dheera_ai_model_info = get_dheera_ai_model_info(model=model)
    ...
    model = remove_sensitive_info_from_deployment(
        deployment_dict=model, excluded_keys={"dheera_ai_credential_name"}
    )
...
async def model_info_v1(  # noqa: PLR0915
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    dheera_ai_model_id: Optional[str] = None,
):
    """
    Provides more info about each model in /models, including config.yaml descriptions (except api key and api base)

    Parameters:
        dheera_ai_model_id: Optional[str] = None (this is the value of `x-dheera_ai-model-id` returned in response headers)

        - When dheera_ai_model_id is passed, it will return the info for that specific model
        - When dheera_ai_model_id is not passed, it will return the info for all models
    ...
    if user_model is not None:
        # user is trying to get specific model from dheera_ai router
        try:
            model_info: Dict = cast(Dict, dheera_ai.get_model_info(model=user_model))
        except Exception:
            model_info = {}
        ...
        _deployment_info_dict = remove_sensitive_info_from_deployment(
            ...
            excluded_keys={"dheera_ai_credential_name"},
        )
    ...
    if dheera_ai_model_id is not None:
        # user is trying to get specific model from dheera_ai router
        deployment_info = llm_router.get_deployment(model_id=dheera_ai_model_id)
        if deployment_info is None:
            raise HTTPException(
                status_code=400,
                detail={
                    "error": f"Model id = {dheera_ai_model_id} not found on dheera_ai proxy"
                },
            )
...
def _get_model_group_info(
    ...
    if dheera_ai.public_model_groups is not None:
        for mg in model_groups:
            if mg.model_group in dheera_ai.public_model_groups:
...
async def model_group_info(
    ...
    """
    Get information about all the deployments on dheera_ai proxy, including config.yaml descriptions (except api key and api base)
    ...
    from dheera_ai.proxy.utils import get_available_models_for_user
...
async def model_settings():
    ...
    for provider in dheera_ai.provider_list:
        returned_list.append(
            ProviderInfo(
                name=provider,
                fields=dheera_ai.get_provider_fields(custom_llm_provider=provider),
            )
        )
...
async def async_queue_request(
    ...
    try:
        ...
        fastapi_response.headers.update({"x-dheera_ai-priority": str(data["priority"])})
        return response
    except Exception as e:
        await proxy_logging_obj.post_call_failure_hook(
            user_api_key_dict=user_api_key_dict, original_exception=e, request_data=data
...
async def fallback_login(request: Request):
    ...
    PROXY_BASE_URL should be the your deployed proxy endpoint, e.g. PROXY_BASE_URL="https://dheera_ai-production-7002.up.railway.app/"
    Example:
    """
    from dheera_ai.proxy.proxy_server import ui_link
...
async def login(request: Request):  # noqa: PLR0915
    ...
    from dheera_ai.proxy.auth.login_utils import authenticate_user, create_ui_token_object
    from dheera_ai.proxy.utils import get_custom_url
    ...
    # Build redirect URL
    dheera_ai_dashboard_ui = get_custom_url(str(request.base_url))
    if dheera_ai_dashboard_ui.endswith("/"):
        dheera_ai_dashboard_ui += "ui/"
    else:
        dheera_ai_dashboard_ui += "/ui/"
    dheera_ai_dashboard_ui += "?login=success"

    # Create redirect response with cookie
    redirect_response = RedirectResponse(url=dheera_ai_dashboard_ui, status_code=303)
...
async def login_v2(request: Request):  # noqa: PLR0915
    ...
    json_response = JSONResponse(
        content={"redirect_url": dheera_ai_dashboard_ui},
...
async def onboarding(invite_link: str, request: Request):
    ...
    from dheera_ai.types.proxy.ui_sso import ReturnedUITokenObject
    ...
    utc_now_date = dheera_ai.utils.get_utc_datetime().date()
    ...
    response = await generate_key_helper_fn(
        ...
        **{
            ...
            "duration": "24hr",
            "key_max_budget": dheera_ai.max_ui_session_budget,
            "models": [],
            "aliases": {},
            "config": {},
            "spend": 0,
    ...
    dheera_ai_dashboard_ui = get_custom_url(str(request.base_url))
    if dheera_ai_dashboard_ui.endswith("/"):
        dheera_ai_dashboard_ui += "ui/onboarding"
    else:
        dheera_ai_dashboard_ui += "/ui/onboarding"
    import jwt
    ...
    returned_ui_token_object = ReturnedUITokenObject(
        ...
        auth_header_name=general_settings.get(
            "dheera_ai_key_header_name", "Authorization"
        ),
    ...
    dheera_ai_dashboard_ui += "?token={}&user_email={}".format(jwt_token, user_email)
    return {
        "login_url": dheera_ai_dashboard_ui,
        "token": jwt_token,
...
async def claim_onboarding_link(data: InvitationClaim):
    ...
    if invite_obj.is_accepted is True:
        ...
        raise HTTPException(
            status_code=401,
            detail={
                "error": "The invitation link was never validated. Please file an issue, if this is not intended - https://github.com/BerriAI/dheera_ai/issues."
            },
        )
...
def get_image():
    ...
    is_non_root = os.getenv("DHEERA_AI_NON_ROOT", "").lower() == "true"
    assets_dir = "/tmp/dheera_ai_assets" if is_non_root else current_dir
...
async def new_invitation(
    ...
    curl -X POST 'http://localhost:4000/invitation/new' \
        -H 'Content-Type: application/json' \
        -d '{
            "user_id": "1234" // 👈 id of user in 'DheeraAI_UserTable'
        }'
    ```
    """
    try:
        from dheera_ai.proxy.management_helpers.user_invitation import (
...
async def invitation_update(
    ...
    curl -X POST 'http://localhost:4000/invitation/update' \
        -H 'Content-Type: application/json' \
        -d '{
            "invitation_id": "1234" // 👈 id of invitation in 'DheeraAI_InvitationTable'
...
async def update_config(config_info: ConfigYAML):  # noqa: PLR0915
    ...
    Currently supports modifying General Settings + DheeraAI settings
    """
    ...
    try:
        ...
        # update the dheera_ai settings
        if config_info.dheera_ai_settings is not None:
            config.setdefault("dheera_ai_settings", {})
            updated_dheera_ai_settings = config_info.dheera_ai_settings
            config["dheera_ai_settings"] = {
                **updated_dheera_ai_settings,
                **config["dheera_ai_settings"],
            }

            # if dheera_ai.success_callback in updated_dheera_ai_settings and config["dheera_ai_settings"]
            if (
                "success_callback" in updated_dheera_ai_settings
                and "success_callback" in config["dheera_ai_settings"]
            ):
                # check both success callback are lists
                if isinstance(
                    config["dheera_ai_settings"]["success_callback"], list
                ) and isinstance(updated_dheera_ai_settings["success_callback"], list):
                    updated_success_callbacks_normalized = normalize_callback_names(
                        updated_dheera_ai_settings["success_callback"]
                    )
                    combined_success_callback = (
                        config["dheera_ai_settings"]["success_callback"]
        ...
        verbose_proxy_logger.error(
            "dheera_ai.proxy.proxy_server.update_config(): Exception occured - {}".format(
                str(e)
            )
        )
        verbose_proxy_logger.debug(traceback.format_exc())
...
async def update_config_general_settings(
    ...
    """
    Update a specific field in dheera_ai general settings
    """
    global prisma_client
    ## VALIDATION ##
    """
...
async def delete_config_general_settings(
    ...
    """
    Delete the db value of this field in dheera_ai general settings. Resets it to it's initial default value on dheera_ai.
    """
    global prisma_client
    ## VALIDATION ##
    """
...
async def delete_callback(
    ...
    try:
        ...
        dheera_ai_settings = config.get("dheera_ai_settings", {})
        success_callbacks = dheera_ai_settings.get("success_callback", [])
        ...
        verbose_proxy_logger.error(
            f"dheera_ai.proxy.proxy_server.delete_callback(): Exception occurred - {str(e)}"
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        raise ProxyException(
...
async def get_config():  # noqa: PLR0915
    ...
    try:
        ...
        config_data = await proxy_config.get_config()
        _dheera_ai_settings = config_data.get("dheera_ai_settings", {})
        _general_settings = config_data.get("general_settings", {})
        environment_variables = config_data.get("environment_variables", {})
        
        _success_callbacks = _dheera_ai_settings.get("success_callback", [])
        _failure_callbacks = _dheera_ai_settings.get("failure_callback", [])
        _success_and_failure_callbacks = _dheera_ai_settings.get("callbacks", [])
        ...
        verbose_proxy_logger.exception(
            "dheera_ai.proxy.proxy_server.get_config(): Exception occured - {}".format(
                str(e)
            )
        )
        if isinstance(e, HTTPException):
...
async def config_yaml_endpoint(config_info: ConfigYAML):
    ...
    - **model_list**: *Optional[List[ModelParams]]* - A list of supported models on the server, along with model-specific configurations. ModelParams includes "model_name" (name of the model), "dheera_ai_params" (dheera_ai-specific parameters for the model), and "model_info" (additional info about the model such as id, mode, cost per token, etc).

    - **dheera_ai_settings**: *Optional[dict]*: Settings for the dheera_ai module. You can specify multiple properties like "drop_params", "set_verbose", "api_base", "cache".
...
async def reload_model_cost_map(
    ...
    try:
        ...
        from dheera_ai.dheera_ai_core_utils.get_model_cost_map import get_model_cost_map
...
async def get_routes():
    ...
    """
    from dheera_ai.proxy.common_utils.get_routes import GetRoutes
