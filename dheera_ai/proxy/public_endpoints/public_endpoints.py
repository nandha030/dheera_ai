...
from dheera_ai.types.agents import AgentCard
from dheera_ai.types.mcp import MCPPublicServer
from dheera_ai.types.proxy.management_endpoints.model_management_endpoints import (
    ModelGroupInfoProxy,
)
from dheera_ai.types.proxy.public_endpoints.public_endpoints import (
    AgentCreateInfo,
    ProviderCreateInfo,
    PublicModelHubInfo,
)
...
async def public_model_hub():
    import dheera_ai
    from dheera_ai.proxy.proxy_server import _get_model_group_info, llm_router
    ...
    if dheera_ai.public_model_groups is not None:
        model_groups = _get_model_group_info(
            ...
            all_models_str=dheera_ai.public_model_groups,
            model_group=None,
        )
...
async def public_model_hub_info():
    import dheera_ai
    from dheera_ai.proxy.proxy_server import _title, version
    ...
    return PublicModelHubInfo(
        ...
        useful_links=dheera_ai.public_model_groups_links,
    )
...
async def get_supported_providers() -> List[str]:
    """
    Return a sorted list of all providers supported by DheeraAI.
    """
...
async def get_dheera_ai_model_cost_map():
    """
    Public endpoint to get the DheeraAI model cost map.
    Returns pricing information for all supported models.
    """
    import dheera_ai

    try:
        _model_cost_map = dheera_ai.model_cost
...
async def get_agent_fields() -> List[AgentCreateInfo]:
    ...
    for agent in agent_create_fields:
        ...
        if inherit_from and inherit_from in provider_map:
            ...
            # Copy provider fields and mark them for inclusion in dheera_ai_params
            inherited_fields = []
            for field in provider.get("credential_fields", []):
                field_copy = field.copy()
                field_copy["include_in_dheera_ai_params"] = True
