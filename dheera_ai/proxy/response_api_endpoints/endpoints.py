...
from dheera_ai.proxy.auth.user_api_key_auth import UserAPIKeyAuth, user_api_key_auth
...
async def cursor_chat_completions(
    ...
    ```
    """
    from dheera_ai.completion_extras.dheera_ai_responses_transformation.handler import (
    ...
    def cursor_data_generator(response, user_api_key_dict, request_data):
        ...
        if isinstance(response, BaseResponsesAPIStreamingIterator):
            ...
            logging_obj = request_data.get("dheera_ai_logging_obj")
    ...
    try:
        ...
        if isinstance(response, ResponsesAPIResponse):
            logging_obj = processor.data.get("dheera_ai_logging_obj")
...
async def get_response(
    ...
    Supports both:
    - Polling IDs (dheera_ai_poll_*): Returns cumulative cached content from background responses
    ...
    curl -X GET http://localhost:4000/v1/responses/dheera_ai_poll_abc123 \
    -H "Authorization: Bearer sk-1234"
...
async def delete_response(
    ...
    Supports both:
    - Polling IDs (dheera_ai_poll_*): Deletes from Redis cache
    - Provider response IDs: Passes through to provider API
...
async def cancel_response(
    ...
    Supports both:
    - Polling IDs (dheera_ai_poll_*): Cancels background response and updates status in Redis
    - Provider response IDs: Passes through to provider API
    ...
    curl -X POST http://localhost:4000/v1/responses/dheera_ai_poll_abc123/cancel \
    -H "Authorization: Bearer sk-1234"
