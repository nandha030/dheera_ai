...
def get_team_id_from_data(data: dict) -> Optional[str]:
    """
    Get the team id from the data's metadata or dheera_ai_metadata params.
    """
    if (
        "metadata" in data
        ...
        and data["dheera_ai_metadata"] is not None
        and "user_api_key_team_id" in data["dheera_ai_metadata"]
    ):
        return data["dheera_ai_metadata"].get("user_api_key_team_id")
    return None
...
def add_shared_session_to_data(data: dict) -> None:
    ...
    """
    try:
        from dheera_ai.proxy.proxy_server import shared_aiohttp_session
...
async def route_request(
    ...
    if "api_key" in data or "api_base" in data:
        if llm_router is not None:
            ...
            return getattr(dheera_ai, f"{route_type}")(**data)

    elif "user_config" in data:
        router_config = data.pop("user_config")
        user_router = dheera_ai.Router(**router_config)
