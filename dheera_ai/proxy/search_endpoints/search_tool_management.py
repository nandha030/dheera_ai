...
from dheera_ai.proxy.search_endpoints.search_tool_registry import SearchToolRegistry
from dheera_ai.types.search import (
        ...
        for search_tool in search_tools:
            search_tool_configs.append(
                SearchToolInfoResponse(
                    ...
                    dheera_ai_params=dict(search_tool.get("dheera_ai_params", {})),
...
async def get_search_tool_info(search_tool_id: str):
    ...
    try:
        ...
        # Mask sensitive data
        dheera_ai_params_dict = dict(result.get("dheera_ai_params", {}))
...
async def test_search_tool_connection(request: TestSearchToolConnectionRequest):
    ...
    """
    try:
        from dheera_ai.search import asearch

        # Extract params from request
        dheera_ai_params = request.dheera_ai_params
        ...
        if not search_provider:
            raise HTTPException(
                status_code=400,
                detail="search_provider is required in dheera_ai_params"
            )
