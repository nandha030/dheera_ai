...
from dheera_ai.proxy._types import ProviderBudgetResponse, ProviderBudgetResponseObject
...
@router.get(
    ...
    dependencies=[Depends(user_api_key_auth)],
    responses={
        200: {"model": List[DheeraAI_SpendLogs]},
    },
)
async def view_spend_tags(
    ...
    """
    DheeraAI Enterprise - View Spend Per Request Tag
...
async def get_global_spend_provider(
    ...
    from dheera_ai.proxy.proxy_server import llm_router, prisma_client
        ...
        for row in db_response:
            ...
            if llm_router is not None:
                ...
                if _deployment is not None:
                    try:
                        _, _provider, _, _ = dheera_ai.get_llm_provider(
                            model=_deployment.dheera_ai_params.model,
                            custom_llm_provider=_deployment.dheera_ai_params.custom_llm_provider,
                            api_base=_deployment.dheera_ai_params.api_base,
                            dheera_ai_params=_deployment.dheera_ai_params,
                        )
...
async def global_view_spend_tags(
    ...
    """
    DheeraAI Enterprise - View Spend Per Request Tag. Used by DheeraAI UI
...
async def _get_spend_report_for_time_range(
    ...
    try:
        ...
        FROM
            "DheeraAI_SpendLogs" s
        LEFT JOIN
            "DheeraAI_TeamTable" t ON s.team_id = t.team_id
        WHERE
...
async def calculate_spend(request: SpendCalculateRequest):
    ...
    try:
        ...
        from dheera_ai.cost_calculator import CostPerToken
        ...
        if request.model is not None:
            ...
            1. user passes model, and model is defined on dheera_ai config.yaml or in DB. use info on config or in DB in this case
            2. user passes model, and model is not defined on dheera_ai config.yaml or in DB. Pass model as is to dheera_ai.completion_cost
            3. user passes completion_response
            
            """
            if _model_in_llm_router is not None:
                _dheera_ai_params = _model_in_llm_router.get("dheera_ai_params")
                _dheera_ai_model_name = _dheera_ai_params.get("model")
                input_cost_per_token = _dheera_ai_params.get("input_cost_per_token")
                output_cost_per_token = _dheera_ai_params.get("output_cost_per_token")
                if (
            ...
            _completion_response = dheera_ai.ModelResponse(**request.completion_response)
...
async def ui_view_request_response_for_request_id(
    ...
    """
    custom_loggers = (
        dheera_ai.logging_callback_manager.get_active_additional_logging_utils_from_custom_logger()
    )
...
async def global_spend_reset():
    ...
    Globally reset spend for All API Keys and Teams, maintain DheeraAI_SpendLogs

    1. DheeraAI_SpendLogs will maintain the logs on spend, no data gets deleted from there
    2. DheeraAI_VerificationTokens spend will be set = 0
    3. DheeraAI_TeamTable spend will be set = 0
...
async def global_spend():
    ...
    try:
        ...
        return {"spend": total_spend, "max_budget": dheera_ai.max_budget}
    except Exception as e:
        error_trace = traceback.format_exc()
        error_str = str(e) + "\n" + error_trace
        if isinstance(e, HTTPException):
...
async def global_spend_key_internal_user(
    ...
    sql_query = """
        ...
        FROM 
            top_api_keys t
        LEFT JOIN 
            "DheeraAI_VerificationToken" v ON t.api_key = v.token
        ORDER BY 
            t.total_spend DESC;
...
async def global_view_all_end_users():
    ...
    sql_query = """
    SELECT DISTINCT end_user FROM "DheeraAI_SpendLogs"
    """
