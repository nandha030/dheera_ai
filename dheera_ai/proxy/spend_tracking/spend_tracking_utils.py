...
from dheera_ai.constants import MAX_STRING_LENGTH_PROMPT_IN_DB, REDACTED_BY_LITELM_STRING
from dheera_ai.dheera_ai_core_utils.core_helpers import get_dheera_ai_metadata_from_kwargs
...
from dheera_ai.proxy.utils import PrismaClient, hash_token
...
def _get_spend_logs_metadata(
    ...
    dheera_ai_overhead_time_ms: Optional[float] = None,
) -> SpendLogsMetadata:
    if metadata is None:
    ...
    clean_metadata["dheera_ai_overhead_time_ms"] = dheera_ai_overhead_time_ms
...
def get_spend_logs_id(
    ...
    if call_type == "aretrieve_batch" or call_type == "acreate_file":
        ...
        id = cast(Optional[str], response_obj.get("id")) or cast(
            Optional[str], kwargs.get("dheera_ai_call_id")
        )
    return id
...
def get_logging_payload(  # noqa: PLR0915
    ...
    from dheera_ai.proxy.proxy_server import general_settings, master_key
    ...
    metadata = get_dheera_ai_metadata_from_kwargs(kwargs)
    ...
    if call_type in ["ocr", "aocr"]:
        ...
        if isinstance(_usage, dheera_ai.Usage):
            usage = dict(_usage)
    ...
    end_user_id = get_end_user_id_for_cost_tracking(dheera_ai_params)
    ...
    if api_key is not None and isinstance(api_key, str):
        ...
        if (
            ...
            api_key = "dheera_ai_proxy_master_key"  # use a known alias, if the user disabled storing master key in db
    ...
    if standard_logging_payload is not None:
        ...
        dheera_ai_overhead_time_ms = hidden_params.get("dheera_ai_overhead_time_ms")
    ...
    clean_metadata = _get_spend_logs_metadata(
        ...
        dheera_ai_overhead_time_ms=dheera_ai_overhead_time_ms,
    )
    ...
    try:
        payload: SpendLogsPayload = SpendLogsPayload(
            ...
            proxy_server_request=_get_proxy_server_request_for_spend_logs_payload(
                metadata=metadata, dheera_ai_params=dheera_ai_params
            ),
...
def _get_session_id_for_spend_log(
    ...
    # Users can dynamically set the trace_id for each request by passing `dheera_ai_trace_id` in kwargs
    if kwargs.get("dheera_ai_trace_id") is not None:
        return str(kwargs.get("dheera_ai_trace_id"))
...
def _sanitize_request_body_for_spend_logs_payload(
    ...
    """
    from dheera_ai.constants import DHEERA_AI_TRUNCATED_PAYLOAD_FIELD
    ...
    def _sanitize_value(value: Any) -> Any:
        if isinstance(value, dict):
            ...
            if len(value) > MAX_STRING_LENGTH_PROMPT_IN_DB:
                ...
                truncated_value = (
                    f"{value[:start_chars]}"
                    f"... ({DHEERA_AI_TRUNCATED_PAYLOAD_FIELD} skipped {skipped_chars} chars) ..."
                    f"{value[-end_chars:]}"
                )
...
def _get_proxy_server_request_for_spend_logs_payload(
    ...
    if _should_store_prompts_and_responses_in_spend_logs():
        _proxy_server_request = cast(
            Optional[dict], dheera_ai_params.get("proxy_server_request", {})
        )
