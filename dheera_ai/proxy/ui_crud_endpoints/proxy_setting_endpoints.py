...
async def _get_settings_with_schema(
    ...
    Args:
        settings_key: The key in dheera_ai_settings to get
        settings_class: The Pydantic class to use for schema
        config: The config dictionary
    """
    from pydantic import TypeAdapter

    dheera_ai_settings = config.get("dheera_ai_settings", {}) or {}
    settings_data = dheera_ai_settings.get(settings_key, {}) or {}
...
async def get_internal_user_settings():
    """
    Get all SSO settings from the dheera_ai_settings configuration.
...
async def _update_dheera_ai_setting(
    ...
    """
    Common utility function to update `dheera_ai_settings` in both memory and config.

    Args:
        settings: The settings object to update
        settings_key: The key in dheera_ai_settings to update
    ...
    config["dheera_ai_settings"][settings_key] = settings.model_dump(exclude_none=True)
...
async def update_internal_user_settings(
    ...
    return await _update_dheera_ai_setting(
        settings=settings,
        settings_key="default_internal_user_params",
        in_memory_var=dheera_ai.default_internal_user_params,
...
async def update_default_team_settings(settings: DefaultTeamSSOParams):
    ...
    return await _update_dheera_ai_setting(
        settings=settings,
        settings_key="default_team_params",
        in_memory_var=dheera_ai.default_team_params,
...
async def get_sso_settings():
    ...
    from dheera_ai.proxy.proxy_server import prisma_client, proxy_config
...
async def update_sso_settings(sso_config: SSOConfig):
    ...
    from dheera_ai.proxy.proxy_server import prisma_client, store_model_in_db, proxy_config
...
async def get_ui_theme_settings():
    """
    Get UI theme configuration from the dheera_ai_settings.
...
async def update_ui_theme_settings(theme_config: UIThemeConfig):
    ...
    # Store UI theme config in dheera_ai_settings (where it's retrieved from)
    if "dheera_ai_settings" not in config:
        config["dheera_ai_settings"] = {}
    config["dheera_ai_settings"]["ui_theme_config"] = theme_data
...
async def get_ui_settings():
    ...
    config: Dict[str, Any] = {"dheera_ai_settings": {"ui_settings": ui_settings}}
...
async def update_ui_settings(
    ...
    """
    from dheera_ai.proxy.proxy_server import prisma_client, store_model_in_db
