...
from dheera_ai.constants import DEFAULT_MODEL_CREATED_AT_TIME, MAX_TEAM_LIST_LIMIT
...
from dheera_ai.types.utils import CallTypes, CallTypesLiteral
...
from dheera_ai._service_logger import ServiceLogging, ServiceTypes
...
from dheera_ai.dheera_ai_core_utils.safe_json_loads import safe_json_loads
...
from dheera_ai.proxy.db.create_views import (
    create_missing_views,
    should_create_missing_views,
)
from dheera_ai.proxy.db.db_spend_update_writer import DBSpendUpdateWriter
from dheera_ai.proxy.db.log_db_metrics import log_db_metrics
from dheera_ai.proxy.db.prisma_client import PrismaWrapper
from dheera_ai.proxy.guardrails.guardrail_hooks.unified_guardrail.unified_guardrail import (
    UnifiedLLMGuardrails,
)
from dheera_ai.proxy.hooks import PROXY_HOOKS, get_proxy_hook
from dheera_ai.proxy.hooks.cache_control_check import _PROXY_CacheControlCheck
from dheera_ai.proxy.hooks.max_budget_limiter import _PROXY_MaxBudgetLimiter
from dheera_ai.proxy.hooks.parallel_request_limiter import (
    _PROXY_MaxParallelRequestsHandler,
)
from dheera_ai.proxy.dheera_ai_pre_call_utils import DheeraAIProxyRequestSetup
from dheera_ai.secret_managers.main import str_to_bool
from dheera_ai.types.integrations.slack_alerting import DEFAULT_ALERT_TYPES
from dheera_ai.types.mcp import (
    MCPDuringCallResponseObject,
    MCPPreCallRequestObject,
    MCPPreCallResponseObject,
)
from dheera_ai.types.utils import LLMResponseTypes, LoggedDheeraAIParams
...
def print_verbose(print_statement):
    """
    Prints the given `print_statement` to the console if `dheera_ai.set_verbose` is True.
    ...
    if dheera_ai.set_verbose:
        print(f"DheeraAI Proxy: {print_statement}")  # noqa
...
class InternalUsageCache:
    ...
    async def async_get_cache(
        self,
        key,
        dheera_ai_parent_otel_span: Union[Span, None],
        ...
        **kwargs,
        ...
        return await self.dual_cache.async_get_cache(
            ...
            parent_otel_span=dheera_ai_parent_otel_span,
            **kwargs,
        )
    ...
    async def async_set_cache(
        ...
        **kwargs,
        ...
        return await self.dual_cache.async_set_cache(
            ...
            value=value,
            local_only=local_only,
            dheera_ai_parent_otel_span=dheera_ai_parent_otel_span,
            **kwargs,
        )
...
class ProxyLogging:
    ...
    def startup_event(
        ...
        self._init_dheera_ai_callbacks(
        ...
        if updated_slack_alerting is True:
            ...
            if self.alerting is not None and "slack" in self.alerting:
                ...
                if (
                    ...
                    dheera_ai.logging_callback_manager.add_dheera_ai_callback(self.slack_alerting_instance)  # type: ignore
    ...
    def _add_proxy_hooks(self, llm_router: Optional[Router] = None):
        """
        Add proxy hooks to dheera_ai.callbacks
        """
        ...
        for hook in PROXY_HOOKS:
            ...
            dheera_ai.logging_callback_manager.add_dheera_ai_callback(proxy_hook_obj)
    ...
    def _init_dheera_ai_callbacks(self, llm_router: Optional[Router] = None):
        self._add_proxy_hooks(llm_router)
        dheera_ai.logging_callback_manager.add_dheera_ai_callback(self.service_logging_obj)  # type: ignore
    ...
    async def update_request_status(
        self, dheera_ai_call_id: str, status: Literal["success", "fail"]
    ):
        ...
        await self.internal_usage_cache.async_set_cache(
            key="request_status:{}".format(dheera_ai_call_id),
            value=status,
            local_only=True,
    ...
    def _convert_mcp_to_llm_format(self, request_obj, kwargs: dict) -> dict:
        ...
        """
        from dheera_ai.types.llms.openai import ChatCompletionUserMessage
    ...
    def _convert_llm_result_to_mcp_response(
        ...
        """
        from dheera_ai.types.mcp import MCPPreCallResponseObject
    ...
    def _create_mcp_request_object_from_kwargs(
        ...
        from dheera_ai.types.mcp import MCPPreCallRequestObject
    ...
    async def _process_prompt_template(
        ...
        from dheera_ai.proxy.prompts.prompt_endpoints import (
        ...
        from dheera_ai.utils import get_non_default_completion_params
        ...
        dheera_ai_prompt_id: Optional[str] = None
        if prompt_spec is not None:
            dheera_ai_prompt_id = prompt_spec.dheera_ai_params.prompt_id
    ...
    def _process_guardrail_metadata(self, data: dict) -> None:
        ...
        metadata_dheera_ai = data.get("dheera_ai_metadata") or {}
        ...
        elif isinstance(metadata_dheera_ai, dict) and "guardrails" in metadata_dheera_ai:
            guardrails_in_metadata = metadata_dheera_ai.get("guardrails", [])
        ...
        if guardrails_in_metadata and isinstance(guardrails_in_metadata, list):
            ...
            if (
                ...
                isinstance(metadata_dheera_ai, dict)
                and "applied_guardrails" in metadata_dheera_ai
            ):
                applied_guardrails = metadata_dheera_ai.get("applied_guardrails", [])
    ...
    async def pre_call_hook(
        ...
        dheera_ai_logging_obj = cast(
            Optional["DheeraAILoggingObj"], data.get("dheera_ai_logging_obj", None)
        )
        ...
        if (
            dheera_ai_logging_obj is not None
        ...
        try:
            for callback in dheera_ai.callbacks:
                ...
                _callback = None
                if isinstance(callback, str):
                    _callback = dheera_ai.dheera_ai_core_utils.dheera_ai_logging.get_custom_logger_compatible_class(
                        cast(_custom_logger_compatible_callbacks_literal, callback)
                    )
                else:
    ...
    async def alerting_handler(
        ...
        """
        Alerting based on thresholds: - https://github.com/BerriAI/dheera_ai/issues/1298
        ...
        for client in self.alerting:
            if client == "slack":
                ...
                if dheera_ai.utils.sentry_sdk_instance is not None:
                    dheera_ai.utils.sentry_sdk_instance.capture_message(formatted_message)
                else:
    ...
    async def failure_handler(
        ...
        if dheera_ai.utils.capture_exception:
            dheera_ai.utils.capture_exception(error=original_exception)
    ...
    async def post_call_failure_hook(
        ...
        await self.update_request_status(
            dheera_ai_call_id=request_data.get("dheera_ai_call_id", ""), status="fail"
        )
        if AlertType.llm_exceptions in self.alert_types and not isinstance(
            ...
            Related issue - https://github.com/BerriAI/dheera_ai/issues/3395
            """
    ...
    async def _handle_logging_proxy_only_error(
        ...
        """
        Handle logging for proxy only errors by calling `dheera_ai_logging_obj.async_failure_handler`

        Is triggered when self._is_proxy_only_error() returns True
        """
        dheera_ai_logging_obj: Optional[Logging] = request_data.get(
            "dheera_ai_logging_obj", None
        )
        if dheera_ai_logging_obj is None:
            from dheera_ai._uuid import uuid

            request_data["dheera_ai_call_id"] = str(uuid.uuid4())
            user_api_key_logged_metadata = (
                DheeraAIProxyRequestSetup.get_sanitized_user_information_from_key(
                    user_api_key_dict=user_api_key_dict
                )
            )

            dheera_ai_logging_obj, data = dheera_ai.utils.function_setup(
                original_function=route or "IGNORE_THIS",
                rules_obj=dheera_ai.utils.Rules(),
        ...
        if dheera_ai_logging_obj is not None:
            ...
            dheera_ai_param_keys = LoggedDheeraAIParams.__annotations__.keys()
            for k, v in request_data.items():
                if k in dheera_ai_param_keys:
                    _dheera_ai_params[k] = v
            ...
            if "messages" in request_data and isinstance(
                ...
                dheera_ai_logging_obj.model_call_details["messages"] = input
                dheera_ai_logging_obj.call_type = CallTypes.acompletion.value
            elif "prompt" in request_data and isinstance(request_data["prompt"], str):
                input = request_data["prompt"]
                dheera_ai_logging_obj.model_call_details["prompt"] = input
                dheera_ai_logging_obj.call_type = CallTypes.atext_completion.value
            elif "input" in request_data and isinstance(request_data["input"], list):
                input = request_data["input"]
                dheera_ai_logging_obj.model_call_details["input"] = input
                dheera_ai_logging_obj.call_type = CallTypes.aembedding.value
            dheera_ai_logging_obj.pre_call(
                input=input,
                api_key="",
            )

            # log the custom exception
            await dheera_ai_logging_obj.async_failure_handler(
                exception=original_exception,
                traceback_exception=traceback.format_exc(),
            )

            threading.Thread(
                target=dheera_ai_logging_obj.failure_handler,
                args=(
                    original_exception,
    ...
    async def async_post_call_streaming_hook(
        ...
        if isinstance(response, (ModelResponse, ModelResponseStream)):
            response_str = dheera_ai.get_response_string(response_obj=response)
...
class PrismaClient:
    ...
    async def check_view_exists(self):
        ...
        try:
            ...
            required_view = "DheeraAI_VerificationTokenView"
            ...
            if ret[0]["view_count"] == expected_total_views:
                ...
                if ret[0]["view_names"] and required_view not in ret[0]["view_names"]:
                    ...
                    verbose_proxy_logger.info(
                        "DheeraAI_VerificationTokenView Created in DB!"
                    )
                else:
                    ...
                    if should_create_views:
                        ...
                        verbose_proxy_logger.warning(
                            "\n\n\033[93mNot all views exist in db, needed for UI 'Usage' tab. Missing={}.\nRun 'create_views.py' from https://github.com/BerriAI/dheera_ai/tree/main/db_scripts to create missing views.\033[0m\n".format(
                                missing_views
                            )
                        )
    ...
    async def get_generic_data(
        ...
        try:
            ...
            error_msg = f"DheeraAI Prisma Client Exception get_generic_data: {str(e)}"
    ...
    async def get_data(  # noqa: PLR0915
        ...
        try:
            ...
            if (token is not None and table_name is None) or (
                ...
                if query_type == "find_unique":
                    ...
                    if expires is not None:
                        ...
                        FROM
                            "DheeraAI_UserTable" u
                        LEFT JOIN "DheeraAI_VerificationToken" v ON u.user_id = v.user_id
                        GROUP BY
                            u.user_id
                ...
                if query_type == "find_unique":
                    ...
                    sql_query = f"""
                        SELECT 
                            ...
                            b.max_budget AS dheera_ai_budget_table_max_budget,
                            b.tpm_limit AS dheera_ai_budget_table_tpm_limit,
                            b.rpm_limit AS dheera_ai_budget_table_rpm_limit,
                            b.model_max_budget as dheera_ai_budget_table_model_max_budget,
                            b.soft_budget as dheera_ai_budget_table_soft_budget,
                        ...
                        FROM "DheeraAI_VerificationToken" AS v
                        LEFT JOIN "DheeraAI_TeamTable" AS t ON v.team_id = t.team_id
                        LEFT JOIN "DheeraAI_TeamMembership" AS tm ON v.team_id = tm.team_id AND tm.user_id = v.user_id
                        LEFT JOIN "DheeraAI_ModelTable" m ON t.model_id = m.id
                        LEFT JOIN "DheeraAI_BudgetTable" AS b ON v.budget_id = b.budget_id
                        LEFT JOIN "DheeraAI_OrganizationTable" AS o ON v.organization_id = o.organization_id
                        LEFT JOIN "DheeraAI_BudgetTable" AS b2 ON o.budget_id = b2.budget_id
                        WHERE v.token = '{token}'
                    """
                    ...
                    if response is not None:
                        ...
                        response = DheeraAI_VerificationTokenView(
            ...
            prisma_query_info = f"DheeraAI Prisma Client Exception: Error with `get_data`. Args passed in: {args_passed_in}"
        ...
        try:
            ...
            if table_name == "key":
                ...
                try:
                    ...
                    if (
                        "Foreign key constraint failed on the field: `DheeraAI_UserTable_organization_id_fkey (index)`"
                        in str(e)
                    ):
                        raise HTTPException(
                            status_code=400,
                            detail={
                                "error": f"Foreign Key Constraint failed. Organization ID={db_data['organization_id']} does not exist in DheeraAI_OrganizationTable. Create via `/organization/new`."
                            },
                        )
                    raise e
            ...
            error_msg = f"DheeraAI Prisma Client Exception in insert_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def update_data(  # noqa: PLR0915
        ...
        try:
            ...
            error_msg = f"DheeraAI Prisma Client Exception - update_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def delete_data(
        ...
        try:
            ...
            error_msg = f"DheeraAI Prisma Client Exception - delete_data: {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def connect(self):
        ...
        try:
            ...
            error_msg = f"DheeraAI Prisma Client Exception connect(): {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def disconnect(self):
        ...
        try:
            ...
            error_msg = f"DheeraAI Prisma Client Exception disconnect(): {str(e)}"
            print_verbose(error_msg)
            error_traceback = error_msg + "\n" + traceback.format_exc()
            end_time = time.time()
            _duration = end_time - start_time
    ...
    async def _get_spend_logs_row_count(self) -> int:
        """
        Get the row count from DheeraAI_SpendLogs table using PostgreSQL system statistics.
        """
        ...
        async def _fetch_row_count() -> int:
            ...
            FROM pg_class
            WHERE oid = '"DheeraAI_SpendLogs"'::regclass;
            """
        ...
        try:
            ...
            verbose_proxy_logger.error(
                f"Error getting DheeraAI_SpendLogs row count: {e}"
            )
            return 0
    ...
    async def _set_spend_logs_row_count_in_proxy_state(self) -> None:
        """
        Set the `DheeraAI_SpendLogs`row count in proxy state.
...
async def _monitor_spend_logs_queue(
    ...
    """
    from dheera_ai.constants import SPEND_LOG_QUEUE_SIZE_THRESHOLD, SPEND_LOG_QUEUE_POLL_INTERVAL
...
def _raise_failed_update_spend_exception(
    ...
    error_msg = (
        f"[Non-Blocking]DheeraAI Prisma Client Exception - update spend logs: {str(e)}"
    )
    error_traceback = error_msg + "\n" + traceback.format_exc()
    end_time = time.time()
    _duration = end_time - start_time
...
def _check_and_merge_model_level_guardrails(
    ...
    model_level_guardrails = deployment.dheera_ai_params.get("guardrails")
...
def _premium_user_check(feature: Optional[str] = None):
    ...
    if feature:
        detail_msg = f"This feature is only available for DheeraAI Enterprise users: {feature}. {CommonProxyErrors.not_premium_user.value}"
    else:
        detail_msg = f"This feature is only available for DheeraAI Enterprise users. {CommonProxyErrors.not_premium_user.value}"
...
def is_known_vector_store_index(index_name: str) -> bool:
    ...
    if dheera_ai.vector_store_index_registry is None:
        return False
    return index_name in dheera_ai.vector_store_index_registry.get_vector_store_indexes()
...
async def count_tokens_with_anthropic_api(
    ...
    try:
        ...
        if deployment is not None:
            anthropic_api_key = deployment.get("dheera_ai_params", {}).get("api_key")
        ...
        verbose_proxy_logger.warning(
            "Anthropic library not available, falling back to DheeraAI tokenizer"
        )
    except Exception as e:
        verbose_proxy_logger.warning(
            f"Error calling Anthropic API: {e}, falling back to DheeraAI tokenizer"
        )
    return None
...
async def get_available_models_for_user(
    ...
    Args:
        ...
        llm_router: DheeraAI router instance
    ...
    from dheera_ai.proxy.management_endpoints.team_endpoints import validate_membership
