...
from dheera_ai.types.vector_stores import IndexCreateRequest
...
def _update_request_data_with_dheera_ai_managed_vector_store_registry(
    ...
    """
    Update the request data with the dheera_ai managed vector store registry.

    """
    if dheera_ai.vector_store_registry is not None:
        vector_store_to_run: Optional[DheeraAI_ManagedVectorStore] = (
...
async def index_create(
    ...
    curl -L -X POST 'http://0.0.0.0:4000/indexes/create' \
        ...
        -H 'DheeraAI-Beta: indexes_beta=v1' \
        -d '{ 
            "index_name": "dall-e-3",
