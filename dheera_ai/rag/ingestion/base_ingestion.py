...
from dheera_ai._uuid import uuid4
from dheera_ai.constants import DEFAULT_CHUNK_OVERLAP, DEFAULT_CHUNK_SIZE
from dheera_ai.llms.custom_httpx.http_handler import (
    get_async_httpx_client,
    httpxSpecialProvider,
)
from dheera_ai.rag.text_splitters import RecursiveCharacterTextSplitter
from dheera_ai.types.rag import RAGIngestOptions, RAGIngestResponse
...
class BaseRAGIngestion(ABC):
    ...
    def __init__(
        ...
        # Load credentials from dheera_ai_credential_name if provided in vector_store config
    ...
    def _load_credentials_from_config(self) -> None:
        ...
        which will be resolved from dheera_ai.credential_list.
        """
        from dheera_ai.dheera_ai_core_utils.credential_accessor import CredentialAccessor

        credential_name = self.vector_store_config.get("dheera_ai_credential_name")
    ...
    async def ocr(
        ...
        if self.router is not None:
            ...
            ocr_response = await dheera_ai.aocr(
                model=ocr_model,
                document={"type": doc_type, url_key: data_url},
            )
    ...
    async def embed(
        ...
        if self.router is not None:
            ...
            response = await dheera_ai.aembedding(model=embedding_model, input=chunks)
