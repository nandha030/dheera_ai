...
from dheera_ai.vector_store_files.main import acreate as vector_store_file_acreate
from dheera_ai.vector_stores.main import acreate as vector_store_acreate
...
class OpenAIRAGIngestion(BaseRAGIngestion):
    ...
    async def store(
        ...
        # Get credentials from vector_store_config (loaded from dheera_ai_credential_name if provided)
        ...
        if not vector_store_id:
            ...
            create_response = await vector_store_acreate(
                name=self.ingest_name or "dheera_ai-rag-ingest",
        ...
        if file_content and filename and vector_store_id:
            ...
            file_response = await dheera_ai.acreate_file(
