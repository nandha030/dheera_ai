...
from dheera_ai.proxy._types import SpendLogsPayload
from dheera_ai.proxy.spend_tracking.cold_storage_handler import ColdStorageHandler
...
from dheera_ai.types.utils import ChatCompletionMessageToolCall, Message, ModelResponse
...
class ResponsesSessionHandler:
    ...
    async def get_chat_completion_message_history_for_previous_response_id(
        ...
        dheera_ai_session_id: Optional[str] = None
        if len(all_spend_logs) > 0:
            dheera_ai_session_id = all_spend_logs[0].get("session_id")
        ...
        return ChatCompletionSession(
            ...
            dheera_ai_session_id=dheera_ai_session_id,
        )
    ...
    async def extend_chat_completion_message_with_spend_log_payload(
        ...
        if response_input_param:
            chat_completion_messages = DheeraAICompletionResponsesConfig.transform_responses_api_input_to_messages(
    ...
    def _should_check_cold_storage_for_full_payload(
        ...
        1. `DHEERA_AI_TRUNCATED_PAYLOAD_FIELD` is in the proxy server request dict
        2. `dheera_ai.cold_storage_custom_logger` is not None
        """
        from dheera_ai.constants import DHEERA_AI_TRUNCATED_PAYLOAD_FIELD
        configured_cold_storage_custom_logger = dheera_ai.cold_storage_custom_logger
        ...
        if len(proxy_server_request_dict) == 0:
            return True
        if DHEERA_AI_TRUNCATED_PAYLOAD_FIELD in str(proxy_server_request_dict):
            return True
        return False
