...
from dheera_ai.responses.streaming_iterator import ResponsesAPIStreamingIterator
...
from dheera_ai.types.utils import Delta as ChatCompletionDelta
...
class DheeraAICompletionStreamingIterator(ResponsesAPIStreamingIterator):
    """
    ...
    def __init__(
        self,
        model: str,
        dheera_ai_custom_stream_wrapper: dheera_ai.CustomStreamWrapper,
        request_input: Union[str, ResponseInputParam],
        responses_api_request: ResponsesAPIOptionalRequestParams,
        custom_llm_provider: Optional[str] = None,
        dheera_ai_metadata: Optional[dict] = None,
    ):
        self.model: str = model
        self.dheera_ai_custom_stream_wrapper: dheera_ai.CustomStreamWrapper = (
            dheera_ai_custom_stream_wrapper
        )
        ...
        self.dheera_ai_metadata: Optional[dict] = dheera_ai_metadata or {}
        self.collected_chat_completion_chunks: List[ModelResponseStream] = []
        self.finished: bool = False
        self.dheera_ai_logging_obj = dheera_ai_custom_stream_wrapper.logging_obj
        ...
        self.dheera_ai_model_response: Optional[
    ...
    def _default_response_created_event_data(self) -> dict:
        ...
        if "tool_choice" in self.responses_api_request:
            ...
            response_created_event_data["tool_choice"] = DheeraAICompletionResponsesConfig._transform_tool_choice(
    ...
    def create_dheera_ai_model_response(
        self,
        ...
        return cast(
            ...
            stream_chunk_builder(
                ...
                logging_obj=self.dheera_ai_logging_obj,
            ),
        )

    def create_output_text_done_event(
        self, dheera_ai_complete_object: ModelResponse
    ) -> OutputTextDoneEvent:
        return OutputTextDoneEvent(
            ...
            output_index=0,
            content_index=0,
            text=getattr(dheera_ai_complete_object.choices[0].message, "content", "")  # type: ignore
            or "",
        )
    ...
    def create_output_content_part_done_event(
        ...
        text = getattr(dheera_ai_complete_object.choices[0].message, "content", "") or ""  # type: ignore
        reasoning_content = getattr(dheera_ai_complete_object.choices[0].message, "reasoning_content", "") or ""  # type: ignore
        annotations = getattr(dheera_ai_complete_object.choices[0].message, "annotations", None)  # type: ignore
        ...
        if reasoning_content:
            ...
            response_annotations = DheeraAICompletionResponsesConfig._transform_chat_completion_annotations_to_response_output_annotations(
    ...
    def create_output_item_done_event(
        ...
        text = self.dheera_ai_model_response.choices[0].message.content or ""  # type: ignore
        annotations = getattr(self.dheera_ai_model_response.choices[0].message, "annotations", None)  # type: ignore
    ...
    def common_done_event_logic(
        ...
        if not self.dheera_ai_model_response or isinstance(
            self.dheera_ai_model_response, TextCompletionResponse
        ):
            self.dheera_ai_model_response = self.create_dheera_ai_model_response()
        if self.dheera_ai_model_response:
            done_event = self.return_default_done_events(self.dheera_ai_model_response)
            if done_event:
                return done_event
        else:
            if sync_mode:
    ...
    async def __anext__(
        ...
        try:
            while True:
                ...
                try:
                    chunk = await self.dheera_ai_custom_stream_wrapper.__anext__()
        ...
        try:
            while True:
                ...
                if result:
                    return result
                try:
                    chunk = self.dheera_ai_custom_stream_wrapper.__next__()
                    self.collected_chat_completion_chunks.append(chunk)
                    response_api_chunk = (
                        self._transform_chat_completion_chunk_to_response_api_chunk(
                            chunk
    ...
    def _get_delta_string_from_streaming_choices(
        ...
        It's unclear how users expect dheera_ai to translate multiple-choices-per-chunk to the responses API output.
        """
        choice = choices[0]
    ...
    def _emit_response_completed_event(
        self, dheera_ai_model_response: ModelResponse
    ) -> Optional[ResponseCompletedEvent]:

        if dheera_ai_model_response:
            # Add cost to usage object if include_cost_in_streaming_usage is True
            if (
                dheera_ai.include_cost_in_streaming_usage
                and self.dheera_ai_logging_obj is not None
            ):
                usage = getattr(dheera_ai_model_response, "usage", None)
                if usage is not None:
                    setattr(
                        usage,
                        "cost",
                        self.dheera_ai_logging_obj._response_cost_calculator(
                            result=dheera_ai_model_response
                        ),
                    )

            # Transform the response
            responses_api_response = DheeraAICompletionResponsesConfig.transform_chat_completion_response_to_responses_api_response(
                request_input=self.request_input,
                chat_completion_response=dheera_ai_model_response,
            ...
            encoded_response = ResponsesAPIRequestUtils._update_responses_api_response_id_with_model_id(
                ...
                dheera_ai_metadata=self.dheera_ai_metadata,
            )
