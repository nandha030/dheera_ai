"""
Handles transforming from Responses API -> DheeraAI completion  (Chat Completion API)
"""
...
from dheera_ai.responses.dheera_ai_completion_transformation.session_handler import (
...
class ChatCompletionSession(TypedDict, total=False):
    ...
    dheera_ai_session_id: Optional[str]


########### End of Initialize Classes used for Responses API  ###########


class DheeraAICompletionResponsesConfig:
    @staticmethod
    def get_supported_openai_params(model: str) -> list:
        """
        DheeraAI Adapter from OpenAI Responses API to Chat Completion API supports a subset of OpenAI Responses API params
        """
        return [
            "input",
            "model",
    ...
    def transform_responses_api_request_to_chat_completion_request(
        ...
        **kwargs,
        ...
        (
            tools,
            web_search_options,
        ) = DheeraAICompletionResponsesConfig.transform_responses_api_tools_to_chat_completion_tools(
        ...
        if text_param:
            response_format = DheeraAICompletionResponsesConfig._transform_text_format_to_response_format(
                text_param
            )

        dheera_ai_completion_request: dict = {
            "messages": DheeraAICompletionResponsesConfig.transform_responses_api_input_to_messages(
                input=input,
                responses_api_request=responses_api_request,
            ),
            "model": model,
            "tool_choice": DheeraAICompletionResponsesConfig._transform_tool_choice(
        ...
        # Responses API `Completed` events require usage, we pass `stream_options` to dheera_ai.completion to include usage
        if stream is True:
            stream_options = {
                "include_usage": True,
            }
            dheera_ai_completion_request["stream_options"] = stream_options
            dheera_ai_logging_obj: Optional[DheeraAILoggingObj] = kwargs.get(
                "dheera_ai_logging_obj"
            )
            if dheera_ai_logging_obj:
                dheera_ai_logging_obj.stream_options = stream_options

        # only pass non-None values
        dheera_ai_completion_request = {
            k: v for k, v in dheera_ai_completion_request.items() if v is not None
        }
    ...
    def transform_responses_api_input_to_messages(
        ...
        if responses_api_request.get("instructions"):
            messages.append(
                DheeraAICompletionResponsesConfig.transform_instructions_to_system_message(
                    responses_api_request.get("instructions")
                )
            )

        messages.extend(
            DheeraAICompletionResponsesConfig._transform_response_input_param_to_chat_completion_message(
                input=input,
            )
        )
    ...
    async def async_responses_api_session_handler(
        ...
        chat_completion_session = ChatCompletionSession(
            messages=[], dheera_ai_session_id=None
        )
        ...
        _messages = dheera_ai_completion_request.get("messages") or []
        session_messages = chat_completion_session.get("messages") or []
        dheera_ai_completion_request["messages"] = session_messages + _messages
        dheera_ai_completion_request["dheera_ai_trace_id"] = chat_completion_session.get(
            "dheera_ai_session_id"
        )
        return dheera_ai_completion_request
    ...
    def _transform_response_input_param_to_chat_completion_message(
        ...
        if isinstance(input, str):
            ...
            for _input in input:
                chat_completion_messages = DheeraAICompletionResponsesConfig._transform_responses_api_input_item_to_chat_completion_message(
                    input_item=_input
                )
                ...
                #########################################################
                if DheeraAICompletionResponsesConfig._is_input_item_tool_call_output(
                    input_item=_input
                ):
    ...
    def _transform_responses_api_input_item_to_chat_completion_message(
        ...
        - ItemReference
        """
        if DheeraAICompletionResponsesConfig._is_input_item_tool_call_output(input_item):
            # handle executed tool call results
            return DheeraAICompletionResponsesConfig._transform_responses_api_tool_call_output_to_chat_completion_message(
                tool_call_output=input_item
            )
        elif DheeraAICompletionResponsesConfig._is_input_item_function_call(input_item):
            # handle function call input items
            return DheeraAICompletionResponsesConfig._transform_responses_api_function_call_to_chat_completion_message(
            ...
            return [
                GenericChatCompletionMessage(
                    ...
                    content=DheeraAICompletionResponsesConfig._transform_responses_api_content_to_chat_completion_content(
                        content
                    ),
                )
            ]
            ...
            for item in content:
                if isinstance(item, str):
                    ...
                    if item.get("type") == "input_file":
                        content_list.append(
                            DheeraAICompletionResponsesConfig._transform_input_file_item_to_file_item(
                                item
                            )
                        )
                        ...
                        content_list.append(
                            dict(
                                DheeraAICompletionResponsesConfig._transform_input_image_item_to_image_item(
                                    item
                                )
                            )
                        )
                        ...
                        content_list.append(
                            {
                                "type": DheeraAICompletionResponsesConfig._get_chat_completion_request_content_type(
    ...
    def transform_chat_completion_response_to_responses_api_response(
        ...
        responses_api_response: ResponsesAPIResponse = ResponsesAPIResponse(
            ...
            output=DheeraAICompletionResponsesConfig._transform_chat_completion_choices_to_responses_output(
            ...
            reasoning=Reasoning(),
            status=DheeraAICompletionResponsesConfig._map_chat_completion_finish_reason_to_responses_status(
                finish_reason
            ),
            text={},
            truncation=getattr(chat_completion_response, "truncation", None),
            usage=DheeraAICompletionResponsesConfig._transform_chat_completion_usage_to_responses_usage(
                chat_completion_response=chat_completion_response
            ),
    ...
    def _transform_chat_completion_choices_to_responses_output(
        ...
        responses_output.extend(
            DheeraAICompletionResponsesConfig._extract_reasoning_output_items(
                chat_completion_response, choices
            )
        )
        responses_output.extend(
            DheeraAICompletionResponsesConfig._extract_message_output_items(
                chat_completion_response, choices
            )
        )
        responses_output.extend(
            DheeraAICompletionResponsesConfig.transform_chat_completion_tools_to_responses_tools(
                chat_completion_response=chat_completion_response
            )
        )
    ...
    def _extract_image_generation_output_items(
        ...
        for idx, image_item in enumerate(images):
            ...
            base64_data = (
                DheeraAICompletionResponsesConfig._extract_base64_from_data_url(
                    image_url
                )
            )
            ...
            if base64_data:
                image_generation_items.append(
                    OutputImageGenerationCall(
                        ...
                        status=DheeraAICompletionResponsesConfig._map_finish_reason_to_image_generation_status(
                            choice.finish_reason
                        ),
                        result=base64_data,
                    )
    ...
    def _extract_message_output_items(
        ...
        for choice in choices:
            ...
            if hasattr(choice.message, "images") and choice.message.images:
                ...
                image_generation_items = DheeraAICompletionResponsesConfig._extract_image_generation_output_items(
                    chat_completion_response=chat_completion_response,
                    choice=choice,
                )
                ...
                message_output_items.append(
                    GenericResponseOutputItem(
                        ...
                        content=[
                            DheeraAICompletionResponsesConfig._transform_chat_message_to_response_output_text(
                                choice.message
                            )
                        ],
                    )
    ...
    def _transform_responses_api_outputs_to_chat_completion_messages(
        ...
        for _output_item in output_items:
            ...
            if output_item.get("type") == "function_call":
                ...
                messages.append(
                    DheeraAICompletionResponsesConfig._transform_responses_output_tool_call_to_chat_completion_output_tool_call(
    ...
    def _transform_chat_message_to_response_output_text(
        ...
        transformed_annotations = DheeraAICompletionResponsesConfig._transform_chat_completion_annotations_to_response_output_annotations(
