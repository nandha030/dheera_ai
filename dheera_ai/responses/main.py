...
from dheera_ai.responses.dheera_ai_completion_transformation.handler import (
    DheeraAICompletionTransformationHandler,
)
...
if TYPE_CHECKING:
    from dheera_ai.types.llms.openai import ResponseText  # type: ignore
else:
...
dheera_ai_completion_transformation_handler = DheeraAICompletionTransformationHandler()
#################################################
...
async def aresponses_api_with_mcp(
    ...
    **kwargs,
    ...
    When MCP tools with server_url="dheera_ai_proxy" are provided, this function will:
    ...
    (
        ...
        other_tools,
    ) = DheeraAI_Proxy_MCP_Handler._parse_mcp_tools(tools)

    # Process MCP tools through the complete pipeline (fetch + filter + deduplicate + transform)
    # Extract user_api_key_auth from dheera_ai_metadata (where it's added by add_user_api_key_auth_to_request_metadata)
    user_api_key_auth = kwargs.get("user_api_key_auth") or kwargs.get("dheera_ai_metadata", {}).get("user_api_key_auth")
    ...
    if stream and mcp_tools_with_dheera_ai_proxy:
        ...
        return DheeraAI_Proxy_MCP_Handler._create_mcp_streaming_response(
            input=input,
            model=model,
            all_tools=all_tools,
            mcp_tools_with_dheera_ai_proxy=mcp_tools_with_dheera_ai_proxy,
    ...
    should_auto_execute = bool(
        mcp_tools_with_dheera_ai_proxy
    ) and DheeraAI_Proxy_MCP_Handler._should_auto_execute_tools(
        mcp_tools_with_dheera_ai_proxy=mcp_tools_with_dheera_ai_proxy
    )

    # Prepare parameters for the initial call
    initial_call_params = DheeraAI_Proxy_MCP_Handler._prepare_initial_call_params(
    ...
    if should_auto_execute and isinstance(
        ...
        tool_calls = DheeraAI_Proxy_MCP_Handler._extract_tool_calls_from_response(
            response=response
        )

        if tool_calls:
            user_api_key_auth = kwargs.get("dheera_ai_metadata", {}).get(
                "user_api_key_auth"
            )
            ...
            tool_results = await DheeraAI_Proxy_MCP_Handler._execute_tool_calls(
                tool_server_map=tool_server_map,
            ...
            if tool_results:
                follow_up_input = DheeraAI_Proxy_MCP_Handler._create_follow_up_input(
                ...
                follow_up_call_params = (
                    DheeraAI_Proxy_MCP_Handler._prepare_follow_up_call_params(
                ...
                if stream:
                    tool_execution_events = (
                        DheeraAI_Proxy_MCP_Handler._create_tool_execution_events(
                            tool_calls=tool_calls, tool_results=tool_results
                        )
                    )

                final_response = await DheeraAI_Proxy_MCP_Handler._make_follow_up_call(
                ...
                # Add custom output elements to the final response (for non-streaming)
                    ...
                    final_response = (
                        DheeraAI_Proxy_MCP_Handler._add_mcp_output_elements_to_response(
...
def responses(
    ...
    **kwargs,
    ...
    try:
        ...
        #########################################################
        if DheeraAI_Proxy_MCP_Handler._should_use_dheera_ai_mcp_gateway(tools=tools):
        ...
        if responses_api_provider_config is None:
            return dheera_ai_completion_transformation_handler.response_api_handler(
                model=model,
                input=input,
    ...
    **kwargs,
    ...
    try:
        ...
        dheera_ai_logging_obj.update_environment_variables(
            ...
            dheera_ai_params={"dheera_ai_call_id": dheera_ai_call_id},
            custom_llm_provider=custom_llm_provider,
        )
