...
from dheera_ai.types.llms.openai import ToolParam
...
async def handle_chat_completion_with_mcp(
    ...
    if not DheeraAI_Proxy_MCP_Handler._should_use_dheera_ai_mcp_gateway(
        tools=tools_for_mcp
    ):
        return None

    mcp_tools, _ = DheeraAI_Proxy_MCP_Handler._parse_mcp_tools(tools)
    if not mcp_tools:
        return None
    ...
    (
        ...
        mcp_tools_with_dheera_ai_proxy=mcp_tools,
    )
    ...
    should_auto_execute = DheeraAI_Proxy_MCP_Handler._should_auto_execute_tools(
        mcp_tools_with_dheera_ai_proxy=mcp_tools
    )
    ...
    follow_up_messages = DheeraAI_Proxy_MCP_Handler._create_follow_up_messages_for_chat(
