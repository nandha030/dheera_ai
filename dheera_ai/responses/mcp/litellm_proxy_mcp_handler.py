...
from dheera_ai.proxy._experimental.mcp_server.utils import split_server_prefix_from_name
from dheera_ai.responses.main import aresponses
from dheera_ai.responses.streaming_iterator import BaseResponsesAPIStreamingIterator
from dheera_ai.types.llms.openai import ResponsesAPIResponse, ToolParam
from dheera_ai.types.utils import Choices, ModelResponse
...
DHEERA_AI_PROXY_MCP_SERVER_URL = "dheera_ai_proxy"
DHEERA_AI_PROXY_MCP_SERVER_URL_PREFIX = f"{DHEERA_AI_PROXY_MCP_SERVER_URL}/mcp/"


class DheeraAI_Proxy_MCP_Handler:
    """
    Helper class with static methods for MCP integration with Responses API.

    This handles when a user passes mcp server_url="dheera_ai_proxy" in their tools.
    """

    @staticmethod
    def _should_use_dheera_ai_mcp_gateway(tools: Optional[Iterable[ToolParam]]) -> bool:
        """
        Returns True if the user passed a MCP tool with server_url="dheera_ai_proxy"
        """
        if tools:
            for tool in tools:
                if isinstance(tool, dict) and tool.get("type") == "mcp":
                    ...
                    if isinstance(server_url, str) and server_url.startswith(
                        DHEERA_AI_PROXY_MCP_SERVER_URL
                    ):
                        return True
        return False
    ...
    def _parse_mcp_tools(
        ...
        """
        Parse tools and separate MCP tools with dheera_ai_proxy from other tools.

        Returns:
            Tuple of (mcp_tools_with_dheera_ai_proxy, other_tools)
        """
        mcp_tools_with_dheera_ai_proxy: List[ToolParam] = []
            ...
            for tool in tools:
                if isinstance(tool, dict) and tool.get("type") == "mcp":
                    ...
                    if isinstance(server_url, str) and server_url.startswith(
                        ...
                        mcp_tools_with_dheera_ai_proxy.append(tool)
                    else:
                        other_tools.append(tool)
                else:
                    other_tools.append(tool)

        return mcp_tools_with_dheera_ai_proxy, other_tools

    @staticmethod
    async def _get_mcp_tools_from_manager(
        user_api_key_auth: Any,
        mcp_tools_with_dheera_ai_proxy: Optional[Iterable[ToolParam]],
        ...
        Args:
            ...
            mcp_tools_with_dheera_ai_proxy: ToolParam objects with server_url starting with "dheera_ai_proxy"
        ...
        if mcp_tools_with_dheera_ai_proxy:
            for _tool in mcp_tools_with_dheera_ai_proxy:
                # if user specifies servers as server_url: dheera_ai_proxy/mcp/zapier,github then return zapier,github
                server_url = (
                    _tool.get("server_url", "") if isinstance(_tool, dict) else ""
                )
                if isinstance(server_url, str) and server_url.startswith(
                    DHEERA_AI_PROXY_MCP_SERVER_URL_PREFIX
                ):
    ...
    def _filter_mcp_tools_by_allowed_tools(
        mcp_tools: List[MCPTool], mcp_tools_with_dheera_ai_proxy: List[ToolParam]
    ) -> List[MCPTool]:
        ...
        for tool_config in mcp_tools_with_dheera_ai_proxy:
    ...
    async def _process_mcp_tools_to_openai_format(
        user_api_key_auth: Any, mcp_tools_with_dheera_ai_proxy: List[ToolParam]
        ...
        (
            ...
            tool_server_map,
        ) = await DheeraAI_Proxy_MCP_Handler._process_mcp_tools_without_openai_transform(
            user_api_key_auth,
            mcp_tools_with_dheera_ai_proxy,
        )

        openai_tools = DheeraAI_Proxy_MCP_Handler._transform_mcp_tools_to_openai(
            deduplicated_mcp_tools
        )
    ...
    async def _process_mcp_tools_without_openai_transform(
        ...
        """
        if not mcp_tools_with_dheera_ai_proxy:
            return [], {}
        ...
        (
            mcp_tools_fetched,
            allowed_mcp_servers,
        ) = await DheeraAI_Proxy_MCP_Handler._get_mcp_tools_from_manager(
            user_api_key_auth=user_api_key_auth,
            mcp_tools_with_dheera_ai_proxy=mcp_tools_with_dheera_ai_proxy,
        )

        # Step 2: Filter tools based on allowed_tools parameter
        filtered_mcp_tools = (
            DheeraAI_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        ...
        (
            deduplicated_mcp_tools,
            tool_server_map,
        ) = DheeraAI_Proxy_MCP_Handler._deduplicate_mcp_tools(
    ...
    def _transform_mcp_tools_to_openai(
        ...
        from dheera_ai.experimental_mcp_client.tools import (
    ...
    def _should_auto_execute_tools(
        mcp_tools_with_dheera_ai_proxy: Union[List[Dict[str, Any]], List[ToolParam]],
    ) -> bool:
        ...
        """
        for tool in mcp_tools_with_dheera_ai_proxy:
            if isinstance(tool, dict):
        ...
        for tool_call in tool_calls:
            try:
                (
                    ...
                    tool_arguments,
                    tool_call_id,
                ) = DheeraAI_Proxy_MCP_Handler._extract_tool_call_details(tool_call)

                if not tool_name:
                    verbose_logger.warning(f"Tool call missing name: {tool_call}")
                    continue

                parsed_arguments = DheeraAI_Proxy_MCP_Handler._parse_tool_arguments(
                    tool_arguments
                )
                ...
                result_text = DheeraAI_Proxy_MCP_Handler._parse_mcp_result(result)
                tool_results.append(
                    {
    ...
    def _create_follow_up_messages_for_chat(
        ...
        from dheera_ai.utils import convert_list_message_to_dict
    ...
    def _create_mcp_streaming_response(
        ...
        model: str,
        all_tools: Optional[List[Any]],
        mcp_tools_with_dheera_ai_proxy: List[Any],
        ...
        **kwargs,
        ...
        """
        from dheera_ai.responses.mcp.mcp_streaming_iterator import (
    ...
    def _create_tool_execution_events(
        ...
        from dheera_ai.responses.mcp.mcp_streaming_iterator import create_mcp_call_events
    ...
    def _add_mcp_output_elements_to_response(
        ...
        from dheera_ai.types.responses.main import GenericResponseOutputItem, OutputText
