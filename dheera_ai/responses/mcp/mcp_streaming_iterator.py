...
async def create_mcp_list_tools_events(
    ...
    try:
        ...
        from dheera_ai.types.llms.openai import OutputItemDoneEvent

        # Extract server label from the first MCP tool config
        server_label = ""
        if mcp_tools_with_dheera_ai_proxy:
            first_tool = mcp_tools_with_dheera_ai_proxy[0]
...
class MCPEnhancedStreamingIterator(BaseResponsesAPIStreamingIterator):
    ...
    def __init__(
        ...
        mcp_tools_with_dheera_ai_proxy: Optional[List[Any]] = None,
        user_api_key_auth: Any = None,
        original_request_params: Optional[Dict[str, Any]] = None,
    ):
        # MCP setup
        self.mcp_tools_with_dheera_ai_proxy = mcp_tools_with_dheera_ai_proxy or []
    ...
    def _should_auto_execute_tools(self) -> bool:
        ...
        return DheeraAI_Proxy_MCP_Handler._should_auto_execute_tools(
            self.mcp_tools_with_dheera_ai_proxy
        )
    ...
    async def _create_initial_response_iterator(self) -> None:
        ...
        try:
            ...
            if hasattr(response, "__aiter__") or hasattr(response, "__iter__"):
                ...
                self.dheera_ai_metadata = getattr(response, "dheera_ai_metadata", {})
    ...
    async def _generate_tool_execution_events(self) -> None:
        ...
        try:
            ...
            if self.collected_response is not None:
                tool_calls = DheeraAI_Proxy_MCP_Handler._extract_tool_calls_from_response(self.collected_response)  # type: ignore[arg-type]
            else:
                tool_calls = []
            if not tool_calls:
                return
                ...
                output_item_done_event = OutputItemDoneEvent(
                    ...
                    item=BaseDheeraAIOpenAIResponseObject(
                        **{
                            ...
                            "name": tool_name,
                            "output": result_text,
                            "server_label": "dheera_ai",  # or extract from tool config
                        }
                    ),
                )
