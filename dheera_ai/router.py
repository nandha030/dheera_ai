...
import dheera_ai.dheera_ai_core_utils.exception_mapping_utils
from dheera_ai import get_secret_str
from dheera_ai._logging import verbose_router_logger
from dheera_ai._uuid import uuid
from dheera_ai.caching.caching import (
    DualCache,
    InMemoryCache,
    RedisCache,
    RedisClusterCache,
)
from dheera_ai.constants import DEFAULT_MAX_LRU_CACHE_SIZE
from dheera_ai.integrations.custom_logger import CustomLogger
from dheera_ai.dheera_ai_core_utils.asyncify import run_async_function
...
from dheera_ai.dheera_ai_core_utils.dd_tracing import tracer
...
from dheera_ai.router_strategy.lowest_tpm_rpm import LowestTPMLoggingHandler
from dheera_ai.router_strategy.lowest_tpm_rpm_v2 import LowestTPMLoggingHandler_v2
from dheera_ai.router_strategy.simple_shuffle import simple_shuffle
from dheera_ai.router_strategy.tag_based_routing import get_deployments_for_tag
from dheera_ai.router_utils.add_retry_fallback_headers import (
...
from dheera_ai.router_utils.client_initalization_utils import InitalizeCachedClient
from dheera_ai.router_utils.clientside_credential_handler import (
    get_dynamic_dheera_ai_params,
    is_clientside_credential,
)
from dheera_ai.router_utils.common_utils import (
    filter_team_based_models,
    filter_web_search_deployments,
)
from dheera_ai.router_utils.cooldown_cache import CooldownCache
...
from dheera_ai.router_utils.handle_error import (
    async_raise_no_deployment_exception,
    send_llm_exception_alert,
)
from dheera_ai.router_utils.pre_call_checks.prompt_caching_deployment_check import (
    PromptCachingDeploymentCheck,
)
from dheera_ai.router_utils.pre_call_checks.responses_api_deployment_check import (
...
from dheera_ai.types.router import (
    ...
    VALID_DHEERA_AI_ENVIRONMENTS,
    AlertingConfig,
    AllowedFailsPolicy,
    AssistantsTypedDict,
    CredentialDheeraAIParams,
...
from dheera_ai.types.services import ServiceTypes
from dheera_ai.types.utils import (
    CustomPricingDheeraAIParams,
    GenericBudgetConfigType,
    DheeraAIBatch,
)
from dheera_ai.types.utils import ModelInfo
...
if TYPE_CHECKING:
    ...
    from dheera_ai.router_strategy.auto_router.auto_router import (
        AutoRouter,
...
class Router:
    ...
    def __init__(  # noqa: PLR0915
        ...
        default_dheera_ai_params: Optional[
            dict
        ...
        Args:
            ...
            default_dheera_ai_params (dict): Default parameters for Router.chat.completion.create. Defaults to {}.
        ...
        Returns:
            Router: An instance of the dheera_ai.Router class.
        ...
        dheera_ai.suppress_debug_info = True  # prevents 'Give Feedback/Get help' message from being emitted on Router - Relevant Issue: https://github.com/BerriAI/dheera_ai/issues/5942
        ...
        self.deployment_names: List = (
            []
        )  # names of models under dheera_ai_params. ex. azure/chatgpt-v-2
        ...
        if cache_responses:
            if dheera_ai.cache is None:
                # the cache can be initialized on the proxy server. We should not overwrite it
                dheera_ai.cache = dheera_ai.Cache(type=cache_type, **cache_config)  # type: ignore
        ...
        if model_list is not None:
            ...
            for m in model_list:
                if "model" in m["dheera_ai_params"]:
                    self.deployment_latency_map[m["dheera_ai_params"]["model"]] = 0
        else:
        ...
        if allowed_fails is not None:
            ...
            self.allowed_fails = dheera_ai.allowed_fails
        ...
        elif dheera_ai.num_retries is not None:
            self.num_retries = dheera_ai.num_retries
        else:
            self.num_retries = openai.DEFAULT_MAX_RETRIES

        if max_fallbacks is not None:
            self.max_fallbacks = max_fallbacks
        elif dheera_ai.max_fallbacks is not None:
            self.max_fallbacks = dheera_ai.max_fallbacks
        else:
            self.max_fallbacks = dheera_ai.ROUTER_MAX_FALLBACKS

        self.timeout = timeout or dheera_ai.request_timeout
        ...
        _fallbacks = fallbacks or dheera_ai.fallbacks

        self.validate_fallbacks(fallback_param=_fallbacks)
        ### set fallbacks
        self.fallbacks = _fallbacks

        if default_fallbacks is not None or dheera_ai.default_fallbacks is not None:
            _fallbacks = default_fallbacks or dheera_ai.default_fallbacks
        ...
        self.context_window_fallbacks = (
            context_window_fallbacks or dheera_ai.context_window_fallbacks
        )

        _content_policy_fallbacks = (
            content_policy_fallbacks or dheera_ai.content_policy_fallbacks
        )
        ...
        default_dheera_ai_params = default_dheera_ai_params or {}
        self.chat = dheera_ai.Chat(params=default_dheera_ai_params, router_obj=self)

        # default dheera_ai args
        self.default_dheera_ai_params = default_dheera_ai_params
        self.default_dheera_ai_params.setdefault("timeout", timeout)
        self.default_dheera_ai_params.setdefault("max_retries", 0)
        self.default_dheera_ai_params.setdefault("metadata", {}).update(
        ...
        ## USAGE TRACKING ##
        if isinstance(dheera_ai._async_success_callback, list):
        ...
        if isinstance(dheera_ai.success_callback, list):
            dheera_ai.logging_callback_manager.add_dheera_ai_success_callback(
                self.sync_deployment_callback_on_success
            )
        else:
            dheera_ai.success_callback = [self.sync_deployment_callback_on_success]
        if isinstance(dheera_ai._async_failure_callback, list):
        ...
        ## COOLDOWNS ##
        if isinstance(dheera_ai.failure_callback, list):
            dheera_ai.logging_callback_manager.add_dheera_ai_failure_callback(
                self.deployment_callback_on_failure
            )
        else:
            dheera_ai.failure_callback = [self.deployment_callback_on_failure]
    ...
    def discard(self):
        ...
        """
        dheera_ai.logging_callback_manager.remove_callback_from_list_by_object(
            dheera_ai._async_success_callback, self
        )
        dheera_ai.logging_callback_manager.remove_callback_from_list_by_object(
            dheera_ai.success_callback, self
        )
        dheera_ai.logging_callback_manager.remove_callback_from_list_by_object(
            dheera_ai._async_failure_callback, self
        )
        dheera_ai.logging_callback_manager.remove_callback_from_list_by_object(
            dheera_ai.failure_callback, self
        )
        dheera_ai.logging_callback_manager.remove_callback_from_list_by_object(
            dheera_ai.input_callback, self
        )
        dheera_ai.logging_callback_manager.remove_callback_from_list_by_object(
            dheera_ai.service_callback, self
        )
        dheera_ai.logging_callback_manager.remove_callback_from_list_by_object(
            dheera_ai.callbacks, self
        )
        ...
        if self.optional_callbacks is not None:
            for callback in self.optional_callbacks:
                dheera_ai.logging_callback_manager.remove_callback_from_list_by_object(
                    dheera_ai.callbacks, callback, require_self=False
                )
    ...
    def routing_strategy_init(
        ...
        if (
            ...
            ## add callback
            if isinstance(dheera_ai.input_callback, list):
                dheera_ai.input_callback.append(self.leastbusy_logger)  # type: ignore
            else:
                dheera_ai.input_callback = [self.leastbusy_logger]  # type: ignore
            if isinstance(dheera_ai.callbacks, list):
                dheera_ai.logging_callback_manager.add_dheera_ai_callback(self.leastbusy_logger)  # type: ignore
        elif (
            ...
            if isinstance(dheera_ai.callbacks, list):
                dheera_ai.logging_callback_manager.add_dheera_ai_callback(self.lowesttpm_logger)  # type: ignore
        elif (
            ...
            if isinstance(dheera_ai.callbacks, list):
                dheera_ai.logging_callback_manager.add_dheera_ai_callback(self.lowesttpm_logger_v2)  # type: ignore
        elif (
            ...
            if isinstance(dheera_ai.callbacks, list):
                dheera_ai.logging_callback_manager.add_dheera_ai_callback(self.lowestlatency_logger)  # type: ignore
        elif (
            ...
            if isinstance(dheera_ai.callbacks, list):
                dheera_ai.logging_callback_manager.add_dheera_ai_callback(self.lowestcost_logger)  # type: ignore
        else:
            pass

    def initialize_assistants_endpoint(self):
        ## INITIALIZE PASS THROUGH ASSISTANTS ENDPOINT ##
        self.acreate_assistants = self.factory_function(dheera_ai.acreate_assistants)
        self.adelete_assistant = self.factory_function(dheera_ai.adelete_assistant)
        self.aget_assistants = self.factory_function(dheera_ai.aget_assistants)
        self.acreate_thread = self.factory_function(dheera_ai.acreate_thread)
        self.aget_thread = self.factory_function(dheera_ai.aget_thread)
        self.a_add_message = self.factory_function(dheera_ai.a_add_message)
        self.aget_messages = self.factory_function(dheera_ai.aget_messages)
        self.arun_thread = self.factory_function(dheera_ai.arun_thread)

    def _initialize_core_endpoints(self):
        """Helper to initialize core router endpoints."""
        self.amoderation = self.factory_function(
            dheera_ai.amoderation, call_type="moderation"
        )
        self.aanthropic_messages = self.factory_function(
            dheera_ai.anthropic_messages, call_type="anthropic_messages"
        )
        self.anthropic_messages = self.factory_function(
            dheera_ai.anthropic_messages, call_type="anthropic_messages"
        )
        self.agenerate_content = self.factory_function(
            dheera_ai.agenerate_content, call_type="agenerate_content"
        )
        self.aadapter_generate_content = self.factory_function(
            dheera_ai.aadapter_generate_content, call_type="aadapter_generate_content"
        )
        self.aresponses = self.factory_function(
            dheera_ai.aresponses, call_type="aresponses"
        )
        self.afile_delete = self.factory_function(
            dheera_ai.afile_delete, call_type="afile_delete"
        )
        self.afile_content = self.factory_function(
            dheera_ai.afile_content, call_type="afile_content"
        )
        self.responses = self.factory_function(dheera_ai.responses, call_type="responses")
        self.aget_responses = self.factory_function(
            dheera_ai.aget_responses, call_type="aget_responses"
        )
        self.acancel_responses = self.factory_function(
            dheera_ai.acancel_responses, call_type="acancel_responses"
        )
        self.adelete_responses = self.factory_function(
            dheera_ai.adelete_responses, call_type="adelete_responses"
        )
        self.alist_input_items = self.factory_function(
            dheera_ai.alist_input_items, call_type="alist_input_items"
        )
        self._arealtime = self.factory_function(
            dheera_ai._arealtime, call_type="_arealtime"
        )
        self.acreate_fine_tuning_job = self.factory_function(
            dheera_ai.acreate_fine_tuning_job, call_type="acreate_fine_tuning_job"
        )
        self.acancel_fine_tuning_job = self.factory_function(
            dheera_ai.acancel_fine_tuning_job, call_type="acancel_fine_tuning_job"
        )
        self.alist_fine_tuning_jobs = self.factory_function(
            dheera_ai.alist_fine_tuning_jobs, call_type="alist_fine_tuning_jobs"
        )
        self.aretrieve_fine_tuning_job = self.factory_function(
            dheera_ai.aretrieve_fine_tuning_job, call_type="aretrieve_fine_tuning_job"
        )
        self.afile_list = self.factory_function(
            dheera_ai.afile_list, call_type="alist_files"
        )
        self.aimage_edit = self.factory_function(
            dheera_ai.aimage_edit, call_type="aimage_edit"
        )
        self.allm_passthrough_route = self.factory_function(
            dheera_ai.allm_passthrough_route, call_type="allm_passthrough_route"
        )
        self.acancel_batch = self.factory_function(
            dheera_ai.acancel_batch, call_type="acancel_batch"
        )

    def _initialize_vector_store_endpoints(self):
        """Initialize vector store endpoints."""
        from dheera_ai.vector_stores.main import acreate, asearch, create, search
    ...
    def _initialize_vector_store_file_endpoints(self):
        ...
        from dheera_ai.vector_store_files.main import (
            acreate as avector_store_file_create_fn,
        )
        from dheera_ai.vector_store_files.main import (
            adelete as avector_store_file_delete_fn,
        )
        from dheera_ai.vector_store_files.main import alist as avector_store_file_list_fn
        from dheera_ai.vector_store_files.main import (
        ...
        from dheera_ai.vector_store_files.main import list as vector_store_file_list_fn
        from dheera_ai.vector_store_files.main import (
    ...
    def _initialize_skills_endpoints(self):
        ...
        self.acreate_skill = self.factory_function(
            dheera_ai.acreate_skill, call_type="acreate_skill"
        )
        self.alist_skills = self.factory_function(
            dheera_ai.alist_skills, call_type="alist_skills"
        )
        self.aget_skill = self.factory_function(
            dheera_ai.aget_skill, call_type="aget_skill"
        )
        self.adelete_skill = self.factory_function(
            dheera_ai.adelete_skill, call_type="adelete_skill"
        )

    def _initialize_interactions_endpoints(self):
        """Initialize Google Interactions API endpoints."""
        from dheera_ai.interactions import acancel as acancel_interaction
        from dheera_ai.interactions import acreate as acreate_interaction
        from dheera_ai.interactions import adelete as adelete_interaction
        from dheera_ai.interactions import aget as aget_interaction
        from dheera_ai.interactions import cancel as cancel_interaction
        from dheera_ai.interactions import create as create_interaction
        from dheera_ai.interactions import delete as delete_interaction
        from dheera_ai.interactions import get as get_interaction
        ...
        if optional_pre_call_checks is not None:
            for pre_call_check in optional_pre_call_checks:
                ...
                if _callback is not None:
                    ...
                    dheera_ai.logging_callback_manager.add_dheera_ai_callback(_callback)
    ...
    def print_deployment(self, deployment: dict):
        ...
        try:
            ...
            dheera_ai_params: dict = _deployment_copy["dheera_ai_params"]

            if dheera_ai.redact_user_api_key_info:
                masker = SensitiveDataMasker(visible_prefix=2, visible_suffix=0)
                _deployment_copy["dheera_ai_params"] = masker.mask_dict(dheera_ai_params)
            elif "api_key" in dheera_ai_params:
                dheera_ai_params["api_key"] = dheera_ai_params["api_key"][:2] + "*" * 10
    ...
    def _completion(
        ...
        try:
            ...
            data = deployment["dheera_ai_params"]
            ...
            verbose_router_logger.info(
                f"dheera_ai.completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            ...
            if isinstance(response, ModelResponse):
                ...
                if _should_raise:
                    raise dheera_ai.ContentPolicyViolationError(
            ...
            verbose_router_logger.info(
                f"dheera_ai.completion(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            raise e
    ...
    async def acompletion(
        ...
        **kwargs,
        ...
        try:
            ...
            asyncio.create_task(
                send_llm_exception_alert(
                    dheera_ai_router_instance=self,
                    request_kwargs=kwargs,
    ...
    async def _acompletion_streaming_iterator(
        ...
        """
        from dheera_ai.exceptions import MidStreamFallbackError
                    ...
                    if hasattr(fallback_response, "__aiter__"):
                        async for fallback_item in fallback_response:  # type: ignore
                            if (
                                ...
                                from dheera_ai.cost_calculator import (
    ...
    async def _acompletion(
        ...
        try:
            ...
            _response = dheera_ai.acompletion(**input_kwargs)

            logging_obj: Optional[DheeraAILogging] = kwargs.get(
            ...
            verbose_router_logger.info(
                f"dheera_ai.acompletion(model={model_name})\033[32m 200 OK\033[0m"
            )
            # debug how often this deployment picked
            self._track_deployment_metrics(
                deployment=deployment,
            ...
            verbose_router_logger.info(
                f"dheera_ai.acompletion(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
    ...
    def _update_kwargs_before_fallbacks(
        ...
        kwargs.setdefault("dheera_ai_trace_id", str(uuid.uuid4()))
    ...
    def _update_kwargs_with_default_dheera_ai_params(
        self, kwargs: dict, metadata_variable_name: Optional[str] = "metadata"
    ) -> None:
        """
        Adds default dheera_ai params to kwargs, if set.

        Handles inserting this as either "metadata" or "dheera_ai_metadata" depending on the metadata_variable_name
        """
        # 1) copy your defaults and pull out metadata
        defaults = self.default_dheera_ai_params.copy()
        ...
        # 3) merge in metadata, this handles inserting this as either "metadata" or "dheera_ai_metadata"
    ...
    def _handle_clientside_credential(
        ...
        dheera_ai_params = deployment["dheera_ai_params"].copy()
        dynamic_dheera_ai_params = get_dynamic_dheera_ai_params(
            dheera_ai_params=dheera_ai_params, request_kwargs=kwargs
        )
        ...
        deployment_pydantic_obj = Deployment(
            ...
            dheera_ai_params=DheeraAI_Params(**dynamic_dheera_ai_params),
    ...
    def _update_kwargs_with_deployment(
        ...
        """
        model_info = deployment.get("model_info", {}).copy()
        deployment_dheera_ai_model_name = deployment["dheera_ai_params"]["model"]
        deployment_api_base = deployment["dheera_ai_params"].get("api_base")
        ...
        if is_clientside_credential(request_kwargs=kwargs):
            ...
            deployment_dheera_ai_model_name = deployment_pydantic_obj.dheera_ai_params.model
            deployment_api_base = deployment_pydantic_obj.dheera_ai_params.api_base
        ...
        kwargs.setdefault(metadata_variable_name, {}).update(
            {
                "deployment": deployment_dheera_ai_model_name,
                "model_info": model_info,
        ...
        kwargs["timeout"] = self._get_timeout(
            kwargs=kwargs, data=deployment["dheera_ai_params"]
        )

        self._update_kwargs_with_default_dheera_ai_params(
    ...
    def _get_stream_timeout(
        ...
        return (
            ...
            or data.get(
                "stream_timeout", None
            )  # timeout set on dheera_ai_params for this deployment
            or self.stream_timeout  # timeout set on router
            or self.default_dheera_ai_params.get("stream_timeout", None)
        )
    ...
    def _get_non_stream_timeout(
        ...
        timeout = (
            ...
            or self.default_dheera_ai_params.get("timeout", None)
        )
        return timeout
    ...
    async def abatch_completion(
        ...
        **kwargs,
        ...
        1. Batch Process 1 request to N models on dheera_ai.Router. Pass messages as List[Dict[str, str]] to use this
        2. Batch Process N requests to M models on dheera_ai.Router. Pass messages as List[List[Dict[str, str]]] to use this
    ...
    async def abatch_completion_one_model_multiple_requests(
        ...
        """
        Async Batch Completion - Batch Process multiple Messages to one model_group on dheera_ai.Router
        ...
        if make_request:
            try:
                ...
                _response._hidden_params["additional_headers"].update(
                    {"x-dheera_ai-request-prioritization-used": True}
                )
                return _response
            except Exception as e:
    ...
    def _is_prompt_management_model(self, model: str) -> bool:
        ...
        dheera_ai_model = model_list[0]["dheera_ai_params"].get("model", None)
        if dheera_ai_model is None or "/" not in dheera_ai_model:
            return False

        split_dheera_ai_model = dheera_ai_model.split("/")[0]
        return split_dheera_ai_model in dheera_ai._known_custom_logger_compatible_callbacks
    ...
    async def _prompt_management_factory(
        ...
        dheera_ai_logging_object = kwargs.get("dheera_ai_logging_obj", None)
        if dheera_ai_logging_object is None:
            dheera_ai_logging_object, kwargs = function_setup(
                **{
        ...
        dheera_ai_logging_object = cast(DheeraAILogging, dheera_ai_logging_object)
        ...
        data = prompt_management_deployment["dheera_ai_params"].copy()

        dheera_ai_model = data.get("model", None)
        ...
        prompt_variables = kwargs.get(
            "prompt_variables"
        ) or prompt_management_deployment["dheera_ai_params"].get(
        ...
        (
            ...
            messages,
            optional_params,
        ) = dheera_ai_logging_object.get_chat_completion_prompt(
            model=dheera_ai_model,
            messages=messages,
        ...
        kwargs["model"] = model
        kwargs["messages"] = messages
        kwargs["dheera_ai_logging_obj"] = dheera_ai_logging_object
        ...
        if _model_list is None or len(_model_list) == 0:  # if direct call to model
            ...
            return await dheera_ai.acompletion(**kwargs)
    ...
    def _image_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            self._update_kwargs_with_deployment(deployment=deployment, kwargs=kwargs)
            data = deployment["dheera_ai_params"].copy()
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"dheera_ai.image_generation(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"dheera_ai.image_generation(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _aimage_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            self.total_calls[model_name] += 1
            response = dheera_ai.aimage_generation(
                **{
                    **data,
                    "prompt": prompt,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"dheera_ai.aimage_generation(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"dheera_ai.aimage_generation(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _atranscription(self, file: FileTypes, model: str, **kwargs):
        ...
        try:
            ...
            self.total_calls[model_name] += 1
            response = dheera_ai.atranscription(
                **{
                    **data,
                    "file": file,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"dheera_ai.atranscription(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"dheera_ai.atranscription(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def aspeech(self, model: str, input: str, voice: str, **kwargs):
        ...
        try:
            ...
            data = deployment["dheera_ai_params"].copy()
            data["model"]
            for k, v in self.default_dheera_ai_params.items():
                if (
                    k not in kwargs
    ...
    async def _arerank(self, model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"dheera_ai.arerank(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"dheera_ai.arerank(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    def text_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            # call via dheera_ai.completion()
            return dheera_ai.text_completion(**{**data, "prompt": prompt, "caching": self.cache_responses, **kwargs})  # type: ignore
        except Exception as e:
            raise e
    ...
    async def _atext_completion(self, model: str, prompt: str, **kwargs):
        try:
            ...
            response = dheera_ai.atext_completion(
                **{
                    **data,
                    "prompt": prompt,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"dheera_ai.atext_completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"dheera_ai.atext_completion(model={model})\033[31m Exception {str(e)}\033[0m"
            )
    ...
    async def _aadapter_completion(self, adapter_id: str, model: str, **kwargs):
        try:
            ...
            response = dheera_ai.aadapter_completion(
                **{
                    **data,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"dheera_ai.aadapter_completion(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"dheera_ai.aadapter_completion(model={model})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def _ageneric_api_call_with_fallbacks(
        ...
        """
        Helper function to make a generic LLM API call through the router, this allows you to use retries/fallbacks with dheera_ai router
        """
        try:
            kwargs["model"] = model
            ...
            self._update_kwargs_before_fallbacks(
                model=model, kwargs=kwargs, metadata_variable_name="dheera_ai_metadata"
            )
            verbose_router_logger.debug(
    ...
    def _generic_api_call_with_fallbacks(
        ...
        """
        Make a generic LLM API call through the router, this allows you to use retries/fallbacks with dheera_ai router
        Args:
            model: The model to use
            original_function: The handler function to call (e.g., dheera_ai.completion)
    ...
    def _embedding(self, input: Union[str, List], model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"dheera_ai.embedding(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"dheera_ai.embedding(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _aembedding(self, input: Union[str, List], model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"dheera_ai.aembedding(model={model_name})\033[32m 200 OK\033[0m"
            )
            return response
        except Exception as e:
            verbose_router_logger.info(
                f"dheera_ai.aembedding(model={model_name})\033[31m Exception {str(e)}\033[0m"
            )
            if model_name is not None:
                self.fail_calls[model_name] += 1
            raise e
    ...
    async def _acreate_file(
        ...
        **kwargs,
        ...
        try:
            from dheera_ai.router_utils.common_utils import add_model_file_id_mappings
            ...
            async def create_file_for_deployment(deployment: dict) -> OpenAIFileObject:
                ...
                response = dheera_ai.acreate_file(
                    **{
                        **data,
                        "custom_llm_provider": custom_llm_provider,
                        "caching": self.cache_responses,
                ...
                self.success_calls[model_name] += 1
                verbose_router_logger.info(
                    f"dheera_ai.acreate_file(model={model_name})\033[32m 200 OK\033[0m"
                )
            ...
            verbose_router_logger.exception(
                f"dheera_ai.acreate_file(model={model}, {kwargs})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def _acreate_batch(
        ...
        **kwargs,
        ...
        try:
            ...
            response = dheera_ai.acreate_batch(
                **{
                    **data,
                    "custom_llm_provider": custom_llm_provider,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info(
                f"dheera_ai.acreate_batch(model={model_name})\033[32m 200 OK\033[0m"
            )

            return response  # type: ignore
        except Exception as e:
            verbose_router_logger.exception(
                f"dheera_ai._acreate_batch(model={model}, {kwargs})\033[31m Exception {str(e)}\033[0m"
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
        ...
        try:
            ...
            async def try_retrieve_batch(model_name: DeploymentTypedDict):
                try:
                    ...
                    model = model_name["dheera_ai_params"].get("model")
                    data = model_name["dheera_ai_params"].copy()
                    custom_llm_provider = data.get("custom_llm_provider")
                    if model is None:
                        raise Exception(
                            f"Model not found in dheera_ai_params for deployment: {model_name}"
                        )
                    ...
                    return await dheera_ai.aretrieve_batch(
                        **{
                            **data,
                            "custom_llm_provider": custom_llm_provider,
            ...
            if results is not None:
                if isinstance(results, DheeraAIBatch):
                    return results
                elif isinstance(results, list):
                    for result in results:
                        if isinstance(result, DheeraAIBatch):
                            return result
    ...
    async def alist_batches(
        ...
        **kwargs,
        ...
        async def try_retrieve_batch(model: DeploymentTypedDict):
            try:
                # Update kwargs with the current model name or any other model-specific adjustments
                return await dheera_ai.alist_batches(
                    **{**model["dheera_ai_params"], **kwargs}
                )
            except Exception:
                return None
    ...
    async def _pass_through_moderation_endpoint_factory(
        ...
        **kwargs,
        ...
        if kwargs.get("model") and self.get_model_list(model_name=kwargs["model"]):
            ...
            kwargs["model"] = deployment["dheera_ai_params"]["model"]
            data = deployment["dheera_ai_params"].copy()
            self._update_kwargs_with_deployment(
                deployment=deployment,
                kwargs=kwargs,
        ...
        **kwargs,
        ...
        if custom_llm_provider is None:
            if self.assistants_config is not None:
                ...
                kwargs.update(self.assistants_config["dheera_ai_params"])
            else:
                raise Exception(
    ...
    async def async_function_with_fallbacks_common_utils(  # noqa: PLR0915
        ...
        try:
            ...
            if isinstance(e, dheera_ai.ContextWindowExceededError):
            ...
            elif isinstance(e, dheera_ai.ContentPolicyViolationError):
            ...
            parent_otel_span = _get_parent_otel_span_from_kwargs(kwargs)
            verbose_router_logger.error(
                "dheera_ai.router.py::async_function_with_fallbacks() - Error occurred while trying to do fallbacks - {}\n{}\n\nDebug Information:\nCooldown Deployments={}".format(
                    str(new_exception),
    ...
    def _handle_mock_testing_fallbacks(
        ...
        """
        Helper function to raise a dheera_ai Error for mock testing purposes.

        Raises:
            dheera_ai.InternalServerError: when `mock_testing_fallbacks=True` passed in request params
            dheera_ai.ContextWindowExceededError: when `mock_testing_context_fallbacks=True` passed in request params
            dheera_ai.ContentPolicyViolationError: when `mock_testing_content_policy_fallbacks=True` passed in request params
        """
        ...
        if (
            ...
            raise dheera_ai.InternalServerError(
                model=model_group,
                llm_provider="",
            ...
            raise dheera_ai.ContextWindowExceededError(
                model=model_group,
                llm_provider="",
    ...
    async def async_function_with_retries(self, *args, **kwargs):  # noqa: PLR0915
        ...
        _metadata: dict = kwargs.get("dheera_ai_metadata", kwargs.get("metadata")) or {}
        ...
        try:
            ...
            if type(original_exception) in dheera_ai.DHEERA_AI_EXCEPTION_TYPES:
    ...
    def _handle_mock_testing_rate_limit_error(
        ...
        """
        Helper function to raise a mock dheera_ai.RateLimitError error for testing purposes.

        Raises:
            dheera_ai.RateLimitError error when `mock_testing_rate_limit_error=True` passed in request params
        """
        ...
        if available_models is not None and len(available_models) == 1:
            num_retries = cast(
                Optional[int], available_models[0]["dheera_ai_params"].get("num_retries")
            )
        ...
        if (
            ...
            verbose_router_logger.info(
                f"dheera_ai.router.py::_mock_rate_limit_error() - Raising mock RateLimitError for model={model_group}"
            )
    ...
    def should_retry_this_error(
        ...
        if (
            isinstance(error, dheera_ai.ContextWindowExceededError)
            and context_window_fallbacks is not None
        ):
            raise error

        if (
            isinstance(error, dheera_ai.ContentPolicyViolationError)
            and content_policy_fallbacks is not None
        ):
            raise error

        if isinstance(error, dheera_ai.NotFoundError):
            raise error
    ...
    def _time_to_sleep_before_retry(
        ...
        if hasattr(e, "dheera_ai_response_headers"):
            response_headers = e.dheera_ai_response_headers  # type: ignore

        if response_headers is not None:
            timeout = dheera_ai._calculate_retry_after(
                remaining_retries=remaining_retries,
                max_retries=num_retries,
    ...
    async def deployment_callback_on_success(
        ...
        """
        from dheera_ai.types.caching import RedisPipelineIncrementOperation
        ...
        try:
            ...
            if kwargs["dheera_ai_params"].get("metadata") is None:
                pass
            else:
                deployment_name = kwargs["dheera_ai_params"]["metadata"].get(
                    "deployment", None
                )  # stable name - works for wildcard routes as well
                # Get model_group and id from kwargs like the sync version does
                model_group = kwargs["dheera_ai_params"]["metadata"].get(
                    "model_group", None
                )
                model_info = kwargs["dheera_ai_params"].get("model_info", {}) or {}
                ...
                if deployment_info is None:
                    ...
                    ## check tpm/rpm in dheera_ai_params
                    tpm_dheera_ai_params = deployment_info.dheera_ai_params.tpm
                    rpm_dheera_ai_params = deployment_info.dheera_ai_params.rpm
                ...
                if (
                    tpm is None
                    and rpm is None
                    and tpm_dheera_ai_params is None
                    and rpm_dheera_ai_params is None
            ...
            verbose_router_logger.debug(
                "dheera_ai.router.Router::deployment_callback_on_success(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    def sync_deployment_callback_on_success(
        ...
        if kwargs["dheera_ai_params"].get("metadata") is None:
            ...
            model_group = kwargs["dheera_ai_params"]["metadata"].get("model_group", None)
            model_info = kwargs["dheera_ai_params"].get("model_info", {}) or {}
            id = model_info.get("id", None)
            if model_group is None or id is None:
                return None
    ...
    def deployment_callback_on_failure(
        ...
        try:
            ...
            # Cache dheera_ai_params to avoid repeated dict lookups
            dheera_ai_params = kwargs.get("dheera_ai_params", {})
            _model_info = dheera_ai_params.get("model_info", {})

            exception_headers = dheera_ai.dheera_ai_core_utils.exception_mapping_utils._get_response_headers(
                original_exception=exception
            )

            # Determine cooldown time with priority: deployment config > response header > router default
            deployment_cooldown = dheera_ai_params.get("cooldown_time", None)

            header_cooldown = None
            if exception_headers is not None:
                header_cooldown = dheera_ai.utils._get_retry_after_from_exception_header(
    ...
    async def async_deployment_callback_on_failure(
        ...
        deployment_name = kwargs["dheera_ai_params"]["metadata"].get(
            "deployment", None
        )  # handles wildcard routes - by giving the original name sent to `dheera_ai.completion`
        model_group = kwargs["dheera_ai_params"]["metadata"].get("model_group", None)
        model_info = kwargs["dheera_ai_params"].get("model_info", {}) or {}
        id = model_info.get("id", None)
        if model_group is None or id is None:
    ...
    def _get_metadata_variable_name_from_kwargs(
        self, kwargs: dict
    ) -> Literal["metadata", "dheera_ai_metadata"]:
        """
        Helper to return what the "metadata" field should be called in the request data

        - New endpoints return `dheera_ai_metadata`
        - Old endpoints return `metadata`

        Context:
        - DheeraAI used `metadata` as an internal field for storing metadata
        - OpenAI then started using this field for their metadata
        - DheeraAI is now moving to using `dheera_ai_metadata` for our metadata
        """
    ...
    def log_retry(self, kwargs: dict, e: Exception) -> dict:
        ...
        try:
            _metadata_var = (
                "dheera_ai_metadata" if "dheera_ai_metadata" in kwargs else "metadata"
            )
    ...
    def _get_healthy_deployments(self, model: str, parent_otel_span: Optional[Span]):
        ...
        unhealthy_deployments = _get_cooldown_deployments(
            dheera_ai_router_instance=self, parent_otel_span=parent_otel_span
        )
    ...
    def routing_strategy_pre_call_checks(self, deployment: dict):
        ...
        """
        for _callback in dheera_ai.callbacks:
    ...
    async def async_routing_strategy_pre_call_checks(
        ...
        logging_obj: Optional[DheeraAILogging] = None,
    ):
        """
    ...
    def _generate_model_id(self, model_group: str, dheera_ai_params: dict):
        """
        Helper function to consistently generate the same id for a deployment

        - create a string from all the dheera_ai params
        - hash
        - use hash as id
        """
    ...
    def _create_deployment(
        ...
        - None: If the deployment is not active for the current environment (if 'supported_environments' is set in dheera_ai_params)
        """
        try:
            dheera_ai_params: DheeraAI_Params = DheeraAI_Params(**_dheera_ai_params)
            deployment = Deployment(
                **deployment_info,
            ...
            for field in CustomPricingDheeraAIParams.model_fields.keys():
                if deployment.dheera_ai_params.get(field) is not None:
                    _model_info[field] = deployment.dheera_ai_params[field]
            ...
            _model_name = deployment.dheera_ai_params.model
            if deployment.dheera_ai_params.custom_llm_provider is not None:
                _model_name = (
                    deployment.dheera_ai_params.custom_llm_provider + "/" + _model_name
                )
    ...
    def _is_auto_router_deployment(self, dheera_ai_params: DheeraAI_Params) -> bool:
        """
        Check if the deployment is an auto-router deployment.

        Returns True if the dheera_ai_params model starts with "auto_router/"
        """
        if dheera_ai_params.model.startswith("auto_router/"):
            return True
        return False
    ...
    def init_auto_router_deployment(self, deployment: Deployment):
        ...
        """
        from dheera_ai.router_strategy.auto_router.auto_router import AutoRouter

        auto_router_config_path: Optional[str] = (
            deployment.dheera_ai_params.auto_router_config_path
        )
        auto_router_config: Optional[str] = deployment.dheera_ai_params.auto_router_config
        if auto_router_config_path is None and auto_router_config is None:
            raise ValueError(
                "auto_router_config_path or auto_router_config is required for auto-router deployments. Please set it in the dheera_ai_params"
            )

        default_model: Optional[str] = (
            deployment.dheera_ai_params.auto_router_default_model
        )
        if default_model is None:
            raise ValueError(
                "auto_router_default_model is required for auto-router deployments. Please set it in the dheera_ai_params"
            )

        embedding_model: Optional[str] = (
            deployment.dheera_ai_params.auto_router_embedding_model
        )
        if embedding_model is None:
            raise ValueError(
                "auto_router_embedding_model is required for auto-router deployments. Please set it in the dheera_ai_params"
            )
    ...
    def deployment_is_active_for_environment(self, deployment: Deployment) -> bool:
        ...
        Requires `DHEERA_AI_ENVIRONMENT` to be set in .env. Valid values for environment:
            - development
            - staging
            - production

        Raises:
        - ValueError: If DHEERA_AI_ENVIRONMENT is not set in .env or not one of the valid values
        ...
        if (
            ...
            return True
        dheera_ai_environment = get_secret_str(secret_name="DHEERA_AI_ENVIRONMENT")
        if dheera_ai_environment is None:
            raise ValueError(
                "Set 'supported_environments' for model but not 'DHEERA_AI_ENVIRONMENT' set in .env"
            )

        if dheera_ai_environment not in VALID_DHEERA_AI_ENVIRONMENTS:
            raise ValueError(
                f"DHEERA_AI_ENVIRONMENT must be one of {VALID_DHEERA_AI_ENVIRONMENTS}. but set as: {dheera_ai_environment}"
            )

        for _env in deployment.model_info["supported_environments"]:
            if _env not in VALID_DHEERA_AI_ENVIRONMENTS:
                raise ValueError(
                    f"supported_environments must be one of {VALID_DHEERA_AI_ENVIRONMENTS}. but set as: {_env} for deployment: {deployment}"
                )

        if dheera_ai_environment in deployment.model_info["supported_environments"]:
            return True
        return False
    ...
    def set_model_list(self, model_list: list):
        ...
        for model in original_model_list:
            ...
            _dheera_ai_params = model.pop("dheera_ai_params")
            ## check if dheera_ai params in os.environ
            if isinstance(_dheera_ai_params, dict):
                for k, v in _dheera_ai_params.items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        _dheera_ai_params[k] = get_secret(v)

            _model_info: dict = model.pop("model_info", {})

            # check if model info has id
            if "id" not in _model_info:
                _id = self._generate_model_id(_model_name, _dheera_ai_params)
                _model_info["id"] = _id

            if _dheera_ai_params.get("organization", None) is not None and isinstance(
                _dheera_ai_params["organization"], list
            ):  # Addresses https://github.com/BerriAI/dheera_ai/issues/3949
                for org in _dheera_ai_params["organization"]:
                    _dheera_ai_params["organization"] = org
                    self._create_deployment(
                        deployment_info=model,
                        _model_name=_model_name,
                        _dheera_ai_params=_dheera_ai_params,
    ...
    def _add_deployment(self, deployment: Deployment) -> Deployment:
        ...
        dheera_ai_model = deployment.dheera_ai_params.model
        is_prompt_management_model = False

        if "/" in dheera_ai_model:
            split_dheera_ai_model = dheera_ai_model.split("/")[0]
            if split_dheera_ai_model in dheera_ai._known_custom_logger_compatible_callbacks:
        ...
        if is_prompt_management_model:
            ...
            (
                ...
                model=deployment.dheera_ai_params.model,
                custom_llm_provider=deployment.dheera_ai_params.get(
                    "custom_llm_provider", None
                ),
            )
            # done reading model["dheera_ai_params"]
            if custom_llm_provider not in dheera_ai.provider_list:
                raise Exception(f"Unsupported provider - {custom_llm_provider}")

        #### DEPLOYMENT NAMES INIT ########
        self.deployment_names.append(deployment.dheera_ai_params.model)
        ############ Users can either pass tpm/rpm as a dheera_ai_param or a router param ###########
        # for get_available_deployment, we use the dheera_ai_param["rpm"]
        # in this snippet we also set rpm to be a dheera_ai_param
        if (
            deployment.dheera_ai_params.rpm is None
            and getattr(deployment, "rpm", None) is not None
        ):
            deployment.dheera_ai_params.rpm = getattr(deployment, "rpm")

        if (
            deployment.dheera_ai_params.tpm is None
            and getattr(deployment, "tpm", None) is not None
        ):
            deployment.dheera_ai_params.tpm = getattr(deployment, "tpm")

        # Check if user is trying to use model_name == "*"
        # this is a catch all model for their specific api key
        # if deployment.model_name == "*":
        #     if deployment.dheera_ai_params.model == "*":
        #         # user wants to pass through all requests to dheera_ai.acompletion for unknown deployments
        ...
        data_sources = deployment.dheera_ai_params.get("dataSources", []) or []
        ...
        #     dheera_ai_router_instance=self, model=deployment.to_json(exclude_none=True)
        # )
        ...
        #########################################################
        if self._is_auto_router_deployment(dheera_ai_params=deployment.dheera_ai_params):
    ...
    def _initialize_deployment_for_pass_through(
        ...
        """
        Optional: Initialize deployment for pass-through endpoints if `deployment.dheera_ai_params.use_in_pass_through` is True
        ...
        """
        if deployment.dheera_ai_params.use_in_pass_through is True:
            from dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
                passthrough_endpoint_router,
            )

            if deployment.dheera_ai_params.dheera_ai_credential_name is not None:
                credential_values = CredentialAccessor.get_credential_values(
                    deployment.dheera_ai_params.dheera_ai_credential_name
                )
            else:
                credential_values = {}
            ...
            if custom_llm_provider == "vertex_ai":
                vertex_project = (
                    ...
                    or deployment.dheera_ai_params.vertex_project
                )
                vertex_location = (
                    credential_values.get("vertex_location")
                    or deployment.dheera_ai_params.vertex_location
                )
                vertex_credentials = (
                    credential_values.get("vertex_credentials")
                    or deployment.dheera_ai_params.vertex_credentials
                )

                if vertex_project is None or vertex_location is None:
                    raise ValueError(
                        "vertex_project, and vertex_location must be set in dheera_ai_params for pass-through endpoints."
                    )
                ...
                api_base = (
                    ...
                    or deployment.dheera_ai_params.api_base
                )
                api_key = (
                    credential_values.get("api_key")
                    or deployment.dheera_ai_params.api_key
                )
    ...
    def upsert_deployment(self, deployment: Deployment) -> Optional[Deployment]:
        ...
        try:
            ...
            if _deployment_on_router is not None:
                ...
                if deployment.dheera_ai_params == _deployment_on_router.dheera_ai_params:
                    # No need to update
                    return None

                # if there is a new dheera_ai param -> then update the deployment
    ...
    def get_deployment_credentials(self, model_id: str) -> Optional[dict]:
        ...
        if deployment is None:
            return None
        return CredentialDheeraAIParams(
            **deployment.dheera_ai_params.model_dump(exclude_none=True)
    ...
    def get_deployment_credentials_with_provider(
        ...
        Args:
            model_id: Model ID or model name from model_list (e.g., "gpt-4o-dheera_ai")
        ...
        Example:
            credentials = router.get_deployment_credentials_with_provider("gpt-4o-dheera_ai")
        ...
        credentials = CredentialDheeraAIParams(
            **deployment.dheera_ai_params.model_dump(exclude_none=True)
        ).model_dump(exclude_none=True)

        # Add custom_llm_provider
        if deployment.dheera_ai_params.custom_llm_provider:
            credentials["custom_llm_provider"] = (
                deployment.dheera_ai_params.custom_llm_provider
            )
        elif "/" in deployment.dheera_ai_params.model:
            # Extract provider from "provider/model" format
            credentials["custom_llm_provider"] = deployment.dheera_ai_params.model.split(
                "/"
            )[0]
        else:
    ...
    def get_router_model_info(
        ...
        Augment dheera_ai info with additional params set in `model_info`.
        ...
        if base_model is None:
            base_model = deployment.get("dheera_ai_params", {}).get("base_model", None)

        model = base_model

        ## GET PROVIDER
        _model, custom_llm_provider, _, _ = dheera_ai.get_llm_provider(
            model=deployment.get("dheera_ai_params", {}).get("model", ""),
            dheera_ai_params=DheeraAI_Params(**deployment.get("dheera_ai_params", {})),
        )
                ...
                for potential_model in potential_models:
                    try:
                        if potential_model.get("model_info", {}).get(
                            ...
                            model = potential_model.get("dheera_ai_params", {}).get(
                                "model"
                            )
                            break
                    except Exception:
        ...
        if model is None:
            ...
            # Use the original model from dheera_ai_params
            model = _model
        ...
        model_info = dheera_ai.get_model_info(model=model_info_name)
    ...
    def get_model_info(self, id: str) -> Optional[dict]:
        ...
        Returns
        - dict: the model in list with 'model_name', 'dheera_ai_params', Optional['model_info']
    ...
    def get_deployment_model_info(
        ...
        2. If not, check if dheera_ai model name is in model info
        ...
        dheera_ai_model_name_model_info: Optional[ModelInfo] = None

        try:
            custom_model_info = dheera_ai.model_cost.get(model_id)
        except Exception:
            pass

        try:
            dheera_ai_model_name_model_info = dheera_ai.get_model_info(model=model_name)
        except Exception:
            pass
        ...
        try:
            if custom_model_info is not None:
                ...
                if base_model is not None:
                    ## update dheera_ai model info with base model info
                    base_model_info = dheera_ai.get_model_info(model=base_model)
        ...
        if custom_model_info is not None and dheera_ai_model_name_model_info is not None:
            model_info = cast(
                ModelInfo,
                _update_dictionary(
                    cast(dict, dheera_ai_model_name_model_info).copy(),
                    custom_model_info,
                ),
            )
        elif dheera_ai_model_name_model_info is not None:
            model_info = dheera_ai_model_name_model_info
    ...
    def _set_model_group_info(  # noqa: PLR0915
        ...
        for model in model_list:
            ...
            dheera_ai_params = DheeraAI_Params(**model["dheera_ai_params"])  # type: ignore
            # get configurable clientside auth params
            configurable_clientside_auth_params = (
                dheera_ai_params.configurable_clientside_auth_params
            )

            # Cache nested dict access to avoid repeated temporary dict allocations
            model_dheera_ai_params = model.get("dheera_ai_params", {})
            ...
            if _deployment_tpm is None:
                _deployment_tpm = model_dheera_ai_params.get("tpm", None)  # type: ignore
            if _deployment_tpm is None:
            ...
            if _deployment_rpm is None:
                _deployment_rpm = model_dheera_ai_params.get("rpm", None)  # type: ignore
            if _deployment_rpm is None:
            ...
            try:
                ...
                if model_id is not None:
                    model_info = self.get_deployment_model_info(
                        model_id=model_id, model_name=dheera_ai_params.model
                    )
                else:
                    model_info = None
            except Exception:
            ...
            # get llm provider
            dheera_ai_model, llm_provider = "", ""
            try:
                dheera_ai_model, llm_provider, _, _ = dheera_ai.get_llm_provider(
                    model=dheera_ai_params.model,
                    custom_llm_provider=dheera_ai_params.custom_llm_provider,
                )
            except dheera_ai.exceptions.BadRequestError as e:
                verbose_router_logger.error(
                    "dheera_ai.router.py::get_model_group_info() - {}".format(str(e))
                )

            if model_info is None:
                supported_openai_params = dheera_ai.get_supported_openai_params(
                    model=dheera_ai_model, custom_llm_provider=llm_provider
                )
                ...
                model_info = ModelMapInfo(
                    ...
                    dheera_ai_provider=llm_provider,
                    mode=mode,
    ...
    async def get_model_group_usage(
        ...
        for model in model_list:
            ...
            dheera_ai_model: Optional[str] = model["dheera_ai_params"].get(
                "model"
            )  # USE THE MODEL SENT TO dheera_ai.completion() - consistent with how global_router cache is written.
            if id is None or dheera_ai_model is None:
                continue
            tpm_keys.append(
    ...
    def _build_model_id_to_deployment_index_map(self, model_list: list):
        ...
        for _, model in enumerate(model_list):
            ...
            if model_id is None:
                ...
                model_id = self._generate_model_id(model_name, dheera_ai_params)
    ...
    def resolve_model_name_from_model_id(self, model_id: Optional[str]) -> Optional[str]:
        ...
        so that dheera_ai_params can be automatically injected from the model config.
        
        Strategy:
        1. First, check if model_id directly matches a model_name or deployment ID
        2. If not, search through router's model_list to find a match by dheera_ai_params.model
        3. Return the model_name if found, None otherwise
        
        Args:
            model_id: The model_id extracted from decoded video_id
                     (could be model_name or dheera_ai_params.model value)
        ...
        # Strategy 2: Search through router's model_list to find by dheera_ai_params.model
        all_models = self.get_model_list(model_name=None)
        if not all_models:
            return None
        
        for deployment in all_models:
            dheera_ai_params = deployment.get("dheera_ai_params", {})
            actual_model = dheera_ai_params.get("model")
        ...
        if client_type == "max_parallel_requests":
            ...
            if client is None:
                InitalizeCachedClient.set_max_parallel_requests_client(
                    dheera_ai_router_instance=self, model=deployment
                )
                client = self.cache.get_cache(
                    key=cache_key, local_only=True, parent_otel_span=parent_otel_span
                )
    ...
    def _pre_call_checks(  # noqa: PLR0915
        ...
        try:
            input_tokens = dheera_ai.token_counter(messages=messages)
        except Exception as e:
            verbose_router_logger.error(
                "dheera_ai.router.py::_pre_call_checks: failed to count tokens. Returning initial list of deployments. Got - {}".format(
                    str(e)
                )
            )
        ...
        for idx, deployment in enumerate(_returned_deployments):
            # Cache nested dict access to avoid repeated temporary dict allocations
            _dheera_ai_params = deployment.get("dheera_ai_params", {})
            ...
            try:
                ...
                if base_model is None:
                    base_model = _dheera_ai_params.get("base_model", None)
                model_info = self.get_router_model_info(
                    deployment=deployment, received_model_name=model
                )
                model = base_model or _dheera_ai_params.get("model", None)
            ...
            if (
                ...
                if (
                    isinstance(_dheera_ai_params, dict)
                    and _dheera_ai_params.get("rpm", None) is not None
                ):
                    if (
                        isinstance(_dheera_ai_params["rpm"], int)
                        and _dheera_ai_params["rpm"] <= current_request
                    ):
                        invalid_model_indices.add(idx)
            ...
            if (
                ...
                if allowed_model_region is not None:
                    if not is_region_allowed(
                        dheera_ai_params=DheeraAI_Params(**_dheera_ai_params),
            ...
            if request_kwargs is not None and dheera_ai.drop_params is False:
                # get supported params
                model, custom_llm_provider, _, _ = dheera_ai.get_llm_provider(
                    model=model, dheera_ai_params=DheeraAI_Params(**_dheera_ai_params)
                )
                ...
                if supported_openai_params is None:
                    ...
                    non_default_params = dheera_ai.utils.get_non_default_params(
        ...
        if len(invalid_model_indices) == len(_returned_deployments):
            ...
            if _rate_limit_error is True:  # allow generic fallback logic to take place
                ...
                raise dheera_ai.ContextWindowExceededError(
                    message="dheera_ai._pre_call_checks: Context Window exceeded for given call. No models have context window large enough for this call.\n{}".format(
                        _potential_error_str
                    ),
                    model=model,
                    llm_provider="",
        ...
        if len(_returned_deployments) > 0:
            _returned_deployments = dheera_ai.utils._get_order_filtered_deployments(
                _returned_deployments
            )
    ...
    def _get_deployment_by_dheera_ai_model(self, model: str) -> List:
        """
        Get the deployment by dheera_ai model.
        """
        return [m for m in self.model_list if m["dheera_ai_params"]["model"] == model]
    ...
    def _common_checks_available_deployment(
        ...
        if request_kwargs is not None:
            ...
            dheera_ai_metadata = request_kwargs.get("dheera_ai_metadata") or {}
            request_team_id = metadata.get(
                "user_api_key_team_id"
            ) or dheera_ai_metadata.get("user_api_key_team_id")
        # check if aliases set on dheera_ai model alias map
        if specific_deployment is True:
            return model, self._get_deployment_by_dheera_ai_model(model=model)
        elif self.has_model_id(model):
            deployment = self.get_deployment(model_id=model)
            if deployment is not None:
                deployment_model = deployment.dheera_ai_params.model
                return deployment_model, deployment.model_dump(exclude_none=True)
            raise ValueError(
                f"DheeraAI Router: Trying to call specific deployment, but Model ID :{model} does not exist in Model ID map"
            )
        ...
        if model not in self.model_names:
            ...
            if self.default_deployment is not None:
                # Shallow copy with nested dheera_ai_params copy (100x+ faster than deepcopy)
                updated_deployment = self.default_deployment.copy()
                updated_deployment["dheera_ai_params"] = self.default_deployment[
                    "dheera_ai_params"
                ].copy()
                updated_deployment["dheera_ai_params"]["model"] = model
        ...
        if len(healthy_deployments) == 0:
            ...
            healthy_deployments = self._get_deployment_by_dheera_ai_model(model=model)
        ...
        if len(healthy_deployments) == 0:
            ...
            if len(healthy_deployments) == 0:
                ...
                raise dheera_ai.BadRequestError(
                    message=message,
                    model=model,
                    llm_provider="",
                )

        if dheera_ai.model_alias_map and model in dheera_ai.model_alias_map:
            model = dheera_ai.model_alias_map[
                model
    ...
    async def async_get_available_deployment(
        ...
        try:
            ...
            if request_kwargs is not None:
                logging_obj = request_kwargs.get("dheera_ai_logging_obj", None)
    ...
    async def async_pre_routing_hook(
        ...
        Used for the dheera_ai auto-router to modify the request before the routing decision is made.
        """
        #########################################################
    ...
    def _initialize_alerting(self):
        ...
        dheera_ai.logging_callback_manager.add_dheera_ai_callback(_slack_alerting_logger)  # type: ignore
        dheera_ai.logging_callback_manager.add_dheera_ai_success_callback(
            _slack_alerting_logger.response_taking_too_long_callback
        )
        verbose_router_logger.info(
            "\033[94m\nInitialized Alerting for dheera_ai.Router\033[0m\n"
        )
    ...
    def set_custom_routing_strategy(
        ...
        """
        Sets get_available_deployment and async_get_available_deployment on an instanced of dheera_ai.Router

        Use this to set your custom routing strategy

        Args:
            CustomRoutingStrategy: dheera_ai.router.CustomRoutingStrategyBase
        """
