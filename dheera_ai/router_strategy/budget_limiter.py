...
from dheera_ai.caching.redis_cache import RedisPipelineIncrementOperation
from dheera_ai.integrations.custom_logger import CustomLogger, Span
from dheera_ai.dheera_ai_core_utils.duration_parser import duration_in_seconds
from dheera_ai.router_strategy.tag_based_routing import _get_tags_from_request_kwargs
from dheera_ai.router_utils.cooldown_callbacks import (
    _get_prometheus_logger_from_callbacks,
)
from dheera_ai.types.llms.openai import AllMessageValues
from dheera_ai.types.router import DeploymentTypedDict, DheeraAI_Params, RouterErrors
from dheera_ai.types.utils import BudgetConfig
from dheera_ai.types.utils import BudgetConfig as GenericBudgetInfo
from dheera_ai.types.utils import GenericBudgetConfigType, StandardLoggingPayload
...
class RouterBudgetLimiting(CustomLogger):
    def __init__(
        ...
        # Add self to dheera_ai callbacks if it's a list
        if isinstance(dheera_ai.callbacks, list):
            dheera_ai.logging_callback_manager.add_dheera_ai_callback(self)  # type: ignore
    ...
    def _filter_out_deployments_above_budget(
        ...
        for deployment in healthy_deployments:
            ...
            if self.deployment_budget_config and is_within_budget:
                ...
                _dheera_ai_params = deployment.get("dheera_ai_params") or {}
                ...
                if model_id in deployment_configs:
                    ...
                    if config.max_budget and current_spend >= config.max_budget:
                        debug_msg = f"Exceeded budget for deployment model_name: {_model_name}, dheera_ai_params.model: {_dheera_ai_model_name}, model_id: {model_id}: {current_spend} >= {config.budget_duration}"
    ...
    async def _increment_spend_in_current_window(
        ...
        - Queues the increment operation to Redis Pipeline (using batched pipeline to optimize performance. Using Redis for multi instance environment of DheeraAI)
        """
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        custom_llm_provider: str = kwargs.get("dheera_ai_params", {}).get(
    ...
    def _get_llm_provider_for_deployment(self, deployment: Dict) -> Optional[str]:
        try:
            _dheera_ai_params: DheeraAI_Params = DheeraAI_Params(
                **deployment.get("dheera_ai_params", {"model": ""})
            )
            _, custom_llm_provider, _, _ = dheera_ai.get_llm_provider(
                model=_dheera_ai_params.model,
    ...
    def should_init_router_budget_limiter(
        ...
        if dheera_ai.tag_budget_config is not None:
            return True
        ...
        for _model in model_list:
            _dheera_ai_params = _model.get("dheera_ai_params", {})
            if (
                _dheera_ai_params.get("max_budget")
                or _dheera_ai_params.get("budget_duration") is not None
            ):
                return True
        return False
    ...
    def _init_deployment_budgets(
        ...
        for _model in model_list:
            ...
            _max_budget = _dheera_ai_params.get("max_budget")
            _budget_duration = _dheera_ai_params.get("budget_duration")
    ...
    def _init_tag_budgets(self):
        if dheera_ai.tag_budget_config is None:
            return
        ...
        for _tag, _tag_budget_config in dheera_ai.tag_budget_config.items():
