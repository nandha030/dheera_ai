...
import dheera_ai
from dheera_ai import ModelResponse, token_counter, verbose_logger
...
class LowestCostLoggingHandler(CustomLogger):
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            if kwargs["dheera_ai_params"].get("metadata") is None:
                ...
                id = kwargs["dheera_ai_params"].get("model_info", {}).get("id", None)
                ...
                if isinstance(response_obj, ModelResponse):
                    ...
                    if _usage is not None and isinstance(_usage, dheera_ai.Usage):
            ...
            verbose_logger.exception(
                "dheera_ai.router_strategy.lowest_cost.py::log_success_event(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            verbose_logger.exception(
                "dheera_ai.proxy.hooks.prompt_injection_detection.py::async_pre_call_hook(): Exception occured - {}".format(
                    str(e)
                )
            )
            pass
    ...
    async def async_get_available_deployments(  # noqa: PLR0915
        ...
        for item, item_map in all_deployments.items():
            ...
            _deployment_tpm = (
                ...
                or _deployment.get("dheera_ai_params", {}).get("tpm", None)
            ...
            _deployment_rpm = (
                ...
                or _deployment.get("dheera_ai_params", {}).get("rpm", None)
                or _deployment.get("model_info", {}).get("rpm", None)
                or float("inf")
            )
            item_dheera_ai_model_name = _deployment.get("dheera_ai_params", {}).get("model")
            item_dheera_ai_model_cost_map = dheera_ai.model_cost.get(
                item_dheera_ai_model_name, {}
            )

            # check if user provided input_cost_per_token and output_cost_per_token in dheera_ai_params
            item_input_cost = None
            item_output_cost = None
            if _deployment.get("dheera_ai_params", {}).get("input_cost_per_token", None):
                item_input_cost = _deployment.get("dheera_ai_params", {}).get(
                    "input_cost_per_token"
                )

            if _deployment.get("dheera_ai_params", {}).get("output_cost_per_token", None):
                item_output_cost = _deployment.get("dheera_ai_params", {}).get(
                    "output_cost_per_token"
                )

            if item_input_cost is None:
                item_input_cost = item_dheera_ai_model_cost_map.get(
                    "input_cost_per_token", 5.0
                )

            if item_output_cost is None:
                item_output_cost = item_dheera_ai_model_cost_map.get(
                    "output_cost_per_token", 5.0
                )

            # if dheera_ai["model"] is not in model_cost map -> use item_cost = $10
            ...
            _deployment_api_base = _deployment.get("dheera_ai_params", {}).get(
                "api_base", ""
            )
