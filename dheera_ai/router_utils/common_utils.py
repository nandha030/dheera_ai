...
def get_dheera_ai_params_sensitive_credential_hash(dheera_ai_params: dict) -> str:
    """
    Hash of the credential params, used for mapping the file id to the right model
    """
    sensitive_params = CredentialDheeraAIParams(**dheera_ai_params)
...
def filter_team_based_models(
    ...
    request_team_id = metadata.get("user_api_key_team_id") or dheera_ai_metadata.get(
...
def _deployment_supports_web_search(deployment: Dict) -> bool:
    ...
    Note: Ideally we'd fall back to dheera_ai.supports_web_search() but
