...
def _is_cooldown_required(
    ...
    try:
        ...
        ):  # don't cooldown on dheera_ai api connection errors errors
...
def _should_run_cooldown_logic(
    ...
    - deployment is in dheera_ai_router_instance.provider_default_deployment_ids
    - exception_status is not one that should be immediately retried (e.g. 401)
    """
    if (
        deployment is None
        or dheera_ai_router_instance.get_model_group(id=deployment) is None
    ):
    ...
    if dheera_ai_router_instance.disable_cooldowns:
        verbose_router_logger.debug(
    ...
    if deployment in dheera_ai_router_instance.provider_default_deployment_ids:
        verbose_router_logger.debug(
...
def _should_cooldown_deployment(
    ...
    cooldown if:
        ...
        - got 401 Auth error, 404 NotFounder - checked by dheera_ai._should_retry()
    ...
    model_group = dheera_ai_router_instance.get_model_group(id=deployment)
    ...
    if (
        dheera_ai_router_instance.allowed_fails_policy is None
        ...
        num_successes_this_minute = get_deployment_successes_for_current_minute(
            dheera_ai_router_instance=dheera_ai_router_instance, deployment_id=deployment
        )
...
def _set_cooldown_deployments(
    ...
    if _should_cooldown_deployment(
        ...
        dheera_ai_router_instance.cooldown_cache.add_deployment_to_cooldown(
            model_id=deployment,
            original_exception=original_exception,
...
async def _async_get_cooldown_deployments(
    ...
    """
    model_ids = dheera_ai_router_instance.get_model_ids()
    cooldown_models = (
        await dheera_ai_router_instance.cooldown_cache.async_get_active_cooldowns(
            model_ids=model_ids,
...
def _get_cooldown_deployments(
    dheera_ai_router_instance: LitellmRouter, parent_otel_span: Optional[Span]
) -> List[str]:
    """
    ...
    cooldown_models = dheera_ai_router_instance.cooldown_cache.get_active_cooldowns(
        model_ids=model_ids, parent_otel_span=parent_otel_span
    )
...
def should_cooldown_based_on_allowed_fails_policy(
    ...
    """
    allowed_fails = (
        dheera_ai_router_instance.get_allowed_fails_from_policy(
            exception=original_exception,
        )
        or dheera_ai_router_instance.allowed_fails
    )
    cooldown_time = (
        dheera_ai_router_instance.cooldown_time or DEFAULT_COOLDOWN_TIME_SECONDS
    )

    current_fails = dheera_ai_router_instance.failed_calls.get_cache(key=deployment) or 0
    updated_fails = current_fails + 1

    if updated_fails > allowed_fails:
        return True
    else:
        dheera_ai_router_instance.failed_calls.set_cache(
...
def _is_allowed_fails_set_on_router(
    ...
    """
    if dheera_ai_router_instance.allowed_fails is None:
        return False
    if dheera_ai_router_instance.allowed_fails != dheera_ai.allowed_fails:
        return True
    return False
