...
from dheera_ai.types.router import DheeraAIParamsTypedDict
...
async def run_async_fallback(
    *args: Tuple[Any],
    dheera_ai_router: LitellmRouter,
    ...
    **kwargs,
    ...
    Args:
        dheera_ai_router: The dheera_ai router instance.
    ...
    for mg in fallback_model_group:
        ...
        try:
            # LOGGING
            kwargs = dheera_ai_router.log_retry(kwargs=kwargs, e=original_exception)
            ...
            response = await dheera_ai_router.async_function_with_fallbacks(
                *args, **kwargs
            )
...
async def log_success_fallback_event(
    ...
    custom_loggers = dheera_ai.logging_callback_manager.get_custom_loggers_for_type(
        CustomLogger
    )
...
def _check_non_standard_fallback_format(fallbacks: Optional[List[Any]]) -> bool:
    ...
    - List[Dict[<DheeraAIParamsTypedDict>, Any]]: e.g. [{"model": "claude-3-haiku", "messages": [{"role": "user", "content": "Hey, how's it going?"}]}]
    ...
    if all(isinstance(item, str) for item in fallbacks):
        ...
        for key in DheeraAIParamsTypedDict.__annotations__.keys():
