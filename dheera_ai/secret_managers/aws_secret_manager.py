...
Relevant issue: https://github.com/BerriAI/dheera_ai/issues/1883
...
import dheera_ai
from dheera_ai.proxy._types import KeyManagementSystem
...
def load_aws_kms(use_aws_kms: Optional[bool]):
    ...
    try:
        ...
        dheera_ai.secret_manager_client = kms_client
        dheera_ai._key_management_system = KeyManagementSystem.AWS_KMS
...
class AWSKeyManagementService_V2:
    ...
    def validate_environment(
        ...
        is_dheera_ai_license_in_env: bool = False

        if os.getenv("DHEERA_AI_LICENSE", None) is not None:
            is_dheera_ai_license_in_env = True
        elif os.getenv("DHEERA_AI_SECRET_AWS_KMS_DHEERA_AI_LICENSE", None) is not None:
            is_dheera_ai_license_in_env = True
        if is_dheera_ai_license_in_env is False:
            raise ValueError(
                "AWSKeyManagementService V2 is an Enterprise Feature. Please add a valid DHEERA_AI_LICENSE to your envionment."
            )
