...
from dheera_ai.secret_managers.secret_manager_handler import get_secret_from_manager
...
def get_secret(  # noqa: PLR0915
    ...
    key_management_system = dheera_ai._key_management_system
    ...
    try:
        if _should_read_secret_from_secret_manager() and dheera_ai.secret_manager_client is not None:
            try:
                client = dheera_ai.secret_manager_client
                key_manager = "local"
...
def _should_read_secret_from_secret_manager() -> bool:
    ...
    """
    if dheera_ai.secret_manager_client is not None:
        if dheera_ai._key_management_settings is not None:
            if (
                dheera_ai._key_management_settings.access_mode == "read_only"
                or dheera_ai._key_management_settings.access_mode == "read_and_write"
            ):
                return True
    return False
