...
class OpenAIVectorStoreOptions(TypedDict, total=False):
    ...
    Example (with credentials):
        {"custom_llm_provider": "openai", "dheera_ai_credential_name": "my-openai-creds"}
    """
    ...
    # Credentials (loaded from dheera_ai.credential_list if dheera_ai_credential_name is provided)
    dheera_ai_credential_name: Optional[str]  # Credential name to load from dheera_ai.credential_list
    api_key: Optional[str]  # Direct API key (alternative to dheera_ai_credential_name)
    api_base: Optional[str]  # Direct API base (alternative to dheera_ai_credential_name)
...
class BedrockVectorStoreOptions(TypedDict, total=False):
    ...
    Example (with credentials):
        {"custom_llm_provider": "bedrock", "dheera_ai_credential_name": "my-aws-creds"}
...
class VertexAIVectorStoreOptions(TypedDict, total=False):
    ...
    - Files are uploaded to GCS via dheera_ai.files.create_file, then imported into RAG corpus
