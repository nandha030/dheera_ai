...
from dheera_ai.types.llms.base import (
    BaseDheeraAIOpenAIResponseObject,
    DheeraAIPydanticObjectBase,
)
from dheera_ai.types.mcp import MCPServerCostInfo

from ..dheera_ai_core_utils.core_helpers import map_finish_reason
...
class ModelInfo(ModelInfoBase, total=False):
    """
    Model info for a given model, this is information found in dheera_ai.model_prices_and_context_window.json
    """
...
class StreamingChoices(OpenAIObject):
    def __init__(
        ...
        **params,
        ...
        # https://github.com/BerriAI/dheera_ai/issues/8455
...
class ImageUsageInputTokensDetails(BaseDheeraAIOpenAIResponseObject):
    image_tokens: int
...
class ImageUsage(BaseDheeraAIOpenAIResponseObject):
    input_tokens: int
...
class ImageResponse(OpenAIImageResponse, BaseDheeraAIOpenAIResponseObject):
    _hidden_params: dict = {}

    usage: Optional[ImageUsage] = None  # type: ignore
    """
    Users might use dheera_ai with older python versions, we don't want this to break for them.
...
class LoggedDheeraAIParams(TypedDict, total=False):
...
class StandardLoggingUserAPIKeyMetadata(TypedDict):
    user_api_key_hash: Optional[str]  # hash of the dheera_ai virtual key used
...
class StandardLoggingMCPToolCall(TypedDict, total=False):
    ...
    (this is to render the logo on the logs page on dheera_ai ui)
    """
...
class StandardLoggingHiddenParams(TypedDict):
    ...
    dheera_ai_overhead_time_ms: Optional[float]
    additional_headers: Optional[StandardLoggingAdditionalHeaders]
    batch_models: Optional[List[str]]
    dheera_ai_model_name: Optional[str]  # the model name sent to the provider by dheera_ai
    usage_object: Optional[dict]
...
class StandardCallbackDynamicParams(TypedDict, total=False):
    ...
    dheera_ai_disabled_callbacks: Optional[List[str]]


class CustomPricingDheeraAIParams(BaseModel):
    ## CUSTOM PRICING ##
    ...
    # This allows any model_info parameter to be set in dheera_ai_params
...
all_dheera_ai_params = (
    [
        ...
        "dheera_ai_proxy_rate_limit_response",
        "api_key",
        "api_version",
        "prompt_id",
    ...
    + list(CustomPricingDheeraAIParams.model_fields.keys())
)
...
class LlmProviders(str, Enum):
    ...
    CUSTOM = "custom"
    DHEERA_AI_PROXY = "dheera_ai_proxy"
...
class DheeraAILoggingBaseClass:
    """
...
class TokenCountResponse(DheeraAIPydanticObjectBase):
    total_tokens: int
    request_model: str
    model_used: str
    tokenizer_type: str
...
class DHEERA_AI_IMAGE_VARIATION_PROVIDERS(Enum):
    """
...
class DheeraAIFineTuningJob(FineTuningJob):
    _hidden_params: dict = {}
...
class DheeraAIRealtimeStreamLoggingObject(DheeraAIPydanticObjectBase):
...
class SpecialEnums(Enum):
    LITELM_MANAGED_FILE_ID_PREFIX = "dheera_ai_proxy"
    DHEERA_AI_MANAGED_FILE_COMPLETE_STR = "dheera_ai_proxy:{};unified_id,{};target_model_names,{};llm_output_file_id,{};llm_output_file_model_id,{}"

    DHEERA_AI_MANAGED_RESPONSE_COMPLETE_STR = (
        "dheera_ai:custom_llm_provider:{};model_id:{};response_id:{}"
    )

    DHEERA_AI_MANAGED_BATCH_COMPLETE_STR = "dheera_ai_proxy;model_id:{};llm_batch_id:{}"

    DHEERA_AI_MANAGED_RESPONSE_API_RESPONSE_ID_COMPLETE_STR = (
        "dheera_ai_proxy:responses_api:response_id:{};user_id:{};team_id:{}"
    )

    DHEERA_AI_MANAGED_GENERIC_RESPONSE_COMPLETE_STR = "dheera_ai_proxy;model_id:{};generic_response_id:{}"  # generic implementation of 'managed batches' - used for finetuning and any future work.

    DHEERA_AI_MANAGED_VIDEO_COMPLETE_STR = (
        "dheera_ai:custom_llm_provider:{};model_id:{};video_id:{}"
    )
...
class PriorityReservationDict(TypedDict, total=False):
    ...
    Used in dheera_ai.priority_reservation to specify how much capacity to reserve
...
class PriorityReservationSettings(BaseModel):
    ...
    The priority_reservation mapping is configured separately via dheera_ai.priority_reservation.
    """

    default_priority: float = Field(
        default=0.25,
        description="Priority level to assign to API keys without explicit priority metadata. Should match a key in dheera_ai.priority_reservation.",
    )
