"""
DheeraAI SDK Functions for Creating and Searching Vector Stores
"""
...
from dheera_ai.vector_stores.utils import VectorStoreRequestUtils
    ...
    **kwargs,
    ...
    try:
        ...
        if dheera_ai_params.mock_response and isinstance(
            dheera_ai_params.mock_response, dict
        ):
            return mock_vector_store_create_response(
                mock_response=VectorStoreCreateResponse(**dheera_ai_params.mock_response)
            )
...
def search(
    ...
    **kwargs,
    ...
    try:
        ...
        if (
            dheera_ai.vector_store_registry is not None
        ...
        # get llm provider logic
        dheera_ai_params = GenericDheeraAIParams(vector_store_id=vector_store_id, **kwargs)

        ## MOCK RESPONSE LOGIC
        if dheera_ai_params.mock_response and isinstance(
            dheera_ai_params.mock_response, (str, list)
        ):
            mock_results = None
            if isinstance(dheera_ai_params.mock_response, list):
                mock_results = dheera_ai_params.mock_response
