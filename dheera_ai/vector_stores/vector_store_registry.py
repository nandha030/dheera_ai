...
class VectorStoreIndexRegistry:
    def __init__(
        self, vector_store_indexes: List[DheeraAI_ManagedVectorStoreIndex] = []
    ):
        self.vector_store_indexes: List[DheeraAI_ManagedVectorStoreIndex] = (
            vector_store_indexes
        )

    def get_vector_store_indexes(self) -> List[DheeraAI_ManagedVectorStoreIndex]:
        """
    ...
    ) -> Optional[DheeraAI_ManagedVectorStoreIndex]:
        """
    ...
    def upsert_vector_store_index(
        self, vector_store_index: DheeraAI_ManagedVectorStoreIndex
    ):
        """
        ...
        """
        for i, _vector_store_index in enumerate[DheeraAI_ManagedVectorStoreIndex](
    ...
    async def _get_vector_store_indexes_from_db(
    ...
    ) -> List[DheeraAI_ManagedVectorStoreIndex]:
        """
        ...
        if prisma_client is not None:
            ...
            for vector_store in _vector_stores_from_db:
                ...
                _dheera_ai_managed_vector_store = DheeraAI_ManagedVectorStoreIndex(
                    **_dict_vector_store
                )
                vector_stores_from_db.append(_dheera_ai_managed_vector_store)
        return vector_stores_from_db


class VectorStoreRegistry:
    def __init__(self, vector_stores: List[DheeraAI_ManagedVectorStore] = []):
        self.vector_stores: List[DheeraAI_ManagedVectorStore] = vector_stores
        self.vector_store_ids_to_vector_store_map: Dict[
            str, DheeraAI_ManagedVectorStore
        ] = {}
    ...
    def get_vector_store_to_run(
        self, non_default_params: Dict, tools: Optional[List[Dict]] = None
    ) -> Optional[DheeraAI_ManagedVectorStore]:
        """
    ...
    def get_dheera_ai_managed_vector_store_from_registry(
    ...
    async def get_dheera_ai_managed_vector_store_from_registry_or_db(
        ...
        vector_store = self.get_dheera_ai_managed_vector_store_from_registry(vector_store_id)
    ...
    def get_dheera_ai_managed_vector_store_from_registry_by_name(
    ...
    def pop_vector_stores_to_run(
        self, non_default_params: Dict, tools: Optional[List[Dict]] = None
    ) -> List[DheeraAI_ManagedVectorStore]:
        """
        ...
        Returns:
            List of vector stores with tool parameters merged into dheera_ai_params
        """
        ...
        vector_stores_to_run: List[DheeraAI_ManagedVectorStore] = []
            ...
            for vector_store in self.vector_stores:
                if vector_store.get("vector_store_id") == vector_store_id:
                    ...
                    if vector_store_id in params_by_id:
                        existing_params = vector_store_copy.get("dheera_ai_params", {}) or {}
                        ...
                        vector_store_copy["dheera_ai_params"] = tool_params_dict
    ...
    async def pop_vector_stores_to_run_with_db_fallback(
        ...
        for vector_store_id in vector_store_ids:
            ...
            if vector_store is None and prisma_client is not None:
                try:
                    vector_store = await self.get_dheera_ai_managed_vector_store_from_registry_or_db(
    ...
    def load_vector_stores_from_config(self, vector_stores_config: List[Dict]):
        """
        Loads vector stores from the dheera_ai proxy config.yaml
        """
        for vector_store_config in vector_stores_config:
            # cast to VectorStoreConfig
            dheera_ai_vector_store_config = DheeraAI_VectorStoreConfig(
                **vector_store_config
            )
            vector_store_name = dheera_ai_vector_store_config.get("vector_store_name")
            vector_store_dheera_ai_params: Dict[str, Any] = (
                dheera_ai_vector_store_config.get("dheera_ai_params") or {}
            )

            vector_store_id = vector_store_dheera_ai_params.get("vector_store_id")
            if vector_store_id is None:
                raise ValueError(
                    f"vector_store_id is required for initializing vector store, got vector_store_id={vector_store_id}"
                )
            custom_llm_provider = vector_store_dheera_ai_params.get("custom_llm_provider")
            if custom_llm_provider is None:
                raise ValueError(
                    f"custom_llm_provider is required for initializing vector store, got custom_llm_provider={custom_llm_provider}"
                )

            dheera_ai_managed_vector_store = DheeraAI_ManagedVectorStore(
                vector_store_id=vector_store_id,
                custom_llm_provider=custom_llm_provider,
                dheera_ai_params=vector_store_dheera_ai_params,
                vector_store_name=vector_store_name,
                vector_store_description=vector_store_dheera_ai_params.get(
                    "vector_store_description"
                ),
                vector_store_metadata=vector_store_dheera_ai_params.get(
                    "vector_store_metadata"
                ),
                created_at=datetime.now(timezone.utc),
                updated_at=datetime.now(timezone.utc),
            )
            self.vector_stores.append(dheera_ai_managed_vector_store)
    ...
    def list_all_vector_stores(self) -> DheeraAI_ManagedVectorStoreListResponse:
        """
        List all vector stores in the required format

        Returns:
            DheeraAI_ManagedVectorStoreListResponse: A standardized response with vector store data
        """
    ...
    def add_vector_store_to_registry(self, vector_store: DheeraAI_ManagedVectorStore):
        """
    ...
    def update_vector_store_in_registry(
        self, vector_store_id: str, updated_data: DheeraAI_ManagedVectorStore
    ):
    ...
    async def _get_vector_stores_from_db(
        ...
        if prisma_client is not None:
            ...
            for vector_store in _vector_stores_from_db:
                _dict_vector_store = dict(vector_store)
                _dheera_ai_managed_vector_store = DheeraAI_ManagedVectorStore(
                    **_dict_vector_store
                )
                vector_stores_from_db.append(_dheera_ai_managed_vector_store)
        return vector_stores_from_db
    ...
    def get_credentials_for_vector_store(self, vector_store_id: str) -> Dict[str, Any]:
        ...
        for vector_store in self.vector_stores:
            if vector_store.get("vector_store_id") == vector_store_id:
                credentials = vector_store.get("dheera_ai_credential_name")
                if credentials:
