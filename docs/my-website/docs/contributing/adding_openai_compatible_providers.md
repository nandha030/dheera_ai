...
1. Edit `dheera_ai/llms/openai_like/providers.json`
2. Add your provider configuration
3. Test with: `dheera_ai.completion(model="your_provider/model-name", ...)`
...
For these cases, create a config class in `dheera_ai/llms/your_provider/chat/transformation.py` that inherits from `OpenAIGPTConfig` or `OpenAILikeChatConfig`.
...
See existing providers in `dheera_ai/llms/openai_like/providers.json` for examples.
