...
Dheera AI Supports logging to the following Datdog Integrations:
...
We will use the `--config` to set `dheera_ai.callbacks = ["datadog"]` this will log all successful LLM calls to DataDog
...
DD_SOURCE="dheera_ai_dev"       # [OPTIONAL] your datadog source. use to differentiate dev vs. prod deployments
```
...
```shell
DHEERA_AI_DD_AGENT_HOST="localhost"         # hostname or IP of DataDog agent
DHEERA_AI_DD_AGENT_PORT="10518"             # [OPTIONAL] port of DataDog agent (default: 10518)
DD_API_KEY="5f2d0f310***********"         # [OPTIONAL] your datadog API Key (agent handles auth)
DD_SOURCE="dheera_ai_dev"                   # [OPTIONAL] your datadog source
```

When `DHEERA_AI_DD_AGENT_HOST` is set, logs are sent to the agent instead of directly to DataDog API. This is useful for:
...
**Note:** We use `DHEERA_AI_DD_AGENT_HOST` instead of `DD_AGENT_HOST` to avoid conflicts with `ddtrace` which automatically sets `DD_AGENT_HOST` for APM tracing.
...
Use `ddtrace-run` to enable [Datadog Tracing](https://ddtrace.readthedocs.io/en/stable/installation_quickstart.html) on dheera_ai proxy

**DD Tracer**
Pass `USE_DDTRACE=true` to the docker run command. When `USE_DDTRACE=true`, the proxy will run `ddtrace-run dheera_ai` as the `ENTRYPOINT` instead of just `dheera_ai`
...
Dheera AI supports customizing the following Datadog environment variables
...
| `DHEERA_AI_DD_AGENT_HOST` | Hostname or IP of DataDog agent (e.g., "localhost"). When set, logs are sent to agent instead of direct API | None | ❌ No |
| `DHEERA_AI_DD_AGENT_PORT` | Port of DataDog agent for log intake | "10518" | ❌ No |
| `DD_ENV` | Environment tag for your logs (e.g., "production", "staging") | "unknown" | ❌ No |
| `DD_SERVICE` | Service name for your logs | "dheera_ai-server" | ❌ No |
| `DD_SOURCE` | Source name for your logs | "dheera_ai" | ❌ No |
...
\* **Optional when using DataDog Agent**: Set `DHEERA_AI_DD_AGENT_HOST` to use agent mode; `DD_API_KEY` and `DD_SITE` are not required
