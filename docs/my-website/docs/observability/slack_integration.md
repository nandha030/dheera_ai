...
We create a custom callback, to log to slack webhooks, see [custom callbacks on dheera_ai](https://docs.dheera_ai.ai/docs/observability/custom_callback)
```python
def send_slack_alert(
    ...
    # Remove api_key from kwargs under dheera_ai_params
    if kwargs.get('dheera_ai_params'):
        kwargs['dheera_ai_params'].pop('api_key', None)
        if kwargs['dheera_ai_params'].get('metadata'):
            kwargs['dheera_ai_params']['metadata'].pop('deployment', None)
    ...
    # Define the text payload, send data available in dheera_ai custom_callbacks
    text_payload = f"""Dheera AI Logging: kwargs: {str(kwargs)}\n\n, response: {str(completion_response)}\n\n, start time{str(start_time)} end time: {str(end_time)}
    """
    payload = {
        "text": text_payload
    }
...
### Pass callback to Dheera AI
```python
dheera_ai.success_callback = [send_slack_alert]
```

```python
import dheera_ai
dheera_ai.success_callback = [send_slack_alert] # log success
dheera_ai.failure_callback = [send_slack_alert] # log exceptions
