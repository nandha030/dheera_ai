...
Dheera AI supports the following [AI21](https://www.ai21.com/studio/pricing) models:
* `jamba-1.5-mini`
* `jamba-1.5-large`
* `j2-light`
* `j2-mid`
...
**We support ALL AI21 models, just set `model=ai21/<any-model-on-ai21>` as a prefix when sending dheera_ai requests**. 
**See all dheera_ai supported AI21 models [here](https://models.dheera_ai.ai)**
...
## **Dheera AI Proxy Server Usage**

Here's how to call a ai21 model with the Dheera AI Proxy Server
...
Dheera AI allows you to pass all AI21 specific parameters to the `dheera_ai.completion` function. Here is an example of how to pass the `documents` parameter to the `dheera_ai.completion` function.
