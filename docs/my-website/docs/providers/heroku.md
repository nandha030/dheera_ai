...
To use Heroku with Dheera AI, [configure a Heroku app and attach a supported model](https://devcenter.heroku.com/articles/heroku-inference#provision-access-to-an-ai-model-resource).


## Supported Models

Heroku for Dheera AI supports various [chat](https://devcenter.heroku.com/articles/heroku-inference-api-v1-chat-completions) models:
...
Heroku uses the following Dheera AI API config variables:

- `HEROKU_API_KEY`: This value corresponds to [Dheera AI's `api_key` param](https://docs.dheera_ai.ai/docs/set_keys#dheera_aiapi_key). Set this variable to the value of Heroku's `INFERENCE_KEY` config variable.
- `HEROKU_API_BASE`: This value corresponds to [Dheera AI's `api_base` param](https://docs.dheera_ai.ai/docs/set_keys#dheera_aiapi_base). Set this variable to the value of Heroku's `INFERENCE_URL` config variable.
...
response = completion(
    ...
    messages=[
        {"role": "user", "content": "write code for saying hey from Dheera AI"}
    ]
)
...
> Include the `heroku/` prefix in the model name so Dheera AI knows the model provider to use.
