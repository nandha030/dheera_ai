...
Dheera AI supports vLLM's Batch and Files API for processing large volumes of requests asynchronously.
...
Define your vLLM model in `config.yaml`. Dheera AI uses the model name to route batch requests to the correct vLLM server.
...
:::tip Model Routing
Dheera AI needs to know which model (and therefore which vLLM server) to use for batch operations. Specify the model using the `x-dheera_ai-model` header when uploading files. Dheera AI will encode this model info into the file ID, so subsequent batch operations automatically route to the correct server.
...
curl http://localhost:4000/v1/files \
  ...
  -H "x-dheera_ai-model: my-vllm-model" \
  -F purpose="batch" \
...
async def run_vllm_batch():
    ...
    # Poll for completion
    while True:
        batch_status = await dheera_ai.aretrieve_batch(
            batch_id=batch.id,
            custom_llm_provider="hosted_vllm",
        )
    ...
    if batch_status.output_file_id:
        results = await dheera_ai.afile_content(
...
When you upload a file with `x-dheera_ai-model: my-vllm-model`, Dheera AI:
...
This enables multi-tenant batch processing where different teams can use different vLLM deployments through the same Dheera AI proxy.
