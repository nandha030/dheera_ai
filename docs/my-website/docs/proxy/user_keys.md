...
Dheera AI Proxy is **OpenAI-Compatible**, and supports:
* /chat/completions 
* /embeddings
* /completions 
* /image/generations 
...
* /audio/speech
* [Assistants API endpoints](https://docs.dheera_ai.ai/docs/assistants)
* [Batches API endpoints](https://docs.dheera_ai.ai/docs/batches)
* [Fine-Tuning API endpoints](https://docs.dheera_ai.ai/docs/fine_tuning)

Dheera AI Proxy is **Azure OpenAI-compatible**:
* /chat/completions
* /completions
* /embeddings 

Dheera AI Proxy is **Anthropic-compatible**: 
* /messages 

Dheera AI Proxy is **Vertex AI compatible**:
...
These are **selected examples**. Dheera AI Proxy is **OpenAI-Compatible**, it works with any project that calls OpenAI. Just change the `base_url`, `api_key` and `model`.

To pass provider-specific args, [go here](https://docs.dheera_ai.ai/docs/completion/provider_specific_params#proxy-usage)

To drop unsupported params (E.g. frequency_penalty for bedrock with librechat), [go here](https://docs.dheera_ai.ai/docs/completion/drop_params#openai-proxy-usage)
...
<TabItem value="dheera_ai_sdk" label="Dheera AI Python SDK">

[**👉 Go Here**](../providers/dheera_ai_proxy#send-all-sdk-requests-to-dheera_ai-proxy)
...
llm = AzureOpenAI(
    engine="azure-gpt-3.5",               # model_name on dheera_ai proxy
    temperature=0.0,
    azure_endpoint="http://0.0.0.0:4000", # dheera_ai proxy endpoint
...
Set `base_url` to the Dheera AI Proxy server
...
Dheera AI Proxy is running on port `4000`, set `4000` as the proxy below
```yaml
...
config_list=[
    {
        "model": "my-fake-model",
        "api_base": "http://localhost:4000",  #dheera_ai compatible endpoint
        "api_type": "open_ai",
...
👉 **[Here's how to use dheera_ai proxy with Vertex, boto3, Anthropic SDK - in the native format](../pass_through/vertex_ai.md)**
...
This same request will be sent to the following model groups on the [dheera_ai proxy config.yaml](https://docs.dheera_ai.ai/docs/proxy/configs)
- `model_name="llama3"`
...
curl --location 'http://localhost:4000/chat/completions' \
    ...
    "messages": [
        {
        "role": "user",
        "content": "is dheera_ai getting better"
        }
    ]
}'
```
