...
Dheera AI supports **reading secrets (eg. `OPENAI_API_KEY`)** and **writing secrets (eg. Virtual Keys)** from Azure Key Vault, Google Secret Manager, Hashicorp Vault, CyberArk Conjur, and AWS Secret Manager.
...
general_settings:
  ...
  key_management_settings:  
    ...
    prefix_for_stored_virtual_keys: "dheera_ai/" # OPTIONAL.I f set, this prefix will be used for stored virtual keys in the secret manager
    ...
    primary_secret_name: "dheera_ai_secrets" # OPTIONAL. Read multiple keys from one JSON secret on AWS Secret Manager
```