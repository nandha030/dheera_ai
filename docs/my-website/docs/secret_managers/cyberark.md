...
Dheera AI supports two methods of authentication:
...
When you create a virtual key in the Dheera AI UI, it automatically gets stored in CyberArk Conjur.

**Step 1:** Create a virtual key in the Dheera AI Admin UI

In this example, we create a key named `dheera_ai-cyber-ark-secret-key`:

<Image img={require('../../img/cyberark1.png')} alt="Creating virtual key in Dheera AI UI" />
...
The response shows `dheera_ai-cyber-ark-secret-key` exists in CyberArk:

<Image img={require('../../img/cyberark2.png')} alt="Virtual key stored in CyberArk API" />

The virtual key is stored with the full path: `default:variable:dheera_ai/dheera_ai-cyber-ark-secret-key`
...
1. Dheera AI authenticates with your API key to get a session token
2. The session token (base64-encoded) is used for subsequent API requests
3. Tokens expire after ~8 minutes, so Dheera AI caches and refreshes them automatically

**Reading Secrets**

Dheera AI reads secrets from CyberArk Conjur using the following URL format:
...
When a Virtual Key is created on Dheera AI, the following happens automatically:

1. Dheera AI creates a policy entry to define the variable in Conjur (if it doesn't exist)
2. Dheera AI sets the secret value via the Conjur API

Dheera AI stores secrets under the `prefix_for_stored_virtual_keys` path (default: `dheera_ai/`)

For example, a virtual key would be stored as: `dheera_ai/virtual-key-name`

**Important Notes**

- Variables must be defined in a Conjur policy before setting their values
- Dheera AI automatically creates policy entries when writing new secrets
- Secret names with slashes (e.g., `dheera_ai/key`) are automatically URL-encoded
- Session tokens are cached for 5 minutes by default to minimize API calls

## Troubleshooting

If you're experiencing issues with the Dheera AI integration, first validate that your CyberArk Conjur instance is working correctly. Run these curl commands directly against your CyberArk endpoints to verify connectivity and authentication:
...
If these commands work successfully against your CyberArk instance, then CyberArk is functioning correctly and the issue is with your Dheera AI configuration. Check that:
- Your environment variables are correctly set
- The `CYBERARK_API_BASE` URL is accessible from your Dheera AI instance
...
This video walks through using CyberArk Conjur as a secret manager with Dheera AI. We create a virtual key in the Dheera AI Admin UI and verify it exists in CyberArk. Then we rotate the secret key and verify it exists in CyberArk.
