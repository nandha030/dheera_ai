...
Dheera AI makes it easy to call your public, private or the default huggingface endpoints. 
...
messages = [{"role": "user", "content": "Hey, how's it going?"}] # Dheera AI follows the OpenAI format 
...
- messages: This is the input. We accept the OpenAI chat format. For huggingface, by default we iterate through the list and add the message["content"] to the prompt. [Relevant Code](https://github.com/BerriAI/dheera_ai/blob/6aff47083be659b80e00cb81eb783cb24db2e183/dheera_ai/llms/huggingface_restapi.py#L46)
- custom_llm_provider: Optional param. This is an optional flag, needed only for Azure, Replicate, Huggingface and Together-ai (platforms where you deploy your own models). This enables dheera_ai to route to the right provider, for your model. 
...
What's happening? 
- api_base: Optional param. Since this uses a deployed endpoint (not the [default huggingface inference endpoint](https://github.com/BerriAI/dheera_ai/blob/6aff47083be659b80e00cb81eb783cb24db2e183/dheera_ai/llms/huggingface_restapi.py#L35)), we pass that to Dheera AI. 
...
On Dheera AI there's 3 ways you can pass in an api_key. 
