...
Reduce costs by up to 90% by using Dheera AI to auto-inject prompt caching checkpoints.
...
Dheera AI can automatically inject prompt caching checkpoints into your requests to LLM providers. This allows:

- **Cost Reduction**: Long, static parts of your prompts can be cached to avoid repeated processing
- **No need to modify your application code**: You can configure the auto-caching behavior in the Dheera AI UI or in the `dheera_ai config.yaml` file.

## Configuration

You need to specify `cache_control_injection_points` in your model configuration. This tells Dheera AI:
1. Where to add the caching directive (`location`)
2. Which message to target (`role`)

Dheera AI will then automatically add a `cache_control` directive to the specified messages in your requests:
...
- Dheera AI automatically adds `cache_control` to the **last content block** of matching messages (per Anthropic's API specification)

**Dheera AI's Modified Request:**

Dheera AI automatically transforms your request by adding `cache_control` to the last content block of the system message:
    ...
    "messages": [
        {
            ...
            "content": [
                ...
                },
                {
                    ...
                    "cache_control": {"type": "ephemeral"}  // Added by Dheera AI
                }
            ]
        },
        {
...
Dheera AI adds `cache_control` to the last content block of the targeted message (index -1 = last message):
        ...
        },
        {
            ...
            "content": [
                ...
                },
                {
                    ...
                    "cache_control": {"type": "ephemeral"}  // Added by Dheera AI to last content block only
                }
            ]
        }
    ]
...
<Tabs>
<TabItem value="dheera_ai config.yaml" label="dheera_ai config.yaml">
...
On the Dheera AI UI, you can specify the `cache_control_injection_points` in the `Advanced Settings` tab when adding a model.
...
### 2. Dheera AI's Modified Request

Dheera AI auto-injects the caching directive into the system message based on our configuration:
