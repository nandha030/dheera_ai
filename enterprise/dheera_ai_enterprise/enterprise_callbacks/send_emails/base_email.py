...
from dheera_ai.integrations.email_templates.key_created_email import (
    KEY_CREATED_EMAIL_TEMPLATE,
)
from dheera_ai.integrations.email_templates.key_rotated_email import (
    KEY_ROTATED_EMAIL_TEMPLATE,
)
from dheera_ai.integrations.email_templates.user_invitation_email import (
    USER_INVITATION_EMAIL_TEMPLATE,
)
from dheera_ai.proxy._types import InvitationNew, UserAPIKeyAuth, WebhookEvent
from dheera_ai.secret_managers.main import get_secret_bool
from dheera_ai.types.integrations.slack_alerting import DHEERA_AI_LOGO_URL


class BaseEmailLogger(CustomLogger):
    DEFAULT_DHEERA_AI_EMAIL = "notifications@alerts.dheera_ai.ai"
    DEFAULT_SUPPORT_EMAIL = "support@berri.ai"
    DEFAULT_SUBJECT_TEMPLATES = {
        EmailEvent.new_user_invitation: "DheeraAI: {event_message}",
        EmailEvent.virtual_key_created: "DheeraAI: {event_message}",
        EmailEvent.virtual_key_rotated: "DheeraAI: {event_message}",
    }
    ...
    async def send_user_invitation_email(self, event: WebhookEvent):
        ...
        await self.send_email(
            from_email=self.DEFAULT_DHEERA_AI_EMAIL,
    ...
    async def _get_email_params(
        ...
        logo_url = get_custom_or_default(custom_logo, DHEERA_AI_LOGO_URL, "logo URL")
        ...
        if email_event == EmailEvent.new_user_invitation:
            ...
            subject_template = "DheeraAI: {event_message}"

        subject = subject_template.format(event_message=event_message) if event_message else "DheeraAI Notification"
        ...
        if unused_custom_fields:
            ...
            warning_msg = (
                ...
                "This is an Enterprise feature. To use custom email fields, please upgrade to DheeraAI Enterprise. "
                "Schedule a meeting here: https://calendly.com/d/4mp-gd3-k5k/dheera_ai-1-1-onboarding-chat"
            )
    ...
    async def _get_or_create_invitation(self, user_id: str):
        ...
        try:
            ...
            existing_invitations = await prisma_client.db.dheera_ai_invitationlink.find_many(
                where={"user_id": user_id},
