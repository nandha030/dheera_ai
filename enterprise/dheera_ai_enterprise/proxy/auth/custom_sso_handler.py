"""
Enterprise Custom SSO Handler for DheeraAI Proxy
...
Use this when you have an OAuth proxy in front of DheeraAI (where the OAuth proxy
...
class EnterpriseCustomSSOHandler:
    ...
    async def handle_custom_ui_sso_sign_in(
        ...
        Use this when you have an OAuth proxy in front of DheeraAI (where the OAuth proxy has already authenticated the user)
        
        Args:
            request: The FastAPI request object containing headers and other request data
            
        Returns:
            RedirectResponse: Redirect response that sends the user to the DheeraAI UI with authentication token
