# What is this?
## This hook is used to check for DheeraAI managed files in the request body, and replace them with model-specific file id
...
from dheera_ai import Router, verbose_logger
...
from dheera_ai.dheera_ai_core_utils.prompt_templates.common_utils import extract_file_data
from dheera_ai.llms.base_llm.files.transformation import BaseFileEndpoints
from dheera_ai.proxy._types import (
    CallTypes,
    DheeraAI_ManagedFileTable,
    DheeraAI_ManagedObjectTable,
    UserAPIKeyAuth,
)
...
if TYPE_CHECKING:
    ...
    from dheera_ai.proxy.utils import InternalUsageCache as _InternalUsageCache
    from dheera_ai.proxy.utils import PrismaClient as _PrismaClient
...
class _PROXY_DheeraAIManagedFiles(CustomLogger, BaseFileEndpoints):
    ...
    async def store_unified_file_id(
        ...
        verbose_logger.info(
            f"Storing DheeraAI Managed File object with id={file_id} in cache"
        )
        if file_object is not None:
            dheera_ai_managed_file_object = DheeraAI_ManagedFileTable(
            ...
            await self.internal_usage_cache.async_set_cache(
                key=file_id,
                value=dheera_ai_managed_file_object.model_dump(),
        ...
        result = await self.prisma_client.db.dheera_ai_managedfiletable.create(
            data=db_data
        )
        verbose_logger.debug(
            f"DheeraAI Managed File object with id={file_id} stored in db: {result}"
        )
    ...
    async def store_unified_object_id(
        ...
        verbose_logger.info(
            f"Storing DheeraAI Managed {file_purpose} object with id={unified_object_id} in cache"
        )
        dheera_ai_managed_object = DheeraAI_ManagedObjectTable(
        ...
        await self.internal_usage_cache.async_set_cache(
            ...
            value=dheera_ai_managed_object.model_dump(),
            dheera_ai_parent_otel_span=dheera_ai_parent_otel_span,
        )

        await self.prisma_client.db.dheera_ai_managedobjecttable.upsert(
    ...
    async def get_unified_file_id(
        self, file_id: str, dheera_ai_parent_otel_span: Optional[Span] = None
    ) -> Optional[DheeraAI_ManagedFileTable]:
        ## CHECK CACHE
        result = cast(
            Optional[dict],
        ...
        if result:
            return DheeraAI_ManagedFileTable(**result)

        ## CHECK DB
        db_object = await self.prisma_client.db.dheera_ai_managedfiletable.find_first(
            where={"unified_file_id": file_id}
        )

        if db_object:
            return DheeraAI_ManagedFileTable(**db_object.model_dump())
        return None
    ...
    async def delete_unified_file_id(
        ...
        ## get old value
        initial_value = await self.prisma_client.db.dheera_ai_managedfiletable.find_first(
            where={"unified_file_id": file_id}
        )
        if initial_value is None:
            raise Exception(f"DheeraAI Managed File object with id={file_id} not found")
        ## delete old value
        await self.internal_usage_cache.async_set_cache(
            key=file_id,
            value=None,
        ...
        await self.prisma_client.db.dheera_ai_managedfiletable.delete(
            where={"unified_file_id": file_id}
        )
    ...
    async def can_user_call_unified_file_id(
        ...
        managed_file = await self.prisma_client.db.dheera_ai_managedfiletable.find_first(
    ...
    async def can_user_call_unified_object_id(
        ...
        user_id = user_api_key_dict.user_id
        managed_object = (
            await self.prisma_client.db.dheera_ai_managedobjecttable.find_first(
                where={"unified_object_id": unified_object_id}
            )
        )
    ...
    async def get_user_created_file_ids(
        ...
        """
        file_ids = await self.prisma_client.db.dheera_ai_managedfiletable.find_many(
            where={
                "created_by": user_api_key_dict.user_id,
    ...
    async def async_pre_call_hook(  # noqa: PLR0915
        ...
        """
        - Detect dheera_ai_proxy/ file_id
        - add dictionary of mappings of dheera_ai_proxy/ file_id -> provider_file_id => {dheera_ai_proxy/file_id: {"model_id": id, "file_id": provider_file_id}}
        """
        ...
        if is_completion_call:
                ...
                if file_ids:
                    ...
                    if is_vertex_ai:
                        await self._convert_storage_files_to_base64(
                            messages=messages,
                            file_ids=file_ids,
                            dheera_ai_parent_otel_span=user_api_key_dict.parent_otel_span,
                        )
            ...
            if potential_llm_object_id and retrieve_object_id:
                ...
                if potential_model_id is None:
                    raise Exception(
                        f"DheeraAI Managed {accessor_key} with id={retrieve_object_id} is invalid - does not contain encoded model_id."
                    )
    ...
    async def get_model_file_id_mapping(
        self, file_ids: List[str], dheera_ai_parent_otel_span: Span
    ) -> dict:
        """
        Get model-specific file IDs for a list of proxy file IDs.
        Returns a dictionary mapping dheera_ai_proxy/ file_id -> model_id -> model_file_id

        1. Get all the dheera_ai_proxy/ file_ids from the messages
        2. For each file_id, search for cache keys matching the pattern file_id:*
        3. Return a dictionary of mappings of dheera_ai_proxy/ file_id -> model_id -> model_file_id

        Example:
        {
            "dheera_ai_proxy/file_id": {
    ...
    async def acreate_file(
        ...
        response = await _PROXY_DheeraAIManagedFiles.return_unified_file_id(
    ...
    async def return_unified_file_id(
        ...
        unified_file_id = SpecialEnums.DHEERA_AI_MANAGED_FILE_COMPLETE_STR.value.format(
            file_type,
            str(uuid.uuid4()),
    ...
    def get_unified_generic_response_id(
        ...
        unified_generic_response_id = (
            SpecialEnums.DHEERA_AI_MANAGED_GENERIC_RESPONSE_COMPLETE_STR.value.format(
    ...
    def get_unified_batch_id(self, batch_id: str, model_id: str) -> str:
        unified_batch_id = SpecialEnums.DHEERA_AI_MANAGED_BATCH_COMPLETE_STR.value.format(
            model_id, batch_id
        )
    ...
    def get_unified_output_file_id(
        ...
        unified_output_file_id = (
            SpecialEnums.DHEERA_AI_MANAGED_FILE_COMPLETE_STR.value.format(
                "application/json",
                str(uuid.uuid4()),
                model_name or "",
                output_file_id,
    ...
    async def async_post_call_success_hook(
        ...
        if isinstance(response, DheeraAIBatch):
            ...
            if (unified_batch_id or unified_file_id) and model_id:
                ...
                if (
                    ...
                    file_object = None
                    try:
                        # Use dheera_ai to retrieve the file object from the provider
                        from dheera_ai import afile_retrieve
        ...
        elif isinstance(response, DheeraAIFineTuningJob):
            ## Check if unified_file_id is in the response
            unified_file_id = response._hidden_params.get(
                "unified_file_id"
            )  # managed file id
    ...
    async def afile_retrieve(
        self, file_id: str, dheera_ai_parent_otel_span: Optional[Span]
    ) -> OpenAIFileObject:
    ...
    async def afile_content(
        ...
        **data: Dict,
        ...
        model_file_id_mapping = (
            model_file_id_mapping
            or await self.get_model_file_id_mapping([file_id], dheera_ai_parent_otel_span)
        )
        ...
        if specific_model_file_id_mapping:
            ...
            raise Exception(
                f"DheeraAI Managed File object with id={file_id} not found. Checked model id's: {specific_model_file_id_mapping.keys()}. Errors: {exception_dict}"
            )
        else:
            raise Exception(f"DheeraAI Managed File object with id={file_id} not found")
    ...
    async def _convert_storage_files_to_base64(
        ...
        for file_id in file_ids:
            ...
            db_file = await self.prisma_client.db.dheera_ai_managedfiletable.find_first(
                where={"unified_file_id": file_id}
            )
            ...
            try:
                from dheera_ai.llms.base_llm.files.storage_backend_factory import get_storage_backend
