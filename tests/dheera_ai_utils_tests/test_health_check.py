...
async def test_azure_health_check():
    response = await dheera_ai.ahealth_check(
        model_params={
...
def test_update_dheera_ai_params_for_health_check():
    """
    Test if _update_dheera_ai_params_for_health_check correctly:
    ...
    """
    from dheera_ai.proxy.health_check import _update_dheera_ai_params_for_health_check
    ...
    updated_params = _update_dheera_ai_params_for_health_check(model_info, dheera_ai_params)
    ...
    for prefix in cris_prefixes:
        ...
        updated_params = _update_dheera_ai_params_for_health_check(
            model_info, dheera_ai_params
        )
        assert (
...
async def test_perform_health_check_with_health_check_model():
    ...
    """
    from dheera_ai.proxy.health_check import _perform_health_check

    # Mock model list with health_check_model specified
    model_list = [
        {
            "dheera_ai_params": {"model": "openai/*", "api_key": "fake-key"},
            "model_info": {
                "mode": "chat",
    ...
    async def mock_health_check(dheera_ai_params, **kwargs):
        health_check_calls.append(dheera_ai_params["model"])
        return {"status": "healthy"}

    with patch("dheera_ai.ahealth_check", side_effect=mock_health_check):
...
async def test_health_check_bad_model():
    ...
    with patch(
        "dheera_ai.ahealth_check", side_effect=mock_health_check
...
async def test_image_generation_health_check_prompt(monkeypatch):
    ...
    import importlib
    import dheera_ai.constants as dheera_ai_constants
    import dheera_ai.proxy.health_check as health_check

    def reload_modules():
        reloaded_constants = importlib.reload(dheera_ai_constants)
    ...
    async def run_health_check(health_check_module):
        ...
        async def mock_health_check(dheera_ai_params, mode=None, prompt=None, input=None):
            health_check_calls.append(
                {
                    "mode": mode,
                    "prompt": prompt,
                    "model": dheera_ai_params.get("model"),
                }
            )
            return {"status": "healthy"}
        ...
        model_list = [
            {
                "dheera_ai_params": {"model": "dall-e-3", "api_key": "fake-key"},
                "model_info": {
                    "mode": "image_generation",
                },
            }
        ...
        with patch(
            "dheera_ai.proxy.health_check.dheera_ai.ahealth_check",
    ...
    dheera_ai_constants, health_check = reload_modules()
    health_check_calls = await run_health_check(health_check)

    assert len(health_check_calls) == 1
    assert (
        health_check_calls[0]["prompt"] == dheera_ai_constants.DEFAULT_HEALTH_CHECK_PROMPT
    )
