...
def test_add_string_callback():
    """
    Test adding a string callback to dheera_ai.callbacks - only 1 instance of the string callback should be added
    """
    manager = LoggingCallbackManager()
    test_callback = "test_callback"

    # Add string callback
    manager.add_dheera_ai_callback(test_callback)
    assert test_callback in dheera_ai.callbacks

    # Test duplicate prevention
    manager.add_dheera_ai_callback(test_callback)
    assert dheera_ai.callbacks.count(test_callback) == 1
...
def test_duplicate_langfuse_logger_test():
    ...
    for _ in range(10):
        ...
        manager.add_dheera_ai_success_callback(langfuse_logger)
    print("dheera_ai.success_callback: ", dheera_ai.success_callback)
...
def test_duplicate_multiple_loggers_test():
    ...
    for _ in range(10):
        ...
        manager.add_dheera_ai_success_callback(langfuse_logger)
        manager.add_dheera_ai_success_callback(otel_logger)
    print("dheera_ai.success_callback: ", dheera_ai.success_callback)
...
def test_add_function_callback():
    ...
    manager.add_dheera_ai_callback(test_func)
    assert test_func in dheera_ai.callbacks

    # Test duplicate prevention
    manager.add_dheera_ai_callback(test_func)
    assert dheera_ai.callbacks.count(test_func) == 1
...
def test_add_custom_logger(mock_custom_logger):
    ...
    # Add custom logger
    manager.add_dheera_ai_callback(mock_custom_logger)
    assert mock_custom_logger in dheera_ai.callbacks
...
def test_add_multiple_callback_types(mock_custom_logger):
    ...
    manager.add_dheera_ai_callback(string_callback)
    manager.add_dheera_ai_callback(test_func)
    manager.add_dheera_ai_callback(mock_custom_logger)

    assert string_callback in dheera_ai.callbacks
    assert test_func in dheera_ai.callbacks
    assert mock_custom_logger in dheera_ai.callbacks
    assert len(dheera_ai.callbacks) == 3
...
def test_success_failure_callbacks():
    ...
    # Add callbacks
    manager.add_dheera_ai_success_callback(success_callback)
    manager.add_dheera_ai_failure_callback(failure_callback)

    assert success_callback in dheera_ai.success_callback
    assert failure_callback in dheera_ai.failure_callback
...
def test_async_callbacks():
    ...
    # Add async callbacks
    manager.add_dheera_ai_async_success_callback(async_success)
    manager.add_dheera_ai_async_failure_callback(async_failure)

    assert async_success in dheera_ai._async_success_callback
    assert async_failure in dheera_ai._async_failure_callback
...
def test_remove_callback_from_list_by_object():
    ...
    def TestObject():
        def __init__(self):
            manager.add_dheera_ai_callback(self.callback)
            manager.add_dheera_ai_success_callback(self.callback)
            manager.add_dheera_ai_failure_callback(self.callback)
            manager.add_dheera_ai_async_success_callback(self.callback)
            manager.add_dheera_ai_async_failure_callback(self.callback)
    ...
    manager.remove_callback_from_list_by_object(dheera_ai.callbacks, obj)
    manager.remove_callback_from_list_by_object(dheera_ai.success_callback, obj)
    manager.remove_callback_from_list_by_object(dheera_ai.failure_callback, obj)
    manager.remove_callback_from_list_by_object(dheera_ai._async_success_callback, obj)
    manager.remove_callback_from_list_by_object(dheera_ai._async_failure_callback, obj)
...
def test_reset_callbacks(callback_manager):
    ...
    callback_manager.add_dheera_ai_callback("test")
    callback_manager.add_dheera_ai_success_callback("success")
    callback_manager.add_dheera_ai_failure_callback("failure")
    callback_manager.add_dheera_ai_async_success_callback("async_success")
    callback_manager.add_dheera_ai_async_failure_callback("async_failure")
...
async def test_slack_alerting_callback_registration(callback_manager):
    """
    Test that dheera_ai callbacks are correctly registered for slack alerting
    ...
    with patch('dheera_ai.integrations.SlackAlerting.slack_alerting.get_async_httpx_client') as mock_http:
        ...
        assert isinstance(dheera_ai.callbacks[0], SlackAlerting)
        ...
        assert len(dheera_ai.callbacks) == 1  # Regular callback for outage alerts
        assert len(dheera_ai.success_callback) == 1  # Success callback for response_taking_too_long
        assert isinstance(dheera_ai.callbacks[0], SlackAlerting)
        ...
        assert dheera_ai.success_callback[0] == response_taking_too_long_callback
