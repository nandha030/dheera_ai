...
import pytest
import dheera_ai
from dheera_ai.llms.azure.azure import get_azure_ad_token_from_oidc
from dheera_ai.llms.bedrock.chat import BedrockConverseLLM, BedrockLLM
...
def test_aws_secret_manager():
    ...
    secret_val = get_secret("dheera_ai_master_key")
    ...
    assert secret_val["dheera_ai_master_key"] == "sk-1234"
...
def test_oidc_circle_v1_with_amazon():
    ...
    aws_role_name = "arn:aws:iam::335785316107:role/dheera_ai-github-unit-tests-circleci-v1-assume-only"
...
def test_google_secret_manager():
    ...
    from dheera_ai.secret_managers.google_secret_manager import GoogleSecretManager
...
def test_should_read_secret_from_secret_manager():
    ...
    """
    from dheera_ai.types.secret_managers.main import KeyManagementSettings

    # Test when secret manager client is None
    dheera_ai.secret_manager_client = None
    ...
    dheera_ai.secret_manager_client = "dummy_client"
    dheera_ai._key_management_settings = KeyManagementSettings(access_mode="read_only")
    ...
    dheera_ai._key_management_settings = KeyManagementSettings(access_mode="write_only")
    assert _should_read_secret_from_secret_manager() is False
...
def test_key_management_settings_defaults():
    ...
    assert settings.prefix_for_stored_virtual_keys == "dheera_ai/"
...
def test_key_management_settings_custom_values():
    ...
    custom_description = "DheeraAI-managed API key for development"

    settings = KeyManagementSettings(
        store_virtual_keys=True,
        prefix_for_stored_virtual_keys="dheera_ai/custom/",
        access_mode="read_and_write",
        primary_secret_name="primary/dheera_ai/keys",
        description=custom_description,
        tags=custom_tags,
    )

    assert settings.store_virtual_keys is True
    assert settings.prefix_for_stored_virtual_keys == "dheera_ai/custom/"
    assert settings.access_mode == "read_and_write"
    assert settings.primary_secret_name == "primary/dheera_ai/keys"
...
async def test_async_write_secret_receives_description_and_tags(monkeypatch):
    ...
    """
    from dheera_ai import dheera_ai
    from dheera_ai.secret_managers.aws_secret_manager_v2 import AWSSecretsManagerV2
    from dheera_ai.types.secret_managers.main import KeyManagementSettings

    # Mock out AWS network calls
    mock_async_write = AsyncMock(return_value={"Name": "dheera_ai/test_secret"})
    monkeypatch.setattr(AWSSecretsManagerV2, "async_write_secret", mock_async_write)

    # Setup settings
    dheera_ai._key_management_settings = KeyManagementSettings(
        store_virtual_keys=True,
        description="DheeraAI Unit Test Secret",
        tags={"Owner": "UnitTest", "Purpose": "Validation"},
    )

    # Instantiate fake client
    dheera_ai.secret_manager_client = AWSSecretsManagerV2()
    ...
    assert kwargs["description"] == "DheeraAI Unit Test Secret"
...
def test_key_management_settings_serialization_roundtrip():
    ...
    original = KeyManagementSettings(
        store_virtual_keys=True,
        prefix_for_stored_virtual_keys="dheera_ai/dev/",
        access_mode="read_and_write",
    ...
    assert reloaded.prefix_for_stored_virtual_keys == "dheera_ai/dev/"
