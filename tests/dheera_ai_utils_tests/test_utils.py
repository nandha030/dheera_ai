...
import dheera_ai
from dheera_ai.llms.custom_httpx.http_handler import AsyncHTTPHandler, headers
from dheera_ai.dheera_ai_core_utils.duration_parser import duration_in_seconds
from dheera_ai.dheera_ai_core_utils.duration_parser import (
...
def reset_mock_cache():
    from dheera_ai.utils import _model_cache
...
def test_basic_trimming_no_max_tokens_specified():
    ...
    ) <= dheera_ai.model_cost["gpt-4"]["max_tokens"]
...
def test_trimming_with_system_message_within_max_tokens():
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": "This is a medium normal message, let's say dheera_ai is awesome.",
        },
    ]
    trimmed_messages = trim_messages(
...
def test_trimming_with_model_cost_max_input_tokens(model):
    ...
    assert (
        ...
        < dheera_ai.model_cost[model]["max_input_tokens"]
    )
...
def test_trimming_with_untokenizable_field(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(level=logging.ERROR, logger="DheeraAI"):
...
def test_aget_valid_models():
    ...
    expected_models = (
        dheera_ai.open_ai_chat_completion_models | dheera_ai.open_ai_text_completion_models
    )
    ...
    # GEMINI
    expected_models = dheera_ai.gemini_models
    old_environ = os.environ
...
def test_supports_function_calling(model, expected_bool):
    try:
        assert dheera_ai.supports_function_calling(model=model) == expected_bool
...
def test_supports_web_search(model, expected_bool):
    try:
        assert dheera_ai.supports_web_search(model=model) == expected_bool
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_supports_reasoning(model, expected_bool):
    ...
    try:
        assert dheera_ai.supports_reasoning(model=model) == expected_bool
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_get_chat_completion_prompt():
    ...
    assert dheera_ai_logging_obj.messages == [
...
def test_redact_msgs_from_logs():
    ...
    _redacted_response_obj = redact_message_input_output_from_logging(
        result=response_obj,
        model_call_details=dheera_ai_logging_obj.model_call_details,
    )
...
def test_redact_embedding_response():
    ...
    original_usage = dheera_ai.Usage(
    ...
    response_obj = dheera_ai.EmbeddingResponse(
    ...
    assert isinstance(_redacted_response_obj, dheera_ai.EmbeddingResponse)
...
def test_redact_msgs_from_logs_with_dynamic_params():
    ...
    In all tests dheera_ai.turn_off_message_logging is True
    ...
    3. standard_callback_dynamic_params.turn_off_message_logging not set, dheera_ai.turn_off_message_logging is True: Redaction should occur.
    """
    from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import Logging
    from dheera_ai.dheera_ai_core_utils.redact_messages import (
        redact_message_input_output_from_logging,
    ...
    dheera_ai_logging_obj.model_call_details["standard_callback_dynamic_params"] = (
    ...
    assert _redacted_response_obj.choices[0].message.content == "redacted-by-dheera_ai"

    # Test Case 3: standard_callback_dynamic_params does not override dheera_ai.turn_off_message_logging
    # since dheera_ai.turn_off_message_logging is True redaction should occur
    standard_callback_dynamic_params = StandardCallbackDynamicParams()
    dheera_ai_logging_obj.model_call_details["standard_callback_dynamic_params"] = (
        standard_callback_dynamic_params
    )
...
def test_convert_model_response_object():
        ...
        "model_response_object": dheera_ai.ModelResponse(
            ...
            choices=[
                dheera_ai.Choices(
                    finish_reason="stop",
                    index=0,
                    message=dheera_ai.Message(content="default", role="assistant"),
                )
            ],
            created=1719376241,
    ...
    try:
        dheera_ai.convert_to_model_response_object(**args)
...
def test_parse_content_for_reasoning(content, expected_reasoning, expected_content):
    assert dheera_ai.utils._parse_content_for_reasoning(content) == (
        expected_reasoning,
        expected_content,
    )
...
def test_usage_object_null_tokens():
    ...
    Fixes https://github.com/BerriAI/dheera_ai/issues/5096
    """
    usage_obj = dheera_ai.Usage(prompt_tokens=2, completion_tokens=None, total_tokens=2)
...
def test_is_base64_encoded():
    ...
    from dheera_ai.utils import is_base64_encoded
...
def test_async_http_handler_force_ipv4(mock_async_client):
    """
    Test AsyncHTTPHandler when dheera_ai.force_ipv4 is True

    This is prod test - we need to ensure that httpx always uses ipv4 when dheera_ai.force_ipv4 is True
    """
    import httpx
    import ssl
    ...
    dheera_ai.force_ipv4 = True
    dheera_ai.disable_aiohttp_transport = True
...
def test_supports_audio_input(model, expected_bool):
    ...
    from dheera_ai.utils import supports_audio_input, supports_audio_output
...
def test_validate_chat_completion_user_messages(messages, expected_bool):
    from dheera_ai.utils import validate_chat_completion_user_messages
...
def test_validate_chat_completion_tool_choice(tool_choice, expected_bool):
    from dheera_ai.utils import validate_chat_completion_tool_choice
...
def test_models_by_provider():
    ...
    from dheera_ai import models_by_provider

    providers = set()
    for k, v in dheera_ai.model_cost.items():
        if "_" in v["dheera_ai_provider"] and "-" in v["dheera_ai_provider"]:
            continue
        elif k == "sample_spec":
            continue
        elif (
            v["dheera_ai_provider"] == "sagemaker"
            or v["dheera_ai_provider"] == "bedrock_converse"
        ):
            continue
            ...
            providers.add(v["dheera_ai_provider"])
...
@pytest.mark.parametrize(
    "dheera_ai_params, disable_end_user_cost_tracking, expected_end_user_id",
    [
        ({}, False, None),
...
def test_get_end_user_id_for_cost_tracking(
    ...
    dheera_ai.disable_end_user_cost_tracking = disable_end_user_cost_tracking
    assert (
        get_end_user_id_for_cost_tracking(dheera_ai_params=dheera_ai_params)
        == expected_end_user_id
    )


@pytest.mark.parametrize(
    "dheera_ai_params, enable_end_user_cost_tracking_prometheus_only, expected_end_user_id",
    [
        ({}, True, None),
...
def test_get_end_user_id_for_cost_tracking_prometheus_only(
    ...
    assert (
        get_end_user_id_for_cost_tracking(
            dheera_ai_params=dheera_ai_params, service_type="prometheus"
        )
        == expected_end_user_id
    )
...
@pytest.mark.parametrize(
    "dheera_ai_params, expected_end_user_id",
    [
        # Test with only metadata field (old behavior)
        ({"metadata": {"user_api_key_end_user_id": "user_from_metadata"}}, "user_from_metadata"),
        # Test with only dheera_ai_metadata field (new behavior)
        ({"dheera_ai_metadata": {"user_api_key_end_user_id": "user_from_dheera_ai_metadata"}}, "user_from_dheera_ai_metadata"),
        ...
        ({"metadata": {"user_api_key_end_user_id": "user_from_metadata"}, 
          "dheera_ai_metadata": {"user_api_key_end_user_id": "user_from_dheera_ai_metadata"}}, 
         "user_from_metadata"),
        # Test with user_api_key_end_user_id in dheera_ai_params (should take precedence over metadata)
        ({"user_api_key_end_user_id": "user_from_params", 
          "metadata": {"user_api_key_end_user_id": "user_from_metadata"}}, 
         "user_from_params"),
        # Test with empty metadata but valid dheera_ai_metadata
        ({"metadata": {}, "dheera_ai_metadata": {"user_api_key_end_user_id": "user_from_dheera_ai_metadata"}}, 
         "user_from_dheera_ai_metadata"),
...
def test_get_end_user_id_for_cost_tracking_metadata_handling(
    ...
    """
    Test that get_end_user_id_for_cost_tracking correctly handles both metadata and dheera_ai_metadata
    fields using the get_dheera_ai_metadata_from_kwargs helper function.
    """
    from dheera_ai.utils import get_end_user_id_for_cost_tracking
    
    # Ensure cost tracking is enabled for this test
    dheera_ai.disable_end_user_cost_tracking = False
    
    result = get_end_user_id_for_cost_tracking(dheera_ai_params=dheera_ai_params)
...
def test_is_prompt_caching_enabled_return_default_image_dimensions():
    ...
    """
    with patch("dheera_ai.utils.token_counter") as mock_token_counter:
        dheera_ai.utils.is_prompt_caching_valid_prompt(
            messages=[
                {
                    "role": "user",
                    "content": [
...
def test_token_counter_with_image_url_with_detail_high():
    ...
    """
    from dheera_ai.constants import DEFAULT_IMAGE_TOKEN_COUNT
    from dheera_ai._logging import verbose_logger
    import logging

    verbose_logger.setLevel(logging.DEBUG)

    _tokens = dheera_ai.utils.token_counter(
        messages=[
            {
                "role": "user",
                "content": [
...
def test_fireworks_ai_document_inlining():
    ...
    - supports_vision
    """
    from dheera_ai.utils import supports_pdf_input, supports_vision
...
def test_logprobs_type():
    from dheera_ai.types.utils import Logprobs
...
def test_get_valid_models_openai_proxy(monkeypatch):
    from dheera_ai.utils import get_valid_models
    import dheera_ai

    dheera_ai._turn_on_debug()

    monkeypatch.setenv("DHEERA_AI_PROXY_API_KEY", "sk-1234")
    monkeypatch.setenv("DHEERA_AI_PROXY_API_BASE", "https://dheera_ai-api.up.railway.app/")
    ...
    with patch.object(
        ...
        assert "dheera_ai_proxy/gpt-4o" in valid_models
...
def test_get_valid_models_fireworks_ai(monkeypatch):
    ...
    monkeypatch.setattr(dheera_ai, "provider_list", ["fireworks_ai"])
...
def test_get_num_retries(num_retries):
    from dheera_ai.utils import _get_wrapper_num_retries
...
def test_add_custom_logger_callback_to_specific_event(monkeypatch):
    ...
    monkeypatch.setattr(dheera_ai, "success_callback", [])
    monkeypatch.setattr(dheera_ai, "failure_callback", [])
...
def test_add_custom_logger_callback_to_specific_event_e2e(monkeypatch):
    ...
    monkeypatch.setattr(dheera_ai, "success_callback", [])
    monkeypatch.setattr(dheera_ai, "failure_callback", [])
    monkeypatch.setattr(dheera_ai, "callbacks", [])

    dheera_ai.success_callback = ["humanloop"]

    curr_len_success_callback = len(dheera_ai.success_callback)
    curr_len_failure_callback = len(dheera_ai.failure_callback)
    ...
    assert len(dheera_ai.success_callback) == curr_len_success_callback
    assert len(dheera_ai.failure_callback) == curr_len_failure_callback
...
def test_custom_logger_exists_in_callbacks_individual_functions(monkeypatch):
    ...
    for list_name in [
        ...
        monkeypatch.setattr(dheera_ai, list_name, [])
    ...
    dheera_ai.success_callback.append(mock_logger)
    ...
    dheera_ai._async_success_callback.append(mock_logger)
    assert _custom_logger_class_exists_in_success_callbacks(mock_logger) == True
    assert _custom_logger_class_exists_in_failure_callbacks(mock_logger) == False
    ...
    dheera_ai.failure_callback.append(mock_logger)
    assert _custom_logger_class_exists_in_success_callbacks(mock_logger) == False
    ...
    dheera_ai._async_failure_callback.append(mock_logger)
    assert _custom_logger_class_exists_in_success_callbacks(mock_logger) == False
    assert _custom_logger_class_exists_in_failure_callbacks(mock_logger) == True
...
async def test_add_custom_logger_callback_to_specific_event_with_duplicates(
    ...
    # Reset all callback lists
    monkeypatch.setattr(dheera_ai, "callbacks", [])
    monkeypatch.setattr(dheera_ai, "_async_success_callback", [])
    monkeypatch.setattr(dheera_ai, "_async_failure_callback", [])
    monkeypatch.setattr(dheera_ai, "success_callback", [])
    monkeypatch.setattr(dheera_ai, "failure_callback", [])

    # Add logger to both success_callback and _async_success_callback
    langfuse_logger = LangfusePromptManagement()
    dheera_ai.success_callback.append(langfuse_logger)
    dheera_ai._async_success_callback.append(langfuse_logger)

    # Get initial lengths
    initial_success_callback_len = len(dheera_ai.success_callback)
    initial_async_success_callback_len = len(dheera_ai._async_success_callback)
    ...
    assert len(dheera_ai.success_callback) == initial_success_callback_len
    assert len(dheera_ai._async_success_callback) == initial_async_success_callback_len
...
async def test_add_custom_logger_callback_to_specific_event_with_duplicates_callbacks(
    ...
    # Add logger to both success_callback and _async_success_callback
    langfuse_logger = LangfusePromptManagement()
    dheera_ai.callbacks.append(langfuse_logger)
    ...
    # Assert no new callbacks were added
    initial_callbacks_len = len(dheera_ai.callbacks)
    initial_async_success_callback_len = len(dheera_ai._async_success_callback)
    initial_success_callback_len = len(dheera_ai.success_callback)
    print(
        f"Num callbacks before: dheera_ai.callbacks: {len(dheera_ai.callbacks)}, dheera_ai._async_success_callback: {len(dheera_ai._async_success_callback)}, dheera_ai.success_callback: {len(dheera_ai.success_callback)}"
    )
    ...
    assert len(dheera_ai.callbacks) == initial_callbacks_len
    assert len(dheera_ai._async_success_callback) == initial_async_success_callback_len
    assert len(dheera_ai.success_callback) == initial_success_callback_len

    print(
        f"Num callbacks after 10 mock calls: dheera_ai.callbacks: {len(dheera_ai.callbacks)}, dheera_ai._async_success_callback: {len(dheera_ai._async_success_callback)}, dheera_ai.success_callback: {len(dheera_ai.success_callback)}"
    )
...
def test_add_custom_logger_callback_to_specific_event_e2e_failure(monkeypatch):
    ...
    dheera_ai.failure_callback = ["openmeter"]
    ...
    assert any(
        isinstance(callback, OpenMeterLogger) for callback in dheera_ai.failure_callback
    )


@pytest.mark.asyncio
async def test_wrapper_kwargs_passthrough():
    from dheera_ai.utils import client
    from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import (
        Logging as DheeraAILoggingObject,
    )
    ...
    # get dheera_ai logging object
    dheera_ai_logging_obj: DheeraAILoggingObject = mock_original.call_args.kwargs.get(
        "dheera_ai_logging_obj"
    )
    assert dheera_ai_logging_obj is not None

    print(
        f"dheera_ai_logging_obj.model_call_details: {dheera_ai_logging_obj.model_call_details}"
    )

    # get base model
    assert (
        dheera_ai_logging_obj.model_call_details["dheera_ai_params"]["base_model"]
        == "gpt-4o-mini"
    )


def test_dict_to_response_format_helper():
    from dheera_ai.llms.base_llm.base_utils import _dict_to_response_format_helper
...
from dheera_ai.utils import get_applied_guardrails
...
def test_get_applied_guardrails(test_case):
    ...
    # Setup
    dheera_ai.callbacks = test_case["callbacks"]
...
def test_should_use_cohere_v1_client(endpoint, params, expected_bool):
    assert dheera_ai.utils.should_use_cohere_v1_client(endpoint, params) == expected_bool
...
def test_add_openai_metadata():
    ...
    metadata = {
        ...
        "dheera_ai_parent_otel_span": MagicMock(),
        "none-val": None,
        "int-val": 1,
        "dict-val": {"a": 1, "b": 2},
    }
...
def test_message_object():
    from dheera_ai.types.utils import Message
...
def test_get_valid_models_from_provider_cache_invalidation(monkeypatch):
    ...
    _model_cache.set_cached_model_info(
        "openai", dheera_ai_params=None, available_models=["gpt-4o-mini"]
    )
...
def test_get_valid_models_from_dynamic_api_key():
    ...
    """
    from dheera_ai.utils import get_valid_models
    from dheera_ai.types.router import CredentialDheeraAIParams

    creds = CredentialDheeraAIParams(api_key="123")

    valid_models = get_valid_models(
        custom_llm_provider="anthropic",
        dheera_ai_params=creds,
        check_provider_endpoint=True,
    )
    assert len(valid_models) == 0

    creds = CredentialDheeraAIParams(api_key=os.getenv("ANTHROPIC_API_KEY"))
    valid_models = get_valid_models(
        custom_llm_provider="anthropic",
        dheera_ai_params=creds,
        check_provider_endpoint=True,
...
def test_get_whitelisted_models():
    ...
    Create whitelist to prevent naming regressions for older dheera_ai versions.
    """
    whitelisted_models = []
    for model, info in dheera_ai.model_cost.items():
        if info.get("dheera_ai_provider") == "bedrock" and info.get("mode") == "chat":
...
def test_delta_tool_calls_sequential_indices():
    ...
    # Create Delta object as DheeraAI would when processing streaming response
    delta = Delta(
        content=None,
...
def test_completion_with_no_model():
    ...
    with pytest.raises(TypeError):
        response = dheera_ai.completion(messages=[{"role": "user", "content": "Hello, how are you?"}])


def test_get_base_model_from_metadata():
    """
    Test _get_base_model_from_metadata function with both metadata and dheera_ai_metadata.
    This ensures cost tracking works for both Chat Completions API and Responses API.

    Related issue: https://github.com/BerriAI/dheera_ai/issues/16772
    """
    from dheera_ai.utils import _get_base_model_from_metadata
    ...
    # Test 2: base_model in dheera_ai_metadata (Responses API and generic API calls pattern)
    model_call_details_with_dheera_ai_metadata = {
        "dheera_ai_params": {
            "dheera_ai_metadata": {
                "model_info": {
    ...
    result = _get_base_model_from_metadata(model_call_details_with_dheera_ai_metadata)
    assert result == "azure/gpt-5-mini", f"Expected 'azure/gpt-5-mini', got {result}"

    # Test 3: base_model in dheera_ai_params (direct base_model)
    ...
    # Test 4: metadata takes precedence over dheera_ai_metadata
    model_call_details_with_both = {
        "dheera_ai_params": {
            ...
            "dheera_ai_metadata": {
                "model_info": {
                    "base_model": "azure/gpt-4-from-dheera_ai-metadata"
                }
            }
        }
    }
