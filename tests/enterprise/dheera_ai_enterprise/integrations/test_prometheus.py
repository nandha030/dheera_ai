...
def test_initialize_budget_metrics_cron_job():
    ...
    dheera_ai.callbacks = [prometheus_logger]
...
def test_end_user_not_tracked_for_all_prometheus_metrics():
    ...
    original_setting = getattr(
        dheera_ai, "enable_end_user_cost_tracking_prometheus_only", None
    )
    dheera_ai.enable_end_user_cost_tracking_prometheus_only = None  # Default behavior
    ...
    try:
        ...
        dheera_ai.enable_end_user_cost_tracking_prometheus_only = True
        ...
        dheera_ai.enable_end_user_cost_tracking_prometheus_only = original_setting
...
def test_prometheus_config_parsing():
    ...
    # Set configuration
    dheera_ai.prometheus_metrics_config = test_config
...
def test_get_metric_labels():
    ...
    test_config = [
        {
            "group": "service_metrics",
            "metrics": ["dheera_ai_deployment_failure_responses"],
            "include_labels": ["dheera_ai_model_name", "api_provider"],
        }
    ]
    ...
    # Get filtered labels
    labels = logger.get_labels_for_metric("dheera_ai_deployment_failure_responses")

    # Verify only configured labels are returned
    assert "dheera_ai_model_name" in labels
    assert "api_provider" in labels
    # These should be filtered out even if they're in the default labels
    assert (
        len([l for l in labels if l not in ["dheera_ai_model_name", "api_provider"]]) == 0
    )
...
def test_no_prometheus_config():
    ...
    dheera_ai.prometheus_metrics_config = None
...
def test_prometheus_metrics_config_type():
    ...
    valid_config = PrometheusMetricsConfig(
        group="service_metrics",
        metrics=["dheera_ai_deployment_failure_responses"],
        include_labels=["dheera_ai_model_name"],
    )

    assert valid_config.group == "service_metrics"
    assert valid_config.metrics == ["dheera_ai_deployment_failure_responses"]
    assert valid_config.include_labels == ["dheera_ai_model_name"]
...
def test_basic_functionality():
    ...
    assert dheera_ai.prometheus_metrics_config is not None
    assert len(dheera_ai.prometheus_metrics_config) == 1
    assert dheera_ai.prometheus_metrics_config[0]["group"] == "service_metrics"
    assert (
        "dheera_ai_deployment_failure_responses"
        in dheera_ai.prometheus_metrics_config[0]["metrics"]
    )
    ...
    test_config = [
        {
            ...
            "metrics": [
                ...
                "dheera_ai_deployment_total_requests",  # valid metric
            ],
            "include_labels": ["dheera_ai_model_name"],
        }
    ]
...
def test_invalid_labels_validation():
    ...
    test_config = [
        {
            "group": "service_metrics",
            "metrics": ["dheera_ai_deployment_total_requests"],
            "include_labels": [
                "dheera_ai_model_name",  # valid label
...
def test_valid_configuration_passes_validation():
    ...
    try:
        ...
        assert "dheera_ai_deployment_total_requests" in logger.enabled_metrics
        assert "dheera_ai_deployment_failure_responses" in logger.enabled_metrics
...
def mock_prometheus_logger():
    ...
    with patch("dheera_ai.proxy.proxy_server.premium_user", True):
        ...
        logger.dheera_ai_proxy_total_requests_metric = MockCounter(
            "dheera_ai_proxy_total_requests_metric"
        )
        logger.dheera_ai_tokens_metric = MockCounter("dheera_ai_total_tokens")
        logger.dheera_ai_input_tokens_metric = MockCounter("dheera_ai_input_tokens")
        logger.dheera_ai_output_tokens_metric = MockCounter("dheera_ai_output_tokens")
        logger.dheera_ai_spend_metric = MockCounter("dheera_ai_spend_metric")
        logger.dheera_ai_requests_metric = MockCounter("dheera_ai_requests_metric")
...
async def test_request_counter_semantic_validation(mock_prometheus_logger):
    ...
    This test specifically catches the bug where dheera_ai_proxy_total_requests_metric
    ...
    total_requests_metric = mock_prometheus_logger.dheera_ai_proxy_total_requests_metric
    assert (
    ...
    tokens_metric = mock_prometheus_logger.dheera_ai_tokens_metric
    assert (
...
async def test_multiple_requests_counter_semantics(mock_prometheus_logger):
    ...
    total_request_increments = sum(
        mock_prometheus_logger.dheera_ai_proxy_total_requests_metric.inc_calls
    )
    total_token_increments = sum(mock_prometheus_logger.dheera_ai_tokens_metric.inc_calls)
...
async def test_streaming_request_counter_semantics(mock_prometheus_logger):
    ...
    for (
        inc_value
    ) in mock_prometheus_logger.dheera_ai_proxy_total_requests_metric.inc_calls:
        assert (
            inc_value == 1
...
async def test_spend_counter_semantics(mock_prometheus_logger):
    ...
    spend_metric = mock_prometheus_logger.dheera_ai_spend_metric
...
def test_callback_failure_metric_increments(prometheus_logger):
    ...
    This tests the dheera_ai_callback_logging_failures_metric counter.
    """
    # Get initial value
    initial_value = 0
    try:
        initial_value = (
            prometheus_logger.dheera_ai_callback_logging_failures_metric.labels(
    ...
    current_value = prometheus_logger.dheera_ai_callback_logging_failures_metric.labels(
        callback_name="S3Logger"
    )._value.get()
    ...
    langfuse_value = prometheus_logger.dheera_ai_callback_logging_failures_metric.labels(
    ...
    s3_value = prometheus_logger.dheera_ai_callback_logging_failures_metric.labels(
        callback_name="S3Logger"
    )._value.get()
...
def test_callback_failure_metric_different_callbacks(prometheus_logger):
    ...
    for callback_name in callbacks_to_test:
        ...
        initial = 0
        try:
            initial = prometheus_logger.dheera_ai_callback_logging_failures_metric.labels(
        ...
        # Verify incremented
        current = prometheus_logger.dheera_ai_callback_logging_failures_metric.labels(
            callback_name=callback_name
        )._value.get()
