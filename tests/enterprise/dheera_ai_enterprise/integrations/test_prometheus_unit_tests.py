...
from dheera_ai.types.utils import BudgetConfig, GenericBudgetConfigType
...
async def test_async_prometheus_success_logging_with_callbacks(prometheus_logger):
    ...
    diff = await op()
    await asyncio.sleep(2)
    assert diff["dheera_ai_requests_metric_total"] == 1.0
...
async def test_async_prometheus_budget_logging_with_callbacks(prometheus_logger):
    ...
    # TODO: Should implement `dheera_ai_provider_remaining_budget_metric` in prometheus.py
    assert diff.get("dheera_ai_provider_remaining_budget_metric", 50.0) == 50.0
...
async def test_prometheus_metric_tracking():
    ...
    dheera_ai._async_success_callback = [mock_prometheus]
