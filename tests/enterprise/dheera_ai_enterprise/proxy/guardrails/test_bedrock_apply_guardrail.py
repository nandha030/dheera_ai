...
from dheera_ai.types.guardrails import ApplyGuardrailRequest, ApplyGuardrailResponse
...
async def test_bedrock_apply_guardrail_endpoint_integration():
    ...
    from dheera_ai.proxy.guardrails.guardrail_endpoints import apply_guardrail
    ...
    with patch(
        "dheera_ai.proxy.guardrails.guardrail_endpoints.GUARDRAIL_REGISTRY"
    ) as mock_registry:
        # Mock the make_bedrock_api_request method
        with patch.object(
            guardrail, "make_bedrock_api_request", new_callable=AsyncMock
