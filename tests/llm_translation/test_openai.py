...
async def test_openai_prediction_param_with_caching():
    ...
    dheera_ai.set_verbose = True
    dheera_ai.cache = dheera_ai.Cache(type=DheeraAICacheType.LOCAL)
    code = """
    /// <summary>
    /// Represents a user with a first name, last name, and username.
    /// </summary>
    ...
    completion_response_1 = dheera_ai.completion(
        model="gpt-4o-mini",
        messages=[
            {
                "role": "user",
    ...
    # cache hit
    completion_response_2 = dheera_ai.completion(
        model="gpt-4o-mini",
        messages=[
            {
                "role": "user",
    ...
    completion_response_3 = dheera_ai.completion(
        model="gpt-4o-mini",
        messages=[
...
@patch("dheera_ai.main.openai_chat_completions._get_openai_client")
...
def test_openai_image_generation_forwards_organization(mock_get_openai_client):
    ...
    org = "org_test_123"
    resp = dheera_ai.image_generation(
        model="gpt-image-1",
...
def test_openai_chat_completion_streaming_handler_reasoning_content():
    from dheera_ai.llms.openai.chat.gpt_transformation import (
...
def validate_web_search_annotations(annotations: ChatCompletionAnnotation):
    """validates dheera_ai response contains web search annotations"""
...
def test_openai_web_search():
    ...
    print("dheera_ai response: ", response.model_dump_json(indent=4))
...
def test_openai_web_search_streaming():
    ...
    for chunk in response:
        print("dheera_ai response chunk: ", chunk)
        if (
...
class TestOpenAIGPT4OAudioTranscription(BaseLLMAudioTranscriptionTest):
    ...
    def get_custom_llm_provider(self) -> dheera_ai.LlmProviders:
        return dheera_ai.LlmProviders.OPENAI


@pytest.mark.asyncio
@pytest.mark.parametrize("model", ["gpt-4o"])
async def test_openai_pdf_url(model):
    from dheera_ai.utils import return_raw_request, CallTypes
...
async def test_openai_codex_stream(sync_mode):
    ...
    chunks = []
    if sync_mode:
        response = dheera_ai.completion(**kwargs)
        for chunk in response:
            chunks.append(chunk)
    else:
        response = await dheera_ai.acompletion(**kwargs)
...
def test_openai_tool_calling():
    ...
    response = dheera_ai.completion(**completion_params)
...
def test_openai_gpt_5_codex_reasoning():
    ...
    response = dheera_ai.completion(**completion_kwargs)
    print("response: ", response)
    for chunk in response:
        print("chunk: ", chunk)
...
# Regression test for: https://github.com/BerriAI/dheera_ai/issues/8977
@pytest.mark.parametrize("model", ["gpt-4o"])
@pytest.mark.asyncio
