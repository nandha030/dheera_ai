...
def test_get_llm_provider():
    _, response, _, _ = dheera_ai.get_llm_provider(model="anthropic.claude-v2:1")
...
def test_get_llm_provider_fireworks():  # tests finetuned fireworks models - https://github.com/BerriAI/dheera_ai/issues/4923
...
def test_get_llm_provider_catch_all():
    _, response, _, _ = dheera_ai.get_llm_provider(model="*")
    assert response == "openai"


def test_get_llm_provider_gpt_instruct():
    _, response, _, _ = dheera_ai.get_llm_provider(model="gpt-3.5-turbo-instruct-0914")

    assert response == "text-completion-openai"


def test_get_llm_provider_mistral_custom_api_base():
    model, custom_llm_provider, dynamic_api_key, api_base = dheera_ai.get_llm_provider(
...
def test_default_api_base():
    ...
    with patch.dict(os.environ, {}, clear=True):
        for provider in dheera_ai.openai_compatible_providers:
            ...
            for other_provider in dheera_ai.provider_list:
...
# -------- Tests for force_use_dheera_ai_proxy ---------

def test_get_dheera_ai_proxy_custom_llm_provider():
    """
    Tests force_use_dheera_ai_proxy uses DHEERA_AI_PROXY_API_BASE and DHEERA_AI_PROXY_API_KEY from env.
    """
    ...
    with patch.dict(os.environ, {
        "DHEERA_AI_PROXY_API_BASE": expected_api_base,
        "DHEERA_AI_PROXY_API_KEY": expected_api_key
    }, clear=True):
        model, provider, key, base = dheera_ai.DheeraAIProxyChatConfig().dheera_ai_proxy_get_custom_llm_provider_info(model=test_model)

    assert model == test_model
    assert provider == "dheera_ai_proxy"
    assert key == expected_api_key
    assert base == expected_api_base

def test_get_dheera_ai_proxy_with_args_override_env_vars():
    """
    Tests force_use_dheera_ai_proxy uses api_base and api_key args over environment variables.
    """
    test_model = "gpt-4"
    ...
    with patch.dict(os.environ, {
        "DHEERA_AI_PROXY_API_BASE": env_api_base,
        "DHEERA_AI_PROXY_API_KEY": env_api_key
    }, clear=True):
        model, provider, key, base = dheera_ai.DheeraAIProxyChatConfig().dheera_ai_proxy_get_custom_llm_provider_info(
            model=test_model,
...
def test_get_dheera_ai_proxy_model_prefix_stripping():
    """
    Tests force_use_dheera_ai_proxy strips 'dheera_ai_proxy/' prefix from model name.
    """
    original_model = "dheera_ai_proxy/claude-2"
    ...
    with patch.dict(os.environ, {
        ...
        model, provider, key, base = dheera_ai.DheeraAIProxyChatConfig().dheera_ai_proxy_get_custom_llm_provider_info(model=original_model)
...
# -------- Tests for get_llm_provider triggering use_dheera_ai_proxy ---------

def test_get_llm_provider_DHEERA_AI_PROXY_ALWAYS_true():
    """
    Tests get_llm_provider uses dheera_ai_proxy when USE_DHEERA_AI_PROXY is "True".
    """
    ...
    with patch.dict(os.environ, {
        ...
        "DHEERA_AI_PROXY_API_BASE": proxy_api_base,
        "DHEERA_AI_PROXY_API_KEY": proxy_api_key
    }, clear=True):
        model, provider, key, base = dheera_ai.get_llm_provider(model=test_model_input)
...
def test_get_llm_provider_DHEERA_AI_PROXY_ALWAYS_true_model_prefix():
    """
    Tests get_llm_provider with USE_DHEERA_AI_PROXY="True" and model prefix "dheera_ai_proxy/".
    """
    test_model_input = "dheera_ai_proxy/gpt-4-turbo"
...
def test_get_llm_provider_use_proxy_arg_true():
    """
    Tests get_llm_provider uses dheera_ai_proxy when use_proxy=True argument is passed.
    """
    test_model_input = "mistral/mistral-large"
    expected_model_output = "mistral/mistral-large" # force_use_dheera_ai_proxy keep the model name
    proxy_api_base = "http://my-arg-proxy.com"
    proxy_api_key = "arg_proxy_key"
    
    # Ensure DHEERA_AI_PROXY_ALWAYS is not set or False
    with patch.dict(os.environ, {
        "DHEERA_AI_PROXY_API_BASE": proxy_api_base,
        "DHEERA_AI_PROXY_API_KEY": proxy_api_key
    }, clear=True): # clear=True removes DHEERA_AI_PROXY_ALWAYS if it was set by other tests
        model, provider, key, base = dheera_ai.get_llm_provider(
            model=test_model_input, 
            dheera_ai_params=DheeraAI_Params(use_dheera_ai_proxy=True, model=test_model_input)
        )
...
def test_get_llm_provider_use_proxy_arg_true_with_direct_args():
    ...
    These args should be passed to force_use_dheera_ai_proxy and override env vars.
    """
    ...
    with patch.dict(os.environ, {
        "DHEERA_AI_PROXY_API_BASE": env_proxy_api_base,
        "DHEERA_AI_PROXY_API_KEY": env_proxy_api_key
    }, clear=True):
        model, provider, key, base = dheera_ai.get_llm_provider(
            model=test_model_input, 
            api_base=arg_api_base,
