...
The search_tool_name parameter is used internally by DheeraAI to identify
...
def test_search_tool_name_in_all_dheera_ai_params():
    """
    Test that search_tool_name is in all_dheera_ai_params.
    
    If missing, it gets passed to provider APIs causing errors.
    """
    assert "search_tool_name" in all_dheera_ai_params


def test_filter_out_search_tool_name():
    """
    Test that filter_out_dheera_ai_params correctly filters search_tool_name.
    """
    kwargs = {
    ...
    assert "dheera_ai_call_id" not in filtered
