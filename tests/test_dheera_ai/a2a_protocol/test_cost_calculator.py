...
async def test_asend_message_uses_cost_per_query():
    ...
    dheera_ai.callbacks = [cost_logger]

    # Mock A2A client
    mock_client = MagicMock()
    mock_client._dheera_ai_agent_card = MagicMock()
    mock_client._dheera_ai_agent_card.name = "test-agent"
...
async def test_asend_message_uses_input_output_cost_per_token():
    ...
    dheera_ai.callbacks = [token_cost_logger]
...
async def test_asend_message_passes_agent_id_to_callback():
    ...
    dheera_ai.callbacks = [agent_id_logger]
...
class MetadataLogger(CustomLogger):
    ...
    def __init__(self):
        ...
        self.dheera_ai_params: Optional[dict] = None
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        self.dheera_ai_params = kwargs.get("dheera_ai_params", {})
        self.metadata = self.dheera_ai_params.get("metadata", {})
...
async def test_asend_message_streaming_propagates_metadata():
    ...
    dheera_ai.logging_callback_manager.add_dheera_ai_async_success_callback(metadata_logger)
...
async def test_asend_message_streaming_triggers_callbacks():
    ...
    dheera_ai.logging_callback_manager.add_dheera_ai_async_success_callback(callback_logger)
    dheera_ai.logging_callback_manager.add_dheera_ai_success_callback(callback_logger)
