...
from enterprise.dheera_ai_enterprise.enterprise_callbacks.send_emails.base_email import (
    BaseEmailLogger,
)
...
from dheera_ai.integrations.email_templates.email_footer import EMAIL_FOOTER
from dheera_ai.proxy._types import Litellm_EntityType, WebhookEvent
...
async def test_send_key_created_email(
    ...
    with mock.patch.dict(
        os.environ,
        {
            "EMAIL_LOGO_URL": "https://dheera_ai-listing.s3.amazonaws.com/dheera_ai_logo.png",
        ...
        assert call_args["from_email"] == BaseEmailLogger.DEFAULT_DHEERA_AI_EMAIL
        assert call_args["to_email"] == ["test@example.com"]
        assert call_args["subject"] == "DheeraAI: Test Key Created"
...
async def test_send_user_invitation_email(
    ...
    with mock.patch.dict(
        ...
        assert call_args["subject"] == "DheeraAI: User Invitation"
...
async def test_send_key_rotated_email(
    ...
    with mock.patch.dict(
        ...
        assert call_args["from_email"] == BaseEmailLogger.DEFAULT_DHEERA_AI_EMAIL
        assert call_args["to_email"] == ["test@example.com"]
        assert call_args["subject"] == "DheeraAI: API Key Rotated"
...
async def test_get_invitation_link(base_email_logger):
    ...
    mock_prisma.db.dheera_ai_invitationlink.find_many = mock_find_many

    with mock.patch("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma):
        ...
        mock_prisma.db.dheera_ai_invitationlink.find_many = mock_find_many_empty
        result = await base_email_logger._get_invitation_link(
            user_id="test-user", base_url="http://test.com"
        )
        assert result == "http://test.com"
...
async def test_get_invitation_link_creates_new_when_none_exist(base_email_logger):
    ...
    with mock.patch("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma):
        with mock.patch(
            "dheera_ai.proxy.management_helpers.user_invitation.create_invitation_for_user",
...
async def test_get_invitation_link_uses_existing_when_available(base_email_logger):
    ...
    mock_prisma.db.dheera_ai_invitationlink.find_many = mock_find_many_existing
...
async def test_get_invitation_link_creates_new_when_list_is_none(base_email_logger):
    ...
    mock_prisma.db.dheera_ai_invitationlink.find_many = mock_find_many_none
...
async def test_get_email_params_user_invitation(
    ...
    with mock.patch.dict(
        ...
        with mock.patch.object(
            ...
            assert result.logo_url == "https://dheera_ai-listing.s3.amazonaws.com/dheera_ai_logo.png"
...
async def test_get_email_params_non_premium_user(mock_env_vars):
    ...
    with patch("dheera_ai.proxy.proxy_server.premium_user", False):
        email_logger = BaseEmailLogger()
        ...
        assert email_params.subject == "DheeraAI: New User Invitation"
        assert email_params.signature == EMAIL_FOOTER
        assert email_params.logo_url == "https://dheera_ai-listing.s3.amazonaws.com/dheera_ai_logo.png"
        ...
        assert key_params.subject == "DheeraAI: API Key Created"
...
async def test_get_email_params_default_templates(monkeypatch):
    ...
    with patch("dheera_ai.proxy.proxy_server.premium_user", True):
        ...
        assert invitation_params.subject == "DheeraAI: New User Invitation"
