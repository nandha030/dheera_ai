...
def test_adapter_import():
    ...
    from dheera_ai.google_genai.adapters.transformation import GoogleGenAIAdapter
...
def test_completion_to_generate_content_with_tool_calls():
    ...
    from dheera_ai.types.utils import Choices, ModelResponse, Usage
...
def test_streaming_tool_calls_transformation():
    ...
    from dheera_ai.google_genai.adapters.transformation import (
        GoogleGenAIAdapter,
...
def test_streaming_partial_tool_calls_accumulation():
    ...
    assert function_call["args"]["path"] == "/Users/ishaanjaffer/Github/dheera_ai/README.md", f"Expected complete path, got {function_call['args']}"
...
def test_completion_to_generate_content_transformation():
    ...
    from dheera_ai.google_genai.adapters.transformation import GoogleGenAIAdapter
    from dheera_ai.types.llms.openai import ChatCompletionAssistantMessage
    from dheera_ai.types.utils import Choices, ModelResponse, Usage
...
def test_api_base_and_api_key_passthrough(function_name, is_async, is_stream):
    """Test that api_base and api_key parameters are passed through to dheera_ai.completion/acompletion when using generate_content"""
    import asyncio
    ...
    if function_name == "generate_content":
        from dheera_ai.google_genai.main import generate_content as test_function
    elif function_name == "agenerate_content":
        from dheera_ai.google_genai.main import agenerate_content as test_function
    elif function_name == "generate_content_stream":
        from dheera_ai.google_genai.main import generate_content_stream as test_function
    elif function_name == "agenerate_content_stream":
        from dheera_ai.google_genai.main import agenerate_content_stream as test_function
    ...
    # Mock the appropriate dheera_ai function (completion vs acompletion)
    mock_target = 'dheera_ai.acompletion' if is_async else 'dheera_ai.completion'
    ...
    with unittest.mock.patch(mock_target) as mock_completion:
        ...
        # Verify that the appropriate dheera_ai function was called
        mock_completion.assert_called_once()
        
        # Get the arguments passed to dheera_ai.completion/acompletion
...
def test_shared_schema_normalization_utilities():
    ...
    from dheera_ai.dheera_ai_core_utils.json_validation_rule import (
...
async def test_google_generate_content_with_openai():
    ...
    with unittest.mock.patch("dheera_ai.acompletion", new_callable=unittest.mock.AsyncMock) as mock_completion:
        ...
        # Print the request args sent to dheera_ai.completion
        call_args, call_kwargs = mock_completion.call_args
        print("Arguments sent to dheera_ai.completion:")
        ...
        ######################################################### 
        # validate only expected fields were sent to dheera_ai.completion
        passed_fields = set(call_kwargs.keys())
        # remove any GenericDheeraAIParams fields
        passed_fields = passed_fields - set(GenericDheeraAIParams.model_fields.keys())
