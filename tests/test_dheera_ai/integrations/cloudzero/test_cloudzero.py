...
class TestCloudZeroHourlyExport:
    ...
    async def test_hourly_export(self):
        ...
        with (
            patch.object(DheeraAIDatabase, "_ensure_prisma_client") as mock_prisma_client_getter,
            patch.object(CloudZeroStreamer, "send_batched") as send_batched_mock,
            patch("dheera_ai.integrations.cloudzero.cloudzero.datetime") as mock_datetime,
        ):
            ...
            async def query_raw_mock(query: str):
                sql_context = pl.SQLContext(
                    DheeraAI_DailyUserSpend=spend_mock_data,
                    DheeraAI_VerificationToken=verification_mock_data,
                    DheeraAI_TeamTable=team_mock_data,
                )
