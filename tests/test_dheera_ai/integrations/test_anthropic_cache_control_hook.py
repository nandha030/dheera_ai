...
async def test_anthropic_cache_control_hook_system_message():
    ...
    with patch.dict(
        ...
        dheera_ai.callbacks = [anthropic_cache_control_hook]
...
async def test_anthropic_cache_control_hook_out_of_bounds_logging():
    ...
    with patch.dict(
        ...
        with patch(
            "dheera_ai.integrations.anthropic_cache_control_hook.verbose_logger"
        ) as mock_logger:
            with patch.object(client, "post", return_value=mock_response) as mock_post:
                messages = [
...
async def test_anthropic_cache_control_hook_multiple_content_items_last_only():
    ...
    This verifies the fix for https://github.com/BerriAI/dheera_ai/issues/15696
    """
    with patch.dict(
        os.environ,
        {
