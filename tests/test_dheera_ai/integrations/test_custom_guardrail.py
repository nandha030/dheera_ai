...
from dheera_ai.proxy._types import CallTypes, UserAPIKeyAuth
...
class TestCustomGuardrailShouldRunGuardrail:
    ...
    def test_should_run_guardrail_with_dheera_ai_metadata(self):
        """Test that should_run_guardrail works with dheera_ai_metadata pattern"""
        ...
        # Test with guardrails in dheera_ai_metadata
        data = {
            "model": "gpt-3.5-turbo",
            "dheera_ai_metadata": {"guardrails": ["test_guardrail"]},
        }
    ...
    def test_should_run_guardrail_no_matching_guardrail(self):
        ...
        data = {
            "model": "gpt-3.5-turbo",
            "dheera_ai_metadata": {"guardrails": ["different_guardrail"]},
        }
    ...
    def test_should_run_guardrail_with_disable_global_guardrail(self):
        ...
        # Test 3: Global guardrail is disabled when disable_global_guardrail=True in dheera_ai_metadata
        data_with_disable_dheera_ai = {
            "model": "gpt-3.5-turbo",
            "messages": [{"role": "user", "content": "test"}],
            "dheera_ai_metadata": {"disable_global_guardrail": True},
        }
        result = custom_guardrail.should_run_guardrail(
            data=data_with_disable_dheera_ai, event_type=GuardrailEventHooks.pre_call
        )
        assert (
            result is False
        ), "Global guardrail should be disabled when disable_global_guardrail=True in dheera_ai_metadata"
...
class TestGuardrailLoggingAggregation:
    ...
    def test_appends_to_dheera_ai_metadata(self):
        request_data = {
            "dheera_ai_metadata": {
                "standard_logging_guardrail_information": [
                    {"guardrail_name": "dheera_ai_existing"}
                ]
            }
        }
        ...
        info = request_data["dheera_ai_metadata"][
            "standard_logging_guardrail_information"
        ]
        assert isinstance(info, list)
        assert len(info) == 2
