...
class TestOpenTelemetryGuardrails(unittest.TestCase):
    @patch("dheera_ai.integrations.opentelemetry.datetime")
...
class TestOpenTelemetryCostBreakdown(unittest.TestCase):
    def test_cost_breakdown_emitted_to_otel_span(self):
        ...
        kwargs = {
            ...
            "optional_params": {},
            "dheera_ai_params": {"custom_llm_provider": "openai"},
...
class TestOpenTelemetry(unittest.TestCase):
    ...
    def test_get_tracer_with_dynamic_headers(self, mock_resource, mock_tracer_provider):
        ...
        with patch.object(otel, "_get_span_processor") as mock_get_span_processor:
            ...
            mock_provider_instance.get_tracer.assert_called_once_with("dheera_ai")
    ...
    def test_get_dheera_ai_resource_with_defaults(
        self, mock_detector_cls, mock_resource_create
    ):
        """Test _get_dheera_ai_resource with default values when no environment variables are set."""
        from dheera_ai.integrations.opentelemetry import _get_dheera_ai_resource
        ...
        # Call the function
        result = _get_dheera_ai_resource()
    ...
    @patch("opentelemetry.sdk.resources.Resource.create")
    @patch("opentelemetry.sdk.resources.OTELResourceDetector")
    def test_get_dheera_ai_resource_with_dheera_ai_env_vars(
        self, mock_detector_cls, mock_resource_create
    ):
        """Test _get_dheera_ai_resource with DheeraAI-specific environment variables."""
        from dheera_ai.integrations.opentelemetry import _get_dheera_ai_resource
    ...
    @patch("opentelemetry.sdk.resources.Resource.create")
    @patch("opentelemetry.sdk.resources.OTELResourceDetector")
    def test_get_dheera_ai_resource_with_otel_resource_attributes(
        self, mock_detector_cls, mock_resource_create
    ):
        """Test _get_dheera_ai_resource with OTEL_RESOURCE_ATTRIBUTES environment variable."""
        from dheera_ai.integrations.opentelemetry import _get_dheera_ai_resource
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_get_dheera_ai_resource_integration_with_real_resource(self):
        """Integration test to verify _get_dheera_ai_resource works with actual OpenTelemetry Resource."""
        from dheera_ai.integrations.opentelemetry import _get_dheera_ai_resource
        ...
        self.assertEqual(attributes.get("service.name"), "dheera_ai")
        self.assertEqual(attributes.get("deployment.environment"), "production")
        self.assertEqual(attributes.get("model_id"), "dheera_ai")
    ...
    def test_get_dheera_ai_resource_real_otel_resource_attributes(self):
    ...
    @patch.dict(
        os.environ,
        {
            "OTEL_SERVICE_NAME": "dheera_ai-service",
            "OTEL_RESOURCE_ATTRIBUTES": "service.name=otel-override,extra.attr=extra-value",
        },
        clear=True,
    )
    def test_get_dheera_ai_resource_precedence(self):
        ...
        attributes = result.attributes
        self.assertEqual(attributes.get("service.name"), "dheera_ai-service")
    ...
    def test_handle_success_spans_only(self):
        ...
        os.environ.pop("DHEERA_AI_OTEL_INTEGRATION_ENABLE_EVENTS", None)
        os.environ.pop("DHEERA_AI_OTEL_INTEGRATION_ENABLE_METRICS", None)
        ...
        # must have the top‐level dheera_ai_request span
        # self.assertIn(
        #     DHEERA_AI_REQUEST_SPAN_NAME,
        #     [s.name for s in spans],
        #     "dheera_ai_request span missing",
        # )
    ...
    def test_handle_success_spans_and_metrics(self):
        # only metrics on
        os.environ.pop("DHEERA_AI_OTEL_INTEGRATION_ENABLE_EVENTS", None)
        os.environ["DHEERA_AI_OTEL_INTEGRATION_ENABLE_METRICS"] = "true"
    ...
    def test_get_span_name_with_generation_name(self):
        ...
        otel = OpenTelemetry()
        kwargs = {"dheera_ai_params": {"metadata": {"generation_name": "custom_span"}}}
    ...
    def test_get_span_name_without_generation_name(self):
        ...
        from dheera_ai.integrations.opentelemetry import DHEERA_AI_REQUEST_SPAN_NAME

        otel = OpenTelemetry()
        kwargs = {"dheera_ai_params": {"metadata": {}}}
        result = otel._get_span_name(kwargs)
        self.assertEqual(result, DHEERA_AI_REQUEST_SPAN_NAME)

    @patch("dheera_ai.turn_off_message_logging", False)
    def test_maybe_log_raw_request_creates_span(self):
        """Test _maybe_log_raw_request creates span when logging enabled"""
        from dheera_ai.integrations.opentelemetry import RAW_REQUEST_SPAN_NAME
    ...
    @patch("dheera_ai.turn_off_message_logging", True)
...
class TestOpenTelemetryEndpointNormalization(unittest.TestCase):
    ...
    def test_normalize_endpoint_invalid_signal_type(self):
        ...
        with patch("dheera_ai._logging.verbose_logger.warning") as mock_warning:
