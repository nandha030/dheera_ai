...
import dheera_ai
from dheera_ai.dheera_ai_core_utils.specialty_caches.dynamic_logging_cache import (
...
class TestLangfuseInMemoryCache:
    ...
    @patch("dheera_ai.initialized_langfuse_clients", 5)
    def test_langfuse_client_count_decrements_on_eviction(self):
        ...
        with patch(
            "dheera_ai.integrations.langfuse.langfuse.LangFuseLogger", MockLangFuseLogger
        ):
            ...
            initial_count = dheera_ai.initialized_langfuse_clients

            # Trigger eviction
            self.cache.evict_cache()

            # Verify client count was decremented
            assert dheera_ai.initialized_langfuse_clients == initial_count - 1

    @patch("dheera_ai.initialized_langfuse_clients", 3)
