...
def test_get_dheera_ai_metadata_from_kwargs():
    kwargs = {
        "dheera_ai_params": {
            "dheera_ai_metadata": {},
    ...
    assert get_dheera_ai_metadata_from_kwargs(kwargs) == {"user_api_key": "1234567890"}


def test_add_missing_spend_metadata_to_dheera_ai_metadata():
    dheera_ai_metadata = {"test_key": "test_value"}
    ...
    assert get_dheera_ai_metadata_from_kwargs(kwargs) == {
        "test_key": "test_value",
...
def test_safe_deep_copy_with_non_pickleables_and_span():
    ...
    data = {
        "metadata": {"dheera_ai_parent_otel_span": rlock, "x": 1},
    ...
    assert copied["metadata"]["dheera_ai_parent_otel_span"] == "placeholder"
    assert data["metadata"]["dheera_ai_parent_otel_span"] is rlock
