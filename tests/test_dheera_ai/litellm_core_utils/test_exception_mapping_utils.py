...
# As per https://github.com/BerriAI/dheera_ai/issues/16189
...
def test_vertex_ai_rate_limit_error_mapping(error_message, should_raise_rate_limit):
    ...
    "Resource exhausted" error to a dheera_ai.RateLimitError.
    """
    ...
    if should_raise_rate_limit:
        with pytest.raises(dheera_ai.RateLimitError) as excinfo:
            exception_type(
                model=model,
        ...
        assert isinstance(excinfo.value, dheera_ai.RateLimitError)
    else:
        # For the negative case, we expect it to raise a generic APIConnectionError
        with pytest.raises(dheera_ai.APIConnectionError):
            exception_type(
                model=model,
                original_exception=original_exception,
                custom_llm_provider=custom_llm_provider,
