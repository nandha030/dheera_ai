...
from dheera_ai.constants import SENTRY_DENYLIST, SENTRY_PII_DENYLIST
...
def test_use_custom_pricing_for_model():
    from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import use_custom_pricing_for_model

    dheera_ai_params = {
        "custom_llm_provider": "azure",
        "input_cost_per_pixel": 10,
    }
    assert use_custom_pricing_for_model(dheera_ai_params) == True
...
async def test_logging_result_for_bridge_calls(logging_obj):
    ...
    with patch.object(
        ...
        await dheera_ai.anthropic_messages(
            max_tokens=100,
            messages=[{"role": "user", "content": "Hey"}],
...
async def test_logging_non_streaming_request():
    ...
    original_callbacks = getattr(dheera_ai, "callbacks", [])

    try:
        mock_logging_obj = MockPrometheusLogger()

        dheera_ai.callbacks = [mock_logging_obj]
...
def test_get_user_agent_tags():
    from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import StandardLoggingPayloadSetup

    tags = StandardLoggingPayloadSetup._get_user_agent_tags(
        proxy_server_request={
            "headers": {
                "user-agent": "dheera_ai/0.1.0",
            }
        }
    )

    assert "User-Agent: dheera_ai" in tags
    assert "User-Agent: dheera_ai/0.1.0" in tags
...
def test_get_request_tags():
    ...
    tags = StandardLoggingPayloadSetup._get_request_tags(
        dheera_ai_params={"metadata": {"tags": ["test-tag"]}},
        proxy_server_request={
            "headers": {
                "user-agent": "dheera_ai/0.1.0",
            }
...
def test_get_request_tags_from_metadata_and_dheera_ai_metadata():
    """
    Test that _get_request_tags correctly picks tags from both 'metadata' and 'dheera_ai_metadata'.

    Scenarios tested:
    1. Tags in metadata only
    2. Tags in dheera_ai_metadata only
    3. Tags in both (metadata should take priority)
    4. No tags in either
    5. None values for metadata/dheera_ai_metadata
    """
    from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import StandardLoggingPayloadSetup

    # Test case 1: Tags in metadata only
    tags = StandardLoggingPayloadSetup._get_request_tags(
        dheera_ai_params={"metadata": {"tags": ["metadata-tag-1", "metadata-tag-2"]}},
        proxy_server_request={},
    )
    ...
    # Test case 2: Tags in dheera_ai_metadata only
    tags = StandardLoggingPayloadSetup._get_request_tags(
        dheera_ai_params={
            "dheera_ai_metadata": {
                "tags": ["dheera_ai-metadata-tag-1", "dheera_ai-metadata-tag-2"]
            }
        },
        proxy_server_request={},
    )
    assert "dheera_ai-metadata-tag-1" in tags
    assert "dheera_ai-metadata-tag-2" in tags
    assert len([t for t in tags if not t.startswith("User-Agent:")]) == 2
    ...
    tags = StandardLoggingPayloadSetup._get_request_tags(
        dheera_ai_params={
            ...
            "dheera_ai_metadata": {"tags": ["dheera_ai-metadata-tag"]},
        },
        proxy_server_request={},
    )
    assert "metadata-tag" in tags
    assert "dheera_ai-metadata-tag" not in tags
    assert len([t for t in tags if not t.startswith("User-Agent:")]) == 1

    # Test case 4: No tags in either
    tags = StandardLoggingPayloadSetup._get_request_tags(
        dheera_ai_params={"metadata": {}, "dheera_ai_metadata": {}},
        proxy_server_request={},
    )
    assert len([t for t in tags if not t.startswith("User-Agent:")]) == 0

    # Test case 5: None values for metadata/dheera_ai_metadata
    tags = StandardLoggingPayloadSetup._get_request_tags(
        dheera_ai_params={"metadata": None, "dheera_ai_metadata": None},
        proxy_server_request={},
    )
    assert isinstance(tags, list)
    assert len([t for t in tags if not t.startswith("User-Agent:")]) == 0

    # Test case 6: Empty dheera_ai_params
    tags = StandardLoggingPayloadSetup._get_request_tags(
        dheera_ai_params={},
        proxy_server_request={},
    )
    ...
    tags = StandardLoggingPayloadSetup._get_request_tags(
        dheera_ai_params={"metadata": {"tags": ["custom-tag"]}},
        proxy_server_request={
            "headers": {
                "user-agent": "dheera_ai/1.0.0",
            }
        },
    )
    assert "custom-tag" in tags
    assert "User-Agent: dheera_ai" in tags
    assert "User-Agent: dheera_ai/1.0.0" in tags
...
def test_get_extra_header_tags():
    ...
    original_extra_headers = getattr(dheera_ai, "extra_spend_tag_headers", None)

    try:
        # Test case 1: No extra headers configured
        dheera_ai.extra_spend_tag_headers = None
        ...
        dheera_ai.extra_spend_tag_headers = []
        result = StandardLoggingPayloadSetup._get_extra_header_tags(
            proxy_server_request={"headers": {"x-custom": "value"}}
        )
        assert result is None
        ...
        dheera_ai.extra_spend_tag_headers = ["x-custom", "x-tenant"]
        result = StandardLoggingPayloadSetup._get_extra_header_tags(
        ...
        dheera_ai.extra_spend_tag_headers = ["x-custom", "x-tenant", "x-missing"]
        result = StandardLoggingPayloadSetup._get_extra_header_tags(
            proxy_server_request={
                "headers": {
        ...
        dheera_ai.extra_spend_tag_headers = ["x-custom", "x-empty"]
        result = StandardLoggingPayloadSetup._get_extra_header_tags(
        ...
        if original_extra_headers is not None:
            dheera_ai.extra_spend_tag_headers = original_extra_headers
        else:
            # Remove the attribute if it didn't exist before
            if hasattr(dheera_ai, "extra_spend_tag_headers"):
                delattr(dheera_ai, "extra_spend_tag_headers")
...
def test_get_masked_values():
    from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import _get_masked_values
...
async def test_e2e_generate_cold_storage_object_key_successful():
    ...
    with patch("dheera_ai.cold_storage_custom_logger", return_value="s3"), patch(
        "dheera_ai.integrations.s3.get_s3_object_key"
    ) as mock_get_s3_key:
...
async def test_e2e_generate_cold_storage_object_key_with_custom_logger_s3_path():
    ...
    with patch("dheera_ai.cold_storage_custom_logger", "s3_v2"), patch(
        "dheera_ai.logging_callback_manager.get_active_custom_logger_for_callback_name"
...
async def test_e2e_generate_cold_storage_object_key_not_configured():
    ...
    with patch.object(dheera_ai, "cold_storage_custom_logger", None):
        # Call the function
        result = StandardLoggingPayloadSetup._generate_cold_storage_object_key(
            start_time=start_time, response_id=response_id, team_alias=team_alias
        )
