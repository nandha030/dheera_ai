...
4. All these exceptions are in DHEERA_AI_EXCEPTION_TYPES
"""
...
def test_bad_gateway_error_preserves_retry_after_header():
    ...
    error = dheera_ai.BadGatewayError(
...
def test_service_unavailable_error_preserves_retry_after_header():
    ...
    error = dheera_ai.ServiceUnavailableError(
...
def test_internal_server_error_preserves_retry_after_header():
    ...
    error = dheera_ai.InternalServerError(
...
def test_bad_gateway_error_in_exception_types():
    """Test that BadGatewayError is in DHEERA_AI_EXCEPTION_TYPES."""
    assert dheera_ai.BadGatewayError in dheera_ai.DHEERA_AI_EXCEPTION_TYPES
...
def test_bad_gateway_error_attributes():
    ...
    assert "DheeraAI Retried: 1 times" in str(error)
    assert "DheeraAI Max Retries: 3" in str(error)
