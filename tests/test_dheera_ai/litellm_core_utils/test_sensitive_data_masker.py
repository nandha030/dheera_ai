...
def test_excluded_keys_exact_match():
    ...
    # With excluded_keys, dheera_ai_credentials_name should NOT be masked (exact match)
    # This ensures that even if pattern matching logic changes, excluded keys won't be masked
    masked = masker.mask_dict(data, excluded_keys={"dheera_ai_credentials_name"})
    assert masked["dheera_ai_credentials_name"] == "my-credential-name"
    ...
    assert masked["dheera_ai_credentials_name"] == "my-credential-name"  # Not masked because it doesn't match patterns anyway
