...
from dheera_ai.types.utils import Delta, ModelResponse, StreamingChoices


def test_anthropic_experimental_pass_through_messages_handler():
    """
    Test that api key is passed to dheera_ai.completion
    """
    from dheera_ai.llms.anthropic.experimental_pass_through.messages.handler import (
        anthropic_messages_handler,
    )

    with patch("dheera_ai.completion", return_value="test-response") as mock_completion:
        try:
            anthropic_messages_handler(
                max_tokens=100,
...
def test_anthropic_experimental_pass_through_messages_handler_custom_llm_provider():
    """
    Test that dheera_ai.completion is called when a custom LLM provider is given
    """
    from dheera_ai.llms.anthropic.experimental_pass_through.messages.handler import (
        anthropic_messages_handler,
    )
    ...
    with patch("dheera_ai.completion", return_value="test-response") as mock_completion:
        ...
        # Assert that dheera_ai.completion was called when using a custom LLM provider
        mock_completion.assert_called_once()
