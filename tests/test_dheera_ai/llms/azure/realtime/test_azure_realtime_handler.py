...
from dheera_ai.llms.custom_httpx.http_handler import get_shared_realtime_ssl_context
...
async def test_async_realtime_uses_max_size_parameter():
    ...
    This verifies the fix for: https://github.com/BerriAI/dheera_ai/issues/15747
    """
    from dheera_ai.constants import REALTIME_WEBSOCKET_MAX_MESSAGE_SIZE_BYTES
    from dheera_ai.llms.azure.realtime.handler import AzureOpenAIRealtime
    ...
    with patch("websockets.connect", return_value=DummyAsyncContextManager(mock_backend_ws)) as mock_ws_connect, \
         patch("dheera_ai.llms.azure.realtime.handler.RealTimeStreaming") as mock_realtime_streaming:
