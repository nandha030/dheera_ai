...
from dheera_ai.llms.azure.responses.o_series_transformation import (
    AzureOpenAIOSeriesResponsesAPIConfig,
)
from dheera_ai.llms.azure.responses.transformation import AzureOpenAIResponsesAPIConfig
from dheera_ai.types.llms.openai import ResponsesAPIOptionalRequestParams
from dheera_ai.types.router import GenericDheeraAIParams


@pytest.mark.serial
def test_validate_environment_api_key_within_dheera_ai_params():
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()
    dheera_ai_params = GenericDheeraAIParams(api_key="test-api-key")

    result = azure_openai_responses_apiconfig.validate_environment(
        headers={}, model="", dheera_ai_params=dheera_ai_params
    )
...
@pytest.mark.serial
def test_validate_environment_api_key_within_dheera_ai():
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()

    with patch("dheera_ai.api_key", "test-api-key"):
...
@pytest.mark.serial
def test_validate_environment_azure_key_within_dheera_ai():
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()

    with patch("dheera_ai.azure_key", "test-azure-key"):
        dheera_ai_params = GenericDheeraAIParams()
        result = azure_openai_responses_apiconfig.validate_environment(
            headers={}, model="", dheera_ai_params=dheera_ai_params
        )
...
def test_validate_environment_azure_key_within_headers():
    ...
    result = azure_openai_responses_apiconfig.validate_environment(
        headers=headers, model="", dheera_ai_params=dheera_ai_params
    )
...
def test_get_complete_url():
    ...
    dheera_ai_params = {"api_version": "2024-05-01-preview"}

    result = azure_openai_responses_apiconfig.get_complete_url(
        api_base=api_base, dheera_ai_params=dheera_ai_params
    )

    expected = "https://dheera_ai8397336933.openai.azure.com/openai/responses?api-version=2024-05-01-preview"
...
def test_provider_config_manager_o_series_selection():
    ...
    o_series_config = ProviderConfigManager.get_provider_responses_api_config(
        provider=dheera_ai.LlmProviders.AZURE, model="o_series/gpt-o1"
    )
    ...
    regular_config = ProviderConfigManager.get_provider_responses_api_config(
        provider=dheera_ai.LlmProviders.AZURE, model="gpt-4o"
    )
    ...
    default_config = ProviderConfigManager.get_provider_responses_api_config(
        provider=dheera_ai.LlmProviders.AZURE, model=None
    )
...
class TestAzureResponsesAPIConfig:
    ...
    def test_azure_get_complete_url_with_version_types(self):
        ...
        base_url = "https://dheera_ai8397336933.openai.azure.com"

        # Test with preview version - should use openai/v1/responses
        result_preview = self.config.get_complete_url(
            api_base=base_url,
            dheera_ai_params={"api_version": "preview"},
        )
        assert (
            result_preview
            == "https://dheera_ai8397336933.openai.azure.com/openai/v1/responses?api-version=preview"
        )

        # Test with latest version - should use openai/v1/responses
        result_latest = self.config.get_complete_url(
            api_base=base_url,
            dheera_ai_params={"api_version": "latest"},
        )
        assert (
            result_latest
            == "https://dheera_ai8397336933.openai.azure.com/openai/v1/responses?api-version=latest"
        )

        # Test with date-based version - should use openai/responses
        result_date = self.config.get_complete_url(
            api_base=base_url,
            dheera_ai_params={"api_version": "2025-01-01"},
        )
        assert (
            result_date
            == "https://dheera_ai8397336933.openai.azure.com/openai/responses?api-version=2025-01-01"
        )
    ...
    def test_azure_get_complete_url_with_default_api_version(self):
        ...
        from dheera_ai.constants import AZURE_DEFAULT_RESPONSES_API_VERSION
        ...
        expected_url = f"https://dheera_ai8397336933.openai.azure.com/openai/v1/responses?api-version={AZURE_DEFAULT_RESPONSES_API_VERSION}"
        assert result_no_version == expected_url

        # Test with empty dheera_ai_params - should use default
        ...
        result_none_version = self.config.get_complete_url(
            api_base=base_url,
            dheera_ai_params={"api_version": None},
        )
    ...
    def test_azure_cancel_response_api_request(self):
        ...
        dheera_ai_params = GenericDheeraAIParams(api_version="2024-05-01-preview")
