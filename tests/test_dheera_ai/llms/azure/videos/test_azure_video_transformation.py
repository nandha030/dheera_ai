...
from dheera_ai.types.videos.main import VideoObject, VideoResponse, VideoCreateOptionalRequestParams
...
class TestAzureVideoConfig:
    ...
    @patch('dheera_ai.llms.azure.common_utils.dheera_ai')
    def test_validate_environment_with_api_key(self, mock_dheera_ai):
        """Test environment validation with provided API key - should use api-key header for Azure."""
        # Since validate_environment passes dheera_ai_params=None, it relies on dheera_ai.api_key or dheera_ai.azure_key
        mock_dheera_ai.api_key = self.api_key
        mock_dheera_ai.azure_key = None
    ...
    @patch('dheera_ai.llms.azure.common_utils.get_secret_str')
    @patch('dheera_ai.llms.azure.common_utils.dheera_ai')
    def test_validate_environment_without_api_key(self, mock_dheera_ai, mock_get_secret):
        """Test environment validation without provided API key - should fallback to secret manager."""
        mock_dheera_ai.api_key = None
        mock_dheera_ai.azure_key = None
    ...
    def test_url_construction_with_trailing_slash(self):
        ...
        dheera_ai_params = {"api_base": api_base_with_slash}
    ...
    def test_url_construction_without_trailing_slash(self):
        ...
        dheera_ai_params = {"api_base": api_base_without_slash}
    ...
    @patch('dheera_ai.llms.azure.common_utils.dheera_ai')
    def test_azure_specific_environment_validation(self, mock_dheera_ai):
        """Test Azure-specific environment validation with different key sources."""
        # Test with azure_key
        mock_dheera_ai.api_key = None
        mock_dheera_ai.azure_key = "azure-test-key"
        mock_dheera_ai.openai_key = None
