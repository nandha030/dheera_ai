...
# Test data
test_input = "Hello world from dheera_ai"
...
def test_bedrock_embedding_with_api_key_bearer_token(model, input_type, embed_response):
    ...
    with patch.object(client, "post") as mock_post:
        ...
        response = dheera_ai.embedding(**kwargs)
...
def test_bedrock_embedding_with_sigv4():
    ...
    with patch("dheera_ai.llms.bedrock.embed.embedding.BedrockEmbedding.embeddings") as mock_bedrock_embed:
        mock_embedding_response = dheera_ai.EmbeddingResponse()
...
def test_twelvelabs_input_type_parameter_mapping():
    ...
    with patch.object(client, "post") as mock_post:
        ...
        # Test with input_type parameter (new DheeraAI parameter)
        response = dheera_ai.embedding(
            model=model,
            input=test_input,
            client=client,
...
def test_bedrock_embedding_header_forwarding(model, embed_response):
    ...
    Relevant Issue: https://github.com/BerriAI/dheera_ai/pull/16042
    """
    dheera_ai.set_verbose = True
    client = HTTPHandler()
    test_api_key = "test-bearer-token-12345"
