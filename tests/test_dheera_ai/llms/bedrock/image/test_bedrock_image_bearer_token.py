...
class TestBedrockImageGeneration:
    def test_image_generation_with_api_key_bearer_token(self):
        ...
        with patch("dheera_ai.llms.bedrock.image.image_handler.BedrockImageGeneration.image_generation") as mock_bedrock_image_gen:
            # Setup mock response
            mock_image_response_obj = dheera_ai.ImageResponse()
    ...
    def test_image_generation_with_env_variable_bearer_token(self, monkeypatch):
        ...
        with patch.dict(os.environ, {"AWS_BEARER_TOKEN_BEDROCK": test_api_key}), \
             patch("dheera_ai.llms.bedrock.image.image_handler.BedrockImageGeneration.image_generation") as mock_bedrock_image_gen:
    ...
    async def test_async_image_generation_with_bearer_token(self):
        ...
        with patch("dheera_ai.llms.bedrock.image.image_handler.BedrockImageGeneration.async_image_generation") as mock_async_bedrock_image_gen:
            mock_image_response_obj = dheera_ai.ImageResponse()
            mock_image_response_obj.data = [{"url": "https://example.com/image.jpg"}]
