...
class TestBaseLLMAIOHTTPHandler:
    ...
    @patch("dheera_ai.llms.custom_httpx.aiohttp_handler.aiohttp.ClientSession")
    ...
    def test_init_with_transport(self):
        ...
        mock_transport = Mock(spec=DheeraAIAiohttpTransport)
    ...
    @patch(
        "dheera_ai.llms.custom_httpx.http_handler.AsyncHTTPHandler._create_aiohttp_transport"
    )
    def test_get_or_create_transport(self, mock_create_transport):
        """Test transport creation when none provided"""
        mock_transport_instance = Mock(spec=DheeraAIAiohttpTransport)
