...
from dheera_ai.llms.custom_httpx.aiohttp_transport import (
...
async def test_handle_async_request_uses_env_proxy(monkeypatch):
    ...
    transport = DheeraAIAiohttpTransport(client=lambda: FakeSession())  # type: ignore
...
async def test_handle_async_request_proxy_cache_per_host(monkeypatch):
    ...
    transport = DheeraAIAiohttpTransport(client=factory)  # type: ignore
...
async def test_handle_async_request_total_timeout_triggers():
    """
    Ensure that DheeraAIAiohttpTransport raises httpx.TimeoutException
