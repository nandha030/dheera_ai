...
from dheera_ai.llms.custom_httpx.http_handler import AsyncHTTPHandler, get_ssl_configuration


@pytest.mark.asyncio
async def test_ssl_security_level(monkeypatch):
    # Ensure aiohttp transport is enabled for this test
    original_disable = dheera_ai.disable_aiohttp_transport
    dheera_ai.disable_aiohttp_transport = False
    ...
    try:
        with patch.dict(os.environ, clear=True):
            ...
            # Get the transport (should be DheeraAIAiohttpTransport)
            transport = client.client._transport
            assert isinstance(transport, DheeraAIAiohttpTransport)
        ...
        dheera_ai.disable_aiohttp_transport = original_disable
...
async def test_ssl_context_transport():
    ...
    if isinstance(transport, DheeraAIAiohttpTransport):
...
async def test_ssl_verification_with_aiohttp_transport():
    ...
    We validate that the ssl settings for a dheera_ai transport match what a ssl verify=False aiohttp client would have.
    ...
    try:
        # Create a test SSL context
        dheera_ai_async_client = AsyncHTTPHandler(ssl_verify=False)

        transport = dheera_ai_async_client.client._transport
        assert isinstance(transport, DheeraAIAiohttpTransport)
        ...
        # assert both dheera_ai transport and aiohttp session have ssl_verify=False
...
async def test_aiohttp_transport_trust_env_setting(monkeypatch):
    ...
    # Default should be False (dheera_ai.aiohttp_trust_env default)
    default_trust_env = getattr(dheera_ai, 'aiohttp_trust_env', False)
    ...
    # Should respect the dheera_ai.aiohttp_trust_env setting when env var is False
...
def test_get_ssl_configuration():
    ...
    from dheera_ai.llms.custom_httpx.http_handler import _ssl_context_cache
...
def test_shared_session_parameter_in_acompletion():
    ...
    import inspect
    from dheera_ai.main import acompletion
...
def test_shared_session_parameter_in_completion():
    ...
    import inspect
    from dheera_ai.main import completion
...
@pytest.mark.parametrize(
    "env_curve,dheera_ai_curve,expected_curve,should_call",
    [
        # env_curve: SSL_ECDH_CURVE env var | dheera_ai_curve: dheera_ai.ssl_ecdh_curve variable
        ...
        (None, "secp384r1", "secp384r1", True),     # dheera_ai variable only
...
def test_ssl_ecdh_curve(env_curve, dheera_ai_curve, expected_curve, should_call, monkeypatch):
    ...
    with patch.dict(os.environ, clear=True):
        ...
        original_value = dheera_ai.ssl_ecdh_curve
        try:
            dheera_ai.ssl_ecdh_curve = dheera_ai_curve
            ...
            dheera_ai.ssl_ecdh_curve = original_value
