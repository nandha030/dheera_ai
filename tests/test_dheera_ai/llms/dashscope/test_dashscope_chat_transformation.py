...
from dheera_ai.llms.dashscope.chat.transformation import DashScopeChatConfig
...
class TestDashScopeConfig:
    ...
    def test_dashscope_completion_mock(self, respx_mock):
        ...
        response = completion(
            model=model,
            messages=[
                {"role": "user", "content": "write code for saying hey from DheeraAI"}
            ],
            api_key=api_key,
            api_base=api_base,
        )
        ...
        assert "Hey from DheeraAI" in response.choices[0].message.content
