...
from dheera_ai.llms.featherless_ai.chat.transformation import FeatherlessAIConfig
...
class TestFeatherlessAIConfig:
    ...
    def test_featherless_ai_completion_mock(self, respx_mock):
                ...
                "choices": [
                    {
                        ...
                        "message": {
                            "role": "assistant",
                            "content": '```python\nprint("Hi from DheeraAI!")\n```\n\nThis simple Python code prints a greeting message from DheeraAI.',
                        },
                        "finish_reason": "stop",
                    }
                ],
        ...
        # Make the actual API call through DheeraAI
        response = completion(
            model=model,
            messages=[
                {"role": "user", "content": "write code for saying hi from DheeraAI"}
            ],
            api_key=api_key,
            api_base=api_base,
        )
        ...
        assert "Hi from DheeraAI" in response.choices[0].message.content
