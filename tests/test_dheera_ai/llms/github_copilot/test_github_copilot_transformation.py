...
import dheera_ai.llms.github_copilot.chat.transformation
from dheera_ai import Choices, Message, ModelResponse, Usage, acompletion, completion
from dheera_ai.exceptions import AuthenticationError
from dheera_ai.llms.github_copilot.authenticator import Authenticator
from dheera_ai.llms.github_copilot.chat.transformation import GithubCopilotConfig
from dheera_ai.llms.github_copilot.common_utils import (
    APIKeyExpiredError,
    GetAccessTokenError,
    GetAPIKeyError,
    GetDeviceCodeError,
...
@patch("dheera_ai.llms.github_copilot.authenticator.Authenticator.get_api_key")
@patch("dheera_ai.llms.openai.openai.OpenAIChatCompletion.completion")
...
def test_transform_messages_disable_copilot_system_to_assistant(monkeypatch):
    ...
    # Save original value
    original_flag = dheera_ai.disable_copilot_system_to_assistant
    try:
        # Case 1: Flag is False (default, conversion happens)
        dheera_ai.disable_copilot_system_to_assistant = False
        config = GithubCopilotConfig()
        messages = [
        ...
        dheera_ai.disable_copilot_system_to_assistant = True
        out = config._transform_messages([m.copy() for m in messages], model="github_copilot/gpt-4")
        ...
        dheera_ai.disable_copilot_system_to_assistant = original_flag
