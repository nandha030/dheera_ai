...
from dheera_ai.llms.oci.chat.transformation import OCIChatConfig, OCIRequestWrapper, version
...
class TestOCIChatConfig:
    def test_validate_environment_with_oci_region(self, supplied_params):
        ...
        assert result["user-agent"] == f"dheera_ai/{version}"
    ...
    def test_transform_response_simple_text(self):
        ...
        assert isinstance(result.choices[0], dheera_ai.Choices)
        ...
        assert isinstance(result.usage, dheera_ai.Usage) # type: ignore
    ...
    def test_transform_response_with_tool_calls(self):
        ...
        model_response = ModelResponse(
            choices=[dheera_ai.Choices(index=0, message=dheera_ai.Message())]
        )
        ...
        assert isinstance(tool_call, dheera_ai.utils.ChatCompletionMessageToolCall)
        ...
        assert isinstance(usage, dheera_ai.Usage) # type: ignore
...
class TestOCISignerSupport:
    ...
    def test_sign_request_with_failing_oci_signer(self):
        ...
        from dheera_ai.llms.oci.common_utils import OCIError
