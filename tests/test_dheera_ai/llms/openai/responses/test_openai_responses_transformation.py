...
class TestOpenAIResponsesAPIConfig:
    ...
    def test_validate_environment(self):
        ...
        dheera_ai_params = GenericDheeraAIParams(api_key=api_key)
        result = self.config.validate_environment(
            headers=headers, model=self.model, dheera_ai_params=dheera_ai_params
        )
        ...
        with patch("dheera_ai.api_key", "dheera_ai_api_key"):
            ...
            assert "Authorization" in result
            assert result["Authorization"] == "Bearer dheera_ai_api_key"

        # Test with existing headers
        headers = {"Content-Type": "application/json"}

        with patch("dheera_ai.openai_key", "openai_key"):
            with patch("dheera_ai.api_key", None):
                dheera_ai_params = GenericDheeraAIParams()
                result = self.config.validate_environment(
                    headers=headers, model=self.model, dheera_ai_params=dheera_ai_params
                )
        ...
        with patch("dheera_ai.api_key", None):
            with patch("dheera_ai.openai_key", None):
                with patch(
                    "dheera_ai.llms.openai.responses.transformation.get_secret_str",
    ...
    def test_get_complete_url(self):
        ...
        # Test with dheera_ai.api_base
        with patch("dheera_ai.api_base", "https://dheera_ai-api-base.example.com/v1"):
            result = self.config.get_complete_url(
                api_base=None,
                dheera_ai_params={},
            )

            assert result == "https://dheera_ai-api-base.example.com/v1/responses"

        # Test with environment variable
        with patch("dheera_ai.api_base", None):
            with patch(
                "dheera_ai.llms.openai.responses.transformation.get_secret_str",
    ...
    def test_get_event_model_class_generic_event(self):
        ...
        from dheera_ai.types.llms.openai import GenericEvent
    ...
    def test_transform_streaming_response_coalesces_null_error_code(self):
        ...
        """
        from dheera_ai.types.llms.openai import ErrorEvent
...
class TestTransformListInputItemsRequest:
    ...
    def setup_method(self):
        ...
        self.dheera_ai_params = GenericDheeraAIParams()
    ...
    @patch("dheera_ai.router.Router")
    def test_mock_dheera_ai_router_with_transform_list_input_items_request(
        self, mock_router
    ):
        """Mock test using dheera_ai.router for transform_list_input_items_request"""
        # Setup mock router
    ...
    @patch("dheera_ai.list_input_items")
    def test_mock_dheera_ai_list_input_items_integration(self, mock_list_input_items):
        """Test integration with dheera_ai.list_input_items function"""
        # Setup mock response
        mock_response = {
            "object": "list",
            "data": [
