...
def test_gpt5_temperature_error(config: OpenAIConfig):
    with pytest.raises(dheera_ai.utils.UnsupportedParamsError):
...
def test_gpt5_codex_temperature_error(config: OpenAIConfig):
    ...
    with pytest.raises(
        dheera_ai.utils.UnsupportedParamsError,
...
def test_gpt5_1_reasoning_effort_none(config: OpenAIConfig):
    ...
    Related issue: https://github.com/BerriAI/dheera_ai/issues/16633
