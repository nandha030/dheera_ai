...
from dheera_ai.llms.openrouter.chat.transformation import (
...
def test_openrouter_extra_body_transformation():
    ...
    # https://github.com/BerriAI/dheera_ai/issues/8425, validate its not contained in extra_body still
...
def test_openrouter_cost_tracking_non_streaming():
    ...
    assert "llm_provider-x-dheera_ai-response-cost" in result._hidden_params["additional_headers"]
    assert result._hidden_params["additional_headers"]["llm_provider-x-dheera_ai-response-cost"] == 0.00015
