...
import dheera_ai
from dheera_ai.llms.ragflow.chat.transformation import RAGFlowConfig
...
class TestRAGFlowChatTransformation:
    ...
    def test_get_complete_url_from_dheera_ai_params(self):
        """Test URL construction with api_base from dheera_ai_params."""
        config = RAGFlowConfig()
        
        model = "ragflow/chat/my-chat-id/gpt-4o-mini"
        # Create a simple dict-like object for dheera_ai_params
        class DheeraAIParams:
            def __init__(self):
                self.api_base = "http://ragflow-server:9380"
        
        dheera_ai_params = DheeraAIParams()
    ...
    def test_validate_environment_stores_actual_model(self):
        ...
        assert dheera_ai_params["_ragflow_actual_model"] == "gpt-4o-mini"
    ...
    def test_validate_environment_from_dheera_ai_params(self):
        """Test that validate_environment gets api_key from dheera_ai_params."""
        config = RAGFlowConfig()
        
        headers = {}
        model = "ragflow/chat/my-chat-id/gpt-4o-mini"
        messages = [{"role": "user", "content": "Hello"}]
        # Create a simple object for dheera_ai_params with api_key attribute
        class DheeraAIParams:
            def __init__(self):
                self.api_key = "dheera_ai-params-key"
        ...
        assert result_headers["Authorization"] == "Bearer dheera_ai-params-key"
    ...
    def test_transform_request_uses_actual_model(self):
        ...
        model = "ragflow/chat/my-chat-id/gpt-4o-mini"
        messages = [{"role": "user", "content": "Hello"}]
        dheera_ai_params = {"_ragflow_actual_model": "gpt-4o-mini"}
    ...
    def test_transform_request_fallback_parsing(self):
        ...
        model = "ragflow/agent/my-agent-id/gpt-4o-mini"
        messages = [{"role": "user", "content": "Hello"}]
        dheera_ai_params = {}  # Missing _ragflow_actual_model
