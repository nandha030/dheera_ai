...
async def test_sap_chat(
    ...
    with patch(
        "dheera_ai.llms.sap.chat.transformation.GenAIHubOrchestrationConfig.deployment_url",
        new_callable=PropertyMock,
        return_value=fake_deployment_url,
    ), patch(
        "dheera_ai.llms.sap.chat.transformation.get_token_creator",
        ...
        if sync_mode:
            response = dheera_ai.completion(model=model, messages=messages)
        else:
...
async def test_sap_streaming(
    ...
    with patch(
        ...
        stream = dheera_ai.completion(model=model, messages=messages, stream=True)
