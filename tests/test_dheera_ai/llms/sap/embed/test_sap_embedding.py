...
async def test_sap_chat(
    ...
    with patch(
        "dheera_ai.llms.sap.embed.transformation.GenAIHubEmbeddingConfig.deployment_url",
        new_callable=PropertyMock,
        return_value=fake_deployment_url,
    ), patch(
        "dheera_ai.llms.sap.embed.transformation.get_token_creator",
        ...
        if sync_mode:
            response = dheera_ai.embedding(model=model, input=input)
        else:
            response = await dheera_ai.aembedding(model=model, input=input)
