...
class TestVertexAIFilesIntegration:
    """Test integration of Vertex AI files with main dheera_ai API"""

    @pytest.mark.asyncio
    async def test_dheera_ai_afile_content_vertex_ai_provider(self):
        """Test dheera_ai.afile_content with vertex_ai provider"""
        file_id = "gs%3A%2F%2Ftest-bucket%2Ftest-file.txt"
        expected_content = b"test file content"

        # Mock the vertex_ai_files_instance.file_content method
        with patch(
            "dheera_ai.files.main.vertex_ai_files_instance.file_content",
    ...
    def test_dheera_ai_file_content_vertex_ai_provider(self):
        """Test dheera_ai.file_content with vertex_ai provider (sync)"""
        file_id = "gs%3A%2F%2Ftest-bucket%2Ftest-file.txt"
        expected_content = b"test file content"
        ...
        with patch(
            ...
            # Call dheera_ai.file_content
            result = dheera_ai.file_content(
                file_id=file_id,
                custom_llm_provider="vertex_ai",
                vertex_project="test-project",
                vertex_location="us-central1",
    ...
    def test_dheera_ai_file_content_vertex_ai_with_model_provider_detection(self):
        """Test dheera_ai.file_content with model parameter for provider detection"""
        file_id = "gs%3A%2F%2Ftest-bucket%2Ftest-file.txt"
        expected_content = b"test file content"
        ...
        with patch(
            ...
            with patch("dheera_ai.files.main.get_llm_provider") as mock_get_provider:
                ...
                # Call dheera_ai.file_content with model to trigger provider detection
                result = dheera_ai.file_content(
                    file_id=file_id,
