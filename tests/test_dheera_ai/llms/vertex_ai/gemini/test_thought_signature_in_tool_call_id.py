...
When using OpenAI clients (instead of DheeraAI SDK), provider_specific_fields are not preserved.
...
def test_tool_call_id_includes_signature_in_response(enable_preview_features):
    ...
    # Save original state
    original_flag = dheera_ai.enable_preview_features
    dheera_ai.enable_preview_features = enable_preview_features
    ...
    try:
        ...
        dheera_ai.enable_preview_features = original_flag
...
def test_get_thought_signature_backward_compatibility():
    ...
    # Test with provider_specific_fields (DheeraAI SDK scenario)
    tool = {
        "id": "call_abc123",
        "type": "function",
        "function": {
...
def test_openai_client_e2e_flow(enable_preview_features):
    ...
    1. DheeraAI receives response from Gemini with thought signature
    2. DheeraAI embeds signature in tool call ID (if preview features enabled)
    3. OpenAI client sends message back with same tool call ID
    4. DheeraAI extracts signature from ID/provider_specific_fields and sends to Gemini
    """
    test_signature = ""
    ...
    try:
        ...
        # Step 2: DheeraAI transforms to OpenAI format
        function, tools, _ = VertexGeminiConfig._transform_parts(
            parts=gemini_parts,
            cumulative_tool_call_idx=0,
            is_function_call=False,
        ...
        # Step 4: DheeraAI converts back to Gemini format, extracting signature
