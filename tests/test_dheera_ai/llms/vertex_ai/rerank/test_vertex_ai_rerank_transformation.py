...
from dheera_ai.llms.vertex_ai.rerank.transformation import VertexAIRerankConfig
...
class TestVertexAIRerankTransform:
    ...
    @patch('dheera_ai.llms.vertex_ai.rerank.transformation.VertexAIRerankConfig._ensure_access_token')
    def test_get_complete_url(self, mock_ensure_access_token):
        ...
        # Test with dheera_ai.vertex_project
        with patch.dict(os.environ, {}, clear=True):
            import dheera_ai

            # Set vertex_project attribute if it doesn't exist
            if not hasattr(dheera_ai, 'vertex_project'):
                dheera_ai.vertex_project = None
            original_project = dheera_ai.vertex_project
            dheera_ai.vertex_project = "dheera_ai-project-456"
            try:
                url = self.config.get_complete_url(api_base=None, model=self.model)
                expected_url = "https://discoveryengine.googleapis.com/v1/projects/dheera_ai-project-456/locations/global/rankingConfigs/default_ranking_config:rank"
                assert url == expected_url
            finally:
                dheera_ai.vertex_project = original_project
    ...
    def test_validate_environment_preserves_optional_params_for_get_complete_url(
        ...
        with patch('dheera_ai.llms.vertex_ai.rerank.transformation.get_secret_str', return_value=None):
