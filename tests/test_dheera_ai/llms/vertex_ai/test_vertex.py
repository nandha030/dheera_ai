...
import dheera_ai.dheera_ai_core_utils.prompt_templates
import dheera_ai.dheera_ai_core_utils.prompt_templates.factory
...
from dheera_ai.llms.vertex_ai.gemini.transformation import _process_gemini_image
...
def test_logprobs_unit_test():
    from dheera_ai import VertexGeminiConfig
...
def test_process_gemini_image():
    ...
    from dheera_ai.llms.vertex_ai.gemini.transformation import _process_gemini_image
    from dheera_ai.types.llms.vertex_ai import FileDataType
...
def test_vertex_embedding_url(model, expected_url):
    ...
    Relevant issue: https://github.com/BerriAI/dheera_ai/issues/6482

    When a fine-tuned embedding model is used, the URL is different from the standard one.
    """
    from dheera_ai.llms.vertex_ai.common_utils import _get_vertex_url
...
def mock_convert_url_to_base64():
    with patch(
        "dheera_ai.dheera_ai_core_utils.prompt_templates.factory.convert_url_to_base64",
    ) as mock:
...
def test_aaavertex_embeddings_distances(
    ...
    with patch.object(vertex_client, "post", return_value=mock_response), patch.object(
        dheera_ai.main.vertex_multimodal_embedding,
    ...
    with patch.object(
        ...
        text_response = dheera_ai.embedding(
            model="vertex_ai/multimodalembedding@001",
            input=[input_string],
            client=vertex_client,
        )
...
def test_vertex_parallel_tool_calls_false_multiple_tools_error():
    ...
    with pytest.raises(dheera_ai.utils.UnsupportedParamsError) as excinfo:
        get_optional_params(
            model="gemini-1.5-pro",
            custom_llm_provider="vertex_ai",
            tools=tools,
...
from dheera_ai.llms.vertex_ai.gemini.transformation import _transform_request_body
