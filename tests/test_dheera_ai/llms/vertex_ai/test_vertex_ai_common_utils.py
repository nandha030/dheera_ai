...
async def test_get_supports_system_message():
    ...
    from dheera_ai.llms.vertex_ai.common_utils import get_supports_system_message
...
def test_build_vertex_schema():
    ...
    from dheera_ai.llms.vertex_ai.common_utils import _build_vertex_schema
...
def test_process_items_with_excessive_nesting():
    ...
    # generate a schema with excessive nesting
    from dheera_ai.llms.vertex_ai.common_utils import process_items
...
def test_get_vertex_url_global_region(stream, expected_endpoint_suffix):
    ...
    # Mock dheera_ai.VertexGeminiConfig.get_model_for_vertex_ai_url to return model as is
    # as we are not testing that part here, just the URL construction
    with patch(
        "dheera_ai.VertexGeminiConfig.get_model_for_vertex_ai_url",
...
def test_is_global_only_vertex_model(supported_regions, expected_result):
    ...
    from dheera_ai.llms.vertex_ai.common_utils import is_global_only_vertex_model

    with patch("dheera_ai.utils.get_supported_regions") as mock_get_supported_regions:
...
def test_get_vertex_region_global_only_model(
    ...
    with patch(
        "dheera_ai.llms.vertex_ai.vertex_llm_base.is_global_only_vertex_model"
...
def test_vertex_filter_format_uri():
    ...
    from dheera_ai.llms.vertex_ai.common_utils import filter_schema_fields
...
def test_convert_schema_types_type_array_conversion():
    ...
    Relevant issue: https://github.com/BerriAI/dheera_ai/issues/14091
    """
    from dheera_ai.llms.vertex_ai.common_utils import _convert_schema_types
...
def test_fix_enum_empty_strings():
    ...
    """
    from dheera_ai.llms.vertex_ai.common_utils import _fix_enum_empty_strings
...
def test_fix_enum_types():
    ...
    """
    from dheera_ai.llms.vertex_ai.common_utils import _fix_enum_types
...
async def test_vertex_ai_token_counter_routes_partner_models():
    ...
    from dheera_ai.llms.vertex_ai.common_utils import VertexAITokenCounter
    ...
    with patch(
        "dheera_ai.llms.vertex_ai.vertex_ai_partner_models.main.VertexAIPartnerModels.count_tokens"
...
async def test_vertex_ai_token_counter_routes_gemini_models():
    ...
    with patch(
        "dheera_ai.llms.vertex_ai.count_tokens.handler.VertexAITokenCounter.acount_tokens"
