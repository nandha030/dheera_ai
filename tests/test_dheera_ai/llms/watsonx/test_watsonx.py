...
async def test_watsonx_gpt_oss_prompt_transformation(monkeypatch):
    ...
    This test starts from dheera_ai.acompletion and verifies what gets sent in the final POST request body.
    ...
    if hf_model in dheera_ai.known_tokenizer_config:
        del dheera_ai.known_tokenizer_config[hf_model]
    
    with patch.object(client, "post") as mock_post, patch.object(
        dheera_ai.module_level_client, "post", return_value=mock_token_response
    ), patch(
        "dheera_ai.dheera_ai_core_utils.prompt_templates.huggingface_template_handler._aget_tokenizer_config",
        side_effect=mock_aget_tokenizer_config,
    ), patch(
        "dheera_ai.dheera_ai_core_utils.prompt_templates.huggingface_template_handler._aget_chat_template_file",
    ...
    print(f"\n{'='*80}")
    print(f"Input messages to dheera_ai.acompletion:")
...
async def test_watsonx_gpt_oss_uses_async_http_handler():
    ...
    with patch(
        "dheera_ai.dheera_ai_core_utils.prompt_templates.huggingface_template_handler.get_async_httpx_client",
...
def test_watsonx_chat_completion_with_reasoning_effort(monkeypatch):
    ...
    # Call dheera_ai.completion with the new parameter
    with patch.object(client, "post") as mock_post, patch.object(
        dheera_ai.module_level_client, "post", return_value=mock_token_response
    ):
        try:
