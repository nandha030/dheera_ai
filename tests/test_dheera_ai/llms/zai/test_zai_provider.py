...
def test_zai_in_provider_lists():
    ...
    assert "zai" in dheera_ai.openai_compatible_providers
    assert "zai" in dheera_ai.provider_list
...
def test_zai_models_in_model_cost():
    ...
    for model in zai_models:
        assert model in dheera_ai.model_cost, f"Model {model} not found in model_cost"
        assert dheera_ai.model_cost[model]["dheera_ai_provider"] == "zai"
