...
import dheera_ai
from dheera_ai.passthrough.main import llm_passthrough_route
...
def test_bedrock_application_inference_profile_url_encoding():
    ...
    with patch(
        "dheera_ai.utils.ProviderConfigManager.get_provider_passthrough_config",
        return_value=mock_provider_config,
    ), patch(
        "dheera_ai.dheera_ai_core_utils.get_dheera_ai_params.get_dheera_ai_params",
        return_value={},
    ), patch(
        "dheera_ai.dheera_ai_core_utils.get_llm_provider_logic.get_llm_provider",
        ...
        response = llm_passthrough_route(
            ...
            client=client,
            dheera_ai_logging_obj=mock_logging_obj,
        )
...
async def test_pass_through_request_stream_param_override(
    ...
    with patch(
        "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints.get_async_httpx_client",
        return_value=mock_client_obj,
    ), patch(
        "dheera_ai.proxy.proxy_server.proxy_logging_obj.pre_call_hook",
