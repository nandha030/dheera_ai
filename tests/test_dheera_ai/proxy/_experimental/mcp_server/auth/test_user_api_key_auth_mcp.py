...
from dheera_ai.proxy._types import SpecialHeaders, UserAPIKeyAuth
...
class TestMCPRequestHandler:
    ...
    async def test_permission_inheritance_edge_cases(self):
        ...
        mock_prisma_client.db.dheera_ai_objectpermissiontable.find_unique.return_value = (
            None
        )
        ...
        with patch("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma_client):
            result = await MCPRequestHandler.get_allowed_mcp_servers(user_api_key_auth)
            assert result == []

        # Test case: Exception handling
        mock_prisma_client.db.dheera_ai_objectpermissiontable.find_unique.side_effect = (
            Exception("DB Error")
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            # Test case 1: x-dheera_ai-api-key header present
            (
                [(b"x-dheera_ai-api-key", b"test-api-key-123")],
                "test-api-key-123",
                None,
                {},
            ),
            ...
            (
                [
                    (b"x-dheera_ai-api-key", b"primary-key"),
    ...
    async def test_process_mcp_request_with_server_auth_headers(
        ...
        with patch(
            "dheera_ai.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.user_api_key_auth",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [
                    (b"x-dheera_ai-api-key", b"test-api-key"),
            ...
            # Test case 6: Using Authorization header instead of x-dheera_ai-api-key
            (
                [
    ...
    async def test_header_extraction(self, headers, expected_result):
        ...
        api_key = MCPRequestHandler.get_dheera_ai_api_key_from_headers(extracted_headers)
...
class TestMCPCustomHeaderName:
    ...
    def test_get_mcp_client_side_auth_header_name(
        ...
        with patch("dheera_ai.secret_managers.main.get_secret_str") as mock_get_secret:
            with patch(
                ...
                for call in mock_get_secret.call_args_list:
                    assert call.args == ("DHEERA_AI_MCP_CLIENT_SIDE_AUTH_HEADER_NAME",)
    ...
    def test_get_mcp_server_auth_headers_from_headers(self):
        ...
        headers = Headers(
            {"x-dheera_ai-api-key": "test-key", "content-type": "application/json"}
        )
        ...
        headers = Headers(
            {"x-dheera_ai-api-key": "test-key", "x-mcp-github-authorization": ""}
        )
        result = MCPRequestHandler._get_mcp_server_auth_headers_from_headers(headers)
        ...
        headers = Headers(
            {"x-dheera_ai-api-key": "test-key", "x-mcp-github-authorization": long_token}
        )
        result = MCPRequestHandler._get_mcp_server_auth_headers_from_headers(headers)
...
def test_mcp_path_based_server_segregation(monkeypatch):
    ...
    from dheera_ai.proxy._experimental.mcp_server.server import app, get_auth_context
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy._experimental.mcp_server.server.initialize_session_managers",
        AsyncMock(),
    )
    ...
    response = client.get(
        "/mcp/zapier,group1/tools", headers={"x-dheera_ai-api-key": "test"}
    )
...
async def test_get_team_object_permission_with_already_loaded_permission():
    ...
    """
    from dheera_ai.proxy._types import DheeraAI_ObjectPermissionTable, DheeraAI_TeamTable

    # Create mock object permission
    mock_object_permission = DheeraAI_ObjectPermissionTable(
    ...
    mock_team_obj = DheeraAI_TeamTable(
        team_id="team-123",
    ...
    with patch(
        ...
        with patch(
            ...
            with patch(
                "dheera_ai.proxy.auth.auth_checks.get_object_permission"
            ) as mock_get_perm:
...
async def test_get_allowed_mcp_servers_for_team_uses_helper():
    ...
    """
    from dheera_ai.proxy._types import DheeraAI_ObjectPermissionTable
