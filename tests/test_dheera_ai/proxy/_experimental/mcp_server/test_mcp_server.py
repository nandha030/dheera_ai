...
async def test_mcp_server_tool_call_body_contains_request_data():
    ...
    # Mock the add_dheera_ai_data_to_request function to capture the data
    captured_data = {}

    async def mock_add_dheera_ai_data_to_request(
    ...
    with patch(
        "dheera_ai.proxy.dheera_ai_pre_call_utils.add_dheera_ai_data_to_request",
        mock_add_dheera_ai_data_to_request,
    ):
        with patch(
            "dheera_ai.proxy._experimental.mcp_server.server.call_mcp_tool",
            mock_call_mcp_tool,
        ):
            with patch(
                "dheera_ai.proxy.proxy_server.proxy_config",
                MagicMock(),
            ):
                # Call the function
...
async def test_get_prompts_from_mcp_servers_success():
    ...
    with patch(
        "dheera_ai.proxy._experimental.mcp_server.server._get_allowed_mcp_servers",
        AsyncMock(return_value=[server_a, server_b]),
    ) as mock_allowed, patch(
        "dheera_ai.proxy._experimental.mcp_server.server._prepare_mcp_server_headers",
...
async def test_mcp_get_prompt_success():
    try:
        from dheera_ai.proxy._experimental.mcp_server.server import mcp_get_prompt
    except ImportError:
        pytest.skip("MCP server not available")
...
async def test_mcp_read_resource_success():
    try:
        from dheera_ai.proxy._experimental.mcp_server.server import mcp_read_resource
    except ImportError:
        pytest.skip("MCP server not available")
...
async def test_get_tools_from_mcp_servers_continues_when_one_server_fails():
    ...
    with patch(
        ...
        with patch(
            "dheera_ai.proxy._experimental.mcp_server.server.verbose_logger",
        ) as mock_logger:
...
async def test_concurrent_initialize_session_managers():
    ...
    import dheera_ai.proxy._experimental.mcp_server.server as mcp_server
...
async def test_mcp_routing_with_conflicting_alias_and_group_name():
    ...
    try:
        ...
        from dheera_ai.proxy._types import MCPTransport
    ...
    with patch(
        "dheera_ai.proxy._experimental.mcp_server.server.global_mcp_server_manager.get_allowed_mcp_servers",
        mock_get_allowed,
    ), patch(
        "dheera_ai.proxy._experimental.mcp_server.server.MCPRequestHandler._get_mcp_servers_from_access_groups",
        mock_db_lookup,
    ), patch(
        "dheera_ai.proxy._experimental.mcp_server.server.global_mcp_server_manager._get_tools_from_server",
        mock_get_tools_spy,
    ):
...
async def test_call_mcp_tool_user_unauthorized_access():
    ...
    from dheera_ai.proxy._experimental.mcp_server.server import call_mcp_tool
    from dheera_ai.proxy._types import UserAPIKeyAuth
    ...
    with patch(
        "dheera_ai.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.MCPRequestHandler.get_allowed_mcp_servers",
        AsyncMock(return_value=["allowed_server", "another_server"]),
    ), patch(
        "dheera_ai.proxy._experimental.mcp_server.server.global_mcp_server_manager.get_mcp_server_by_id",
...
async def test_list_tools_filters_by_key_team_permissions():
    ...
    try:
        ...
        from dheera_ai.proxy._types import DheeraAI_ObjectPermissionTable, UserAPIKeyAuth
    except ImportError:
        pytest.skip("MCP server not available")

    # Create object permission with tool-level restrictions
    object_permission = DheeraAI_ObjectPermissionTable(
...
async def test_list_tools_with_team_tool_permissions_inheritance():
    ...
    team_object_permission = DheeraAI_ObjectPermissionTable(
    ...
    key_object_permission = DheeraAI_ObjectPermissionTable(
    ...
    with patch(
        ...
        with patch(
            "dheera_ai.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.MCPRequestHandler._get_team_object_permission",
...
async def test_list_tools_strips_prefix_when_matching_permissions():
    ...
    Tools from MCP servers are prefixed (e.g., "GITMCP-fetch_dheera_ai_documentation"),
    but allowed tools in DB are stored without prefix (e.g., "fetch_dheera_ai_documentation").
    ...
    object_permission = DheeraAI_ObjectPermissionTable(
        ...
        mcp_tool_permissions={
            "gitmcp_server": [
                "fetch_dheera_ai_documentation",  # No prefix in DB
                "search_dheera_ai_code",  # No prefix in DB
            ],
        },
    )
    ...
    async def mock_get_tools_from_server(
        ...
        tool1 = MagicMock()
        tool1.name = "GITMCP-fetch_dheera_ai_documentation"  # Prefixed
        tool1.description = "Fetch docs"
        tool1.inputSchema = {}

        tool2 = MagicMock()
        tool2.name = (
            "GITMCP-search_dheera_ai_documentation"  # Prefixed, not in allowed list
        )
        tool2.description = "Search docs"
        tool2.inputSchema = {}

        tool3 = MagicMock()
        tool3.name = "GITMCP-search_dheera_ai_code"  # Prefixed
    ...
    assert tool_names == [
        "GITMCP-fetch_dheera_ai_documentation",
        "GITMCP-search_dheera_ai_code",
    ]
...
def test_filter_tools_by_allowed_tools():
    ...
    from dheera_ai.types.mcp import MCPTransport
    from dheera_ai.types.mcp_server.mcp_server_manager import MCPServer
