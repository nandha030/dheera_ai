...
Tests that invoke_agent_a2a properly integrates with add_dheera_ai_data_to_request.
"""
...
@pytest.mark.asyncio
async def test_invoke_agent_a2a_adds_dheera_ai_data():
    """
    Test that invoke_agent_a2a calls add_dheera_ai_data_to_request
    and the resulting data includes proxy_server_request.
    """
    from dheera_ai.proxy._types import UserAPIKeyAuth

    # Track the data passed to add_dheera_ai_data_to_request
    captured_data = {}

    async def mock_add_dheera_ai_data(data, **kwargs):
        # Simulate what add_dheera_ai_data_to_request does
    ...
    # This test focuses on DheeraAI integration, not A2A protocol correctness,
    ...
    with patch(
        "dheera_ai.proxy.agent_endpoints.a2a_endpoints._get_agent",
        return_value=mock_agent,
    ), patch(
        "dheera_ai.proxy.dheera_ai_pre_call_utils.add_dheera_ai_data_to_request",
        side_effect=mock_add_dheera_ai_data,
    ) as mock_add_data, patch(
        "dheera_ai.a2a_protocol.create_a2a_client",
        new_callable=AsyncMock,
    ), patch(
        "dheera_ai.a2a_protocol.asend_message",
        new_callable=AsyncMock,
        ...
        "dheera_ai.proxy.proxy_server.version",
        "1.0.0",
    ), patch.dict(
        sys.modules,
        {"a2a": MagicMock(), "a2a.types": mock_a2a_types},
    ):
        from dheera_ai.proxy.agent_endpoints.a2a_endpoints import invoke_agent_a2a
        ...
        # Verify add_dheera_ai_data_to_request was called
