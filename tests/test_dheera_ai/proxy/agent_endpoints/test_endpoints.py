...
from dheera_ai.proxy.agent_endpoints import endpoints as agent_endpoints
from dheera_ai.proxy.agent_endpoints.endpoints import (
    get_agent_daily_activity,
    router,
    user_api_key_auth,
)
from dheera_ai.types.agents import AgentResponse
...
def _sample_agent_config() -> dict:
    return {
        ...
        "dheera_ai_params": {"make_public": False},
    }
...
def mock_user_api_key_auth():
    with patch("dheera_ai.proxy.agent_endpoints.endpoints.user_api_key_auth") as mock:
...
def test_update_agent_success(mock_prisma_client, mock_user_api_key_auth, monkeypatch):
    ...
    mock_prisma_client.db.dheera_ai_agentstable.find_unique = AsyncMock(
...
def test_update_agent_not_found(
    ...
    mock_prisma_client.db.dheera_ai_agentstable.find_unique = AsyncMock(return_value=None)
...
async def test_get_agent_daily_activity_admin_param_passing(monkeypatch):
    ...
    mock_prisma.db.dheera_ai_agentstable.find_many = AsyncMock(return_value=[])
    ...
    assert kwargs["table_name"] == "dheera_ai_dailyagentspend"
...
async def test_get_agent_daily_activity_with_agent_names(monkeypatch):
    ...
    mock_prisma.db.dheera_ai_agentstable.find_many = AsyncMock(
