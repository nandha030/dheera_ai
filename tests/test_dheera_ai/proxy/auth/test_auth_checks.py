...
from dheera_ai.proxy.common_utils.encrypt_decrypt_utils import decrypt_value_helper
from dheera_ai.utils import get_utc_datetime


@pytest.fixture(autouse=True)
def set_salt_key(monkeypatch):
    """Automatically set DHEERA_AI_SALT_KEY for all tests"""
    monkeypatch.setenv("DHEERA_AI_SALT_KEY", "sk-1234")


@pytest.fixture
def valid_sso_user_defined_values():
    return DheeraAI_UserTable(
        user_id="test_user",
...
def test_get_experimental_ui_login_jwt_auth_token_valid(valid_sso_user_defined_values):
    ...
    assert token_data["max_budget"] == dheera_ai.max_ui_session_budget
...
def test_get_key_object_from_ui_hash_key_valid(
    ...
    assert key_object.max_budget == dheera_ai.max_ui_session_budget
...
async def test_default_internal_user_params_with_get_user_object(monkeypatch):
    ...
    monkeypatch.setattr(dheera_ai, "default_internal_user_params", default_params)
    ...
    mock_prisma_client.db.dheera_ai_usertable.create = AsyncMock(return_value=mock_user)
    ...
    try:
        ...
        # this fails since the mock object is a MagicMock and not a DheeraAI_UserTable
        print(e)

    # Verify the user was created with the default params
    mock_prisma_client.db.dheera_ai_usertable.create.assert_called_once()
    creation_args = mock_prisma_client.db.dheera_ai_usertable.create.call_args[1]["data"]
...
@pytest.mark.asyncio
@patch("dheera_ai.proxy.management_endpoints.team_endpoints.new_team", new_callable=AsyncMock)
async def test_get_team_db_check_calls_new_team_on_upsert(mock_new_team, monkeypatch):
    ...
    mock_db = AsyncMock()
    mock_prisma_client.db = mock_db
    mock_prisma_client.db.dheera_ai_teamtable.find_unique.return_value = None
...
async def test_get_team_db_check_does_not_call_new_team_if_exists(mock_new_team, monkeypatch):
    ...
    mock_db = AsyncMock()
    mock_prisma_client.db = mock_db
    mock_prisma_client.db.dheera_ai_teamtable.find_unique.return_value = MagicMock()
...
async def test_vector_store_access_check_early_returns(
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client", prisma_client), patch(
        "dheera_ai.vector_store_registry", vector_store_registry
    ):
...
async def test_vector_store_access_check_with_permissions():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma_client), patch(
        "dheera_ai.vector_store_registry", mock_vector_store_registry
    ):
        result = await vector_store_access_check(
            request_body=request_body,
            team_object=None,
...
def test_can_object_call_model_with_alias():
    ...
    from dheera_ai.proxy.auth.auth_checks import _can_object_call_model
...
def test_can_object_call_model_no_access_to_alias_or_underlying():
    ...
    """
    from dheera_ai import Router
    from dheera_ai.proxy._types import ProxyErrorTypes, ProxyException
    from dheera_ai.proxy.auth.auth_checks import _can_object_call_model
...
async def test_get_tag_objects_batch():
    ...
    """
    from dheera_ai.proxy._types import DheeraAI_TagTable
    from dheera_ai.proxy.auth.auth_checks import get_tag_objects_batch
    ...
    uncached_tag_1.dheera_ai_budget_table = None
    ...
    uncached_tag_2.dheera_ai_budget_table = None
    ...
    uncached_tag_3.dheera_ai_budget_table = None
    ...
    mock_prisma.db.dheera_ai_tagtable.find_many = AsyncMock(
    ...
    mock_prisma.db.dheera_ai_tagtable.find_many.assert_called_once()
    call_args = mock_prisma.db.dheera_ai_tagtable.find_many.call_args
...
async def test_reject_clientside_metadata_tags_enabled_with_tags():
    ...
    from dheera_ai.proxy.auth.auth_checks import common_checks
