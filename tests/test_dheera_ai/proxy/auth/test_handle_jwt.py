...
async def test_map_user_to_teams_user_already_in_team():
    ...
    # Setup test data
    user = DheeraAI_UserTable(user_id="test_user_1")
...
async def test_map_user_to_teams_add_new_user():
    ...
    # Setup test data
    user = DheeraAI_UserTable(user_id="test_user_1")
    team = DheeraAI_TeamTable(team_id="test_team_1", members_with_roles=[])
...
async def test_map_user_to_teams_handles_already_in_team_exception():
    ...
    with patch(
        ...
        with patch("dheera_ai.proxy.auth.handle_jwt.verbose_proxy_logger") as mock_logger:
...
async def test_map_user_to_teams_null_inputs():
    ...
    await JWTAuthManager.map_user_to_teams(
        user_object=None, team_object=DheeraAI_TeamTable(team_id="test_team_1")
    )

    # Test with null team
    await JWTAuthManager.map_user_to_teams(
        user_object=DheeraAI_UserTable(user_id="test_user_1"), team_object=None
    )
...
async def test_auth_builder_proxy_admin_user_role():
    ...
    user_object = DheeraAI_UserTable(
...
async def test_sync_user_role_and_teams():
    ...
    jwt_handler.update_environment(
        ...
        dheera_ai_jwtauth=DheeraAI_JWTAuth(
            jwt_dheera_ai_role_map=[
                JWTDheeraAIRoleMap(jwt_role="ADMIN", dheera_ai_role=LitellmUserRoles.PROXY_ADMIN)
            ],
    ...
    user = DheeraAI_UserTable(user_id="u1", user_role=LitellmUserRoles.INTERNAL_USER.value, teams=["team2"])

    prisma = AsyncMock()
    prisma.db.dheera_ai_usertable.update = AsyncMock()
    ...
    prisma.db.dheera_ai_usertable.update.assert_called_once()
...
@pytest.mark.asyncio
async def test_map_jwt_role_to_dheera_ai_role():
    """Test JWT role mapping to DheeraAI roles with various patterns"""
    from unittest.mock import MagicMock
    ...
    jwt_handler.update_environment(
        ...
        dheera_ai_jwtauth=DheeraAI_JWTAuth(
            jwt_dheera_ai_role_map=[
                ...
                JWTDheeraAIRoleMap(jwt_role="ADMIN", dheera_ai_role=LitellmUserRoles.PROXY_ADMIN),
                # Wildcard patterns
                JWTDheeraAIRoleMap(jwt_role="user_*", dheera_ai_role=LitellmUserRoles.INTERNAL_USER),
                JWTDheeraAIRoleMap(jwt_role="team_?", dheera_ai_role=LitellmUserRoles.TEAM),
                JWTDheeraAIRoleMap(jwt_role="dev_[123]", dheera_ai_role=LitellmUserRoles.INTERNAL_USER),
            ],
            roles_jwt_field="roles"
        ),
    )
    ...
    # Test exact match
    token = {"roles": ["ADMIN"]}
    result = jwt_handler.map_jwt_role_to_dheera_ai_role(token)
    ...
    jwt_handler.dheera_ai_jwtauth.jwt_dheera_ai_role_map = None
    token = {"roles": ["ADMIN"]}
    result = jwt_handler.map_jwt_role_to_dheera_ai_role(token)
    assert result is None

    # Test empty role mappings
    jwt_handler.dheera_ai_jwtauth.jwt_dheera_ai_role_map = []
    token = {"roles": ["ADMIN"]}
    result = jwt_handler.map_jwt_role_to_dheera_ai_role(token)
    assert result is None
...
async def test_nested_jwt_field_access():
    ...
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(user_id_jwt_field="user.sub")
    assert jwt_handler.get_user_id(nested_token, None) == "u123"
    
    # Test 1b: user_id_jwt_field with flat access (backward compatibility)
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(user_id_jwt_field="sub")
    assert jwt_handler.get_user_id(flat_token, None) == "u123"

    # Test 2: user_email_jwt_field with nested access
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(user_email_jwt_field="user.email")
    ...
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(user_email_jwt_field="email")
    assert jwt_handler.get_user_email(flat_token, None) == "user@example.com"

    # Test 3: team_ids_jwt_field with nested access
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(team_ids_jwt_field="groups")
    ...
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(end_user_id_jwt_field="customer.end_user_id")
    ...
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(end_user_id_jwt_field="end_user_id")
    assert jwt_handler.get_end_user_id(flat_token, None) == "customer123"

    # Test 7: team_id_jwt_field with nested access
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(team_id_jwt_field="tenant.team_id")
    ...
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(team_id_jwt_field="team_id")
    ...
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(roles_jwt_field="resource_access.my-client.roles")
...
async def test_metadata_prefix_handling_in_nested_fields():
    ...
    jwt_handler.dheera_ai_jwtauth = DheeraAI_JWTAuth(user_email_jwt_field="metadata.user.email")
...
async def test_find_team_with_model_access_model_group(monkeypatch):
        ...
        model_list=[
            {
                ...
                "dheera_ai_params": {"model": "gpt-4o-mini"},
    ...
    monkeypatch.setitem(sys.modules, "dheera_ai.proxy.proxy_server", proxy_server_module)

    team = DheeraAI_TeamTable(team_id="team-1", models=["test-group"])

    async def mock_get_team_object(*args, **kwargs):  # type: ignore
        return team

    monkeypatch.setattr(
        "dheera_ai.proxy.auth.handle_jwt.get_team_object", mock_get_team_object
    )
...
async def test_auth_builder_returns_team_membership_object():
    ...
    # Create mock objects
    from dheera_ai.proxy._types import DheeraAI_BudgetTable, DheeraAI_TeamMembership
    
    mock_team_membership = DheeraAI_TeamMembership(
        user_id=_user_id,
        team_id=_team_id,
    ...
    team_object = DheeraAI_TeamTable(team_id=_team_id)
...
def test_get_team_id_from_header():
    ...
    result = JWTAuthManager.get_team_id_from_header(
        request_headers={"x-dheera_ai-team-id": "team-1"},
    ...
    with pytest.raises(HTTPException) as exc_info:
        JWTAuthManager.get_team_id_from_header(
            request_headers={"x-dheera_ai-team-id": "invalid-team"},
            allowed_team_ids={"team-1", "team-2"},
        )
...
async def test_auth_builder_uses_team_from_header_e2e():
    """Test auth_builder e2e flow: selects team from x-dheera_ai-team-id header."""
    from dheera_ai.caching import DualCache
    from dheera_ai.proxy.utils import ProxyLogging
    ...
    team_object = DheeraAI_TeamTable(team_id="team-2")
    user_object = DheeraAI_UserTable(user_id="user-1", user_role=LitellmUserRoles.INTERNAL_USER)
    ...
    with patch.object(jwt_handler, "auth_jwt", new_callable=AsyncMock) as mock_auth_jwt, \
         ...
         patch("dheera_ai.proxy.auth.handle_jwt.get_team_object", new_callable=AsyncMock) as mock_get_team, \
        ...
        result = await JWTAuthManager.auth_builder(
            ...
            parent_otel_span=None,
            proxy_logging_obj=ProxyLogging(user_api_key_cache=user_api_key_cache),
            request_headers={"x-dheera_ai-team-id": "team-2"},
        )
