...
from dheera_ai.proxy.auth.login_utils import (
    LoginResult,
    authenticate_user,
    get_ui_credentials,
)
...
async def test_authenticate_user_admin_login_with_ui_credentials():
    ...
    with patch.dict(
        ...
        with patch(
            "dheera_ai.proxy.auth.login_utils.generate_key_helper_fn",
            ...
            with patch(
                "dheera_ai.proxy.auth.login_utils.user_update",
                new_callable=AsyncMock,
                return_value=None,
            ) as mock_user_update:
                with patch(
                    "dheera_ai.proxy.auth.login_utils.get_secret_bool",
                    return_value=False,
                ):
                    ...
                    assert result.user_id == DHEERA_AI_PROXY_ADMIN_NAME
...
async def test_authenticate_user_wrong_password():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.dheera_ai_usertable.find_first = AsyncMock(
