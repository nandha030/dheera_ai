...
async def test_organization_budget_exceeded_blocks_request():
    ...
    org_object = DheeraAI_OrganizationTable(
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma:
        with patch("dheera_ai.proxy.proxy_server.user_api_key_cache") as mock_cache:
            with patch("dheera_ai.proxy.auth.auth_checks.get_org_object", new_callable=AsyncMock) as mock_get_org:
...
async def test_organization_budget_fields_are_checked():
    ...
    org_over_budget = DheeraAI_OrganizationTable(
        organization_id="test-org",
        budget_id="budget-1",
        ...
        created_by="test",
        updated_by="test",
        dheera_ai_budget_table=DheeraAI_BudgetTable(max_budget=100.0),
    )
...
async def test_both_team_and_org_budget_enforced():
    ...
    team_over_budget = DheeraAI_TeamTable(
        team_id="team-over",
        max_budget=100.0,
        spend=150.0,  # Over budget
        models=["gpt-4"],
    ...
    team_under_budget = DheeraAI_TeamTable(
        team_id="team-under",
        organization_id="org-over",
        max_budget=50.0,
