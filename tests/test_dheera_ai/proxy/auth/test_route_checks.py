...
def test_virtual_key_allowed_routes_with_dheera_ai_routes_member_name_allowed():
    """Test that virtual key is allowed to call routes when allowed_routes contains DheeraAIRoutes member name"""

    # Create a UserAPIKeyAuth with allowed_routes containing a DheeraAIRoutes member name
    valid_token = UserAPIKeyAuth(
        user_id="test_user",
        allowed_routes=["openai_routes"],  # This is a member name in DheeraAIRoutes enum
    )

    # Test that a route from the openai_routes group is allowed
    result = RouteChecks.is_virtual_key_allowed_to_call_route(
        route="/chat/completions",  # This is in DheeraAIRoutes.openai_routes.value
        valid_token=valid_token,
    )
...
def test_virtual_key_allowed_routes_with_dheera_ai_routes_member_name_denied():
    """Test that virtual key is denied when route is not in the allowed DheeraAIRoutes group"""

    # Create a UserAPIKeyAuth with allowed_routes containing a DheeraAIRoutes member name
    valid_token = UserAPIKeyAuth(
        user_id="test_user",
        allowed_routes=["info_routes"],  # This is a member name in DheeraAIRoutes enum
    )

    # Test that a route NOT in the info_routes group raises an HTTPException
    with pytest.raises(HTTPException) as exc_info:
        RouteChecks.is_virtual_key_allowed_to_call_route(
            route="/chat/completions",  # This is NOT in DheeraAIRoutes.info_routes.value
            valid_token=valid_token,
        )
...
def test_virtual_key_llm_api_route_includes_passthrough_prefix(route):
    ...
    Relevant issue: https://github.com/BerriAI/dheera_ai/issues/14017
    """
...
def test_virtual_key_allowed_routes_with_multiple_dheera_ai_routes_member_names():
    """Test that virtual key works with multiple DheeraAIRoutes member names in allowed_routes"""

    # Create a UserAPIKeyAuth with multiple DheeraAIRoutes member names
    valid_token = UserAPIKeyAuth(
...
def test_virtual_key_allowed_routes_with_mixed_member_names_and_explicit_routes():
    """Test that virtual key works with both DheeraAIRoutes member names and explicit routes"""
...
def test_virtual_key_allowed_routes_with_no_member_names_only_explicit():
    ...
    # Create a UserAPIKeyAuth with only explicit routes (no DheeraAIRoutes member names)
    valid_token = UserAPIKeyAuth(
        user_id="test_user",
...
def test_virtual_key_llm_api_routes_allows_registered_pass_through_endpoints():
    ...
    with patch(
        "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints._registered_pass_through_routes",
        mock_registered_routes,
    ):
...
def test_videos_route_accessible_to_internal_users():
    ...
    https://github.com/BerriAI/dheera_ai/issues/16470
...
def test_route_in_additional_public_routes_wildcard_match():
    ...
    """
    from dheera_ai.proxy.auth.auth_utils import route_in_additonal_public_routes

    with patch("dheera_ai.proxy.proxy_server.general_settings", {"public_routes": ["/api/*"]}), \
         patch("dheera_ai.proxy.proxy_server.premium_user", True):
...
def test_route_in_additional_public_routes_exact_match():
    ...
    with patch("dheera_ai.proxy.proxy_server.general_settings", {"public_routes": ["/health", "/status"]}), \
         patch("dheera_ai.proxy.proxy_server.premium_user", True):
