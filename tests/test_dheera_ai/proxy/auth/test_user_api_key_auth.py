...
from dheera_ai.proxy.auth.user_api_key_auth import get_api_key
...
def test_get_api_key():
    ...
    assert get_api_key(
        custom_dheera_ai_key_header=None,
...
@pytest.mark.parametrize(
    "custom_dheera_ai_key_header, api_key, passed_in_key",
    [
...
def test_get_api_key_with_custom_dheera_ai_key_header(
    custom_dheera_ai_key_header, api_key, passed_in_key
):
    assert get_api_key(
...
def test_team_metadata_with_tags_flows_through_jwt_auth():
    ...
    team_metadata, causing team-level tags to be missing in dheera_ai_pre_call_utils.py
    """
...
async def test_proxy_admin_expired_key_from_cache():
    ...
    from dheera_ai.proxy.auth.user_api_key_auth import _user_api_key_auth_builder
    ...
    with patch(
        "dheera_ai.proxy.auth.user_api_key_auth.get_key_object",
        new_callable=AsyncMock,
    ) as mock_get_key_object, \
         patch("dheera_ai.proxy.auth.user_api_key_auth._delete_cache_key_object", new_callable=AsyncMock) as mock_delete_cache:
        ...
        setattr(dheera_ai.proxy.proxy_server, "prisma_client", mock_prisma_client)
        setattr(dheera_ai.proxy.proxy_server, "user_api_key_cache", mock_cache)
        setattr(dheera_ai.proxy.proxy_server, "proxy_logging_obj", mock_proxy_logging_obj)
        setattr(dheera_ai.proxy.proxy_server, "master_key", "sk-master-key")
        setattr(dheera_ai.proxy.proxy_server, "general_settings", {})
        setattr(dheera_ai.proxy.proxy_server, "llm_model_list", [])
        setattr(dheera_ai.proxy.proxy_server, "llm_router", None)
        setattr(dheera_ai.proxy.proxy_server, "open_telemetry_logger", None)
        setattr(dheera_ai.proxy.proxy_server, "model_max_budget_limiter", MagicMock())
        setattr(dheera_ai.proxy.proxy_server, "user_custom_auth", None)
        setattr(dheera_ai.proxy.proxy_server, "jwt_handler", None)
        setattr(dheera_ai.proxy.proxy_server, "dheera_ai_proxy_admin_name", "admin")
