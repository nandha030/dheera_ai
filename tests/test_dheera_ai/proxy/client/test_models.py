...
from dheera_ai.proxy.client import Client, ModelsManagementClient
from dheera_ai.proxy.client.exceptions import NotFoundError, UnauthorizedError
...
def test_new_without_model_info(client):
    ...
    assert request.json == {"model_name": model_name, "dheera_ai_params": model_params}
...
def test_update_without_model_info(client):
    ...
    # Check request body doesn't include model_info
    assert request.json == {"id": model_id, "dheera_ai_params": model_params}
