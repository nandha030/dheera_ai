...
from dheera_ai.proxy.common_utils.callback_utils import process_callback
...
def test_get_remaining_tokens_and_requests_from_request_data():
    ...
    casedata = {
        "metadata": {
            ...
            f"dheera_ai-key-remaining-requests-{model_group}": 100,
            f"dheera_ai-key-remaining-tokens-{model_group}": 200,
        }
    }
    ...
    assert headers == {
        f"x-dheera_ai-key-remaining-requests-{expected_name}": 100,
        f"x-dheera_ai-key-remaining-tokens-{expected_name}": 200,
    }


@patch(
    "dheera_ai.proxy.common_utils.callback_utils.CustomLogger.get_callback_env_vars",
