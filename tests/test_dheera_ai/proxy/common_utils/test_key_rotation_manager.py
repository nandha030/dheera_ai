...
from dheera_ai.proxy.common_utils.key_rotation_manager import KeyRotationManager
...
class TestKeyRotationManager:
    ...
    async def test_should_rotate_key_logic(self):
        ...
        key_no_rotation_time = DheeraAI_VerificationToken(
            token="test-token-1",
            auto_rotate=True,
        ...
        key_future_rotation = DheeraAI_VerificationToken(
            token="test-token-2",
            auto_rotate=True,
            rotation_interval="30s",
        ...
        key_past_rotation = DheeraAI_VerificationToken(
            token="test-token-3",
            auto_rotate=True,
            rotation_interval="30s",
            key_rotation_at=now - timedelta(seconds=10),
        ...
        key_exact_rotation = DheeraAI_VerificationToken(
            token="test-token-4",
            auto_rotate=True,
            rotation_interval="30s",
            key_rotation_at=now,
        ...
        key_no_interval = DheeraAI_VerificationToken(
            token="test-token-5",
            auto_rotate=True,
    ...
    async def test_find_keys_needing_rotation(self):
        ...
        mock_prisma_client.db.dheera_ai_verificationtoken.find_many.return_value = mock_keys
        
        # Mock datetime.now to return our fixed timestamp
        from unittest.mock import patch
        with patch('dheera_ai.proxy.common_utils.key_rotation_manager.datetime') as mock_datetime:
    ...
    async def test_rotate_key_updates_database(self):
        ...
        # Mock key to rotate
        key_to_rotate = DheeraAI_VerificationToken(
            token="old-token",
            auto_rotate=True,
            rotation_interval="30s",
        ...
        from unittest.mock import patch
        with patch('dheera_ai.proxy.common_utils.key_rotation_manager.regenerate_key_fn', return_value=mock_response):
            with patch('dheera_ai.proxy.common_utils.key_rotation_manager.KeyManagementEventHooks.async_key_rotated_hook'):
                # Execute
                await manager._rotate_key(key_to_rotate)
        
        # Verify database update was called with correct data
        mock_prisma_client.db.dheera_ai_verificationtoken.update.assert_called_once()
        
        call_args = mock_prisma_client.db.dheera_ai_verificationtoken.update.call_args
