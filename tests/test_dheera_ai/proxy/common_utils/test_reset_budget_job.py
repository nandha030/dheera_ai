...
class MockDheeraAITeamMembership:
    async def update_many(
        self, where: Dict[str, Any], data: Dict[str, Any]
    ) -> Dict[str, Any]:
        # Mock the update_many method for dheera_ai_teammembership
        return {"count": 1}


class MockDB:
    def __init__(self):
        self.dheera_ai_teammembership = MockDheeraAITeamMembership()
    ...
    async def get_data(self, table_name, query_type, **kwargs):
        ...
        if (
            ...
            return [
                item
                for item in data
                if hasattr(item, "dheera_ai_budget_table")
                and hasattr(item.dheera_ai_budget_table, "budget_id")
                and item.dheera_ai_budget_table.budget_id in budget_id_list
            ]
...
def test_reset_budget_for_key(reset_budget_job, mock_prisma_client):
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_dheera_ai_keys())
...
def test_reset_budget_for_user(reset_budget_job, mock_prisma_client):
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_dheera_ai_users())
...
def test_reset_budget_for_team(reset_budget_job, mock_prisma_client):
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_dheera_ai_teams())
...
def test_reset_budget_for_enduser(reset_budget_job, mock_prisma_client):
    ...
    test_enduser = type(
        ...
        {
            "spend": 20.0,
            "dheera_ai_budget_table": test_budget,
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_dheera_ai_budget_table())
