...
async def test_daily_spend_tracking_with_disabled_spend_logs():
    ...
    ), patch("dheera_ai.proxy.proxy_server.user_api_key_cache", MagicMock()), patch(
        "dheera_ai.proxy.proxy_server.dheera_ai_proxy_budget_name", "test-budget"
    ):
        # Test data
        test_data = {
            "token": "test-token",
...
async def test_update_daily_spend_with_null_entity_id():
    ...
    mock_batcher.dheera_ai_dailyuserspend = mock_table
...
async def test_update_daily_spend_tag_with_request_id():
    ...
    mock_table = MagicMock()
    mock_prisma_client.db.batch_.return_value.__aenter__.return_value = mock_batcher
    mock_batcher.dheera_ai_dailytagspend = mock_table
...
async def test_update_tag_db_with_valid_tags():
    ...
    """
    from dheera_ai.proxy._types import Litellm_EntityType, SpendUpdateQueueItem
...
async def test_add_spend_log_transaction_to_daily_tag_transaction_with_request_id():
    ...
    This tests that request_id is included in the DailyTagSpendTransaction for the DheeraAI_DailyTagSpend table.
    """
    writer = DBSpendUpdateWriter()
    mock_prisma = MagicMock()
