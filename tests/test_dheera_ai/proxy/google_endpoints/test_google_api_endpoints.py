...
def test_google_generate_content_with_cost_tracking_metadata():
    ...
    with patch("dheera_ai.proxy.proxy_server.llm_router") as mock_router, \
         ...
         patch("dheera_ai.proxy.proxy_server.proxy_config") as mock_proxy_config, \
         patch("dheera_ai.proxy.proxy_server.version", "1.0.0"), \
         patch("dheera_ai.proxy.dheera_ai_pre_call_utils.add_dheera_ai_data_to_request") as mock_add_data:
        
        mock_router.agenerate_content = AsyncMock(return_value={"test": "response"})
        
        # Mock add_dheera_ai_data_to_request to return data with metadata
        async def mock_add_dheera_ai_data(data, request, user_api_key_dict, proxy_config, general_settings, version):
        ...
        mock_add_data.side_effect = mock_add_dheera_ai_data
        ...
        # Verify that add_dheera_ai_data_to_request was called
        ...
        # Verify that dheera_ai_metadata exists and contains user information
        assert "dheera_ai_metadata" in called_data
        assert called_data["dheera_ai_metadata"]["user_api_key_user_id"] == "test-user-id"
        assert called_data["dheera_ai_metadata"]["user_api_key_team_id"] == "test-team-id"
...
def test_google_generate_content_with_system_instruction():
    ...
    with patch("dheera_ai.proxy.proxy_server.llm_router") as mock_router, \
        ...
        # Mock add_dheera_ai_data_to_request to pass through data unchanged
        async def mock_add_dheera_ai_data(data, request, user_api_key_dict, proxy_config, general_settings, version):
            return data
