...
from dheera_ai.types.llms.openai import OpenAIModerationResponse, OpenAIModerationResult
...
@pytest.mark.asyncio
async def test_openai_moderation_guardrail_adds_to_dheera_ai_callbacks():
    """Test that OpenAI moderation guardrail adds itself to dheera_ai callbacks during initialization"""
    import dheera_ai
    from dheera_ai.proxy.guardrails.guardrail_hooks.openai import (
    ...
    original_callbacks = dheera_ai.callbacks.copy()
    dheera_ai.logging_callback_manager._reset_all_callbacks()
    
    try:
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            guardrail_dheera_ai_params = LitellmParams(
            ...
            guardrail = openai_initialize_guardrail(
                dheera_ai_params=guardrail_dheera_ai_params,
                guardrail=Guardrail(
                    guardrail_name="test-openai-moderation",
                    dheera_ai_params=guardrail_dheera_ai_params
                )
            ...
            # Check that the guardrail was added to dheera_ai callbacks
            assert guardrail in dheera_ai.callbacks
            assert len(dheera_ai.callbacks) == 1
            
            # Verify it's the correct guardrail
            callback = dheera_ai.callbacks[0]
...
async def test_openai_moderation_guardrail_streaming_safe_content():
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
        ...
        with patch.object(guardrail, 'async_make_request', return_value=mock_response), \
             patch('dheera_ai.main.stream_chunk_builder', return_value=mock_model_response), \
             patch('dheera_ai.llms.base_llm.base_model_iterator.MockResponseIterator') as mock_iterator:
