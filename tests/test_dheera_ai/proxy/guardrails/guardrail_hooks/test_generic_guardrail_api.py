...
from dheera_ai.proxy.guardrails.guardrail_hooks.generic_guardrail_api import (
    GenericGuardrailAPI,
)
...
class TestGenericGuardrailAPIConfiguration:
    ...
    def test_init_with_config(self):
        ...
        assert (
            guardrail.api_base
            == "https://api.test.guardrail.com/beta/dheera_ai_basic_guardrail_api"
        )
    ...
    def test_init_with_env_vars(self):
        ...
        with patch.dict(
            ...
            assert (
                guardrail.api_base
                == "https://env.api.guardrail.com/beta/dheera_ai_basic_guardrail_api"
            )
    ...
    def test_api_base_appends_endpoint(self):
        ...
        assert (
            guardrail.api_base
            == "https://api.test.guardrail.com/v1/beta/dheera_ai_basic_guardrail_api"
        )

    def test_api_base_not_duplicated(self):
        """Test that endpoint path is not duplicated if already present"""
        guardrail = GenericGuardrailAPI(
            api_base="https://api.test.guardrail.com/beta/dheera_ai_basic_guardrail_api",
        )
        assert (
            guardrail.api_base
            == "https://api.test.guardrail.com/beta/dheera_ai_basic_guardrail_api"
...
class TestMetadataExtraction:
    ...
    async def test_extract_metadata_from_output_response(
        ...
        """Test extracting metadata from output response (dheera_ai_metadata field)"""
        ...
        for key, value in user_dict.items():
            if value is not None and not key.startswith("_"):
                if key.startswith("user_api_key_"):
                    ...
                    dheera_ai_metadata[f"user_api_key_{key}"] = value
        ...
        with patch.object(
            ...
            # Verify metadata was extracted from request_data["dheera_ai_metadata"]
