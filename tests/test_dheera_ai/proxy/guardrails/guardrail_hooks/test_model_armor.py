...
from dheera_ai.proxy.guardrails.guardrail_hooks.model_armor import ModelArmorGuardrail
...
async def test_model_armor_post_call_hook_sanitization():
    ...
    mock_llm_response = dheera_ai.ModelResponse()
...
async def test_model_armor_streaming_response():
    ...
    async def mock_stream():
        chunks = [
            dheera_ai.ModelResponseStream(
                choices=[
                    dheera_ai.types.utils.StreamingChoices(
                        delta=dheera_ai.types.utils.Delta(content="Sensitive ")
                    )
                ]
            ),
            dheera_ai.ModelResponseStream(
                choices=[
                    dheera_ai.types.utils.StreamingChoices(
                        delta=dheera_ai.types.utils.Delta(content="information")
                    )
                ]
            ),
        ]
...
def test_model_armor_ui_friendly_name():
    ...
    from dheera_ai.types.proxy.guardrails.guardrail_hooks.model_armor import (
...
async def test_model_armor_missing_sanitized_text():
    ...
    mock_llm_response.choices = [
        dheera_ai.Choices(
            message=dheera_ai.Message(content="Original content")
        )
    ]
...
async def test_async_moderation_hook_with_sanitization():
    ...
    from dheera_ai.dheera_ai_core_utils.prompt_templates.common_utils import get_last_user_message
...
async def test_async_moderation_hook_should_not_run():
    ...
    request_data = {
        ...
        "metadata": {"guardrails": ["some-other-guardrail"]}  # Different guardrail name
    }
