...
from dheera_ai.proxy.guardrails.guardrail_hooks.noma import (
    NomaGuardrail,
    initialize_guardrail,
)
from dheera_ai.proxy.guardrails.guardrail_hooks.noma.noma import NomaBlockedMessage
...
from dheera_ai.types.utils import Choices, Message
...
class TestNomaGuardrailConfiguration:
    ...
    def test_initialize_guardrail_function(self):
        ...
        with patch("dheera_ai.logging_callback_manager.add_dheera_ai_callback") as mock_add:
            result = initialize_guardrail(dheera_ai_params, guardrail)
...
class TestNomaApplicationIdResolution:
    ...
    @pytest.mark.asyncio
    async def test_application_id_defaults_to_dheera_ai(
        self, noma_guardrail, mock_user_api_key_dict, mock_request_data
    ):
        request_data = self._clone_request_data(mock_request_data)
        user_auth = self._clone_user_auth(mock_user_api_key_dict)
        ...
        assert application_id == "dheera_ai"
...
class TestBackgroundProcessing:
    ...
    async def test_handle_verdict_background_blocked(self, monitor_mode_guardrail):
        ...
        with patch("dheera_ai._logging.verbose_proxy_logger.warning") as mock_warning:
    ...
    async def test_handle_verdict_background_allowed(self, monitor_mode_guardrail):
        ...
        with patch("dheera_ai._logging.verbose_proxy_logger.info") as mock_info:
            await monitor_mode_guardrail._handle_verdict_background(
...
class TestNomaImageProcessing:
    ...
    def test_extract_user_message_with_image_url(self):
        ...
        from dheera_ai.completion_extras.dheera_ai_responses_transformation.transformation import (
            DheeraAIResponsesTransformationHandler,
        )

        handler = DheeraAIResponsesTransformationHandler()
    ...
    async def test_image_content_blocked(
        ...
        from dheera_ai.proxy.guardrails.guardrail_hooks.noma.noma import (
            NomaBlockedMessage,
        )
        ...
        with patch.dict(
            ...
            custom_loggers = (
                dheera_ai.logging_callback_manager.get_custom_loggers_for_type(
                    callback_type=dheera_ai.integrations.custom_guardrail.CustomGuardrail
                )
            )
