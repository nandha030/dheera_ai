...
This test file follows DheeraAI's testing patterns and covers:
...
def mock_panw_client():
    ...
    with patch(
        "dheera_ai.proxy.guardrails.guardrail_hooks.panw_prisma_airs.panw_prisma_airs.get_async_httpx_client"
    ) as mock_client:
...
class TestPanwAirsInitialization:
    ...
    def test_initialize_guardrail_function(self):
        ...
        from dheera_ai.types.guardrails import LitellmParams
        ...
        with patch("dheera_ai.logging_callback_manager.add_dheera_ai_callback"):
            handler = initialize_guardrail(dheera_ai_params, guardrail_config)
...
class TestPanwAirsAdvancedFeatures:
    ...
    async def test_streaming_hook_adds_guardrail_header(self):
        ...
        from dheera_ai.types.utils import StreamingChoices, Delta
        ...
        with patch.object(
            ...
            with patch(
                "dheera_ai.proxy.common_utils.callback_utils.add_guardrail_to_applied_guardrails_header"
            ) as mock_header:
                mock_api.return_value = mock_scan_result
...
class TestPanwAirsDeduplication:
    ...
    async def test_duplicate_pre_call_scan_prevented(self):
        ...
        data = {
            "model": "gpt-3.5-turbo",
            "messages": [{"role": "user", "content": "Test"}],
            "dheera_ai_call_id": "test-call-123",
        }
    ...
    async def test_duplicate_post_call_scan_prevented(self):
        ...
        data = {
            "model": "gpt-3.5-turbo",
            "dheera_ai_call_id": "test-call-456",
        }
        response = ModelResponse(
            id="test_id",
            choices=[
    ...
    async def test_duplicate_streaming_scan_prevented(self):
        ...
        request_data = {
            "model": "gpt-3.5-turbo",
            "dheera_ai_call_id": "test-call-789",
            "messages": [{"role": "user", "content": "test"}],
        }
...
class TestPanwAirsSessionTracking:
    """Test session tracking with dheera_ai_trace_id."""

    @pytest.mark.asyncio
    async def test_dheera_ai_trace_id_used_as_transaction_id(self):
        """Test that dheera_ai_trace_id is used as PANW transaction ID."""
        handler = PanwPrismaAirsHandler(
            guardrail_name="test_panw_airs",
            api_key="test_api_key",
            profile_name="test_profile",
    ...
    async def test_fallback_to_call_id_when_trace_id_missing(self):
        """Test fallback to call_id when dheera_ai_trace_id is missing."""
        handler = PanwPrismaAirsHandler(
            guardrail_name="test_panw_airs",
            api_key="test_api_key",
            profile_name="test_profile",
    ...
    async def test_trace_id_extraction_from_request_data(self):
        """Test that dheera_ai_trace_id is extracted from request data."""
        handler = PanwPrismaAirsHandler(
            guardrail_name="test_panw_airs",
            api_key="test_api_key",
            profile_name="test_profile",
        ...
        assert "dheera_ai_trace_id" in metadata
        assert metadata["dheera_ai_trace_id"] == trace_id
...
class TestPanwAirsAppUserMetadata:
    ...
    async def test_app_user_priority_chain(self):
        """Test that app_user follows priority: app_user > user > dheera_ai_user."""
        handler = PanwPrismaAirsHandler(
            guardrail_name="test_panw_airs",
            api_key="test_api_key",
            profile_name="test_profile",
        ...
        test_cases = [
            ...
            ({}, "dheera_ai_user", "dheera_ai_user is default"),
        ]
