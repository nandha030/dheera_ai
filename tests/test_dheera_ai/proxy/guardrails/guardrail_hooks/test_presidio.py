...
from dheera_ai.types.guardrails import LitellmParams, PiiAction, PiiEntityType
...
async def test_presidio_filter_scope_initializer(monkeypatch):
    ...
    class DummyGuardrail:
        ...
        def update_in_memory_dheera_ai_params(self, dheera_ai_params):
            pass
    ...
    class DummyManager:
        ...
        def add_dheera_ai_callback(self, cb):
            self.added.append(cb)

    mgr = DummyManager()
    monkeypatch.setattr(dheera_ai, "logging_callback_manager", mgr, raising=False)
    import dheera_ai.proxy.guardrails.guardrail_initializers as gi
    import dheera_ai.proxy.guardrails.guardrail_hooks.presidio as presidio_mod
    monkeypatch.setattr(
    ...
    # input-only
    created.clear()
    from dheera_ai.proxy.guardrails.guardrail_initializers import initialize_presidio
...
def test_update_in_memory_applies_score_thresholds():
    """
    update_in_memory_dheera_ai_params should refresh score thresholds.
    """
    guardrail = _OPTIONAL_PresidioPIIMasking(mock_testing=True)
    ...
    guardrail.update_in_memory_dheera_ai_params(params)
