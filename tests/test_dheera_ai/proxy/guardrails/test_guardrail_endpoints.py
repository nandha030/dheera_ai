...
from dheera_ai.proxy.guardrails.guardrail_endpoints import (
...
MOCK_GUARDRAIL = Guardrail(
    ...
    dheera_ai_params=LitellmParams(**MOCK_CONFIG_GUARDRAIL["dheera_ai_params"]),
...
MOCK_PATCH_REQUEST = PatchGuardrailRequest(
    ...
    dheera_ai_params={"guardrail": "updated.guardrail", "mode": "post_call"},
...
def mock_prisma_client(mocker):
    ...
    mock_client.db.dheera_ai_guardrailstable = mocker.Mock()
    mock_client.db.dheera_ai_guardrailstable.find_many = AsyncMock(
        return_value=[MOCK_DB_GUARDRAIL]
    )
    mock_client.db.dheera_ai_guardrailstable.find_unique = AsyncMock(
        return_value=MOCK_DB_GUARDRAIL
    )
    return mock_client
...
async def test_list_guardrails_v2_with_db_and_config(
    ...
    mocker.patch(
        "dheera_ai.proxy.guardrails.guardrail_registry.IN_MEMORY_GUARDRAIL_HANDLER",
    ...
    assert isinstance(db_guardrail.dheera_ai_params, BaseLitellmParams)
    ...
    assert isinstance(config_guardrail.dheera_ai_params, BaseLitellmParams)
...
async def test_get_guardrail_info_from_db(mocker, mock_prisma_client):
    ...
    assert isinstance(response.dheera_ai_params, BaseLitellmParams)
...
async def test_get_guardrail_info_from_config(
    ...
    mock_prisma_client.db.dheera_ai_guardrailstable.find_unique = AsyncMock(
        return_value=None
    )
...
def test_get_provider_specific_params():
    ...
    from dheera_ai.proxy.guardrails.guardrail_endpoints import _get_fields_from_model
    from dheera_ai.proxy.guardrails.guardrail_hooks.azure import (
...
async def test_bedrock_guardrail_prepare_request_without_api_key():
    ...
    with patch("dheera_ai.proxy.guardrails.guardrail_hooks.bedrock_guardrails.get_secret_str") as mock_get_secret, \
...
async def test_create_guardrail_endpoint(
    ...
    if scenario == "success_with_sync":
        ...
        mocker.patch("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma_client)
        mocker.patch("dheera_ai.proxy.guardrails.guardrail_endpoints.GUARDRAIL_REGISTRY", mock_guardrail_registry)
        mocker.patch("dheera_ai.proxy.guardrails.guardrail_registry.IN_MEMORY_GUARDRAIL_HANDLER", mock_in_memory_handler)
        
    elif scenario == "success_sync_fails":
        mock_prisma_client = mocker.Mock()
        mock_in_memory_handler.initialize_guardrail.side_effect = Exception("Sync failed")
        mock_logger = mocker.patch("dheera_ai.proxy.guardrails.guardrail_endpoints.verbose_proxy_logger")
        ...
        mocker.patch("dheera_ai.proxy.proxy_server.prisma_client", None)
...
async def test_apply_guardrail_not_found(mocker):
    ...
    mocker.patch("dheera_ai.proxy.guardrails.guardrail_endpoints.GUARDRAIL_REGISTRY", mock_registry)
...
async def test_get_guardrail_info_endpoint_config_guardrail(mocker):
    ...
    """
    from dheera_ai.proxy.guardrails.guardrail_endpoints import get_guardrail_info

    # Mock prisma_client to not be None (patch at the source where it's imported from)
    mock_prisma = mocker.Mock()
    mocker.patch("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma)
    ...
    mocker.patch(
        "dheera_ai.dheera_ai_core_utils.dheera_ai_logging._get_masked_values",
