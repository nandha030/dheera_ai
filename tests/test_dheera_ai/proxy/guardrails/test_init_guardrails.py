...
from dheera_ai.proxy.guardrails.guardrail_registry import InMemoryGuardrailHandler
...
def test_initialize_presidio_guardrail():
    ...
    assert (
        result["dheera_ai_params"].guardrail
        == SupportedGuardrailIntegrations.PRESIDIO.value
    )
    assert result["dheera_ai_params"].mode == "pre_call"
