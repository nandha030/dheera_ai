"""
Pillar Security Guardrail Tests for DheeraAI

Tests for the Pillar Security guardrail integration using pytest fixtures
and following DheeraAI testing patterns and best practices.
"""
...
from dheera_ai.proxy.common_utils.callback_utils import get_logging_caching_headers
from dheera_ai.proxy.guardrails.guardrail_hooks.pillar import (
    PillarGuardrail,
    PillarGuardrailAPIError,
    PillarGuardrailMissingSecrets,
)
from dheera_ai.proxy.guardrails.guardrail_hooks.pillar.pillar import (
...
def setup_and_teardown():
    """
    Standard DheeraAI fixture that reloads dheera_ai before every function
    to speed up testing by removing callbacks being chained.
    """
    import importlib
    import asyncio

    # Reload dheera_ai to ensure clean state
...
def test_pillar_guard_config_missing_api_key(pillar_guardrail_config, monkeypatch):
    ...
    pillar_guardrail_config["dheera_ai_params"].pop("api_key", None)
...
@pytest.mark.asyncio
async def test_dheera_ai_context_headers_automatically_added(
    sample_request_data,
    user_api_key_dict_with_context,
    dual_cache,
    pillar_clean_response,
):
    """Test that DheeraAI context headers are automatically added (always enabled)."""
    guardrail = PillarGuardrail(
    ...
    # Verify DheeraAI context headers are present
    assert "X-DheeraAI-Key-Name" in captured_headers
    assert captured_headers["X-DheeraAI-Key-Name"] == "production-api-key"
    assert "X-DheeraAI-Key-Alias" in captured_headers
    assert captured_headers["X-DheeraAI-Key-Alias"] == "prod-key"
    assert "X-DheeraAI-User-Id" in captured_headers
    assert captured_headers["X-DheeraAI-User-Id"] == "user-123"
    assert "X-DheeraAI-User-Email" in captured_headers
    assert captured_headers["X-DheeraAI-User-Email"] == "test@example.com"
    assert "X-DheeraAI-Team-Id" in captured_headers
    assert captured_headers["X-DheeraAI-Team-Id"] == "team-456"
    assert "X-DheeraAI-Team-Name" in captured_headers
    assert captured_headers["X-DheeraAI-Team-Name"] == "engineering-team"
    assert "X-DheeraAI-Org-Id" in captured_headers
    assert captured_headers["X-DheeraAI-Org-Id"] == "org-789"
    
    # Metadata is NOT sent (may contain sensitive information)
    assert "X-DheeraAI-Metadata" not in captured_headers


@pytest.mark.asyncio
async def test_dheera_ai_context_with_partial_fields(
    sample_request_data,
    dual_cache,
    pillar_clean_response,
):
    """Test that partial DheeraAI context (only some fields present) is handled correctly."""
    ...
    guardrail = PillarGuardrail(
        ...
        api_key="test-pillar-key",
        api_base="https://api.pillar.security",
        pass_dheera_ai_key_header=True,
    )
    ...
    assert "X-DheeraAI-User-Id" in captured_headers
    assert captured_headers["X-DheeraAI-User-Id"] == "user-only"
    assert "X-DheeraAI-Team-Id" in captured_headers
    assert captured_headers["X-DheeraAI-Team-Id"] == "team-only"

    # Verify empty fields are not present
    assert "X-DheeraAI-Key-Name" not in captured_headers
    assert "X-DheeraAI-User-Email" not in captured_headers
...
def test_metadata_inclusion(pillar_guardrail_instance):
    ...
    assert payload["metadata"]["source"] == "dheera_ai"
