...
from tests.test_dheera_ai.proxy.conftest import create_proxy_test_client
...
async def test_health_services_endpoint_sqs(status, error_message):
    ...
    """
    with patch("dheera_ai.integrations.sqs.SQSLogger") as MockSQSLogger:
...
def test_health_readiness(proxy_client):
    ...
    assert "dheera_ai_version" in response_data, "Response should contain 'dheera_ai_version' field"
    ...
    print(f"DheeraAI Version: {response_data.get('dheera_ai_version', 'unknown')}")
