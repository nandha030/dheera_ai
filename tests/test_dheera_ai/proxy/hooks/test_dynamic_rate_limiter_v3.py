...
import dheera_ai
from dheera_ai import DualCache, Router
...
async def test_priority_weight_allocation():
    ...
    os.environ["DHEERA_AI_LICENSE"] = "test-license-key"

    # Set up priority reservations
    dheera_ai.priority_reservation = {"high": 0.9, "low": 0.1}
...
async def test_concurrent_pre_call_hooks_stress():
    ...
    dheera_ai.priority_reservation = {"premium": 0.8, "standard": 0.2}
    ...
    async def mock_get_cache(key, dheera_ai_parent_otel_span=None, local_only=False):
...
async def test_fake_calls_case_1_no_rate_limiting_at_capacity():
    ...
    # Set up priority reservations
    dheera_ai.priority_reservation = {"key_a": 0.75, "key_b": 0.25}
...
async def test_fake_calls_case_3_spillover_capacity_default_keys():
    ...
    dheera_ai.priority_reservation = {"key_a": 0.75}
    dheera_ai.priority_reservation_settings.default_priority = 0.25
...
async def test_fake_calls_case_4_over_allocated_with_normalization():
    ...
    dheera_ai.priority_reservation = {"key_a": 0.60, "key_b": 0.80}
...
async def test_fake_calls_case_5_default_value_priority_reservation():
    ...
    dheera_ai.priority_reservation = {"key_a": 0.50, "key_b": 0.20, "key_c": 0.05}
    dheera_ai.priority_reservation_settings.default_priority = 0.05
...
async def test_default_priority_shared_pool():
    ...
    dheera_ai.priority_reservation = {"prod": 0.75}
    dheera_ai.priority_reservation_settings.default_priority = 0.25
...
async def test_async_log_success_event_increments_by_actual_tokens():
    ...
    os.environ["DHEERA_AI_LICENSE"] = "test-license-key"
    dheera_ai.priority_reservation = {"dev": 0.1, "prod": 0.9}
    ...
    with patch(
        "dheera_ai.proxy.common_utils.callback_utils.get_model_group_from_dheera_ai_kwargs",
        return_value=model,
    ):
...
async def test_saturation_check_cache_ttl_configuration():
    ...
    original_ttl = dheera_ai.priority_reservation_settings.saturation_check_cache_ttl
    dheera_ai.priority_reservation_settings.saturation_check_cache_ttl = 5
    ...
    try:
        ...
        async def mock_get_cache(key, dheera_ai_parent_otel_span=None, local_only=False, **kwargs):
        ...
        dheera_ai.priority_reservation_settings.saturation_check_cache_ttl = 30
        ...
        dheera_ai.priority_reservation_settings.saturation_check_cache_ttl = original_ttl
...
async def test_async_log_success_event_uses_team_priority_from_auth_metadata():
    ...
    os.environ["DHEERA_AI_LICENSE"] = "test-license-key"
    dheera_ai.priority_reservation = {"team_priority": 0.8, "default": 0.2}
