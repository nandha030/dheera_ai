...
from dheera_ai.proxy.utils import InternalUsageCache, ProxyLogging, hash_token
...
async def test_sliding_window_rate_limit_v3(monkeypatch, time_controller):
    ...
    """
    monkeypatch.setenv("DHEERA_AI_RATE_LIMIT_WINDOW_SIZE", "2")
    _api_key = "sk-12345"
...
async def test_normal_router_call_tpm_v3(monkeypatch, rate_limit_object, time_controller):
    ...
    parallel_request_handler.batch_rate_limiter_script = mock_batch_rate_limiter
    monkeypatch.setattr(dheera_ai, "callbacks", [parallel_request_handler])
...
async def test_token_rate_limit_type_respected_v3(monkeypatch, token_rate_limit_type):
    ...
    monkeypatch.setenv("DHEERA_AI_RATE_LIMIT_WINDOW_SIZE", "60")
...
async def test_async_log_success_event_with_dict_usage(monkeypatch, token_rate_limit_type, expected_field):
    ...
    from dheera_ai.types.utils import BaseDheeraAIOpenAIResponseObject
    
    # Use spec to make isinstance checks work correctly with MagicMock
    mock_response = MagicMock(spec=BaseDheeraAIOpenAIResponseObject)
    mock_response.usage = {
        "prompt_tokens": 25,
...
async def test_async_log_success_event_with_dict_usage_missing_fields(monkeypatch):
    ...
    from dheera_ai.types.utils import BaseDheeraAIOpenAIResponseObject
    mock_response.__class__ = type('MockResponse', (BaseDheeraAIOpenAIResponseObject,), {})
