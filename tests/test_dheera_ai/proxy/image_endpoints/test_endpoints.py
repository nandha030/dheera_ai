...
from dheera_ai.proxy.image_endpoints import endpoints
...
async def test_image_generation_prompt_rerouting(monkeypatch):
    ...
    async def fake_add_dheera_ai_data_to_request(**kwargs):
        return kwargs["data"]
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy.proxy_server.add_dheera_ai_data_to_request",
        fake_add_dheera_ai_data_to_request,
    )
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.general_settings", {})
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.llm_router", None)
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.proxy_config", {})
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.proxy_logging_obj", fake_proxy_logger)
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.user_model", None)
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.version", "test-version")
    monkeypatch.setattr(
        "dheera_ai.proxy.common_request_processing.ProxyBaseLLMRequestProcessing.get_custom_headers",
        classmethod(lambda *args, **kwargs: {}),
    )
    monkeypatch.setattr(
        "dheera_ai.proxy.image_endpoints.endpoints.route_request", fake_route_request
    )
