...
from dheera_ai.proxy._types import LitellmUserRoles, NewUserRequest, ProxyException
from dheera_ai.proxy.management_endpoints.scim.scim_v2 import (
...
from dheera_ai.types.proxy.management_endpoints.scim_v2 import (
    SCIMFeature,
    SCIMGroup,
    SCIMMember,
    SCIMPatchOp,
...
async def test_create_user_existing_user_conflict(mocker):
    ...
    mock_prisma_client.db.dheera_ai_usertable = mocker.MagicMock()
    mock_prisma_client.db.dheera_ai_usertable.find_unique = AsyncMock(return_value={"user_id": "existing-user"})

    # Mock the _get_prisma_client_or_raise_exception to return our mock
    mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2._get_prisma_client_or_raise_exception",
        AsyncMock(return_value=mock_prisma_client),
    )
    
    mocked_new_user = mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2.new_user",
        AsyncMock(),
    )
...
async def test_create_user_defaults_to_viewer(mocker, monkeypatch):
    ...
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.dheera_ai_usertable = mocker.MagicMock()
    mock_prisma_client.db.dheera_ai_usertable.find_unique = AsyncMock(return_value=None)
    mock_prisma_client.db.dheera_ai_usertable.find_first = AsyncMock(return_value=None)

    monkeypatch.setattr(
        "dheera_ai.default_internal_user_params", None, raising=False
    )
    ...
    mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_dheera_ai_user_to_scim_user",
...
async def test_create_user_uses_default_internal_user_params_role(mocker, monkeypatch):
    ...
    monkeypatch.setattr(
        "dheera_ai.default_internal_user_params", default_params, raising=False
    )
...
async def test_handle_existing_user_by_email_no_existing_user(mocker):
    ...
    assert result is None
    mock_prisma_client.db.dheera_ai_usertable.find_first.assert_called_once_with(
...
async def test_handle_existing_user_by_email_existing_user_updated(mocker):
    ...
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.dheera_ai_usertable = mocker.MagicMock()
    mock_prisma_client.db.dheera_ai_usertable.find_first = AsyncMock(return_value=existing_user)
    mock_prisma_client.db.dheera_ai_usertable.update = AsyncMock(return_value=updated_user)
    ...
    mock_prisma_client.db.dheera_ai_usertable.update.assert_called_once_with(
...
async def test_handle_team_membership_changes_no_changes(mocker):
    ...
    mock_patch_team_membership = mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2.patch_team_membership",
        AsyncMock()
    )
...
async def test_update_user_success(mocker):
    ...
    mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2._check_user_exists",
        AsyncMock(return_value=existing_user)
    )
    mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2._handle_team_membership_changes",
        AsyncMock()
    )
    mock_transform = mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_dheera_ai_user_to_scim_user",
    ...
    mock_prisma_client.db.dheera_ai_usertable.update.assert_called_once()
    call_args = mock_prisma_client.db.dheera_ai_usertable.update.call_args
...
async def test_update_group_metadata_serialization_issue(mocker):
    ...
    """
    from dheera_ai.proxy.management_endpoints.scim.scim_v2 import update_group
    from dheera_ai.types.proxy.management_endpoints.scim_v2 import SCIMGroup, SCIMMember
    ...
    mock_prisma_client = mocker.MagicMock()
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.dheera_ai_teamtable = mocker.MagicMock()
    mock_prisma_client.db.dheera_ai_usertable = mocker.MagicMock()
    
    # Mock team operations
    mock_prisma_client.db.dheera_ai_teamtable.find_unique = AsyncMock(return_value=mock_existing_team)
    mock_prisma_client.db.dheera_ai_teamtable.update = AsyncMock(return_value=mock_updated_team)
    ...
    mock_prisma_client.db.dheera_ai_usertable.find_unique = AsyncMock(return_value=mock_user)
    mock_prisma_client.db.dheera_ai_usertable.update = AsyncMock(return_value=mock_user)
    ...
    mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_dheera_ai_team_to_scim_group",
    ...
    mock_prisma_client.db.dheera_ai_teamtable.update.assert_called_once()
...
async def test_team_membership_management(mocker):
    ...
    """
    from dheera_ai.proxy._types import Member
    from dheera_ai.proxy.management_endpoints.scim.scim_v2 import (
...
async def test_update_group_e2e(mocker):
    ...
    """
    from dheera_ai.proxy._types import DheeraAI_TeamTable, Member
    from dheera_ai.proxy.management_endpoints.scim.scim_transformations import (
        ScimTransformations,
    )
    from dheera_ai.proxy.utils import safe_dumps

    # Setup test data
    group_id = "test-team-123"
    
    # Mock existing team in database
    existing_team = DheeraAI_TeamTable(
        team_id=group_id,
    ...
    mock_prisma_client.db.dheera_ai_teamtable.find_unique = AsyncMock(return_value=existing_team)
    
    # Mock the updated team that gets returned from database
    updated_team = DheeraAI_TeamTable(
        team_id=group_id,
    ...
    mock_prisma_client.db.dheera_ai_teamtable.update = AsyncMock(return_value=updated_team)
    ...
    mocker.patch.object(
        ScimTransformations,
        "transform_dheera_ai_team_to_scim_group",
    ...
    # Verify database update was called with correct data
    mock_prisma_client.db.dheera_ai_teamtable.update.assert_called_once()
    update_call_args = mock_prisma_client.db.dheera_ai_teamtable.update.call_args
    ...
    ScimTransformations.transform_dheera_ai_team_to_scim_group.assert_called_once_with(updated_team)
...
async def test_create_group_with_nonexistent_users_creates_users(mocker):
    ...
    mock_prisma_client.db.dheera_ai_teamtable.find_unique = AsyncMock(return_value=None)
    ...
    mock_prisma_client.db.dheera_ai_usertable.find_unique = AsyncMock(side_effect=mock_user_lookup)
    ...
    mock_new_user = mocker.patch(
        "dheera_ai.proxy.management_endpoints.internal_user_endpoints.new_user",
        AsyncMock()
    )
    ...
    mock_new_team = mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2.new_team",
...
async def test_update_group_with_nonexistent_users_creates_users(mocker):
    ...
    mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2._check_team_exists",
    ...
    mock_handle_group_membership_changes = mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2._handle_group_membership_changes",
        AsyncMock()
    )
    ...
    mock_prisma_client.db.dheera_ai_teamtable.update.assert_called_once()
    update_call = mock_prisma_client.db.dheera_ai_teamtable.update.call_args
...
async def test_patch_group_refreshes_team_data_to_prevent_race_conditions(mocker):
    ...
    updated_team_after_patch = DheeraAI_TeamTable(
        team_id=group_id,
    ...
    refreshed_team_before_membership = DheeraAI_TeamTable(
        team_id=group_id,
        team_alias="Updated Team",
    ...
    final_refreshed_team = DheeraAI_TeamTable(
        team_id=group_id,
        team_alias="Updated Team",
    ...
    mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2._process_group_patch_operations",
    ...
    mocker.patch(
        "dheera_ai.proxy.management_endpoints.scim.scim_v2._apply_group_patch_updates",
    ...
    # model_dump() should return a dict that can be used to construct DheeraAI_TeamTable
    ...
    mock_prisma_client.db.dheera_ai_teamtable.find_unique = AsyncMock(side_effect=refresh_calls)
    ...
    assert mock_prisma_client.db.dheera_ai_teamtable.find_unique.call_count == 2
    
    # Verify first refresh was called after applying updates
    first_refresh_call = mock_prisma_client.db.dheera_ai_teamtable.find_unique.call_args_list[0]
    ...
    second_refresh_call = mock_prisma_client.db.dheera_ai_teamtable.find_unique.call_args_list[1]
    ...
    from dheera_ai.proxy.management_endpoints.scim.scim_v2 import ScimTransformations
    ScimTransformations.transform_dheera_ai_team_to_scim_group.assert_called_once()
    transform_call = ScimTransformations.transform_dheera_ai_team_to_scim_group.call_args[0][0]
    # Verify it was called with final_refreshed_team (has user5)
    assert isinstance(transform_call, DheeraAI_TeamTable)
