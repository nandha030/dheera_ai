...
from dheera_ai.proxy.management_endpoints.customer_endpoints import update_end_user
...
def mock_existing_customer():
    ...
    return MagicMock(spec=DheeraAI_EndUserTable)


@pytest.fixture
def mock_budget_table():
    """Mock budget table data."""
    return MagicMock(spec=DheeraAI_BudgetTable)


@pytest.mark.asyncio
@patch('dheera_ai.proxy.proxy_server.prisma_client')
@patch('dheera_ai.proxy.proxy_server.dheera_ai_proxy_admin_name', 'admin')
async def test_update_customer_with_budget_id(
    ...
    mock_prisma_client.db.dheera_ai_endusertable.update.assert_called_once()
    call_args = mock_prisma_client.db.dheera_ai_endusertable.update.call_args
    ...
    assert not mock_prisma_client.db.dheera_ai_budgettable.create.called
...
async def test_update_customer_creates_budget_with_proper_relations(
    ...
    call_args = mock_prisma_client.db.dheera_ai_budgettable.create.call_args
...
async def test_update_customer_budget_creation_with_fallback_admin(
    ...
    creation_data = call_args[1]['data']  # kwargs['data']
    assert creation_data['created_by'] == "admin"  # dheera_ai_proxy_admin_name
