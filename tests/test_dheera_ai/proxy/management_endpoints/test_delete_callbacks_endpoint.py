...
class MockPrismaClient:
    def __init__(self):
        ...
        self.db.dheera_ai_config.find_first = AsyncMock(
    ...
    async def _mock_find_first(self, where=None):
        ...
        if where and "param_name" in where:
            ...
            if param_name == "dheera_ai_settings":
                return MagicMock(
                    param_name="dheera_ai_settings",
                    param_value=self.config_data["dheera_ai_settings"]
                )
    ...
    def remove_callback_from_config(self, callback_name):
        ...
        if "success_callback" in self.config_data["dheera_ai_settings"]:
            callbacks = self.config_data["dheera_ai_settings"]["success_callback"]
