...
async def test_ui_view_users_with_null_email(mocker, caplog):
    ...
    mock_prisma_client.db.dheera_ai_usertable.find_many = mock_find_many

    # Patch the prisma client import in the endpoint
    mocker.patch("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma_client)
    ...
    assert response == [
        DheeraAI_UserTableFiltered(user_id="test-user-null-email", user_email=None)
    ]
...
async def test_get_users_includes_timestamps(mocker):
    ...
    mock_prisma_client.db.dheera_ai_usertable.find_many = mock_find_many
    mock_prisma_client.db.dheera_ai_usertable.count = mock_count
    ...
    mocker.patch(
        "dheera_ai.proxy.management_endpoints.internal_user_endpoints.get_user_key_counts",
...
def test_update_internal_user_params_email():
    ...
    Test that _update_internal_user_params correctly processes an email-only update
    """
    from dheera_ai.proxy._types import UpdateUserRequest
    from dheera_ai.proxy.management_endpoints.internal_user_endpoints import (
        _update_internal_user_params,
    )
...
def test_update_internal_user_params_reset_spend_and_max_budget():
    """
    Relevant Issue: https://github.com/BerriAI/dheera_ai/issues/10495
    """
    from dheera_ai.proxy._types import UpdateUserRequest
    from dheera_ai.proxy.management_endpoints.internal_user_endpoints import (
        _update_internal_user_params,
...
async def test_new_user_license_over_limit(mocker):
    ...
    from dheera_ai.proxy._types import NewUserRequest, UserAPIKeyAuth
    ...
    mocker.patch(
        "dheera_ai.proxy.management_endpoints.internal_user_endpoints._check_duplicate_user_email",
    ...
    mocker.patch("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma_client)
    mocker.patch("dheera_ai.proxy.proxy_server._license_check", mock_license_check)
...
async def test_user_info_url_encoding_plus_character(mocker):
    ...
    from dheera_ai.proxy._types import DheeraAI_UserTable, UserAPIKeyAuth
    from dheera_ai.proxy.management_endpoints.internal_user_endpoints import user_info

    # Mock the prisma client
    mock_prisma_client = mocker.MagicMock()
    
    # Create a real DheeraAI_UserTable instance (BaseModel) so isinstance check passes
    mock_user = DheeraAI_UserTable(
...
async def test_new_user_default_teams_flow(mocker):
    ...
    """
    import dheera_ai
    from dheera_ai.proxy._types import NewUserRequest, NewUserRequestTeam, UserAPIKeyAuth
    from dheera_ai.proxy.management_endpoints.internal_user_endpoints import new_user
    ...
    original_default_params = getattr(dheera_ai, "default_internal_user_params", None)
    ...
    try:
        ...
        mocker.patch("dheera_ai.proxy.proxy_server._license_check", mock_license_check)
        mocker.patch(
            "dheera_ai.proxy.management_endpoints.internal_user_endpoints.generate_key_helper_fn",
            mock_generate_key_helper_fn,
        )
        mocker.patch(
            "dheera_ai.proxy.management_endpoints.internal_user_endpoints._add_user_to_team",
            mock_add_user_to_team,
        )
        mocker.patch(
            "dheera_ai.proxy.management_endpoints.internal_user_endpoints.UserManagementEventHooks.async_user_created_hook",
        ...
        if original_default_params is not None:
            dheera_ai.default_internal_user_params = original_default_params
        else:
            if hasattr(dheera_ai, "default_internal_user_params"):
                delattr(dheera_ai, "default_internal_user_params")
...
def test_update_internal_new_user_params_proxy_admin_role():
    ...
    """
    import dheera_ai
    from dheera_ai.proxy._types import LitellmUserRoles, NewUserRequest
    from dheera_ai.proxy.management_endpoints.internal_user_endpoints import (
...
async def test_check_duplicate_user_email_case_insensitive(mocker):
    ...
    mock_prisma_client.db.dheera_ai_usertable.find_first = mock_find_first_duplicate
    ...
    mock_prisma_client.db.dheera_ai_usertable.find_first = mock_find_first_no_duplicate
...
def test_process_keys_for_user_info_filters_dashboard_keys(monkeypatch):
    """
    Test that _process_keys_for_user_info filters out keys with team_id='dheera_ai-dashboard'
    
    UI session tokens (team_id='dheera_ai-dashboard') should be excluded from user info responses
    ...
    # Mock general_settings and dheera_ai_master_key_hash (they're imported from proxy_server)
    monkeypatch.setattr(
        "dheera_ai.proxy.proxy_server.general_settings",
        {},
    )
    monkeypatch.setattr(
        "dheera_ai.proxy.proxy_server.dheera_ai_master_key_hash",
        "different-hash",
    )
