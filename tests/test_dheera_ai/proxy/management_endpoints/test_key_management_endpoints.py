...
async def test_list_keys():
    ...
    mock_prisma_client.db.dheera_ai_verificationtoken.find_many = mock_find_many
    args = {
    ...
    assert json.dumps({"team_id": {"not": "dheera_ai-dashboard"}}) in json.dumps(
        where_condition
    )
...
async def test_list_keys_include_created_by_keys():
    ...
    mock_prisma_client.db.dheera_ai_verificationtoken.find_many = mock_find_many
    mock_prisma_client.db.dheera_ai_verificationtoken.count = mock_count
...
async def test_key_token_handling(monkeypatch):
    ...
    mock_insert_data = AsyncMock(
        return_value=MagicMock(
            token="hashed_token_123", dheera_ai_budget_table=None, object_permission=None
        )
    )
    mock_prisma_client.insert_data = mock_insert_data
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.dheera_ai_verificationtoken = MagicMock()
    mock_prisma_client.db.dheera_ai_verificationtoken.find_unique = AsyncMock(
        return_value=None
    )
    mock_prisma_client.db.dheera_ai_verificationtoken.find_many = AsyncMock(
        return_value=[]
    )
    mock_prisma_client.db.dheera_ai_verificationtoken.count = AsyncMock(return_value=0)
    mock_prisma_client.db.dheera_ai_verificationtoken.update = AsyncMock(
        return_value=MagicMock(
            token="hashed_token_123", dheera_ai_budget_table=None, object_permission=None
        )
    )

    from dheera_ai.proxy._types import GenerateKeyRequest, LitellmUserRoles
    from dheera_ai.proxy.auth.user_api_key_auth import UserAPIKeyAuth
    from dheera_ai.proxy.management_endpoints.key_management_endpoints import (
        generate_key_fn,
    )
...
async def test_budget_reset_and_expires_at_first_of_month(monkeypatch):
    ...
    mock_prisma_client = AsyncMock()
    mock_insert_data = AsyncMock(
        return_value=MagicMock(token="hashed_token_123", dheera_ai_budget_table=None)
    )
    mock_prisma_client.insert_data = mock_insert_data
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.dheera_ai_verificationtoken = MagicMock()
...
async def test_key_generation_with_object_permission(monkeypatch):
    ...
    # Mock the prisma_client.db.dheera_ai_objectpermissiontable.create call
    mock_object_permission_create = AsyncMock(
        return_value=MagicMock(object_permission_id="objperm123")
    )
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.dheera_ai_objectpermissiontable = MagicMock()
    mock_prisma_client.db.dheera_ai_objectpermissiontable.create = (
...
async def test_key_generation_with_mcp_tool_permissions(monkeypatch):
    ...
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.dheera_ai_objectpermissiontable = MagicMock()
    mock_prisma_client.db.dheera_ai_objectpermissiontable.create = mock_create
...
async def test_key_update_object_permissions_no_existing_permission(monkeypatch):
    ...
    DheeraAI_ObjectPermissionTable and the key is updated with the new object_permission_id.
    """
    from unittest.mock import AsyncMock, MagicMock
    ...
    existing_key_row_no_perm = DheeraAI_VerificationToken(
...
async def test_key_update_object_permissions_missing_permission_record(monkeypatch):
    ...
    existing_key_row_missing_perm = DheeraAI_VerificationToken(
...
def test_get_new_token_with_valid_key():
    ...
    from dheera_ai.proxy._types import RegenerateKeyRequest
    from dheera_ai.proxy.management_endpoints.key_management_endpoints import (
        get_new_token,
    )
...
async def test_generate_service_account_works_with_team_id():
    ...
    ) as mock_router, patch("dheera_ai.proxy.proxy_server.premium_user", False), patch(
        "dheera_ai.proxy.management_endpoints.key_management_endpoints.generate_key_helper_fn"
...
async def test_update_service_account_requires_team_id():
    ...
    existing_key = DheeraAI_VerificationToken(token="hashed", team_id=None)
...
async def test_update_service_account_works_with_team_id():
    ...
    existing_key = DheeraAI_VerificationToken(token="hashed")
...
async def test_validate_team_id_used_in_service_account_request_checks_team_exists():
    ...
    mock_prisma_client.db.dheera_ai_teamtable.find_unique = mock_find_unique
...
async def test_generate_service_account_key_endpoint_validation():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma:
        # Mock team not found
        mock_find_unique = AsyncMock(return_value=None)
        mock_prisma.db.dheera_ai_teamtable.find_unique = mock_find_unique
...
async def test_unblock_key_supports_both_sk_and_hashed_tokens(monkeypatch):
    ...
    from dheera_ai.proxy._types import BlockKeyRequest
    from dheera_ai.proxy.management_endpoints.key_management_endpoints import unblock_key
    ...
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr(
        "dheera_ai.proxy.proxy_server.user_api_key_cache", mock_user_api_key_cache
    )
    monkeypatch.setattr(
        "dheera_ai.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging_obj
    )
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.hash_token", mock_hash_token)
    monkeypatch.setattr(
        "dheera_ai.store_audit_logs", False
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy.management_endpoints.key_management_endpoints.get_key_object",
        mock_get_key_object,
    )
    monkeypatch.setattr(
        "dheera_ai.proxy.management_endpoints.key_management_endpoints._cache_key_object",
    ...
    mock_prisma_client.db.dheera_ai_verificationtoken.update.assert_called_with(
    ...
    mock_prisma_client.db.dheera_ai_verificationtoken.update.reset_mock()
...
async def test_unblock_key_invalid_key_format(monkeypatch):
    ...
    from dheera_ai.proxy._types import BlockKeyRequest
    from dheera_ai.proxy.management_endpoints.key_management_endpoints import unblock_key
    from dheera_ai.proxy.utils import ProxyException
...
def test_validate_key_team_change_with_member_permissions():
    ...
    from dheera_ai.proxy._types import KeyManagementRoutes
    ...
    with patch(
        "dheera_ai.proxy.management_endpoints.key_management_endpoints.can_team_access_model"
    ):
        with patch(
            "dheera_ai.proxy.management_endpoints.key_management_endpoints._get_user_in_team"
        ) as mock_get_user:
            with patch(
                "dheera_ai.proxy.management_endpoints.key_management_endpoints._is_user_team_admin"
            ) as mock_is_admin:
                with patch(
                    "dheera_ai.proxy.management_endpoints.key_management_endpoints.TeamMemberPermissionChecks.does_team_member_have_permissions_for_endpoint"
                ) as mock_has_perms:
...
async def test_update_key_fn_auto_rotate_enable():
    ...
    from dheera_ai.proxy._types import DheeraAI_VerificationToken, UpdateKeyRequest
    from dheera_ai.proxy.management_endpoints.key_management_endpoints import (
        prepare_key_update_data,
    )

    # Mock existing key
    existing_key = DheeraAI_VerificationToken(
        token="test-token",
        key_alias="test-key",
...
async def test_check_team_key_limits_no_existing_keys():
    ...
    mock_prisma_client.db.dheera_ai_verificationtoken.find_many.assert_called_once_with(
...
def test_check_team_key_model_specific_limits_rpm_overallocation():
    ...
    existing_key1 = DheeraAI_VerificationToken(
        token="test-token-1",
        user_id="test-user-1",
        team_id="test-team-456",
        metadata={
    ...
    existing_key2 = DheeraAI_VerificationToken(
        token="test-token-2",
        user_id="test-user-2",
        team_id="test-team-456",
        metadata={
...
async def test_generate_key_with_object_permission():
    ...
    mock_prisma_client.db.dheera_ai_objectpermissiontable = MagicMock()
    mock_prisma_client.db.dheera_ai_objectpermissiontable.create = mock_object_perm_create
    ...
    with patch(
        ...
        mock_prisma_client,
    ), patch("dheera_ai.proxy.proxy_server.llm_router", None), patch(
        "dheera_ai.proxy.proxy_server.premium_user",
        False,
    ), patch(
        "dheera_ai.proxy.proxy_server.dheera_ai_proxy_admin_name",
        "admin",
    ):
        # Execute
...
async def test_check_org_key_limits_no_existing_keys():
    ...
    org_table = DheeraAI_OrganizationTable(
        ...
        created_by="admin",
        updated_by="admin",
        dheera_ai_budget_table=DheeraAI_BudgetTable(
            budget_id="budget-123",
            tpm_limit=20000,
            rpm_limit=2000,
        ),
