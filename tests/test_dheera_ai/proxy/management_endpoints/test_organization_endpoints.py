...
async def test_get_organization_daily_activity_admin_param_passing(monkeypatch):
    ...
    from dheera_ai.proxy.management_endpoints import organization_endpoints
    from dheera_ai.proxy.management_endpoints.organization_endpoints import (
    ...
    # Mock prisma client
    mock_prisma_client = AsyncMock()
    mock_prisma_client.db.dheera_ai_organizationtable.find_many = AsyncMock(
        return_value=[]
    )
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma_client)
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy.management_endpoints.organization_endpoints._user_has_admin_view",
        lambda _: True,
    )
    ...
    assert kwargs["table_name"] == "dheera_ai_dailyorganizationspend"
...
async def test_get_organization_daily_activity_non_admin_defaults_to_admin_orgs(monkeypatch):
    ...
    mock_prisma_client.db.dheera_ai_organizationmembership.find_many = AsyncMock(
        return_value=[
            SimpleNamespace(
...
async def test_organization_update_object_permissions_no_existing_permission(
    ...
    DheeraAI_ObjectPermissionTable and the organization is updated with the new object_permission_id.
    """
    from unittest.mock import AsyncMock, MagicMock
    ...
    existing_organization_row_no_perm = DheeraAI_OrganizationTable(
...
async def test_organization_update_object_permissions_missing_permission_record(
    ...
    existing_organization_row_missing_perm = DheeraAI_OrganizationTable(
