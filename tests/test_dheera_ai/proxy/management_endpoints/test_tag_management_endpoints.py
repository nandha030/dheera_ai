...
from dheera_ai.types.tag_management import TagDeleteRequest, TagInfoRequest, TagNewRequest
...
async def test_create_and_get_tag():
    ...
    try:
        with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
            ...
            "dheera_ai.proxy.proxy_server.dheera_ai_proxy_admin_name", "default_user_id"
        ), patch(
            "dheera_ai.proxy.management_endpoints.tag_management_endpoints.get_deployments_by_model"
            ...
            mock_db.dheera_ai_tagtable.find_unique = AsyncMock(return_value=None)
            
            # Mock find_many for model lookup
            mock_db.dheera_ai_proxymodeltable.find_many = AsyncMock(return_value=[])
            ...
            mock_db.dheera_ai_tagtable.create = AsyncMock(return_value=created_tag)
            ...
            retrieved_tag.dheera_ai_budget_table = None
            mock_db.dheera_ai_tagtable.find_many = AsyncMock(return_value=[retrieved_tag])
...
async def test_update_tag():
    ...
    try:
        with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
            ...
            mock_db.dheera_ai_tagtable.find_unique = AsyncMock(return_value=existing_tag)
            ...
            mock_db.dheera_ai_tagtable.update = AsyncMock(return_value=updated_tag)
...
async def test_delete_tag():
    ...
    try:
        with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            # Mock delete
            mock_db.dheera_ai_tagtable.delete = AsyncMock(return_value=existing_tag)
            ...
            mock_db.dheera_ai_tagtable.delete.assert_called_once()
    finally:
        # Clean up dependency overrides
        app.dependency_overrides.clear()
...
async def test_get_deployments_by_model_id():
    ...
    mock_deployment = Deployment(
        ...
        dheera_ai_params=DheeraAI_Params(model="gpt-3.5-turbo"),
...
async def test_get_deployments_by_model_name():
    ...
    mock_router.get_model_list.return_value = [
        {
            "model_name": "gpt-3.5-turbo",
            "dheera_ai_params": {"model": "gpt-3.5-turbo", "api_key": "test-key"},
