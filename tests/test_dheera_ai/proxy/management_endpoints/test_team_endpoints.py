...
from dheera_ai.proxy._types import UserAPIKeyAuth  # Import UserAPIKeyAuth
from dheera_ai.proxy._types import (
    DheeraAI_OrganizationMembershipTable,
    DheeraAI_OrganizationTable,
    DheeraAI_OrganizationTableWithMembers,
    DheeraAI_TeamTable,
    LitellmUserRoles,
    Member,
    ProxyErrorTypes,
...
from dheera_ai.proxy.management_helpers.team_member_permission_checks import (
    TeamMemberPermissionChecks,
)
from dheera_ai.proxy.proxy_server import app
from dheera_ai.router import Router
from dheera_ai.types.proxy.management_endpoints.team_endpoints import (
...
mock_prisma_client.db.dheera_ai_teamtable = MagicMock()
mock_prisma_client.db.dheera_ai_teamtable.update = AsyncMock()
...
async def test_validate_team_org_change_same_org_id():
    ...
    # Mock team
    team = MagicMock(spec=DheeraAI_TeamTable)
    ...
    # Mock organization
    organization = MagicMock(spec=DheeraAI_OrganizationTableWithMembers)
    organization.organization_id = org_id
    organization.models = []
    organization.dheera_ai_budget_table = MagicMock()
    organization.dheera_ai_budget_table.max_budget = (
        50.0  # This would normally fail validation
    )
    organization.dheera_ai_budget_table.tpm_limit = (
        500  # This would normally fail validation
    )
    organization.dheera_ai_budget_table.rpm_limit = (
    ...
    with patch(
        "dheera_ai.proxy.management_endpoints.team_endpoints.can_org_access_model"
...
async def test_validate_team_org_change_members_in_org():
    ...
    # Mock organization with members (using DheeraAI_OrganizationMembershipTable structure)
    organization = MagicMock(spec=DheeraAI_OrganizationTableWithMembers)
    organization.organization_id = new_org_id
    organization.models = []
    organization.dheera_ai_budget_table = None

    # Create mock organization members - these should match team members
    org_member_1 = MagicMock(spec=DheeraAI_OrganizationMembershipTable)
    org_member_1.user_id = user_id_1
    org_member_2 = MagicMock(spec=DheeraAI_OrganizationMembershipTable)
...
async def test_get_team_permissions_list_success(mock_db_client, mock_admin_auth):
    ...
    with patch(
        "dheera_ai.proxy.management_endpoints.team_endpoints.get_team_object",
...
async def test_update_team_permissions_success(mock_db_client, mock_admin_auth):
    ...
    mock_existing_team_row = MagicMock(spec=DheeraAI_TeamTable)
    ...
    mock_updated_team_row = MagicMock(spec=DheeraAI_TeamTable)
    ...
    with patch(
        ...
        mock_db_client.db.dheera_ai_teamtable.update = AsyncMock(
        ...
        mock_db_client.db.dheera_ai_teamtable.update.assert_awaited_once_with(
...
async def test_new_team_with_object_permission(mock_db_client, mock_admin_auth):
    ...
    1. Creating a record in dheera_ai_objectpermissiontable
    ...
    mock_db_client.db.dheera_ai_objectpermissiontable = MagicMock()
    mock_db_client.db.dheera_ai_objectpermissiontable.create = mock_object_perm_create

    # Mock model table creation
    mock_db_client.db.dheera_ai_modeltable = MagicMock()
    mock_db_client.db.dheera_ai_modeltable.create = AsyncMock(
    ...
    mock_db_client.db.dheera_ai_teamtable = MagicMock()
    mock_db_client.db.dheera_ai_teamtable.create = mock_team_create
    mock_db_client.db.dheera_ai_teamtable.count = mock_team_count
    mock_db_client.db.dheera_ai_teamtable.update = AsyncMock(
        return_value=team_create_result
    )

    # Mock user table
    mock_db_client.db.dheera_ai_usertable = MagicMock()
    mock_db_client.db.dheera_ai_usertable.update = AsyncMock(return_value=MagicMock())

    from fastapi import Request

    from dheera_ai.proxy._types import DheeraAI_ObjectPermissionBase, NewTeamRequest
    from dheera_ai.proxy.management_endpoints.team_endpoints import new_team

    # Build request with object_permission
    team_request = NewTeamRequest(
        team_alias="my-team",
        object_permission=DheeraAI_ObjectPermissionBase(vector_stores=["my-vector"]),
    )
...
async def test_new_team_with_mcp_tool_permissions(mock_db_client, mock_admin_auth):
    ...
    2. The field is properly stored in the DheeraAI_ObjectPermissionTable
    ...
    mock_db_client.db.dheera_ai_objectpermissiontable = MagicMock()
    mock_db_client.db.dheera_ai_objectpermissiontable.create = mock_obj_perm_create
    ...
    mock_db_client.db.dheera_ai_teamtable = MagicMock()
    mock_db_client.db.dheera_ai_teamtable.create = AsyncMock(return_value=team_create_result)
    mock_db_client.db.dheera_ai_teamtable.count = AsyncMock(return_value=0)
    mock_db_client.db.dheera_ai_teamtable.update = AsyncMock(return_value=team_create_result)
    ...
    team_request = NewTeamRequest(
        team_alias="mcp-team",
        object_permission=DheeraAI_ObjectPermissionBase(
...
async def test_team_update_object_permissions_existing_permission(monkeypatch):
    ...
    object_permission_id, the existing DheeraAI_ObjectPermissionTable record is updated
    ...
    from dheera_ai.proxy._types import DheeraAI_ObjectPermissionBase, DheeraAI_TeamTable
    from dheera_ai.proxy.management_endpoints.team_endpoints import (
    ...
    existing_team_row = DheeraAI_TeamTable(
        team_id="test_team_id",
    ...
    mock_prisma_client.db.dheera_ai_objectpermissiontable.find_unique = AsyncMock(
    ...
    mock_prisma_client.db.dheera_ai_objectpermissiontable.upsert = AsyncMock(
    ...
    mock_prisma_client.db.dheera_ai_objectpermissiontable.find_unique.assert_called_once_with(
        where={"object_permission_id": "existing_perm_id_123"}
    )
    mock_prisma_client.db.dheera_ai_objectpermissiontable.upsert.assert_called_once()
...
async def test_team_update_object_permissions_no_existing_permission(monkeypatch):
    ...
    DheeraAI_ObjectPermissionTable and the team is updated with the new object_permission_id.
    """
    from unittest.mock import AsyncMock, MagicMock
    ...
    existing_team_row_no_perm = DheeraAI_TeamTable(
...
async def test_team_update_object_permissions_missing_permission_record(monkeypatch):
    ...
    a new entry is created in the DheeraAI_ObjectPermissionTable with the new permissions.
    """
    from unittest.mock import AsyncMock, MagicMock
    ...
    existing_team_row_missing_perm = DheeraAI_TeamTable(
...
def test_team_member_add_duplication_check_raises_proxy_exception():
    ...
    existing_team_row = MagicMock(spec=DheeraAI_TeamTable)
...
async def test_add_team_member_budget_table_success():
    ...
    """
    from dheera_ai.proxy._types import TeamInfoResponseObjectTeamTable
    from dheera_ai.proxy.management_endpoints.team_endpoints import (
    ...
    mock_prisma_client.db.dheera_ai_budgettable.find_unique.assert_called_once_with(
...
async def test_add_team_member_budget_table_exception_handling():
    ...
    with patch(
        "dheera_ai.proxy.management_endpoints.team_endpoints.verbose_proxy_logger"
    ) as mock_logger:
        # Call the function
        result = await _add_team_member_budget_table(
...
async def test_add_team_member_budget_table_budget_not_found():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.dheera_ai_budgettable.find_unique = AsyncMock(return_value=None)
...
def test_add_new_models_to_team():
    ...
    """
    from dheera_ai.proxy._types import SpecialModelNames
    from dheera_ai.proxy.management_endpoints.team_endpoints import add_new_models_to_team

    team_obj = MagicMock(spec=DheeraAI_TeamTable)
    team_obj.models = []
...
async def test_validate_team_member_add_permissions_non_admin():
    ...
    with patch(
        "dheera_ai.proxy.management_endpoints.team_endpoints._is_user_team_admin",
        return_value=False,
    ), patch(
        "dheera_ai.proxy.management_endpoints.team_endpoints._is_available_team",
        return_value=False,
    ):
...
async def test_process_team_members_single_member():
    ...
    """
    from dheera_ai.proxy._types import DheeraAI_TeamMembership, DheeraAI_UserTable
    from dheera_ai.proxy.management_endpoints.team_endpoints import _process_team_members

    # Mock dependencies
    mock_prisma_client = MagicMock()
    mock_team = MagicMock(spec=DheeraAI_TeamTable)
    mock_team.metadata = {"team_member_budget_id": "budget-123"}

    # Mock user and membership objects
    mock_user = MagicMock(spec=DheeraAI_UserTable)
    mock_user.user_id = "new-user-123"
    mock_membership = MagicMock(spec=DheeraAI_TeamMembership)
    ...
    with patch(
        "dheera_ai.proxy.management_endpoints.team_endpoints.add_new_member",
        new_callable=AsyncMock,
        ...
        users, memberships = await _process_team_members(
            ...
            dheera_ai_proxy_admin_name="admin",
        )
...
async def test_process_team_members_multiple_members():
    ...
    mock_users = [MagicMock(spec=DheeraAI_UserTable) for _ in range(2)]
    mock_memberships = [MagicMock(spec=DheeraAI_TeamMembership) for _ in range(2)]
...
async def test_update_team_team_member_budget_not_passed_to_db():
    """
    Test that 'team_member_budget' is never passed to prisma_client.db.dheera_ai_teamtable.update
    ...
    from dheera_ai.proxy._types import LitellmUserRoles, UpdateTeamRequest, UserAPIKeyAuth
    from dheera_ai.proxy.management_endpoints.team_endpoints import update_team
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma_client, patch(
        "dheera_ai.proxy.proxy_server.llm_router"
    ) as mock_llm_router, patch(
        "dheera_ai.proxy.proxy_server.user_api_key_cache"
    ) as mock_cache, patch(
        "dheera_ai.proxy.proxy_server.proxy_logging_obj"
    ) as mock_logging, patch(
        "dheera_ai.proxy.proxy_server.dheera_ai_proxy_admin_name", "admin"
    ), patch(
        "dheera_ai.proxy.auth.auth_checks._cache_team_object"
    ) as mock_cache_team, patch(
        "dheera_ai.proxy.management_endpoints.team_endpoints.TeamMemberBudgetHandler.upsert_team_member_budget_table"
        ...
        mock_prisma_client.db.dheera_ai_teamtable.find_unique = AsyncMock(
        ...
        assert mock_prisma_client.db.dheera_ai_teamtable.update.called

        # Get the call arguments
        call_args = mock_prisma_client.db.dheera_ai_teamtable.update.call_args
        ...
        mock_prisma_client.db.dheera_ai_teamtable.update.reset_mock()
...
async def test_bulk_team_member_add_success():
    ...
    """
    from dheera_ai.proxy._types import (
        DheeraAI_TeamMembership,
        DheeraAI_UserTable,
        TeamAddMemberResponse,
    )
    from dheera_ai.proxy.management_endpoints.team_endpoints import bulk_team_member_add
    ...
    mock_user_1 = MagicMock(spec=DheeraAI_UserTable)
    ...
    mock_user_2 = MagicMock(spec=DheeraAI_UserTable)
    ...
    mock_membership_1 = MagicMock(spec=DheeraAI_TeamMembership)
    ...
    mock_membership_2 = MagicMock(spec=DheeraAI_TeamMembership)
    ...
    with patch(
        "dheera_ai.proxy.management_endpoints.team_endpoints.team_member_add",
        new_callable=AsyncMock,
...
async def test_bulk_team_member_add_all_users_flag():
    ...
    """
    from dheera_ai.proxy._types import DheeraAI_UserTable, TeamAddMemberResponse
    from dheera_ai.proxy.management_endpoints.team_endpoints import bulk_team_member_add
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_prisma.db.dheera_ai_usertable.find_many = AsyncMock(
        ...
        mock_prisma.db.dheera_ai_usertable.find_many.assert_called_once_with(
...
async def test_bulk_team_member_add_no_db_connection():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client", None):
        mock_auth = UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN)
...
async def test_list_team_v2_security_check_non_admin_user():
    ...
    from dheera_ai.proxy.management_endpoints.team_endpoints import list_team_v2
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma_client:
...
async def test_list_team_v2_security_check_non_admin_user_own_teams():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma_client:
        ...
        mock_db.dheera_ai_usertable.find_unique = AsyncMock(return_value=mock_user_object)
        ...
        mock_db.dheera_ai_teamtable.find_many = AsyncMock(return_value=mock_teams)
        mock_db.dheera_ai_teamtable.count = AsyncMock(return_value=2)
...
async def test_team_member_delete_cleans_membership(mock_db_client, mock_admin_auth):
    """
    Verify that /team/member_delete removes the corresponding DheeraAI_TeamMembership row
    so the same user can be re-added without unique constraint issues.
    """
    from dheera_ai.proxy._types import TeamMemberDeleteRequest
    from dheera_ai.proxy.management_endpoints.team_endpoints import team_member_delete
    ...
    mock_db_client.db.dheera_ai_teamtable.find_unique = AsyncMock(return_value=mock_team_row)
    mock_db_client.db.dheera_ai_teamtable.update = AsyncMock(return_value=mock_team_row)

    # User row to allow removal from user's teams list
    mock_user_row = MagicMock()
    mock_user_row.user_id = test_user_id
    mock_user_row.teams = [test_team_id]
    mock_db_client.db.dheera_ai_usertable.find_many = AsyncMock(return_value=[mock_user_row])
    mock_db_client.db.dheera_ai_usertable.update = AsyncMock(return_value=MagicMock())

    # Membership deletion should be called
    mock_db_client.db.dheera_ai_teammembership = MagicMock()
    mock_db_client.db.dheera_ai_teammembership.delete_many = AsyncMock(return_value=MagicMock())

    # Verification token deletion should be called
    mock_db_client.db.dheera_ai_verificationtoken = MagicMock()
    mock_db_client.db.dheera_ai_verificationtoken.delete_many = AsyncMock(return_value=MagicMock())
    ...
    mock_db_client.db.dheera_ai_teammembership.delete_many.assert_awaited_with(
...
async def test_team_member_delete_cleans_verification_tokens(mock_db_client, mock_admin_auth):
    ...
    mock_db_client.db.dheera_ai_verificationtoken.delete_many.assert_awaited_once_with(
        where={
...
async def test_new_team_max_budget_exceeds_user_max_budget():
    ...
    from dheera_ai.proxy._types import NewTeamRequest, ProxyException, UserAPIKeyAuth
    from dheera_ai.proxy.management_endpoints.team_endpoints import new_team
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        "dheera_ai.proxy.proxy_server._license_check"
    ) as mock_license, patch(
        "dheera_ai.proxy.proxy_server.user_api_key_cache"
    ) as mock_cache, patch(
        "dheera_ai.proxy.proxy_server.dheera_ai_proxy_admin_name", "admin"
    ), patch(
        "dheera_ai.proxy.proxy_server.create_audit_log_for_update", new=AsyncMock()
    ) as mock_audit:
        # Setup basic mocks
        mock_prisma.db.dheera_ai_teamtable.count = AsyncMock(return_value=0)
        ...
        from dheera_ai.proxy._types import DheeraAI_UserTable
        mock_user_obj = DheeraAI_UserTable(
            user_id="non-admin-user-123",
            max_budget=100.0,
        )
...
async def test_new_team_max_budget_within_user_limit():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_prisma.db.dheera_ai_teamtable.create = AsyncMock(return_value=mock_created_team)
        mock_prisma.db.dheera_ai_teamtable.update = AsyncMock(return_value=mock_created_team)
        
        # Mock model table
        mock_prisma.db.dheera_ai_modeltable = MagicMock()
        mock_prisma.db.dheera_ai_modeltable.create = AsyncMock(return_value=MagicMock(id="model123"))
        ...
        mock_prisma.db.dheera_ai_usertable = MagicMock()
        mock_prisma.db.dheera_ai_usertable.upsert = AsyncMock(return_value=mock_user)
        mock_prisma.db.dheera_ai_usertable.update = AsyncMock(return_value=mock_user)
        ...
        mock_prisma.db.dheera_ai_teammembership = MagicMock()
        mock_prisma.db.dheera_ai_teammembership.create = AsyncMock(return_value=mock_membership)
...
async def test_new_team_org_scoped_budget_bypasses_user_limit():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        "dheera_ai.proxy.management_endpoints.team_endpoints.get_org_object"
    ) as mock_get_org:
        ...
        mock_org = MagicMock(spec=DheeraAI_OrganizationTable)
        mock_org.organization_id = "test-org-123"
        mock_org.max_budget = 100.0
        mock_org.models = ["gpt-4", "gpt-3.5-turbo", "claude-3-opus"]
        mock_org.dheera_ai_budget_table = None  # No budget table for this test
...
async def test_new_team_org_scoped_models_bypasses_user_limit():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_org.max_budget = 100.0
        mock_org.models = ["gpt-4", "gpt-3.5-turbo", "claude-3-opus"]
        mock_org.dheera_ai_budget_table = None
        mock_get_org.return_value = mock_org
...
async def test_update_team_standalone_budget_exceeds_user_limit():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_prisma.db.dheera_ai_teamtable.find_unique = AsyncMock(return_value=mock_existing_team)
...
async def test_update_team_standalone_models_exceeds_user_limit():
    ...
    from dheera_ai.proxy._types import ProxyException, UpdateTeamRequest, UserAPIKeyAuth
    from dheera_ai.proxy.management_endpoints.team_endpoints import update_team
...
async def test_update_team_org_scoped_budget_bypasses_user_limit():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        mock_updated_team.dheera_ai_model_table = None
        mock_updated_team.model_dump.return_value = {
            "team_id": "org-team-update-budget-123",
            "organization_id": "test-org-update-budget",
            "max_budget": 50.0,
        }
        mock_prisma.db.dheera_ai_teamtable.update = AsyncMock(return_value=mock_updated_team)
...
async def test_new_team_org_scoped_tpm_rpm_bypasses_user_limit():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        "dheera_ai.proxy.management_endpoints.team_endpoints._add_team_members_to_team",
        new=AsyncMock()
    ):
        mock_license.is_team_count_over_limit.return_value = False
        mock_prisma.db.dheera_ai_teamtable.count = AsyncMock(return_value=0)
        ...
        # Mock team creation
        mock_created_team = MagicMock(spec=DheeraAI_TeamTable)
...
async def test_update_team_org_scoped_tpm_rpm_bypasses_user_limit():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        # Mock team update
        mock_updated_team = MagicMock(spec=DheeraAI_TeamTable)
...
async def test_update_team_guardrails_with_org_id():
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma, patch(
        ...
        "dheera_ai.proxy.proxy_server.premium_user", True  # Required for guardrails feature
    ):
        ...
        mock_prisma.db.dheera_ai_teamtable.find_unique = AsyncMock(
            return_value=mock_existing_team
        )
        mock_cache.async_set_cache = AsyncMock()
        ...
        mock_prisma.db.dheera_ai_organizationtable.find_unique = AsyncMock(
        ...
        mock_prisma.db.dheera_ai_teamtable.update = AsyncMock(
            return_value=mock_updated_team
        )
        mock_prisma.jsonify_team_object = MagicMock(side_effect=lambda db_data: db_data)
        ...
        with patch("dheera_ai.proxy.proxy_server.llm_router", mock_router):
            ...
            assert mock_prisma.db.dheera_ai_organizationtable.find_unique.call_count >= 1
            
            # Get the first call (from fetch_and_validate_organization)
            first_call_kwargs = mock_prisma.db.dheera_ai_organizationtable.find_unique.call_args_list[0].kwargs
