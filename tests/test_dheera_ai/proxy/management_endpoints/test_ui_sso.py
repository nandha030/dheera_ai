...
import dheera_ai
from dheera_ai.proxy._types import DheeraAI_UserTable, NewTeamRequest, NewUserResponse
from dheera_ai.proxy.auth.handle_jwt import JWTHandler
from dheera_ai.proxy.management_endpoints.types import CustomOpenID
...
async def test_get_user_groups_from_graph_api():
    ...
    with patch(
        "dheera_ai.proxy.management_endpoints.ui_sso.get_async_httpx_client"
    ) as mock_client:
...
async def test_default_team_params(team_params):
    """
    When dheera_ai.default_team_params is set, it should be used to create a new team
    """
    # Arrange
    dheera_ai.default_team_params = team_params
    ...
    mock_prisma.db.dheera_ai_teamtable.find_first = AsyncMock(return_value=None)
    mock_prisma.db.dheera_ai_teamtable.create = AsyncMock()
    ...
    with patch("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma):
        # Act
        team_id = str(uuid.uuid4())
        await MicrosoftSSOHandler.create_dheera_ai_teams_from_service_principal_team_ids(
        ...
        mock_prisma.db.dheera_ai_teamtable.create.assert_called_once()
        print(
            "mock_prisma.db.dheera_ai_teamtable.create.call_args",
            mock_prisma.db.dheera_ai_teamtable.create.call_args,
        )
        create_call_args = mock_prisma.db.dheera_ai_teamtable.create.call_args.kwargs[
            "data"
        ]
...
async def test_create_team_without_default_params():
    """
    Test team creation when dheera_ai.default_team_params is None
    Should create team with just the basic required fields
    """
    # Arrange
    dheera_ai.default_team_params = None
...
def test_apply_user_info_values_to_sso_user_defined_values():
    from dheera_ai.proxy._types import DheeraAI_UserTable, SSOUserDefinedValues
    from dheera_ai.proxy.management_endpoints.ui_sso import (
        apply_user_info_values_to_sso_user_defined_values,
    )

    user_info = DheeraAI_UserTable(
        user_id="123",
...
def test_build_sso_user_update_data_with_valid_role():
    ...
    from dheera_ai.proxy.management_endpoints.types import CustomOpenID
    from dheera_ai.proxy.management_endpoints.ui_sso import _build_sso_user_update_data
...
async def test_upsert_sso_user_updates_role_for_existing_user():
    ...
    updated in the DheeraAI database on subsequent logins, not just at initial user creation.
    """
    from dheera_ai.proxy._types import DheeraAI_UserTable, LitellmUserRoles
    from dheera_ai.proxy.management_endpoints.types import CustomOpenID
    from dheera_ai.proxy.management_endpoints.ui_sso import SSOAuthenticationHandler

    # Mock prisma client
    mock_prisma = MagicMock()
    mock_prisma.db.dheera_ai_usertable.update_many = AsyncMock()

    # Existing user in DB with old role
    existing_user = DheeraAI_UserTable(
        user_id="test-user-123",
        user_email="test@example.com",
    ...
    mock_prisma.db.dheera_ai_usertable.update_many.assert_called_once()
    call_args = mock_prisma.db.dheera_ai_usertable.update_many.call_args
...
async def test_upsert_sso_user_does_not_update_invalid_role():
    ...
    If the SSO returns a role that is not a valid DheeraAI role, it should be ignored
...
async def test_get_user_info_from_db():
    """
    received args in get_user_info_from_db: {'result': CustomOpenID(id='krrishd', email='krrishdholakia@gmail.com', first_name=None, last_name=None, display_name='a3f1c107-04dc-4c93-ae60-7f32eb4b05ce', picture=None, provider=None, team_ids=[]), 'prisma_client': <dheera_ai.proxy.utils.PrismaClient object at 0x14a74e3c0>, 'user_api_key_cache': <dheera_ai.caching.dual_cache.DualCache object at 0x148d37110>, 'proxy_logging_obj': <dheera_ai.proxy.utils.ProxyLogging object at 0x148dd9090>, 'user_email': 'krrishdholakia@gmail.com', 'user_defined_values': {'models': [], 'user_id': 'krrishd', 'user_email': 'krrishdholakia@gmail.com', 'max_budget': None, 'user_role': None, 'budget_duration': None}}
    """
    from dheera_ai.proxy.management_endpoints.ui_sso import get_user_info_from_db
    ...
    with patch(
        "dheera_ai.proxy.management_endpoints.ui_sso.get_user_object"
...
async def test_check_and_update_if_proxy_admin_id():
    ...
    # Mock Prisma client
    mock_prisma = MagicMock()
    mock_prisma.db.dheera_ai_usertable.update = AsyncMock()
    ...
    with patch.dict(os.environ, {"PROXY_ADMIN_ID": test_user_id}):
        ...
        mock_prisma.db.dheera_ai_usertable.update.assert_called_once_with(
...
async def test_check_and_update_if_proxy_admin_id_already_admin():
    ...
    with patch.dict(os.environ, {"PROXY_ADMIN_ID": test_user_id}):
        ...
        # Assert
        assert updated_role == LitellmUserRoles.PROXY_ADMIN.value
        mock_prisma.db.dheera_ai_usertable.update.assert_not_called()
...
async def test_get_generic_sso_response_with_additional_headers():
    ...
    """
    from dheera_ai.proxy.management_endpoints.ui_sso import get_generic_sso_response
...
class TestAuthCallbackRouting:
    ...
    def test_cli_state_detection_and_routing(self):
        ...
        from dheera_ai.constants import DHEERA_AI_CLI_SESSION_TOKEN_PREFIX

        # Test CLI state detection logic
        cli_state = f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:sk-test123"

        # This mimics the logic in auth_callback
        if cli_state and cli_state.startswith(f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:"):
    ...
    def test_non_cli_state_routing(self):
        ...
        for state in non_cli_states:
            ...
            should_route_to_cli = state and state.startswith(
                f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:"
            )
...
class TestSSOHandlerIntegration:
    ...
    def test_get_redirect_url_for_sso(self):
        ...
        mock_request.base_url = "https://test.dheera_ai.ai/"
        ...
        assert redirect_url.startswith("https://test.dheera_ai.ai")
...
class TestSSOStateHandling:
    ...
    def test_get_cli_state_valid(self):
        ...
        state = SSOAuthenticationHandler._get_cli_state(
            source="dheera_ai-cli", key="sk-test123"
        )

        assert state is not None
        assert state.startswith("dheera_ai-session-token:")
    ...
    def test_get_cli_state_no_key(self):
        ...
        state = SSOAuthenticationHandler._get_cli_state(source="dheera_ai-cli", key=None)
    ...
    def test_get_cli_state_with_existing_key(self):
        ...
        assert state == "dheera_ai-session-token:sk-new-key-123:sk-existing-key-456"
    ...
    def test_get_cli_state_without_existing_key(self):
        ...
        state = SSOAuthenticationHandler._get_cli_state(
            source="dheera_ai-cli", key="sk-new-key-789", existing_key=None
        )
        ...
        assert state == "dheera_ai-session-token:sk-new-key-789"
...
class TestStateRouting:
    ...
    def test_cli_state_detection(self):
        ...
        cli_state = f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:sk-test123"
        assert cli_state.startswith(f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:")
    ...
    def test_cli_state_parsing_with_existing_key(self):
        ...
        cli_state = (
            f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:sk-new-key-456:sk-existing-key-789"
        )
    ...
    def test_cli_state_parsing_without_existing_key(self):
        ...
        cli_state = f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:sk-new-key-999"
    ...
    def test_non_cli_state_detection(self):
        ...
        for state in test_states:
            if state:
                assert not state.startswith(f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:")
            else:
                assert state != f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:"
...
class TestHTMLIntegration:
    ...
    def test_html_render_utils_import(self):
        ...
        from dheera_ai.proxy.common_utils.html_forms.cli_sso_success import (
...
class TestCustomUISSO:
    ...
    def test_enterprise_import_error_handling(self):
        ...
        with patch("dheera_ai.proxy.proxy_server.premium_user", True):
            with patch(
                "dheera_ai.proxy.proxy_server.user_custom_ui_sso_sign_in_handler",
                MagicMock(),
            ):
                with patch.dict(
                    "sys.modules",
                    {
                        "enterprise.dheera_ai_enterprise.proxy.auth.custom_sso_handler": None
                    },
                ):
                    ...
                    async def mock_google_login():
                        ...
                        try:
                            from enterprise.dheera_ai_enterprise.proxy.auth.custom_sso_handler import (
                            ...
                            raise ValueError(
                                "Enterprise features are not available. Custom UI SSO sign-in requires DheeraAI Enterprise."
                            )
    ...
    async def test_handle_custom_ui_sso_sign_in_success(self):
        ...
        mock_request.headers = {
            "x-dheera_ai-user-id": "test_user_123",
            "x-dheera_ai-user-email": "test@example.com",
    ...
    async def test_custom_ui_sso_handler_execution_with_real_class(self):
        ...
        class TestCustomSSOHandler(CustomSSOLoginHandler):
            ...
            async def handle_custom_ui_sso_sign_in(self, request: Request) -> OpenID:
                ...
                return OpenID(
                    id=request_headers_dict.get("x-dheera_ai-user-id", "default_user"),
                    email=request_headers_dict.get(
                        "x-dheera_ai-user-email", "default@test.com"
                    ),
                    first_name="Custom",
                    last_name="Handler",
        ...
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {
            "x-dheera_ai-user-id": "custom_test_user_456",
            "x-dheera_ai-user-email": "custom@example.com",
            "x-forwarded-for": "10.0.0.1",
        }
        mock_request.base_url = "https://custom.dheera_ai.ai/"
...
class TestCLIKeyRegenerationFlow:
    ...
    async def test_cli_sso_callback_stores_session(self):
        ...
        # Mock user info
        mock_user_info = DheeraAI_UserTable(
            user_id="test-user-123",
            user_role="internal_user",
            teams=["team1", "team2"],
            models=["gpt-4"]
        ...
        with patch(
            "dheera_ai.proxy.management_endpoints.ui_sso.get_user_info_from_db",
            return_value=mock_user_info
        ), patch(
            "dheera_ai.proxy.proxy_server.prisma_client", MagicMock()
        ), patch(
            "dheera_ai.proxy.proxy_server.user_api_key_cache", mock_cache
        ), patch(
            "dheera_ai.proxy.common_utils.html_forms.cli_sso_success.render_cli_sso_success_page",
    ...
    async def test_auth_callback_routes_to_cli_with_existing_key(self):
        ...
        cli_state = f"{DHEERA_AI_CLI_SESSION_TOKEN_PREFIX}:sk-new-session-key-456:sk-existing-cli-key-123"

        # Mock the CLI callback and required proxy server components
        mock_result = {"user_id": "test-user", "email": "test@example.com"}

        with patch(
            "dheera_ai.proxy.management_endpoints.ui_sso.cli_sso_callback"
        ) as mock_cli_callback, patch(
            "dheera_ai.proxy.proxy_server.prisma_client", MagicMock()
        ), patch(
            "dheera_ai.proxy.proxy_server.master_key", "test-master-key"
        ), patch(
            "dheera_ai.proxy.proxy_server.general_settings", {}
        ), patch(
            "dheera_ai.proxy.proxy_server.jwt_handler", MagicMock()
        ), patch(
            "dheera_ai.proxy.proxy_server.user_api_key_cache", MagicMock()
        ), patch.dict(
            os.environ, {"GOOGLE_CLIENT_ID": "test-google-id"}, clear=True
        ), patch(
            "dheera_ai.proxy.management_endpoints.ui_sso.GoogleSSOHandler.get_google_callback_response",
    ...
    def test_get_redirect_url_does_not_include_existing_key_in_url(self):
        ...
        with patch(
            "dheera_ai.proxy.utils.get_custom_url", return_value="https://test.dheera_ai.ai"
        ):
            ...
            assert "https://test.dheera_ai.ai/sso/callback" == redirect_url
    ...
    async def test_cli_poll_key_generates_jwt_with_team(self):
        ...
        with patch(
            ...
            "dheera_ai.proxy.auth.auth_checks.ExperimentalUIJWTToken.get_cli_jwt_auth_token",
            return_value=mock_jwt_token
        ) as mock_get_jwt:
            
            # Mock the user lookup
            mock_prisma.db.dheera_ai_usertable.find_unique = AsyncMock(return_value=mock_user_info)
...
async def test_get_ui_settings_includes_api_doc_base_url():
    ...
    with patch.dict(
        os.environ,
        {
            "DHEERA_AI_UI_API_DOC_BASE_URL": "https://custom.docs",
        },
    ):
        response = await get_ui_settings(mock_request)
        assert response["DHEERA_AI_UI_API_DOC_BASE_URL"] == "https://custom.docs"
...
class TestGenericResponseConvertorNestedAttributes:
    ...
    def test_generic_response_convertor_with_nested_attributes(self):
        ...
        """
        from dheera_ai.proxy.management_endpoints.ui_sso import generic_response_convertor
...
class TestGenericResponseConvertorUserRole:
    ...
    def test_generic_response_convertor_extracts_valid_user_role(self):
        """
        Test that generic_response_convertor extracts a valid DheeraAI user role
...
class TestGetGenericSSORedirectParams:
    ...
    def test_state_priority_cli_state_provided(self):
        ...
        # Arrange
        cli_state = "dheera_ai-session-token:sk-test123"
...
class TestPKCEFunctionality:
    ...
    async def test_prepare_token_exchange_parameters_with_pkce(self):
        ...
        with patch("dheera_ai.proxy.proxy_server.user_api_key_cache", mock_cache):
            # Act
...
class TestAddMissingTeamMember:
    ...
    async def test_add_missing_team_member_with_new_user_response_teams_none(self):
        ...
        """
        from dheera_ai.proxy._types import NewUserResponse
        from dheera_ai.proxy.management_endpoints.ui_sso import add_missing_team_member
        ...
        with patch(
            "dheera_ai.proxy.management_endpoints.ui_sso.create_team_member_add_task"
        ) as mock_add_task:
    ...
    @pytest.mark.asyncio
    async def test_add_missing_team_member_with_dheera_ai_user_table_empty_teams(self):
        """
        Control test: When a DheeraAI_UserTable has teams=[] (existing user, no teams),
        add_missing_team_member() should add the user to SSO teams.

        This test PASSES because DheeraAI_UserTable defaults teams to [] not None.
        """
        from dheera_ai.proxy._types import DheeraAI_UserTable
        from dheera_ai.proxy.management_endpoints.ui_sso import add_missing_team_member
    ...
    async def test_add_user_to_teams_from_sso_response_new_user(self):
        ...
        with patch(
            "dheera_ai.proxy.management_endpoints.ui_sso.add_missing_team_member"
        ) as mock_add_member:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Working case: DheeraAI_UserTable with teams=[]
            pytest.param(
                lambda uid: DheeraAI_UserTable(user_id=uid, teams=[]),
                [],
                ["team-1", "team-2"],
            ...
            pytest.param(
                lambda uid: DheeraAI_UserTable(user_id=uid, teams=["team-1"]),
                ["team-1"],
