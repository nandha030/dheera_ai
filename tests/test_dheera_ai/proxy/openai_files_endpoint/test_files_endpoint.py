...
from dheera_ai.proxy._types import DheeraAI_UserTableFiltered, UserAPIKeyAuth
from dheera_ai.proxy.hooks import get_proxy_hook
from dheera_ai.proxy.management_endpoints.internal_user_endpoints import ui_view_users
from dheera_ai.proxy.proxy_server import app
from dheera_ai.types.llms.openai import OpenAIFileObject
...
def setup_proxy_logging_object(monkeypatch, llm_router: Router) -> ProxyLogging:
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy.proxy_server.proxy_logging_obj", proxy_logging_object
    )
...
def test_mock_create_audio_file(mocker: MockerFixture, monkeypatch, llm_router: Router):
    ...
    mock_create_file = mocker.patch("dheera_ai.files.main.create_file", new=mocker.AsyncMock())
    ...
    class DummyManagedFiles(BaseFileEndpoints):
        async def acreate_file(self, llm_router, create_file_request, target_model_names_list, dheera_ai_parent_otel_span, user_api_key_dict):
            ...
            # Call the mocked dheera_ai.files.main.create_file to ensure asserts work
            await dheera_ai.files.main.create_file(
        ...
        async def afile_retrieve(self, file_id, dheera_ai_parent_otel_span):
            raise NotImplementedError("Not implemented for test")
        
        async def afile_list(self, purpose, dheera_ai_parent_otel_span):
            raise NotImplementedError("Not implemented for test")
        
        async def afile_delete(self, file_id, dheera_ai_parent_otel_span):
            raise NotImplementedError("Not implemented for test")
        
        async def afile_content(self, file_id, dheera_ai_parent_otel_span):
            raise NotImplementedError("Not implemented for test")

    # Manually add the hook to the proxy_hook_mapping
    proxy_logging_obj.proxy_hook_mapping["managed_files"] = DummyManagedFiles()

    monkeypatch.setattr("dheera_ai.proxy.proxy_server.llm_router", llm_router)
    monkeypatch.setattr(
        "dheera_ai.proxy.proxy_server.proxy_logging_obj", proxy_logging_obj
    )
...
def test_target_storage_invokes_storage_backend(
    ...
    mocker.patch(
        "dheera_ai.proxy.openai_files_endpoints.files_endpoints.StorageBackendFileService.upload_file_to_storage_backend",
        new=async_mock,
    )
...
@pytest.mark.skip(reason="function migrated to dheera_ai/proxy/hooks/managed_files.py")
def test_create_file_for_each_model(
    ...
    from dheera_ai import CreateFileRequest
    ...
    from dheera_ai.proxy.utils import ProxyLogging
    from dheera_ai.types.llms.openai import OpenAIFileObject, OpenAIFilesPurpose
