...
from dheera_ai.proxy.pass_through_endpoints.llm_provider_handlers.openai_passthrough_logging_handler import (
...
class TestOpenAIPassthroughLoggingHandler:
    ...
    def _create_mock_logging_obj(self) -> DheeraAILoggingObj:
    ...
    @patch('dheera_ai.completion_cost')
    @patch('dheera_ai.dheera_ai_core_utils.dheera_ai_logging.get_standard_logging_object_payload')
    ...
    def test_openai_passthrough_handler_with_user_tracking(self, mock_get_standard_logging, mock_completion_cost):
        ...
        # Verify user information is included in dheera_ai_params
        assert "dheera_ai_params" in result["kwargs"]
        assert "proxy_server_request" in result["kwargs"]["dheera_ai_params"]
        assert "body" in result["kwargs"]["dheera_ai_params"]["proxy_server_request"]
        assert result["kwargs"]["dheera_ai_params"]["proxy_server_request"]["body"]["user"] == "test_user_123"
    ...
    def test_build_complete_streaming_response(self):
        ...
        result = self.handler._build_complete_streaming_response(
            ...
            dheera_ai_logging_obj=self._create_mock_logging_obj(),
            model="gpt-4o",
        )
    ...
    def test_azure_passthrough_tags_metadata_model_provider(self, mock_get_standard_logging, mock_completion_cost):
        ...
        # Set up kwargs with existing dheera_ai_params containing metadata tags
        kwargs = {
            "passthrough_logging_payload": passthrough_payload,
            "model": "gpt-4o",
        ...
        # Verify metadata tags are preserved in dheera_ai_params
        assert "dheera_ai_params" in result["kwargs"]
        assert "metadata" in result["kwargs"]["dheera_ai_params"]
        assert "tags" in result["kwargs"]["dheera_ai_params"]["metadata"]
        assert result["kwargs"]["dheera_ai_params"]["metadata"]["tags"] == ["production", "azure-deployment"]
        assert result["kwargs"]["dheera_ai_params"]["metadata"]["user_id"] == "user_123"
    ...
    @patch('dheera_ai.completion_cost')
    @patch('dheera_ai.dheera_ai_core_utils.dheera_ai_logging.get_standard_logging_object_payload')
    @patch('dheera_ai.proxy.pass_through_endpoints.llm_provider_handlers.openai_passthrough_logging_handler.OpenAIPassthroughLoggingHandler.get_provider_config')
...
class TestOpenAIPassthroughIntegration:
    ...
    @patch('dheera_ai.proxy.pass_through_endpoints.llm_provider_handlers.openai_passthrough_logging_handler.OpenAIPassthroughLoggingHandler.openai_passthrough_handler')
    @pytest.mark.asyncio
    ...
    @patch('dheera_ai.cost_calculator.default_image_cost_calculator')
