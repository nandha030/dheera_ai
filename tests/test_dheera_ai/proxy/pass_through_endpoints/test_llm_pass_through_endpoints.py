...
from dheera_ai.types.passthrough_endpoints.vertex_ai import VertexPassThroughCredentials
...
class TestBaseOpenAIPassThroughHandler:
    def test_join_url_paths(self):
        ...
        result = BaseOpenAIPassThroughHandler._join_url_paths(
            base_url, path, dheera_ai.LlmProviders.OPENAI.value
        )
    ...
    @patch(
        "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
    )
    async def test_base_openai_pass_through_handler(self, mock_create_pass_through):
        ...
        result = await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
            ...
            api_key="test_api_key",
            custom_llm_provider=dheera_ai.LlmProviders.OPENAI.value,
        )
...
class TestVertexAIPassThroughHandler:
    ...
    async def test_vertex_passthrough_with_credentials(self, monkeypatch):
        ...
        """
        from dheera_ai.proxy.pass_through_endpoints.passthrough_endpoint_router import (
        ...
        monkeypatch.setattr(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.passthrough_endpoint_router",
            pass_through_router,
        )
        ...
        with mock.patch(
            "dheera_ai.llms.vertex_ai.vertex_llm_base.VertexBase.load_auth"
        ) as mock_load_auth, mock.patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
        ) as mock_create_route, mock.patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_dheera_ai_virtual_key"
        ) as mock_get_virtual_key, mock.patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.user_api_key_auth"
        ) as mock_user_auth, mock.patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_vertex_pass_through_handler"
        ) as mock_get_handler:
    ...
    async def test_vertex_passthrough_with_no_default_credentials(self, monkeypatch):
        ...
        with mock.patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.vertex_llm_base._ensure_access_token_async"
        ) as mock_ensure_token, mock.patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.vertex_llm_base._get_token_and_url"
    ...
    async def test_async_vertex_proxy_route_api_key_auth(self):
        ...
        # Mock dependencies
        mock_request = Mock()
        mock_request.headers = {"x-dheera_ai-api-key": "test-key-123"}
        mock_request.method = "POST"
        mock_response = Mock()
    ...
    def test_vertex_passthrough_handler_multimodal_embedding_response(self):
        ...
        from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import (
            Logging as DheeraAILoggingObj,
        )
        from dheera_ai.proxy.pass_through_endpoints.llm_provider_handlers.vertex_passthrough_logging_handler import (
        ...
        mock_logging_obj.dheera_ai_call_id = "test-call-id-123"
        ...
        with patch(
            "dheera_ai.llms.vertex_ai.multimodal_embeddings.transformation.VertexAIMultimodalEmbeddingConfig"
            ...
            from dheera_ai.types.utils import Embedding, EmbeddingResponse, Usage
    ...
    def test_vertex_passthrough_handler_predict_cost_tracking(self):
        ...
        with patch("dheera_ai.completion_cost") as mock_completion_cost:
...
class TestBedrockLLMProxyRoute:
    ...
    async def test_bedrock_llm_proxy_route_application_inference_profile(self):
        ...
        with patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints._read_request_body",
            return_value=mock_request_body,
        ), patch(
            "dheera_ai.proxy.common_request_processing.ProxyBaseLLMRequestProcessing",
    ...
    async def test_bedrock_error_handling_returns_actual_error(self):
        ...
        with patch(
            "dheera_ai.proxy.common_request_processing.ProxyBaseLLMRequestProcessing.base_passthrough_process_llm_request",
    ...
    async def test_bedrock_passthrough_uses_model_specific_credentials(self):
        ...
        with patch.dict(
            ...
            # Test 1: Verify get_dheera_ai_params extracts AWS credentials from kwargs
            kwargs_with_creds = {
            ...
            dheera_ai_params = get_dheera_ai_params(**kwargs_with_creds)

            # Verify credentials are extracted
            assert dheera_ai_params.get("aws_access_key_id") == model_access_key
            assert dheera_ai_params.get("aws_secret_access_key") == model_secret_key
            assert dheera_ai_params.get("aws_region_name") == model_region
            assert dheera_ai_params.get("aws_session_token") == model_session_token
            ...
            deployment_dheera_ai_params = deployment.get("dheera_ai_params", {})

            # Verify model-specific credentials are in the deployment
            assert deployment_dheera_ai_params.get("aws_access_key_id") == model_access_key
            assert deployment_dheera_ai_params.get("aws_secret_access_key") == model_secret_key
            assert deployment_dheera_ai_params.get("aws_region_name") == model_region
            assert deployment_dheera_ai_params.get("aws_session_token") == model_session_token

            # Verify environment variables are NOT in the deployment
            assert deployment_dheera_ai_params.get("aws_access_key_id") != env_access_key
            assert deployment_dheera_ai_params.get("aws_secret_access_key") != env_secret_key
            assert deployment_dheera_ai_params.get("aws_region_name") != env_region
            ...
            with patch(
                "dheera_ai.passthrough.main.llm_passthrough_route",
                new_callable=AsyncMock,
                ...
                # The key verification is that get_dheera_ai_params extracts the credentials
...
class TestLLMPassthroughFactoryProxyRoute:
    ...
    async def test_llm_passthrough_factory_proxy_route_success(self):
        ...
        with patch(
            "dheera_ai.utils.ProviderConfigManager.get_provider_model_info"
        ) as mock_get_provider, patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.passthrough_endpoint_router.get_credentials"
            ...
            mock_get_provider.assert_called_once_with(
                provider=dheera_ai.LlmProviders(LlmProviders.VLLM), model=None
            )
...
class TestVLLMProxyRoute:
    @pytest.mark.asyncio
    @patch(
        "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_request_body",
        return_value={"model": "router-model", "stream": False},
    )
    @patch(
        "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.is_passthrough_request_using_router_model",
        return_value=True,
    )
    ...
    @patch(
        "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.llm_passthrough_factory_proxy_route"
    )
...
class TestMilvusProxyRoute:
    ...
    async def test_milvus_proxy_route_success(self):
        ...
        mock_index_object.dheera_ai_params.vector_store_name = vector_store_name
        mock_index_object.dheera_ai_params.vector_store_index = vector_store_index
        ...
        with patch(
            ...
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.ProviderConfigManager.get_provider_vector_stores_config"
        ) as mock_get_config, patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.is_allowed_to_call_vector_store_endpoint"
        ) as mock_is_allowed, patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints._safe_set_request_parsed_body"
        ) as mock_safe_set, patch(
            "dheera_ai.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
        ) as mock_create_route, patch.object(
            dheera_ai, "vector_store_index_registry"
        ) as mock_index_registry, patch.object(
            dheera_ai, "vector_store_registry"
            ...
            mock_vector_registry.get_dheera_ai_managed_vector_store_from_registry_by_name.return_value = (
                mock_vector_store
            )
    ...
    async def test_milvus_proxy_route_no_index_registry(self):
        ...
        with patch(
            ...
            dheera_ai, "vector_store_index_registry", None
        ):
            mock_get_config.return_value = MagicMock()
    ...
    async def test_milvus_proxy_route_not_managed_index(self):
        ...
        with patch(
            ...
            dheera_ai, "vector_store_registry", MagicMock()
        ):
            mock_get_config.return_value = MagicMock()
            ...
            assert exc_info.value.status_code == 400
            assert (
                f"Collection {collection_name} is not a dheera_ai managed vector store index"
                in str(exc_info.value.detail)
            )
    ...
    async def test_milvus_proxy_route_no_api_base(self):
        ...
        mock_vector_store = {"dheera_ai_params": {}}  # No api_base
    ...
    async def test_milvus_proxy_route_endpoint_without_leading_slash(self):
        ...
        mock_vector_store = {"dheera_ai_params": {"api_base": api_base}}
