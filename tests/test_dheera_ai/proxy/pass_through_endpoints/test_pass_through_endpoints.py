...
async def test_pass_through_request_failure_handler():
    ...
    Critical Test: When a users pass through endpoint request fails, we must log the failure code, exception in dheera_ai spend logs.
    """
    ...
    with patch("dheera_ai.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging:
        with patch(
            "dheera_ai.llms.custom_httpx.http_handler.get_async_httpx_client"
        ) as mock_get_client:
            with patch(
                "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints.ProxyBaseLLMRequestProcessing"
            ) as mock_processing:
...
async def test_initialize_pass_through_endpoints_with_include_subpath():
    ...
    with patch(
        "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints.InitPassThroughEndpointHelpers.add_exact_path_route"
    ) as mock_add_exact_route:
        with patch(
            "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints.InitPassThroughEndpointHelpers.add_subpath_route"
            ...
            with patch(
                ...
                with patch(
                    "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints.set_env_variables_in_header"
                ) as mock_set_env:
...
async def test_create_pass_through_route_with_cost_per_request():
    ...
    with patch(
        "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints.pass_through_request"
...
async def test_pass_through_request_contains_proxy_server_request_in_kwargs():
    ...
    with patch("dheera_ai.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging:
        with patch(
            "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints.HttpPassThroughEndpointHelpers.non_streaming_http_request_handler"
        ) as mock_http_handler:
            with patch(
                "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints.ProxyBaseLLMRequestProcessing"
            ) as mock_processing:
                with patch(
                    "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints.pass_through_endpoint_logging.pass_through_async_success_handler"
                ) as mock_success_handler:
                    with patch(
                        "dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints.get_response_body"
                        ...
                        # Verify that dheera_ai_params exists in kwargs
                        assert "dheera_ai_params" in call_kwargs
                        dheera_ai_params = call_kwargs["dheera_ai_params"]

                        # Verify that proxy_server_request exists in dheera_ai_params
                        assert "proxy_server_request" in dheera_ai_params
                        proxy_server_request = dheera_ai_params["proxy_server_request"]
...
async def test_create_pass_through_endpoint():
    ...
    with patch(
        "dheera_ai.proxy.proxy_server.get_config_general_settings"
    ) as mock_get_config:
        with patch(
            "dheera_ai.proxy.proxy_server.update_config_general_settings"
...
async def test_delete_pass_through_endpoint_not_found():
    ...
    from dheera_ai.proxy._types import ConfigFieldInfo, UserAPIKeyAuth
    from dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints import (
        delete_pass_through_endpoints,
    )
...
async def test_filter_endpoints_by_team_allowed_routes_with_filter():
    ...
    """
    from dheera_ai.proxy._types import PassThroughGenericEndpoint
    from dheera_ai.proxy.pass_through_endpoints.pass_through_endpoints import (
    ...
    mock_prisma_client.db.dheera_ai_teamtable.find_unique.assert_called_once_with(
...
async def test_bedrock_router_passthrough_metadata_initialization():
    ...
    dheera_ai_params.proxy_server_request were missing for /bedrock passthrough
...
@pytest.mark.asyncio
async def test_add_dheera_ai_data_to_request_adds_headers_to_metadata():
    """
    Test that add_dheera_ai_data_to_request adds headers to metadata for guardrails.
