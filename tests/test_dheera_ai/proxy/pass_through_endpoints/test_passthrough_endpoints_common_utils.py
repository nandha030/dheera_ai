...
@pytest.mark.asyncio
async def test_get_dheera_ai_virtual_key():
    """
    Test that the get_dheera_ai_virtual_key function correctly handles the API key authentication
    """
    # Test with x-dheera_ai-api-key
    mock_request = Mock()
    mock_request.headers = {"x-dheera_ai-api-key": "test-key-123"}
    result = get_dheera_ai_virtual_key(mock_request)
    ...
    # Test with both headers (x-dheera_ai-api-key should take precedence)
    mock_request.headers = {
        "x-dheera_ai-api-key": "test-key-123",
