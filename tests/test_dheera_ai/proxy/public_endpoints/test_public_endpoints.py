...
from dheera_ai.proxy.public_endpoints import router
...
def test_get_provider_create_fields():
    ...
    assert "dheera_ai_provider" in first_provider
...
def test_get_dheera_ai_model_cost_map_returns_cost_map():
    app = FastAPI()
    app.include_router(router)
    client = TestClient(app)

    response = client.get("/public/dheera_ai_model_cost_map")
