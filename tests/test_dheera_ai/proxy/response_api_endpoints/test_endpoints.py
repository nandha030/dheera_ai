...
class TestResponsesAPIEndpoints(unittest.TestCase):
    ...
    @patch("dheera_ai.proxy.proxy_server.user_api_key_auth")
    ...
    async def test_cursor_chat_completions_route(self, mock_auth, mock_router):
        ...
        from dheera_ai.types.utils import ResponseOutputMessage, ResponseOutputText
    ...
    async def test_responses_api_key_spend_header_includes_response_cost(
        ...
        # Verify x-dheera_ai-key-spend header includes current request cost
        assert "x-dheera_ai-key-spend" in response.headers
        key_spend_value = float(response.headers["x-dheera_ai-key-spend"])
        ...
        # Verify x-dheera_ai-response-cost header is present
        assert "x-dheera_ai-response-cost" in response.headers
        response_cost_value = float(response.headers["x-dheera_ai-response-cost"])
