...
import dheera_ai
import dheera_ai.proxy.proxy_server as ps
...
from dheera_ai.proxy.proxy_server import app, prisma_client
from dheera_ai.proxy.spend_tracking import spend_management_endpoints
...
async def test_can_team_member_view_log_none_team_id():
    ...
    class MockPrisma:
        class DB:
            ...
            def __init__(self):
                self.dheera_ai_teamtable = self.TeamTable()
...
def disable_budget_sync(monkeypatch):
    ...
    monkeypatch.setattr(
        "dheera_ai.router_strategy.budget_limiter.RouterBudgetLimiting.periodic_sync_in_memory_spend_with_redis",
        noop,
    )
...
async def test_ui_view_spend_logs_with_user_id(client, monkeypatch):
    ...
    class MockPrismaClient:
        def __init__(self):
            self.db = MockDB()
            self.db.dheera_ai_spendlogs = self.db
...
async def test_ui_view_spend_logs_with_team_id(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy.spend_tracking.spend_management_endpoints._is_admin_view_safe",
...
async def test_ui_view_spend_logs_team_admin_can_view_team_spend(client, monkeypatch):
    ...
    class MockPrismaClient:
        def __init__(self):
            ...
            self.db.dheera_ai_teamtable = self
            self.dheera_ai_teamtable = self
...
class TestSpendLogsPayload:
    ...
    async def test_spend_logs_payload_e2e(self):
        dheera_ai.callbacks = [_ProxyDBLogger(message_logging=False)]
        # dheera_ai._turn_on_debug()

        with patch.object(
            dheera_ai.proxy.db.db_spend_update_writer.DBSpendUpdateWriter,
            "_insert_spend_log_to_db",
        ) as mock_client, patch.object(dheera_ai.proxy.proxy_server, "prisma_client"):
...
async def test_view_spend_tags(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy.spend_tracking.spend_management_endpoints.get_spend_by_tags",
...
async def test_provider_budget_provider_budgets(disable_budget_sync):
    ...
    with patch("dheera_ai.proxy.proxy_server.llm_router", router):
...
async def test_view_spend_logs_with_date_range_summarized(client, monkeypatch):
    ...
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.prisma_client", MockPrismaClient())
