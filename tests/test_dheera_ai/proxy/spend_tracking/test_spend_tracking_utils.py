...
import dheera_ai
from dheera_ai.constants import DHEERA_AI_TRUNCATED_PAYLOAD_FIELD, REDACTED_BY_LITELM_STRING
...
def test_sanitize_request_body_for_spend_logs_payload_long_string():
    ...
    expected_truncation_message = f"... ({DHEERA_AI_TRUNCATED_PAYLOAD_FIELD} skipped {skipped_chars} chars) ..."
...
@patch(
    "dheera_ai.proxy.spend_tracking.spend_tracking_utils._should_store_prompts_and_responses_in_spend_logs"
)
...
def test_get_response_for_spend_logs_payload_truncates_large_base64(mock_should_store):
    ...
    assert DHEERA_AI_TRUNCATED_PAYLOAD_FIELD in truncated_value
...
@patch("dheera_ai.proxy.proxy_server.master_key", None)
@patch("dheera_ai.proxy.proxy_server.general_settings", {})
...
@pytest.mark.asyncio
@patch("dheera_ai.proxy.proxy_server.master_key", "sk-master-key")
@patch("dheera_ai.proxy.proxy_server.general_settings", {})
...
def test_get_logging_payload_includes_overhead_in_spend_logs_metadata():
    """
    Test that get_logging_payload extracts dheera_ai_overhead_time_ms from hidden_params
    ...
    standard_logging_payload = StandardLoggingPayload(
        ...
        hidden_params=StandardLoggingHiddenParams(
            ...
            api_base="https://api.openai.com",
            response_cost="0.001",
            dheera_ai_overhead_time_ms=test_overhead_ms,
    ...
    assert (
        metadata.get("dheera_ai_overhead_time_ms") == test_overhead_ms
    ), f"Expected overhead '{test_overhead_ms}', got '{metadata.get('dheera_ai_overhead_time_ms')}'"
...
def test_get_logging_payload_handles_missing_overhead_gracefully():
    ...
    standard_logging_payload = StandardLoggingPayload(
        ...
        hidden_params=StandardLoggingHiddenParams(
            ...
            api_base="https://api.openai.com",
            response_cost="0.001",
            dheera_ai_overhead_time_ms=None,  # No overhead
            additional_headers=None,
            batch_models=None,
            dheera_ai_model_name=None,
            usage_object=None,
    ...
    # When overhead is None, dheera_ai_overhead_time_ms should be None or not present
    assert (
        metadata.get("dheera_ai_overhead_time_ms") is None
    ), "dheera_ai_overhead_time_ms should be None when overhead is not provided"
