...
def test_add_key_level_controls_with_none_metadata():
    ...
    result = DheeraAIProxyRequestSetup.add_key_level_controls(
        key_metadata=None,
        data=data,
...
def test_batch_create_with_dheera_ai_sdk():
    """
    Test creating a batch using dheera_ai SDK with metadata=None.
    This is a more direct test of the original issue.
    """
    # Mock the OpenAI batches instance to avoid actual API calls
    with patch('dheera_ai.batches.main.openai_batches_instance') as mock_openai_batches:
        # Mock the response
...
if __name__ == "__main__":
    ...
    test_batch_create_with_dheera_ai_sdk()
    print("✓ test_batch_create_with_dheera_ai_sdk passed")
