...
def mock_redis_success(mocker):
    ...
    mocker.patch.object(dheera_ai, "cache", mock_cache)
    return mock_cache
...
def test_cache_ping_with_complex_objects(mock_redis_success, mocker):
    ...
    assert data["status"] == "healthy"
    assert "dheera_ai_cache_params" in data

    # Verify complex objects were converted to strings
    cache_params = json.loads(data["dheera_ai_cache_params"])
...
def test_cache_ping_failure(mock_redis_failure):
    ...
    assert "dheera_ai_cache_params" in error_details
...
def test_cache_ping_no_cache_initialized():
    ...
    # Set cache to None
    original_cache = dheera_ai.cache
    ...
    assert "message" in error
    error_details = json.loads(error["message"])
    assert "Cache not initialized. dheera_ai.cache is None" in error_details["message"]

    # Restore original cache
    dheera_ai.cache = original_cache


def test_cache_ping_health_check_includes_only_cache_attributes(mock_redis_success):
    """
    Ensure that the /cache/ping endpoint only pulls HealthCheckCacheParams from dheera_ai.cache.cache,
    and not from other attributes on dheera_ai.cache.
    """
    ...
    # The unrelated field we attached at the top-level of dheera_ai.cache should *not* be present
    assert (
    ...
    ), "Expected field on `dheera_ai.cache.cache` was not found in health_check_cache_params"
