...
from dheera_ai.integrations.opentelemetry import UserAPIKeyAuth
...
class TestProxyBaseLLMRequestProcessing:
    @pytest.mark.asyncio
    async def test_common_processing_pre_call_logic_pre_call_hook_receives_dheera_ai_call_id(
        self, monkeypatch
    ):
        ...
        async def mock_add_dheera_ai_data_to_request(*args, **kwargs):
            return {}
        ...
        monkeypatch.setattr(
            dheera_ai.proxy.common_request_processing,
            "add_dheera_ai_data_to_request",
            mock_add_dheera_ai_data_to_request,
        )
        ...
        assert "dheera_ai_call_id" in data_passed
        try:
            uuid.UUID(data_passed["dheera_ai_call_id"])
        except ValueError:
            pytest.fail("dheera_ai_call_id is not a valid UUID")
        assert data_passed["dheera_ai_call_id"] == returned_data["dheera_ai_call_id"]

    @pytest.mark.asyncio
    async def test_stream_timeout_header_processing(self):
        """
        Test that x-dheera_ai-stream-timeout header gets processed and added to request data as stream_timeout.
        """
        from dheera_ai.proxy.dheera_ai_pre_call_utils import DheeraAIProxyRequestSetup

        # Test with stream timeout header
        headers_with_timeout = {"x-dheera_ai-stream-timeout": "30.5"}
        result = DheeraAIProxyRequestSetup._get_stream_timeout_from_request(headers_with_timeout)
        assert result == 30.5
        
        # Test without stream timeout header
        headers_without_timeout = {}
        result = DheeraAIProxyRequestSetup._get_stream_timeout_from_request(headers_without_timeout)
        assert result is None
        
        # Test with invalid header value (should raise ValueError when converting to float)
        headers_with_invalid = {"x-dheera_ai-stream-timeout": "invalid"}
        with pytest.raises(ValueError):
            DheeraAIProxyRequestSetup._get_stream_timeout_from_request(headers_with_invalid)

    @pytest.mark.asyncio
    async def test_add_dheera_ai_data_to_request_with_stream_timeout_header(self):
        """
        Test that x-dheera_ai-stream-timeout header gets processed and added to request data 
        when calling add_dheera_ai_data_to_request.
        """
        from dheera_ai.integrations.opentelemetry import UserAPIKeyAuth
        ...
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {"x-dheera_ai-stream-timeout": "45.0"}
        ...
        result_data = await add_dheera_ai_data_to_request(
            data=test_data,
            request=mock_request,
    ...
    def test_get_custom_headers_with_discount_info(self):
        ...
        assert "x-dheera_ai-response-cost" in headers
        assert float(headers["x-dheera_ai-response-cost"]) == 0.000095
        
        assert "x-dheera_ai-response-cost-original" in headers
        assert float(headers["x-dheera_ai-response-cost-original"]) == 0.0001
        
        assert "x-dheera_ai-response-cost-discount-amount" in headers
        assert float(headers["x-dheera_ai-response-cost-discount-amount"]) == 0.000005
    ...
    def test_get_custom_headers_without_discount_info(self):
        ...
        assert "x-dheera_ai-response-cost" in headers
        assert float(headers["x-dheera_ai-response-cost"]) == 0.0001
        
        # Discount headers should not be in the final dict
        assert "x-dheera_ai-response-cost-original" not in headers
        assert "x-dheera_ai-response-cost-discount-amount" not in headers
    ...
    def test_get_cost_breakdown_from_logging_obj_helper(self):
        ...
        logging_obj_no_discount = DheeraAILoggingObj(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "test"}],
            stream=False,
            call_type="completion",
            start_time=None,
            dheera_ai_call_id="test-call-id-2",
    ...
    def test_get_custom_headers_key_spend_includes_response_cost(self):
        """
        Test that x-dheera_ai-key-spend header includes the current request's response_cost.
        ...
        assert "x-dheera_ai-key-spend" in headers_1
        expected_spend_1 = 0.001 + 0.0005  # Initial spend + current request cost
        assert float(headers_1["x-dheera_ai-key-spend"]) == pytest.approx(expected_spend_1, abs=1e-10)
        assert float(headers_1["x-dheera_ai-response-cost"]) == response_cost_1
        ...
        assert "x-dheera_ai-key-spend" in headers_2
        expected_spend_2 = 0.001 + 0.0003  # Initial spend + current request cost
        assert float(headers_2["x-dheera_ai-key-spend"]) == pytest.approx(expected_spend_2, abs=1e-10)
        ...
        assert "x-dheera_ai-key-spend" in headers_3
        assert float(headers_3["x-dheera_ai-key-spend"]) == 0.001  # Should use original spend
        ...
        assert "x-dheera_ai-key-spend" in headers_4
        assert float(headers_4["x-dheera_ai-key-spend"]) == 0.001  # Should remain unchanged for 0 cost
        ...
        assert "x-dheera_ai-key-spend" in headers_5
        assert float(headers_5["x-dheera_ai-key-spend"]) == 0.0002  # 0.0 + 0.0002
        ...
        assert "x-dheera_ai-key-spend" in headers_6
        assert float(headers_6["x-dheera_ai-key-spend"]) == 0.001  # Should use original spend
        ...
        assert "x-dheera_ai-key-spend" in headers_7
        assert float(headers_7["x-dheera_ai-key-spend"]) == 0.001  # Should use original spend on error
...
class TestCommonRequestProcessingHelpers:
    ...
    async def test_create_streaming_response_all_chunks_have_dd_trace(self):
        ...
        with patch("dheera_ai.proxy.common_request_processing.tracer", mock_tracer):
            response = await create_streaming_response(
                mock_generator(), "text/event-stream", {}
            )
