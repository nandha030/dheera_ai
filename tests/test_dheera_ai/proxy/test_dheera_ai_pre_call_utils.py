...
import dheera_ai
from dheera_ai.proxy._types import TeamCallbackMetadata, UserAPIKeyAuth
...
def test_get_enforced_params(
    ...
    from dheera_ai.proxy.dheera_ai_pre_call_utils import _get_enforced_params
...
@pytest.mark.asyncio
async def test_add_dheera_ai_data_to_request_parses_string_metadata():
    ...
    # Call
    updated_data = await add_dheera_ai_data_to_request(
        data=data,
    ...
    # Assert
    dheera_ai_metadata = updated_data.get("metadata", {})
    assert isinstance(dheera_ai_metadata, dict)
    assert updated_data["metadata"]["generation_name"] == "gen123"


@pytest.mark.asyncio
async def test_add_dheera_ai_data_to_request_audio_transcription_multipart():
    from dheera_ai.proxy.dheera_ai_pre_call_utils import add_dheera_ai_data_to_request
    ...
    dheera_ai_metadata = updated_data.get("dheera_ai_metadata", {})
...
@pytest.mark.asyncio
async def test_add_dheera_ai_data_to_request_disabled_callbacks():
    """
    Test that dheera_ai_disabled_callbacks from key metadata is properly added to the request data.
    """
    from dheera_ai.proxy.dheera_ai_pre_call_utils import add_dheera_ai_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={"dheera_ai_disabled_callbacks": ["langfuse", "langsmith", "datadog"]},
    )
    ...
    # Call add_dheera_ai_data_to_request
    ...
    # Verify that dheera_ai_disabled_callbacks was added to the request data
    assert "dheera_ai_disabled_callbacks" in result
    assert result["dheera_ai_disabled_callbacks"] == ["langfuse", "langsmith", "datadog"]
...
@pytest.mark.asyncio
async def test_add_dheera_ai_data_to_request_disabled_callbacks_empty():
    """
    Test that dheera_ai_disabled_callbacks is not added when it's empty.
    """
    from dheera_ai.proxy.dheera_ai_pre_call_utils import add_dheera_ai_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={"dheera_ai_disabled_callbacks": []},
    )
    ...
    # Verify that dheera_ai_disabled_callbacks is not added when empty
    assert "dheera_ai_disabled_callbacks" not in result
...
@pytest.mark.asyncio
async def test_add_dheera_ai_data_to_request_disabled_callbacks_not_present():
    """
    Test that dheera_ai_disabled_callbacks is not added when it's not present in metadata.
    """
    from dheera_ai.proxy.dheera_ai_pre_call_utils import add_dheera_ai_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={},  # No dheera_ai_disabled_callbacks
    )
    ...
    # Verify that dheera_ai_disabled_callbacks is not added when not present
    assert "dheera_ai_disabled_callbacks" not in result
...
@pytest.mark.asyncio
async def test_add_dheera_ai_data_to_request_disabled_callbacks_invalid_type():
    """
    Test that dheera_ai_disabled_callbacks is not added when it's not a list.
    """
    from dheera_ai.proxy.dheera_ai_pre_call_utils import add_dheera_ai_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={"dheera_ai_disabled_callbacks": "not_a_list"},  # Should be a list
    )
    ...
    # Verify that dheera_ai_disabled_callbacks is not added when invalid type
    assert "dheera_ai_disabled_callbacks" not in result
...
@pytest.mark.asyncio
async def test_add_dheera_ai_data_to_request_disabled_callbacks_with_logging_settings():
    """
    Test that dheera_ai_disabled_callbacks works correctly alongside logging settings.
    """
    from dheera_ai.proxy.dheera_ai_pre_call_utils import add_dheera_ai_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        metadata={
            ...
            "dheera_ai_disabled_callbacks": ["langsmith", "datadog"],
        },
    )
    ...
    assert "dheera_ai_disabled_callbacks" in result
    assert result["dheera_ai_disabled_callbacks"] == ["langsmith", "datadog"]
...
def test_get_num_retries_from_request():
    """
    Test DheeraAIProxyRequestSetup._get_num_retries_from_request method
    """
    # Test case 1: Header is present with valid integer string
    headers_with_retries = {"x-dheera_ai-num-retries": "3"}
    result = DheeraAIProxyRequestSetup._get_num_retries_from_request(
    ...
    empty_headers = {}
    result = DheeraAIProxyRequestSetup._get_num_retries_from_request(empty_headers)
    assert result is None

    # Test case 4: Header present with zero value
    headers_with_zero = {"x-dheera_ai-num-retries": "0"}
    result = DheeraAIProxyRequestSetup._get_num_retries_from_request(headers_with_zero)
    assert result == 0

    # Test case 5: Header present with large number
    headers_with_large_number = {"x-dheera_ai-num-retries": "100"}
    result = DheeraAIProxyRequestSetup._get_num_retries_from_request(
    ...
    headers_multiple = {
        ...
        "x-dheera_ai-num-retries": "5",
        "Authorization": "Bearer token",
    }
    result = DheeraAIProxyRequestSetup._get_num_retries_from_request(headers_multiple)
    assert result == 5

    # Test case 7: Header present with invalid value (should raise ValueError when int() is called)
    headers_with_invalid = {"x-dheera_ai-num-retries": "invalid"}
    with pytest.raises(ValueError):
        DheeraAIProxyRequestSetup._get_num_retries_from_request(headers_with_invalid)

    # Test case 8: Header present with float string (should raise ValueError when int() is called)
    headers_with_float = {"x-dheera_ai-num-retries": "3.5"}
    with pytest.raises(ValueError):
        DheeraAIProxyRequestSetup._get_num_retries_from_request(headers_with_float)

    # Test case 9: Header present with negative number
    headers_with_negative = {"x-dheera_ai-num-retries": "-1"}
    result = DheeraAIProxyRequestSetup._get_num_retries_from_request(
...
def test_add_user_api_key_auth_to_request_metadata():
    ...
    data = {
        "model": "gpt-3.5-turbo",
        "messages": [{"role": "user", "content": "Hello"}],
        "dheera_ai_metadata": {},  # This will be the metadata variable name
    }
    ...
    metadata_variable_name = "dheera_ai_metadata"

    # Call the function
    result = DheeraAIProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
        data=data,
        user_api_key_dict=user_api_key_dict,
...
def test_add_headers_to_llm_call_by_model_group(
    ...
    """
    Test DheeraAIProxyRequestSetup.add_headers_to_llm_call_by_model_group method
    ...
    original_model_group_settings = getattr(dheera_ai, "model_group_settings", None)
    dheera_ai.model_group_settings = model_group_settings

    try:
        # Mock the add_headers_to_llm_call method to return expected headers
        expected_returned_headers = {
            "X-DheeraAI-User": "test-user",
            "X-DheeraAI-Org": "test-org",
        }
        ...
        with patch.object(
            ...
            result = DheeraAIProxyRequestSetup.add_headers_to_llm_call_by_model_group(
        ...
        dheera_ai.model_group_settings = original_model_group_settings
...
def test_add_headers_to_llm_call_by_model_group_empty_headers_returned():
    ...
    original_model_group_settings = getattr(dheera_ai, "model_group_settings", None)
    dheera_ai.model_group_settings = mock_settings
...
def test_add_headers_to_llm_call_by_model_group_existing_headers_in_data():
    ...
    try:
        new_headers = {"X-DheeraAI-User": "test-user"}
...
@pytest.mark.asyncio
async def test_add_dheera_ai_metadata_from_request_headers():
    """
    Test that add_dheera_ai_metadata_from_request_headers properly adds dheera_ai metadata from request headers,
    ...
    # Prepare test data (ensure no streaming, add mock_response and api_key to route to dheera_ai.acompletion)
    headers = {"x-dheera_ai-spend-logs-metadata": '{"user_id": "12345", "project_id": "proj_abc", "request_type": "chat_completion", "timestamp": "2025-09-02T10:30:00Z"}'}
    ...
    assert SPEND_LOGS_METADATA == dict(json.loads(headers["x-dheera_ai-spend-logs-metadata"])), "spend_logs_metadata should be the same as the headers"
...
def test_get_internal_user_header_from_mapping_returns_expected_header():
    ...
    header_name = DheeraAIProxyRequestSetup.get_internal_user_header_from_mapping(mappings)
...
def test_get_internal_user_header_from_mapping_none_when_absent():
    ...
    single = {"header_name": "X-Only-Customer", "dheera_ai_user_role": "customer"}
    header_name = DheeraAIProxyRequestSetup.get_internal_user_header_from_mapping(single)
    assert header_name is None
...
def test_add_internal_user_from_user_mapping_sets_user_id_when_header_present():
    ...
    result = DheeraAIProxyRequestSetup.add_internal_user_from_user_mapping(
...
def test_get_sanitized_user_information_from_key_includes_guardrails_metadata():
    ...
    result = DheeraAIProxyRequestSetup.get_sanitized_user_information_from_key(
        user_api_key_dict=user_api_key_dict
    )
...
async def test_team_guardrails_append_to_key_guardrails():
    ...
    with patch("dheera_ai.proxy.utils._premium_user_check"):
        updated_data = await add_dheera_ai_data_to_request(
            data=data,
            request=request_mock,
            user_api_key_dict=user_api_key_dict,
...
async def test_request_guardrails_do_not_override_key_guardrails():
    ...
    with patch("dheera_ai.proxy.utils._premium_user_check"):
        updated_data_empty = await add_dheera_ai_data_to_request(
