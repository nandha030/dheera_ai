...
def mock_prisma():
    ...
    client = MagicMock()
    client.db.dheera_ai_healthchecktable.create = AsyncMock(return_value={"id": "test-id"})
    client.db.dheera_ai_healthchecktable.find_many = AsyncMock(return_value=[{"id": "1", "model_name": "test"}])
...
async def test_save_health_check_result(mock_prisma, status, healthy, unhealthy, should_succeed):
    ...
    if not should_succeed:
        mock_prisma.db.dheera_ai_healthchecktable.create.side_effect = Exception("DB Error")
    ...
    if should_succeed:
        mock_prisma.db.dheera_ai_healthchecktable.create.assert_called_once()
    else:
...
async def test_get_health_check_history(mock_prisma):
    ...
    mock_prisma.db.dheera_ai_healthchecktable.find_many.assert_called_once()
    assert len(result) == 1
...
def test_build_model_param_to_info_mapping():
    ...
    model_list = [
        ...
        },
        {
            ...
            "dheera_ai_params": {"model": "gpt-3.5-turbo"},  # Same model param
        },
    ]
...
async def test_get_all_latest_health_checks_with_model_id(mock_prisma):
    ...
    mock_prisma.db.dheera_ai_healthchecktable.find_many = AsyncMock(
