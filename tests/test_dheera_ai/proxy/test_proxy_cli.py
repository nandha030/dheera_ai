...
from dheera_ai.proxy.health_endpoints.health_app_factory import build_health_app
from dheera_ai.proxy.proxy_cli import ProxyInitializationHelpers


class TestProxyInitializationHelpers:
    @patch("importlib.metadata.version")
    @patch("click.echo")
    def test_echo_dheera_ai_version(self, mock_echo, mock_version):
        # Setup
        ...
        # Assert
        mock_version.assert_called_once_with("dheera_ai")
        mock_echo.assert_called_once_with("\nDheeraAI: Current Version = 1.0.0\n")
    ...
    def test_get_default_unvicorn_init_args(self):
        ...
        assert args["app"] == "dheera_ai.proxy.proxy_server:app"
        ...
        with patch("dheera_ai.json_logs", True):
            args = ProxyInitializationHelpers._get_default_unvicorn_init_args(
                "localhost", 8000
            )
    ...
    def test_skip_server_startup(self, mock_print, mock_uvicorn_run):
        ...
        from dheera_ai.proxy.proxy_cli import run_server
        ...
        with patch.dict(
            ...
            "dheera_ai.proxy.proxy_cli.ProxyInitializationHelpers._get_default_unvicorn_init_args"
        ) as mock_get_args:
    ...
    def test_run_server_no_config_passed(self, mock_print, mock_uvicorn_run):
        ...
        async def mock_get_config(config_file_path=None):
            return {"general_settings": {}, "dheera_ai_settings": {}}
...
class TestHealthAppFactory:
    ...
    def test_build_health_app(self):
        ...
        # Assert
        assert health_app.title == "DheeraAI Health Endpoints"
    ...
    @patch("dheera_ai.proxy.db.prisma_client.PrismaManager.setup_database")
    @patch("dheera_ai.proxy.db.check_migration.check_prisma_schema_diff")
    @patch("dheera_ai.proxy.db.prisma_client.should_update_prisma_schema")
    @patch.dict(
