...
def test_login_v2_returns_redirect_url_and_sets_cookie(monkeypatch):
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy.auth.login_utils.authenticate_user",
        mock_authenticate_user,
    )
    monkeypatch.setattr(
        "dheera_ai.proxy.auth.login_utils.create_ui_token_object",
        mock_create_ui_token_object,
    )
    monkeypatch.setattr("jwt.encode", mock_jwt_encode)
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.master_key", "test-master-key")
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.general_settings", {})
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.premium_user", False)
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("dheera_ai.proxy.utils.get_server_root_path", lambda: "")
...
def test_sso_key_generate_shows_deprecation_banner(client_no_auth, monkeypatch):
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy.management_endpoints.ui_sso.show_missing_vars_in_env",
        lambda: None,
    )
    monkeypatch.setattr(
        "dheera_ai.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.get_redirect_url_for_sso",
        lambda *args, **kwargs: "http://test/redirect",
    )
    monkeypatch.setattr(
        "dheera_ai.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler._get_cli_state",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr(
        "dheera_ai.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.should_use_sso_handler",
...
def test_restructure_ui_html_files_handles_nested_routes(tmp_path):
    ...
    write_file(ui_root / "dheera_ai-asset-prefix" / "ignore.html", "asset")
    ...
    assert (
        (ui_root / "dheera_ai-asset-prefix" / "ignore.html").read_text()
        == "asset"
    )
...
async def test_initialize_scheduled_jobs_credentials(monkeypatch):
    ...
    with patch("dheera_ai.proxy.proxy_server.proxy_config", mock_proxy_config), patch(
        "dheera_ai.proxy.proxy_server.store_model_in_db", False
    ...
    with patch("dheera_ai.proxy.proxy_server.proxy_config", mock_proxy_config), patch(
        "dheera_ai.proxy.proxy_server.store_model_in_db", True
    ), patch("dheera_ai.proxy.proxy_server.get_secret_bool", return_value=True):
        await ProxyStartupEvent.initialize_scheduled_background_jobs(
            general_settings={},
            prisma_client=mock_prisma_client,
            proxy_budget_rescheduler_min_time=1,
...
def test_get_config_custom_callback_api_env_vars(monkeypatch):
    ...
    """
    from dheera_ai.proxy.proxy_server import app, proxy_config, user_api_key_auth

    # Mock config with custom_callback_api enabled and generic logger env vars present
    config_data = {
        "dheera_ai_settings": {"success_callback": ["custom_callback_api"]},
        "general_settings": {},
    ...
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.llm_router", mock_router)
    monkeypatch.setattr(
...
@patch(
    "dheera_ai.proxy.proxy_server.ProxyStartupEvent._setup_prisma_client",
...
async def test_aaaproxy_startup_master_key(mock_prisma, monkeypatch, tmp_path):
    ...
    from dheera_ai.proxy.proxy_server import proxy_startup_event

    # Mock the Prisma import
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.PrismaClient", MockPrisma)
    ...
    async with proxy_startup_event(app):
        from dheera_ai.proxy.proxy_server import master_key
    ...
    monkeypatch.setenv("DHEERA_AI_MASTER_KEY", test_env_master_key)
...
def test_embedding_input_array_of_tokens(mock_aembedding, client_no_auth):
    ...
    Ref: https://github.com/BerriAI/dheera_ai/issues/10113
    """
    try:
        test_data = {
...
async def test_get_all_team_models():
    ...
    from dheera_ai.proxy._types import DheeraAI_TeamTable
    from dheera_ai.proxy.proxy_server import get_all_team_models
    ...
    mock_prisma_client = MagicMock()
    mock_db = MagicMock()
    mock_dheera_ai_teamtable = MagicMock()

    mock_prisma_client.db = mock_db
    mock_db.dheera_ai_teamtable = mock_dheera_ai_teamtable

    # Make find_many async
    mock_dheera_ai_teamtable.find_many = AsyncMock()
    ...
    mock_dheera_ai_teamtable.find_many.return_value = [mock_team1, mock_team2]

    with patch("dheera_ai.proxy.proxy_server.DheeraAI_TeamTable") as mock_team_table_class:
        ...
        mock_dheera_ai_teamtable.find_many.assert_called_with()
    ...
    mock_dheera_ai_teamtable.reset_mock()
    mock_router.reset_mock()
    mock_router.get_model_list.side_effect = mock_get_model_list

    # Only return team1 for specific team query
    mock_dheera_ai_teamtable.find_many.return_value = [mock_team1]
    ...
    mock_dheera_ai_teamtable.reset_mock()
    mock_router.reset_mock()
    mock_dheera_ai_teamtable.find_many.return_value = []
    ...
    mock_dheera_ai_teamtable.find_many.assert_called_with(where={"team_id": {"in": []}})
...
def test_add_team_models_to_all_models():
    ...
    """
    from dheera_ai.proxy._types import DheeraAI_TeamTable
    from dheera_ai.proxy.proxy_server import _add_team_models_to_all_models

    team_db_objects_typed = MagicMock(spec=DheeraAI_TeamTable)
...
async def test_delete_deployment_type_mismatch():
    ...
    with patch("dheera_ai.proxy.proxy_server.llm_router", mock_llm_router), patch(
        "dheera_ai.proxy.proxy_server.user_config_file_path", "test_config.yaml"
    ):
...
async def test_get_config_from_file(tmp_path, monkeypatch):
    ...
    test_config = {
        "model_list": [{"model_name": "gpt-4", "dheera_ai_params": {"model": "gpt-4"}}],
        "general_settings": {"master_key": "sk-test"},
        "router_settings": {"enable_pre_call_checks": True},
        "dheera_ai_settings": {"drop_params": True},
    }
    ...
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.user_config_file_path", None)

    result = await proxy_config._get_config_from_file(str(config_file))
    assert result == test_config

    # Verify that user_config_file_path was set
    from dheera_ai.proxy.proxy_server import user_config_file_path
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy.proxy_server.user_config_file_path", str(config_file)
    )
...
async def test_add_proxy_budget_to_db_only_creates_user_no_keys():
    ...
    which should prevent key creation in DheeraAI_VerificationToken table.
    """
    ...
    # Set up required dheera_ai settings
    dheera_ai.budget_duration = "30d"
    dheera_ai.max_budget = 100.0
    ...
    with patch(
        "dheera_ai.proxy.proxy_server.generate_key_helper_fn", mock_generate_key_helper
    ):
        # Call the function under test
        ProxyStartupEvent._add_proxy_budget_to_db(dheera_ai_proxy_budget_name)
        ...
        assert call_args.kwargs["user_id"] == dheera_ai_proxy_budget_name
...
async def test_custom_ui_sso_sign_in_handler_config_loading():
    ...
    try:
        with patch(
            "dheera_ai.proxy.proxy_server.get_instance_fn",
            ...
            from dheera_ai.proxy.proxy_server import user_custom_ui_sso_sign_in_handler
...
async def test_load_environment_variables_direct_and_os_environ():
    ...
    with patch(
        "dheera_ai.proxy.proxy_server.get_secret_str", return_value=mock_secret_value
    ) as mock_get_secret:
        with patch.dict(
...
@pytest.mark.asyncio
async def test_load_environment_variables_dheera_ai_license_and_edge_cases():
    """
    Test _load_environment_variables method with DHEERA_AI_LICENSE special handling and edge cases
    """
    from unittest.mock import MagicMock, patch
    ...
    # Test Case 1: DHEERA_AI_LICENSE in environment_variables
    test_config_with_license = {
        "environment_variables": {
            "DHEERA_AI_LICENSE": "test_license_key",
    ...
    with patch("dheera_ai.proxy.proxy_server._license_check", mock_license_check):
        with patch.dict(os.environ, {}, clear=False):
            # Call the method under test
            proxy_config._load_environment_variables(test_config_with_license)

            # Verify DHEERA_AI_LICENSE was set in environment
            assert os.environ["DHEERA_AI_LICENSE"] == "test_license_key"
    ...
    with patch("dheera_ai.proxy.proxy_server.get_secret_str", return_value=None):
        with patch.dict(os.environ, {}, clear=False):
            # Call the method under test
...
async def test_write_config_to_file(monkeypatch):
    ...
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.store_model_in_db", True)
    ...
    monkeypatch.setattr(
        "dheera_ai.proxy.proxy_server.general_settings", mock_general_settings
    )

    # Mock user_config_file_path
    test_config_path = "/tmp/test_config.yaml"
    monkeypatch.setattr(
        "dheera_ai.proxy.proxy_server.user_config_file_path", test_config_path
    )
...
async def test_write_config_to_file_when_store_model_in_db_false(monkeypatch):
    ...
    # Set store_model_in_db to False
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.store_model_in_db", False)

    # Mock prisma_client to be None (so file path is taken)
    monkeypatch.setattr("dheera_ai.proxy.proxy_server.prisma_client", None)
...
async def test_async_data_generator_midstream_error():
    ...
    from dheera_ai.proxy.proxy_server import async_data_generator
    from dheera_ai.proxy.utils import ProxyLogging
    ...
    with patch("dheera_ai.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging_obj):
...
async def test_chat_completion_result_no_nested_none_values():
    ...
    mock_model_response = dheera_ai.ModelResponse()
    ...
    setattr(
        mock_model_response,
        "usage",
        dheera_ai.Usage(prompt_tokens=10, completion_tokens=5, total_tokens=15),
    )
    ...
    with patch(
        "dheera_ai.proxy.proxy_server._read_request_body",
        return_value={"model": "gpt-3.5-turbo", "messages": []},
    ), patch(
        "dheera_ai.proxy.proxy_server.ProxyBaseLLMRequestProcessing",
...
class TestPriceDataReloadAPI:
    ...
    def test_reload_model_cost_map_admin_access(self, client_with_auth):
        ...
        with patch(
            "dheera_ai.dheera_ai_core_utils.get_model_cost_map.get_model_cost_map"
        ) as mock_get_map:
            ...
            with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma:
                mock_prisma.db.dheera_ai_config.upsert = AsyncMock(return_value=None)
    ...
    def test_get_model_cost_map_public_access(self, client_no_auth):
        ...
        with patch(
            "dheera_ai.model_cost", {"gpt-3.5-turbo": {"input_cost_per_token": 0.001}}
        ):
            response = client_no_auth.get("/public/dheera_ai_model_cost_map")
    ...
    def test_cancel_model_cost_map_reload_admin_access(self, client_with_auth):
        ...
        with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            mock_prisma.db.dheera_ai_config.delete = AsyncMock(return_value=None)
    ...
    def test_get_model_cost_map_reload_status_admin_access(self, client_with_auth):
        ...
        with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            mock_prisma.db.dheera_ai_config.find_unique = AsyncMock(
                return_value=mock_config
            )

            # Mock the last reload time and current time
            with patch(
                "dheera_ai.proxy.proxy_server.last_model_cost_map_reload",
                "2024-01-01T06:00:00",
            ):
                with patch("dheera_ai.proxy.proxy_server.datetime") as mock_datetime:
    ...
    def test_get_model_cost_map_reload_status_no_config(self, client_with_auth):
        ...
        with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma:
            mock_prisma.db.dheera_ai_config.find_unique = AsyncMock(return_value=None)
...
class TestPriceDataReloadIntegration:
    ...
    def test_complete_reload_flow(self, client_with_auth):
        ...
        with patch(
            ...
            with patch("dheera_ai.proxy.proxy_server.prisma_client") as mock_prisma:
                ...
                # Test get endpoint
                response = client_with_auth.get("/public/dheera_ai_model_cost_map")
                assert response.status_code == 200
    ...
    def test_distributed_reload_check_function(self):
        ...
        mock_config = MagicMock()
        mock_config.param_value = {"interval_hours": 6, "force_reload": False}
        mock_prisma.db.dheera_ai_config.find_unique = AsyncMock(return_value=mock_config)
        ...
        with patch(
            ...
            mock_prisma.db.dheera_ai_config.upsert.assert_called()
            call_args = mock_prisma.db.dheera_ai_config.upsert.call_args
...
"""
    ...
    def test_database_config_storage(self):
        ...
        asyncio.run(
            mock_prisma.db.dheera_ai_config.upsert(
        ...
        mock_prisma.db.dheera_ai_config.upsert.assert_called_once()
        call_args = mock_prisma.db.dheera_ai_config.upsert.call_args
...
async def test_add_router_settings_from_db_config_merge_logic():
    ...
    # Mock prisma client
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.dheera_ai_config.find_first = AsyncMock(
    ...
    mock_prisma_client.db.dheera_ai_config.find_first.assert_called_once_with(
...
async def test_add_router_settings_from_db_config_edge_cases():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.dheera_ai_config.find_first = AsyncMock(return_value=None)
...
async def test_model_info_v1_oci_secrets_not_leaked():
    ...
    from dheera_ai.proxy._types import UserAPIKeyAuth
    from dheera_ai.proxy.proxy_server import model_info_v1
    ...
    with patch("dheera_ai.proxy.proxy_server.llm_router", mock_router), \
         patch("dheera_ai.proxy.proxy_server.llm_model_list", [mock_model_data]), \
         patch("dheera_ai.proxy.proxy_server.general_settings", {"infer_model_from_keys": False}), \
         patch("dheera_ai.proxy.proxy_server.user_model", None):
        
        # Call the model_info_v1 endpoint
        result = await model_info_v1(
            user_api_key_dict=mock_user_api_key_dict,
            dheera_ai_model_id=None
        )
        ...
        dheera_ai_params = model_info["dheera_ai_params"]
        
        # Verify that sensitive OCI fields are masked
        assert "****" in dheera_ai_params["oci_key"], "oci_key should be masked"
        assert "****" in dheera_ai_params["oci_fingerprint"], "oci_fingerprint should be masked"
        assert "****" in dheera_ai_params["oci_tenancy"], "oci_tenancy should be masked"
        assert "****" in dheera_ai_params["oci_key_file"], "oci_key_file should be masked"
        
        # Verify that non-sensitive fields are NOT masked
        assert dheera_ai_params["model"] == "oci/xai.grok-4", "model field should not be masked"
        assert dheera_ai_params["oci_region"] == "us-phoenix-1", "oci_region should not be masked"
        assert dheera_ai_params["drop_params"] is True, "drop_params should not be masked"
        
        # Verify the model field specifically is not masked (this was the original issue)
        assert "****" not in dheera_ai_params["model"], "model field should never be masked"
        assert dheera_ai_params["model"].startswith("oci/"), "model should retain its full value"
...
def test_add_callback_from_db_to_in_memory_dheera_ai_callbacks():
    """
    Test that _add_callback_from_db_to_in_memory_dheera_ai_callbacks correctly adds callbacks
    ...
    with patch("dheera_ai.proxy.proxy_server.dheera_ai") as mock_dheera_ai:
        # Set up mock dheera_ai attributes
        mock_dheera_ai._known_custom_logger_compatible_callbacks = []
        mock_dheera_ai.logging_callback_manager = mock_callback_manager
        
        # Test Case 1: Add success callback
        mock_success_callbacks = []
        proxy_config._add_callback_from_db_to_in_memory_dheera_ai_callbacks(
            callback="prometheus",
            event_types=["success"],
            existing_callbacks=mock_success_callbacks,
        )
        mock_callback_manager.add_dheera_ai_success_callback.assert_called_once_with("prometheus")
        ...
        mock_callback_manager.add_dheera_ai_failure_callback.assert_called_once_with("langfuse")
        mock_callback_manager.reset_mock()
        ...
        mock_callback_manager.add_dheera_ai_callback.assert_called_once_with("s3")
        mock_callback_manager.reset_mock()
        ...
        mock_callback_manager.add_dheera_ai_success_callback.assert_not_called()
...
def test_should_load_db_object_with_supported_db_objects():
    ...
    with patch("dheera_ai.proxy.proxy_server.general_settings", {}):
    ...
    with patch(
        "dheera_ai.proxy.proxy_server.general_settings",
...
async def test_init_sso_settings_in_db():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.dheera_ai_ssoconfig.find_unique = AsyncMock(
    ...
    with patch.object(
        ...
        mock_prisma_client.db.dheera_ai_ssoconfig.find_unique.assert_awaited_once_with(
            where={"id": "sso_config"}
        )
...
async def test_init_sso_settings_in_db_no_settings():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.dheera_ai_ssoconfig.find_unique = AsyncMock(return_value=None)
...
def test_get_prompt_spec_for_db_prompt_with_versions():
    ...
    mock_prompt_v1.model_dump.return_value = {
        ...
        "version": 1,
        "dheera_ai_params": '{"prompt_id": "chat_prompt", "prompt_integration": "dotprompt", "model": "gpt-3.5-turbo", "messages": [{"role": "user", "content": "v1 content"}]}',
    ...
    mock_prompt_v2.model_dump.return_value = {
        ...
        "prompt_id": "chat_prompt",
        "version": 2,
        "dheera_ai_params": '{"prompt_id": "chat_prompt", "prompt_integration": "dotprompt", "model": "gpt-4", "messages": [{"role": "user", "content": "v2 content"}]}',
        "prompt_info": '{"prompt_type": "db"}',
...
def test_root_redirect_when_docs_url_not_root_and_redirect_url_set(monkeypatch):
    from dheera_ai.proxy.proxy_server import cleanup_router_config_variables
    from dheera_ai.proxy.utils import _get_docs_url
...
def test_get_image_non_root_uses_tmp_assets_dir(monkeypatch):
    """
    Test that get_image uses /tmp/dheera_ai_assets when DHEERA_AI_NON_ROOT is true.
    """
    from unittest.mock import patch

    from dheera_ai.proxy.proxy_server import get_image

    # Set DHEERA_AI_NON_ROOT to true
    monkeypatch.setenv("DHEERA_AI_NON_ROOT", "true")
    monkeypatch.delenv("UI_LOGO_PATH", raising=False)

    # Mock os.path operations
    with patch("dheera_ai.proxy.proxy_server.os.makedirs") as mock_makedirs, \
         patch("dheera_ai.proxy.proxy_server.os.path.exists", return_value=True), \
         patch("dheera_ai.proxy.proxy_server.os.getenv") as mock_getenv, \
         patch("dheera_ai.proxy.proxy_server.FileResponse") as mock_file_response:

        # Setup mock_getenv to return empty string for UI_LOGO_PATH
        def getenv_side_effect(key, default=""):
            if key == "UI_LOGO_PATH":
                return ""
            elif key == "DHEERA_AI_NON_ROOT":
                return "true"
            return default
        ...
        # Verify makedirs was called with /tmp/dheera_ai_assets
        mock_makedirs.assert_called_once_with("/tmp/dheera_ai_assets", exist_ok=True)
...
def test_get_image_non_root_fallback_to_default_logo(monkeypatch):
    ...
    in /tmp/dheera_ai_assets for non-root case.
    """
    from unittest.mock import patch
    ...
    # Track path.exists calls to verify it checks /tmp/dheera_ai_assets/logo.jpg
    exists_calls = []

    def exists_side_effect(path):
        exists_calls.append(path)
        # Return False for /tmp/dheera_ai_assets/logo.jpg to trigger fallback
        if "/tmp/dheera_ai_assets/logo.jpg" in path:
            return False
        return True

    # Mock os.path operations
    with patch("dheera_ai.proxy.proxy_server.os.makedirs") as mock_makedirs, \
         patch("dheera_ai.proxy.proxy_server.os.path.exists", side_effect=exists_side_effect), \
         patch("dheera_ai.proxy.proxy_server.os.getenv") as mock_getenv, \
         patch("dheera_ai.proxy.proxy_server.FileResponse") as mock_file_response:
        ...
        # Verify that exists was called to check /tmp/dheera_ai_assets/logo.jpg
        tmp_logo_path = "/tmp/dheera_ai_assets/logo.jpg"
...
def test_get_image_root_case_uses_current_dir(monkeypatch):
    """
    Test that get_image uses current_dir when DHEERA_AI_NON_ROOT is not true.
    """
    from unittest.mock import patch

    from dheera_ai.proxy.proxy_server import get_image

    # Don't set DHEERA_AI_NON_ROOT (or set it to false)
    monkeypatch.delenv("DHEERA_AI_NON_ROOT", raising=False)
    monkeypatch.delenv("UI_LOGO_PATH", raising=False)
    ...
    with patch("dheera_ai.proxy.proxy_server.os.makedirs") as mock_makedirs, \
        ...
        # Verify makedirs was NOT called with /tmp/dheera_ai_assets (should not create it for root case)
        tmp_assets_calls = [
            call for call in mock_makedirs.call_args_list
            if "/tmp/dheera_ai_assets" in str(call)
        ]
        assert len(tmp_assets_calls) == 0, "Should not create /tmp/dheera_ai_assets for root case"
