...
async def test_cleanup_old_spend_logs_batch_deletion():
    ...
    # Wire up mocks
    mock_db.dheera_ai_spendlogs = mock_spendlogs
...
async def test_cleanup_old_spend_logs_no_retention_period():
    ...
    """
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.dheera_ai_spendlogs.find_many = AsyncMock()
    mock_prisma_client.db.dheera_ai_spendlogs.delete = AsyncMock()

    cleaner = SpendLogCleanup(general_settings={})  # no retention
    await cleaner.cleanup_old_spend_logs(mock_prisma_client)

    mock_prisma_client.db.dheera_ai_spendlogs.find_many.assert_not_called()
    mock_prisma_client.db.dheera_ai_spendlogs.delete.assert_not_called()
...
def test_cleanup_batch_size_env_var(monkeypatch):
    ...
    import dheera_ai.constants as constants_module
    import dheera_ai.proxy.db.db_transaction_queue.spend_log_cleanup as cleanup_module
