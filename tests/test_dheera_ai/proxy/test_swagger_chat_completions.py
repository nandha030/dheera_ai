...
class TestSwaggerChatCompletions:
    ...
    def test_openapi_schema_includes_chat_completions_request_body(self, client):
        ...
        # Check for DheeraAI-specific fields added by ProxyChatCompletionRequest
        expected_dheera_ai_fields = [
            "guardrails",
            "caching", 
            "num_retries",
        ...
        for field in expected_dheera_ai_fields:
            assert field in properties, f"Expected DheeraAI field '{field}' not found in ProxyChatCompletionRequest schema"
    ...
    def test_chat_completions_endpoints_have_expanded_request_body(self, client):
        ...
        # Should have DheeraAI-specific fields  
        dheera_ai_fields = ["guardrails", "caching", "fallbacks", "num_retries"]
        for field in dheera_ai_fields:
            assert field in properties, f"DheeraAI field '{field}' should be in expanded properties"
    ...
    @patch('dheera_ai.proxy.common_utils.custom_openapi_spec.CustomOpenAPISpec.add_chat_completion_request_schema')
    ...
    def test_proxy_chat_completion_request_pydantic_model_works(self):
        ...
        and includes the expected DheeraAI-specific fields.
        """
        from dheera_ai.proxy._types import ProxyChatCompletionRequest
        ...
        # Check for DheeraAI-specific fields
        dheera_ai_fields = ["guardrails", "caching", "num_retries", "context_window_fallback_dict", "fallbacks"]
        for field in dheera_ai_fields:
            assert field in properties, f"DheeraAI field '{field}' should be in ProxyChatCompletionRequest schema"
