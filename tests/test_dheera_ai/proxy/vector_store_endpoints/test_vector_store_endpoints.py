...
from dheera_ai.proxy.vector_store_endpoints.endpoints import (
    _update_request_data_with_dheera_ai_managed_vector_store_registry,
)
...
async def test_router_avector_store_search_passes_correct_args():
    ...
    to downstream dheera_ai.vector_stores.asearch() with custom_llm_provider and query.
    """
    # Create a router
    router = dheera_ai.Router(model_list=[])
...
async def test_router_avector_store_file_list_passes_correct_args():
    with patch(
        "dheera_ai.vector_store_files.main.alist",
...
def test_router_vector_store_file_delete_passes_correct_args():
    with patch(
        "dheera_ai.vector_store_files.main.delete",
...
def test_update_request_data_with_dheera_ai_managed_vector_store_registry():
    """
    Test that _update_request_data_with_dheera_ai_managed_vector_store_registry
    ...
    mock_vector_store: DheeraAI_ManagedVectorStore = {
        "vector_store_id": "test_store_id",
        "custom_llm_provider": "bedrock",
        "dheera_ai_credential_name": "test_credential",
        "dheera_ai_params": {"api_key": "test_key", "aws_region_name": "us-east-1"},
    }

    mock_registry = MagicMock()
    mock_registry.get_dheera_ai_managed_vector_store_from_registry.return_value = (
        mock_vector_store
    )

    # Test with vector store registry
    with patch.object(dheera_ai, "vector_store_registry", mock_registry):
        result = _update_request_data_with_dheera_ai_managed_vector_store_registry(
        ...
        assert result["dheera_ai_credential_name"] == "test_credential"
        assert result["api_key"] == "test_key"
        assert result["aws_region_name"] == "us-east-1"

        # Verify registry was called correctly
        mock_registry.get_dheera_ai_managed_vector_store_from_registry.assert_called_once_with(
            vector_store_id="test_store_id"
        )

    # Test with no vector store registry
    with patch.object(dheera_ai, "vector_store_registry", None):
...
class TestIsAllowedToCallVectorStoreEndpoint:
    ...
    def test_read_permission_allowed(self):
        ...
        with patch(
            "dheera_ai.proxy.vector_store_endpoints.utils.ProviderConfigManager.get_provider_vector_stores_config",
...
class TestIsAllowedToCallVectorStoreFilesEndpoint:
    ...
    def test_allows_access_with_permissions(self):
        ...
        with patch(
            "dheera_ai.proxy.vector_store_endpoints.utils.ProviderConfigManager.get_provider_vector_store_files_config",
...
async def test_vector_store_synchronization_across_instances():
    ...
    mock_prisma_client.db.dheera_ai_managedvectorstorestable.find_unique = AsyncMock(
        side_effect=mock_find_unique
    )
    mock_prisma_client.db.dheera_ai_managedvectorstorestable.find_many = AsyncMock(
        side_effect=mock_find_many
    )
    mock_prisma_client.db.dheera_ai_managedvectorstorestable.create = AsyncMock(
        side_effect=mock_create
    )
    mock_prisma_client.db.dheera_ai_managedvectorstorestable.delete = AsyncMock(
        side_effect=mock_delete
    )
    
    # Test vector store data
    test_vector_store_id = "test-sync-store-001"
    test_vector_store: DheeraAI_ManagedVectorStore = {
    ...
    await mock_prisma_client.db.dheera_ai_managedvectorstorestable.create(
    ...
    assert instance_1_registry.get_dheera_ai_managed_vector_store_from_registry(
        test_vector_store_id
    ) is not None, "Vector store should be in Instance 1's memory"
    
    # Verify it's in the database
    db_store = await mock_prisma_client.db.dheera_ai_managedvectorstorestable.find_unique(
    ...
    found_store = await instance_2_registry.get_dheera_ai_managed_vector_store_from_registry_or_db(
        vector_store_id=test_vector_store_id,
    ...
    assert instance_2_registry.get_dheera_ai_managed_vector_store_from_registry(
        test_vector_store_id
    ...
    await mock_prisma_client.db.dheera_ai_managedvectorstorestable.delete(
        where={"vector_store_id": test_vector_store_id}
    )
    ...
    db_store_after_delete = await mock_prisma_client.db.dheera_ai_managedvectorstorestable.find_unique(
        where={"vector_store_id": test_vector_store_id}
    )
