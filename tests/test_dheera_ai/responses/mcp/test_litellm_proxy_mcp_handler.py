...
from dheera_ai.types.responses.main import OutputFunctionToolCall
...
def _setup_mcp_call_environment(monkeypatch: pytest.MonkeyPatch) -> AsyncMock:
    ...
    monkeypatch.setitem(sys.modules, "dheera_ai.proxy.proxy_server", proxy_module)

    fake_manager = types.SimpleNamespace(
        call_tool=AsyncMock(return_value=_DummyMCPResult())
    )
    monkeypatch.setattr(
        "dheera_ai.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager",
        fake_manager,
    )
...
def test_deduplicate_mcp_tools_single_allowed_server():
    ...
    deduped, server_map = DheeraAI_Proxy_MCP_Handler._deduplicate_mcp_tools(
        tools,
        ["everything"],
    )
...
def test_deduplicate_mcp_tools_prefixed_names(tool_name, expected_server):
    ...
    _, server_map = DheeraAI_Proxy_MCP_Handler._deduplicate_mcp_tools(
        tools,
        ["alpha", "beta"],
    )
...
def test_extract_tool_calls_from_chat_response_handles_tool_calls():
    ...
    tool_calls = DheeraAI_Proxy_MCP_Handler._extract_tool_calls_from_chat_response(
        response
    )
...
def test_create_follow_up_messages_for_chat_appends_tool_results():
    ...
    follow_up = DheeraAI_Proxy_MCP_Handler._create_follow_up_messages_for_chat(
        original_messages,
        response,
        tool_results,
    )
...
def test_transform_mcp_tools_to_openai_uses_chat_format(monkeypatch):
    ...
    monkeypatch.setattr(
        "dheera_ai.experimental_mcp_client.tools.transform_mcp_tool_to_openai_tool",
        fake_transform_chat,
    )
    monkeypatch.setattr(
        "dheera_ai.experimental_mcp_client.tools.transform_mcp_tool_to_openai_responses_api_tool",
        fake_transform_responses,
    )

    chat_tools = DheeraAI_Proxy_MCP_Handler._transform_mcp_tools_to_openai(
        ["tool"], target_format="chat"
    )
    resp_tools = DheeraAI_Proxy_MCP_Handler._transform_mcp_tools_to_openai(["tool"])
...
def test_create_follow_up_input_handles_response_function_tool_call():
    ...
    follow_up = DheeraAI_Proxy_MCP_Handler._create_follow_up_input(
        response=response,
        tool_results=[],
        original_input=None,
    )
...
async def test_execute_tool_calls_strips_server_prefix(monkeypatch):
    ...
    await DheeraAI_Proxy_MCP_Handler._execute_tool_calls(
