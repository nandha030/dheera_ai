...
def test_logging_object_not_popped():
    """
    Test that dheera_ai_logging_obj is not popped from kwargs.
    ...
    from dheera_ai.responses import main as responses_module

    # Get the source code of the responses function
    source = inspect.getsource(responses_module.responses)

    # Check that .pop("dheera_ai_logging_obj") is NOT used
    # The bug was using kwargs.pop("dheera_ai_logging_obj") which removes it
    assert 'kwargs.pop("dheera_ai_logging_obj")' not in source, (
        "FAIL: Found kwargs.pop('dheera_ai_logging_obj') in responses() function. "
        "This causes duplicate spend logs. Use kwargs.get('dheera_ai_logging_obj') instead."
    )

    # Check that .get("dheera_ai_logging_obj") IS used
    assert 'kwargs.get("dheera_ai_logging_obj")' in source, (
        "FAIL: Expected kwargs.get('dheera_ai_logging_obj') but not found. "
...
async def test_async_no_duplicate_spend_logs():
    ...
    class SpendLogCounter(CustomLogger):
        ...
        async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
            ...
            dheera_ai_call_id = kwargs.get("dheera_ai_call_id", "")
            if dheera_ai_call_id == self.tracking_id:
                self.log_count += 1
    ...
    dheera_ai.callbacks = original_callbacks + [spend_logger]

    try:
        # Call responses API with Anthropic model using mock_response
        # Pass our unique ID as dheera_ai_call_id to track this specific request
        response = await dheera_ai.aresponses(
            ...
            dheera_ai_call_id=test_request_id,
        )
