...
def test_tag_routing_with_list_of_tags():
    ...
    """
    from dheera_ai.router_strategy.tag_based_routing import is_valid_deployment_tag
...
def test_get_tags_from_request_kwargs_various_inputs():
    ...
    # Indirect via "dheera_ai_params" - metadata inside
    assert (
        _get_tags_from_request_kwargs(
            {"dheera_ai_params": {"metadata": {"tags": ["paid"]}}}
        )
        == ["paid"]
    )
    assert _get_tags_from_request_kwargs({"dheera_ai_params": {"metadata": None}}) == []
    assert _get_tags_from_request_kwargs({"dheera_ai_params": {}}) == []

    # Alternate metadata variable name: "dheera_ai_metadata"
    assert (
        _get_tags_from_request_kwargs(
            {"dheera_ai_metadata": {"tags": ["alt"]}},
    ...
    assert (
        _get_tags_from_request_kwargs(
            {"dheera_ai_params": {"dheera_ai_metadata": {"tags": ["nested-alt"]}}},
            metadata_variable_name="dheera_ai_metadata",
        )
        == ["nested-alt"]
    )
