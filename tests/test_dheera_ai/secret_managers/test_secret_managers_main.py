...
@patch("dheera_ai.secret_managers.main.oidc_cache")
@patch("dheera_ai.secret_managers.main.HTTPHandler")
...
def test_oidc_google_cached(mock_oidc_cache):
    ...
    secret_name = "oidc/google/[invalid url, do not cite]"
    with patch("dheera_ai.HTTPHandler") as mock_http:
        result = get_secret(secret_name)
...
def test_oidc_google_failure(mock_oidc_cache):
    ...
    with patch("dheera_ai.secret_managers.main.HTTPHandler", return_value=mock_handler):
        mock_oidc_cache.get_cache.return_value = None
...
@patch("dheera_ai.secret_managers.main.get_azure_ad_token_provider")
