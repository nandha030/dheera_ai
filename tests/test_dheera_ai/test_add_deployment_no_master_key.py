...
failed when master_key was None. [https://github.com/BerriAI/dheera_ai/issues/16428]
"""
...
async def test_add_deployment_without_master_key():
    ...
    with patch("dheera_ai.proxy.proxy_server.master_key", None):
        # Mock the required dependencies
        mock_prisma_client = MagicMock(spec=PrismaClient)
        mock_prisma_client.db = MagicMock()
        mock_prisma_client.db.dheera_ai_config = MagicMock()
...
async def test_add_deployment_without_salt_key_or_master_key():
    """
    Test that add_deployment() works when both master_key and DHEERA_AI_SALT_KEY are None.
    ...
    """
    # Remove DHEERA_AI_SALT_KEY from environment
    old_salt_key = os.environ.pop("DHEERA_AI_SALT_KEY", None)
    ...
    try:
        ...
        # Restore DHEERA_AI_SALT_KEY if it was set
        if old_salt_key:
            os.environ["DHEERA_AI_SALT_KEY"] = old_salt_key
