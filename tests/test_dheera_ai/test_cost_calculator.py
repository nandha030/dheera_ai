...
from dheera_ai.types.llms.openai import OpenAIRealtimeStreamList
from dheera_ai.types.utils import ModelResponse, PromptTokensDetailsWrapper, Usage
from dheera_ai.utils import TranscriptionResponse


def test_cost_calculator_with_response_cost_in_additional_headers():
    class MockResponse(BaseModel):
        _hidden_params = {
            "additional_headers": {"llm_provider-x-dheera_ai-response-cost": 1000}
        }
...
def test_cost_calculator_with_usage():
    ...
    model_info = dheera_ai.model_cost["gemini-2.0-flash-001"]
...
def test_handle_realtime_stream_cost_calculation():
    from dheera_ai.cost_calculator import RealtimeAPITokenUsageProcessor
...
def test_azure_realtime_cost_calculator():
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        dheera_ai_model_name="my-custom-azure-deployment",
    )
...
def test_cost_calculator_with_cache_creation():
    ...
    assert dheera_ai_model_response.usage.prompt_tokens_details.cached_tokens == 28491
...
def test_bedrock_cost_calculator_comparison_with_without_cache():
    ...
    from dheera_ai import completion_cost
    from dheera_ai.types.utils import Choices, Message, PromptTokensDetailsWrapper, Usage
...
def test_gemini_25_explicit_caching_cost_direct_usage():
    ...
    a 75% discount.
    """
    from dheera_ai.dheera_ai_core_utils.llm_cost_calc.utils import generic_cost_per_token
    from dheera_ai.types.utils import (
        CompletionTokensDetailsWrapper,
        PromptTokensDetailsWrapper,
        Usage,
...
def test_cost_discount_vertex_ai():
    ...
    original_discount_config = dheera_ai.cost_discount_config.copy()
    ...
    dheera_ai.cost_discount_config = {}
    ...
    dheera_ai.cost_discount_config = {"vertex_ai": 0.05}
    ...
    dheera_ai.cost_discount_config = original_discount_config
