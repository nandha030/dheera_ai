...
from dheera_ai._lazy_imports import (
    ...
    _lazy_import_dheera_ai_logging,
    _lazy_import_utils,
...
def _clear_names_from_globals(names: tuple):
    """Clear all names from dheera_ai globals."""
    for name in names:
        if name in dheera_ai.__dict__:
            del dheera_ai.__dict__[name]
...
def _verify_only_requested_name_imported(name: str, all_names: tuple):
    ...
    for other_name in all_names:
        if other_name != name:
            assert other_name not in dheera_ai.__dict__, f"{other_name} should not be imported when importing {name}"
...
def test_cost_calculator_lazy_imports():
    ...
    for name in COST_CALCULATOR_NAMES:
        ...
        assert name in dheera_ai.__dict__
...
def test_dheera_ai_logging_lazy_imports():
    """Test that all dheera_ai_logging items can be lazy imported."""
    # Test each name individually - only that name should be imported
    for name in DHEERA_AI_LOGGING_NAMES:
        # Clear all names before importing just one
        _clear_names_from_globals(DHEERA_AI_LOGGING_NAMES)
        
        item = _lazy_import_dheera_ai_logging(name)
        assert item is not None
        assert name in dheera_ai.__dict__
        
        # Verify only the requested name is in globals, not the others
        _verify_only_requested_name_imported(name, DHEERA_AI_LOGGING_NAMES)
...
def test_unknown_attribute_raises_error():
    ...
    with pytest.raises(AttributeError):
        _lazy_import_dheera_ai_logging("unknown")
