...
from dheera_ai import main as dheera_ai_main
...
def test_strip_input_examples_for_non_anthropic_providers():
    ...
    assert not dheera_ai_main._should_allow_input_examples(
        custom_llm_provider="openai", model="gpt-4o-mini"
    )

    cleaned = dheera_ai_main._drop_input_examples_from_tools(tools=tools)
...
async def test_extra_body_with_fallback(
    ...
    """
    test regression for https://github.com/BerriAI/dheera_ai/issues/8425.
...
async def test_openai_env_base(
    ...
    response = await dheera_ai.acompletion(model=model, messages=messages)
...
def test_responses_api_bridge_check_strips_responses_prefix():
    ...
    from dheera_ai.main import responses_api_bridge_check

    with patch("dheera_ai.main._get_model_info_helper") as mock_get_model_info:
...
def test_stream_chunk_builder_thinking_blocks():
    ...
    from dheera_ai.types.utils import Delta, ModelResponseStream, StreamingChoices
...
from dheera_ai.llms.openai.openai import OpenAIChatCompletion
...
async def test_retrying() -> None:
    ...
    with (
        ...
        pytest.raises(dheera_ai.InternalServerError, match="DheeraAI Retried: 10 times"),
    ):
...
def test_anthropic_disable_url_suffix_env_var():
    """Test that DHEERA_AI_ANTHROPIC_DISABLE_URL_SUFFIX prevents /v1/messages suffix."""
    import os
    from unittest.mock import MagicMock, patch
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_BASE": "https://api.example.com"}):
        ...
        with patch("dheera_ai.main.anthropic_chat_completions") as mock_anthropic:
    ...
    with patch.dict(
        os.environ,
        {
            ...
            "DHEERA_AI_ANTHROPIC_DISABLE_URL_SUFFIX": "true",
        },
    ):
        actual_api_base = None
...
def test_anthropic_text_disable_url_suffix_env_var():
    """Test that DHEERA_AI_ANTHROPIC_DISABLE_URL_SUFFIX prevents /v1/complete suffix for anthropic_text."""
    import os
    from unittest.mock import MagicMock, patch
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_BASE": "https://api.example.com"}):
        ...
        with patch("dheera_ai.main.base_llm_http_handler") as mock_handler:
...
def test_image_edit_merges_headers_and_extra_headers():
    ...
    with (
        patch(
            "dheera_ai.images.main.ProviderConfigManager.get_provider_image_edit_config",
            return_value=mock_image_edit_config,
        ) as mock_config,
        patch(
            "dheera_ai.images.main.base_llm_http_handler.image_edit_handler",
            return_value="ok",
        ) as mock_handler,
    ):
...
class TestCallTypesOCR:
    ...
    Fixes https://github.com/BerriAI/dheera_ai/issues/17381
    """
