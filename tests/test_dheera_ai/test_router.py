...
import dheera_ai
from dheera_ai.router_utils.fallback_event_handlers import run_async_fallback
...
def test_update_kwargs_does_not_mutate_defaults_and_merges_metadata():
    ...
    router.default_dheera_ai_params = {
        "foo": "bar",
        "metadata": {"baz": 123},
    }
    original = copy.deepcopy(router.default_dheera_ai_params)
    kwargs: dict = {}

    # invoke the helper
    router._update_kwargs_with_default_dheera_ai_params(
        kwargs=kwargs,
        metadata_variable_name="dheera_ai_metadata",
    )

    # 1) router.defaults must be unchanged
    assert router.default_dheera_ai_params == original

    # 2) non‑metadata keys get merged
    assert kwargs["foo"] == "bar"

    # 3) metadata lands under "metadata"
    assert kwargs["dheera_ai_metadata"] == {"baz": 123}
...
async def test_async_router_acreate_file():
    ...
    router = dheera_ai.Router(
        model_list=[
            ...
            {"model_name": "gpt-3.5-turbo", "dheera_ai_params": {"model": "gpt-4o-mini"}},
        ],
    )

    with patch("dheera_ai.acreate_file", return_value=MagicMock()) as mock_acreate_file:
...
async def test_arouter_async_get_healthy_deployments():
    ...
    assert result[0]["dheera_ai_params"]["model"] == "gpt-3.5-turbo"


@pytest.mark.asyncio
@patch("dheera_ai.amoderation")
async def test_arouter_amoderation_with_credential_name(mock_amoderation):
    """
    Test that router.amoderation passes dheera_ai_credential_name to the underlying dheera_ai.amoderation call
    """
        ...
        model_list=[
            {
                ...
                "dheera_ai_params": {
                    ...
                    "dheera_ai_credential_name": "my-custom-auth",
                },
            },
        ],
    )
    ...
    assert call_kwargs["dheera_ai_credential_name"] == "my-custom-auth"
...
def test_arouter_ignore_invalid_deployments():
    ...
    router = dheera_ai.Router(
        model_list=[
            {
                "model_name": "gpt-3.5-turbo",
                "dheera_ai_params": {"model": "my-bad-model"},
            },
        ],
    ...
    router.upsert_deployment(
        Deployment(
            model_name="gpt-3.5-turbo",
            dheera_ai_params={"model": "my-bad-model"},  # type: ignore
...
async def test_arouter_aretrieve_batch():
    ...
    with patch.object(
        dheera_ai, "aretrieve_batch", return_value=AsyncMock()
...
async def test_arouter_aretrieve_file_content():
    ...
    with patch.object(
        dheera_ai, "afile_content", return_value=AsyncMock()
...
async def test_arouter_filter_team_based_models():
    ...
    router.add_deployment(
        Deployment(
            model_name="gpt-3.5-turbo",
            dheera_ai_params={"model": "gpt-3.5-turbo"},  # type: ignore
            model_info={"tpm": 1000, "rpm": 1000},
        )
    )
...
def test_arouter_responses_api_bridge():
    ...
    with patch.object(dheera_ai, "responses", return_value=AsyncMock()) as mock_responses:
...
async def test_acompletion_streaming_iterator():
    ...
    router = dheera_ai.Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "dheera_ai_params": {"model": "gpt-4", "api_key": "fake-key-1"},
            },
            {
                "model_name": "gpt-3.5-turbo",
                "dheera_ai_params": {"model": "gpt-3.5-turbo", "api_key": "fake-key-2"},
            },
        ],
...
async def test_acompletion_streaming_iterator_edge_cases():
    ...
    router = dheera_ai.Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "dheera_ai_params": {"model": "gpt-4", "api_key": "fake-key"},
            }
        ],
        set_verbose=True,
    )
...
def test_get_deployment_model_info_base_model_flow():
    ...
    mock_dheera_ai_model_name_info = {
        "key": "test-model",
        "max_tokens": 2048,
        ...
        "dheera_ai_provider": "test_provider",
        "mode": "completion",
    ...
    with patch.object(
        dheera_ai, "model_cost", {"test-custom-model": mock_custom_model_info}
    ):
        with patch.object(dheera_ai, "get_model_info") as mock_get_model_info:
            # Configure mock returns
            mock_get_model_info.side_effect = lambda model: {
                "gpt-3.5-turbo": mock_base_model_info,
                "test-model": mock_dheera_ai_model_name_info,
            }.get(model)
            ...
            # 2. The result of step 1 gets merged into dheera_ai_model_name_info (custom+base override dheera_ai)
            ...
            assert result["dheera_ai_provider"] == "openai"  # From base model
            assert (
                result["mode"] == "chat"
            )  # From base model (overrides dheera_ai "completion")

            # The key field comes from base model since both base and dheera_ai have it
            # and base model info overrides dheera_ai model name info in final merge
            assert (
                result["key"] == "gpt-3.5-turbo"
            )  # From base model (overrides dheera_ai key)
    ...
    with patch.object(
        ...
        with patch.object(dheera_ai, "get_model_info") as mock_get_model_info:
            ...
            assert result["max_tokens"] == 2048  # From dheera_ai model name info
            assert result["custom_field"] == "custom_value"  # From custom model
            assert result["mode"] == "completion"  # From dheera_ai model name info

    # Test Case 3: No custom model info, only dheera_ai model name info
    with patch.object(dheera_ai, "model_cost", {}):  # Empty model cost
        with patch.object(dheera_ai, "get_model_info") as mock_get_model_info:
            mock_get_model_info.side_effect = lambda model: {
                "test-model": mock_dheera_ai_model_name_info,
            }.get(model)
            ...
            # Result should be just the dheera_ai model name info
            assert result is not None
            assert result == mock_dheera_ai_model_name_info
        ...
        with patch.object(dheera_ai, "get_model_info") as mock_get_model_info:
            ...
            def mock_get_model_info_side_effect(model):
                if model == "invalid-base-model":
                    ...
                    return mock_dheera_ai_model_name_info
                return None
    ...
    with patch.object(dheera_ai, "model_cost", {}):
        with patch.object(
            dheera_ai, "get_model_info", side_effect=Exception("Not found")
        ):
            result = router.get_deployment_model_info(
...
def test_get_deployment_model_info_base_model_merge_priority():
    ...
    mock_dheera_ai_model_name_info = {
        ...
        "mode": "completion",
        "dheera_ai_only_field": "dheera_ai_value",
    }

    with patch.object(
        dheera_ai, "model_cost", {"custom-model-id": mock_custom_model_info}
    ):
        with patch.object(dheera_ai, "get_model_info") as mock_get_model_info:
            mock_get_model_info.side_effect = lambda model: {
            ...
            # 3. Result from steps 1-2 overrides dheera_ai_model_name_info
            ...
            assert (
                result["dheera_ai_provider"] == "openai"
            )  # From base model (not in custom)
            assert (
                result["base_only_field"] == "base_value"
            )  # From base model (not in custom)

            # Fields that should come from dheera_ai model name info (not overridden by custom+base)
            assert (
                result["mode"] == "completion"
            )  # From dheera_ai model name info (not in custom or base)
            assert (
                result["dheera_ai_only_field"] == "dheera_ai_value"
            )  # From dheera_ai model name info (not in custom or base)

            # Key comes from base model since both base and dheera_ai have key fields
            # and the merged custom+base overrides dheera_ai in the final merge
...
async def test_router_acompletion_with_unknown_model_and_default_fallback():
    ...
    router = dheera_ai.Router(model_list=model_list, default_fallbacks=["gpt-4o"])
...
async def test_router_acompletion_with_unknown_model_and_no_fallback():
    ...
    with pytest.raises(dheera_ai.BadRequestError) as excinfo:
...
def test_get_deployment_credentials_with_provider_aws_bedrock_runtime_endpoint():
    ...
    aws_bedrock_runtime_endpoint from deployment dheera_ai_params to credentials.
    """
    router = dheera_ai.Router(
        model_list=[
            {
