...
import dheera_ai
from dheera_ai.proxy.utils import is_valid_api_key
...
def test_get_optional_params_image_gen():
    from dheera_ai.llms.azure.image_generation import AzureGPTImageGenerationConfig
...
def test_all_model_configs():
    from dheera_ai.llms.vertex_ai.vertex_ai_partner_models.ai21.transformation import (
        VertexAIAi21Config,
    )
    from dheera_ai.llms.vertex_ai.vertex_ai_partner_models.llama3.transformation import (
    ...
    from dheera_ai.llms.nvidia_nim.chat.transformation import NvidiaNimConfig
    ...
    from dheera_ai.llms.ollama.chat.transformation import OllamaChatConfig
    ...
    from dheera_ai.llms.predibase.chat.transformation import PredibaseConfig
    ...
    from dheera_ai.llms.codestral.completion.transformation import (
    ...
    from dheera_ai.llms.volcengine.chat.transformation import (
    ...
    from dheera_ai.llms.ai21.chat.transformation import AI21ChatConfig
    ...
    from dheera_ai import AmazonAnthropicClaudeConfig, AmazonAnthropicConfig
    ...
    from dheera_ai.llms.databricks.chat.transformation import DatabricksConfig
    ...
    from dheera_ai.llms.vertex_ai.vertex_ai_partner_models.anthropic.transformation import (
    ...
    from dheera_ai.llms.gemini.chat.transformation import GoogleAIStudioGeminiConfig
...
def test_anthropic_web_search_in_model_info():
    ...
    for model in supported_models:
        from dheera_ai.utils import get_model_info
...
def test_cohere_embedding_optional_params():
    from dheera_ai import get_optional_params_embeddings
    ...
    INTENDED_SCHEMA = {
        ...
        "additionalProperties": {
            ...
            "properties": {
                ...
                "dheera_ai_provider": {"type": "string"},
...
def test_get_model_info_gemini():
    ...
    model_map = dheera_ai.model_cost
...
def test_openai_models_in_model_info():
    ...
    for model, info in model_map.items():
        if (
            info.get("dheera_ai_provider") == "openai"
...
def test_supports_tool_choice_simple_tests():
    ...
    """
    assert dheera_ai.utils.supports_tool_choice(model="gpt-4o") == True
    assert (
        dheera_ai.utils.supports_tool_choice(
    ...
    assert (
        dheera_ai.utils.supports_tool_choice(model="us.amazon.nova-micro-v1:0") is False
    )
    assert (
        dheera_ai.utils.supports_tool_choice(model="bedrock/us.amazon.nova-micro-v1:0")
        is False
    )
    assert (
        dheera_ai.utils.supports_tool_choice(
    ...
    assert dheera_ai.utils.supports_tool_choice(model="perplexity/sonar") is False
...
def test_check_provider_match():
    ...
    model_info = {"dheera_ai_provider": "bedrock"}
    assert dheera_ai.utils._check_provider_match(model_info, "bedrock") is True
    assert dheera_ai.utils._check_provider_match(model_info, "bedrock_converse") is True

    # Test bedrock_converse provider
    model_info = {"dheera_ai_provider": "bedrock_converse"}
    assert dheera_ai.utils._check_provider_match(model_info, "bedrock") is True
    assert dheera_ai.utils._check_provider_match(model_info, "bedrock_converse") is True

    # Test non-matching provider
    model_info = {"dheera_ai_provider": "bedrock"}
    assert dheera_ai.utils._check_provider_match(model_info, "openai") is False
...
def test_get_provider_rerank_config():
    ...
    """
    from dheera_ai import HostedVLLMRerankConfig
    from dheera_ai.utils import LlmProviders, ProviderConfigManager
...
def test_supports_computer_use_utility():
    """
    Tests the dheera_ai.utils.supports_computer_use utility function.
    """
    from dheera_ai.utils import supports_computer_use

    # Ensure DHEERA_AI_LOCAL_MODEL_COST_MAP is set for consistent test behavior,
    # as supports_computer_use relies on get_model_info.
    # This also requires dheera_ai.model_cost to be populated.
    original_env_var = os.getenv("DHEERA_AI_LOCAL_MODEL_COST_MAP")
    original_model_cost = getattr(dheera_ai, "model_cost", None)

    os.environ["DHEERA_AI_LOCAL_MODEL_COST_MAP"] = "True"
    dheera_ai.model_cost = dheera_ai.get_model_cost_map(url="")  # Load with local/backup
    ...
    try:
        ...
        if original_env_var is None:
            del os.environ["DHEERA_AI_LOCAL_MODEL_COST_MAP"]
        else:
            os.environ["DHEERA_AI_LOCAL_MODEL_COST_MAP"] = original_env_var

        if original_model_cost is not None:
            dheera_ai.model_cost = original_model_cost
        elif hasattr(dheera_ai, "model_cost"):
            delattr(dheera_ai, "model_cost")
...
def test_get_model_info_shows_supports_computer_use():
    ...
    """
    os.environ["DHEERA_AI_LOCAL_MODEL_COST_MAP"] = "True"
    # Ensure dheera_ai.model_cost is loaded, relying on the backup mechanism if primary fails
    ...
    info = dheera_ai.get_model_info(model_known_to_support_computer_use)
    ...
    info_gpt = dheera_ai.get_model_info(model_known_not_to_support_computer_use)
...
def test_pre_process_non_default_params(model, custom_llm_provider):
    ...
    from dheera_ai.utils import ProviderConfigManager, pre_process_non_default_params
...
class TestProxyFunctionCalling:
    ...
    @pytest.mark.parametrize(
        ...
        [
            # OpenAI models
            ("gpt-3.5-turbo", "dheera_ai_proxy/gpt-3.5-turbo", True),
            ("gpt-4", "dheera_ai_proxy/gpt-4", True),
            ("gpt-4o", "dheera_ai_proxy/gpt-4o", True),
            ("gpt-4o-mini", "dheera_ai_proxy/gpt-4o-mini", True),
            ("gpt-4-turbo", "dheera_ai_proxy/gpt-4-turbo", True),
            ("gpt-4-1106-preview", "dheera_ai_proxy/gpt-4-1106-preview", True),
            # Azure OpenAI models
            ("azure/gpt-4", "dheera_ai_proxy/azure/gpt-4", True),
            ("azure/gpt-3.5-turbo", "dheera_ai_proxy/azure/gpt-3.5-turbo", True),
            (
                "azure/gpt-4-1106-preview",
                "dheera_ai_proxy/azure/gpt-4-1106-preview",
                True,
            ),
            # Anthropic models (Claude supports function calling)
            (
                "claude-3-5-sonnet-20240620",
                "dheera_ai_proxy/claude-3-5-sonnet-20240620",
                True,
            ),
            # Google models
            ("gemini-pro", "dheera_ai_proxy/gemini-pro", True),
            ("gemini/gemini-1.5-pro", "dheera_ai_proxy/gemini/gemini-1.5-pro", True),
            ("gemini/gemini-1.5-flash", "dheera_ai_proxy/gemini/gemini-1.5-flash", True),
            # Groq models (mixed support)
            ("groq/gemma-7b-it", "dheera_ai_proxy/groq/gemma-7b-it", True),
            (
                "groq/llama-3.3-70b-versatile",
                "dheera_ai_proxy/groq/llama-3.3-70b-versatile",
                True,
            ),
            # Cohere models (generally don't support function calling)
            ("command-nightly", "dheera_ai_proxy/command-nightly", False),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # These will return False because DheeraAI cannot determine the underlying model
            (
                "dheera_ai_proxy/bedrock-claude-3-haiku",
                "bedrock/anthropic.claude-3-haiku-20240307-v1:0",
                False,
            ),
            (
                "dheera_ai_proxy/bedrock-claude-3-sonnet",
                "bedrock/anthropic.claude-3-sonnet-20240229-v1:0",
                False,
            ),
            (
                "dheera_ai_proxy/bedrock-claude-3-opus",
                "bedrock/anthropic.claude-3-opus-20240229-v1:0",
                False,
            ),
            (
                "dheera_ai_proxy/bedrock-claude-instant",
                "bedrock/anthropic.claude-instant-v1",
                False,
            ),
            (
                "dheera_ai_proxy/bedrock-titan-text",
                "bedrock/amazon.titan-text-express-v1",
                False,
            ),
            # Azure with custom deployment names (cannot be resolved)
            ("dheera_ai_proxy/my-gpt4-deployment", "azure/gpt-4", False),
            ("dheera_ai_proxy/production-gpt35", "azure/gpt-3.5-turbo", False),
            ("dheera_ai_proxy/dev-gpt4o", "azure/gpt-4o", False),
            # Custom OpenAI deployments (cannot be resolved)
            ("dheera_ai_proxy/company-gpt4", "gpt-4", False),
            ("dheera_ai_proxy/internal-gpt35", "gpt-3.5-turbo", False),
            # Vertex AI with custom names (cannot be resolved)
            ("dheera_ai_proxy/vertex-gemini-pro", "vertex_ai/gemini-1.5-pro", False),
            ("dheera_ai_proxy/vertex-gemini-flash", "vertex_ai/gemini-1.5-flash", False),
            # Anthropic with custom names (cannot be resolved)
            ("dheera_ai_proxy/claude-prod", "anthropic/claude-3-sonnet-20240229", False),
            ("dheera_ai_proxy/claude-dev", "anthropic/claude-3-haiku-20240307", False),
            # Groq with custom names (cannot be resolved)
            ("dheera_ai_proxy/fast-llama", "groq/llama-3.1-8b-instant", False),
            ("dheera_ai_proxy/groq-gemma", "groq/gemma-7b-it", False),
            # Cohere with custom names (cannot be resolved)
            ("dheera_ai_proxy/cohere-command", "cohere/command-r", False),
            ("dheera_ai_proxy/cohere-command-plus", "cohere/command-r-plus", False),
            # Together AI with custom names (cannot be resolved)
            (
                "dheera_ai_proxy/together-llama",
                "together_ai/meta-llama/Llama-2-70b-chat-hf",
                False,
            ),
            (
                "dheera_ai_proxy/together-mistral",
                "together_ai/mistralai/Mistral-7B-Instruct-v0.1",
                False,
            ),
            # Ollama with custom names (cannot be resolved)
            ("dheera_ai_proxy/local-llama", "ollama/llama2", False),
            ("dheera_ai_proxy/local-mistral", "ollama/mistral", False),
        ],
    )
    def test_proxy_custom_model_names_without_config(
        ...
        Without proxy configuration context, DheeraAI cannot resolve custom model names
    ...
    def test_proxy_model_resolution_with_custom_names_documentation(self):
        ...
        custom_model = "dheera_ai_proxy/my-custom-claude"
        ...
        resolvable_model = "dheera_ai_proxy/claude-sonnet-4-5-20250929"
        ...
        ✅ WORKS (with current fallback logic):
           - dheera_ai_proxy/gpt-4
           - dheera_ai_proxy/claude-sonnet-4-5-20250929
           - dheera_ai_proxy/anthropic/claude-3-haiku-20240307
           
        ❌ DOESN'T WORK (requires proxy server config):
           - dheera_ai_proxy/my-custom-gpt4
           - dheera_ai_proxy/bedrock-claude-3-haiku
           - dheera_ai_proxy/production-model
           
        💡 SOLUTION: Use DheeraAI proxy server with proper model_list configuration
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("dheera_ai_proxy/gpt-4-with-functions", True),  # Hints at GPT-4
            ("dheera_ai_proxy/claude-3-haiku-prod", True),  # Hints at Claude 3 Haiku
            (
                "dheera_ai_proxy/bedrock-anthropic-claude-3-sonnet",
                True,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("dheera_ai_proxy/gpt-3.5-turbo", True),
            ("dheera_ai_proxy/gpt-4", True),
            ("dheera_ai_proxy/gpt-4o", True),
            ("dheera_ai_proxy/claude-3-5-sonnet-20240620", True),
            ("dheera_ai_proxy/gemini/gemini-1.5-pro", True),
            # Test proxy models that should not support function calling
            ("dheera_ai_proxy/command-nightly", False),
            ("dheera_ai_proxy/anthropic.claude-instant-v1", False),
        ],
    )
    ...
    def test_dheera_ai_utils_supports_function_calling_import(self):
        """Test that supports_function_calling can be imported from dheera_ai.utils."""
        try:
            from dheera_ai.utils import supports_function_calling
    ...
    def test_dheera_ai_supports_function_calling_import(self):
        """Test that supports_function_calling can be imported from dheera_ai directly."""
        try:
            import dheera_ai

            assert hasattr(dheera_ai, "supports_function_calling")
            assert callable(dheera_ai.supports_function_calling)
        except Exception as e:
            pytest.fail(f"Failed to access dheera_ai.supports_function_calling: {e}")

    @pytest.mark.parametrize(
        "model_name",
        [
            "dheera_ai_proxy/gpt-3.5-turbo",
            "dheera_ai_proxy/gpt-4",
            "dheera_ai_proxy/claude-3-5-sonnet-20240620",
            "dheera_ai_proxy/gemini/gemini-1.5-pro",
        ],
    )
    ...
    def test_edge_cases_and_malformed_proxy_models(self):
        ...
        test_cases = [
            ("dheera_ai_proxy/", False),  # Empty model name after proxy prefix
            ("dheera_ai_proxy", False),  # Just the proxy prefix without slash
            ("dheera_ai_proxy//gpt-3.5-turbo", False),  # Double slash
            ("dheera_ai_proxy/nonexistent-model", False),  # Non-existent model
        ]
    ...
    def test_proxy_model_resolution_demonstration(self):
        ...
        proxy_model = "dheera_ai_proxy/gpt-3.5-turbo"
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "dheera_ai_proxy/bedrock-claude-3-5-sonnet",
            ...
            (
                "dheera_ai_proxy/bedrock-claude-v2",
                "bedrock/anthropic.claude-v2",
                False,
                "Bedrock Claude v2 Legacy API",
            ),
            (
                "dheera_ai_proxy/bedrock-claude-v2-1",
            ...
            (
                "dheera_ai_proxy/prod-claude-haiku",
                "bedrock/converse/anthropic.claude-3-haiku-20240307-v1:0",
                False,
                "Production Claude Haiku",
            ),
            (
                "dheera_ai_proxy/dev-claude-sonnet",
                "bedrock/converse/anthropic.claude-3-sonnet-20240229-v1:0",
                False,
                "Development Claude Sonnet",
            ),
            (
                "dheera_ai_proxy/staging-claude-opus",
                "bedrock/converse/anthropic.claude-3-opus-20240229-v1:0",
                False,
                "Staging Claude Opus",
            ),
            (
                "dheera_ai_proxy/cost-optimized-claude",
                "bedrock/converse/anthropic.claude-3-haiku-20240307-v1:0",
                False,
                "Cost-optimized Claude deployment",
            ),
            (
                "dheera_ai_proxy/high-performance-claude",
                "bedrock/converse/anthropic.claude-3-opus-20240229-v1:0",
                False,
            ...
            (
                "dheera_ai_proxy/us-east-claude",
                "bedrock/converse/anthropic.claude-3-sonnet-20240229-v1:0",
                False,
                "US East Claude deployment",
            ),
            (
                "dheera_ai_proxy/eu-west-claude",
                "bedrock/converse/anthropic.claude-3-haiku-20240307-v1:0",
                False,
                "EU West Claude deployment",
            ),
            (
                "dheera_ai_proxy/ap-south-llama",
                "bedrock/converse/meta.llama3-70b-instruct-v1:0",
                False,
    ...
    def test_bedrock_converse_api_proxy_mappings(
        ...
        and cannot be resolved by DheeraAI without that context.
        """
        print(f"\nTesting: {description}")
    ...
    def test_real_world_proxy_config_documentation(self):
        ...
        3. DheeraAI evaluates the underlying model's capabilities
        4. Returns: True (because Claude 3 Haiku supports function calling)
        
        Without the proxy server configuration context, DheeraAI cannot resolve
...
def test_register_model_with_scientific_notation():
    ...
    dheera_ai.register_model(model_cost_dict)

    registered_model = dheera_ai.model_cost["my-custom-model"]
    print(registered_model)
    assert registered_model["input_cost_per_token"] == 3e-07
    assert registered_model["output_cost_per_token"] == 6e-07
    assert registered_model["dheera_ai_provider"] == "openai"
...
def test_anthropic_claude_4_invoke_chat_provider_config():
    ...
    from dheera_ai.llms.bedrock.chat.invoke_transformations.anthropic_claude3_transformation import (
...
def test_bedrock_application_inference_profile():
    ...
    from dheera_ai.utils import supports_tool_choice
...
def test_generate_gcp_iam_access_token():
    ...
    with patch.dict(
        ...
        from dheera_ai._redis import _generate_gcp_iam_access_token
...
def test_model_info_for_vertex_ai_deepseek_model():
    ...
    assert model_info is not None
    assert model_info["dheera_ai_provider"] == "vertex_ai-deepseek_models"
...
class TestGetValidModelsWithCLI:
    ...
    def test_get_valid_models_with_cli_pattern(self):
        """Test get_valid_models with dheera_ai_proxy provider and CLI token pattern"""
        ...
        mock_response.json.return_value = {
            "data": [
                ...
                {"id": "dheera_ai_proxy/gemini/gemini-2.5-flash", "object": "model"},
        ...
        with patch.object(
            ...
            result = dheera_ai.get_valid_models(
            ...
            assert len(result) == 4
            # All models get prefixed with "dheera_ai_proxy/" by the get_models method
            assert "dheera_ai_proxy/gpt-3.5-turbo" in result
            assert "dheera_ai_proxy/gpt-4" in result
            # Note: This model already had the prefix, so it gets double-prefixed
            assert "dheera_ai_proxy/dheera_ai_proxy/gemini/gemini-2.5-flash" in result
            assert "dheera_ai_proxy/claude-3-sonnet" in result
