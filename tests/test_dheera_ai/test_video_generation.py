...
import dheera_ai
from dheera_ai.cost_calculator import default_video_cost_calculator
from dheera_ai.integrations.custom_logger import CustomLogger
from dheera_ai.dheera_ai_core_utils.dheera_ai_logging import Logging as LitellmLogging
...
from dheera_ai.types.videos.main import VideoObject, VideoResponse
from dheera_ai.videos.main import (
    avideo_generation,
    avideo_status,
    video_generation,
    video_status,
...
class TestVideoGeneration:
    ...
    def test_video_generation_basic(self):
        ...
        with patch('dheera_ai.videos.main.base_llm_http_handler') as mock_handler:
    ...
    def test_video_generation_cost_calculation(self):
        ...
        with open(cost_map_path, "r") as f:
            dheera_ai.model_cost = json.load(f)
    ...
    def test_video_generation_uses_api_key_from_dheera_ai_params(self):
        """Test that video generation handler uses api_key from dheera_ai_params when function parameter is None."""
        ...
        with patch.object(config, 'validate_environment') as mock_validate:
            ...
            with patch.object(config, 'transform_video_create_request') as mock_transform:
                ...
                with patch(
                    "dheera_ai.llms.custom_httpx.llm_http_handler._get_httpx_client",
                    ...
                    handler.video_generation_handler(
                        ...
                        custom_llm_provider="openai",
                        dheera_ai_params={"api_key": "deployment-api-key", "api_base": "https://api.openai.com/v1"},
                        logging_obj=MagicMock(),
                        timeout=5.0,
                ...
                # Verify validate_environment was called with api_key from dheera_ai_params
    ...
    def test_video_generation_unsupported_parameters(self):
        ...
        from dheera_ai.videos.utils import VideoGenerationRequestUtils
...
class TestVideoLogging:
    ...
    async def test_video_generation_logging(self):
        ...
        with patch('dheera_ai.videos.main.base_llm_http_handler') as mock_handler:
            ...
            response = await dheera_ai.avideo_generation(
...
def test_video_content_handler_uses_get_for_openai():
    ...
    with patch(
        ...
        result = handler.video_content_handler(
            ...
            custom_llm_provider="openai",
            dheera_ai_params=GenericDheeraAIParams(api_base="https://api.openai.com/v1"),
            logging_obj=MagicMock(),
            timeout=5.0,
            api_key="sk-test",
            _is_async=False,
...
def test_video_content_respects_api_base_and_api_key_from_kwargs():
    ...
    from dheera_ai.videos.main import video_content
    
    # Mock the handler to capture dheera_ai_params
    captured_dheera_ai_params = None
    
    def capture_dheera_ai_params(*args, **kwargs):
        nonlocal captured_dheera_ai_params
        captured_dheera_ai_params = kwargs.get("dheera_ai_params")
        return b"mp4-bytes"
    
    with patch('dheera_ai.videos.main.base_llm_http_handler') as mock_handler:
        mock_handler.video_content_handler = capture_dheera_ai_params
    ...
    # Verify that api_base and api_key from kwargs were included in dheera_ai_params
    assert captured_dheera_ai_params is not None
    assert captured_dheera_ai_params.get("api_base") == "https://test-resource.openai.azure.com/"
    assert captured_dheera_ai_params.get("api_key") == "test-api-key-from-db"
    assert result == b"mp4-bytes"
...
def test_encode_video_id_with_provider_handles_azure_video_prefix():
    ...
    """
    from dheera_ai.types.videos.utils import (
...
class TestVideoEndpointsProxyLitellmParams:
    """Test that video proxy endpoints (status, content, remix) respect dheera_ai_params from proxy config."""

    @pytest.fixture
    def client_with_vertex_config(self, monkeypatch):
        """Create a test client with a proxy config that includes Vertex AI model with dheera_ai_params."""
        import asyncio
        import tempfile
        import yaml
        from fastapi import FastAPI
        from fastapi.testclient import TestClient
        from dheera_ai.proxy.proxy_server import cleanup_router_config_variables, router, initialize
    ...
    def mock_video_generation_response(self):
        ...
        from dheera_ai.types.videos.utils import encode_video_id_with_provider
    ...
    @pytest.mark.asyncio
    async def test_video_status_respects_dheera_ai_params(
        self, client_with_vertex_config, mock_video_generation_response, mock_video_status_response
    ):
        """Test that video_status endpoint uses dheera_ai_params from proxy config."""
        ...
        with patch("dheera_ai.proxy.proxy_server.llm_router", mock_router_instance):
            with patch("dheera_ai.proxy.common_request_processing.route_request", side_effect=create_mock_coroutine) as mock_route_request:
    ...
    @pytest.mark.asyncio
    async def test_video_content_respects_dheera_ai_params(
        self, client_with_vertex_config, mock_video_generation_response, mock_video_content_response
    ):
        """Test that video_content endpoint uses dheera_ai_params from proxy config."""
        from unittest.mock import AsyncMock, MagicMock, patch
