...
from dheera_ai.vector_stores.main import search
...
def test_get_credentials_for_vector_store():
    ...
    vector_store_1 = DheeraAI_ManagedVectorStore(
        vector_store_id="test_id_1",
        custom_llm_provider="openai",
        vector_store_name="test_store_1",
        dheera_ai_credential_name="test_creds_1",
        created_at=datetime.now(timezone.utc),
        updated_at=datetime.now(timezone.utc),
    )

    vector_store_2 = DheeraAI_ManagedVectorStore(
        vector_store_id="test_id_2",
        custom_llm_provider="bedrockc",
        vector_store_name="test_store_2",
        dheera_ai_credential_name="test_creds_2",
        created_at=datetime.now(timezone.utc),
        updated_at=datetime.now(timezone.utc),
    )
    ...
    with patch(
        "dheera_ai.dheera_ai_core_utils.credential_accessor.CredentialAccessor.get_credential_values"
    ) as mock_get_creds:
...
def test_add_vector_store_to_registry():
    ...
    existing_store_1 = DheeraAI_ManagedVectorStore(
    ...
    existing_store_2 = DheeraAI_ManagedVectorStore(
    ...
    new_store = DheeraAI_ManagedVectorStore(
    ...
    duplicate_store = DheeraAI_ManagedVectorStore(
...
def test_search_uses_registry_credentials():
    ...
    try:
        ...
        with patch.object(
            ...
            "dheera_ai.vector_stores.main.ProviderConfigManager.get_provider_vector_stores_config",
            return_value=MagicMock(),
        ), patch(
            "dheera_ai.vector_stores.main.base_llm_http_handler.vector_store_search_handler",
            return_value={},
        ) as mock_handler:
            search(vector_store_id="vs1", query="test", dheera_ai_logging_obj=logger)
            mock_get_creds.assert_called_once_with("vs1")
            called_params = mock_handler.call_args.kwargs["dheera_ai_params"]
