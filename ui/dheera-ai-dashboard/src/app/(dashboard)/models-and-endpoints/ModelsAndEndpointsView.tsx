...
const ModelsAndEndpointsView: React.FC<ModelDashboardProps> = ({
  ...
  for (let i = 0; i < modelData.data.length; i++) {
    ...
    let dheera_ai_model_name = curr_model?.dheera_ai_params?.model;
    let custom_llm_provider = curr_model?.dheera_ai_params?.custom_llm_provider;
    ...
    const getProviderFromModel = (model: string) => {
       ...
       * - return it's dheera_ai_provider, if so
       */
      if (modelMap !== null && modelMap !== undefined) {
        if (typeof modelMap == "object" && model in modelMap) {
          return modelMap[model]["dheera_ai_provider"];
        }
      }
      return "openai";
    };

    // Check if dheera_ai_model_name is null or undefined
    if (dheera_ai_model_name) {
      // Split dheera_ai_model_name based on "/"
      let splitModel = dheera_ai_model_name.split("/");
      ...
      if (!provider) {
        provider = splitModel.length === 1 ? getProviderFromModel(dheera_ai_model_name) : firstElement;
      }
    } else {
      // dheera_ai_model_name is null or undefined, default provider to openai
      provider = "-";
    }
    ...
    if (curr_model?.dheera_ai_params) {
      cleanedLitellmParams = Object.fromEntries(
        Object.entries(curr_model?.dheera_ai_params).filter(([key]) => key !== "model" && key !== "api_base"),
      );
    }
    ...
    modelData.data[i].dheera_ai_model_name = dheera_ai_model_name;
    ...
    modelData.data[i].api_base = curr_model?.dheera_ai_params?.api_base;
