...
const AdvancedSettings: React.FC<AdvancedSettingsProps> = ({
  ...
  const handlePassThroughChange = (checked: boolean) => {
    const currentParams = form.getFieldValue("dheera_ai_extra_params");
    try {
      ...
      if (Object.keys(paramsObj).length > 0) {
        form.setFieldValue("dheera_ai_extra_params", JSON.stringify(paramsObj, null, 2));
      } else {
        form.setFieldValue("dheera_ai_extra_params", "");
      }
    } catch (error) {
      // If JSON parsing fails, only create new object if checked is true
      if (checked) {
        form.setFieldValue("dheera_ai_extra_params", JSON.stringify({ use_in_pass_through: true }, null, 2));
      } else {
        form.setFieldValue("dheera_ai_extra_params", "");
      }
    }
      ...
      <Accordion className="mt-2 mb-4">
        ...
        <AccordionBody>
          <div className="bg-white rounded-lg">
            ...
            <Form.Item
              ...
              tooltip={
                <span>
                  ...
                  <Link href="https://docs.dheera_ai.ai/docs/pass_through/vertex_ai" target="_blank">
                    Learn more
                  </Link>
                </span>
              }
            ...
            />
            <Form.Item
              label="DheeraAI Params"
              name="dheera_ai_extra_params"
              tooltip="Optional dheera_ai params used for making a dheera_ai.completion() call."
              className="mb-4 mt-4"
            ...
            <Row className="mb-4">
              ...
              <Col span={10}>
                <Text className="text-gray-600 text-sm">
                  Pass JSON of dheera_ai supported params{" "}
                  <Link href="https://docs.dheera_ai.ai/docs/completion/input" target="_blank">
