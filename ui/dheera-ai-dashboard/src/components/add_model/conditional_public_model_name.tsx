...
const ConditionalPublicModelName: React.FC = () => {
  ...
  useEffect(() => {
    if (customModelName && selectedModels.includes("custom")) {
      ...
      const updatedMappings = currentMappings.map((mapping: any) => {
        if (mapping.public_name === "custom" || mapping.dheera_ai_model === "custom") {
          if (selectedProvider === Providers.Azure) {
            return {
              public_name: customModelName,
              dheera_ai_model: `azure/${customModelName}`,
            };
          }
          return {
            public_name: customModelName,
            dheera_ai_model: customModelName,
          };
        }
        return mapping;
      });
    ...
    if (selectedModels.length > 0 && !selectedModels.includes("all-wildcard")) {
      ...
      const shouldUpdateMappings =
        ...
        !selectedModels.every((model) =>
          currentMappings.some((mapping: { public_name: string; dheera_ai_model: string }) => {
            if (model === "custom") {
              return mapping.dheera_ai_model === "custom" || mapping.dheera_ai_model === customModelName;
            }
            if (selectedProvider === Providers.Azure) {
              return mapping.dheera_ai_model === `azure/${model}`;
            }
            return mapping.dheera_ai_model === model;
          }),
        );
      ...
      if (shouldUpdateMappings) {
        const mappings = selectedModels.map((model: string) => {
          ...
          if (selectedProvider === Providers.Azure) {
            return {
              public_name: model,
              dheera_ai_model: `azure/${model}`,
            };
          }
          return {
            public_name: model,
            dheera_ai_model: model,
          };
        });
  ...
  const publicNameTooltipContent = (
    <>
      <div className="mb-2 font-normal">The name you specify in your API calls to DheeraAI Proxy</div>
      <div className="mb-2 font-normal">
        ...
        <code className="bg-gray-700 px-1 py-0.5 rounded text-xs">openai/qwen-plus-latest</code> as the DheeraAI model
      </div>
      <div className="mb-2 font-normal">
        <strong>Usage:</strong> You make an API call to the DheeraAI proxy with{" "}
        <code className="bg-gray-700 px-1 py-0.5 rounded text-xs">model = &quot;example-name&quot;</code>
      </div>
      <div className="font-normal">
        <strong>Result:</strong> DheeraAI sends{" "}
        <code className="bg-gray-700 px-1 py-0.5 rounded text-xs">qwen-plus-latest</code> to the provider
      </div>
    </>
  );

  const liteLLMModelTooltipContent = <div>The model name DheeraAI will send to the LLM API</div>;
  ...
  return (
    <>
      <Form.Item
        ...
        name="model_mappings"
        tooltip="Map public model names to DheeraAI model names for load balancing"
        labelCol={{ span: 10 }}
        wrapperCol={{ span: 16 }}
        labelAlign="left"
        rules={[
