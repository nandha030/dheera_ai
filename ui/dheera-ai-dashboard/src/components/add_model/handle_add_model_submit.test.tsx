...
describe("prepareModelAddRequest", () => {
  it("returns deployment data for the most basic form", async () => {
      ...
      model_mappings: [
        {
          ...
          dheera_ai_model: "dheera_ai/public",
        },
      ],
    ...
    expect(deployment.dheera_aiParamsObj.model).toBe("custom-model-name");
    expect(deployment.dheera_aiParamsObj.input_cost_per_token).toBe(2);
    expect(deployment.dheera_aiParamsObj.output_cost_per_token).toBe(1);
  ...
  it("uses a lowercase fallback for unrecognized custom providers", async () => {
    ...
    expect(deployments).toHaveLength(1);
    const [deployment] = deployments!;
    expect(deployment.dheera_aiParamsObj.custom_llm_provider).toBe("petals");
  });
});
