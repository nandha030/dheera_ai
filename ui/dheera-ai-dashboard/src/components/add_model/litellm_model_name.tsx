...
interface DheeraAIModelNameFieldProps {
  selectedProvider: Providers;
  providerModels: string[];
  getPlaceholder: (provider: Providers) => string;
}

const DheeraAIModelNameField: React.FC<DheeraAIModelNameFieldProps> = ({
  selectedProvider,
  providerModels,
  getPlaceholder,
}) => {
      ...
      ? [
          {
            ...
            dheera_ai_model: `azure/${deploymentName}`,
          },
        ]
      : [];
      ...
      if (mapping.public_name === "custom" || mapping.dheera_ai_model === "custom") {
        if (selectedProvider === Providers.Azure) {
          return {
            ...
            dheera_ai_model: `azure/${customName}`,
          };
        }
        return {
          public_name: customName,
          dheera_ai_model: customName,
        };
      }
      return mapping;
    });
  ...
  return (
    <>
      <Form.Item
        label="DheeraAI Model Name(s)"
        tooltip="The model name DheeraAI will send to the LLM API"
        className="mb-0"
      >
        <Form.Item
          name="model"
      ...
      <Row>
        ...
        <Col span={14}>
          <Text className="mb-3 mt-1">
            {selectedProvider === Providers.Azure
              ? "Your deployment name will be saved as the public model name, and DheeraAI will use 'azure/deployment-name' internally"
              : "The model name DheeraAI will send to the LLM API"}
          </Text>
        </Col>
      </Row>
    </>
...
export default DheeraAIModelNameField;
