...
const ModelConnectionTest: React.FC<ModelConnectionTestProps> = ({
  ...
  const testModelConnection = async () => {
    ...
    try {
      ...
      const { dheera_aiParamsObj, modelInfoObj, modelName: returnedModelName } = result[0];

      const response = await testConnectionRequest(accessToken, dheera_aiParamsObj, modelInfoObj, modelInfoObj?.mode);
      if (response.status === "success") {
        ...
        setRawRequest(dheera_aiParamsObj);
  ...
  const getCleanErrorMessage = (errorMsg: string) => {
    ...
    const cleanedError = mainError.replace(/^dheera_ai\.(.*?)Error: /, "");
  ...
  return (
    <div style={{ padding: "24px", borderRadius: "8px", backgroundColor: "#fff" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Button type="link" href="https://docs.dheera_ai.ai/docs/providers" target="_blank" icon={<InfoCircleOutlined />}>
          View Documentation
        </Button>
      </div>
    </div>
