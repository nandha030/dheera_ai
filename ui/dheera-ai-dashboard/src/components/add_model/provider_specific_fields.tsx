...
export const createCredentialFromModel = (provider: string, modelData: any): CredentialItem => {
  ...
  providerFields.forEach((field) => {
    const value = modelData.dheera_ai_params[field.key];
...
const ProviderSpecificFields: React.FC<ProviderSpecificFieldsProps> = ({ selectedProvider, uploadProps }) => {
  ...
  const cacheEntries = React.useMemo(() => {
    ...
    providerMetadata.forEach((providerInfo) => {
      ...
      if (providerInfo.dheera_ai_provider) {
        entries[providerInfo.dheera_ai_provider] = mappedFields;
      }
    });
    return entries;
  }, [providerMetadata]);
  ...
  const allFields = React.useMemo(() => {
    ...
    const providerInfo = providerMetadata.find(
      (p) =>
        ...
        p.dheera_ai_provider === selectedProvider,
    );
    if (!providerInfo) {
      return [];
    }
    ...
    if (providerInfo.dheera_ai_provider) {
      providerFieldsByDisplayName[providerInfo.dheera_ai_provider] = mapped;
    }
    return mapped;
            ...
            <Row>
              ...
              <Col span={10}>
                <Text className="mb-2">
                  ...
                  <Link
                    href="https://github.com/BerriAI/dheera_ai/blob/main/model_prices_and_context_window.json"
                    target="_blank"
                  >
                    here
                  </Link>
