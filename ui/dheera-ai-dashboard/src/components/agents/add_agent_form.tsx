  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      if (agentType === "a2a") {
        ...
        // but need to add dheera_ai_params from the agent type config
        agentData = buildAgentDataFromForm(values);
        
        // Merge dheera_ai_params_template
        if (selectedAgentTypeInfo.dheera_ai_params_template) {
          agentData.dheera_ai_params = {
            ...agentData.dheera_ai_params,
            ...selectedAgentTypeInfo.dheera_ai_params_template,
          };
        }
        
        // Add credential fields to dheera_ai_params
        for (const field of selectedAgentTypeInfo.credential_fields) {
          const value = values[field.key];
          if (value && field.include_in_dheera_ai_params !== false) {
            agentData.dheera_ai_params[field.key] = value;
          }
        }
