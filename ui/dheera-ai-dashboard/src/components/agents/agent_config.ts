...
export const buildAgentDataFromForm = (values: any, existingAgent?: any) => {
  ...
  if (Object.keys(params).length > 0) {
    agentData.dheera_ai_params = params;
  }
...
export const parseAgentForForm = (agent: any) => {
  ...
  return {
    ...
    model: agent.dheera_ai_params?.model,
    make_public: agent.dheera_ai_params?.make_public,
    cost_per_query: agent.dheera_ai_params?.cost_per_query,
    input_cost_per_token: agent.dheera_ai_params?.input_cost_per_token,
    output_cost_per_token: agent.dheera_ai_params?.output_cost_per_token,
  };
};
