...
export const buildDynamicAgentData = (
  ...
  // Build dheera_ai_params from template
  const dheera_aiParams: Record<string, any> = {
    ...(agentTypeInfo.dheera_ai_params_template || {}),
  };

  // Add credential fields marked with include_in_dheera_ai_params
  for (const field of agentTypeInfo.credential_fields) {
    const value = values[field.key];
    if (value && field.include_in_dheera_ai_params !== false) {
      dheera_aiParams[field.key] = value;
    }
  }

  // Add cost configuration
  if (values.cost_per_query) {
    dheera_aiParams.cost_per_query = parseFloat(values.cost_per_query);
  }
  if (values.input_cost_per_token) {
    dheera_aiParams.input_cost_per_token = parseFloat(values.input_cost_per_token);
  }
  if (values.output_cost_per_token) {
    dheera_aiParams.output_cost_per_token = parseFloat(values.output_cost_per_token);
  }
