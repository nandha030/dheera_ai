...
const EditAutoRouterModal: React.FC<EditAutoRouterModalProps> = ({
  ...
  const initializeForm = () => {
    try {
      ...
      if (modelData.dheera_ai_params?.auto_router_config) {
        if (typeof modelData.dheera_ai_params.auto_router_config === "string") {
          parsedConfig = JSON.parse(modelData.dheera_ai_params.auto_router_config);
        } else {
          parsedConfig = modelData.dheera_ai_params.auto_router_config;
        }
      }
      ...
      form.setFieldsValue({
        ...
        auto_router_default_model: modelData.dheera_ai_params?.auto_router_default_model || "",
        auto_router_embedding_model: modelData.dheera_ai_params?.auto_router_embedding_model || "",
      ...
      setShowCustomDefaultModel(!allModelGroups.has(modelData.dheera_ai_params?.auto_router_default_model));
      setShowCustomEmbeddingModel(!allModelGroups.has(modelData.dheera_ai_params?.auto_router_embedding_model));
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Prepare the updated dheera_ai_params
      const updatedLitellmParams = {
        ...modelData.dheera_ai_params,
