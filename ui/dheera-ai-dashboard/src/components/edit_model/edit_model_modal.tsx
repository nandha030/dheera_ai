...
export const handleEditModelSubmit = async (
  ...
  let newDheeraAIParams: Record<string, any> = {};
  ...
  for (const [key, value] of Object.entries(formValues)) {
    if (key !== "model_id") {
      ...
      newDheeraAIParams[key] = value === "" ? null : value;
    } else {
  ...
  let payload: {
    dheera_ai_params: Record<string, any> | undefined;
    model_info: { id: any } | undefined;
  } = {
    dheera_ai_params: Object.keys(newDheeraAIParams).length > 0 ? newDheeraAIParams : undefined,
    model_info:
...
const EditModelModal: React.FC<EditModelModalProps> = ({ visible, onCancel, model, onSubmit }) => {
  ...
  let dheera_ai_params_to_edit: Record<string, any> = {};
  let model_name = "";
  let model_id = "";
  if (model) {
    dheera_ai_params_to_edit = {
      ...model.dheera_ai_params,
      input_cost_per_token: model.dheera_ai_params?.input_cost_per_token
        ? model.dheera_ai_params.input_cost_per_token * 1_000_000
        : undefined,
      output_cost_per_token: model.dheera_ai_params?.output_cost_per_token
        ? model.dheera_ai_params.output_cost_per_token * 1_000_000
        : undefined,
    };
    ...
    if (model_info) {
      ...
      dheera_ai_params_to_edit.model_id = model_id;
    }
  }
  ...
  return (
    <Modal
      title={"Edit '" + model_name + "' DheeraAI Params"}
      visible={visible}
      width={800}
      footer={null}
      onOk={handleOk}
    ...
    >
      <Form
        form={form}
        onFinish={onSubmit}
        initialValues={dheera_ai_params_to_edit}
        labelCol={{ span: 8 }}
        wrapperCol={{ span: 16 }}
        labelAlign="left"
      >
