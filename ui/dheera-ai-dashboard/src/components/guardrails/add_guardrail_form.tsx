  ...
  const handleSubmit = async () => {
    try {
      ...
      if (values.provider === "PresidioPII" && selectedEntities.length > 0) {
        ...
        guardrailData.dheera_ai_params.pii_entities_config = piiEntitiesConfig;

        // Add Presidio API bases if provided
        if (values.presidio_analyzer_api_base) {
          guardrailData.dheera_ai_params.presidio_analyzer_api_base = values.presidio_analyzer_api_base;
        }
        if (values.presidio_anonymizer_api_base) {
          guardrailData.dheera_ai_params.presidio_anonymizer_api_base = values.presidio_anonymizer_api_base;
        }
      }
      ...
      if (shouldRenderContentFilterConfigSettings(values.provider)) {
        if (selectedPatterns.length > 0) {
          guardrailData.dheera_ai_params.patterns = selectedPatterns.map((p) => ({
        ...
        if (blockedWords.length > 0) {
          guardrailData.dheera_ai_params.blocked_words = blockedWords.map((w) => ({
            keyword: w.keyword,
            action: w.action,
      ...
      // Add config values to the guardrail_info if provided
        ...
        try {
          ...
          // For some guardrails, the config values need to be in dheera_ai_params
      ...
      if (guardrailProvider === "tool_permission") {
        ...
        guardrailData.dheera_ai_params.rules = toolPermissionConfig.rules;
        guardrailData.dheera_ai_params.default_action = toolPermissionConfig.default_action;
        guardrailData.dheera_ai_params.on_disallowed_action = toolPermissionConfig.on_disallowed_action;
        if (toolPermissionConfig.violation_message_template) {
          guardrailData.dheera_ai_params.violation_message_template = toolPermissionConfig.violation_message_template;
        }
      }
      ...
      if (providerParams && selectedProvider) {
        ...
        allowedParams.forEach((paramName) => {
          ...
          if (paramValue !== undefined && paramValue !== null && paramValue !== "") {
            guardrailData.dheera_ai_params[paramName] = paramValue;
          }
        });
      }
