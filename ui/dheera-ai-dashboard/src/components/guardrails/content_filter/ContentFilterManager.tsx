...
const ContentFilterManager: React.FC<ContentFilterManagerProps> = ({
  ...
  useEffect(() => {
    if (guardrailData?.dheera_ai_params?.patterns) {
      const patterns = guardrailData.dheera_ai_params.patterns.map((p: any, index: number) => ({
        id: `pattern-${index}`,
    ...
    if (guardrailData?.dheera_ai_params?.blocked_words) {
      const words = guardrailData.dheera_ai_params.blocked_words.map((w: any, index: number) => ({
        id: `word-${index}`,
        keyword: w.keyword,
