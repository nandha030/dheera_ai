...
const EditGuardrailForm: React.FC<EditGuardrailFormProps> = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Prepare the guardrail data with proper types for dheera_ai_params
      const guardrailData: {
        guardrail_id: string;
        guardrail: {
          guardrail_name: string;
      ...
      if (values.provider === "PresidioPII" && selectedEntities.length > 0) {
        ...
        guardrailData.guardrail.dheera_ai_params.pii_entities_config = piiEntitiesConfig;
      }
        ...
        try {
          ...
          if (values.provider === "Bedrock" && configObj) {
            if (configObj.guardrail_id) {
              guardrailData.guardrail.dheera_ai_params.guardrailIdentifier = configObj.guardrail_id;
            }
            if (configObj.guardrail_version) {
              guardrailData.guardrail.dheera_ai_params.guardrailVersion = configObj.guardrail_version;
            }
          } else {
