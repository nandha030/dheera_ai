...
const GuardrailInfoView: React.FC<GuardrailInfoProps> = ({ guardrailId, onClose, accessToken, isAdmin }) => {
  ...
  const fetchGuardrailInfo = async () => {
    try {
      ...
      if (response.dheera_ai_params?.pii_entities_config) {
        const piiConfig = response.dheera_ai_params.pii_entities_config;
  ...
  useEffect(() => {
    if (guardrailData && form) {
      form.setFieldsValue({
        ...
        ...guardrailData.dheera_ai_params,
        ...
        ...(guardrailData.dheera_ai_params?.optional_params && {
          optional_params: guardrailData.dheera_ai_params.optional_params,
        }),
      });
    }
  ...
  const resetToolPermissionEditor = useCallback(() => {
    if (guardrailData?.dheera_ai_params?.guardrail === "tool_permission") {
      setToolPermissionConfig({
        rules: (guardrailData.dheera_ai_params?.rules as ToolPermissionConfig["rules"]) || [],
        default_action: ((guardrailData.dheera_ai_params?.default_action || "deny") as ToolPermissionConfig["default_action"]).toLowerCase() as ToolPermissionConfig["default_action"],
        on_disallowed_action: ((guardrailData.dheera_ai_params?.on_disallowed_action || "block") as ToolPermissionConfig["on_disallowed_action"]).toLowerCase() as ToolPermissionConfig["on_disallowed_action"],
        violation_message_template: guardrailData.dheera_ai_params?.violation_message_template || "",
      });
    } else {
  ...
  const handleGuardrailUpdate = async (values: any) => {
    try {
      ...
      if (values.default_on !== guardrailData.dheera_ai_params?.default_on) {
        updateData.dheera_ai_params.default_on = values.default_on;
      }
      ...
      const originalPiiConfig = guardrailData.dheera_ai_params?.pii_entities_config || {};
      ...
      if (JSON.stringify(originalPiiConfig) !== JSON.stringify(newPiiEntitiesConfig)) {
        updateData.dheera_ai_params.pii_entities_config = newPiiEntitiesConfig;
      }

      // Only add Content Filter patterns if there are changes
      if (guardrailData.dheera_ai_params?.guardrail === "dheera_ai_content_filter") {
        const originalPatterns = guardrailData.dheera_ai_params?.patterns || [];
        const originalBlockedWords = guardrailData.dheera_ai_params?.blocked_words || [];
        ...
        if (JSON.stringify(originalPatterns) !== JSON.stringify(formattedData.patterns)) {
          updateData.dheera_ai_params.patterns = formattedData.patterns;
        }

        if (JSON.stringify(originalBlockedWords) !== JSON.stringify(formattedData.blocked_words)) {
          updateData.dheera_ai_params.blocked_words = formattedData.blocked_words;
        }
      }

      if (guardrailData.dheera_ai_params?.guardrail === "tool_permission") {
        const originalRules = guardrailData.dheera_ai_params?.rules || [];
        const currentRules = toolPermissionConfig.rules || [];
        const rulesChanged = JSON.stringify(originalRules) !== JSON.stringify(currentRules);

        const originalDefault = (guardrailData.dheera_ai_params?.default_action || "deny").toLowerCase();
        ...
        const originalOnDisallowed = (guardrailData.dheera_ai_params?.on_disallowed_action || "block").toLowerCase();
        ...
        const originalMessage = guardrailData.dheera_ai_params?.violation_message_template || "";
        ...
        if (toolPermissionDirty || rulesChanged || defaultChanged || onDisallowedChanged || messageChanged) {
          updateData.dheera_ai_params.rules = currentRules;
          updateData.dheera_ai_params.default_action = currentDefault;
          updateData.dheera_ai_params.on_disallowed_action = currentOnDisallowed;
          updateData.dheera_ai_params.violation_message_template = currentMessage || null;
        }
      }
      ...
      const currentProvider = Object.keys(guardrail_provider_map).find(
        (key) => guardrail_provider_map[key] === guardrailData.dheera_ai_params?.guardrail,
      );
      ...
      const isToolPermissionGuardrail = guardrailData.dheera_ai_params?.guardrail === "tool_permission";
      if (guardrailProviderSpecificParams && currentProvider && !isToolPermissionGuardrail) {
        ...
        allowedParams.forEach((paramName) => {
          ...
          const originalValue = guardrailData.dheera_ai_params?.[paramName];
          ...
          if (hasChanged) {
            if (paramValue !== undefined && paramValue !== null && paramValue !== "") {
              ...
              updateData.dheera_ai_params[paramName] = paramValue;
              ...
              updateData.dheera_ai_params[paramName] = null;
            }
          }
        });
      }

      // Remove empty dheera_ai_params object if no parameters were changed
      if (Object.keys(updateData.dheera_ai_params).length === 0) {
        delete updateData.dheera_ai_params;
      }
  ...
  const { logo, displayName } = getGuardrailLogoAndName(guardrailData.dheera_ai_params?.guardrail || "");
          ...
          <TabPanel>
            <Grid numItems={1} numItemsSm={2} numItemsLg={3} className="gap-6">
              ...
              <Card>
                <Text>Mode</Text>
                <div className="mt-2">
                  <Title>{guardrailData.dheera_ai_params?.mode || "-"}</Title>
                  <Badge color={guardrailData.dheera_ai_params?.default_on ? "green" : "gray"}>
                    {guardrailData.dheera_ai_params?.default_on ? "Default On" : "Default Off"}
                  </Badge>
                </div>
              </Card>
            ...
            {guardrailData.dheera_ai_params?.pii_entities_config &&
              Object.keys(guardrailData.dheera_ai_params.pii_entities_config).length > 0 && (
                <Card className="mt-6">
                  <div className="flex justify-between items-center">
                    <Text className="font-medium">PII Protection</Text>
                    <Badge color="blue">
                      {Object.keys(guardrailData.dheera_ai_params.pii_entities_config).length} PII entities configured
                    </Badge>
                  </div>
                </Card>
              )}
                ...
                <Card className="mt-6">
                  ...
                  <div className="border rounded-lg overflow-hidden shadow-sm">
                    ...
                    <div className="max-h-[400px] overflow-y-auto">
                      {Object.entries(guardrailData.dheera_ai_params?.pii_entities_config).map(([key, value]) => (
            ...
            {guardrailData.dheera_ai_params?.guardrail === "tool_permission" && (
              <Card className="mt-6">
              ...
              <Card>
                ...
                {isEditing ? (
                  ...
                  >
                    ...
                    {guardrailData.dheera_ai_params?.guardrail === "presidio" && (
                      <>
                    ...
                    {guardrailData.dheera_ai_params?.guardrail === "tool_permission" ? (
                      ...
                      <>
                        ...
                        <GuardrailProviderFields
                          ...
                          value={guardrailData.dheera_ai_params}
                        />
                          ...
                          (() => {
                            ...
                            return (
                              <GuardrailOptionalParams
                                ...
                                values={guardrailData.dheera_ai_params}
                              />
                            );
                          })()}
                      </>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div>{guardrailData.dheera_ai_params?.mode || "-"}</div>
                    </div>
                    <div>
                      <Text className="font-medium">Default On</Text>
                      <Badge color={guardrailData.dheera_ai_params?.default_on ? "green" : "gray"}>
                        {guardrailData.dheera_ai_params?.default_on ? "Yes" : "No"}
                      </Badge>
                    </div>
                    ...
                    {guardrailData.dheera_ai_params?.pii_entities_config &&
                      Object.keys(guardrailData.dheera_ai_params.pii_entities_config).length > 0 && (
                        <div>
                          ...
                          <div className="mt-2">
                            <Badge color="blue">
                              {Object.keys(guardrailData.dheera_ai_params.pii_entities_config).length} PII entities
                              configured
                            </Badge>
                          </div>
                        </div>
