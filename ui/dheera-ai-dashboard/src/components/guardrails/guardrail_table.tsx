...
const GuardrailTable: React.FC<GuardrailTableProps> = ({
  ...
  const columns: ColumnDef<Guardrail>[] = [
    ...
    {
      header: "Provider",
      accessorKey: "dheera_ai_params.guardrail",
      cell: ({ row }) => {
        const guardrail = row.original;
        const { logo, displayName } = getGuardrailLogoAndName(guardrail.dheera_ai_params.guardrail);
        return (
    ...
    {
      header: "Mode",
      accessorKey: "dheera_ai_params.mode",
      cell: ({ row }) => {
        const guardrail = row.original;
        return <span className="text-xs">{guardrail.dheera_ai_params.mode}</span>;
      },
    },
    {
      header: "Default On",
      accessorKey: "dheera_ai_params.default_on",
      cell: ({ row }) => {
        const guardrail = row.original;
        return (
          <Badge
            color={guardrail.dheera_ai_params?.default_on ? "green" : "gray"}
            className="text-xs font-normal"
            size="xs"
          >
            {guardrail.dheera_ai_params?.default_on ? "Default On" : "Default Off"}
          </Badge>
        );
      },
    },
          ...
          initialValues={{
            ...
            provider:
              Object.keys(guardrail_provider_map).find(
                (key) => guardrail_provider_map[key] === selectedGuardrail?.dheera_ai_params.guardrail,
              ) || "",
            mode: selectedGuardrail.dheera_ai_params.mode,
            default_on: selectedGuardrail.dheera_ai_params.default_on,
            pii_entities_config: selectedGuardrail.dheera_ai_params.pii_entities_config,
