...
export default function ModelInfoView({
  ...
  const isAutoRouter = modelData?.dheera_ai_params?.auto_router_config != null;

  const usingExistingCredential =
    modelData?.dheera_ai_params?.dheera_ai_credential_name != null &&
    modelData?.dheera_ai_params?.dheera_ai_credential_name != undefined;
  console.log("usingExistingCredential, ", usingExistingCredential);
  console.log("modelData.dheera_ai_params.dheera_ai_credential_name, ", modelData?.dheera_ai_params?.dheera_ai_credential_name);

  console.log("tagsList, ", modelData.dheera_ai_params?.tags);
  ...
  useEffect(() => {
    ...
    const getModelInfo = async () => {
      ...
      if (specificModelData && !specificModelData.dheera_ai_model_name) {
        specificModelData = {
          ...specificModelData,
          dheera_ai_model_name:
            specificModelData?.dheera_ai_params?.dheera_ai_model_name ??
            specificModelData?.dheera_ai_params?.model ??
      ...
      if (specificModelData?.dheera_ai_params?.cache_control_injection_points) {
  ...
  const handleReuseCredential = async (values: any) => {
    ...
    let credentialItem = {
      ...
      model_id: modelId,
      credential_info: {
        custom_llm_provider: localModelData.dheera_ai_params?.custom_llm_provider,
      },
    };
  ...
  const handleModelUpdate = async (values: any) => {
    try {
      ...
      // Parse DheeraAI extra params from JSON text area
      let parsedExtraParams: Record<string, any> = {};
      try {
        parsedExtraParams = values.dheera_ai_extra_params ? JSON.parse(values.dheera_ai_extra_params) : {};
      } catch (e) {
        NotificationsManager.fromBackend("Invalid JSON in DheeraAI Params");
        setIsSaving(false);
        return;
      }

      let updatedLitellmParams = {
        ...values.dheera_ai_params,
        ...parsedExtraParams,
        model: values.dheera_ai_model_name,
      ...
      const updateData = {
        ...
        dheera_ai_params: updatedLitellmParams,
      ...
      const updatedModelData = {
        ...localModelData,
        model_name: values.model_name,
        dheera_ai_model_name: values.dheera_ai_model_name,
        dheera_ai_params: updatedLitellmParams,
        model_info: updatedModelInfo,
      };
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      const response = await testConnectionRequest(
        ...
        {
          custom_llm_provider: localModelData.dheera_ai_params.custom_llm_provider,
          dheera_ai_credential_name: localModelData.dheera_ai_params.dheera_ai_credential_name,
          model: localModelData.dheera_ai_model_name,
        },
        {
  ...
  return (
    <div className="p-4">
        ...
        <TabPanels>
          <TabPanel>
            ...
            <Grid numItems={1} numItemsSm={2} numItemsLg={3} className="gap-6 mb-6">
              ...
              <Card>
                <Text>DheeraAI Model</Text>
                <div className="mt-2 overflow-hidden">
                  <Tooltip title={modelData.dheera_ai_model_name || "Not Set"}>
                    <div className="break-all text-sm font-medium leading-relaxed cursor-pointer">
                      {modelData.dheera_ai_model_name || "Not Set"}
                    </div>
                  </Tooltip>
                </div>
              </Card>
              ...
              {localModelData ? (
                <Form
                  ...
                  initialValues={{
                    ...
                    dheera_ai_model_name: localModelData.dheera_ai_model_name,
                    api_base: localModelData.dheera_ai_params.api_base,
                    custom_llm_provider: localModelData.dheera_ai_params.custom_llm_provider,
                    organization: localModelData.dheera_ai_params.organization,
                    tpm: localModelData.dheera_ai_params.tpm,
                    rpm: localModelData.dheera_ai_params.rpm,
                    max_retries: localModelData.dheera_ai_params.max_retries,
                    timeout: localModelData.dheera_ai_params.timeout,
                    stream_timeout: localModelData.dheera_ai_params.stream_timeout,
                    input_cost: localModelData.dheera_ai_params.input_cost_per_token
                      ? localModelData.dheera_ai_params.input_cost_per_token * 1_000_000
                      : localModelData.model_info?.input_cost_per_token * 1_000_000 || null,
                    output_cost: localModelData.dheera_ai_params?.output_cost_per_token
                      ? localModelData.dheera_ai_params.output_cost_per_token * 1_000_000
                      : localModelData.model_info?.output_cost_per_token * 1_000_000 || null,
                    cache_control: localModelData.dheera_ai_params?.cache_control_injection_points ? true : false,
                    cache_control_injection_points: localModelData.dheera_ai_params?.cache_control_injection_points || [],
                    model_access_group: Array.isArray(localModelData.model_info?.access_groups)
                      ? localModelData.model_info.access_groups
                      : [],
                    guardrails: Array.isArray(localModelData.dheera_ai_params?.guardrails)
                      ? localModelData.dheera_ai_params.guardrails
                      : [],
                    tags: Array.isArray(localModelData.dheera_ai_params?.tags) ? localModelData.dheera_ai_params.tags : [],
                    dheera_ai_extra_params: JSON.stringify(localModelData.dheera_ai_params || {}, null, 2),
                  }}
                  layout="vertical"
                ...
                >
                  <div className="space-y-4">
                    <div className="space-y-4">
                      ...
                      <div>
                        <Text className="font-medium">DheeraAI Model Name</Text>
                        {isEditing ? (
                          <Form.Item name="dheera_ai_model_name" className="mb-0">
                            <TextInput placeholder="Enter DheeraAI model name" />
                          </Form.Item>
                        ) : (
                          <div className="mt-1 p-2 bg-gray-50 rounded">{localModelData.dheera_ai_model_name}</div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData?.dheera_ai_params?.input_cost_per_token
                              ? (localModelData.dheera_ai_params?.input_cost_per_token * 1_000_000).toFixed(4)
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData?.dheera_ai_params?.output_cost_per_token
                              ? (localModelData.dheera_ai_params.output_cost_per_token * 1_000_000).toFixed(4)
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.api_base || "Not Set"}
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.custom_llm_provider || "Not Set"}
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.organization || "Not Set"}
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.tpm || "Not Set"}
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.rpm || "Not Set"}
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.max_retries || "Not Set"}
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.timeout || "Not Set"}
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.stream_timeout || "Not Set"}
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        <Text className="font-medium">
                          ...
                          <Tooltip title="Apply safety guardrails to this model to filter content or enforce policies">
                            <a
                              href="https://docs.dheera_ai.ai/docs/proxy/guardrails/quick_start"
                              target="_blank"
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.guardrails ? (
                              Array.isArray(localModelData.dheera_ai_params.guardrails) ? (
                                localModelData.dheera_ai_params.guardrails.length > 0 ? (
                                  <div className="flex flex-wrap gap-1">
                                    {localModelData.dheera_ai_params.guardrails.map(
                      ...
                      <div>
                        ...
                        {isEditing ? (
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.tags ? (
                              Array.isArray(localModelData.dheera_ai_params.tags) ? (
                                localModelData.dheera_ai_params.tags.length > 0 ? (
                                  <div className="flex flex-wrap gap-1">
                                    {localModelData.dheera_ai_params.tags.map((tag: string, index: number) => (
                                      <span
                                        key={index}
                      ...
                      {isEditing ? (
                        ...
                        <div>
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            {localModelData.dheera_ai_params?.cache_control_injection_points ? (
                              <div>
                                <p>Enabled</p>
                                <div className="mt-2">
                                  {localModelData.dheera_ai_params.cache_control_injection_points.map(
                                    (point: any, i: number) => (
                      ...
                      <div>
                        <Text className="font-medium">
                          DheeraAI Params
                          <Tooltip title="Optional dheera_ai params used for making a dheera_ai.completion() call. Some params are automatically added by DheeraAI.">
                            <a
                              href="https://docs.dheera_ai.ai/docs/completion/input"
                              target="_blank"
                              rel="noopener noreferrer"
                              onClick={(e) => e.stopPropagation()}
                            >
                        ...
                        {isEditing ? (
                          <Form.Item name="dheera_ai_extra_params" rules={[{ validator: formItemValidateJSON }]}>
                            <Input.TextArea
                              rows={4}
                              placeholder='{
                  "rpm": 100,
                  ...
                  "stream_timeout": 0
                          ...
                          <div className="mt-1 p-2 bg-gray-50 rounded">
                            <pre className="bg-gray-100 p-2 rounded text-xs overflow-auto mt-1">
                              {JSON.stringify(localModelData.dheera_ai_params, null, 2)}
                            </pre>
                          </div>
                        )}
                      </div>
      ...
      {isCredentialModalOpen && !usingExistingCredential ? (
        ...
        >
          <Text>{modelData.dheera_ai_params.dheera_ai_credential_name}</Text>
        </Modal>
      )}
