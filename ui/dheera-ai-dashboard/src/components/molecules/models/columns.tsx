...
export const columns = (
    ...
    cell: ({ row }) => {
      ...
      const tooltipContent = (
        <div>
          ...
          <div>
            <strong>DheeraAI Model Name:</strong> {model.dheera_ai_model_name || "-"}
          </div>
        </div>
      );
  ...
  },
  {
    ...
    accessorKey: "dheera_ai_credential_name",
    size: 180, // Fixed column width
    cell: ({ row }) => {
      const model = row.original;
      const credentialName = model.dheera_ai_params?.dheera_ai_credential_name;
