...
const VersionHistorySidePanel: React.FC<VersionHistorySidePanelProps> = ({
  ...
  const getVersionNumber = (prompt: PromptSpec) => {
    // Use explicit version field if available, otherwise try to extract from dheera_ai_params.prompt_id
    if (prompt.version) {
      return `v${prompt.version}`;
    }
    
    // Fallback: try to extract from dheera_ai_params.prompt_id
    const versionedId = (prompt.dheera_ai_params as any)?.prompt_id || prompt.prompt_id;
