...
/**
 * Get versioned prompt ID from dheera_ai_params (preserves version)
 */
export const getVersionedPromptId = (promptData?: PromptSpec): string => {
  const baseId = getBasePromptId(promptData);
  const versionedId = (promptData?.dheera_ai_params as any)?.prompt_id || baseId;
  return versionedId;
};
...
export const getCurrentVersion = (promptData?: PromptSpec): string => {
  ...
  // Fallback: extract from versioned ID in dheera_ai_params
  const versionedId = getVersionedPromptId(promptData);
  return getVersionNumber(versionedId);
};

/**
 * Extract model from prompt dheera_ai_params
 */
export const extractModel = (prompt: PromptSpec): string | null => {
  try {
    const params = prompt.dheera_ai_params as any;
    ...
    // Try to extract model from dheera_ai_params directly
    if (params?.model) {
      return params.model;
    }
