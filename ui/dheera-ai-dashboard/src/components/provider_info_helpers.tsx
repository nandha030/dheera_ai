  ...
  if (providerKey && typeof modelMap === "object") {
    Object.entries(modelMap).forEach(([key, value]) => {
      if (
        value !== null &&
        typeof value === "object" &&
        "dheera_ai_provider" in (value as object) &&
        ((value as any)["dheera_ai_provider"] === custom_llm_provider ||
          (value as any)["dheera_ai_provider"].includes(custom_llm_provider))
      ) {
    ...
    if (providerKey == Providers.Cohere) {
      ...
      Object.entries(modelMap).forEach(([key, value]) => {
        if (
          ...
          typeof value === "object" &&
          "dheera_ai_provider" in (value as object) &&
          (value as any)["dheera_ai_provider"] === "cohere_chat"
        ) {
          providerModels.push(key);
        }
      });
    ...
    if (providerKey == Providers.SageMaker) {
      ...
      Object.entries(modelMap).forEach(([key, value]) => {
        if (
          ...
          typeof value === "object" &&
          "dheera_ai_provider" in (value as object) &&
          (value as any)["dheera_ai_provider"] === "sagemaker_chat"
        ) {
          providerModels.push(key);
        }
      });
