...
interface SearchConnectionTestProps {
  dheera_aiParams: Record<string, any>;
  accessToken: string;
  onTestComplete?: () => void;
}
...
const SearchConnectionTest: React.FC<SearchConnectionTestProps> = ({
  ...
  useEffect(() => {
    const runTest = async () => {
      setIsLoading(true);
      try {
        const result = await testSearchToolConnection(accessToken, dheera_aiParams);
    ...
    runTest();
  }, [accessToken, dheera_aiParams, onTestComplete]);
  ...
  const getCleanErrorMessage = (errorMsg: string) => {
    ...
    // Remove dheera_ai error prefixes
    const cleanedError = mainError.replace(/^dheera_ai\.(.*?)Error:\s*/, "");
      ...
      <div style={{ padding: "24px", borderRadius: "8px", backgroundColor: "#fff" }}>
        <div style={{ textAlign: "center", padding: "32px 20px" }}>
          ...
          <Text style={{ fontSize: "16px" }}>
            Testing connection to {dheera_aiParams.search_provider || "search provider"}...
          </Text>
          <style jsx>{`
            @keyframes spin {
              0% {
  ...
  return (
    <div style={{ padding: "24px", borderRadius: "8px", backgroundColor: "#fff" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "32px 20px" }}>
          ...
          <div style={{ marginLeft: "12px" }}>
            <Text type="success" style={{ fontSize: "18px", fontWeight: 500, display: "block" }}>
              Connection to {dheera_aiParams.search_provider} successful!
            </Text>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", marginBottom: "20px" }}>
              ...
              <Text type="danger" style={{ fontSize: "18px", fontWeight: 500 }}>
                Connection to {dheera_aiParams.search_provider || "search provider"} failed
              </Text>
            </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Button type="link" href="https://docs.dheera_ai.ai/docs/search" target="_blank" icon={<InfoCircleOutlined />}>
