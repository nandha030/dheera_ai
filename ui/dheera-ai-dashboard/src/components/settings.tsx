...
interface genericCallbackParams {
  dheera_ai_callback_name: string; // what to send in request
  ui_callback_name: string; // what to show on UI
  dheera_ai_callback_params: string[] | null; // known required params for this callback
}
...
const Settings: React.FC<SettingsPageProps> = ({ accessToken, userRole, userID, premiumUser }) => {
  ...
  const [allCallbacks, setAllCallbacks] = useState<
    Record<
      string,
      {
        dheera_ai_callback_name: string;
        dheera_ai_callback_params: string[];
  ...
  return (
    <div className="w-full mx-4">
      ...
      >
        <a
          href="https://docs.dheera_ai.ai/docs/proxy/logging"
          className="mb-8 mt-4"
          target="_blank"
          style={{ color: "blue" }}
        >
          {" "}
          DheeraAI Docs: Logging
        </a>
