      ...
      if (tagData) {
        ...
        if (editTag) {
          form.setFieldsValue({
            ...
            max_budget: tagData.dheera_ai_budget_table?.max_budget,
            budget_duration: tagData.dheera_ai_budget_table?.budget_duration,
          });
        }
      }
    } catch (error) {
    ...
    <div className="p-4">
      ...
      {isEditing ? (
              ...
              <AccordionBody>
                ...
                <div className="mt-4 p-3 bg-gray-50 rounded-md border border-gray-200">
                  <p className="text-sm text-gray-600">
                    ...
                    <a
                      href="https://github.com/BerriAI/dheera_ai/issues/new"
                      target="_blank"
        ...
        <div className="space-y-6">
          ...
          {tagDetails.dheera_ai_budget_table && (
            <Card>
              <Title>Budget & Rate Limits</Title>
              <div className="space-y-4 mt-4">
                {tagDetails.dheera_ai_budget_table.max_budget !== undefined &&
                  tagDetails.dheera_ai_budget_table.max_budget !== null && (
                    <div>
                      <Text className="font-medium">Max Budget</Text>
                      <Text>${tagDetails.dheera_ai_budget_table.max_budget}</Text>
                    </div>
                  )}
                {tagDetails.dheera_ai_budget_table.budget_duration && (
                  <div>
                    <Text className="font-medium">Budget Duration</Text>
                    <Text>{tagDetails.dheera_ai_budget_table.budget_duration}</Text>
                  </div>
                )}
                {tagDetails.dheera_ai_budget_table.tpm_limit !== undefined &&
                  tagDetails.dheera_ai_budget_table.tpm_limit !== null && (
                    <div>
                      <Text className="font-medium">TPM Limit</Text>
                      <Text>{tagDetails.dheera_ai_budget_table.tpm_limit.toLocaleString()}</Text>
                    </div>
                  )}
                {tagDetails.dheera_ai_budget_table.rpm_limit !== undefined &&
                  tagDetails.dheera_ai_budget_table.rpm_limit !== null && (
                    <div>
                      <Text className="font-medium">RPM Limit</Text>
                      <Text>{tagDetails.dheera_ai_budget_table.rpm_limit.toLocaleString()}</Text>
                    </div>
                  )}
              </div>
            </Card>
