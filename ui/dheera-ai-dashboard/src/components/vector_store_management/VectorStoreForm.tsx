  ...
  const handleCreate = async (formValues: any) => {
    ...
    try {
      ...
      const payload: any = {
        ...
        dheera_ai_credential_name: formValues.dheera_ai_credential_name,
      };

      // pass all provider fields as dheera_ai params dict
      const providerFields = getProviderSpecificFields(formValues.custom_llm_provider);
      const dheera_aiParams = providerFields.reduce(
        (acc, field) => {
      ...
      payload["dheera_ai_params"] = dheera_aiParams;
      ...
      <Form form={form} onFinish={handleCreate} labelCol={{ span: 8 }} wrapperCol={{ span: 16 }} labelAlign="left">
        ...
        {selectedProvider === "pg_vector" && (
          <Alert
            ...
            description={
              <div>
                <p>DheeraAI provides a server to connect to PG Vector. To use this provider:</p>
                <ol style={{ marginLeft: "16px", marginTop: "8px" }}>
                  <li>
                    Deploy the dheera_ai-pgvector server from:{" "}
                    <a href="https://github.com/BerriAI/dheera_ai-pgvector" target="_blank" rel="noopener noreferrer">
        ...
        <Form.Item
          label={
            <span>
              Vector Store Name{" "}
              <Tooltip title="Custom name you want to give to the vector store, this name will be rendered on the DheeraAI UI">
                <InfoCircleOutlined style={{ marginLeft: "4px" }} />
              </Tooltip>
            </span>
          }
        ...
        <Form.Item
          ...
          name="dheera_ai_credential_name"
        >
          <Select
            showSearch
