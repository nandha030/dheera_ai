    ...
    return allLogsQuery.data.filter((log) => {
      ...
      if (selectedTableFilter !== "all") {
        ...
        switch (selectedTableFilter) {
          case "keys":
            tableMatchName = "dheera_ai_verificationtoken";
            break;
          case "teams":
            tableMatchName = "dheera_ai_teamtable";
            break;
          case "users":
            tableMatchName = "dheera_ai_usertable";
            break;
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-blue-700 mt-1">
            To enable audit logging, add the following configuration to your DheeraAI proxy configuration file:
          </p>
  ...
  const renderSubComponent = useCallback(({ row }: { row: any }) => {
    const AuditLogRowExpansionPanel = ({ rowData }: { rowData: AuditLogEntry }) => {
      ...
      if ((action === "updated" || action === "rotated") && before_value && updated_values) {
        if (
          table_name === "DheeraAI_TeamTable" ||
          table_name === "DheeraAI_UserTable" ||
          table_name === "DheeraAI_VerificationToken"
        ) {
      ...
      return (
        <div className="-mx-4 p-4 bg-slate-100 border-y border-slate-300 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            {renderValue(displayBeforeValue, table_name === "DheeraAI_VerificationToken")}
          </div>
          <div>
            <h4 className="font-semibold mb-2 text-sm text-slate-700">Updated Value:</h4>
            {renderValue(displayUpdatedValue, table_name === "DheeraAI_VerificationToken")}
          </div>
        </div>
      );
    };
    ...
    return (
      <div style={{ textAlign: "center", marginTop: "20px" }}>
        ...
        <Text style={{ display: "block", marginBottom: "10px" }}>
          This is a DheeraAI Enterprise feature, and requires a valid key to use.
        </Text>
