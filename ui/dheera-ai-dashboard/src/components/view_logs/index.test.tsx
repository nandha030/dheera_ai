...
describe("Request Viewer", () => {
  ...
  it("should display DheeraAI Overhead when dheera_ai_overhead_time_ms is present in metadata", () => {
    render(
      <RequestViewer
        row={createRow({
          metadata: {
            status: "success",
            dheera_ai_overhead_time_ms: 150,
            additional_usage_values: {
              cache_read_input_tokens: 0,
              cache_creation_input_tokens: 0,
            },
    ...
    expect(screen.getByText("DheeraAI Overhead:")).toBeInTheDocument();
    expect(screen.getByText("150 ms")).toBeInTheDocument();
  });

  it("should not display DheeraAI Overhead when dheera_ai_overhead_time_ms is not present in metadata", () => {
    render(<RequestViewer row={createRow()} />);

    expect(screen.queryByText("DheeraAI Overhead:")).not.toBeInTheDocument();
  });
});
